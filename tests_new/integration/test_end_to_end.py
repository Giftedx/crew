            ...
            with patch("ultimate_discord_intelligence_bot.services.memory_service.MemoryService") as mock_memory:
                with patch("ultimate_discord_intelligence_bot.services.prompt_engine.PromptEngine") as mock_prompt:
                    with patch(
                        ...
                        # Configure mocks
                        mock_memory.return_value.store_content.return_value = MagicMock(
                            success=True, data={"id": "test_id"}
                        )
            ...
            with patch("ultimate_discord_intelligence_bot.services.memory_service.MemoryService") as mock_memory:
                with patch("ultimate_discord_intelligence_bot.services.prompt_engine.PromptEngine") as mock_prompt:
                    with patch(
                        ...
                        mock_memory.return_value.store_content.return_value = MagicMock(
                            success=False, error="Memory service failed"
                        )
                        mock_prompt.return_value.generate_prompt.return_value = MagicMock(
                            success=False, error="Prompt engine failed"
                        )
                        mock_openrouter.return_value.generate_response.return_value = MagicMock(
                            success=False, error="OpenRouter service failed"
                        )
            ...
            with patch("ultimate_discord_intelligence_bot.services.memory_service.MemoryService") as mock_memory:
                with patch("ultimate_discord_intelligence_bot.services.prompt_engine.PromptEngine") as mock_prompt:
                    with patch(
                        ...
                        assert result is not None
                        # Verify OpenRouter service was called
