...
def live_client(monkeypatch):
    ...
    # Bridge core.http_utils to in-process TestClient
    def fake_resilient_post(url, **kwargs):
        body = kwargs.get("json_payload")
        headers = kwargs.get("headers") or {}
        # Map absolute URL to path
        path = url.split("//", 1)[-1]
...
def test_e2e_agent_card_and_call(live_client):
    ...
    # Run a simple tool call; tolerate either success or error depending on optional tool availability
    result = live_client.call(
        "agent.execute",
        {"skill": "tools.text_analyze", "args": {"text": "hello"}},
        id_value=1,
    )
...
def test_e2e_batch_with_api_key(monkeypatch):
    # Enable API and API key protection
    monkeypatch.setenv("ENABLE_A2A_API", "1")
    ...
    # Unauthorized client (no API key)
    c_bad = A2AClient(A2AClientConfig(base_url="http://local"))
    # jsonrpc should be unauthorized when API key is required
    ...
    # Batch call two methods
    out = c.call_batch(
        [
            ("tools.text_analyze", {"text": "x"}, 1),
            (
                "agent.execute",
                {"skill": "tools.text_analyze", "args": {"text": "y"}},
                2,
            ),
        ]
    )
