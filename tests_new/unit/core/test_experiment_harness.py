...
from ultimate_discord_intelligence_bot.core.rl.experiment import (
    Experiment,
    ExperimentManager,
)
...
def test_shadow_mode_returns_control():
    ...
    exp = Experiment(
        experiment_id="exp:model_select",
        control="baseline",
        variants={"alt": 0.5},
        phase="shadow",
    ...
    mgr.register(exp)
    # Even after multiple recommends, control should be returned in shadow phase
    for _ in range(10):
...
def test_auto_activation_flips_phase():
    ...
    mgr = ExperimentManager()
    exp = Experiment(
        experiment_id="exp:auto_act",
        control="c",
        variants={"v1": 0.7},
        phase="shadow",
        auto_activate_after=5,
    )
    mgr.register(exp)
    for _i in range(5):
        chosen = mgr.recommend("exp:auto_act", {}, ["c", "v1"])
        assert chosen == "c"  # still control
        mgr.record("exp:auto_act", "c", reward=1.0)
    # After threshold, phase should flip automatically
