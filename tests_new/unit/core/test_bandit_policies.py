...
class TestBanditPolicies(unittest.TestCase):
    ...
    def test_ucb1_explore_each_once(self) -> None:
        bandit = UCB1Bandit()
        # First call should pick unseen arm
        first = bandit.recommend({}, ["x", "y"])
        self.assertIn(first, ("x", "y"))
        # After updating the chosen arm, next recommend should pick the other unseen
    ...
    def test_thompson_sampling_deterministic_rng(self) -> None:
        ...
        # With identical priors, selection depends on RNG, ensure deterministic
