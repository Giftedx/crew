...
def sample_vectors():
    ...
    return [
        VectorRecord(
            vector=[0.1, 0.2, 0.3],
            payload={"text": "test content 1", "id": 1},
        ),
        VectorRecord(
            vector=[0.2, 0.3, 0.4],
            payload={"text": "test content 2", "id": 2},
        ),
    ]
...
class TestMemoryQuery:
    ...
    async def test_query_isolation(self, memory_service, sample_vectors):
        ...
        # Query from tenant2 should not see tenant1 data
        query_vector = [0.15, 0.25, 0.35]
        result = await memory_service.query(
            tenant="tenant2",
        ...
        # Should succeed but return no results (different namespace)
        assert result.success
