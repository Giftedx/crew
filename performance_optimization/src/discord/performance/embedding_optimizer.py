...
class EmbeddingOptimizer:
    ...
    def __init__(
        self, config: EmbeddingConfig, embedding_function: Callable[[str | list[str]], np.ndarray | list[np.ndarray]]
    ):
        self.config = config
    ...
    async def _get_embeddings_batch(self, contents: list[str]) -> StepResult:
        ...
        if uncached_contents:
            ...
            for i, (content, embedding) in enumerate(zip(uncached_contents, new_embeddings, strict=False)):
                content_hash = self._hash_content(content)
                await self._cache_embedding(content_hash, content, embedding)
...
def create_embedding_optimizer(
    config: EmbeddingConfig, embedding_function: Callable[[str | list[str]], np.ndarray | list[np.ndarray]]
) -> EmbeddingOptimizer:
