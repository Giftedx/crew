...
def compare_structures(actual: Dict[Any, Any], expected_keys: List[str], file_name: str) -> List[str]:
    ...
    actual_keys = set(str(k) for k in actual.keys())
...
def validate_policy_config():
    ...
    expected_keys = ["allowed_sources", "forbidden_types", "pii_types", "masks", "storage", "consent", "per_command"]
    ...
    if "pii_types" in actual:
        expected_pii_types = ["email", "phone", "ip", "credit_like", "gov_id_like", "address_like", "geo_exact"]
