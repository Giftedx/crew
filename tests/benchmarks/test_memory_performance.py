...
class TestMemoryPerformance:
    ...
    async def test_vector_store_retrieve_performance(self, mock_vector_store, sample_tenant_context):
        ...
        mock_vector_store.retrieve.return_value = StepResult.ok(data={'content': 'retrieved content', 'metadata': {'id': 'test_id'}})
        iterations = 100
        retrieve_times = []
        for i in range(iterations):
            content_id = f'test_content_{i}'
            start_time = time.time()
            result = await mock_vector_store.retrieve(content_id=content_id, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            end_time = time.time()
    ...
    async def test_vector_store_store_performance(self, mock_vector_store, sample_tenant_context):
        ...
        for i in range(iterations):
            ...
            start_time = time.time()
            result = await mock_vector_store.store(content=content, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            end_time = time.time()
    ...
    async def test_vector_store_search_performance(self, mock_vector_store, sample_tenant_context):
        ...
        mock_vector_store.search.return_value = StepResult.ok(data={'results': [{'id': f'result_{i}', 'score': 0.9 - i * 0.1} for i in range(10)]})
        iterations = 100
        search_times = []
        for _i in range(iterations):
            query_vector = [0.1] * 768
            start_time = time.time()
            result = await mock_vector_store.search(query_vector=query_vector, limit=10, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            end_time = time.time()
    ...
    async def test_batch_store_performance(self, mock_vector_store, sample_content_items, sample_tenant_context):
        ...
        mock_vector_store.batch_store.return_value = [StepResult.ok(data={'id': f'stored_{i}'}) for i in range(len(sample_content_items))]
        batch_sizes = [10, 25, 50, 100]
        batch_results = []
        for batch_size in batch_sizes:
            batch_content = sample_content_items[:batch_size]
            start_time = time.time()
            results = await mock_vector_store.batch_store(content_list=batch_content, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            end_time = time.time()
            batch_time = end_time - start_time
            batch_results.append({'batch_size': batch_size, 'time': batch_time, 'throughput': batch_size / batch_time})
            assert all((result.success for result in results))
        for result in batch_results:
            print(f'Batch size {result['batch_size']}: {result['time']:.3f} seconds, {result['throughput']:.2f} items/second')
    ...
    async def test_batch_embedding_performance(self, mock_embedding_service):
        ...
        for result in batch_results:
            print(f'Batch embedding size {result['batch_size']}: {result['time']:.3f} seconds, {result['throughput']:.2f} embeddings/second')
    ...
    async def test_search_query_latency_at_scale(self, mock_vector_store, sample_tenant_context):
        ...
        for scale in query_scales:
            query_vector = [0.1] * 768
            start_time = time.time()
            result = await mock_vector_store.search(query_vector=query_vector, limit=scale, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            end_time = time.time()
            search_time = end_time - start_time
            scale_results.append({'scale': scale, 'time': search_time, 'latency_per_result': search_time / scale})
            assert result.success
        for result in scale_results:
            print(f'Search scale {result['scale']}: {result['time']:.4f} seconds, {result['latency_per_result']:.6f} seconds/result')
    ...
    async def test_concurrent_memory_operations(self, mock_vector_store, sample_tenant_context):
        ...
        for i in range(concurrent_operations):
            ...
            if op_type == 'store':
                ...
                task = mock_vector_store.store(content=content, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            elif op_type == 'retrieve':
                task = mock_vector_store.retrieve(content_id=f'concurrent_{i}', tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            else:
                task = mock_vector_store.search(query_vector=[0.1] * 768, limit=10, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
            tasks.append(task)
    ...
    async def test_memory_compaction_performance(self, mock_vector_store, sample_tenant_context):
        ...
        mock_vector_store.compact.return_value = StepResult.ok(data={'compacted_items': 100, 'space_saved': '50MB', 'compaction_time': 2.5})
        start_time = time.time()
        result = await mock_vector_store.compact(tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
        end_time = time.time()
    ...
    async def test_memory_usage_under_load(self, mock_vector_store, sample_tenant_context):
        ...
        for scale in load_scales:
            ...
            memory_results.append({'scale': scale, 'time': operation_time, 'estimated_memory': estimated_memory, 'throughput': scale / operation_time})
            assert all((result.success for result in results))
        for result in memory_results:
            print(f'Load scale {result['scale']}: {result['time']:.3f} seconds, {result['estimated_memory']:.1f}MB, {result['throughput']:.2f} ops/second')
