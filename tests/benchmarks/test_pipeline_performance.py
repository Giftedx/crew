...
class TestPipelinePerformance:
    ...
    def sample_urls(self):
        ...
        return ['https://example.com/video1', 'https://example.com/video2', 'https://example.com/video3', 'https://example.com/video4', 'https://example.com/video5']

    @pytest.fixture
    def mock_config(self):
        """Mock configuration for performance testing."""
        return {'max_retries': 3, 'timeout': 30, 'enable_rollback': True, 'enable_monitoring': True, 'concurrency_limit': 10}
    ...
    async def test_single_pipeline_processing_time(self, pipeline, mock_config, sample_tenant_context):
        ...
        with patch.object(pipeline, '_download_phase', return_value=StepResult.ok(data={'content': 'test'})), patch.object(pipeline, '_transcription_phase', return_value=StepResult.ok(data={'transcript': 'test transcript'})), patch.object(pipeline, '_analysis_phase', return_value=StepResult.ok(data={'analysis': 'test analysis'})), patch.object(pipeline, '_memory_storage_phase', return_value=StepResult.ok()):
            start_time = time.time()
            result = await pipeline.process_content(url=sample_url, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'], config=mock_config)
            end_time = time.time()
    ...
    async def test_concurrent_pipeline_processing(self, pipeline, mock_config, sample_tenant_context, sample_urls):
        ...
        with patch.object(pipeline, '_download_phase', return_value=StepResult.ok(data={'content': 'test'})), patch.object(pipeline, '_transcription_phase', return_value=StepResult.ok(data={'transcript': 'test transcript'})), patch.object(pipeline, '_analysis_phase', return_value=StepResult.ok(data={'analysis': 'test analysis'})), patch.object(pipeline, '_memory_storage_phase', return_value=StepResult.ok()):
            start_time = time.time()
            tasks = [pipeline.process_content(url=url, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'], config=mock_config) for url in sample_urls]
    ...
    async def test_pipeline_error_recovery_performance(self, pipeline, mock_config, sample_tenant_context):
        ...
        with patch.object(pipeline, '_download_phase', side_effect=mock_failing_then_success), patch.object(pipeline, '_transcription_phase', return_value=StepResult.ok(data={'transcript': 'test transcript'})), patch.object(pipeline, '_analysis_phase', return_value=StepResult.ok(data={'analysis': 'test analysis'})), patch.object(pipeline, '_memory_storage_phase', return_value=StepResult.ok()):
            start_time = time.time()
            result = await pipeline.process_content(url=sample_url, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'], config=mock_config)
            end_time = time.time()
    ...
    async def test_pipeline_configuration_performance_impact(self, pipeline, sample_tenant_context):
        ...
        configs = [{'timeout': 10, 'max_retries': 1}, {'timeout': 30, 'max_retries': 3}, {'timeout': 60, 'max_retries': 5}]
        config_results = []
        with patch.object(pipeline, '_download_phase', return_value=StepResult.ok(data={'content': 'test'})), patch.object(pipeline, '_transcription_phase', return_value=StepResult.ok(data={'transcript': 'test transcript'})), patch.object(pipeline, '_analysis_phase', return_value=StepResult.ok(data={'analysis': 'test analysis'})), patch.object(pipeline, '_memory_storage_phase', return_value=StepResult.ok()):
            for config in configs:
                start_time = time.time()
                result = await pipeline.process_content(url=sample_url, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'], config=config)
                end_time = time.time()
                processing_time = end_time - start_time
