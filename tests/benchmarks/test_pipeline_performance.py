...
class TestPipelinePerformance:
    ...
    async def test_single_pipeline_processing_time(self, pipeline, mock_config, sample_tenant_context):
        ...
        with patch.object(pipeline, "_download_phase", return_value=StepResult.ok(data={"content": "test"})):
            with patch.object(
                pipeline, "_transcription_phase", return_value=StepResult.ok(data={"transcript": "test transcript"})
            ):
                with patch.object(
                    pipeline, "_analysis_phase", return_value=StepResult.ok(data={"analysis": "test analysis"})
                ):
                    with patch.object(pipeline, "_memory_storage_phase", return_value=StepResult.ok()):
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(pipeline, "_memory_storage_phase", return_value=StepResult.ok()):
                        ...
                        memory_trend = statistics.mean(memory_measurements[-10:]) - statistics.mean(
                            memory_measurements[:10]
                        )
    ...
    async def test_pipeline_error_recovery_performance(self, pipeline, mock_config, sample_tenant_context):
        ...
        with patch.object(pipeline, "_download_phase", side_effect=mock_failing_then_success):
            with patch.object(
                pipeline, "_transcription_phase", return_value=StepResult.ok(data={"transcript": "test transcript"})
            ):
                with patch.object(
                    ...
                    with patch.object(pipeline, "_memory_storage_phase", return_value=StepResult.ok()):
                        for config in configs:
                            ...
                            config_results.append(
                                {"config": config, "time": processing_time, "success": result.success}
                            )
