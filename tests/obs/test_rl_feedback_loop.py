...
def rl_metric_stubs(monkeypatch):
    ...
    return {'queue_depth': queue_depth, 'processed': processed, 'failed': failed, 'latency': latency, 'labels': base_labels}

class _FakeRouter:

    def __init__(self, initial_items: int=0, result_factory: Callable[[int, int, int], StepResult] | None=None, raise_exc: Exception | None=None):
