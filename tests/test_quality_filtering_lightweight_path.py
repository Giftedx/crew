...
class _DummyStepTool:
    ...
    async def run(self, *args, **kwargs):  # async stub for orchestrator compatibility
...
def enable_quality(monkeypatch):
    ...
    # Force high thresholds so our tiny transcript will bypass
...
async def test_lightweight_processing_bypass(monkeypatch):
    # Minimal transcript that will clearly fail thresholds
    transcript_text = "Uh okay yeah short fragment."  # very low quality
    ...
    # Lightweight path sets processing_type and includes bypass reason
    assert result["processing_type"] == "lightweight"
    assert "bypass_reason" in result and result["bypass_reason"]
    # Quality metrics should be present in the result for observability/analytics
    ...
    # Check for expected keys (subset, not all are always present)
    expected_keys = {
        "word_count",
        "sentence_count",
        "avg_sentence_length",
    ...
    # For lightweight path finalize payload includes summary & quality_score only; we check those
    assert "summary" in result
    assert "quality_score" in result
    # Ensure heavy analysis sections are absent (since we bypassed)
