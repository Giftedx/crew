...
class TestRateLimiting:
    ...
    def sample_tenants(self) -> dict[str, dict[str, str | int]]:
        ...
        return {'tenant_a': {'workspace': 'workspace_a', 'rate_limit': 100, 'burst_limit': 10}, 'tenant_b': {'workspace': 'workspace_b', 'rate_limit': 50, 'burst_limit': 5}, 'tenant_c': {'workspace': 'workspace_c', 'rate_limit': 200, 'burst_limit': 20}}
    ...
    def test_rate_limit_bypass_prevention(self, mock_rate_limiter: Mock) -> None:
        ...
        def mock_check_rate_limit(tenant, workspace, endpoint=None, client_ip=None):
            bypass_attempts.append({'tenant': tenant, 'workspace': workspace, 'endpoint': endpoint, 'client_ip': client_ip, 'timestamp': time.time()})
    ...
    async def test_concurrent_rate_limiting(self, mock_rate_limiter: Mock, sample_tenants: dict) -> None:
        ...
        tasks = [make_request(tenant_id, tenant_info) for tenant_id, tenant_info in sample_tenants.items() for _ in range(35)]
