...
from platform.core.rl.advanced_config import (
...
class TestOffsetTreeConfig:
    ...
    def test_custom_initialization(self):
        """Test custom configuration values."""
        config = OffsetTreeConfig(
            max_depth=5, min_samples_split=20, split_strategy="information_gain", base_bandit_type="epsilon_greedy"
        )
...
class TestAdvancedBanditConfigManager:
    ...
    @patch.dict(
        "os.environ", {"RL_ROLLOUT_DOMAINS": "model_routing,content_analysis", "RL_ROLLOUT_TENANTS": "tenant1,tenant2"}
    )
