...
from platform.core.rl.advanced_experiments import AdvancedBanditExperimentManager, AdvancedBanditStats, create_default_advanced_bandit_experiments
...
class TestAdvancedBanditStats:
    ...
    def test_as_dict_includes_advanced_metrics(self):
        ...
        stats = AdvancedBanditStats(pulls=10, reward_sum=5.0, reward_model_mse=0.1, tree_depth_sum=20, importance_weight_sum=15.0, confidence_interval_width=3.0)
        result = stats.as_dict()
        assert result['pulls'] == 10.0
        assert result['reward_sum'] == 5.0
        assert result['reward_mean'] == 0.5
        assert result['reward_model_mse'] == 0.1
        assert result['tree_depth_avg'] == 2.0
        assert result['importance_weight_avg'] == 1.5
        assert result['confidence_interval_avg'] == 0.3
...
class TestAdvancedBanditExperimentManager:
    ...
    def test_register_advanced_bandit_experiment(self):
        ...
        manager = AdvancedBanditExperimentManager()
        manager.register_advanced_bandit_experiment(domain='test_domain', baseline_policy='epsilon_greedy', advanced_policies={'doubly_robust': 0.5}, shadow_samples=100)
    ...
    def test_record_advanced_metrics_doubly_robust(self, mock_getenv, mock_tenant):
        ...
        with patch.object(bandit, '_predict_reward', return_value=0.7):
            manager.record_advanced_metrics('advanced_bandits::test', 'doubly_robust', 0.8, context, bandit)
        ...
        assert abs(stats.reward_model_mse - 0.01) < 1e-10
    ...
    def test_analyze_advanced_performance(self):
        ...
        assert abs(dr_comparison['reward_improvement_pct'] - 20.0) < 0.01
...
class TestUtilityFunctions:
    ...
    def test_create_default_experiments_enabled(self, mock_getenv):
        ...
        expected_domains = ['model_routing', 'content_analysis', 'transcription_service', 'memory_retrieval']
...
class TestIntegration:
    ...
    def test_full_experiment_lifecycle(self):
        ...
        manager = AdvancedBanditExperimentManager()
        manager.register_advanced_bandit_experiment(domain='integration_test', shadow_samples=5)
