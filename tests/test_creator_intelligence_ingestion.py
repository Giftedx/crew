...
class TestCreatorIntelligenceIngestionTools:
    ...
    def setup_method(self) -> None:
        ...
        self.tools = CreatorIntelligenceIngestionTools(
            sqlite_path=":memory:",
            enable_vector_storage=False,  # Disable for unit tests
        )
    ...
    @patch("ingest.providers.youtube.fetch_metadata")
    def test_ingest_youtube_video_metadata_failure(
        self,
        mock_fetch_metadata: MagicMock,
    ) -> None:
    ...
    def test_ingest_twitch_clip_success(
        self,
        mock_fetch_transcript: MagicMock,
        mock_fetch_metadata: MagicMock,
    ) -> None:
    ...
    @patch("ingest.providers.twitch.fetch_metadata")
    def test_ingest_twitch_clip_failure(
        self,
        mock_fetch_metadata: MagicMock,
    ) -> None:
...
class TestIngestionWithVectorStorage:
    ...
    @patch("ingest.providers.youtube.fetch_metadata")
    @patch("ingest.providers.youtube.fetch_transcript")
    def test_ingest_with_vector_storage(
        self,
        mock_fetch_transcript: MagicMock,
        mock_fetch_metadata: MagicMock,
    ) -> None:
        ...
        assert result.success
        # Note: Actual vector storage depends on embedding model availability
        # In test environment with fallback, stored_in_vector_db may vary
