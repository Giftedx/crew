...
from src.ultimate_discord_intelligence_bot.orchestrator import discord_result_persistence
...
class TestDiscordResultPersistence:
    ...
    async def test_persist_workflow_results_success(self):
        ...
        with tempfile.TemporaryDirectory():
            with patch(
                "src.ultimate_discord_intelligence_bot.orchestrator.discord_result_persistence.Path"
            ) as mock_path:
        ...
        with patch("src.ultimate_discord_intelligence_bot.orchestrator.discord_result_persistence.Path") as mock_path:
            ...
            with patch("builtins.open", mock_open()):
                with patch("json.dump", side_effect=TypeError("Object not JSON serializable")):
                    # Act
                    result_path = await discord_result_persistence.persist_workflow_results(
                        self.workflow_id, non_serializable_results, self.url, self.depth
                    )
        ...
        with patch("src.ultimate_discord_intelligence_bot.orchestrator.discord_result_persistence.Path") as mock_path:
            ...
            with patch("builtins.open", mock_open()):
                with patch(
                    ...
                    # Act
                    result_path = await discord_result_persistence.persist_workflow_results(
                        self.workflow_id, self.results, self.url, self.depth, log=custom_logger
                    )
    ...
    async def test_persist_workflow_results_data_structure(self):
        ...
        with patch("src.ultimate_discord_intelligence_bot.orchestrator.discord_result_persistence.Path") as mock_path:
            ...
            with patch("builtins.open", mock_open()):
                with patch("json.dump", side_effect=capture_json_data):
                    with patch(
                        "src.ultimate_discord_intelligence_bot.orchestrator.discord_result_persistence._get_metrics"
                    ):
                        # Act
