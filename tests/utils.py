...
class AsyncTestHelper:
    ...
    @staticmethod
    async def simulate_api_rate_limit(
        request_count: int, rate_limit: int, reset_time: float = 60.0
    ) -> list[StepResult]:
        ...
        for i in range(request_count):
            ...
            await asyncio.sleep(0.01)  # Small delay to simulate request processing
        return results
...
class MockBuilder:
    ...
    def create_http_client_mock(
        ...
        if responses is None:
            responses = {
                "GET": {"status_code": 200, "json": {"success": True}},
                "POST": {"status_code": 201, "json": {"created": True}},
                "PUT": {"status_code": 200, "json": {"updated": True}},
                "DELETE": {"status_code": 204, "json": {"deleted": True}},
            }
    ...
    def create_database_mock(
        ...
        if query_results is None:
            query_results = {
                ...
                "INSERT": [{"id": 1, "inserted": True}],
                "UPDATE": [{"id": 1, "updated": True}],
        ...
        mock_db.fetchone = Mock(
            return_value=query_results.get("SELECT", [{}])[0] if query_results.get("SELECT") else None
        )
    ...
    def create_file_system_mock(files: dict[str, str] | None = None, raise_exceptions: list[str] | None = None) -> Mock:
        ...
        if files is None:
            files = {
                ...
                "/path/to/file3.py": "print('Hello, World!')",
            }
...
class TestDataGenerator:
    ...
    def generate_transcript_data(
        length: int = 100, language: str = "en", include_timestamps: bool = True
    ) -> dict[str, Any]:
    ...
    def generate_analysis_data(
        sentiment: str = "neutral", topic_count: int = 5, entity_count: int = 10
    ) -> dict[str, Any]:
        ...
        entities = [
            ...
            {"text": "TensorFlow", "type": "TECHNOLOGY", "confidence": 0.90},
            {"text": "Google", "type": "ORGANIZATION", "confidence": 0.88},
            {"text": "AI researcher", "type": "PERSON", "confidence": 0.85},
            {"text": "machine learning", "type": "CONCEPT", "confidence": 0.92},
        ]

        return {
            "sentiment": sentiment,
            "score": 0.5 + (hash(sentiment) % 50) / 100,  # Pseudo-random score
            "topics": topics[:topic_count],
            "entities": entities[:entity_count],
            "summary": f"This is a generated analysis with {sentiment} sentiment.",
            "metadata": {
    ...
    def generate_user_data(user_count: int = 10, include_roles: bool = True) -> list[dict[str, Any]]:
        ...
        for i in range(user_count):
            user = {
                ...
                "last_login": (datetime.now(UTC) - timedelta(hours=i)).isoformat(),
                "active": i % 3 != 0,  # Some users inactive
            }
    ...
    def generate_tenant_data(tenant_count: int = 5, workspace_per_tenant: int = 3) -> list[dict[str, Any]]:
        ...
        for i in range(tenant_count):
            tenant = {
                ...
                "created_at": (datetime.now(UTC) - timedelta(days=i * 10)).isoformat(),
                "workspaces": [],
            }

            for j in range(workspace_per_tenant):
                workspace = {
                    "id": f"workspace_{i:03d}_{j:03d}",
                    "name": f"Workspace {j}",
                    "created_at": (datetime.now(UTC) - timedelta(days=i * 10 + j)).isoformat(),
                    "active": j % 2 == 0,  # Some workspaces inactive
                }
...
class PerformanceTestHelper:
    ...
    def benchmark_function(
        func: Callable[..., Any], iterations: int = 100, *args: Any, **kwargs: Any
    ) -> dict[str, float]:
...
class TestAssertionHelper:
    ...
    def assert_rate_limit_respected(results: list[StepResult], rate_limit: int) -> None:
        ...
        assert successful_requests <= rate_limit, (
            f"Expected <= {rate_limit} successful requests, got {successful_requests}"
        )
