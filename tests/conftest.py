...
def mock_routing_manager():
    ...
    routing_manager.suggest_model.return_value = AsyncMock(success=True, data={'model': 'gpt-4o-mini', 'reasoning': 'Fast and cost-effective'})
...
def mock_prompt_engine():
    ...
    prompt_engine.generate_response.return_value = AsyncMock(success=True, data='{"should_respond": true, "confidence": 0.8, "reasoning": "Direct mention detected", "priority": "high", "suggested_personality_traits": {"humor": 0.6, "formality": 0.3, "enthusiasm": 0.8, "knowledge_confidence": 0.9, "debate_tolerance": 0.5, "empathy": 0.7, "creativity": 0.6, "directness": 0.8}, "context_relevance_score": 0.9}')
...
def sample_discord_message():
    ...
    return {'id': '123456789', 'channel_id': '987654321', 'guild_id': '111222333', 'author': {'id': '444555666', 'username': 'testuser'}, 'content': '@bot help me with something', 'timestamp': 1640995200.0}

@pytest.fixture
def sample_recent_messages():
    """Sample recent messages for context."""
    return [{'author': {'username': 'user1'}, 'content': 'Previous message 1', 'timestamp': 1640995100.0}, {'author': {'username': 'user2'}, 'content': 'Previous message 2', 'timestamp': 1640995150.0}]
...
def sample_personality_traits():
    ...
    return PersonalityTraits(humor=0.8, formality=0.3, enthusiasm=0.9, knowledge_confidence=0.85, debate_tolerance=0.4, empathy=0.7, creativity=0.8, directness=0.6)
...
def sample_interaction_metrics():
    ...
    return InteractionMetrics(message_id='msg123', user_id='user123', guild_id='guild123', timestamp=1640995200.0, user_replies=2, user_reactions=['👍', '❤️'], follow_up_messages=1, conversation_continuation=True, time_to_first_reply=30.0, conversation_duration=300.0, message_length=50, response_length=80)
...
def sample_personality_context():
    ...
    return PersonalityContext(channel_type='casual', time_of_day='afternoon', user_history=[], message_sentiment=0.5, conversation_length=5, guild_culture='gaming')
...
class TestDataFactory:
    ...
    @staticmethod
    def create_discord_message(message_id='test123', user_id='user123', guild_id='guild123', content='@bot help', timestamp=1640995200.0):
        """Create Discord message data."""
        return {'id': message_id, 'channel_id': f'channel_{message_id}', 'guild_id': guild_id, 'author': {'id': user_id, 'username': f'user_{user_id}'}, 'content': content, 'timestamp': timestamp}
    ...
    def create_interaction_metrics(message_id='msg123', user_id='user123', engagement_level='high'):
        ...
        if engagement_level == 'high':
            return InteractionMetrics(message_id=message_id, user_id=user_id, guild_id='guild123', timestamp=1640995200.0, user_replies=3, user_reactions=['👍', '❤️', '🔥'], follow_up_messages=2, conversation_continuation=True, time_to_first_reply=15.0, conversation_duration=600.0, message_length=80, response_length=120)
        elif engagement_level == 'medium':
            return InteractionMetrics(message_id=message_id, user_id=user_id, guild_id='guild123', timestamp=1640995200.0, user_replies=1, user_reactions=['👍'], follow_up_messages=1, conversation_continuation=True, time_to_first_reply=120.0, conversation_duration=300.0, message_length=40, response_length=60)
        else:
            return InteractionMetrics(message_id=message_id, user_id=user_id, guild_id='guild123', timestamp=1640995200.0, user_replies=0, user_reactions=[], follow_up_messages=0, conversation_continuation=False, time_to_first_reply=300.0, conversation_duration=60.0, message_length=10, response_length=5)
