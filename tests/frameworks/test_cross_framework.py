...
class TestCrossFrameworkCompatibility:
    ...
    def test_feature_support_matrix(self) -> None:
        ...
        features_to_check = [FrameworkFeature.SEQUENTIAL_EXECUTION, FrameworkFeature.PARALLEL_EXECUTION, FrameworkFeature.ASYNC_EXECUTION, FrameworkFeature.STATE_PERSISTENCE, FrameworkFeature.MULTI_AGENT_COLLABORATION, FrameworkFeature.CUSTOM_TOOLS, FrameworkFeature.STREAMING]
        feature_matrix = {}
        for framework_name in frameworks:
            adapter = get_framework_adapter(framework_name)
            feature_matrix[framework_name] = {feature: adapter.supports_feature(feature) for feature in features_to_check}
    ...
    async def test_execute_task_interface_consistency(self) -> None:
        ...
        task = CrewTask(task_id='cross_framework_test', task_type='general', description='Simple test task for cross-framework compatibility', inputs={'message': 'hello world'})
        config = CrewConfig(tenant_id='test')
        frameworks_to_test = ['crewai']
