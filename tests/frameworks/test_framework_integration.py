...
class TestCrewAIAdapter:
    ...
    async def test_create_agent(self) -> None:
        ...
        adapter = get_framework_adapter('crewai')
        role = AgentRole(role_name='researcher', goal='Find and analyze information', backstory='An expert researcher', capabilities=['web_search'])
...
class TestFrameworkInteroperability:
    ...
    async def test_execute_task_via_adapter(self) -> None:
        ...
        adapter = get_framework_adapter('crewai')
        task = CrewTask(task_id='test-task-001', task_type='simple_test', description='Test task for framework validation', inputs={'test_data': 'validation'}, priority=CrewPriority.NORMAL)
        config = CrewConfig(tenant_id='test-tenant', timeout_seconds=30)
...
class TestAutoGenAdapter:
    ...
    def test_get_capabilities(self) -> None:
        ...
        try:
            ...
            assert len(capabilities['state_backends']) == 0
        except ValueError:
            pytest.skip('AutoGen adapter not available')
...
class TestBackwardCompatibility:
    ...
    def test_crew_task_unchanged(self) -> None:
        ...
        task = CrewTask(task_id='test', task_type='test_type', description='Test task', inputs={'key': 'value'})
