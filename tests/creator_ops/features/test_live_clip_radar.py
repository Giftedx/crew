    ...
    async def test_check_sentiment_flip_positive_flip(self):
        ...
        for i in range(10):
            self.clip_radar.sentiment_history[stream_key].append(
                {"timestamp": now - timedelta(minutes=15, seconds=i), "sentiment": -0.8, "message": f"Bad message {i}"}
            )
        for i in range(10):
            self.clip_radar.sentiment_history[stream_key].append(
                {"timestamp": now - timedelta(seconds=i), "sentiment": 0.8, "message": f"Good message {i}"}
            )
