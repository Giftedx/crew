...
from ultimate_discord_intelligence_bot.creator_ops.features.intelligence_models import AgendaItem, BrandSafetyAnalysis, DefamationRisk, EpisodeIntelligence, FactCheckableClaim, GuestInfo, IntelligenceConfig, IntelligenceResult, OutboundLink, RiskLevel
...
class TestEpisodeIntelligencePack:
    ...
    def setup_method(self):
        ...
        self.transcript = AlignedTranscript(segments=[AlignedSegment(start_time=0.0, end_time=30.0, speakers=['Host'], text="Welcome to the show! Today we're discussing technology trends.", topics=['technology', 'trends'], sentiment_score=0.2), AlignedSegment(start_time=30.0, end_time=60.0, speakers=['Guest'], text='According to recent studies, AI is transforming industries.', topics=['AI', 'industries'], sentiment_score=0.1), AlignedSegment(start_time=60.0, end_time=90.0, speakers=['Host'], text="That's fascinating! Can you share more details?", topics=['AI', 'details'], sentiment_score=0.3)], duration=90.0, metadata={'title': 'Tech Trends Discussion'})
    ...
    async def test_generate_intelligence_pack_success(self):
        ...
        with patch.object(self.intelligence_pack, '_generate_agenda') as mock_agenda, patch.object(self.intelligence_pack, '_generate_guest_info') as mock_guests, patch.object(self.intelligence_pack, '_generate_claims') as mock_claims, patch.object(self.intelligence_pack, '_generate_quotations') as mock_quotations, patch.object(self.intelligence_pack, '_generate_links') as mock_links, patch.object(self.intelligence_pack, '_generate_thumbnail_suggestions') as mock_thumbnails, patch.object(self.intelligence_pack, '_generate_brand_safety_analysis') as mock_brand_safety, patch.object(self.intelligence_pack, '_generate_defamation_risk') as mock_defamation, patch.object(self.intelligence_pack, '_generate_key_insights') as mock_insights, patch.object(self.intelligence_pack, '_export_intelligence_pack') as mock_export:
            ...
            result = await self.intelligence_pack.generate_intelligence_pack(episode_id='test_episode', transcript=self.transcript)
    ...
    async def test_generate_links(self):
        ...
        transcript_with_url = AlignedTranscript(segments=[AlignedSegment(start_time=0.0, end_time=30.0, speakers=['Host'], text='Check out this link: https://example.com for more info', topics=['technology'], sentiment_score=0.2)], duration=30.0, metadata={'title': 'Test Episode'})
    ...
    async def test_generate_key_insights(self):
        ...
        intelligence = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=3600, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=2, total_segments=10, guests=[Mock(), Mock()], claims=[Mock() for _ in range(6)])
    ...
    async def test_export_intelligence_pack(self):
        ...
        intelligence = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=90.0, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=2, total_segments=3)
    ...
    def test_generate_markdown_content(self):
        ...
        intelligence = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=90.0, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=2, total_segments=3, agenda=[AgendaItem(title='Introduction', start_time=0.0, end_time=30.0, duration=30.0, description='Welcome segment', speakers=['Host'], topics=['introduction'])])
    ...
    def test_helper_methods(self):
        ...
        segment = AlignedSegment(start_time=0.0, end_time=30.0, speakers=['Host'], text='This is exciting!', topics=['excitement'], sentiment_score=0.8)
...
class TestEpisodeIntelligenceAgent:
    ...
    async def test_analyze_episode_intelligence(self):
        ...
        intelligence_pack = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=90.0, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=2, total_segments=3)
        ...
        with patch.object(self.agent.crew, 'kickoff') as mock_kickoff:
            mock_kickoff.return_value = {'content_analysis': 'Content is engaging and well-structured', 'risk_assessment': 'Low risk content with no major concerns', 'monetization_analysis': 'Good opportunities for sponsorships', 'audience_engagement': {'retention': 0.8, 'engagement': 0.7}}
    ...
    def test_parse_crew_results(self):
        ...
        mock_result = {'content_analysis': 'Content is engaging and well-structured', 'risk_assessment': 'Low risk content with no major concerns', 'monetization_analysis': 'Good opportunities for sponsorships', 'audience_engagement': {'retention': 0.8, 'engagement': 0.7}}
    ...
    def test_generate_episode_summary(self):
        ...
        analysis = {'content_analysis': 'Content is engaging and well-structured', 'risk_assessment': 'Low risk content with no major concerns', 'monetization_analysis': 'Good opportunities for sponsorships'}
    ...
    async def test_generate_recommendations(self):
        ...
        intelligence_pack = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=90.0, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=1, total_segments=3, claims=[Mock()], quotations=[Mock()], links=[Mock()], guests=[Mock()], average_sentiment=-0.5)
    ...
    async def test_get_intelligence_dashboard_data(self):
        ...
        intelligence_pack = EpisodeIntelligence(episode_id='test', episode_title='Test Episode', episode_duration=90.0, created_at=datetime.now(), updated_at=datetime.now(), total_speakers=2, total_segments=3, agenda=[Mock()], guests=[Mock()], claims=[Mock()], quotations=[Mock()], links=[Mock()], brand_safety=Mock(overall_score=4.0, flagged_segments=[]), defamation_risk=Mock(risk_level=RiskLevel.LOW), top_topics=['technology', 'AI'])
        analysis = Mock()
        analysis.key_insights = ['Insight 1', 'Insight 2']
        analysis.content_recommendations = ['Rec 1', 'Rec 2']
        analysis.monetization_opportunities = ['Opp 1', 'Opp 2']
