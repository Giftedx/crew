...
class TestClipRadar:
    ...
    def setup_method(self):
        ...
        self.config = ClipRadarConfig(enabled=True, check_interval=30, moment_threshold=0.8, min_clip_duration=10, max_clip_duration=60, platforms=['twitch', 'youtube'], keywords=['hype', 'epic', 'amazing', 'wow'], sentiment_threshold=0.7, engagement_threshold=100, auto_create_clips=True, notification_webhook='https://example.com/webhook')
    ...
    def test_detect_moment_success(self):
        ...
        stream_data = {'stream_id': '12345', 'title': 'Epic Gaming Session', 'viewer_count': 1000, 'chat_rate': 50, 'timestamp': datetime.now()}
        chat_messages = [{'message': 'This is AMAZING!', 'sentiment': 0.9, 'engagement': 150}, {'message': 'EPIC moment!', 'sentiment': 0.8, 'engagement': 120}, {'message': 'WOW that was incredible!', 'sentiment': 0.95, 'engagement': 200}]
        with patch.object(self.clip_radar, '_analyze_chat_engagement') as mock_analyze:
            mock_analyze.return_value = {'avg_sentiment': 0.88, 'total_engagement': 470, 'keyword_matches': 3, 'moment_score': 0.9}
    ...
    def test_detect_moment_low_engagement(self):
        ...
        stream_data = {'stream_id': '12345', 'title': 'Quiet Stream', 'viewer_count': 100, 'chat_rate': 5, 'timestamp': datetime.now()}
        chat_messages = [{'message': 'hello', 'sentiment': 0.5, 'engagement': 10}, {'message': 'hi there', 'sentiment': 0.4, 'engagement': 8}]
        with patch.object(self.clip_radar, '_analyze_chat_engagement') as mock_analyze:
            mock_analyze.return_value = {'avg_sentiment': 0.45, 'total_engagement': 18, 'keyword_matches': 0, 'moment_score': 0.3}
            result = self.clip_radar.detect_moment(stream_data, chat_messages)
        assert result.success
        assert 'moment' in result.data
        moment = result.data['moment']
    ...
    def test_create_clip_success(self):
        ...
        moment = ClipMoment(moment_id='moment_123', stream_id='12345', timestamp=datetime.now(), moment_score=0.9, title='Epic Gaming Moment', description='Amazing gameplay sequence', start_time=120.0, end_time=150.0, keywords=['epic', 'amazing'], sentiment_score=0.85, engagement_score=200, is_clipworthy=True)
        with patch.object(self.clip_radar, '_create_twitch_clip') as mock_twitch:
            mock_twitch.return_value = StepResult.ok(data={'clip_id': 'clip_123', 'url': 'https://clips.twitch.tv/clip_123'})
    ...
    def test_create_clip_platform_not_supported(self):
        ...
        moment = ClipMoment(moment_id='moment_123', stream_id='12345', timestamp=datetime.now(), moment_score=0.9, title='Epic Moment', description='Test moment', start_time=120.0, end_time=150.0, keywords=['epic'], sentiment_score=0.85, engagement_score=200, is_clipworthy=True)
    ...
    def test_analyze_chat_engagement_success(self):
        ...
        chat_messages = [{'message': 'This is AMAZING!', 'sentiment': 0.9, 'engagement': 150}, {'message': 'EPIC moment!', 'sentiment': 0.8, 'engagement': 120}, {'message': 'WOW that was incredible!', 'sentiment': 0.95, 'engagement': 200}, {'message': 'hello', 'sentiment': 0.5, 'engagement': 10}]
    ...
    def test_calculate_moment_score(self):
        ...
        engagement_data = {'avg_sentiment': 0.85, 'total_engagement': 300, 'keyword_matches': 5, 'moment_score': 0.0}
    ...
    def test_monitoring_workflow_e2e(self):
        ...
        user_id = 'test_user'
        stream_data = {'stream_id': '12345', 'title': 'Epic Gaming Session', 'viewer_count': 1000, 'chat_rate': 50, 'timestamp': datetime.now(), 'is_live': True}
        chat_messages = [{'message': 'This is AMAZING!', 'sentiment': 0.9, 'engagement': 150}, {'message': 'EPIC moment!', 'sentiment': 0.8, 'engagement': 120}, {'message': 'WOW that was incredible!', 'sentiment': 0.95, 'engagement': 200}]
            ...
            with patch.object(self.clip_radar, '_get_chat_messages') as mock_chat:
                with patch.object(self.clip_radar, '_analyze_chat_engagement') as mock_analyze:
                    with patch.object(self.clip_radar, '_create_twitch_clip') as mock_clip:
                        ...
                        mock_analyze.return_value = {'avg_sentiment': 0.88, 'total_engagement': 470, 'keyword_matches': 3, 'moment_score': 0.9}
                        mock_clip.return_value = StepResult.ok(data={'clip_id': 'clip_123', 'url': 'https://clips.twitch.tv/clip_123'})
    ...
    def test_notification_webhook_success(self):
        ...
        clip_data = {'clip_id': 'clip_123', 'url': 'https://clips.twitch.tv/clip_123', 'title': 'Epic Moment', 'moment_score': 0.9, 'timestamp': datetime.now().isoformat()}
        ...
        assert result.success
        mock_post.assert_called_once_with(self.config.notification_webhook, json=clip_data, headers={'Content-Type': 'application/json'}, timeout=10)
    ...
    def test_concurrent_moment_detection(self):
        ...
        def detect_moment_worker(stream_id):
            try:
                stream_data = {'stream_id': stream_id, 'title': f'Stream {stream_id}', 'viewer_count': 1000, 'chat_rate': 50, 'timestamp': datetime.now()}
                chat_messages = [{'message': 'This is AMAZING!', 'sentiment': 0.9, 'engagement': 150}, {'message': 'EPIC moment!', 'sentiment': 0.8, 'engagement': 120}]
                with patch.object(self.clip_radar, '_analyze_chat_engagement') as mock_analyze:
                    mock_analyze.return_value = {'avg_sentiment': 0.85, 'total_engagement': 270, 'keyword_matches': 2, 'moment_score': 0.8}
                    result = self.clip_radar.detect_moment(stream_data, chat_messages)
    ...
    def test_configuration_validation(self):
        ...
        invalid_config = ClipRadarConfig(enabled=True, check_interval=-1, moment_threshold=2.0, min_clip_duration=100, max_clip_duration=60, platforms=[], keywords=[], sentiment_threshold=0.7, engagement_threshold=100, auto_create_clips=True, notification_webhook='invalid_url')
    ...
    def test_platform_integration_youtube(self):
        ...
        with patch.object(self.clip_radar, '_create_youtube_clip') as mock_youtube:
            mock_youtube.return_value = StepResult.ok(data={'clip_id': 'clip_456', 'url': 'https://youtube.com/watch?v=clip_456'})
            moment = ClipMoment(moment_id='moment_123', stream_id='67890', timestamp=datetime.now(), moment_score=0.9, title='Epic Moment', description='Test moment', start_time=120.0, end_time=150.0, keywords=['epic'], sentiment_score=0.85, engagement_score=200, is_clipworthy=True)
