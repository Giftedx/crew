...
class TestDiarizationMinimal:
    ...
    def test_diarization_result_creation(self):
        ...
        segments = [
            Mock(start_time=0.0, end_time=2.5, speaker="SPEAKER_00"),
            Mock(start_time=2.5, end_time=5.0, speaker="SPEAKER_01"),
        ]

        result = MockDiarizationResult(
            segments=segments,
    ...
    def test_overlap_detection_logic(self):
        ...
        segments = [
            {"start_time": 0.0, "end_time": 3.0, "speaker": "SPEAKER_00"},
            {"start_time": 2.0, "end_time": 5.0, "speaker": "SPEAKER_01"},
            {"start_time": 6.0, "end_time": 8.0, "speaker": "SPEAKER_00"},
        ]

        overlaps = detect_overlaps(segments)
        assert len(overlaps) == 1
        assert overlaps[0]["duration"] == 1.0  # 3.0 - 2.0 = 1.0
        assert overlaps[0]["speaker1"] == "SPEAKER_00"
        assert overlaps[0]["speaker2"] == "SPEAKER_01"

        # Test with no overlaps
        segments_no_overlap = [
            {"start_time": 0.0, "end_time": 2.0, "speaker": "SPEAKER_00"},
            {"start_time": 2.0, "end_time": 4.0, "speaker": "SPEAKER_01"},
        ]
    ...
    def test_der_calculation_logic(self):
        ...
        def calculate_der(reference_segments, predicted_segments):
            # Simple DER calculation based on speaker mismatches
            total_duration = 0
            error_duration = 0
        ...
        assert der == 0.5  # 50% error rate

    def test_file_handling(self):
        """Test file handling for audio files."""
        # Test creating and cleaning up temporary files
