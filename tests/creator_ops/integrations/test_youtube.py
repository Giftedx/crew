...
class TestYouTubeClient:
    ...
    def test_rate_limiting_backoff(self):
        ...
        with (
            patch.object(self.client.session, "get", side_effect=[mock_429_response, mock_success_response]),
    ...
    def test_partial_response_handling(self):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {
            "items": [{"id": self.test_video_id, "snippet": {"title": "Test Video", "channelId": self.test_channel_id}}]
        }
        mock_response.status_code = 200
        mock_response.raise_for_status.return_value = None
        with patch.object(self.client.session, "get", return_value=mock_response):
