...
class TestYouTubeContract:
    ...
    def test_get_video_info_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': 'test_video_id', 'snippet': {'title': 'Test Video', 'description': 'Test Description', 'channelId': 'test_channel_id', 'channelTitle': 'Test Channel', 'publishedAt': '2023-01-01T00:00:00Z', 'thumbnails': {'default': {'url': 'https://example.com/thumb.jpg'}}}, 'statistics': {'viewCount': '1000', 'likeCount': '100', 'commentCount': '50'}}]})
    ...
    def test_get_channel_info_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': 'test_channel_id', 'snippet': {'title': 'Test Channel', 'description': 'Test Channel Description', 'publishedAt': '2023-01-01T00:00:00Z', 'thumbnails': {'default': {'url': 'https://example.com/channel_thumb.jpg'}}}, 'statistics': {'subscriberCount': '10000', 'videoCount': '100', 'viewCount': '1000000'}}]})
    ...
    def test_get_video_comments_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': 'comment_id', 'snippet': {'topLevelComment': {'snippet': {'textDisplay': 'Test comment', 'authorDisplayName': 'Test User', 'publishedAt': '2023-01-01T00:00:00Z', 'likeCount': 5}}}}]})
    ...
    def test_get_video_captions_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': 'caption_id', 'snippet': {'language': 'en', 'name': 'English', 'isDraft': False, 'isAutoGenerated': False}}]})
    ...
    def test_search_videos_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': {'videoId': 'test_video_id'}, 'snippet': {'title': 'Test Video', 'description': 'Test Description', 'channelId': 'test_channel_id', 'channelTitle': 'Test Channel', 'publishedAt': '2023-01-01T00:00:00Z', 'thumbnails': {'default': {'url': 'https://example.com/thumb.jpg'}}}}]})
    ...
    def test_get_live_chat_messages_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'items': [{'id': 'message_id', 'snippet': {'type': 'textMessageEvent', 'liveChatId': 'live_chat_id', 'authorChannelId': 'author_channel_id', 'publishedAt': '2023-01-01T00:00:00Z', 'textMessageDetails': {'messageText': 'Test message'}}, 'authorDetails': {'channelId': 'author_channel_id', 'displayName': 'Test User', 'isChatModerator': False, 'isChatOwner': False, 'isChatSponsor': False, 'isVerified': False}}]})
...
class TestTwitchContract:
    ...
    def test_get_user_info_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': '123456789', 'login': 'testuser', 'display_name': 'Test User', 'type': '', 'broadcaster_type': 'partner', 'description': 'Test description', 'profile_image_url': 'https://example.com/profile.jpg', 'offline_image_url': 'https://example.com/offline.jpg', 'view_count': 1000000, 'created_at': '2023-01-01T00:00:00Z'}]})
    ...
    def test_get_stream_info_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'stream_id', 'user_id': '123456789', 'user_login': 'testuser', 'user_name': 'Test User', 'game_id': 'game_id', 'game_name': 'Test Game', 'type': 'live', 'title': 'Test Stream', 'viewer_count': 1000, 'started_at': '2023-01-01T00:00:00Z', 'language': 'en', 'thumbnail_url': 'https://example.com/thumb.jpg', 'tag_ids': ['tag1', 'tag2'], 'is_mature': False}]})
    ...
    def test_get_video_info_contract(self, client):
        """Test that get_video_info returns expected data structure."""
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'video_id', 'stream_id': 'stream_id', 'user_id': '123456789', 'user_login': 'testuser', 'user_name': 'Test User', 'title': 'Test Video', 'description': 'Test description', 'created_at': '2023-01-01T00:00:00Z', 'published_at': '2023-01-01T00:00:00Z', 'url': 'https://example.com/video.mp4', 'thumbnail_url': 'https://example.com/thumb.jpg', 'viewable': 'public', 'view_count': 1000, 'language': 'en', 'type': 'archive', 'duration': '1h30m45s'}]})
    ...
    def test_get_clip_info_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'clip_id', 'url': 'https://clips.twitch.tv/clip_id', 'embed_url': 'https://clips.twitch.tv/embed?clip=clip_id', 'broadcaster_id': '123456789', 'broadcaster_name': 'Test User', 'creator_id': '987654321', 'creator_name': 'Clip Creator', 'video_id': 'video_id', 'game_id': 'game_id', 'title': 'Test Clip', 'view_count': 1000, 'created_at': '2023-01-01T00:00:00Z', 'thumbnail_url': 'https://example.com/clip_thumb.jpg', 'duration': 30}]})
    ...
    def test_get_chat_messages_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'message_id', 'user_id': '123456789', 'user_name': 'testuser', 'user_login': 'testuser', 'message': 'Test message', 'timestamp': '2023-01-01T00:00:00Z', 'message_type': 'chat', 'badges': ['subscriber/6', 'premium/1'], 'color': '#FF0000', 'emotes': ['25:0-4'], 'reply_parent_msg_id': None, 'reply_parent_user_id': None, 'reply_parent_user_login': None, 'reply_parent_display_name': None, 'reply_parent_msg_body': None, 'thread_parent_msg_id': None, 'thread_parent_user_login': None, 'thread_parent_display_name': None, 'thread_parent_msg_body': None}]})
...
class TestTikTokContract:
    ...
    def test_get_user_info_contract(self, client):
        """Test that get_user_info returns expected data structure."""
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': {'user': {'open_id': 'user_open_id', 'union_id': 'user_union_id', 'avatar_url': 'https://example.com/avatar.jpg', 'avatar_url_100': 'https://example.com/avatar_100.jpg', 'avatar_url_200': 'https://example.com/avatar_200.jpg', 'display_name': 'Test User', 'bio_description': 'Test bio', 'profile_deep_link': 'https://example.com/profile', 'is_verified': True, 'follower_count': 10000, 'following_count': 1000, 'likes_count': 50000, 'video_count': 100}}})
    ...
    def test_get_user_videos_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': {'videos': [{'id': 'video_id', 'create_time': 1672531200, 'cover_image_url': 'https://example.com/cover.jpg', 'share_url': 'https://example.com/share', 'video_description': 'Test video description', 'duration': 30, 'height': 1920, 'width': 1080, 'origin_cover_image_url': 'https://example.com/origin_cover.jpg', 'play_addr': {'url_list': ['https://example.com/video.mp4']}, 'ratio': '9:16', 'reflow_cover_image_url': 'https://example.com/reflow_cover.jpg', 'video_id': 'video_id', 'video_quality': 'normal'}]}})
    ...
    def test_get_video_comments_contract(self, client):
        """Test that get_video_comments returns expected data structure."""
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': {'comments': [{'comment_id': 'comment_id', 'comment': 'Test comment', 'create_time': 1672531200, 'digg_count': 10, 'reply_comment_total': 5, 'reply_to_comment_id': None, 'reply_to_user_id': None, 'text_extra': [], 'user': {'avatar_url': 'https://example.com/avatar.jpg', 'display_name': 'Test User', 'open_id': 'user_open_id', 'union_id': 'user_union_id'}}]}})
    ...
    def test_get_video_insights_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': {'list': [{'date': '2023-01-01', 'video_id': 'video_id', 'video_title': 'Test Video', 'video_description': 'Test description', 'like_count': 1000, 'comment_count': 100, 'share_count': 50, 'view_count': 10000, 'profile_views': 500, 'video_play_time': 30000, 'video_views': 10000, 'real_time_result': {'like_count': 1000, 'comment_count': 100, 'share_count': 50, 'view_count': 10000}}]}})
...
class TestInstagramContract:
    ...
    def test_get_user_info_contract(self, client):
        """Test that get_user_info returns expected data structure."""
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'id': '17841400000000000', 'username': 'testuser', 'account_type': 'BUSINESS', 'media_count': 100, 'followers_count': 10000, 'follows_count': 1000, 'name': 'Test User', 'biography': 'Test bio', 'website': 'https://example.com', 'profile_picture_url': 'https://example.com/profile.jpg'})
    ...
    def test_get_user_media_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'media_id', 'media_type': 'IMAGE', 'media_url': 'https://example.com/media.jpg', 'username': 'testuser', 'timestamp': '2023-01-01T00:00:00+0000', 'caption': 'Test caption', 'permalink': 'https://example.com/p/ABC123', 'thumbnail_url': 'https://example.com/thumb.jpg', 'like_count': 1000, 'comments_count': 100, 'children': {'data': [{'id': 'child_media_id', 'media_type': 'VIDEO', 'media_url': 'https://example.com/video.mp4'}]}}]})
    ...
    def test_get_media_comments_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'comment_id', 'text': 'Test comment', 'timestamp': '2023-01-01T00:00:00+0000', 'like_count': 5, 'media': {'id': 'media_id'}, 'user': {'id': 'user_id', 'username': 'testuser', 'account_type': 'PERSONAL'}, 'replies': {'data': [{'id': 'reply_id', 'text': 'Test reply', 'timestamp': '2023-01-01T00:01:00+0000', 'like_count': 2, 'user': {'id': 'reply_user_id', 'username': 'replyuser', 'account_type': 'PERSONAL'}}]}}]})
    ...
    def test_get_user_stories_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'story_id', 'media_type': 'VIDEO', 'media_url': 'https://example.com/story.mp4', 'timestamp': '2023-01-01T00:00:00+0000', 'permalink': 'https://example.com/s/ABC123', 'thumbnail_url': 'https://example.com/story_thumb.jpg', 'like_count': 100, 'comments_count': 10, 'reactions': {'data': [{'reaction_type': 'like', 'user': {'id': 'user_id', 'username': 'testuser'}}]}}]})
    ...
    def test_get_media_insights_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': 'media_id', 'impressions': 10000, 'reach': 8000, 'likes': 1000, 'comments': 100, 'saves': 50, 'shares': 25, 'video_views': 5000, 'video_plays': 4500, 'profile_visits': 200, 'website_clicks': 50, 'email_contacts': 10, 'phone_call_clicks': 5, 'text_message_clicks': 3, 'get_directions_clicks': 2, 'follows': 15, 'unfollows': 5, 'engagement': 0.12, 'reach_rate': 0.08, 'impression_rate': 0.1}]})
...
class TestXContract:
    ...
    def test_get_user_by_username_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': {'id': '1234567890123456789', 'username': 'testuser', 'name': 'Test User', 'created_at': '2023-01-01T00:00:00.000Z', 'description': 'Test description', 'entities': {'description': {'urls': [{'start': 0, 'end': 23, 'url': 'https://example.com', 'expanded_url': 'https://example.com', 'display_url': 'example.com'}]}}, 'location': 'Test Location', 'pinned_tweet_id': '1234567890123456789', 'profile_image_url': 'https://example.com/profile.jpg', 'protected': False, 'public_metrics': {'followers_count': 10000, 'following_count': 1000, 'tweet_count': 100, 'listed_count': 50}, 'url': 'https://example.com', 'verified': True, 'verified_type': 'blue'}}, Mock())
    ...
    def test_get_user_tweets_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': '1234567890123456789', 'text': 'Test tweet', 'created_at': '2023-01-01T00:00:00.000Z', 'author_id': '1234567890123456789', 'conversation_id': '1234567890123456789', 'in_reply_to_user_id': None, 'referenced_tweets': [], 'attachments': {'media_keys': ['media_key'], 'poll_ids': [], 'urls': [{'start': 0, 'end': 23, 'url': 'https://example.com', 'expanded_url': 'https://example.com', 'display_url': 'example.com', 'images': [{'url': 'https://example.com/image.jpg', 'width': 1200, 'height': 630}], 'status': 200, 'title': 'Test Title', 'description': 'Test Description', 'unwound_url': 'https://example.com'}]}, 'entities': {'hashtags': [{'start': 0, 'end': 9, 'tag': 'test'}], 'mentions': [{'start': 0, 'end': 9, 'username': 'testuser'}], 'urls': [{'start': 0, 'end': 23, 'url': 'https://example.com', 'expanded_url': 'https://example.com', 'display_url': 'example.com'}]}, 'public_metrics': {'retweet_count': 10, 'reply_count': 5, 'like_count': 100, 'quote_count': 2}, 'context_annotations': [{'domain': {'id': 'domain_id', 'name': 'Test Domain', 'description': 'Test Domain Description'}, 'entity': {'id': 'entity_id', 'name': 'Test Entity', 'description': 'Test Entity Description'}}], 'lang': 'en', 'source': 'Twitter Web App', 'withheld': {'copyright': False, 'country_codes': []}}]}, Mock())
    ...
    def test_get_tweet_mentions_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': '1234567890123456789', 'text': '@testuser Test mention', 'created_at': '2023-01-01T00:00:00.000Z', 'author_id': '9876543210987654321', 'conversation_id': '1234567890123456789', 'in_reply_to_user_id': '1234567890123456789', 'referenced_tweets': [{'type': 'replied_to', 'id': '1234567890123456789'}], 'attachments': {'media_keys': [], 'poll_ids': [], 'urls': []}, 'entities': {'hashtags': [], 'mentions': [{'start': 0, 'end': 9, 'username': 'testuser'}], 'urls': []}, 'public_metrics': {'retweet_count': 5, 'reply_count': 2, 'like_count': 50, 'quote_count': 1}, 'context_annotations': [], 'lang': 'en', 'source': 'Twitter Web App', 'withheld': {'copyright': False, 'country_codes': []}}]}, Mock())
    ...
    def test_search_tweets_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'data': [{'id': '1234567890123456789', 'text': 'Test search result', 'created_at': '2023-01-01T00:00:00.000Z', 'author_id': '1234567890123456789', 'conversation_id': '1234567890123456789', 'in_reply_to_user_id': None, 'referenced_tweets': [], 'attachments': {'media_keys': [], 'poll_ids': [], 'urls': []}, 'entities': {'hashtags': [{'start': 0, 'end': 9, 'tag': 'test'}], 'mentions': [], 'urls': []}, 'public_metrics': {'retweet_count': 20, 'reply_count': 10, 'like_count': 200, 'quote_count': 5}, 'context_annotations': [], 'lang': 'en', 'source': 'Twitter Web App', 'withheld': {'copyright': False, 'country_codes': []}}]}, Mock())
    ...
    def test_upload_media_contract(self, client):
        ...
        with patch.object(client, '_make_api_request') as mock_request:
            mock_request.return_value = StepResult.ok({'media_id': '1234567890123456789', 'media_id_string': '1234567890123456789', 'size': 1024, 'expires_after_secs': 3600, 'processing_info': {'state': 'in_progress', 'check_after_secs': 30, 'progress_percent': 50}}, Mock())
