...
def test_transcript_chunking_with_topic_preservation():
    ...
    segments = [
        ...
        Segment(
            5.0,
            10.0,
            "Neural networks are a fundamental component of deep learning systems.",
        ),
        Segment(
            10.0,
            15.0,
            "Data preprocessing is crucial for training effective algorithms.",
        ),
        Segment(
            15.0,
            20.0,
...
def test_analysis_integration_with_different_content_types():
    ...
    social_media = next(r for r in results if r["type"] == "social_media")
    assert social_media["hashtag_count"] > 0  # Should detect hashtags

    academic = next(r for r in results if r["type"] == "academic")
    assert academic["keyword_count"] > 5  # Should extract many technical terms

    technical = next(r for r in results if r["type"] == "technical")
    assert technical["keyword_count"] > 0  # Should extract programming terms

    conversational = next(r for r in results if r["type"] == "conversational")
