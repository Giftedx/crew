...
from platform.core.secure_config import reload_config
...
from ultimate_discord_intelligence_bot.tools.yt_dlp_download_tool import InstagramDownloadTool, KickDownloadTool, RedditDownloadTool, TikTokDownloadTool, TwitchDownloadTool, TwitterDownloadTool, YouTubeDownloadTool

def test_process_video(monkeypatch):
    downloader = MagicMock()
    downloader.run.return_value = {'status': 'success', 'platform': 'Example', 'video_id': '1', 'title': 't', 'uploader': 'u', 'duration': '1', 'file_size': '1000', 'local_path': '/tmp/video.mp4'}
    drive = MagicMock()
    ...
    pipeline = ContentPipeline(webhook_url='http://example.com', downloader=downloader, transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory, transcript_cache=TranscriptCache(enabled=False))
...
def test_transcription_cache_hit(tmp_path):
    ...
    def _download_response():
        return {'status': 'success', 'platform': 'Example', 'video_id': 'video-1', 'title': 't', 'uploader': 'u', 'duration': '1', 'file_size': '1000', 'local_path': '/tmp/video.mp4'}
    downloader = MagicMock()
    downloader.run.side_effect = lambda *args, **kwargs: _download_response()
    drive = MagicMock()
    drive.run.return_value = {'status': 'success', 'links': {'preview_link': 'x'}}
    transcriber = MagicMock()
    transcriber.run.return_value = {'status': 'success', 'transcript': 'hello', 'segments': [{'start': 0.0, 'end': 1.0, 'text': 'hello'}]}
    analyzer = MagicMock()
    analyzer.run.return_value = {'status': 'success', 'sentiment': {}, 'keywords': []}
    fallacy = MagicMock()
    fallacy.run.return_value = {'status': 'success', 'fallacies': []}
    ...
    pipeline = ContentPipeline(webhook_url='http://example.com', downloader=downloader, transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory, transcript_cache=cache, pipeline_rate_limit=1000.0, tool_rate_limit=1000.0)
...
def test_drive_upload_retry(monkeypatch):
    ...
    downloader.run.return_value = {'status': 'success', 'platform': 'Example', 'video_id': '1', 'title': 't', 'uploader': 'u', 'duration': '1', 'file_size': '1000', 'local_path': '/tmp/video.mp4'}
    drive = MagicMock()
    drive.run.side_effect = [{'status': 'error', 'error': 'boom'}, {'status': 'success', 'links': {'preview_link': 'x'}}]
    transcriber = MagicMock()
    transcriber.run.return_value = {'status': 'success', 'transcript': 'hello'}
    analyzer = MagicMock()
    analyzer.run.return_value = {'status': 'success', 'sentiment': {}, 'keywords': []}
...
def test_unsupported_platform(monkeypatch):
    ...
    pipeline = ContentPipeline(webhook_url='http://example.com', drive=MagicMock(), transcriber=MagicMock(), analyzer=MagicMock(), discord=MagicMock(), fallacy_detector=MagicMock(), perspective=MagicMock(), memory=MagicMock(), transcript_cache=TranscriptCache(enabled=False))
    result = asyncio.run(pipeline.process_video('http://unknown.com'))
    assert result['status'] == 'error'
    assert result['step'] == 'download'

@pytest.mark.parametrize('url, tool_cls, platform', [('https://www.youtube.com/watch?v=1', YouTubeDownloadTool, 'YouTube'), ('https://www.twitch.tv/videos/1', TwitchDownloadTool, 'Twitch'), ('https://kick.com/video/1', KickDownloadTool, 'Kick'), ('https://twitter.com/video/1', TwitterDownloadTool, 'Twitter'), ('https://www.instagram.com/reel/1', InstagramDownloadTool, 'Instagram'), ('https://www.tiktok.com/@user/video/1', TikTokDownloadTool, 'TikTok'), ('https://vm.tiktok.com/ZM1/', TikTokDownloadTool, 'TikTok'), ('https://vt.tiktok.com/ZM2/', TikTokDownloadTool, 'TikTok'), ('https://www.reddit.com/r/videos/comments/1/demo/', RedditDownloadTool, 'Reddit'), ('https://cdn.discordapp.com/attachments/1/2/video.mp4', DiscordDownloadTool, 'Discord')])
def test_pipeline_selects_correct_downloader(url, tool_cls, platform, monkeypatch):
    ...
    def fake_run(self, video_url, quality='1080p'):
        return {'status': 'success', 'platform': platform, 'video_id': '1', 'title': 't', 'uploader': 'u', 'duration': '1', 'file_size': '1000', 'local_path': '/tmp/video.mp4', 'command': 'yt-dlp stub', 'requested_quality': quality}
    ...
    discord = MagicMock()
    discord.run.return_value = {'status': 'success'}
    pipeline = ContentPipeline(webhook_url='http://example.com', transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory, transcript_cache=TranscriptCache(enabled=False))
...
def test_transcript_memory_toggle_via_secure_config(monkeypatch):
    downloader = MagicMock()
    downloader.run.return_value = {'status': 'success', 'platform': 'Example', 'video_id': 'vid', 'title': 'title', 'uploader': 'u', 'duration': '1', 'file_size': '1000', 'local_path': '/tmp/video.mp4', 'command': 'yt-dlp stub', 'requested_quality': '1080p'}
    drive = MagicMock()
    drive.run.return_value = {'status': 'success', 'links': {'preview_link': 'x'}}
    transcriber = MagicMock()
    transcriber.run.return_value = {'status': 'success', 'transcript': 'hello'}
    ...
    try:
        ...
        pipeline_disabled = ContentPipeline(webhook_url='http://example.com', downloader=downloader, transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory_disabled, transcript_cache=TranscriptCache(enabled=False))
        ...
        pipeline_enabled = ContentPipeline(webhook_url='http://example.com', downloader=downloader, transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory_enabled, transcript_cache=TranscriptCache(enabled=False))
...
def test_process_video_handles_missing_download(monkeypatch):
    downloader = MagicMock()
    downloader.run.return_value = {'status': 'error', 'error': 'not found', 'platform': 'Example'}
    transcriber = MagicMock()
    analyzer = MagicMock()
    drive = MagicMock()
    discord = MagicMock()
