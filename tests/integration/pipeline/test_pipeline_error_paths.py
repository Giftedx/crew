...
class TestPipelineErrorPaths:
    ...
    def test_download_service_failure(self, mock_download_service: Mock, sample_url: str) -> None:
        ...
        mock_download_service.download_content.return_value = StepResult(success=False, error='Download failed: Network timeout', retryable=True, custom_status='retryable')
    ...
    def test_download_invalid_url(self, mock_download_service: Mock) -> None:
        ...
        mock_download_service.download_content.return_value = StepResult(success=False, error='Invalid URL format', custom_status='bad_request')
    ...
    def test_download_unsupported_platform(self, mock_download_service: Mock) -> None:
        ...
        mock_download_service.download_content.return_value = StepResult(success=False, error='Unsupported platform: unsupported-platform', custom_status='bad_request')
    ...
    def test_transcription_service_failure(self, mock_transcription_service: Mock) -> None:
        ...
        mock_transcription_service.transcribe.return_value = StepResult(success=False, error='Transcription failed: Audio format not supported', retryable=True, custom_status='retryable')
    ...
    def test_transcription_timeout(self, mock_transcription_service: Mock) -> None:
        ...
        mock_transcription_service.transcribe.return_value = StepResult(success=False, error='Transcription timeout: Audio too long', retryable=True, custom_status='retryable')
    ...
    def test_transcription_empty_audio(self, mock_transcription_service: Mock) -> None:
        ...
        empty_audio = b''
        mock_transcription_service.transcribe.return_value = StepResult(success=False, error='Empty audio content', custom_status='bad_request')
    ...
    def test_analysis_service_failure(self, mock_analysis_service: Mock) -> None:
        ...
        mock_analysis_service.analyze.return_value = StepResult(success=False, error='Analysis failed: LLM service unavailable', retryable=True, custom_status='retryable')
    ...
    def test_analysis_invalid_transcript(self, mock_analysis_service: Mock) -> None:
        ...
        invalid_transcript = ''
        mock_analysis_service.analyze.return_value = StepResult(success=False, error='Invalid transcript: Empty content', custom_status='bad_request')
    ...
    def test_analysis_rate_limit(self, mock_analysis_service: Mock) -> None:
        ...
        transcript = 'Sample transcript text'
        mock_analysis_service.analyze.return_value = StepResult(success=False, error='Rate limit exceeded', custom_status='rate_limited')
        result = mock_analysis_service.analyze(transcript)
        assert not result.success
    ...
    def test_memory_storage_failure(self, mock_memory_service: Mock) -> None:
        ...
        mock_memory_service.store_content.return_value = StepResult(success=False, error='Memory storage failed: Vector store unavailable', retryable=True, custom_status='retryable')
    ...
    def test_memory_tenant_isolation_failure(self, mock_memory_service: Mock) -> None:
        ...
        tenant = ''
        workspace = 'test_workspace'
        mock_memory_service.store_content.return_value = StepResult(success=False, error='Tenant isolation failed: Missing tenant identifier', custom_status='bad_request')
        result = mock_memory_service.store_content(content, tenant, workspace)
        assert not result.success
        assert result['status'] == 'bad_request'
    ...
    def test_memory_quota_exceeded(self, mock_memory_service: Mock) -> None:
        ...
        tenant = 'test_tenant'
        workspace = 'test_workspace'
        mock_memory_service.store_content.return_value = StepResult(success=False, error='Memory quota exceeded', custom_status='forbidden')
    ...
    async def test_pipeline_rollback_on_analysis_failure(self) -> None:
        ...
        mock_analysis.analyze.return_value = StepResult(success=False, error='Analysis failed', retryable=True, custom_status='retryable')
    ...
    async def test_pipeline_rollback_on_memory_failure(self) -> None:
        ...
        mock_memory.store_content.return_value = StepResult(success=False, error='Memory storage failed', retryable=True, custom_status='retryable')
        download_result = await mock_download.download_content('https://example.com')
        transcription_result = await mock_transcription.transcribe(download_result['audio'])
        analysis_result = await mock_analysis.analyze(transcription_result['transcript'])
        assert download_result.success
    ...
    async def test_pipeline_recovery_with_retry(self) -> None:
        ...
        mock_download = AsyncMock()
        mock_transcription = AsyncMock()
        mock_download.download_content.side_effect = [StepResult(success=False, error='Network timeout', retryable=True, custom_status='retryable'), StepResult.ok(audio='fake_audio')]
    ...
    def test_pipeline_error_aggregation(self) -> None:
        ...
        errors = [StepResult.fail(error='Download failed', status='retryable'), StepResult.fail(error='Transcription timeout', status='retryable'), StepResult.fail(error='Analysis rate limited', status='rate_limited')]
    ...
    async def test_full_pipeline_error_handling(self) -> None:
        ...
        mock_transcription.transcribe.return_value = StepResult(success=False, error='Transcription service unavailable', retryable=True, custom_status='retryable')
    ...
    def test_pipeline_error_logging(self) -> None:
        ...
        error = StepResult(success=False, error='Pipeline stage failed', retryable=True, custom_status='retryable', metadata={'stage': 'transcription', 'tenant': 'test_tenant'})
    ...
    def test_stepresult_error_consistency(self) -> None:
        ...
        errors = [StepResult.fail(error='Network error', status='retryable'), StepResult.fail(error='Invalid input', status='bad_request'), StepResult.fail(error='Rate limited', status='rate_limited'), StepResult.fail(error='Access denied', status='forbidden'), StepResult.fail(error='Service unavailable', status='retryable')]
        for error in errors:
    ...
    def test_stepresult_success_consistency(self) -> None:
        ...
        successes = [StepResult.ok(data={'audio': 'fake_audio'}), StepResult.ok(data={'transcript': 'sample text'}), StepResult.ok(data={'analysis': {'sentiment': 'positive'}}), StepResult.ok(data={'stored': True})]
