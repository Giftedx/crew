...
def test_pipeline_emits_step_upsert(monkeypatch):
    prov = types.SimpleNamespace(fetch_metadata=lambda url: types.SimpleNamespace(id='x', channel='c', published_at=None), fetch_transcript=lambda url: 'a\nb')
...
def test_pipeline_emits_failed_on_exception(monkeypatch):
    prov = types.SimpleNamespace(fetch_metadata=lambda url: types.SimpleNamespace(id=None, channel=None, published_at=None), fetch_transcript=lambda url: 'a\nb')
    monkeypatch.setattr(ip, '_get_provider', lambda src: (prov, 'channel'))
    failed = _CounterStub()
