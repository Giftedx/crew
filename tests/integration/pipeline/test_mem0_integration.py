...
class TestMem0MemoryService:
    ...
    def mock_memory(self):
        ...
        with patch('ultimate_discord_intelligence_bot.services.mem0_service.Memory') as mock:
            ...
            mock_instance.search.return_value = {'results': [{'id': 'mem_123', 'memory': 'test content', 'score': 0.95}]}
    ...
    def test_remember_success(self, mock_memory):
        ...
        result = service.remember(content='User prefers concise summaries', user_id='tenant1:workspace1', metadata={'category': 'formatting'})
    ...
    def test_update_memory_success(self, mock_memory):
        ...
        service = Mem0MemoryService()
        result = service.update_memory(memory_id='mem_123', content='Updated preference', user_id='tenant1:workspace1', metadata={'category': 'formatting'})
        assert result.success
...
class TestMem0MemoryTool:
    ...
    def test_remember_action(self, mock_service):
        ...
        tool = Mem0MemoryTool()
        result = tool._run(action='remember', content='User prefers summaries', tenant='tenant1', workspace='workspace1')
        assert result.success
    ...
    def test_recall_action(self, mock_service):
        ...
        tool = Mem0MemoryTool()
        result = tool._run(action='recall', query='What are user preferences?', tenant='tenant1', workspace='workspace1')
        assert result.success
    ...
    def test_update_action(self, mock_service):
        ...
        tool = Mem0MemoryTool()
        result = tool._run(action='update', memory_id='mem_123', content='Updated preference', tenant='tenant1', workspace='workspace1')
        assert result.success
