...
class TestTestEpisode:
    ...
    def test_episode_initialization(self):
        ...
        episode = TestEpisode(url='https://example.com/video', title='Test Episode', creator='Test Creator', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=['topic1', 'topic2'], expected_claims=['claim1', 'claim2'])
    ...
    def test_episode_to_dict(self):
        ...
        episode = TestEpisode(url='https://example.com/video', title='Test Episode', creator='Test Creator', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=['topic1'], expected_claims=['claim1'], ground_truth_transcript='Test transcript', ground_truth_diarization={'speaker1': [(0.0, 100.0)]})
...
class TestPipelineEvaluationHarness:
    ...
    def setup_method(self):
        ...
        self.harness = PipelineEvaluationHarness(pipeline=self.mock_pipeline, kg_store=self.mock_kg_store, tenant='test_tenant', workspace='test_workspace')
    ...
    def test_calculate_cost_estimate(self):
        ...
        stage_metrics = {'download': {'duration_seconds': 3600}, 'transcription': {'duration_seconds': 3600}, 'content_analysis': {'duration_seconds': 1800}}
        cost = self.harness.calculate_cost_estimate(stage_metrics)
        assert cost == 0.2

    def test_calculate_aggregate_metrics(self):
        """Test aggregate metrics calculation."""
        episode1 = TestEpisode(url='url1', title='title1', creator='creator1', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=[], expected_claims=[])
        episode2 = TestEpisode(url='url2', title='title2', creator='creator2', platform='youtube', duration_seconds=1800, expected_speakers=1, expected_topics=[], expected_claims=[])
    ...
    def test_generate_summary(self):
        ...
        episode = TestEpisode(url='url1', title='title1', creator='creator1', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=[], expected_claims=[])
        metrics = EvaluationMetrics()
    ...
    def test_evaluate_episode_success(self):
        ...
        mock_stage_results = {'download': {'success': True, 'data': {'url': 'test_url'}, 'start_time': 0, 'end_time': 10}, 'transcription': {'success': True, 'data': {'transcript': 'test transcript'}, 'start_time': 10, 'end_time': 20}, 'diarization': {'success': True, 'data': {'speakers': {'speaker1': [(0.0, 100.0)]}}, 'start_time': 20, 'end_time': 30}}
        mock_pipeline_result = PipelineResult(success=True, stages_completed=['download', 'transcription', 'diarization'], stages_failed=[], total_duration=30.0, intermediate_results={}, final_kg_nodes=[], final_kg_edges=[], errors=[], warnings=[], stage_results=mock_stage_results)
    ...
    def test_evaluate_episode_pipeline_failure(self):
        ...
        episode = TestEpisode(url='https://example.com/video', title='Test Episode', creator='Test Creator', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=['topic1'], expected_claims=['claim1'])
        metrics = asyncio.run(self.harness.evaluate_episode(episode))
        assert metrics.word_error_rate == 0.0
        assert metrics.diarization_error_rate == 0.0
        assert metrics.total_cost == 0.0
    ...
    def test_run_evaluation(self):
        ...
        from src.pipeline.multimodal_pipeline import PipelineResult
        mock_stage_results = {'download': {'success': True, 'data': {'url': 'test_url'}, 'start_time': 0, 'end_time': 10}, 'transcription': {'success': True, 'data': {'transcript': 'test transcript'}, 'start_time': 10, 'end_time': 20}}
        mock_pipeline_result = PipelineResult(success=True, stages_completed=['download', 'transcription'], stages_failed=[], total_duration=20.0, intermediate_results={}, final_kg_nodes=[], final_kg_edges=[], errors=[], warnings=[], stage_results=mock_stage_results)
        self.mock_pipeline.process_content = AsyncMock(return_value=StepResult.ok(data=mock_pipeline_result))
        episodes = [TestEpisode(url='https://example.com/video1', title='Test Episode 1', creator='Test Creator', platform='youtube', duration_seconds=1800, expected_speakers=1, expected_topics=['topic1'], expected_claims=['claim1']), TestEpisode(url='https://example.com/video2', title='Test Episode 2', creator='Test Creator', platform='youtube', duration_seconds=3600, expected_speakers=2, expected_topics=['topic2'], expected_claims=['claim2'])]
