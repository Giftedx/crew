...
class TestServiceIntegrationErrors:
    ...
    async def test_openrouter_api_rate_limit_exceeded(self, mock_openrouter_service, sample_tenant_context):
        ...
        with pytest.raises(Exception, match='Rate limit exceeded'):
            await mock_openrouter_service.generate_response(prompt='Test prompt', tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
    ...
    async def test_qdrant_connection_pool_exhaustion(self, mock_qdrant_client, sample_tenant_context):
        ...
        with pytest.raises(Exception, match='Connection pool exhausted'):
            await mock_qdrant_client.search(collection_name='test_collection', query_vector=[0.1] * 768, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
    ...
    async def test_qdrant_collection_not_found(self, mock_qdrant_client, sample_tenant_context):
        ...
        with pytest.raises(Exception, match='Collection not found'):
            await mock_qdrant_client.search(collection_name='nonexistent_collection', query_vector=[0.1] * 768, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
    ...
    async def test_qdrant_write_permission_error(self, mock_qdrant_client, sample_tenant_context):
        ...
        with pytest.raises(PermissionError, match='Write permission denied'):
            await mock_qdrant_client.upsert(collection_name='test_collection', points=[], tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
    ...
    async def test_openrouter_fallback_to_alternative_model(self, mock_openrouter_service, sample_tenant_context):
        ...
        result = await mock_openrouter_service.generate_response(prompt='Test prompt', tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
    ...
    async def test_qdrant_fallback_to_local_storage(self, mock_qdrant_client, sample_tenant_context):
        ...
        result = await mock_qdrant_client.search(collection_name='test_collection', query_vector=[0.1] * 768, tenant=sample_tenant_context['tenant'], workspace=sample_tenant_context['workspace'])
        assert result.success
        assert fallback_activated
    ...
    @pytest.mark.asyncio
    async def test_multi_service_cascading_failure(self, mock_openrouter_service, mock_qdrant_client, sample_tenant_context):
    ...
    @pytest.mark.asyncio
    async def test_multi_service_graceful_degradation(self, mock_openrouter_service, mock_qdrant_client, mock_redis_client, sample_tenant_context):
