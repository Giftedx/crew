...
class TestMultiPlatformContentProcessing:
    ...
    def test_urls(self):
        ...
        return {'youtube': 'https://youtube.com/watch?v=test123', 'twitch': 'https://twitch.tv/videos/test123', 'tiktok': 'https://tiktok.com/@user/video/test123', 'reddit': 'https://reddit.com/r/test/comments/test123', 'rumble': 'https://rumble.com/vtest123', 'bitchute': 'https://bitchute.com/video/test123', 'odysee': 'https://odysee.com/@user/test123', 'kick': 'https://kick.com/user/video/test123', 'x': 'https://x.com/user/status/test123'}
    ...
    async def test_youtube_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_youtube_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Video', 'duration': 300, 'quality': '720p'})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                ...
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'positive', 'topics': ['technology', 'AI'], 'debate_score': 0.7})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
    ...
    async def test_twitch_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_twitch_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Stream', 'duration': 1800, 'viewer_count': 1000})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test stream transcript', 'language': 'en', 'confidence': 0.9})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'neutral', 'topics': ['gaming', 'entertainment'], 'debate_score': 0.3})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_tiktok_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_tiktok_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test TikTok', 'duration': 60, 'hashtags': ['#test', '#viral']})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test TikTok transcript', 'language': 'en', 'confidence': 0.88})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'positive', 'topics': ['social', 'trending'], 'debate_score': 0.2})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_reddit_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_reddit_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Reddit Post', 'subreddit': 'test', 'upvotes': 100, 'comments': 50})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test Reddit post content', 'language': 'en', 'confidence': 0.92})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'mixed', 'topics': ['discussion', 'community'], 'debate_score': 0.8})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_rumble_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_rumble_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Rumble Video', 'duration': 600, 'views': 5000})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test Rumble video transcript', 'language': 'en', 'confidence': 0.89})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'neutral', 'topics': ['alternative', 'independent'], 'debate_score': 0.6})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_bitchute_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_bitchute_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test BitChute Video', 'duration': 900, 'views': 2000})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test BitChute video transcript', 'language': 'en', 'confidence': 0.87})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'neutral', 'topics': ['alternative', 'free_speech'], 'debate_score': 0.7})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_odysee_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_odysee_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Odysee Video', 'duration': 1200, 'views': 3000})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test Odysee video transcript', 'language': 'en', 'confidence': 0.91})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'positive', 'topics': ['decentralized', 'blockchain'], 'debate_score': 0.5})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_kick_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_kick_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test Kick Stream', 'duration': 2400, 'viewer_count': 500})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test Kick stream transcript', 'language': 'en', 'confidence': 0.86})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'neutral', 'topics': ['streaming', 'gaming'], 'debate_score': 0.4})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_x_content_processing(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_x_content') as mock_download:
            mock_download.return_value = StepResult.ok(data={'url': url, 'title': 'Test X Post', 'text': 'This is a test post', 'likes': 100, 'retweets': 50})
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                mock_transcribe.return_value = StepResult.ok(data={'transcript': 'This is a test X post content', 'language': 'en', 'confidence': 0.94})
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'positive', 'topics': ['social', 'microblogging'], 'debate_score': 0.3})
                    result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
        assert result.success
    ...
    async def test_cross_platform_content_processing(self, content_pipeline, test_tenant_context, test_urls):
        ...
        for platform, url in test_urls.items():
            with patch.object(content_pipeline, f'_download_{platform}_content') as mock_download:
                mock_download.return_value = StepResult.ok(data={'url': url, 'title': f'Test {platform.title()} Content', 'platform': platform})
                with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                    mock_transcribe.return_value = StepResult.ok(data={'transcript': f'This is a test {platform} transcript', 'language': 'en', 'confidence': 0.9})
                    with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                        mock_analyze.return_value = StepResult.ok(data={'sentiment': 'neutral', 'topics': [platform, 'content'], 'debate_score': 0.5})
                        result = await content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace)
    ...
    async def test_content_processing_with_concurrent_execution(self, content_pipeline, test_tenant_context):
        ...
        with patch.object(content_pipeline, '_download_youtube_content') as mock_download:
            ...
            with patch.object(content_pipeline, '_transcribe_audio') as mock_transcribe:
                ...
                with patch.object(content_pipeline, '_analyze_content') as mock_analyze:
                    mock_analyze.return_value = StepResult.ok(data={'sentiment': 'positive', 'topics': ['technology'], 'debate_score': 0.7})
                    tasks = [content_pipeline.process_content(url=url, tenant=test_tenant_context.tenant, workspace=test_tenant_context.workspace) for url in urls]
