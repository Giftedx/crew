...
from ai.response_models import ConfidenceLevel, ContentQuality, FallacyAnalysisResult, FallacyInstance, FallacyType, SeverityLevel
...
def mock_all_configs(mock_config):
    ...
    patches = [patch('ai.structured_outputs.get_config', return_value=mock_config), patch('ai.litellm_router.get_config', return_value=mock_config), patch('obs.logfire_config.get_config', return_value=mock_config), patch('obs.logfire_spans.get_config', return_value=mock_config)]
    for p in patches:
        p.start()
    yield mock_config
    for p in patches:
...
class TestInstructorIntegration:
    ...
    def test_structured_output_with_validation(self, mock_all_configs):
        ...
        result = FallacyAnalysisResult(fallacies=[FallacyInstance(fallacy_type=FallacyType.SLIPPERY_SLOPE, quote='If we allow X, then Y will inevitably happen', explanation='Assumes causal chain without evidence', severity=SeverityLevel.MEDIUM, confidence=ConfidenceLevel.HIGH)], overall_quality=ContentQuality.FAIR, credibility_score=0.65, summary='Analysis identifies logical fallacies with moderate confidence')
    ...
    def test_instructor_with_logfire_spans(self, mock_all_configs):
        ...
        with patch('obs.logfire_spans._logfire') as mock_logfire:
            ...
            with span('llm.structured_call'):
                result = FallacyAnalysisResult(fallacies=[], overall_quality=ContentQuality.GOOD, credibility_score=0.85, summary='Clean analysis with no fallacies detected')
