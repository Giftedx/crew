...
class TestRoutingQualityIntegration:
    ...
    async def test_model_selection_accuracy(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.return_value = {'status': 'success', 'response': 'test response', 'model': 'openai/gpt-3.5-turbo', 'task_type': 'analysis'}
    ...
    async def test_fallback_logic(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.side_effect = [Exception('Primary model unavailable'), {'status': 'success', 'response': 'fallback response', 'model': 'anthropic/claude-3-haiku'}]
    ...
    async def test_cost_optimization(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.return_value = {'status': 'success', 'response': 'simple response', 'model': 'openai/gpt-3.5-turbo', 'cost_optimized': True}
    ...
    async def test_latency_constraints(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.return_value = {'status': 'success', 'response': 'fast response', 'model': 'anthropic/claude-3-haiku', 'latency_ms': 150, 'latency_optimized': True}
    ...
    async def test_rl_routing_convergence(self, mock_openrouter_service):
        ...
        def mock_route_with_convergence(prompt, **kwargs):
            ...
            if call_count < 5:
                ...
                model = models[0]
            result = {'status': 'success', 'response': f'response from {model}', 'model': model, 'rl_converged': call_count >= 5}
    ...
    async def test_task_type_routing(self, mock_openrouter_service):
        ...
        task_type_model_mapping = {'analysis': 'openai/gpt-4', 'summarization': 'anthropic/claude-3-sonnet', 'translation': 'openai/gpt-3.5-turbo', 'coding': 'anthropic/claude-3-opus'}
    ...
    async def test_routing_with_context(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.return_value = {'status': 'success', 'response': 'contextual response', 'model': 'openai/gpt-4', 'context_used': True, 'context_length': 1500}
    ...
    async def test_routing_error_handling(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.side_effect = [Exception('Rate limit exceeded'), Exception('Model unavailable'), {'status': 'success', 'response': 'recovered response', 'model': 'fallback_model'}]
            result = mock_openrouter_service.route('Test prompt')
            assert result['status'] == 'success'
    ...
    async def test_routing_metrics_collection(self, mock_openrouter_service):
        ...
        with patch.object(mock_openrouter_service, 'route') as mock_route:
            mock_route.return_value = {'status': 'success', 'response': 'test response', 'model': 'openai/gpt-3.5-turbo', 'metrics': {'routing_time_ms': 45, 'model_selection_confidence': 0.95, 'fallback_used': False}}
            result = mock_openrouter_service.route('Test prompt')
            assert result['status'] == 'success'
