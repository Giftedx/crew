...
class TestDiscordBotWorkflow:
    ...
    def bot_tools(self):
        ...
        return {
            "post_tool": DiscordPostTool(),
            "monitor_tool": DiscordMonitorTool(),
    ...
    def sample_discord_message(self):
        ...
        return {
            "content": "This is a political statement about healthcare policy that needs analysis.",
            "author": "test_user",
    ...
    def test_discord_message_processing_workflow(self, bot_tools, sample_discord_message, sample_tenant_context):
        ...
        # Step 1: Monitor Discord message
        with patch.object(bot_tools["monitor_tool"], "_monitor_message") as mock_monitor:
            mock_monitor.return_value = {
                "success": True,
                "message_id": "msg_123",
                "channel_id": sample_discord_message["channel_id"],
                "guild_id": sample_discord_message["guild_id"],
                "author": sample_discord_message["author"],
                "content": sample_discord_message["content"],
                "timestamp": sample_discord_message["timestamp"],
                "mentions": [],
                "attachments": [],
            }

            monitor_result = bot_tools["monitor_tool"]._run(
                sample_discord_message["channel_id"],
        ...
        with patch.object(bot_tools["analysis_tool"], "_analyze_comprehensive") as mock_analyze:
            ...
            analysis_result = bot_tools["analysis_tool"]._run(
                monitor_result.data["content"],
                "comprehensive",
                sample_tenant_context["tenant"],
                sample_tenant_context["workspace"],
            )
        ...
        if analysis_result.data["extracted_claims"]:
            with patch.object(bot_tools["verification_tool"], "_verify_claim") as mock_verify:
                ...
                verification_result = bot_tools["verification_tool"]._run(
        ...
        # Step 4: Post analysis results back to Discord
        with patch.object(bot_tools["post_tool"], "_post_message") as mock_post:
            mock_post.return_value = {
                "success": True,
                "message_id": "response_123",
                "channel_id": sample_discord_message["channel_id"],
                "content": "Analysis complete: Healthcare policy topic detected with neutral sentiment.",
...
"""
            ...
            post_result = bot_tools["post_tool"]._run(
                sample_discord_message["channel_id"],
                response_content,
                sample_tenant_context["tenant"],
                sample_tenant_context["workspace"],
    ...
    def test_discord_monitoring_workflow(self, bot_tools, sample_tenant_context):
        ...
        with patch.object(bot_tools["monitor_tool"], "_monitor_channel") as mock_monitor:
            ...
            monitor_result = bot_tools["monitor_tool"]._run(
                "123456789", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
            )
    ...
    def test_discord_analysis_workflow(self, bot_tools, sample_tenant_context):
        ...
        sample_messages = [
            "This is a political statement about healthcare.",
        ...
        with patch.object(bot_tools["analysis_tool"], "_analyze_comprehensive") as mock_analyze:
            mock_analyze.return_value = {
                "political_topics": ["healthcare", "technology", "climate"],
                "sentiment": "neutral",
                "sentiment_confidence": 0.8,
                "extracted_claims": ["Healthcare is important", "Technology advances", "Climate change is serious"],
                "processing_time": 2.0,
            }
            ...
            for message in sample_messages:
                result = bot_tools["analysis_tool"]._run(
                    message, "comprehensive", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
                )
    ...
    def test_discord_verification_workflow(self, bot_tools, sample_tenant_context):
        ...
        with patch.object(bot_tools["verification_tool"], "_verify_claim") as mock_verify:
            ...
            for claim in sample_claims:
                result = bot_tools["verification_tool"]._run(
                    claim, "Discord context", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
                )
                results.append(result)
    ...
    def test_discord_posting_workflow(self, bot_tools, sample_tenant_context):
        ...
        sample_messages = [
            "Analysis complete: Healthcare topic detected.",
            "Verification results: Claim verified with 90% confidence.",
            "Summary: 3 political topics identified.",
        ]
        ...
        with patch.object(bot_tools["post_tool"], "_post_message") as mock_post:
            ...
            results = []
            for message in sample_messages:
                result = bot_tools["post_tool"]._run(
                    "123456789", message, sample_tenant_context["tenant"], sample_tenant_context["workspace"]
                )
                results.append(result)
    ...
    def test_discord_workflow_error_handling(self, bot_tools, sample_tenant_context):
        ...
        with patch.object(bot_tools["monitor_tool"], "_monitor_channel") as mock_monitor:
            ...
            result = bot_tools["monitor_tool"]._run(
                "123456789", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
            )
        ...
        with patch.object(bot_tools["post_tool"], "_post_message") as mock_post:
            ...
            result = bot_tools["post_tool"]._run(
                "123456789", "Test message", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
            )
    ...
    def test_discord_concurrent_processing(self, bot_tools, sample_tenant_context):
        ...
        with patch.object(bot_tools["monitor_tool"], "_monitor_channel") as mock_monitor:
            ...
            # Monitor multiple channels concurrently
            results = []
            for channel in channels:
                result = bot_tools["monitor_tool"]._run(
                    channel, sample_tenant_context["tenant"], sample_tenant_context["workspace"]
                )
                results.append(result)
        ...
        with patch.object(bot_tools["monitor_tool"], "_monitor_channel") as mock_monitor:
            mock_monitor.return_value = {
                ...
                "messages": [
                    {
                        "message_id": "msg_1",
                        "content": "This is a political statement about healthcare.",
                        "author": "user1",
                        "timestamp": "2024-01-01T00:00:00Z",
                        "mentions": [],
                        "attachments": [],
                ...
                "total_messages": 2,
                "filtered_messages": 1,  # Only political content
            }
    ...
    def test_discord_workflow_error_recovery(self, bot_tools, sample_tenant_context):
        ...
        with patch.object(bot_tools["monitor_tool"], "_monitor_channel") as mock_monitor:
            ...
            mock_monitor.side_effect = [
                ...
                {"success": True, "channel_id": "123456789", "messages": [], "total_messages": 0},
            ]

            # First attempt should fail
            result1 = bot_tools["monitor_tool"]._run(
                "123456789", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
            )
            assert not result1.success

            # Second attempt should succeed
            result2 = bot_tools["monitor_tool"]._run(
                "123456789", sample_tenant_context["tenant"], sample_tenant_context["workspace"]
            )
            assert result2.success
