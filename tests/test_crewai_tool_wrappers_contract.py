...
class _DummyTool:
    def run(self, **_kwargs):  # pragma: no cover - simple shim
        raise TypeError("missing required arg: text")


class _DummyDiscord:
    def _run(self, *_args, **_kwargs):  # pragma: no cover - simple shim
        return {"posted": True}
...
def test_crewai_tool_wrapper_returns_step_result_on_type_error() -> None:
    ...
    res = wrapper._run()  # missing args triggers TypeError
