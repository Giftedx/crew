...
class TestMemoryV2Tool:
    ...
    @patch(
        "ultimate_discord_intelligence_bot.tools.memory_v2_tool.get_memory_namespace"
    )
    def test_upsert_operation_success(self, mock_namespace, mock_memory_factory):
        """Test successful upsert operation."""
        # Setup mocks
        mock_memory = AsyncMock()
        mock_memory.upsert.return_value = StepResult.ok(
            upserted=1, namespace="test:main:default"
        )
