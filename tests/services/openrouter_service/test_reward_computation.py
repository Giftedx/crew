...
def test_reward_includes_quality_when_weighted(monkeypatch):
    ...
    svc.tenant_registry = _FakeTenantRegistry(overrides={'reward_cost_weight': 0.0, 'reward_latency_weight': 0.0, 'reward_quality_weight': 1.0, 'reward_latency_ms_window': 2000})
    state = prepare_route_state(svc, prompt='hello world', task_type='general', model='openai/gpt-4o-mini', provider_opts=None)
