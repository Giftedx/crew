...
def test_openrouter_metrics_use_effective_labels_non_strict(monkeypatch):
    ...
    assert re.search(
        r'llm_estimated_cost_usd(_bucket|_sum|_count)?\{[^}]*tenant="default"[^}]*workspace="main"', data
    ), data

    # llm_latency_ms histogram labels should also carry effective tenant/workspace
    assert re.search(r'llm_latency_ms(_bucket|_sum|_count)?\{[^}]*tenant="default"[^}]*workspace="main"', data), data
