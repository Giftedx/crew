...
from ultimate_discord_intelligence_bot.core.cache.adaptive_semantic_cache import (
...
class TestAdaptiveSemanticCache:
    ...
    def test_initialization(self, mock_create_cache):
        ...
        assert cache.current_threshold == 0.75  # Default initial threshold
        assert cache.min_threshold == 0.60
    ...
    def test_custom_initialization(self, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache(
            initial_threshold=0.85, min_threshold=0.70, max_threshold=0.98, adjustment_step=0.02, evaluation_window=50
        )

        assert cache.current_threshold == 0.85
        assert cache.min_threshold == 0.70
    ...
    def test_cache_set(self, mock_create_cache):
        ...
        stored_response = args[2]  # Third argument is the response
    ...
    def test_should_adjust_threshold_low_hit_rate(self, mock_time, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache()
        cache.last_adjustment = 500  # 500 seconds ago (past cooldown)

        # Simulate low hit rate with high similarity
        cache.metrics.total_requests = 100
        cache.metrics.cache_hits = 15  # 15% hit rate
        cache.metrics.similarity_scores = [0.85] * 15  # High similarity
    ...
    def test_should_adjust_threshold_high_hit_rate_low_similarity(self, mock_time, mock_create_cache):
        ...
        # Simulate high hit rate with low similarity
        cache.metrics.total_requests = 100
        cache.metrics.cache_hits = 75  # 75% hit rate
        cache.metrics.similarity_scores = [0.65] * 75  # Low similarity
    ...
    def test_should_not_adjust_threshold_cooldown(self, mock_time, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache()
        cache.last_adjustment = 800  # 200 seconds ago (within cooldown)
    ...
    def test_should_not_adjust_threshold_insufficient_data(self, mock_create_cache):
        ...
        cache.metrics.total_requests = 50  # Less than evaluation window
    ...
    def test_adjust_threshold_lower(self, mock_time, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache()
        cache.metrics.total_requests = 100
        cache.metrics.cache_hits = 15  # Low hit rate
        cache.metrics.similarity_scores = [0.85] * 15  # High similarity

        cache._adjust_threshold()

        assert cache.current_threshold == 0.70  # Lowered by 0.05
        assert cache.metrics.total_requests == 0  # Reset for next window
    ...
    def test_adjust_threshold_raise(self, mock_time, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache()
        cache.metrics.total_requests = 100
        cache.metrics.cache_hits = 75  # High hit rate
        cache.metrics.similarity_scores = [0.65] * 75  # Low similarity

        cache._adjust_threshold()

        assert cache.current_threshold == 0.80  # Raised by 0.05
        assert cache.metrics.total_requests == 0  # Reset for next window
    ...
    def test_adjust_threshold_bounds(self, mock_create_cache):
        ...
        cache._adjust_threshold()
        assert cache.current_threshold == 0.60  # Should not go below min
        ...
        cache._adjust_threshold()
        assert cache.current_threshold == 0.95  # Should not go above max
    ...
    def test_force_threshold_adjustment(self, mock_create_cache):
        ...
        cache.force_threshold_adjustment(0.50)  # Below min
        assert cache.current_threshold == 0.60  # Should be clamped to min

        cache.force_threshold_adjustment(0.99)  # Above max
        assert cache.current_threshold == 0.95  # Should be clamped to max
...
class TestAdaptiveSemanticCacheIntegration:
    ...
    def test_adaptive_behavior_simulation(self, mock_create_cache):
        ...
        cache = AdaptiveSemanticCache(evaluation_window=10)  # Small window for testing

        # Simulate cache misses (should lower threshold)
        for i in range(10):
            cache.get(f"prompt_{i}", "model")

        # Should have 0% hit rate, trigger threshold lowering
        assert cache.metrics.hit_rate == 0.0

        # Simulate cache hits with high similarity (should raise threshold)
        ...
        # Should have 100% hit rate with low similarity, trigger threshold raising
...
class TestFactoryFunctions:
    ...
    def test_create_adaptive_semantic_cache(self):
        ...
        with patch("core.cache.adaptive_semantic_cache.create_semantic_cache") as mock_create:
            ...
            cache = create_adaptive_semantic_cache(initial_threshold=0.85, min_threshold=0.70, max_threshold=0.98)
