...
class TestClaimExtractorTool:
    ...
    def test_run_with_valid_text(self, tool):
        ...
        with patch.object(tool, '_extract_claims') as mock_extract:
            mock_extract.return_value = StepResult.ok(data={'claims': [{'text': 'The Earth is round', 'confidence': 0.95, 'type': 'factual'}], 'total_claims': 1})
        ...
        with patch.object(tool, '_identify_claim_patterns') as mock_patterns:
            with patch.object(tool, '_classify_claim_type') as mock_classify:
                with patch.object(tool, '_calculate_confidence') as mock_confidence:
                    mock_patterns.return_value = ['The study shows that 90% of participants improved their performance.']
    ...
    def test_filter_duplicate_claims(self, tool):
        ...
        claims = [{'text': 'The Earth is round', 'confidence': 0.9}, {'text': 'The Earth is round', 'confidence': 0.8}, {'text': 'The Sun is hot', 'confidence': 0.95}]
    ...
    def test_rank_claims_by_confidence(self, tool):
        ...
        claims = [{'text': 'Claim A', 'confidence': 0.7}, {'text': 'Claim B', 'confidence': 0.9}, {'text': 'Claim C', 'confidence': 0.6}]
