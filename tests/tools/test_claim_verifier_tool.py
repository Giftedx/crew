...
import pytest
from domains.intelligence.verification.claim_verifier_tool import ClaimVerification, ClaimVerifierTool, VerificationSource
...
class TestClaimVerifierTool:
    ...
    def mock_verification_result(self):
        ...
        return ClaimVerification(claim_id='test_claim_123', claim_text='The Earth is round and orbits around the Sun.', overall_confidence=0.95, verification_status='verified', sources=[VerificationSource(source_id='source_1', title='NASA - Earth Facts', url='https://nasa.gov/earth-facts', snippet='Earth is a spherical planet that orbits the Sun...', relevance_score=0.98, backend='serply', timestamp='2024-01-01T00:00:00Z', domain='nasa.gov')], processing_time=1.5, backends_used=['serply', 'exa'], error_message=None)
    ...
    def test_confidence_threshold_validation(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.5, verification_status='unverified', sources=[], processing_time=1.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_source_validation(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.9, verification_status='verified', sources=[VerificationSource(source_id='source_1', title='Reliable Source', url='https://reliable.com/article', snippet='Reliable information about the claim...', relevance_score=0.95, backend='serply', timestamp='2024-01-01T00:00:00Z', domain='reliable.com'), VerificationSource(source_id='source_2', title='Less Reliable Source', url='https://unreliable.com/article', snippet='Less reliable information...', relevance_score=0.3, backend='exa', timestamp='2024-01-01T00:00:00Z', domain='unreliable.com')], processing_time=2.0, backends_used=['serply', 'exa'], error_message=None)
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_backend_failure_handling(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.8, verification_status='partially_verified', sources=[], processing_time=1.0, backends_used=['serply'], error_message="Backend 'exa' failed: Connection timeout")
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_tenant_workspace_isolation(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.9, verification_status='verified', sources=[], processing_time=1.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
    ...
    def test_claim_text_preprocessing(self, tool, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text='  Processed claim text  ', overall_confidence=0.9, verification_status='verified', sources=[], processing_time=1.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
    ...
    def test_verification_status_mapping(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.95, verification_status='verified', sources=[], processing_time=1.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_confidence_score_validation(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=1.5, verification_status='verified', sources=[], processing_time=1.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_source_limit_handling(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            sources = []
            for i in range(10):
                sources.append(VerificationSource(source_id=f'source_{i}', title=f'Source {i}', url=f'https://example{i}.com', snippet=f'Snippet {i}', relevance_score=0.9 - i * 0.1, backend='serply', timestamp='2024-01-01T00:00:00Z', domain=f'example{i}.com'))
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.9, verification_status='verified', sources=sources, processing_time=2.0, backends_used=['serply'], error_message=None)
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
    ...
    def test_error_message_handling(self, tool, sample_claim, sample_context):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verification_result = ClaimVerification(claim_id='test_claim_123', claim_text=sample_claim, overall_confidence=0.0, verification_status='error', sources=[], processing_time=0.5, backends_used=[], error_message='All backends failed to respond')
            mock_verify.return_value = mock_verification_result
            result = tool._run(sample_claim, sample_context, 'test_tenant', 'test_workspace')
            assert result.success
