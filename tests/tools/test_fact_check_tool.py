...
class TestFactCheckTool:
    ...
    def test_run_with_valid_claim(self, tool):
        ...
        with patch.object(tool, '_verify_claim') as mock_verify:
            mock_verify.return_value = StepResult.ok(data={'claim': 'The Earth is round', 'verification_status': 'verified', 'confidence_score': 0.95, 'sources': ['NASA', 'Scientific consensus']})
