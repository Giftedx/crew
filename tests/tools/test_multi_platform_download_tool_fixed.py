...
class TestMultiPlatformDownloadTool:
    ...
    def test_successful_youtube_download(self, tool):
        ...
        with patch.object(tool, '_run') as mock_run:
            mock_run.return_value = StepResult.ok(data={'file_path': '/tmp/video.mp4', 'platform': 'youtube', 'title': 'Test Video'})
    ...
    def test_successful_twitter_download(self, tool):
        ...
        with patch.object(tool, '_run') as mock_run:
            mock_run.return_value = StepResult.ok(data={'file_path': '/tmp/tweet.mp4', 'platform': 'twitter', 'title': 'Test Tweet'})
    ...
    def test_download_result_structure(self, tool):
        ...
        youtube_url = 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'
        with patch.object(tool, '_run') as mock_run:
            mock_run.return_value = StepResult.ok(data={'file_path': '/tmp/video.mp4', 'platform': 'youtube', 'title': 'Test Video', 'duration': 180, 'thumbnail': 'https://example.com/thumb.jpg'})
            result = tool._run(url=youtube_url, quality='720p')
            assert isinstance(result, StepResult)
            assert result.success
    ...
    def test_platform_specific_handling(self, tool):
        ...
        platforms = [('https://www.youtube.com/watch?v=dQw4w9WgXcQ', 'youtube'), ('https://twitter.com/user/status/123456789', 'twitter'), ('https://www.instagram.com/p/ABC123/', 'instagram'), ('https://www.tiktok.com/@user/video/123456789', 'tiktok')]
        for url, expected_platform in platforms:
            with patch.object(tool, '_run') as mock_run:
                mock_run.return_value = StepResult.ok(data={'file_path': f'/tmp/{expected_platform}_video.mp4', 'platform': expected_platform})
