...
from analysis.transcribe import run_whisper, Transcript, Segment
from analysis.segmenter import chunk_transcript

# Mock heavy dependencies
from memory import vector_store, api
...
def test_full_analysis_to_memory_pipeline(sample_audio_file, memory_stores, mock_embedding_function):
    ...
    with patch('memory.embeddings.embed', mock_embedding_function):
        ...
        tech_keywords = [k for k in all_keywords if k.lower() in ['artificial', 'intelligence', 'machine', 'learning', 'neural', 'algorithms']]
        ...
        ai_related = [hit for hit in hits if any(word in hit.text.lower() for word in ['artificial', 'intelligence', 'machine', 'learning'])]
...
def test_topics_integration_with_memory_service():
    ...
    topic_results = memory_service.retrieve(
        "algorithms",
        metadata={'source': 'analysis_pipeline'}
    )
...
def test_transcript_chunking_with_topic_preservation():
    ...
    segments = [
        Segment(0.0, 5.0, "Welcome to our discussion about artificial intelligence and machine learning."),
        ...
        Segment(15.0, 20.0, "Computer vision applications use convolutional neural networks extensively."),
    ]
    ...
    tech_terms = ['artificial', 'intelligence', 'machine', 'learning', 'neural', 'networks', 'data', 'algorithms']
...
def test_multi_modal_content_analysis():
    ...
    tech_keywords = [k for k in topics_result.keywords if k.lower() in [
        'neural', 'networks', 'deep', 'learning', 'transformer', 'tensorflow', 'pytorch', 'python'
    ]]
    assert len(tech_keywords) >= 3
    
    # Should extract entities
    tech_entities = [e for e in topics_result.entities if any(
        term in e.lower() for term in ['github', 'tensorflow', 'pytorch', 'python']
    )]
...
def test_content_pipeline_metadata_enrichment():
    ...
    tech_keywords = [k for k in unique_keywords if k.lower() in [
        'artificial', 'intelligence', 'machine', 'learning', 'neural', 'network', 
        'data', 'accuracy', 'classification', 'preprocessing', 'transformer', 'language'
    ]]
...
def test_analysis_integration_with_different_content_types():
    ...
    content_types = [
        ...
        {
            'type': 'academic',
            'content': 'The convolutional neural network architecture demonstrated superior performance on image classification tasks with 97.3% accuracy.',
            'expected_hashtags': 0,
            'expected_keywords': 8
        },
        {
            'type': 'technical',
            'content': 'import tensorflow as tf; model = tf.keras.Sequential([tf.keras.layers.Dense(128, activation="relu")])',
            'expected_hashtags': 0,
            'expected_keywords': 5
        },
        {
    ...
    all_successful = all(r['meets_hashtag_expectation'] and r['meets_keyword_expectation'] for r in results)
