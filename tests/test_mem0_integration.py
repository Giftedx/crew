...
class TestMem0MemoryService:
    ...
    def mock_memory(self):
        ...
        with patch(
            "ultimate_discord_intelligence_bot.services.mem0_service.Memory"
        ) as mock:
            mock_instance = mock.return_value
            # Mock successful responses
            mock_instance.add.return_value = [
                {"id": "mem_123", "memory": "test content"}
            ]
    ...
    def test_recall_success(self, mock_memory):
        ...
        result = service.recall(
            query="How should I format output?", user_id="tenant1:workspace1"
        )
    ...
    def test_delete_memory_success(self, mock_memory):
        ...
        result = service.delete_memory(
            memory_id="mem_123", user_id="tenant1:workspace1"
        )
    ...
    def test_get_memory_history_success(self, mock_memory):
        ...
        service = Mem0MemoryService()
        result = service.get_memory_history(
            memory_id="mem_123", user_id="tenant1:workspace1"
        )
...
class TestMem0MemoryTool:
    ...
    def mock_service(self):
        ...
        with patch(
            "ultimate_discord_intelligence_bot.tools.mem0_memory_tool.Mem0MemoryService"
        ) as mock:
            mock_instance = mock.return_value
            mock_instance.remember.return_value = StepResult.ok(
                data=[{"id": "mem_123", "memory": "test"}]
            )
            mock_instance.recall.return_value = StepResult.ok(
                data={"results": [{"memory": "test", "score": 0.95}]}
            )
            mock_instance.update_memory.return_value = StepResult.ok(
                data={"updated": True}
            )
            mock_instance.delete_memory.return_value = StepResult.ok(
                data={"deleted": "mem_123"}
            )
            mock_instance.get_all_memories.return_value = StepResult.ok(
                data={"results": []}
            )
    ...
    def test_tenant_isolation(self, mock_service):
        ...
        tool._run(
            ...
            content="test",
            tenant="tenant_a",
            workspace="workspace_x",
        )

        # Verify user_id was constructed correctly
        call_args = mock_service.remember.call_args
        assert call_args[0][1] == "tenant_a:workspace_x"  # user_id argument
    ...
    def test_validation_update_missing_fields(self):
        ...
        # Missing memory_id
        result1 = tool._run(
            action="update", content="test", tenant="tenant1", workspace="workspace1"
        )
