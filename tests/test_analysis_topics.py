...
import pytest
from analysis.topics import extract, TopicResult
...
def test_extract_keywords():
    ...
    meaningful_words = ["artificial", "intelligence", "system", "processes", "data", "efficiently", "machine", "learning", "algorithms"]
...
def test_extract_topics():
    ...
    tech_terms_found = any(
        ...
        for term in ["artificial", "intelligence", "machine", "learning", "python", "data", "cloud", "aws"]
    )
...
def test_long_text():
    ...
    tech_terms = ["artificial", "intelligence", "machine", "learning", "algorithms", "neural", "networks", "deep"]
...
def test_social_media_content():
    ...
    expected_hashtags = ["#ai", "#machinelearning", "#technews", "#innovation"]  # lowercase
...
def test_technical_content():
    ...
    tech_terms = ["API", "OAuth", "JWT", "PostgreSQL", "Redis", "React", "TypeScript", "Docker", "Kubernetes"]
    found_terms = [
        term for term in tech_terms 
        if any(term.lower() in item.lower() for item in all_extracted)
    ]
...
def test_topic_categories():
    ...
    all_extracted = result.keywords + result.topics + result.entities
    tech_terms = ["python", "django", "data", "science", "pandas", "numpy", "machine", "learning", "tensorflow"]
    found_terms = [
        term for term in tech_terms
        if any(term.lower() in item.lower() for item in all_extracted)
    ]
