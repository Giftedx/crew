...
def test_reward_aware_picks_cheaper_and_faster(monkeypatch):
    ...
    svc = OpenRouterService(models_map={'general': ['provider/expensive_slow', 'provider/cheap_fast']}, api_key=None, token_meter=tm)
...
def test_budget_pushes_to_affordable(monkeypatch):
    ...
    svc = OpenRouterService(models_map={'general': ['provider/fast_costly', 'provider/slow_cheap']}, api_key=None, token_meter=tm)
