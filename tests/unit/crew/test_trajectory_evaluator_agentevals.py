...
def test_agentevals_flag_skip(monkeypatch):
    ...
    traj = AgentTrajectory(session_id='s1', user_input='hello', steps=[TrajectoryStep(0.0, 'agent', 'response', 'hi')], final_output='done', total_duration=0.1, success=True)
...
def test_agentevals_messages_adapter(monkeypatch):
    ...
    ev = TrajectoryEvaluator()
    traj = AgentTrajectory(session_id='s2', user_input='weather in SF?', steps=[TrajectoryStep(0.0, 'agent', 'tool_call', '', tool_name='get_weather', tool_args={'city': 'SF'}), TrajectoryStep(0.1, 'agent', 'response', "It's 80 degrees and sunny.")], final_output='The weather in SF is 80 degrees and sunny.', total_duration=0.2, success=True)
    msgs = ev._to_agentevals_messages(traj)
