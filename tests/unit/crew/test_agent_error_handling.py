...
class TestAgentErrorHandling:
    ...
    def sample_agent_config(self) -> dict[str, Any]:
        ...
        return {'role': 'Content Analyzer', 'goal': 'Analyze content for debate quality', 'backstory': 'Expert in content analysis', 'tools': ['content_analysis_tool', 'fact_checking_tool'], 'verbose': True, 'allow_delegation': False}

    @pytest.fixture
    def sample_task_config(self) -> dict[str, Any]:
        """Sample task configuration for testing."""
        return {'description': 'Analyze the provided content', 'expected_output': 'Detailed analysis report', 'agent': 'content_analyzer', 'tools': ['content_analysis_tool']}
    ...
    async def test_task_execution_failure(self, mock_agent: Mock, mock_task: Mock) -> None:
        ...
        mock_task.execute.return_value = StepResult(success=False, error='Task execution failed: Timeout', retryable=True, custom_status='retryable')
    ...
    async def test_task_execution_invalid_input(self, mock_task: Mock) -> None:
        ...
        invalid_input = {'url': ''}
        mock_task.execute.return_value = StepResult(success=False, error='Invalid input: Empty URL provided', custom_status='bad_request')
    ...
    async def test_task_execution_memory_error(self, mock_task: Mock) -> None:
        ...
        mock_task.execute.return_value = StepResult(success=False, error='Memory allocation failed', retryable=True, custom_status='retryable')
        result = await mock_task.execute()
        assert not result.success
        assert result['status'] == 'retryable'
    ...
    def test_tool_execution_failure(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.return_value = StepResult(success=False, error='Tool execution failed: External service unavailable', retryable=True, custom_status='retryable')
    ...
    def test_tool_invalid_parameters(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.return_value = StepResult(success=False, error='Invalid parameters: Missing required argument', custom_status='bad_request')
    ...
    def test_tool_rate_limit_exceeded(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.return_value = StepResult(success=False, error='Rate limit exceeded', custom_status='rate_limited')
        result = mock_tool._run('input_data', 'tenant', 'workspace')
        assert not result.success
    ...
    def test_tool_authentication_failure(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.return_value = StepResult(success=False, error='Authentication failed: Invalid API key', custom_status='unauthorized')
        result = mock_tool._run('input_data', 'tenant', 'workspace')
        assert not result.success
    ...
    async def test_agent_retry_mechanism(self, mock_agent: Mock) -> None:
        ...
        mock_agent.execute_task.side_effect = [StepResult(success=False, error='Transient failure', retryable=True, custom_status='retryable'), StepResult.ok(result='success')]
    ...
    async def test_tool_retry_mechanism(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.side_effect = [StepResult(success=False, error='Network timeout', retryable=True, custom_status='retryable'), StepResult(success=False, error='Service busy', retryable=True, custom_status='retryable'), StepResult.ok(processed=True)]
    ...
    async def test_retry_max_attempts_exceeded(self, mock_tool: Mock) -> None:
        ...
        mock_tool._run.return_value = StepResult(success=False, error='Persistent failure', retryable=True, custom_status='retryable')
        max_attempts = 3
    ...
    async def test_agent_graceful_degradation(self, mock_agent: Mock) -> None:
        ...
        mock_tool_primary._run.return_value = StepResult(success=False, error='Primary tool unavailable', retryable=True, custom_status='retryable')
    ...
    async def test_agent_delegation_failure(self, mock_agent: Mock) -> None:
        ...
        mock_agent.delegate_task.return_value = StepResult(success=False, error='Delegation failed: Target agent unavailable', retryable=True, custom_status='retryable')
    ...
    async def test_agent_state_recovery(self, mock_agent: Mock) -> None:
        ...
        mock_agent.execute_task.return_value = StepResult(success=False, error='Task failed', retryable=True, custom_status='retryable')
    ...
    def test_agent_error_context_preservation(self, mock_agent: Mock) -> None:
        ...
        error_context = {'agent_id': 'content_analyzer', 'task_id': 'analyze_content_123', 'timestamp': '2025-01-04T10:00:00Z', 'tenant': 'test_tenant'}
    ...
    async def test_agent_resource_exhaustion(self, mock_agent: Mock) -> None:
        ...
        mock_agent.execute_task.return_value = StepResult(success=False, error='Resource exhaustion: Memory limit reached', retryable=True, custom_status='retryable')
    ...
    async def test_agent_timeout_handling(self, mock_agent: Mock) -> None:
        ...
        mock_agent.execute_task.return_value = StepResult(success=False, error='Task timeout: Execution exceeded time limit', retryable=True, custom_status='retryable')
    ...
    def test_agent_error_aggregation(self) -> None:
        ...
        agent_errors = [StepResult(success=False, error='Agent 1 failed', retryable=True, custom_status='retryable'), StepResult(success=False, error='Agent 2 failed', custom_status='bad_request'), StepResult(success=False, error='Agent 3 failed', custom_status='rate_limited')]
        error_summary = {'total_failures': len(agent_errors), 'error_types': list({error['status'] for error in agent_errors}), 'error_messages': [error.error for error in agent_errors]}
