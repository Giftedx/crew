...
class _FakeAgentEvals:
    ...
    def __call__(self, data):
        return {'score': True, 'reasoning': 'ok', 'accuracy_score': 0.88, 'efficiency_score': 0.77, 'error_handling_score': 0.66}
...
def test_agent_evals_adapter_happy_path(monkeypatch: pytest.MonkeyPatch):
    ...
    traj = AgentTrajectory(session_id='s1', user_input='do it', steps=[TrajectoryStep(timestamp=0.0, agent_role='a1', action_type='tool_call', content='', tool_name='t1')], final_output='done', total_duration=1.0, success=True)
...
def test_agent_evals_fallback_on_error(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr('src.eval.trajectory_evaluator._AGENTEVALS_AVAILABLE', True, raising=False)
    ev = TrajectoryEvaluator(router=None, learning_engine=None)
    traj = AgentTrajectory(session_id='s2', user_input='do it', steps=[], final_output='ok', total_duration=0.1, success=True)
    res = ev.evaluate_trajectory_accuracy(traj)
    assert res.success is True
