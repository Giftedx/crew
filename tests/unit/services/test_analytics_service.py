...
class TestAnalyticsRecommendations:
    ...
    def test_healthy_components_no_recommendations(self, analytics_service):
        ...
        components = [{'healthy': True, 'score': 90.0}, {'healthy': True, 'score': 85.0}, {'healthy': True, 'score': 80.0}]
    ...
    def test_unhealthy_components_generate_recommendations(self, analytics_service):
        ...
        components = [{'healthy': False, 'score': 20.0}, {'healthy': True, 'score': 50.0}]
        recommendations = analytics_service._generate_recommendations(components)
...
class TestAgentPerformanceMonitoring:
    ...
    def test_record_agent_performance_success(self, analytics_service):
        ...
        result = analytics_service.record_agent_performance(agent_name='test_agent', task_type='analysis', quality_score=0.87, response_time=2.5, tools_used=['tool1', 'tool2'])
        assert isinstance(result, StepResult)
        if result.success:
    ...
    def test_record_agent_performance_with_context(self, analytics_service):
        ...
        result = analytics_service.record_agent_performance(agent_name='test_agent', task_type='analysis', quality_score=0.92, response_time=1.8, tools_used=['tool1'], error_occurred=False, user_feedback={'satisfaction': 0.9}, error_details={})
        assert isinstance(result, StepResult)

    def test_record_agent_performance_with_error(self, analytics_service):
        """Test recording agent performance with error flag."""
        result = analytics_service.record_agent_performance(agent_name='failing_agent', task_type='analysis', quality_score=0.4, response_time=8.0, tools_used=['tool1'], error_occurred=True, error_details={'error_type': 'timeout'})
        assert isinstance(result, StepResult)

    def test_get_agent_performance_report(self, analytics_service):
        """Test getting agent performance report."""
        for i in range(3):
            analytics_service.record_agent_performance(agent_name='report_test_agent', task_type='analysis', quality_score=0.8 + i * 0.05, response_time=2.0 + i * 0.2, tools_used=['tool1', 'tool2'])
        result = analytics_service.get_agent_performance_report('report_test_agent', days=1)
        assert isinstance(result, StepResult)
        if result.success:
    ...
    def test_get_agent_performance_report_nonexistent_agent(self, analytics_service):
        ...
        result = analytics_service.get_agent_performance_report('nonexistent_agent', days=30)
        assert isinstance(result, StepResult)
    ...
    def test_comparative_agent_analysis(self, analytics_service):
        ...
        for idx, agent in enumerate(agents):
            analytics_service.record_agent_performance(agent_name=agent, task_type='analysis', quality_score=0.75 + idx * 0.05, response_time=2.0, tools_used=['tool1'])
        result = analytics_service.get_comparative_agent_analysis(agent_names=agents, days=1)
        assert isinstance(result, StepResult)
        if result.success:
    ...
    def test_comparative_analysis_empty_agent_list(self, analytics_service):
        ...
        result = analytics_service.get_comparative_agent_analysis(agent_names=[], days=30)
        assert isinstance(result, StepResult)

    def test_comparative_analysis_single_agent(self, analytics_service):
        """Test comparative analysis with single agent."""
        analytics_service.record_agent_performance(agent_name='solo_agent', task_type='analysis', quality_score=0.85, response_time=2.0)
        result = analytics_service.get_comparative_agent_analysis(agent_names=['solo_agent'], days=1)
        assert isinstance(result, StepResult)
        if result.success:
...
class TestAgentMonitorLazyLoading:
    ...
    def test_record_without_monitor_returns_failure(self, analytics_service):
        ...
        result = analytics_service.record_agent_performance(agent_name='test', task_type='test', quality_score=0.5, response_time=1.0)
        assert isinstance(result, StepResult)
