...
class TestMemoryV2Tool:
    ...
    def test_upsert_operation_success(self, mock_namespace, mock_memory_factory):
        ...
        result = self.tool._run(operation='upsert', content='test content', metadata={'type': 'test'}, creator='test_creator', tenant='test_tenant', workspace='test_workspace')
    ...
    def test_upsert_operation_missing_content(self):
        ...
        result = self.tool._run(operation='upsert', creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
    ...
    def test_query_operation_success(self, mock_namespace, mock_memory_factory):
        ...
        mock_memory = AsyncMock()
        mock_memory.query.return_value = StepResult.ok(results=[{'content': 'result1', 'score': 0.9}], namespace='test:main:default', count=1)
        mock_memory_factory.return_value = mock_memory
        mock_namespace.return_value = Mock()
        result = self.tool._run(operation='query', vector=[0.1, 0.2, 0.3], top_k=5, creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert result_obj.success
    ...
    def test_query_operation_missing_vector(self):
        ...
        result = self.tool._run(operation='query', top_k=5, creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert not result_obj.success
    ...
    def test_delete_operation_not_supported(self, mock_memory_factory):
        ...
        result = self.tool._run(operation='delete', creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert not result_obj.success
    ...
    def test_get_stats_operation(self, mock_namespace, mock_memory_factory):
        ...
        mock_memory_factory.return_value = mock_memory
        mock_namespace.return_value = Mock()
        result = self.tool._run(operation='get_stats', creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert result_obj.success
    ...
    def test_unknown_operation(self):
        ...
        result = self.tool._run(operation='unknown', creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert not result_obj.success
    ...
    def test_memory_exception_handling(self, mock_memory_factory):
        ...
        result = self.tool._run(operation='upsert', content='test content', creator='test_creator', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert not result_obj.success
    ...
    def test_default_creator_handling(self, mock_namespace, mock_memory_factory):
        ...
        mock_memory_factory.return_value = mock_memory
        mock_namespace.return_value = Mock()
        result = self.tool._run(operation='upsert', content='test content', tenant='test_tenant', workspace='test_workspace')
        result_obj = StepResult.from_json(result)
        assert result_obj.success
