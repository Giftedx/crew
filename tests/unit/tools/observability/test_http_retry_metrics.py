...
def test_http_retry_metrics_giveup(monkeypatch):
    ...
    with pytest.raises(requests.ConnectionError):
        http_request_with_retry(
            "GET", "https://example.com/x", request_callable=failing, max_attempts=3, base_backoff=0.0, jitter=0.0
        )
...
def test_http_retry_metrics_success_after_retries(monkeypatch):
    ...
    resp = http_request_with_retry(
        "GET", "https://example.com/y", request_callable=flaky, max_attempts=5, base_backoff=0.0, jitter=0.0
    )
