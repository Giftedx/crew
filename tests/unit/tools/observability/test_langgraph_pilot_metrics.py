...
def test_pilot_emits_pipeline_duration(monkeypatch: pytest.MonkeyPatch):
    ...
    assert 'pipeline_duration_seconds' in rendered and 'pipeline_total_duration_seconds' in rendered and ('pipeline_step_duration_seconds' in rendered) and ('pipeline_requests_total' in rendered) or not metrics.PROMETHEUS_AVAILABLE