...
def test_semantic_cache_promotion_increments_counter(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_sc.store['default:main', model, prompt] = {'status': 'success', 'model': first['model'], 'response': 'CACHED-HIGH-SIM', 'tokens': first['tokens'], 'provider': first['provider'], 'similarity': 0.95}
