...
def test_ingest_transcript_fallback_metric(monkeypatch):
    prov = types.SimpleNamespace(
        fetch_metadata=lambda url: types.SimpleNamespace(id="id1", channel="c"), fetch_transcript=lambda url: None
    )
...
def test_ingest_missing_id_fallback_metric(monkeypatch):
    prov = types.SimpleNamespace(
        fetch_metadata=lambda url: types.SimpleNamespace(id=None, channel="c"), fetch_transcript=lambda url: "hi\nthere"
    )
    monkeypatch.setattr(ip, "_get_provider", lambda src: (prov, "channel"))
    stub = _CounterStub()
