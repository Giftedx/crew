...
class _StubLangSmithAdapter:
    ...
    def evaluate(self, trajectory: AgentTrajectory) -> StepResult:
        result = StepResult.ok(score=True, reasoning='High confidence', accuracy_score=0.9, efficiency_score=0.8, error_handling_score=0.7)
...
class _StubOrchestrator:
    ...
    def submit_trajectory_feedback(self, trajectory: AgentTrajectory, evaluation_result: dict[str, float]) -> StepResult:
        self.calls.append((trajectory, evaluation_result))
        return StepResult.ok(message='queued')

@pytest.fixture()
def sample_trajectory() -> AgentTrajectory:
    return AgentTrajectory(session_id='sess-1', user_input='hello', steps=[TrajectoryStep(timestamp=0.0, agent_role='assistant', action_type='response', content='All good')], final_output='done', total_duration=2.0, success=True, tenant='tenant-alpha', workspace='workspace-beta')

def test_evaluate_and_submit_success(sample_trajectory: AgentTrajectory) -> None:
    orchestrator = _StubOrchestrator()
    evaluator = LangSmithTrajectoryEvaluator(orchestrator=orchestrator, settings=SimpleNamespace(enable_langsmith_eval=True, enable_unified_feedback=True), adapter=_StubLangSmithAdapter())
...
def test_evaluate_skip_when_disabled(sample_trajectory: AgentTrajectory) -> None:
    evaluator = LangSmithTrajectoryEvaluator(orchestrator=_StubOrchestrator(), settings=SimpleNamespace(enable_langsmith_eval=False, enable_unified_feedback=True), adapter=_StubLangSmithAdapter())
    result = evaluator.evaluate_and_submit(sample_trajectory)
    assert result.skipped is True

def test_submit_feedback_without_orchestrator(sample_trajectory: AgentTrajectory) -> None:
    evaluator = LangSmithTrajectoryEvaluator(orchestrator=None, settings=SimpleNamespace(enable_langsmith_eval=True, enable_unified_feedback=True), adapter=None)
    evaluation = StepResult.ok(score=True, reasoning='Great', accuracy_score=0.95, efficiency_score=0.85, error_handling_score=0.9)
