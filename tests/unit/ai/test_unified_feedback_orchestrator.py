...
import pytest
from platform.rl.rl.unified_feedback_orchestrator import ComponentType, FeedbackSignal, FeedbackSource, UnifiedFeedbackOrchestrator, get_orchestrator
...
class TestFeedbackSignal:
    ...
    def test_create_signal(self):
        ...
        signal = FeedbackSignal(source=FeedbackSource.TRAJECTORY, component_type=ComponentType.MODEL, component_id='gpt-4', reward=0.85, context={'task': 'analysis'}, metadata={'latency_ms': 1200})
...
class TestUnifiedFeedbackOrchestrator:
    ...
    def test_submit_tool_feedback(self, orchestrator):
        ...
        orchestrator.submit_tool_feedback(tool_id='sentiment_tool', context={'complexity': 0.7}, success=True, latency_ms=800, quality_score=0.92)
    ...
    def test_submit_agent_feedback(self, orchestrator):
        ...
        orchestrator.submit_agent_feedback(agent_id='verification_analyst', context={'urgency': 0.8}, success=True, duration_s=45.2, quality_score=0.88)
    ...
    def test_submit_rag_feedback(self, orchestrator):
        ...
        orchestrator.submit_rag_feedback(query_id='query_456', relevance_score=0.75, retrieval_latency_ms=200, num_results=10)
    ...
    def test_calculate_component_health(self, orchestrator):
        ...
        for i in range(10):
            signal = FeedbackSignal(source=FeedbackSource.TOOL, component_type=ComponentType.TOOL, component_id='test_tool', reward=0.8, context={}, metadata={})
    ...
    async def test_process_model_feedback(self, orchestrator, mock_model_router):
        ...
        for i in range(5):
            signal = FeedbackSignal(source=FeedbackSource.TRAJECTORY, component_type=ComponentType.MODEL, component_id=f'model_{i}', reward=0.7 + i * 0.05, context={'test': i}, metadata={})
    ...
    def test_get_component_health_report(self, orchestrator):
        ...
        for i in range(5):
            signal = FeedbackSignal(source=FeedbackSource.TOOL, component_type=ComponentType.TOOL, component_id='healthy_tool', reward=0.9, context={}, metadata={})
            orchestrator._update_component_stats(signal)
    ...
    def test_health_based_disable(self, orchestrator):
        ...
        for i in range(20):
            signal = FeedbackSignal(source=FeedbackSource.TOOL, component_type=ComponentType.TOOL, component_id='unhealthy_tool', reward=0.1, context={}, metadata={})
            orchestrator._update_component_stats(signal)
