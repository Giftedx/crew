...
import pytest
from platform.rl.rl.unified_feedback_orchestrator import UnifiedFeedbackOrchestrator
...
def _build_trajectory(*steps: TrajectoryStep) -> AgentTrajectory:
    return AgentTrajectory(session_id='traj-tool-test', user_input='mock', steps=list(steps), final_output='done', total_duration=float(len(steps)), success=True, tenant=None, workspace=None)
...
def test_tool_performance_uses_step_success(orchestrator: UnifiedFeedbackOrchestrator) -> None:
    ...
    steps = [TrajectoryStep(timestamp=0.0, agent_role='assistant', action_type='tool_call', content='call', tool_name='search', success=True), TrajectoryStep(timestamp=1.0, agent_role='assistant', action_type='tool_call', content='call', tool_name='search', success=False)]
...
def test_tool_performance_handles_dict_result(orchestrator: UnifiedFeedbackOrchestrator) -> None:
    ...
    step = TrajectoryStep(timestamp=0.0, agent_role='assistant', action_type='tool_call', content='call', tool_name='search', success=True)
    step.success = None
