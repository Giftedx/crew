...
from ultimate_discord_intelligence_bot.services.rl_model_router import RLModelRouter, RoutingReward, TaskComplexity, TrajectoryFeedback
...
def router_metric_stubs(monkeypatch):
    ...
    return {'queue_depth': queue_depth, 'processed': processed, 'failed': failed, 'trajectory_processed': trajectory_processed, 'labels': base_labels}
...
class _BanditStub:
    ...
    def update(self, model_id: str, context: np.ndarray, reward: float, trajectory_feedback: TrajectoryFeedback | None=None) -> None:
...
def _make_reward(task_id: str, model_id: str) -> RoutingReward:
    return RoutingReward(model_id=model_id, task_id=task_id, reward=0.8, latency_ms=120.0, cost_usd=0.02, quality_score=0.9, success=True, context={'complexity': TaskComplexity.MODERATE.value, 'token_estimate': 800})

def _make_feedback(task_id: str, model_id: str, success: bool=True) -> TrajectoryFeedback:
    return TrajectoryFeedback(trajectory_id=task_id, model_id=model_id, accuracy_score=0.85 if success else 0.45, efficiency_score=0.8 if success else 0.4, error_handling_score=0.82 if success else 0.5, overall_score=0.83 if success else 0.45, trajectory_length=12, success=success, reasoning='ok' if success else 'missing context')
