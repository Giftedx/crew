...
class TestHighlightDetectionWithMocking:
    ...
    def test_detect_highlights_with_audio_mock(self) -> None:
        ...
        with (
            ...
            pytest.importorskip("unittest.mock").patch("librosa.load") as mock_load,
            pytest.importorskip("unittest.mock").patch("librosa.feature.rms") as mock_rms,
        ):
            # Mock audio loading
            mock_load.return_value = (
                pytest.importorskip("numpy").array([0.1, 0.2, 0.3]),
                44100,
            )
