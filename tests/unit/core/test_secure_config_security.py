    ...
    def test_webhook_verification_blocks_default_secrets(self):
        ...
        with patch.dict(os.environ, {'WEBHOOK_SECRET_DEFAULT': 'CHANGE_ME'}):
            with pytest.raises(SecurityError, match='default/empty value'):
                verify_incoming(body=b'test', headers={'X-Signature': 'test', 'X-Timestamp': '123456789', 'X-Nonce': 'test'}, secret_id='default')
...
class TestSecureConfigIntegration:
    ...
    def test_backward_compatibility_helpers(self):
        ...
        from platform.core.secure_config import get_api_key, get_webhook_url, is_feature_enabled, reload_config
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'sk-test', 'DISCORD_WEBHOOK': 'https://discord.com/webhook', 'ENABLE_HTTP_RETRY': 'true'}):
            reload_config()
