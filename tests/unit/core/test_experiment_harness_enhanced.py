...
def test_experiment_registration_logging():
    ...
    with patch('core.rl.experiment.logger') as mock_logger:
        exp = Experiment(experiment_id='exp:logging_test', control='baseline', variants={'variant_a': 0.3, 'variant_b': 0.2}, phase='shadow', auto_activate_after=10)
        mgr.register(exp)
...
def test_allocation_with_logging():
    ...
    mgr = ExperimentManager()
    with patch('core.rl.experiment.logger') as mock_logger:
        exp = Experiment(experiment_id='exp:allocation_test', control='control', variants={'variant': 0.5}, phase='shadow')
        mgr.register(exp)
...
def test_auto_activation_with_logging():
    ...
    mgr = ExperimentManager()
    with patch('core.rl.experiment.logger') as mock_logger:
        exp = Experiment(experiment_id='exp:auto_activate', control='control', variants={'variant': 0.5}, phase='shadow', auto_activate_after=3)
        mgr.register(exp)
        for _i in range(3):
    ...
    with patch('core.rl.experiment.METRICS_AVAILABLE', True):
        with patch('core.rl.experiment.metrics') as mock_metrics_module:
            ...
            with patch('core.rl.experiment.current_tenant') as mock_current_tenant:
                ...
                mgr = ExperimentManager()
                exp = Experiment(experiment_id='exp:metrics_test', control='control', variants={'variant': 0.5}, phase='shadow')
                mgr.register(exp)
