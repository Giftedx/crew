...
class TestMemoryFailureModes:
    ...
    async def test_vector_store_connection_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.connect.return_value = StepResult(
            success=False, error="Connection failed: Network unreachable", retryable=True, custom_status="retryable"
        )
    ...
    async def test_vector_store_connection_timeout(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.connect.return_value = StepResult(
            success=False, error="Connection timeout: Service unavailable", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.connect()
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_vector_store_authentication_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.connect.return_value = StepResult(
            success=False, error="Authentication failed: Invalid credentials", custom_status="unauthorized"
        )
        result = await mock_vector_store.connect()
        assert not result.success
    ...
    async def test_vector_store_insert_failure(
        self, mock_vector_store: Mock, sample_content: dict[str, Any], sample_embedding: list[float]
    ) -> None:
        """Test vector store insert operation failure."""
        mock_vector_store.insert.return_value = StepResult(
            success=False, error="Insert failed: Vector store unavailable", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.insert(
            content=sample_content, embedding=sample_embedding, tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_vector_store_search_failure(self, mock_vector_store: Mock, sample_embedding: list[float]) -> None:
        ...
        mock_vector_store.search.return_value = StepResult(
            success=False, error="Search failed: Index corrupted", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.search(
            query_embedding=sample_embedding, limit=10, tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_vector_store_delete_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.delete.return_value = StepResult(
            success=False, error="Delete failed: Vector not found", custom_status="bad_request"
        )
    ...
    async def test_vector_store_update_failure(
        ...
        mock_vector_store.update.return_value = StepResult(
            success=False, error="Update failed: Version conflict", retryable=True, custom_status="retryable"
        )
    ...
    def test_embedding_invalid_input(self, mock_embedding_service: Mock) -> None:
        ...
        mock_embedding_service.generate_embedding.return_value = StepResult(
            success=False, error="Invalid input: Empty text provided", custom_status="bad_request"
        )
    ...
    def test_embedding_text_too_long(self, mock_embedding_service: Mock) -> None:
        ...
        long_text = "x" * 100000
        mock_embedding_service.generate_embedding.return_value = StepResult(
            success=False, error="Text too long: Exceeds maximum length", custom_status="bad_request"
        )
    ...
    async def test_vector_search_timeout(self, mock_vector_store: Mock, sample_embedding: list[float]) -> None:
        ...
        mock_vector_store.search.return_value = StepResult(
            success=False, error="Search timeout: Query took too long", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.search(
            query_embedding=sample_embedding, limit=100, tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_semantic_search_timeout(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.semantic_search.return_value = StepResult(
            success=False, error="Semantic search timeout: Complex query", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.semantic_search(
            query="complex query with many terms", limit=50, tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_tenant_isolation_during_failure(self, mock_vector_store: Mock) -> None:
        ...
        def mock_search_side_effect(query_embedding: list[float], tenant: str, **kwargs: Any) -> StepResult:
            if tenant == "failing_tenant":
                return StepResult(
                    success=False, error="Tenant-specific failure", retryable=True, custom_status="retryable"
                )
            else:
        ...
        failing_result = await mock_vector_store.search(
            query_embedding=[0.1, 0.2, 0.3], tenant="failing_tenant", workspace="test_workspace"
        )
        ...
        working_result = await mock_vector_store.search(
            query_embedding=[0.1, 0.2, 0.3], tenant="working_tenant", workspace="test_workspace"
        )
    ...
    async def test_workspace_isolation_during_failure(self, mock_vector_store: Mock) -> None:
        ...
        def mock_insert_side_effect(content: dict[str, Any], workspace: str, **kwargs: Any) -> StepResult:
            if workspace == "failing_workspace":
                return StepResult(
                    success=False, error="Workspace-specific failure", retryable=True, custom_status="retryable"
                )
            else:
        ...
        failing_result = await mock_vector_store.insert(
            content={"text": "sample"}, embedding=[0.1, 0.2, 0.3], tenant="test_tenant", workspace="failing_workspace"
        )
        assert not failing_result.success
        assert "Workspace-specific failure" in failing_result.error
        working_result = await mock_vector_store.insert(
            content={"text": "sample"}, embedding=[0.1, 0.2, 0.3], tenant="test_tenant", workspace="working_workspace"
        )
        assert working_result.success
    ...
    async def test_compaction_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.compact.return_value = StepResult(
            success=False, error="Compaction failed: Insufficient disk space", retryable=True, custom_status="retryable"
        )
    ...
    async def test_memory_recovery_after_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.connect.return_value = StepResult(
            success=False, error="Connection failed", retryable=True, custom_status="retryable"
        )
    ...
    async def test_memory_backup_restore_failure(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.backup.return_value = StepResult(
            success=False, error="Backup failed: Storage unavailable", retryable=True, custom_status="retryable"
        )
    ...
    async def test_memory_resource_exhaustion(self, mock_vector_store: Mock) -> None:
        ...
        result = await mock_vector_store.insert(
            content={"text": "large content"}, embedding=[0.1] * 10000, tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_memory_disk_space_exhaustion(self, mock_vector_store: Mock) -> None:
        ...
        mock_vector_store.insert.return_value = StepResult(
            success=False, error="Disk space exhausted", retryable=True, custom_status="retryable"
        )
        result = await mock_vector_store.insert(
            content={"text": "sample"}, embedding=[0.1, 0.2, 0.3], tenant="test_tenant", workspace="test_workspace"
        )
        assert not result.success
        assert result["status"] == "retryable"
    ...
    async def test_cache_fallback_to_vector_store(self, mock_redis_client: Mock, mock_vector_store: Mock) -> None:
        ...
        try:
            ...
            result = await mock_vector_store.search(
                query_embedding=[0.1, 0.2, 0.3], limit=10, tenant="test_tenant", workspace="test_workspace"
            )
    ...
    def test_memory_error_aggregation(self) -> None:
        ...
        memory_errors = [
            StepResult(
                success=False, error="Vector store connection failed", retryable=True, custom_status="retryable"
            ),
            StepResult(success=False, error="Embedding generation failed", retryable=True, custom_status="retryable"),
            StepResult(success=False, error="Search timeout", retryable=True, custom_status="retryable"),
            StepResult(success=False, error="Compaction failed", retryable=True, custom_status="retryable"),
        ]
        error_summary = {
