...
"""
import pytest
from platform.core.rl import CostModel, CostQualityOptimizer, LearningAlgorithm, LearningConfig, ModelSpecification, OptimizationAlgorithm, OptimizationConfig, OptimizationObjective, PreferenceMetric, ProviderPreferenceLearning, RewardType, ThompsonSamplingBandit, ThompsonSamplingConfig, UCBBandit, UCBConfig, UCBStrategy
...
class TestProviderPreferenceLearning:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.config = LearningConfig(algorithm=LearningAlgorithm.EXPONENTIAL_SMOOTHING, alpha=0.3, min_samples_for_learning=5)
...
class TestCostQualityOptimizer:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.config = OptimizationConfig(objective=OptimizationObjective.BALANCED, algorithm=OptimizationAlgorithm.WEIGHTED_SUM, cost_weight=0.3, quality_weight=0.7, max_cost_per_request=1.0, min_quality_threshold=0.6)
        self.optimizer = CostQualityOptimizer(self.config)
        self.model1 = ModelSpecification(model_id='model1', provider_id='provider1', model_name='Fast Model', cost_per_token=0.001, expected_quality_score=0.7, expected_response_time=1.0)
        self.model2 = ModelSpecification(model_id='model2', provider_id='provider2', model_name='Quality Model', cost_per_token=0.002, expected_quality_score=0.9, expected_response_time=2.0)
        self.model3 = ModelSpecification(model_id='model3', provider_id='provider3', model_name='Expensive Model', cost_per_token=0.005, expected_quality_score=0.8, expected_response_time=1.5)
    ...
    def test_model_cost_calculation(self):
        ...
        assert cost == 1.0
        model_per_request = ModelSpecification(model_id='test', provider_id='test', model_name='Test', cost_per_request=0.1, cost_model=CostModel.PER_REQUEST)
    ...
    def test_constraint_filtering(self):
        ...
        expensive_model = ModelSpecification(model_id='expensive', provider_id='provider4', model_name='Too Expensive', cost_per_token=0.01, expected_quality_score=0.9)
        self.optimizer.add_model(self.model1)
    ...
    def test_update_model_performance(self):
        ...
        self.optimizer.add_model(self.model1)
        success = self.optimizer.update_model_performance('model1', actual_cost=0.8, actual_quality=0.75, actual_response_time=0.9)
        assert success
...
class TestIntegration:
    ...
    def test_provider_learning_with_optimizer(self):
        ...
        for _ in range(10):
            result = optimizer.optimize_model_selection(tokens=1000)
            learner.update_provider_request(result.selected_model.provider_id, True, result.predicted_response_time, result.predicted_cost, result.predicted_quality)
