...
def sample_vectors():
    ...
    return [VectorRecord(vector=[0.1, 0.2, 0.3], payload={'text': 'test content 1', 'id': 1}), VectorRecord(vector=[0.2, 0.3, 0.4], payload={'text': 'test content 2', 'id': 2})]
...
class TestMemoryUpsert:
    ...
    async def test_upsert_with_creator(self, memory_service, sample_vectors):
        ...
        result = await memory_service.upsert(tenant='test', workspace='test', records=sample_vectors, creator='creator1')
        assert result.success
