...
from platform.core.structured_llm_service import CacheEntry, CacheKeyGenerator, ProgressCallback, ProgressEvent, ProgressTracker, ResponseCache, StreamingResponse, StreamingStructuredRequest, StructuredLLMService, StructuredRequest, create_structured_llm_service
...
class TestStructuredRequest:
    ...
    def test_structured_request_creation(self):
        ...
        request = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o', provider_opts={'temperature': 0.7}, max_retries=5)
...
class TestStructuredLLMService:
    ...
    def test_route_structured_fallback_with_retries(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        responses = [{'status': 'success', 'response': 'invalid json'}, {'status': 'success', 'response': '{"name": "Bob"}'}, {'status': 'success', 'response': '{"name": "Charlie", "age": 35, "email": "charlie@test.com"}'}]
...
class TestStructuredLLMServiceIntegration:
    ...
    def test_full_integration_workflow(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'success', 'response': '\n            {\n                "name": "Sarah Johnson",\n                "age": 32,\n                "email": "sarah.johnson@example.com",\n                "is_active": true\n            }\n            ', 'model': 'openai/gpt-4o', 'tokens': 150}
        request = StructuredRequest(prompt='Create a profile for a software engineer named Sarah', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
        result = service.route_structured(request)
        assert isinstance(result, UserProfile)
...
class TestStreamingDataStructures:
    ...
    def test_progress_event_creation(self):
        ...
        event = ProgressEvent(event_type='chunk_received', message='Received chunk 1', progress_percent=25.0, data={'chunk': 'test data', 'chunk_index': 1}, timestamp=1234567890.0)
    ...
    def test_streaming_structured_request_creation(self):
        ...
        def callback_func(event: ProgressEvent):
            pass
        request = StreamingStructuredRequest(prompt='Generate a user profile', response_model=UserProfile, progress_callback=callback_func, task_type='general', model='openai/gpt-4o', provider_opts={'temperature': 0.7}, max_retries=5, enable_streaming=True, streaming_chunk_size=1024)
        assert request.prompt == 'Generate a user profile'
        assert request.response_model == UserProfile
    ...
    def test_streaming_response_creation(self):
        ...
        response = StreamingResponse(partial_result=None, is_complete=False, progress_percent=50.0, raw_chunks=['chunk1', 'chunk2'], error=None)
...
class TestStreamingLLMService:
    ...
    def test_route_structured_streaming_instructor_mode(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'success', 'response': '{"name": "Alice", "age": 28, "email": "alice@test.com"}'}
    ...
    def test_route_structured_streaming_fallback_mode(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'success', 'response': '{"name": "Bob", "age": 35, "email": "bob@test.com"}'}
        request = StreamingStructuredRequest(prompt='Generate a user profile', response_model=UserProfile)
        import asyncio
    ...
    def test_route_structured_streaming_with_progress_callback(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'success', 'response': '{"name": "Charlie", "age": 25, "email": "charlie@test.com"}'}
        callback_calls = []

        def progress_callback(event: ProgressEvent):
            callback_calls.append(event)
        request = StreamingStructuredRequest(prompt='Generate a user profile', response_model=UserProfile, progress_callback=progress_callback)
        import asyncio
    ...
    def test_route_structured_streaming_max_retries_exceeded(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'error', 'error': 'Service unavailable'}
        request = StreamingStructuredRequest(prompt='Generate a user profile', response_model=UserProfile, max_retries=2)
        import asyncio
...
class TestStreamingIntegration:
    ...
    def test_full_streaming_workflow_with_instructor(self, mock_openrouter_service):
        ...
        request = StreamingStructuredRequest(prompt='Create a profile for Diana Prince', response_model=UserProfile, progress_callback=track_progress, model='openai/gpt-4o')
        import asyncio
    ...
    def test_full_streaming_workflow_with_fallback(self, mock_openrouter_service):
        ...
        request = StreamingStructuredRequest(prompt='Create a product listing for wireless headphones', response_model=ProductInfo)
        import asyncio
...
class TestCacheEntry:
    ...
    def test_cache_entry_creation(self):
        ...
        entry = CacheEntry(key='test_key', value={'test': 'data'}, created_at=current_time, ttl_seconds=3600, access_count=5, last_accessed=current_time + 100)
    ...
    def test_cache_entry_defaults(self):
        ...
        import time
        current_time = time.time()
        entry = CacheEntry(key='test_key', value='test_value', created_at=current_time, ttl_seconds=1800)
    ...
    def test_cache_entry_is_expired(self):
        ...
        import time
        current_time = time.time()
        entry = CacheEntry(key='test_key', value='test_value', created_at=current_time, ttl_seconds=3600)
        assert not entry.is_expired()
        entry = CacheEntry(key='test_key', value='test_value', created_at=current_time - 7200, ttl_seconds=3600)
...
class TestCacheKeyGenerator:
    ...
    def test_generate_key_basic(self):
        ...
        request = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general')
    ...
    def test_generate_key_with_model(self):
        ...
        request1 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general')
        request2 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
    ...
    def test_generate_key_with_provider_opts(self):
        ...
        request1 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general')
        request2 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', provider_opts={'temperature': 0.7, 'max_tokens': 100})
        key1 = CacheKeyGenerator.generate_key(request1)
        key2 = CacheKeyGenerator.generate_key(request2)
        assert isinstance(key1, str)
        assert isinstance(key2, str)
    ...
    def test_generate_key_deterministic(self):
        ...
        request1 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
        request2 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
        key1 = CacheKeyGenerator.generate_key(request1)
        key2 = CacheKeyGenerator.generate_key(request2)
        assert key1 == key2
    ...
    def test_generate_key_different_prompts(self):
        ...
        request1 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general')
        request2 = StructuredRequest(prompt='Generate a product listing', response_model=UserProfile, task_type='general')
        key1 = CacheKeyGenerator.generate_key(request1)
        key2 = CacheKeyGenerator.generate_key(request2)
        assert key1 != key2
    ...
    def test_generate_key_different_models(self):
        ...
        request1 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
        request2 = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='anthropic/claude-3')
        key1 = CacheKeyGenerator.generate_key(request1)
        key2 = CacheKeyGenerator.generate_key(request2)
        assert key1 != key2
    ...
    def test_generate_key_streaming_request(self):
        ...
        request = StreamingStructuredRequest(prompt='Generate a user profile', response_model=UserProfile, task_type='general', model='openai/gpt-4o')
        key = CacheKeyGenerator.generate_key(request)
        assert isinstance(key, str)
        assert key.startswith('structured_llm:')
...
class TestResponseCache:
    ...
    def test_cache_cleanup_stale_entries(self):
        ...
        cache = ResponseCache()
        current_time = time.time()
        entry1 = CacheEntry('key1', 'value1', current_time, 3600, 0, current_time)
        entry2 = CacheEntry('key2', 'value2', current_time, 3600, 0, current_time - 7200)
        entry3 = CacheEntry('key3', 'value3', current_time, 3600, 0, current_time - 10800)
    ...
    def test_cache_get_ttl_for_request(self):
        ...
        assert cache.get_ttl_for_request(request_general) == 3600
        assert cache.get_ttl_for_request(request_analysis) == 7200
        assert cache.get_ttl_for_request(request_code) == 1800
        assert cache.get_ttl_for_request(request_factual) == 86400
    ...
    def test_cache_size_limits_max_memory(self):
        ...
        cache = ResponseCache(max_memory_mb=0.001)
    ...
    def test_cache_memory_tracking(self):
        ...
        size_info = cache.get_size_info()
        assert abs(size_info['approximate_size_bytes'] - after_removal_memory) < 100
    ...
    def test_cache_size_info_with_limits(self):
        ...
        cache = ResponseCache(max_entries=50, max_memory_mb=5.0, enable_compression=True, compression_min_size_bytes=512)
        cache.set('key1', 'value1')
        cache.set('key2', 'value2')
        size_info = cache.get_size_info()
    ...
    def test_cache_compression_memory_savings(self):
        ...
        assert compressed_memory <= uncompressed_memory * 1.1
...
class TestCachingIntegration:
    ...
    def test_cache_maintenance_integration(self, mock_openrouter_service):
        ...
        service = StructuredLLMService(mock_openrouter_service)
        mock_openrouter_service.route.return_value = {'status': 'success', 'response': '{"name": "David", "age": 40, "email": "david@test.com"}'}
        request = StructuredRequest(prompt='Generate a user profile', response_model=UserProfile)
        result = service.route_structured(request)
        assert isinstance(result, UserProfile)
        assert len(service.cache.cache) == 1
        import time
        service._last_cleanup = time.time() - 400
        result2 = service.route_structured(request)
        assert isinstance(result2, UserProfile)
        assert len(service.cache.cache) == 1
    ...
    def test_cache_ttl_different_task_types(self):
        ...
        cache = ResponseCache()
        tasks_and_ttls = [('general', 3600), ('analysis', 7200), ('code', 1800), ('creative', 1800), ('factual', 86400), ('unknown', 3600)]
    ...
    def test_cache_key_uniqueness(self):
        ...
        keys = set()
        requests = [StructuredRequest(prompt='test1', response_model=UserProfile, task_type='general'), StructuredRequest(prompt='test2', response_model=UserProfile, task_type='general'), StructuredRequest(prompt='test1', response_model=ProductInfo, task_type='general'), StructuredRequest(prompt='test1', response_model=UserProfile, task_type='analysis'), StructuredRequest(prompt='test1', response_model=UserProfile, task_type='general', model='gpt-4'), StructuredRequest(prompt='test1', response_model=UserProfile, task_type='general', model='claude-3')]
