...
from platform.core.structured_llm_service import (
    CacheEntry,
    CacheKeyGenerator,
    ProgressCallback,
    ProgressEvent,
...
class TestStructuredLLMService:
    ...
    def test_route_structured_fallback_with_retries(self, mock_openrouter_service):
        ...
        responses = [
            ...
            {"status": "success", "response": '{"name": "Bob"}'},
...
class TestCacheKeyGenerator:
    ...
    def test_generate_key_with_model(self):
        ...
        request2 = StructuredRequest(
            prompt="Generate a user profile", response_model=UserProfile, task_type="general", model="openai/gpt-4o"
        )
    ...
    def test_generate_key_different_prompts(self):
        ...
        request1 = StructuredRequest(prompt="Generate a user profile", response_model=UserProfile, task_type="general")
        request2 = StructuredRequest(
            prompt="Generate a product listing", response_model=UserProfile, task_type="general"
        )
        key1 = CacheKeyGenerator.generate_key(request1)
        key2 = CacheKeyGenerator.generate_key(request2)
        assert key1 != key2
