...
@pytest.mark.parametrize(
    ...
    [
        (
            "docs/a2a_postman_collection.json",
            ["A2A_BASE_URL", "A2A_API_KEY", "A2A_TENANT_ID", "A2A_WORKSPACE_ID"],
        ),
        (
            "docs/a2a_insomnia_collection.json",
            ["A2A_BASE_URL", "A2A_API_KEY", "A2A_TENANT_ID", "A2A_WORKSPACE_ID"],
        ),
    ],
)
        ...
        for r in resources:
            if isinstance(r, dict) and r.get("_type") == "environment":
                ...
                if isinstance(env_data, dict):
                    keys_found.update(k for k in env_data if isinstance(k, str))
