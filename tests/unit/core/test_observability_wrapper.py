...
from ultimate_discord_intelligence_bot.observability.stepresult_observer import (
    observe_step_result,
)
...
def _extract_logged_json_records(
...
def test_observer_logs_success_and_metadata(caplog: pytest.LogCaptureFixture) -> None:
    ...
    # tenant/workspace may be unknown in tests
...
def test_observer_logs_exception(caplog: pytest.LogCaptureFixture) -> None:
    ...
    # At least one error log with event stepresult
    entries = _extract_logged_json_records(caplog)
    assert len(entries) >= 1
    entry = entries[-1]
