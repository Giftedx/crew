        ...
        try:
            ...
            StepResult.ok(
                data={
                    ...
                    "segments": [
                        {"start": 0.0, "end": 5.0, "text": "This is a test", "confidence": 0.9},
                        {"start": 5.0, "end": 10.0, "text": "transcript", "confidence": 0.8},
                    ],
                    "model": "test-asr",
                    "duration": 10.0,
                    "confidence": 0.85,
            ...
            with patch.object(service, "_diarize_audio") as mock_diarize:
                ...
                result = service.diarize_with_transcript(
                    audio_path=audio_path, asr_service=asr_service, model="balanced", use_cache=False
                )
                assert result.success
                assert result.data["num_speakers"] == 1
...
class TestSpeakerSegment:
    ...
    def test_speaker_segment_to_dict(self) -> None:
        ...
        segment = SpeakerSegment(
            start=1.0, end=6.0, speaker_id="SPEAKER_01", speaker_name="Guest", speaker_role="guest", confidence=0.85
        )
