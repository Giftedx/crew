...
def test_detection_and_redaction_enabled():
    redacted, report = privacy_filter.filter_text(SAMPLE, {'enable_detection': True, 'enable_redaction': True})
...
def test_detection_enabled_redaction_disabled():
    clean, report = privacy_filter.filter_text(SAMPLE, {'enable_detection': True, 'enable_redaction': False})
...
def test_detection_disabled():
    clean, report = privacy_filter.filter_text(SAMPLE, {'enable_detection': False, 'enable_redaction': True})
    assert not report.found, 'No spans expected when detection disabled'
    assert clean == SAMPLE

class _StrictLearning:

    def recommend(self, domain, ctx, arms):
        return 'strict'

    def record(self, domain, ctx, arm, reward):
        pass

def test_strict_arm_forces_detection_and_redaction():
    redacted, report = privacy_filter.filter_text(SAMPLE, {'enable_detection': False, 'enable_redaction': False}, learning=_StrictLearning())
