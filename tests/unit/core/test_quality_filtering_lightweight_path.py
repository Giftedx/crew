...
async def test_lightweight_processing_bypass(monkeypatch):
    ...
    downloader = _DummyStepTool({'status': 'success', 'local_path': '/tmp/video.mp4', 'video_id': 'vid123', 'title': 'Low Quality Clip', 'platform': 'youtube', 'source_url': 'https://example.com/video', 'duration': 42})
    ...
    pipeline = ContentPipeline(webhook_url='https://discord/webhook', downloader=downloader, transcriber=transcriber, analyzer=analyzer, drive=drive, discord=discord, fallacy_detector=fallacy, perspective=perspective, memory=memory)
    ...
    expected_keys = {'word_count', 'sentence_count', 'avg_sentence_length', 'coherence_score', 'topic_clarity_score', 'language_quality_score', 'overall_quality_score'}
