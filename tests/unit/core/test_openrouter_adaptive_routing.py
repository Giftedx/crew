import pytest
from platform.llm.providers.openrouter import context as context_module
from platform.llm.providers.openrouter import execution as execution_module
from platform.llm.providers.openrouter import service as service_module
...
class DummyTokenMeter:
    ...
    def affordable_model(self, tokens_in: int, models: list[str] | None, prices: dict[str, float] | None=None) -> str | None:
        if not models:
            return None
        return models[0]
...
class DummyLogger:
    ...
    def log_llm_call(self, task: str, model: str, provider: str, tokens_in: int, tokens_out: int, cost: float, latency_ms: float, profile_id: str | None, success: bool, error: str | None) -> None:
        self.llm_calls.append((task, model, provider, tokens_in, tokens_out, cost, latency_ms, profile_id, success, error))
...
class DummyAdaptiveManager:
    ...
    def suggest(self, task_type: str, candidates: list[str], context: dict[str, object]) -> tuple[int, str] | None:
        self.suggest_calls.append((task_type, candidates, context))
        return (1, self.suggestion)

    def observe(self, task_type: str, trial_index: int, reward: float, metadata: dict[str, object]) -> None:
        self.observe_calls.append((task_type, trial_index, reward, metadata))
        event = {'task_type': task_type, 'reward': reward, 'model': metadata.get('model') or metadata.get('chosen_model') or self.suggestion}
...
def _build_service(monkeypatch: pytest.MonkeyPatch, *, token_meter: DummyTokenMeter | None=None, manager: DummyAdaptiveManager | None=None) -> tuple[OpenRouterService, DummyAdaptiveManager, DummyLogger]:
    ...
    svc = OpenRouterService(models_map={'general': ['model-a', 'model-b']}, api_key=None, token_meter=tm, logger=logger)
...
def test_adaptive_routing_success_records_reward(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_adaptive_routing_budget_rejection_records_observation(monkeypatch: pytest.MonkeyPatch) -> None:
