...
class DummyTokenMeter:
    ...
    def affordable_model(
        self, tokens_in: int, models: list[str] | None, prices: dict[str, float] | None = None
    ) -> str | None:
        if not models:
            return None
        return models[0]
...
class DummyLogger:
    ...
    def log_llm_call(
        ...
        self.llm_calls.append(
            (task, model, provider, tokens_in, tokens_out, cost, latency_ms, profile_id, success, error)
        )
