...
class TestCacheShadowHarness:
    ...
    @pytest.mark.asyncio
    @patch('ultimate_discord_intelligence_bot.services.cache_shadow_harness.ENABLE_CACHE_V2', True)
    ...
    async def test_shadow_get_unified_hit_legacy_miss(self, mock_namespace, mock_cache_factory):
        ...
        value, metrics = await self.harness.shadow_get(cache_name='test', key='test_key', tenant='test_tenant', workspace='test_workspace')
    ...
    @pytest.mark.asyncio
    @patch('ultimate_discord_intelligence_bot.services.cache_shadow_harness.ENABLE_CACHE_V2', False)
    ...
    async def test_shadow_set_dual_write(self, mock_namespace, mock_cache_factory):
        ...
        mock_namespace.return_value = Mock()
        self.harness.legacy_cache = Mock()
        result = await self.harness.shadow_set(cache_name='test', key='test_key', value='test_value', tenant='test_tenant', workspace='test_workspace', dependencies={'dep1', 'dep2'})
    ...
    async def test_shadow_set_unified_cache_error(self, mock_cache_factory):
        ...
        self.harness.legacy_cache = Mock()
        result = await self.harness.shadow_set(cache_name='test', key='test_key', value='test_value', tenant='test_tenant', workspace='test_workspace')
        assert result.success
    ...
    def test_get_shadow_metrics(self):
        ...
        self.harness._shadow_metrics = {'unified_hits': 80, 'unified_misses': 20, 'legacy_hits': 70, 'legacy_misses': 30}
        metrics = self.harness.get_shadow_metrics()
        assert metrics['unified_hit_rate'] == 0.8
        assert metrics['legacy_hit_rate'] == 0.7
        assert metrics['hit_rate_delta'] == 0.1
    ...
    def test_reset_metrics(self):
        ...
        self.harness._shadow_metrics = {'unified_hits': 100, 'unified_misses': 50, 'legacy_hits': 90, 'legacy_misses': 60}
        self.harness.reset_metrics()
        expected = {'unified_hits': 0, 'unified_misses': 0, 'legacy_hits': 0, 'legacy_misses': 0}
