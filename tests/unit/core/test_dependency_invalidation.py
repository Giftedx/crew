...
import asyncio
from platform.cache.dependency_tracker import DependencyNode, DependencyTracker
from platform.cache.invalidation_engine import InvalidationEngine, InvalidationResult
from platform.cache.multi_level_cache import CacheEntry, MultiLevelCache
...
class TestDependencyTracker:
    ...
    async def test_circular_reference_detection(self, tracker: DependencyTracker):
        ...
        await tracker.register_dependencies("cache:A", {"cache:B"})
        await tracker.register_dependencies("cache:B", {"cache:C"})
        await tracker.register_dependencies("cache:C", {"cache:A"})
...
async def test_complete_invalidation_workflow():
    ...
    try:
        ...
        assert len(template_dependents) == 0
