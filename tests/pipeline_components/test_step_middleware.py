...
class _StubPipeline(PipelineExecutionMixin):
    def __init__(
        self, result: StepResult | Exception, *, middlewares: Iterable[BasePipelineStepMiddleware] = ()
    ) -> None:
        self._result = result
...
def test_execute_step_invokes_middleware_hooks() -> None:
    ...
    async def runner() -> None:
        outcome = await pipeline._execute_step(
            "analysis", lambda x, *, y: StepResult.ok(data={"value": x + y}), 40, y=2
        )
...
def test_tracing_middleware_records_span(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ultimate_discord_intelligence_bot.pipeline_components.middleware.tracing_module", DummyTracing()
    )
