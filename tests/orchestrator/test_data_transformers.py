...
def orchestrator(monkeypatch):
    ...
    monkeypatch.setattr('ultimate_discord_intelligence_bot.autonomous_orchestrator.UltimateDiscordIntelligenceBotCrew', lambda: None)
...
class TestEvidenceToVerdictsTransformation:
    ...
    def test_transform_evidence_to_verdicts(self, orchestrator):
        ...
        fact_verification_data = {'fact_checks': [{'claim': 'AI will replace all jobs by 2030', 'verdict': 'disputed'}, {'claim': 'Transformers use attention mechanisms', 'verdict': 'verified'}]}
    ...
    def test_transform_assigns_verdict_labels(self, orchestrator):
        ...
        fact_verification_data = {'fact_checks': [{'claim': 'Test claim', 'verdict': 'verified', 'confidence': 0.9}, {'claim': 'Weak claim', 'verdict': 'unverified', 'confidence': 0.2}]}
        verdicts = orchestrator._transform_evidence_to_verdicts(fact_verification_data)
        assert isinstance(verdicts, list)
...
class TestSchemaConversion:
    ...
    def test_convert_handles_extra_fields(self, orchestrator):
        ...
        extra_data = {'url': 'https://example.com', 'title': 'Test', 'duration': 100, 'extra_field': 'should_be_ignored', 'another_extra': 123}
        try:
            from ultimate_discord_intelligence_bot.autonomous_orchestrator import AcquisitionOutput
