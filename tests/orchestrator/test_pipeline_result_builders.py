...
import pytest
from ultimate_discord_intelligence_bot.orchestrator.pipeline_result_builders import build_knowledge_payload, build_pipeline_content_analysis_result, create_executive_summary, extract_key_findings, extract_system_status_from_crew, merge_threat_payload
...
class TestBuildPipelineContentAnalysisResult:
    ...
    def minimal_inputs(self) -> dict[str, Any]:
        ...
        return {'transcript': 'This is a test transcript with some words.', 'transcription_data': {'quality_score': 0.8, 'timeline_anchors': [{'time': 0, 'text': 'intro'}], 'transcript_index': {'intro': [0]}}, 'pipeline_analysis': {'keywords': ['test', 'transcript'], 'sentiment': 'positive', 'sentiment_score': 0.7, 'summary': 'Test summary'}, 'media_info': {'title': 'Test Video', 'platform': 'youtube', 'duration': 120}, 'pipeline_fallacy': None, 'pipeline_perspective': None, 'pipeline_metadata': None, 'source_url': 'https://example.com/video'}
    ...
    def test_sentiment_from_sentiment_details(self, minimal_inputs):
        ...
        minimal_inputs['pipeline_analysis']['sentiment_details'] = {'label': 'very_positive', 'score': 0.95, 'confidence': 0.88}
        result = build_pipeline_content_analysis_result(**minimal_inputs)
    ...
    def test_summary_from_pipeline_perspective(self, minimal_inputs):
        ...
        minimal_inputs['pipeline_perspective'] = {'summary': 'Perspective-derived summary', 'perspectives': ['viewpoint1', 'viewpoint2']}
        result = build_pipeline_content_analysis_result(**minimal_inputs)
    ...
    def test_media_info_merged_into_metadata(self, minimal_inputs):
        ...
        minimal_inputs['media_info'] = {'title': 'Amazing Video', 'platform': 'youtube', 'duration': 300, 'uploader': 'TestChannel', 'video_id': 'abc123'}
        result = build_pipeline_content_analysis_result(**minimal_inputs)
    ...
    def test_pipeline_metadata_merged(self, minimal_inputs):
        ...
        minimal_inputs['pipeline_metadata'] = {'workflow_type': 'autonomous_intelligence', 'acquisition_timestamp': 1234567890}
        result = build_pipeline_content_analysis_result(**minimal_inputs)
        metadata = result.data['content_metadata']
...
class TestBuildKnowledgePayload:
    ...
    def minimal_inputs(self) -> dict[str, Any]:
        """Minimal valid inputs for the function."""
        return {'acquisition_data': {}, 'intelligence_data': {}, 'verification_data': {}, 'threat_data': {}, 'fact_data': {}, 'behavioral_data': {}}
    ...
    def test_complete_integration_scenario(self):
        ...
        acquisition_data = {'source_url': 'https://example.com/video', 'download': {'title': 'Test Video', 'platform': 'youtube'}, 'transcription': {'quality_score': 0.9}, 'pipeline_metadata': {'url': 'https://fallback.com'}}
        intelligence_data = {'content_metadata': {'summary': 'Content summary', 'duration': 300}, 'thematic_insights': ['keyword1', 'keyword2'], 'transcript_index': {'word1': [0, 10]}, 'timeline_anchors': [{'time': 0, 'text': 'intro'}]}
        ...
        result = build_knowledge_payload(acquisition_data, intelligence_data, verification_data, threat_data, fact_data, behavioral_data)
