...
if TYPE_CHECKING:
    from ultimate_discord_intelligence_bot.services.openrouter_service.state import RouteState
...
class FeatureExtractor:
    ...
    # Feature dimension (expandable as new features added)
    FEATURE_DIM = 18
    ...
    def extract(
        ...
        if route_state is not None:
            ...
            return features  # Return zeros for missing context

        # Extract individual features
        idx = 0

        # 1. Task complexity score (0-1): based on token count normalized
        token_count = ctx.get("tokens_in", 0)
        features[idx] = self._normalize_feature(
            idx,
            min(1.0, token_count / 4000.0),  # Normalize by typical max
        )
        idx += 1

        # 2. Prompt entropy (0-1): measure of prompt randomness/complexity
        prompt = ctx.get("prompt", "")
        features[idx] = self._normalize_feature(idx, self._calculate_prompt_entropy(prompt))
        idx += 1

        # 3. Recent latency percentile (0-1): p90 of recent latencies
        tenant = ctx.get("tenant", "default")
        features[idx] = self._normalize_feature(idx, self._get_latency_percentile(tenant, 0.9))
        idx += 1

        # 4. Budget utilization ratio (0-1): used/allocated
        features[idx] = self._normalize_feature(idx, self._get_budget_utilization(tenant))
        idx += 1

        # 5. SLA tier encoding (0-1): higher tier = higher value
        ...
        # 6. Content-type embedding dimension 1 (learned)
        ...
        # 17. Provider preference strength (0-1): if provider_opts specified
        has_provider_pref = bool(ctx.get("provider_overrides"))
        features[idx] = 1.0 if has_provider_pref else 0.0
        idx += 1

        # 18. Compression enabled indicator (binary)
    ...
    def _normalize_feature(self, feature_idx: int, value: float) -> float:
        ...
        # Use min-max normalization for better stability
    ...
    def _calculate_prompt_entropy(self, prompt: str) -> float:
        ...
        # Normalize by theoretical max entropy (log2 of alphabet size ~36)
    ...
    def _get_content_type_embedding(self, content_type: str) -> np.ndarray:
        ...
        # Generate deterministic embedding from content_type hash
        hash_int = int(hashlib.md5(content_type.encode()).hexdigest(), 16)
        rng = np.random.RandomState(hash_int % (2**32))
        embedding = rng.normal(0.5, 0.15, 2)  # 2D embedding
    ...
    def _get_request_rate(self, key: str) -> float:
        ...
        # Normalize by 60 req/min threshold (high activity)
        return min(1.0, rate / 60.0)

    def _get_remaining_budget(self, tenant: str) -> float:
        """Get remaining budget for tenant."""
        budget_info = self.budget_utilization[tenant]
        allocated = budget_info.get("allocated", 10.0)  # Default budget
        used = budget_info.get("used", 0.0)
    ...
    def get_feature_importance_report(self) -> dict[str, Any]:
        ...
        report = {
            "features": [],
            "total_samples": sum(f.samples for f in self.feature_importance.values()),
        }

        # Sort by mean magnitude (proxy for importance)
        sorted_features = sorted(
            self.feature_importance.values(),
            key=lambda f: f.mean_magnitude,
            reverse=True,
        )
            ...
            report["features"].append(
                {
                    ...
                    "relative_importance": round(
                        feature.mean_magnitude / max(sorted_features[0].mean_magnitude, 0.001),
                        4,
                    ),
                }
            )
...
# Global instance for shared feature extraction
