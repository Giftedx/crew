...
_metrics_store: dict[str, Any] = {'total_processed': 0, 'quality_bypassed': 0, 'early_exits': 0, 'full_processing': 0, 'content_types': {}, 'checkpoints': {}, 'quality_history': [], 'time_savings': []}

@router.get('/', response_model=DashboardResponse)
async def get_dashboard_data(hours: int=Query(24, description='Hours of data to retrieve', ge=1, le=168)) -> DashboardResponse:
    """
    ...
    overall = PerformanceMetrics(total_processed=total, quality_bypassed=bypassed, early_exits=exits, full_processing=full, bypass_rate=bypass_rate, early_exit_rate=exit_rate, avg_time_savings=avg_savings, total_time_saved_hours=total_saved)
    content_types = []
    for ct_name, ct_data in _metrics_store['content_types'].items():
        content_types.append(ContentTypeStats(content_type=ct_name, count=ct_data['count'], bypass_rate=ct_data['bypass_rate'], avg_quality_score=ct_data['avg_quality'], avg_processing_time=ct_data['avg_time']))
    checkpoints = []
    for cp_name, cp_data in _metrics_store['checkpoints'].items():
        checkpoints.append(CheckpointStats(checkpoint_name=cp_name, exit_count=cp_data['exit_count'], exit_rate=cp_data['exit_rate'], top_exit_reasons=cp_data['top_reasons'], avg_confidence=cp_data['avg_confidence']))
    cutoff_time = datetime.utcnow() - timedelta(hours=hours)
    quality_trends = [QualityTrend(**qt) for qt in _metrics_store['quality_history'] if qt['timestamp'] >= cutoff_time]
    return DashboardResponse(overall_metrics=overall, content_type_breakdown=content_types, checkpoint_analytics=checkpoints, quality_trends=quality_trends)
...
async def get_metrics_summary() -> PerformanceMetrics:
    ...
    estimated_baseline_time = total * 10.0 / 60.0
    total_saved = estimated_baseline_time * avg_savings
    return PerformanceMetrics(total_processed=total, quality_bypassed=bypassed, early_exits=exits, full_processing=full, bypass_rate=bypass_rate, early_exit_rate=exit_rate, avg_time_savings=avg_savings, total_time_saved_hours=total_saved)
...
async def get_content_type_stats() -> list[ContentTypeStats]:
    ...
    return [ContentTypeStats(content_type=ct_name, count=ct_data['count'], bypass_rate=ct_data['bypass_rate'], avg_quality_score=ct_data['avg_quality'], avg_processing_time=ct_data['avg_time']) for ct_name, ct_data in _metrics_store['content_types'].items()]
...
async def get_checkpoint_stats() -> list[CheckpointStats]:
    ...
    return [CheckpointStats(checkpoint_name=cp_name, exit_count=cp_data['exit_count'], exit_rate=cp_data['exit_rate'], top_exit_reasons=cp_data['top_reasons'], avg_confidence=cp_data['avg_confidence']) for cp_name, cp_data in _metrics_store['checkpoints'].items()]

@router.get('/quality-trends', response_model=list[QualityTrend])
async def get_quality_trends(hours: int=Query(24, description='Hours of historical data', ge=1, le=168)) -> list[QualityTrend]:
...
@router.post('/record')
async def record_pipeline_result(processing_type: str, content_type: str | None=None, quality_score: float | None=None, processing_time: float | None=None, exit_checkpoint: str | None=None, exit_reason: str | None=None, exit_confidence: float | None=None, time_saved_pct: float | None=None) -> dict[str, str]:
    """
    ...
    if content_type:
        if content_type not in _metrics_store['content_types']:
            _metrics_store['content_types'][content_type] = {'count': 0, 'total_quality': 0.0, 'total_time': 0.0, 'bypassed': 0}
    ...
    if exit_checkpoint:
        ...
        cp_data['top_reasons'] = [{'reason': reason, 'count': count} for reason, count in sorted(cp_data['reasons'].items(), key=lambda x: x[1], reverse=True)[:5]]
    if quality_score is not None:
        ...
        if hour_entry is None:
            hour_entry = {'timestamp': current_hour, 'total_quality': 0.0, 'total_coherence': 0.0, 'total_completeness': 0.0, 'total_informativeness': 0.0, 'items_processed': 0, 'avg_quality_score': 0.0, 'coherence_score': 0.0, 'completeness_score': 0.0, 'informativeness_score': 0.0}
...
async def reset_metrics() -> dict[str, str]:
    ...
    _metrics_store.update({'total_processed': 0, 'quality_bypassed': 0, 'early_exits': 0, 'full_processing': 0, 'content_types': {}, 'checkpoints': {}, 'quality_history': [], 'time_savings': []})
