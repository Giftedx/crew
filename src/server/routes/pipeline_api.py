...
def register_pipeline_routes(app: FastAPI, settings: Any) -> None:
    ...
    async def _pipeline_run(payload: dict[str, Any]=Body(..., embed=False)) -> JSONResponse:
        ...
        try:
            ...
            result: StepResult = await run_mission({'url': url.strip(), 'quality': resolved_quality or '1080p'}, tenant_ctx=ctx)
    ...
    try:
        ...
        return
    queue = JobQueue(max_concurrent=int(getattr(settings, 'pipeline_max_concurrent_jobs', 5)), job_ttl_seconds=int(getattr(settings, 'pipeline_job_ttl_seconds', 3600)))
    ...
    async def _execute_job_background(job_id: str) -> None:
        ...
        try:
            ...
            await queue.update_status(job_id, JobStatus.COMPLETED, result=result.to_dict(), completed_at=datetime.utcnow(), progress=100.0)
        except Exception as exc:
    ...
    async def _create_pipeline_job(payload: dict[str, Any]=Body(..., embed=False)) -> JSONResponse:
        ...
        job_id = await queue.create_job(url=url.strip(), quality=resolved_quality, tenant_id=tenant_id, workspace_id=workspace_id)
    ...
    async def _delete_pipeline_job(job_id: str) -> JSONResponse:
        ...
        return JSONResponse(status_code=status.HTTP_200_OK, content={'job_id': job_id, 'status': 'cancelled' if job.status == JobStatus.RUNNING else 'deleted'})

    @app.get('/pipeline/jobs', summary='List pipeline jobs')
    async def _list_pipeline_jobs(tenant_id: str | None=None, workspace_id: str | None=None, status: str | None=None) -> JSONResponse:
        ...
        if status:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f'Invalid status: {status}. Must be one of: queued, running, completed, failed, cancelled') from exc
        jobs = await queue.list_jobs(tenant_id=tenant_id, workspace_id=workspace_id, status=status_filter)
        return JSONResponse(status_code=status.HTTP_200_OK, content={'jobs': [job.to_dict() for job in jobs], 'count': len(jobs)})
