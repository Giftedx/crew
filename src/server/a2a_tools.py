...
def load_tools() -> dict[str, ToolFunc]:
    ...
    if is_enabled('ENABLE_A2A_SKILL_SUMMARIZE', True):
        try:
            from ultimate_discord_intelligence_bot.tools.lc_summarize_tool import summarize as lc_summarize
            tools['tools.lc_summarize'] = lc_summarize
        except Exception:
    ...
    if is_enabled('ENABLE_A2A_SKILL_RAG_OFFLINE', True):
        try:
            from ultimate_discord_intelligence_bot.tools.rag_offline_tool import rag_query
            tools['tools.rag_query'] = rag_query
        except Exception:
    ...
    if is_enabled('ENABLE_A2A_SKILL_RAG_VECTOR', True):
        try:
            from ultimate_discord_intelligence_bot.tools.rag_vector_tool import rag_query_vs
            tools['tools.rag_query_vs'] = rag_query_vs
        except Exception:
            pass
    if is_enabled('ENABLE_A2A_SKILL_RAG_INGEST', False):
        try:
            from ultimate_discord_intelligence_bot.tools.rag_ingest_tool import rag_ingest
            tools['tools.rag_ingest'] = rag_ingest
        except Exception:
            pass
    if is_enabled('ENABLE_A2A_SKILL_RAG_INGEST_URL', False):
        try:
            from ultimate_discord_intelligence_bot.tools.rag_ingest_url_tool import rag_ingest_url
            tools['tools.rag_ingest_url'] = rag_ingest_url
        except Exception:
            pass
    if is_enabled('ENABLE_A2A_SKILL_RAG_HYBRID', True):
        try:
            from ultimate_discord_intelligence_bot.tools.rag_hybrid_tool import rag_hybrid
            tools['tools.rag_hybrid'] = rag_hybrid
        except Exception:
            pass
    if is_enabled('ENABLE_A2A_SKILL_RESEARCH_BRIEF', True):
        try:
            from ultimate_discord_intelligence_bot.tools.research_and_brief_tool import research_and_brief
            tools['tools.research_and_brief'] = research_and_brief
        except Exception:
            pass
    if is_enabled('ENABLE_A2A_SKILL_RESEARCH_BRIEF_MULTI', False) or is_enabled('ENABLE_A2A_SKILL_RESEARCH_AND_BRIEF_MULTI', False):
        try:
            from ultimate_discord_intelligence_bot.tools.research_and_brief_multi_tool import research_and_brief_multi
            tools['tools.research_and_brief_multi'] = research_and_brief_multi
        except Exception:

            def _multi(query: str, sources_text: list[str] | None=None, max_items: int=5, max_time: float | None=None, enable_alerts: bool=False) -> StepResult:
                q = str(query or '').strip()
                sources = [str(s) for s in sources_text or []]
                outline = [f'Overview: {q}'] + [f'Point {i + 1}: {s[:60]}' for i, s in enumerate(sources[:max(0, int(max_items or 5))])]
                ...
                meta = {'multi_agent': True, 'quality_score': None, 'execution_time': None, 'max_time_budget': max_time, 'alerts_enabled': enable_alerts}
                data = {'outline': outline or ['Overview'], 'key_findings': key_findings or ['No sources provided'], 'citations': citations, 'risks': risks, 'counts': counts, 'meta': meta}
                return StepResult.ok(data=data)
