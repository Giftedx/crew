...
class CrossPlatformNarrativeTracker:
    ...
    def track_narrative_evolution(self, content_items: list[dict[str, Any]], time_window_hours: int=24, similarity_threshold: float=0.7, max_threads: int=50, model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if use_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'narrative_threads': [t.__dict__ for t in cache_result.narrative_threads], 'total_content_analyzed': cache_result.total_content_analyzed, 'cross_platform_connections': cache_result.cross_platform_connections, 'tracking_confidence': cache_result.tracking_confidence, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            model_name = self._select_model(model)
            tracking_result = self._track_narrative_evolution(content_items, time_window_hours, similarity_threshold, max_threads, model_name)
            if tracking_result:
                if use_cache:
                    self._cache_result(content_items, time_window_hours, similarity_threshold, model, tracking_result)
                processing_time = (time.time() - start_time) * 1000
                return StepResult.ok(data={'narrative_threads': [t.__dict__ for t in tracking_result.narrative_threads], 'total_content_analyzed': tracking_result.total_content_analyzed, 'cross_platform_connections': tracking_result.cross_platform_connections, 'tracking_confidence': tracking_result.tracking_confidence, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def find_contradictions_and_clarifications(self, content_items: list[dict[str, Any]], similarity_threshold: float=0.8, model: Literal['fast', 'balanced', 'quality']='balanced') -> StepResult:
                ...
                for i, item1 in enumerate(cluster_items):
                    for item2 in cluster_items[i + 1:]:
                        ...
                        if relationship['type'] == 'contradiction':
                            contradictions.append({'item1': item1, 'item2': item2, 'contradiction_score': relationship['score'], 'contradiction_type': relationship['subtype']})
                        elif relationship['type'] == 'clarification':
                            clarifications.append({'original_item': item1, 'clarifying_item': item2, 'clarification_score': relationship['score'], 'clarification_type': relationship['subtype']})
            return StepResult.ok(data={'contradictions': contradictions, 'clarifications': clarifications, 'total_clusters_analyzed': len(topic_clusters), 'contradictions_found': len(contradictions), 'clarifications_found': len(clarifications)})
        except Exception as e:
            logger.error(f'Contradiction analysis failed: {e}')
            return StepResult.fail(f'Contradiction analysis failed: {e!s}')

    def generate_narrative_timeline(self, narrative_thread: NarrativeThread, include_confidence_intervals: bool=True) -> StepResult:
        ...
        try:
            ...
            for event in narrative_thread.events:
                sorted_content = sorted([event.primary_content, *event.related_content], key=lambda x: x.get('timestamp', 0))
                timeline_event = {'event_id': event.event_id, 'timestamp': event.created_at, 'platform_sequence': [{'platform': item.get('platform', 'unknown'), 'timestamp': item.get('timestamp', 0), 'content_type': item.get('content_type', 'unknown'), 'title': item.get('title', ''), 'confidence': item.get('confidence', 1.0)} for item in sorted_content], 'narrative_type': event.narrative_type, 'confidence': event.confidence}
                if include_confidence_intervals:
                    timeline_event['confidence_interval'] = self._calculate_confidence_interval(event)
                timeline_events.append(timeline_event)
            timeline_events.sort(key=lambda x: x['timestamp'])
            return StepResult.ok(data={'timeline_events': timeline_events, 'thread_id': narrative_thread.thread_id, 'total_events': len(timeline_events), 'platforms_involved': narrative_thread.platforms_involved})
        except Exception as e:
    ...
    def _track_narrative_evolution(self, content_items: list[dict[str, Any]], time_window_hours: int, similarity_threshold: float, max_threads: int, model_name: str) -> NarrativeTrackerResult | None:
        ...
        try:
            ...
            if not recent_items:
                return NarrativeTrackerResult(narrative_threads=[], total_content_analyzed=0, cross_platform_connections=0, tracking_confidence=0.0)
            ...
            avg_confidence = sum((t.confidence for t in narrative_threads)) / len(narrative_threads) if narrative_threads else 0.0
            return NarrativeTrackerResult(narrative_threads=narrative_threads, total_content_analyzed=len(recent_items), cross_platform_connections=total_connections, tracking_confidence=avg_confidence)
        except Exception as e:
    ...
    def _cluster_by_topic(self, content_items: list[dict[str, Any]], threshold: float) -> list[dict[str, Any]]:
        ...
        for item in content_items:
            ...
            for cluster in clusters:
                ...
                similarity = self._calculate_text_similarity(item_text, representative.get('text', representative.get('title', '')))
            ...
            if not found_cluster:
                new_cluster = {'id': f'cluster_{len(clusters)}', 'representative': item, 'items': [item], 'platforms': {item.get('platform', 'unknown')}, 'created_at': time.time()}
    ...
    def _create_narrative_thread(self, topic_cluster: dict[str, Any]) -> NarrativeThread | None:
        ...
        try:
            ...
            for i, item in enumerate(sorted_items):
                event = NarrativeEvent(event_id=f'event_{topic_cluster['id']}_{i}', primary_content=item, related_content=[], narrative_type=self._classify_narrative_type(item, i, len(sorted_items)), evolution_timeline=[{'item': item, 'timestamp': item.get('timestamp', 0)}], confidence=self._calculate_event_confidence(item), created_at=time.time(), updated_at=time.time())
                events.append(event)
            ...
            return NarrativeThread(thread_id=topic_cluster['id'], title=thread_title, summary=thread_summary, events=events, platforms_involved=list(topic_cluster['platforms']), key_participants=self._extract_key_participants(sorted_items), total_reach=sum((item.get('reach', 0) for item in sorted_items)), evolution_score=evolution_score, created_at=time.time(), updated_at=time.time())
        except Exception as e:
    ...
    def _calculate_confidence_interval(self, event: NarrativeEvent) -> dict[str, float]:
        ...
        return {'lower_bound': event.created_at - 60 * (1 - adjusted_confidence), 'upper_bound': event.created_at + 60 * (1 - adjusted_confidence), 'confidence': adjusted_confidence}

    def _check_cache(self, content_items: list[dict[str, Any]], time_window_hours: int, similarity_threshold: float, model: str) -> NarrativeTrackerResult | None:
    ...
    def _cache_result(self, content_items: list[dict[str, Any]], time_window_hours: int, similarity_threshold: float, model: str, result: NarrativeTrackerResult) -> None:
    ...
    def get_cache_stats(self) -> StepResult:
        ...
        """
        try:
            stats = {'total_cached': len(self._tracking_cache), 'cache_size_limit': self.cache_size, 'utilization': len(self._tracking_cache) / self.cache_size if self.cache_size > 0 else 0.0}
