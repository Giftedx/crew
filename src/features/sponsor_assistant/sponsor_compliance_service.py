...
class SponsorComplianceAssistant:
    ...
    def generate_compliant_cut_list(
        ...
        max_video_duration: float = 3600.0,
        use_cache: bool = True,
    ) -> StepResult:
    ...
    def _analyze_compliance(
        self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines | None, policy_pack: str
        ...
        try:
            ...
            for segment in content_segments:
                ...
                safety_result = safety_service.analyze_content(
                    content=segment.get("text", ""), model="balanced", use_cache=False
                )
            ...
            if segment_analysis and segment_analysis["is_compliant"]:
                ...
                if accumulated_duration > 300 and len(sponsor_placements) < 3:
                    sponsor_placements.append(
                        {"position": current_time, "type": brand_guidelines.sponsorship_format, "duration": 30}
                    )
            else:
    ...
    def _generate_compliance_recommendations(
        self, safety_data: dict[str, Any], policy_config: dict[str, Any], brand_guidelines: BrandGuidelines | None
    ) -> list[str]:
    ...
    def _generate_overall_recommendations(
        self, segment_analyses: list[dict[str, Any]], safe_percentage: float, policy_config: dict[str, Any]
    ) -> list[str]:
