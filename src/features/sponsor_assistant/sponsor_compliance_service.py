...
class SponsorComplianceAssistant:
    ...
    def _load_default_policies(self) -> dict[str, Any]:
        ...
        return {'family_friendly': {'prohibited_topics': ['violence', 'adult_content', 'profanity', 'drugs', 'alcohol'], 'required_warnings': ['mild_language', 'intense_discussion'], 'min_compliance_score': 0.9}, 'professional': {'prohibited_topics': ['controversial_politics', 'profanity', 'adult_content'], 'required_warnings': ['business_content', 'technical_discussion'], 'min_compliance_score': 0.85}, 'general_audience': {'prohibited_topics': ['extreme_violence', 'explicit_content'], 'required_warnings': ['mild_violence', 'language'], 'min_compliance_score': 0.8}}

    def analyze_compliance(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines | None=None, policy_pack: str='general_audience', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if use_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'compliance_report': cache_result.__dict__, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            ...
            if compliance_result:
                ...
                return StepResult.ok(data={'compliance_report': compliance_result.__dict__, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def generate_compliant_cut_list(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines, max_video_duration: float=3600.0, use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            cut_list = self._generate_cut_list(content_segments, compliance_report, brand_guidelines, max_video_duration)
            return StepResult.ok(data={'cut_list': cut_list.__dict__, 'compliance_report': compliance_report.__dict__})
        except Exception as e:
            logger.error(f'Cut list generation failed: {e}')
            return StepResult.fail(f'Cut list generation failed: {e!s}')

    def generate_sponsor_script(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines, sponsor_product: str, sponsor_message: str, use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if not compliance_result.success:
                return StepResult.fail(f'Compliance analysis failed: {compliance_result.error}')
            sponsor_script = self._generate_sponsor_script(content_segments, compliance_result.data['compliance_report'], brand_guidelines, sponsor_product, sponsor_message)
    ...
    def _analyze_compliance(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines | None, policy_pack: str) -> ComplianceReport | None:
        ...
        """
        try:
            from domains.intelligence.analysis.safety.safety_brand_suitability_service import get_safety_brand_suitability_service
            ...
            for segment in content_segments:
                ...
                safety_result = safety_service.analyze_content(content=segment.get('text', ''), model='balanced', use_cache=False)
                if safety_result.success:
                    ...
                    recommendations = self._generate_compliance_recommendations(safety_data, policy_config, brand_guidelines)
                    segment_analyses.append({'segment': segment, 'compliance_score': compliance_score, 'is_compliant': is_compliant, 'recommendations': recommendations})
            ...
            recommendations = self._generate_overall_recommendations(segment_analyses, safe_content_percentage, policy_config)
            ...
            return ComplianceReport(overall_compliance_score=overall_compliance, brand_suitability_score=safe_content_percentage, policy_violations=policy_violations, content_warnings=policy_config['required_warnings'], recommendations=recommendations, safe_content_percentage=safe_content_percentage, audit_trail=segment_analyses)
        except Exception as e:
            logger.error(f'Compliance analysis failed: {e}')
            return None

    def _generate_cut_list(self, content_segments: list[dict[str, Any]], compliance_report: ComplianceReport, brand_guidelines: BrandGuidelines, max_duration: float) -> SponsorCutList:
        ...
        for _i, segment in enumerate(content_segments):
            ...
            if segment_analysis and segment_analysis['is_compliant']:
                safe_segment = ComplianceSegment(start_time=current_time, end_time=current_time + segment_duration, duration=segment_duration, content_type='discussion', safety_level='safe', compliance_score=segment_analysis['compliance_score'], brand_suitability=1.0, risk_factors=[], recommendations=segment_analysis['recommendations'], transcript_text=segment.get('text'))
                safe_segments.append(safe_segment)
                accumulated_duration += segment_duration
                current_time += segment_duration
                if accumulated_duration > 300 and len(sponsor_placements) < 3:
                    sponsor_placements.append({'position': current_time, 'type': brand_guidelines.sponsorship_format, 'duration': 30})
            else:
                unsafe_segment = ComplianceSegment(start_time=segment.get('start_time', 0), end_time=segment.get('end_time', 0), duration=segment_duration, content_type='unsafe', safety_level='unsafe', compliance_score=segment_analysis['compliance_score'] if segment_analysis else 0.0, brand_suitability=0.0, risk_factors=['policy_violation'], recommendations=['Remove or edit this segment'], transcript_text=segment.get('text'))
                unsafe_segments.append(unsafe_segment)
        for i in range(len(safe_segments) - 1):
            transitions.append({'from_segment': i, 'to_segment': i + 1, 'transition_type': 'fade', 'duration': 1.0})
        return SponsorCutList(total_duration=accumulated_duration, safe_segments=safe_segments, unsafe_segments=unsafe_segments, transitions=transitions, sponsor_placements=sponsor_placements, compliance_summary={'overall_compliance': compliance_report.overall_compliance_score, 'safe_percentage': compliance_report.safe_content_percentage, 'violations': len(compliance_report.policy_violations)})

    def _generate_sponsor_script(self, content_segments: list[dict[str, Any]], compliance_report: ComplianceReport, brand_guidelines: BrandGuidelines, sponsor_product: str, sponsor_message: str) -> SponsorScript:
        """Generate a sponsor-safe script with brand integration.
        ...
        for i, segment in enumerate(content_segments):
            ...
            for analysis in compliance_report.audit_trail:
                if analysis['segment']['start_time'] == segment['start_time']:
                    ...
                    break
            if segment_analysis and segment_analysis['is_compliant']:
                script_segment = {'segment_index': i, 'start_time': segment['start_time'], 'end_time': segment['end_time'], 'content': segment.get('text', ''), 'speaker': segment.get('speaker', 'Host'), 'segment_type': 'content'}
                script_segments.append(script_segment)
                compliance_annotations.append({'segment_index': i, 'compliance_score': segment_analysis['compliance_score'], 'recommendations': segment_analysis['recommendations']})
        if brand_guidelines.sponsorship_format == 'integrated':
            ...
            sponsor_segment = {'segment_index': len(script_segments), 'start_time': 0, 'end_time': 0, 'content': self._generate_sponsor_content(sponsor_product, sponsor_message, brand_guidelines), 'speaker': 'Host', 'segment_type': 'sponsor'}
            script_segments.append(sponsor_segment)
        return SponsorScript(script_segments=script_segments, brand_guidelines_applied=[f'target_audience: {brand_guidelines.target_audience}', f'tone: {', '.join(brand_guidelines.tone_requirements)}', f'format: {brand_guidelines.sponsorship_format}'], compliance_annotations=compliance_annotations, total_script_duration=sum((s['end_time'] - s['start_time'] for s in script_segments)), sponsor_integration_points=sponsor_integration_points)

    def _generate_compliance_recommendations(self, safety_data: dict[str, Any], policy_config: dict[str, Any], brand_guidelines: BrandGuidelines | None) -> list[str]:
    ...
    def _generate_overall_recommendations(self, segment_analyses: list[dict[str, Any]], safe_percentage: float, policy_config: dict[str, Any]) -> list[str]:
    ...
    def _calculate_sponsor_integration_points(self, script_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines) -> list[dict[str, Any]]:
        ...
        for i, segment in enumerate(script_segments):
            ...
            transition_indicators = ['speaking of', 'that reminds me', 'by the way', 'also', 'meanwhile', 'interestingly', 'actually', 'so']
            if any((indicator in content for indicator in transition_indicators)):
                integration_points.append({'segment_index': i, 'integration_type': 'natural_transition', 'suggested_duration': 15, 'position': 'start'})
        if script_segments and brand_guidelines.sponsorship_format == 'integrated':
            integration_points.append({'segment_index': len(script_segments) - 1, 'integration_type': 'segment_end', 'suggested_duration': 20, 'position': 'end'})
        return integration_points

    def _generate_sponsor_content(self, sponsor_product: str, sponsor_message: str, brand_guidelines: BrandGuidelines) -> str:
        ...
        templates = {'casual': f'Hey everyone, I wanted to quickly mention {sponsor_product}. {sponsor_message}', 'professional': f'As a {brand_guidelines.target_audience} content creator, I recommend {sponsor_product}. {sponsor_message}', 'educational': f'Today I want to share information about {sponsor_product}, which {sponsor_message}'}
        return templates.get(tone, f'I recommend {sponsor_product} because {sponsor_message}')

    def _check_cache(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines | None, policy_pack: str) -> ComplianceReport | None:
        ...
        guidelines_hash = hashlib.sha256(str(brand_guidelines).encode()).hexdigest()[:16] if brand_guidelines else 'none'
    ...
    def _cache_result(self, content_segments: list[dict[str, Any]], brand_guidelines: BrandGuidelines | None, policy_pack: str, result: ComplianceReport) -> None:
    ...
    def get_cache_stats(self) -> StepResult:
        ...
        """
        try:
            stats = {'total_cached': len(self._compliance_cache), 'cache_size_limit': self.cache_size, 'utilization': len(self._compliance_cache) / self.cache_size if self.cache_size > 0 else 0.0}
