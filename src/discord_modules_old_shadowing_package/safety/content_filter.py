...
class ContentFilter:
    ...
    def _load_harassment_patterns(self) -> list[str]:
        ...
        return [
            "\\b(kill|murder|destroy)\\s+(yourself|urself|urself)\\b",
            "\\b(you\\s+should\\s+die|die\\s+already)\\b",
            "\\b(waste\\s+of\\s+space|useless)\\b",
        ]
    ...
    def _load_misinformation_patterns(self) -> list[str]:
        ...
        return [
            "\\b(covid\\s+is\\s+fake|vaccines\\s+cause\\s+autism)\\b",
            "\\b(flat\\s+earth|moon\\s+landing\\s+fake)\\b",
        ]
