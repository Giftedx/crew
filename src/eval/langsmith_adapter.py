...
class LangSmithEvaluationAdapter:
    ...
    def evaluate(self, trajectory: AgentTrajectory) -> StepResult:
        ...
        payload = {'trajectory': self._serialize_trajectory(trajectory), 'metadata': {'session_id': trajectory.session_id, 'tenant': trajectory.tenant, 'workspace': trajectory.workspace}}
        try:
            response = self._evaluate_fn(evaluation=self.evaluation_name, data=payload, project_name=self.project_name, dataset_name=self.dataset_name)
        except Exception as exc:
            self._record_degradation('runtime_error', context, error=str(exc))
            return StepResult.with_context(success=False, error=f'LangSmith evaluation failed: {exc}', error_category=ErrorCategory.DEPENDENCY_FAILURE, context=context)
        metrics_data = self._extract_metrics(response)
        if not metrics_data:
            self._record_degradation('empty_metrics', context)
            return StepResult.with_context(success=False, error='LangSmith evaluation returned no metrics', error_category=ErrorCategory.PROCESSING, context=context)
        score = bool(metrics_data.get('score') or metrics_data.get('passed') or metrics_data.get('success') or metrics_data.get('accuracy_score'))
        accuracy = self._coerce_float(metrics_data.get('accuracy_score') or metrics_data.get('accuracy') or 0.0)
        efficiency = self._coerce_float(metrics_data.get('efficiency_score') or metrics_data.get('efficiency') or accuracy)
        error_handling = self._coerce_float(metrics_data.get('error_handling_score') or metrics_data.get('error_handling') or accuracy)
        reasoning = str(metrics_data.get('reasoning')) if metrics_data.get('reasoning') is not None else str(metrics_data.get('explanation')) if metrics_data.get('explanation') is not None else str(metrics_data.get('comment')) if metrics_data.get('comment') is not None else 'LangSmith evaluation completed'
        evaluation_id = metrics_data.get('evaluation_id') or metrics_data.get('eval_run_id') or metrics_data.get('run_id')
        result = StepResult.with_context(success=True, context=context, score=score, reasoning=reasoning, accuracy_score=accuracy, efficiency_score=efficiency, error_handling_score=error_handling, evaluation_id=evaluation_id, raw_metrics=self._sanitize(metrics_data))
    ...
    def _handle_unavailable_adapter(self, context: ErrorContext) -> StepResult:
        ...
        return StepResult.with_context(success=False, error=error_message, error_category=ErrorCategory.DEPENDENCY_FAILURE, context=context)
    ...
    def _serialize_trajectory(self, trajectory: AgentTrajectory) -> dict[str, Any]:
        ...
        for step in trajectory.steps:
            steps.append({'timestamp': step.timestamp, 'agent_role': step.agent_role, 'action_type': step.action_type, 'content': step.content, 'tool_name': step.tool_name, 'tool_args': step.tool_args, 'success': step.success, 'error': step.error})
        return {'session_id': trajectory.session_id, 'user_input': trajectory.user_input, 'steps': steps, 'final_output': trajectory.final_output, 'total_duration': trajectory.total_duration, 'success': trajectory.success, 'tenant': trajectory.tenant, 'workspace': trajectory.workspace}
    ...
    def _build_context(self, trajectory: AgentTrajectory) -> ErrorContext:
        ...
        return ErrorContext(operation='trajectory_evaluation', component='langsmith', tenant=tenant, workspace=workspace, session_id=trajectory.session_id)

    def _record_degradation(self, event_type: str, context: ErrorContext, error: str | None=None) -> None:
        try:
            metrics.DEGRADATION_EVENTS.labels(tenant=context.tenant, workspace=context.workspace, component='langsmith_eval', event_type=event_type, severity='warn').inc()
        except Exception:
