            ...
            if evaluation_result is None:
                ...
                try:
                    ...
                    metrics.DEGRADATION_IMPACT_LATENCY.labels(
                        **metrics.label_ctx(), component="trajectory_eval", event_type="langsmith_fallback"
                    ).observe(fallback_latency_ms)
                    logger.warning(
                        "LangSmith evaluation fallback activated", extra={"langsmith_fallback": fallback_payload}
                    )
                except Exception as fallback_exc:
                    fallback_latency_ms = (time.perf_counter() - fallback_start) * 1000.0
                    fallback_payload.update(
                        {"fallback_latency_ms": fallback_latency_ms, "fallback_error": str(fallback_exc)}
                    )
                    metrics.DEGRADATION_EVENTS.labels(
                        **metrics.label_ctx(),
                        component="trajectory_eval",
                    ...
                    metrics.DEGRADATION_IMPACT_LATENCY.labels(
                        **metrics.label_ctx(), component="trajectory_eval", event_type="heuristic_failure"
                    ).observe(fallback_latency_ms)
    ...
    def _emit_routing_feedback(self, trajectory: AgentTrajectory, evaluation_result: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            metrics.TRAJECTORY_FEEDBACK_EMISSIONS.labels(
                **metrics.label_ctx(), model_id=str(model_id), success=str(trajectory.success).lower()
            ).inc()
