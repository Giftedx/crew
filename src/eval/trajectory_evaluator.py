...
def _lc_evaluate_trajectory(outputs):
    summary = {}
    if isinstance(outputs, dict):
        summary = {'keys': sorted(outputs.keys()), 'total_items': len(outputs)}
    elif isinstance(outputs, list):
        summary = {'sample_items': outputs[:3], 'total_items': len(outputs)}
    return {'score': True, 'reasoning': 'local chain evaluator placeholder', 'accuracy_score': 0.5, 'efficiency_score': 0.5, 'error_handling_score': 0.5, 'output_summary': summary}
_DEFAULT_LC_EVAL = _lc_evaluate_trajectory
try:
    if os.getenv('ENABLE_AGENT_EVALS', '0') == '1':
        from agentevals.trajectory.llm import create_trajectory_llm_as_judge as _create_judge
...
class TrajectoryEvaluator:
    ...
    def __init__(self, router: Router | None=None, learning_engine: LearningEngine | None=None, rl_model_router: RLModelRouter | None=None):
        self.router = router
        ...
        if getattr(self._settings, 'enable_langsmith_eval', False):
            ...
            if self.langsmith_adapter is not None:
                try:
                    from ai.rl.langsmith_trajectory_evaluator import LangSmithTrajectoryEvaluator
                    self.langsmith_feedback = LangSmithTrajectoryEvaluator(settings=self._settings, adapter=self.langsmith_adapter)
                except Exception as feedback_exc:
                    logger.warning('Failed to initialize unified feedback bridge: %s', feedback_exc)
    ...
    def extract_trajectory_from_crew_execution(self, crew_execution_log: dict[str, Any]) -> AgentTrajectory:
        ...
        for step_data in crew_execution_log.get('steps', []):
            step = TrajectoryStep(timestamp=step_data.get('timestamp', time.time()), agent_role=step_data.get('agent_role', 'unknown'), action_type=step_data.get('action_type', 'unknown'), content=step_data.get('content', ''), tool_name=step_data.get('tool_name'), tool_args=step_data.get('tool_args'), success=step_data.get('success', True), error=step_data.get('error'))
            steps.append(step)
        ctx = current_tenant()
        return AgentTrajectory(session_id=crew_execution_log.get('session_id', 'unknown'), user_input=crew_execution_log.get('user_input', ''), steps=steps, final_output=crew_execution_log.get('final_output', ''), total_duration=crew_execution_log.get('total_duration', time.time() - start_time), success=crew_execution_log.get('success', True), tenant=getattr(ctx, 'tenant_id', None) if ctx else None, workspace=getattr(ctx, 'workspace', None) if ctx else None)
        ...
        try:
            if os.getenv('ENABLE_AGENT_EVALS', '0') == '1':
                try:
                    ...
                    injected_func = None
                    try:
                        import eval.trajectory_evaluator as _mod_src
                        if hasattr(_mod_src, '_lc_evaluate_trajectory') and _mod_src._lc_evaluate_trajectory is not _DEFAULT_LC_EVAL:
                            injected_func = _mod_src._lc_evaluate_trajectory
                    except Exception:
                        pass
                    if injected_func is None and '_lc_evaluate_trajectory' in globals() and (_lc_evaluate_trajectory is not _DEFAULT_LC_EVAL):
                        injected_func = _lc_evaluate_trajectory
                    if injected_func is not None:
                        try:
                            return StepResult.ok(evaluator='AgentEvals', **injected_func(outputs))
                        except Exception:
                            raise
                    if _AGENTEVALS_AVAILABLE and _agentevals_create_judge:
                        ...
                        ae_res = evaluator(outputs=outputs)
                        if isinstance(ae_res, dict):
                            score_bool = bool(ae_res.get('score', True))
                            return StepResult.ok(score=score_bool, reasoning=str(ae_res.get('reasoning', '')), accuracy_score=float(ae_res.get('accuracy_score', 0.0)), efficiency_score=float(ae_res.get('efficiency_score', 0.0)), error_handling_score=float(ae_res.get('error_handling_score', 0.0)), trajectory_id=trajectory.session_id, evaluator='AgentEvals')
                        return StepResult.ok(score=True, reasoning='AgentEvals evaluator returned non-dict result; see raw', trajectory_id=trajectory.session_id, evaluator='AgentEvals', raw=str(ae_res))
                except Exception:
                    metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='trajectory_eval', event_type='agent_evals_fallback', severity='warn').inc()
            ...
            if self.langsmith_adapter:
                ...
                metadata_updates['langsmith_adapter_status'] = 'skipped' if langsmith_result.skipped else 'success' if langsmith_result.success else 'error'
                ...
                if langsmith_result.success and (not langsmith_result.skipped):
                    evaluation_result = {'score': bool(langsmith_result.data.get('score', False)), 'reasoning': str(langsmith_result.data.get('reasoning', '')), 'accuracy_score': float(langsmith_result.data.get('accuracy_score', 0.0)), 'efficiency_score': float(langsmith_result.data.get('efficiency_score', 0.0)), 'error_handling_score': float(langsmith_result.data.get('error_handling_score', 0.0))}
                    evaluator_name = 'LangSmith'
                    evaluation_id = langsmith_result.metadata.get('langsmith_evaluation_id') or langsmith_result.data.get('evaluation_id')
                    if evaluation_id:
            ...
            if evaluation_result is None:
                ...
                if self.router:
                    try:
                        selected_model = self.router.route(task='trajectory_evaluation', candidates=['gpt-4o-mini', 'gpt-3.5-turbo', 'claude-3-haiku'], context={'prompt': evaluation_prompt, 'expected_output_tokens': 200, 'estimated_cost_usd': 0.01})
                        if selected_model:
                ...
                fallback_payload: dict[str, Any] = {'trajectory_id': trajectory.session_id, 'selected_model': selected_model, 'langsmith_status': metadata_updates.get('langsmith_adapter_status', 'unknown'), 'langsmith_error': metadata_updates.get('langsmith_adapter_error')}
                try:
                    evaluation_result = self._simulate_llm_evaluation(trajectory)
                    evaluator_name = 'LLMHeuristic'
                    fallback_latency_ms = (time.perf_counter() - fallback_start) * 1000.0
                    metadata_updates.update({'fallback_evaluator': evaluator_name, 'fallback_latency_ms': round(fallback_latency_ms, 3), 'fallback_model': selected_model or 'heuristic_default', 'fallback_reason': metadata_updates.get('langsmith_adapter_error', 'langsmith_failure')})
                    fallback_payload.update({'fallback_latency_ms': fallback_latency_ms, 'fallback_model': metadata_updates['fallback_model']})
                    fallback_payload.update({'heuristic_score': evaluation_result.get('accuracy_score'), 'heuristic_efficiency': evaluation_result.get('efficiency_score')})
                    metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='trajectory_eval', event_type='langsmith_fallback', severity='warn').inc()
                    metrics.DEGRADATION_IMPACT_LATENCY.labels(**metrics.label_ctx(), component='trajectory_eval', event_type='langsmith_fallback').observe(fallback_latency_ms)
                    logger.warning('LangSmith evaluation fallback activated', extra={'langsmith_fallback': fallback_payload})
                except Exception as fallback_exc:
                    fallback_latency_ms = (time.perf_counter() - fallback_start) * 1000.0
                    fallback_payload.update({'fallback_latency_ms': fallback_latency_ms, 'fallback_error': str(fallback_exc)})
                    metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='trajectory_eval', event_type='heuristic_failure', severity='error').inc()
                    metrics.DEGRADATION_IMPACT_LATENCY.labels(**metrics.label_ctx(), component='trajectory_eval', event_type='heuristic_failure').observe(fallback_latency_ms)
                    logger.error('LangSmith fallback heuristic failed', extra={'langsmith_fallback': fallback_payload})
            ...
            if self.learning_engine:
                ...
                self.learning_engine.record('trajectory_evaluation', context={'trajectory_length': len(trajectory.steps)}, action='evaluate', reward=reward)
            metrics.TRAJECTORY_EVALUATIONS.labels(**metrics.label_ctx(), success=str(evaluation_result['score']).lower()).inc()
            result = StepResult.ok(score=evaluation_result['score'], reasoning=evaluation_result['reasoning'], accuracy_score=evaluation_result['accuracy_score'], efficiency_score=evaluation_result['efficiency_score'], error_handling_score=evaluation_result['error_handling_score'], trajectory_id=trajectory.session_id, evaluator=evaluator_name)
            if metadata_updates:
                result.metadata.update(metadata_updates)
            if self.langsmith_feedback is not None:
                try:
                    result = self.langsmith_feedback.submit_feedback(trajectory, result)
                except Exception as feedback_exc:
                    logger.warning('LangSmith unified feedback submission failed: %s', feedback_exc)
    ...
    def _emit_routing_feedback(self, trajectory: AgentTrajectory, evaluation_result: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            feedback = TrajectoryFeedback(trajectory_id=trajectory.session_id, model_id=str(model_id), accuracy_score=accuracy, efficiency_score=efficiency, error_handling_score=error_handling, overall_score=overall, trajectory_length=len(trajectory.steps), success=trajectory.success, reasoning=str(evaluation_result.get('reasoning', '')), metadata={'tenant': trajectory.tenant, 'workspace': trajectory.workspace})
            ...
            if isinstance(max_queue, int) and max_queue > 0 and (len(queue) > max_queue):
                queue.pop(0)
            metrics.TRAJECTORY_FEEDBACK_EMISSIONS.labels(**metrics.label_ctx(), model_id=str(model_id), success=str(trajectory.success).lower()).inc()
            return StepResult.ok(model_id=model_id, overall_score=overall, queue_size=len(queue))
        except Exception as exc:
            logger = logging.getLogger(__name__)
    ...
    def evaluate_trajectory_match(self, trajectory: AgentTrajectory, reference_trajectory: AgentTrajectory, match_mode: str='strict') -> StepResult:
    ...
    def _format_trajectory_for_evaluation(self, trajectory: AgentTrajectory) -> str:
        ...
        formatted = [f'User Input: {trajectory.user_input}', f'Duration: {trajectory.total_duration:.2f}s', f'Success: {trajectory.success}', '', 'Steps:']
    ...
    def _simulate_llm_evaluation(self, trajectory: AgentTrajectory) -> dict[str, Any]:
        ...
        return {'score': overall_score, 'reasoning': f'Trajectory {('succeeded' if trajectory.success else 'failed')} with {len(trajectory.steps)} steps', 'accuracy_score': accuracy_score, 'efficiency_score': efficiency_score, 'error_handling_score': error_handling_score}
    ...
    def _to_agentevals_messages(self, trajectory: AgentTrajectory) -> list[dict[str, Any]]:
        ...
        for s in trajectory.steps:
            if s.tool_name:
                call = {'function': {'name': str(s.tool_name), 'arguments': json.dumps(s.tool_args or {})}}
    ...
    def _strict_trajectory_match(self, trajectory: AgentTrajectory, reference: AgentTrajectory) -> StepResult:
        ...
        return StepResult.ok(match=match, score=1.0 if match else 0.0, trajectory_tools=traj_tools, reference_tools=ref_tools, match_type='strict')
    ...
    def _superset_trajectory_match(self, trajectory: AgentTrajectory, reference: AgentTrajectory) -> StepResult:
        ...
        return StepResult.ok(match=match, score=coverage, coverage=coverage, missing_tools=list(ref_tools - traj_tools), extra_tools=list(traj_tools - ref_tools), match_type='superset')
    ...
    def _unordered_trajectory_match(self, trajectory: AgentTrajectory, reference: AgentTrajectory) -> StepResult:
        ...
        ref_tools = {step.tool_name for step in reference.steps if step.tool_name}
        match = traj_tools == ref_tools
        return StepResult.ok(match=match, score=1.0 if match else 0.0, trajectory_tools=sorted(traj_tools), reference_tools=sorted(ref_tools), match_type='unordered')
...
class EnhancedCrewEvaluator:
    ...
    def evaluate_crew_execution(self, crew_execution_log: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            combined_result = {'trajectory_evaluation': accuracy_result.data, 'crew_metrics': crew_metrics, 'session_id': trajectory.session_id, 'evaluation_timestamp': time.time()}
    ...
    def _calculate_crew_metrics(self, trajectory: AgentTrajectory) -> dict[str, Any]:
        ...
        return {'total_steps': len(trajectory.steps), 'unique_agents': len({step.agent_role for step in trajectory.steps}), 'tool_usage_count': len([step for step in trajectory.steps if step.tool_name]), 'error_rate': len([step for step in trajectory.steps if step.error]) / len(trajectory.steps) if trajectory.steps else 0, 'average_step_duration': trajectory.total_duration / len(trajectory.steps) if trajectory.steps else 0, 'success_rate': 1.0 if trajectory.success else 0.0}
__all__ = ['AgentTrajectory', 'EnhancedCrewEvaluator', 'TrajectoryEvaluator', 'TrajectoryStep']