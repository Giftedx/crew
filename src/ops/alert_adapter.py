...
async def receive_alert(request: Request) -> dict[str, str]:
    try:
        ...
        ctx = current_tenant()
        verify_incoming(body, dict(request.headers), secret_id='alert', actor='alertmanager', tenant=ctx.tenant_id if ctx else None, workspace=ctx.workspace_id if ctx else None)
...
async def _send_alert_payload(webhook: str, payload: dict[str, Any]) -> dict[str, str]:
    ...
    resp = resilient_post(webhook, json_payload={'content': content[:1900]}, headers={'Content-Type': 'application/json'}, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
