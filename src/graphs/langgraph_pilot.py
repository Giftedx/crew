...
def _resolve_tenancy():
    try:
        import importlib
        for name in ('ultimate_discord_intelligence_bot.tenancy.context', 'src.ultimate_discord_intelligence_bot.tenancy.context'):
            try:
...
def run_ingest_analysis_pilot(job: dict[str, Any], ingest_fn: Callable[[dict[str, Any]], dict[str, Any]], analyze_fn: Callable[[dict[str, Any]], dict[str, Any]], *, segment_fn: Callable[[dict[str, Any]], dict[str, Any]] | None=None, embed_fn: Callable[[dict[str, Any]], dict[str, Any]] | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        if TenantContext and with_tenant and current_tenant and tenant_id and workspace_id and (current_tenant() is None):
    ...
    with with_tenant(_tenant_ctx_obj) if _has_tenant_ctx and with_tenant else nullcontext():
        ...
        if orchestrator == 'sequential':
            with suppress(Exception):
                metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='langgraph_pilot', event_type='fallback_sequential', severity='info').inc()
        try:
    ...
    def _ingest_node(ctx: dict[str, Any]) -> dict[str, Any]:
        with tracing.start_span('langgraph_pilot.step.ingest') as step_span:
            ...
            try:
                ...
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='ingest', orchestrator=orchestrator, status='success').observe(time.monotonic() - _step_start)
                return res
            except Exception:
                if hasattr(step_span, 'set_attribute'):
                ...
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='ingest', orchestrator=orchestrator, status='error').observe(time.monotonic() - _step_start)
                raise
        ...
        with tracing.start_span('langgraph_pilot.step.segment') as step_span:
            ...
            try:
                ...
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'success')
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='segment', orchestrator=orchestrator, status='success').observe(time.monotonic() - _step_start)
                return res
            except Exception:
                with suppress(Exception):
                    metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='langgraph_pilot', event_type='segment_failure', severity='error').inc()
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'error')
                with suppress(Exception):
                    metrics.PIPELINE_STEPS_FAILED.labels(**metrics.label_ctx(), step='segment').inc()
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='segment', orchestrator=orchestrator, status='error').observe(time.monotonic() - _step_start)
                return {}
        ...
        with tracing.start_span('langgraph_pilot.step.embed') as step_span:
            ...
            try:
                ...
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'success')
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='embed', orchestrator=orchestrator, status='success').observe(time.monotonic() - _step_start)
                return res
            except Exception:
                with suppress(Exception):
                    metrics.DEGRADATION_EVENTS.labels(**metrics.label_ctx(), component='langgraph_pilot', event_type='embed_failure', severity='error').inc()
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'error')
                with suppress(Exception):
                    metrics.PIPELINE_STEPS_FAILED.labels(**metrics.label_ctx(), step='embed').inc()
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='embed', orchestrator=orchestrator, status='error').observe(time.monotonic() - _step_start)
                return {}
        ...
        with tracing.start_span('langgraph_pilot.step.analyze') as step_span:
            ...
            try:
                ...
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'success')
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='analyze', orchestrator=orchestrator, status='success').observe(time.monotonic() - _step_start)
                return res
            except Exception:
                if hasattr(step_span, 'set_attribute'):
                    step_span.set_attribute('outcome', 'error')
                ...
                with suppress(Exception):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='analyze', orchestrator=orchestrator, status='error').observe(time.monotonic() - _step_start)
                raise
    with with_tenant(_tenant_ctx_obj) if _has_tenant_ctx and with_tenant else nullcontext(), tracing.start_span('langgraph_pilot.run') as span:
        ...
        try:
            ...
            try:
                ...
                with suppress(Exception):
                    metrics.PIPELINE_TOTAL_DURATION.labels(**metrics.label_ctx(), orchestrator=orchestrator, status=_status).observe(elapsed)
                with suppress(Exception):
