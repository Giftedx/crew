...
from .circuit_breaker_canonical import get_platform_circuit_breaker
...
class QdrantAPIWrapper:
    ...
    async def search(self, collection_name: str, query_vector: list, limit: int=10, **kwargs) -> StepResult:
        ...
        try:
            result = await self.circuit_breaker.call(self.qdrant_client.search, collection_name, query_vector, limit, **kwargs)
            return StepResult.ok(data=result)
        except Exception as e:
...
__all__ = ['InstagramAPIWrapper', 'OpenRouterAPIWrapper', 'PlatformAPIWrapper', 'QdrantAPIWrapper', 'TikTokAPIWrapper', 'TwitchAPIWrapper', 'XAPIWrapper', 'YouTubeAPIWrapper', 'create_instagram_wrapper', 'create_openrouter_wrapper', 'create_qdrant_wrapper', 'create_tiktok_wrapper', 'create_twitch_wrapper', 'create_x_wrapper', 'create_youtube_wrapper', 'get_all_api_health_status', 'register_wrapped_api']