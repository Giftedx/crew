...
class CostAwareRoutingShadow:
    ...
    def evaluate_routing_decision(self, task_type: str, available_models: list[str], baseline_model: str, model_costs: dict[str, float], model_quality_estimates: dict[str, float], model_latency_estimates: dict[str, float], prompt_tokens: int) -> ShadowRoutingResult | None:
        ...
        try:
            ...
            for model in available_models:
                ...
                latency = model_latency_estimates.get(model, 1000.0)
            ...
            result = ShadowRoutingResult(baseline_model=baseline_model, cost_aware_model=cost_aware_model, baseline_cost=baseline_cost, cost_aware_cost=cost_aware_cost, baseline_utility=baseline_utility, cost_aware_utility=cost_aware_utility, potential_cost_savings=potential_savings, utility_improvement=utility_improvement, recommendation_confidence=confidence)
            ...
            logger.debug('Cost-aware shadow routing: baseline=%s cost=%.4f utility=%.3f, cost_aware=%s cost=%.4f utility=%.3f, savings=%.4f', baseline_model, baseline_cost, baseline_utility, cost_aware_model, cost_aware_cost, cost_aware_utility, potential_savings)
            return result
    ...
    def record_actual_outcome(self, model: str, actual_cost: float, actual_reward: float, latency_ms: float) -> None:
        """Record actual outcome for utility learning."""
        if not self.is_enabled():
            return
        record = CostUtilityRecord(model=model, estimated_cost=actual_cost, estimated_utility=self._calculate_utility(actual_reward, actual_cost, latency_ms), actual_reward=actual_reward, actual_cost=actual_cost, latency_ms=latency_ms)
    ...
    def get_shadow_performance_summary(self, lookback_hours: int=24) -> dict[str, Any]:
        ...
        cost_reduction_percentage = total_potential_savings / total_baseline_cost * 100 if total_baseline_cost > 0 else 0.0
        ...
        return {'enabled': True, 'lookback_hours': lookback_hours, 'total_evaluations': len(recent_results), 'potential_cost_reduction_pct': cost_reduction_percentage, 'total_potential_savings': total_potential_savings, 'avg_utility_improvement': avg_utility_improvement, 'model_switch_rate': switch_rate, 'avg_recommendation_confidence': avg_confidence, 'recommendations': {'would_switch_models': switch_rate > 0.1, 'significant_savings': cost_reduction_percentage > 5.0, 'high_confidence': avg_confidence > 0.7}}
    ...
    def get_model_performance_stats(self) -> dict[str, dict[str, float]]:
        ...
        for record in self.routing_history:
            if record.model not in model_stats:
                model_stats[record.model] = {'count': 0, 'avg_cost': 0.0, 'avg_reward': 0.0, 'avg_latency': 0.0, 'avg_utility': 0.0}
    ...
    def update_optimization_parameters(self, cost_weight: float | None=None, quality_weight: float | None=None, latency_weight: float | None=None, min_quality_threshold: float | None=None) -> None:
...
__all__ = ['CostAwareRoutingShadow', 'CostUtilityRecord', 'ShadowRoutingResult', 'get_cost_aware_shadow']