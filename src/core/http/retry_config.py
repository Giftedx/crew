...
except Exception:  # pragma: no cover - optional dependency not always present

    def current_tenant() -> Any:  # type: ignore[override]
        return None
...
def _load_retry_config() -> dict[str, int | None]:  # pragma: no cover - simple IO
...
def resolve_retry_attempts(call_arg: int | None = None) -> int:
    ...
    # 4. Secure config object
    _config: Any | None = None
    try:
        from core.secure_config import get_config  # local import to avoid cycles
