...
class AdvancedSemanticCache:
    ...
    def __init__(self, similarity_threshold: float=0.85, compression_ratio: float=0.3, max_entries: int=10000, embedding_model: str='all-MiniLM-L6-v2'):
        ...
        self.compression_patterns = [('Please analyze the following', 'Analyze:'), ('Can you help me with', 'Help:'), ('I would like you to', 'Do:'), ('Could you please', 'Please:'), ('Thank you in advance', ''), ('Let me know if you need', '')]
        self.metrics = {'l1_hits': 0, 'l2_hits': 0, 'l3_hits': 0, 'misses': 0, 'compressions_applied': 0, 'total_savings_usd': 0.0, 'total_latency_saved': 0.0}
    ...
    def get(self, prompt: str, expected_tokens: int=1000) -> tuple[Any | None, dict[str, Any]]:
        ...
        """
        metadata = {'cache_level': None, 'similarity_score': None, 'compression_applied': False, 'cost_saved': 0.0, 'latency_saved': 0.0}
        if prompt in self.l1_cache:
            entry = self.l1_cache[prompt]
            entry.access_count += 1
            entry.last_accessed = time.time()
            self.metrics['l1_hits'] += 1
            metadata.update({'cache_level': 'L1', 'cost_saved': self._calculate_cost_savings(expected_tokens), 'latency_saved': 0.5})
        ...
        if compressed in self.l2_cache:
            ...
            metadata.update({'cache_level': 'L2', 'compression_applied': True, 'cost_saved': self._calculate_cost_savings(expected_tokens), 'latency_saved': 0.3})
            return (entry.value, metadata)
        if self.embedding_model:
            ...
            if semantic_result:
                ...
                metadata.update({'cache_level': 'L3', 'similarity_score': similarity, 'cost_saved': self._calculate_cost_savings(expected_tokens), 'latency_saved': 0.2})
                return (entry.value, metadata)
    ...
    def put(self, prompt: str, response: Any, expected_tokens: int=1000, cost_estimate: float=0.0) -> StepResult:
        ...
        try:
            ...
            entry = CacheEntry(key=prompt, value=response, compressed_prompt=self._compress_prompt(prompt), embedding=embedding or [], created_at=time.time(), cost_saved=cost_estimate)
            ...
            if embedding:
                embedding_key = hashlib.md5(json.dumps(embedding).encode(), usedforsecurity=False).hexdigest()[:16]
                self.l3_cache[embedding_key] = entry
            self._cleanup_if_needed()
            return StepResult.ok(data={'cached': True, 'levels_stored': ['L1', 'L2', 'L3'] if embedding else ['L1', 'L2'], 'compression_applied': compressed != prompt})
        except Exception as e:
    ...
    def _compress_prompt(self, prompt: str) -> str:
        ...
        compressed = re.sub('\\s+', ' ', compressed).strip()
        if len(compressed) / len(prompt) > self.compression_ratio:
            filler_patterns = ['\\b(please|kindly|would you|could you|can you)\\b', '\\b(thank you|thanks|appreciate|grateful)\\b', '\\b(let me know|please let me know)\\b', '\\b(in advance|ahead of time)\\b']
    ...
    def warm_cache(self, common_prompts: list[str], responses: list[Any]) -> StepResult:
        ...
        try:
            ...
            for prompt, response in zip(common_prompts, responses, strict=False):
                ...
                if result.success:
                    warmed_count += 1
            return StepResult.ok(data={'warmed_count': warmed_count, 'total_attempted': len(common_prompts)})
        except Exception as e:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {**self.metrics, 'hit_rate': hit_rate, 'total_requests': total_requests, 'cache_size': {'l1_entries': len(self.l1_cache), 'l2_entries': len(self.l2_cache), 'l3_entries': len(self.l3_cache), 'total_entries': len(self.l1_cache) + len(self.l2_cache) + len(self.l3_cache)}, 'configuration': {'similarity_threshold': self.similarity_threshold, 'compression_ratio': self.compression_ratio, 'max_entries': self.max_entries, 'embedding_model_loaded': self.embedding_model is not None}}
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'advanced_cache_healthy': health_status == 'healthy', 'health_status': health_status, 'issues': issues, 'metrics': metrics})
        except Exception as e:
