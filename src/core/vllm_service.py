...
class vLLMLocalInferenceService:
    ...
    def __init__(self, config: vLLMConfig | None=None):
        ...
        try:
            ...
            self.config.tensor_parallel_size = int(getattr(secure_config, 'vllm_tensor_parallel_size', self.config.tensor_parallel_size))
            self.config.max_model_len = int(getattr(secure_config, 'vllm_max_model_len', self.config.max_model_len))
            self.config.gpu_memory_utilization = float(getattr(secure_config, 'vllm_gpu_memory_utilization', self.config.gpu_memory_utilization))
    ...
    def initialize_sync_engine(self) -> bool:
        ...
        with self.lock:
            ...
            try:
                ...
                self.sync_engine = LLM(model=self.config.model_name, tensor_parallel_size=self.config.tensor_parallel_size, max_model_len=self.config.max_model_len, gpu_memory_utilization=self.config.gpu_memory_utilization, quantization=self.config.quantization, dtype=self.config.dtype, max_num_batched_tokens=self.config.max_num_batched_tokens, max_num_seqs=self.config.max_num_seqs, enable_prefix_caching=self.config.enable_prefix_caching, disable_log_stats=self.config.disable_log_stats)
    ...
    async def initialize_async_engine(self) -> bool:
        ...
        try:
            ...
            engine_args = AsyncEngineArgs(model=self.config.model_name, tensor_parallel_size=self.config.tensor_parallel_size, max_model_len=self.config.max_model_len, gpu_memory_utilization=self.config.gpu_memory_utilization, quantization=self.config.quantization, dtype=self.config.dtype, max_num_batched_tokens=self.config.max_num_batched_tokens, max_num_seqs=self.config.max_num_seqs, max_paddings=self.config.max_paddings, enable_prefix_caching=self.config.enable_prefix_caching, disable_log_stats=self.config.disable_log_stats)
    ...
    def generate_sync(self, prompts: list[str], temperature: float=0.7, top_p: float=0.9, max_tokens: int=512, stop: list[str] | None=None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for output in outputs:
                result = {'text': output.outputs[0].text, 'finish_reason': output.outputs[0].finish_reason, 'tokens_generated': len(output.outputs[0].token_ids), 'generation_time': generation_time / len(outputs)}
                results.append(result)
            logger.debug(f'Generated {len(results)} responses in {generation_time:.2f}s ({len(results) / generation_time:.1f} requests/sec)')
            return results
        except Exception as e:
            logger.error(f'vLLM sync generation failed: {e}')
            return [{'error': str(e)} for _ in prompts]

    async def generate_async(self, prompt: str, temperature: float=0.7, top_p: float=0.9, max_tokens: int=512, stop: list[str] | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            result = {'text': final_output.outputs[0].text, 'finish_reason': final_output.outputs[0].finish_reason, 'tokens_generated': len(final_output.outputs[0].token_ids), 'generation_time': generation_time}
    ...
    async def generate_stream(self, prompt: str, temperature: float=0.7, top_p: float=0.9, max_tokens: int=512, stop: list[str] | None=None) -> AsyncGenerator[dict[str, Any], None]:
        ...
        try:
            ...
            async for request_output in results_generator:
                if request_output.outputs:
                    ...
                    yield {'text': output.text, 'finish_reason': output.finish_reason, 'tokens_generated': len(output.token_ids), 'finished': request_output.finished}
        except Exception as e:
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        return {'model_name': self.config.model_name, 'tensor_parallel_size': self.config.tensor_parallel_size, 'max_model_len': self.config.max_model_len, 'gpu_memory_utilization': self.config.gpu_memory_utilization, 'quantization': self.config.quantization, 'dtype': self.config.dtype, 'max_num_seqs': self.config.max_num_seqs, 'prefix_caching_enabled': self.config.enable_prefix_caching, 'vllm_available': VLLM_AVAILABLE, 'initialized': self.initialized}
...
class vLLMOpenRouterAdapter:
    ...
    def route_to_local_model(self, prompt: str, model: str, task_type: str='general') -> dict[str, Any]:
        """Route request to local vLLM model."""
        if not model.startswith('local/'):
            return {'error': 'Not a local model'}
        try:
            results = self.vllm_service.generate_sync(prompts=[prompt], temperature=0.1, max_tokens=512)
            if results and 'error' not in results[0]:
                result = results[0]
                return {'status': 'success', 'model': model, 'response': result['text'], 'tokens': result['tokens_generated'], 'generation_time': result['generation_time'], 'local_inference': True}
            else:
                error = results[0].get('error', 'Unknown error') if results else 'No results'
                return {'status': 'error', 'error': f'vLLM generation failed: {error}', 'model': model}
        except Exception as e:
            return {'status': 'error', 'error': f'Local model routing failed: {e}', 'model': model}