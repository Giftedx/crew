...
class ThreatDetectionEngine:
    ...
    def __init__(self):
        self.suspicious_patterns = {'sql_injection': ['union\\s+select', 'drop\\s+table', 'insert\\s+into', "'\\s*or\\s*'1'\\s*=\\s*'1", ';\\s*drop\\s+'], 'xss_attempt': ['<script.*?>', 'javascript:', 'onerror\\s*=', 'onload\\s*=', 'eval\\s*\\('], 'path_traversal': ['\\.\\./', '\\.\\.\\\\', '/etc/passwd', 'windows/system32'], 'command_injection': [';\\s*rm\\s+', ';\\s*cat\\s+', '`.*`', '\\$\\(.*\\)', '&&\\s*']}
        self.anomaly_thresholds = {'request_rate': 100, 'failed_auth_rate': 10, 'data_volume': 10 * 1024 * 1024}
        ...
        for attack_type, patterns in self.suspicious_patterns.items():
            for pattern in patterns:
                if re.search(pattern, request_text, re.IGNORECASE):
                    threat = SecurityThreat(threat_id=self._generate_threat_id(), threat_type=SecurityEventType.INJECTION_ATTEMPT, threat_level=ThreatLevel.HIGH, description=f'Detected {attack_type} pattern: {pattern}', source_ip=request_data.get('source_ip'), user_id=request_data.get('user_id'), tenant_id=request_data.get('tenant_id'), metadata={'attack_type': attack_type, 'pattern': pattern})
    ...
    def _detect_anomalies(self, request_data: dict[str, Any]) -> list[SecurityThreat]:
        ...
        if self._is_rate_anomaly(request_data):
            threat = SecurityThreat(threat_id=self._generate_threat_id(), threat_type=SecurityEventType.SUSPICIOUS_ACTIVITY, threat_level=ThreatLevel.MEDIUM, description='Abnormal request rate detected', source_ip=request_data.get('source_ip'), user_id=request_data.get('user_id'), metadata={'anomaly_type': 'request_rate'})
            threats.append(threat)
        if self._is_data_volume_anomaly(request_data):
            threat = SecurityThreat(threat_id=self._generate_threat_id(), threat_type=SecurityEventType.DATA_EXFILTRATION_ATTEMPT, threat_level=ThreatLevel.HIGH, description='Abnormal data volume detected', source_ip=request_data.get('source_ip'), user_id=request_data.get('user_id'), metadata={'anomaly_type': 'data_volume'})
            threats.append(threat)
        return threats
    ...
    def _analyze_behavior(self, request_data: dict[str, Any]) -> list[SecurityThreat]:
        ...
        if baseline and self._is_behavioral_anomaly(current_pattern, baseline):
            threat = SecurityThreat(threat_id=self._generate_threat_id(), threat_type=SecurityEventType.SUSPICIOUS_ACTIVITY, threat_level=ThreatLevel.MEDIUM, description='Abnormal user behavior detected', user_id=user_id, metadata={'anomaly_type': 'behavioral'})
            threats.append(threat)
...
class VulnerabilityScanner:
    ...
    def _scan_file_vulnerabilities(self, file_path: Path) -> list[VulnerabilityReport]:
        ...
        try:
            ...
            if self._has_hardcoded_secrets(content):
                vulnerabilities.append(VulnerabilityReport(vulnerability_id=f'VULN_{secrets.token_hex(4)}', severity=ThreatLevel.HIGH, category='hardcoded_secrets', description='Hardcoded secrets detected in source code', affected_component=str(file_path), remediation_steps=['Move secrets to environment variables', 'Use secure configuration management', 'Implement secret rotation']))
            if self._has_sql_injection_risk(content):
                vulnerabilities.append(VulnerabilityReport(vulnerability_id=f'VULN_{secrets.token_hex(4)}', severity=ThreatLevel.CRITICAL, category='sql_injection', description='Potential SQL injection vulnerability', affected_component=str(file_path), remediation_steps=['Use parameterized queries', 'Implement input validation', 'Add SQL injection protection']))
            if self._has_insecure_randomness(content):
                vulnerabilities.append(VulnerabilityReport(vulnerability_id=f'VULN_{secrets.token_hex(4)}', severity=ThreatLevel.MEDIUM, category='weak_randomness', description='Insecure random number generation', affected_component=str(file_path), remediation_steps=['Use cryptographically secure random functions', 'Import secrets module instead of random', 'Review cryptographic implementations']))
    ...
    def _load_vulnerability_database(self) -> dict[str, Any]:
        ...
        return {'known_patterns': {'hardcoded_secrets': ['password\\s*=\\s*["\\\'][^"\\\']+["\\\']', 'api_key\\s*=\\s*["\\\'][^"\\\']+["\\\']', 'secret\\s*=\\s*["\\\'][^"\\\']+["\\\']'], 'sql_injection': ['execute\\s*\\(\\s*["\\\'].*%.*["\\\']', 'query\\s*\\(\\s*["\\\'].*\\+.*["\\\']'], 'weak_crypto': ['md5\\s*\\(', 'sha1\\s*\\(', 'random\\.random\\s*\\(']}}
...
class SecurityOrchestrator:
        ...
        try:
            ...
            for vuln in vulnerabilities:
                if vuln.severity in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
                    log_security_event(action='vulnerability_detected', decision='alert', resource=vuln.affected_component, reason=vuln.description, actor='security_scanner', severity=vuln.severity.value)
            return vulnerabilities
        except Exception as e:
    ...
    def _handle_threat(self, threat: SecurityThreat) -> None:
        ...
        log_security_event(action='threat_detected', decision='alert' if threat.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL] else 'monitor', resource=threat.source_ip or 'unknown', reason=threat.description, actor=threat.user_id or 'unknown', severity=threat.threat_level.value)
        ...
        try:
            metrics.SECURITY_THREATS.labels(**metrics.label_ctx(), threat_type=threat.threat_type.value, threat_level=threat.threat_level.value).inc()
        except Exception as e:
    ...
    def _auto_mitigate_threat(self, threat: SecurityThreat) -> None:
        ...
        if mitigation_action:
            ...
            log_security_event(action='threat_mitigated', decision='block', resource=threat.source_ip or 'unknown', reason=f'Auto-mitigation: {mitigation_action}', actor='security_orchestrator', severity=threat.threat_level.value)

    def _load_response_rules(self) -> dict[str, Any]:
        """Load threat response rules."""
        return {SecurityEventType.INJECTION_ATTEMPT: {'auto_block': True, 'notification_level': 'immediate'}, SecurityEventType.DATA_EXFILTRATION_ATTEMPT: {'auto_block': True, 'notification_level': 'immediate'}, SecurityEventType.RATE_LIMIT_EXCEEDED: {'auto_block': False, 'notification_level': 'delayed'}}

    def generate_security_report(self) -> dict[str, Any]:
        """Generate comprehensive security report."""
        return {'security_score': self._calculate_security_score(), 'threat_summary': self._generate_threat_summary(), 'vulnerability_summary': self._generate_vulnerability_summary(), 'metrics': {'threats_detected': self.security_metrics.threats_detected, 'threats_mitigated': self.security_metrics.threats_mitigated, 'mitigation_rate': self.security_metrics.mitigation_rate, 'vulnerabilities_found': self.security_metrics.vulnerabilities_found, 'vulnerability_fix_rate': self.security_metrics.vulnerability_fix_rate}, 'recommendations': self._generate_security_recommendations()}

    def _calculate_security_score(self) -> float:
        """Calculate overall security score."""
        base_score = 100.0
        critical_threats = len([t for t in self.active_threats.values() if t.threat_level == ThreatLevel.CRITICAL and (not t.mitigated)])
        high_threats = len([t for t in self.active_threats.values() if t.threat_level == ThreatLevel.HIGH and (not t.mitigated)])
    ...
    def _generate_threat_summary(self) -> dict[str, Any]:
        ...
        return {'total_active_threats': len(active_threats), 'critical_threats': len([t for t in active_threats if t.threat_level == ThreatLevel.CRITICAL]), 'high_threats': len([t for t in active_threats if t.threat_level == ThreatLevel.HIGH]), 'recent_threats': sorted(active_threats, key=lambda x: x.timestamp, reverse=True)[:5]}

    def _generate_vulnerability_summary(self) -> dict[str, Any]:
        """Generate vulnerability summary."""
        return {'total_vulnerabilities': self.security_metrics.vulnerabilities_found, 'last_scan': self.security_metrics.last_scan_time, 'fix_rate': self.security_metrics.vulnerability_fix_rate}
...
__all__ = ['SecurityEventType', 'SecurityMetrics', 'SecurityOrchestrator', 'SecurityThreat', 'ThreatDetectionEngine', 'ThreatLevel', 'VulnerabilityReport', 'VulnerabilityScanner']