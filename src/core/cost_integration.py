...
class CostMonitoringIntegration:
    ...
    def __init__(self) -> None:
        ...
        # Enhanced thresholds for cost monitoring
        self.alert_thresholds = AlertThresholds(
            budget_warning_percent=75.0,
            budget_critical_percent=90.0,
            budget_emergency_percent=95.0,
            cost_spike_multiplier=3.0,
            cost_spike_window_minutes=15,
            high_cost_request_usd=0.10,
            daily_warning_percent=80.0,
            daily_critical_percent=95.0,
            model_cost_anomaly_multiplier=2.5,
        )

    def process_request_with_monitoring(
        self,
        model: str,
        tokens_input: int,
        tokens_output: int,
        ...
        try:
            ...
            cost_result = self.cost_optimizer.record_cost(
                ...
                cache_hit=False,  # Will be updated if cache hit occurs
            ...
            if check_alerts:
                try:
                    ...
                    alert_result = self.alert_manager.check_cost_alerts(
                        cost_usd=cost_usd,
                        model=model,
                        component=component,
                        tenant_id=tenant_id,
            ...
            return StepResult.ok(
                data={
                    ...
                    "tenant_id": tenant_id,
                    "workspace_id": workspace_id,
                    "budget_remaining": self.budget_store._get().daily_budget - self.budget_store._get().spent_today,
                }
            )
    ...
    def get_comprehensive_cost_report(
        ...
        try:
            # Get cost analysis from optimizer
            ...
            return StepResult.ok(
                data={
                    ...
                    "optimization_recommendations": recommendations.data.get("optimization_recommendations", []),
                    "alert_summary": alert_summary.data,
                    "budget_status": {
                        "daily_spend_usd": daily_spend,
                        "daily_budget_usd": daily_budget,
                        "daily_utilization_percent": daily_utilization,
                        "per_request_limit_usd": budget_manager.max_per_request,
                        "budget_status": self._get_budget_status(daily_utilization),
                    },
                    "timestamp": time.time(),
                }
            )
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(
                data={
                    "cost_monitoring_integration_healthy": True,
                    "budget_store": {
                        "daily_spend": budget_manager.spent_today,
                        "daily_budget": budget_manager.daily_budget,
                        "per_request_limit": budget_manager.max_per_request,
                        "utilization_percent": (budget_manager.spent_today / budget_manager.daily_budget) * 100,
                    },
                    "cost_optimizer": optimizer_health.data if optimizer_health.success else {"healthy": False},
                    "alert_manager": alert_health.data if alert_health.success else {"healthy": False},
                    "timestamp": time.time(),
                }
            )
