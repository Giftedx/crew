...
class CircuitBreaker(Generic[T]):
    ...
    def _open_circuit(self):
        ...
        try:
            ...
            log_error(
                e,
                message="Failed to update circuit open metrics - invalid metrics configuration",
    ...
    def _close_circuit(self):
        ...
        # Update metrics
        try:
            metrics.CIRCUIT_BREAKER_STATE.labels(**metrics.label_ctx(), circuit=self.name).set(0)  # 0 = closed
        except (AttributeError, TypeError, ValueError) as e:
            log_error(
                e,
                message="Failed to update circuit closed metrics - invalid metrics configuration",
                context={"circuit": self.name, "operation": "metrics_update"},
            )
        except Exception as e:
            log_error(
    ...
    def _half_open_circuit(self):
        ...
        # Update metrics
        try:
            metrics.CIRCUIT_BREAKER_STATE.labels(**metrics.label_ctx(), circuit=self.name).set(0.5)  # 0.5 = half-open
        except (AttributeError, TypeError, ValueError) as e:
            log_error(
                e,
                message="Failed to update circuit half-open metrics - invalid metrics configuration",
                context={"circuit": self.name, "operation": "metrics_update"},
            )
        except Exception as e:
            log_error(
