...
try:
    ...
    from langfuse.model import CreateSpan, CreateTrace, UpdateSpan, UpdateTrace
    _LANGFUSE_IMPORT_ERROR: Exception | None = None
except Exception as exc:
    _LangfuseClient = None
    CreateSpan = CreateTrace = UpdateSpan = UpdateTrace = None
...
class LangfuseService:
    ...
    def __init__(self, settings: SecureConfig | None=None) -> None:
        ...
        if not self.public_key or not self.secret_key:
            ...
            self.enabled = False
            return
        client_kwargs: dict[str, Any] = {'public_key': self.public_key, 'secret_key': self.secret_key}
        if self.base_url:
    ...
    def create_trace(self, name: str, user_id: str, *, metadata: dict[str, Any] | None=None, input_data: dict[str, Any] | None=None, tags: list[str] | None=None) -> StepResult:
        ...
        tags = tags or []
        try:
            trace = self._client.trace(CreateTrace(name=name, user_id=user_id, metadata=metadata, input=input_payload, tags=tags))
            try:
            ...
            return StepResult.with_context(success=False, error=f'Langfuse trace creation failed: {exc}', error_category=ErrorCategory.DEPENDENCY_FAILURE, context=ErrorContext(operation='langfuse_trace', component='langfuse', tenant=user_id, workspace=user_id))

    def create_span(self, trace: Any, name: str, input_data: dict[str, Any], *, metadata: dict[str, Any] | None=None) -> StepResult:
        ...
        try:
            ...
            return StepResult.with_context(success=False, error=f'Langfuse span creation failed: {exc}', error_category=ErrorCategory.DEPENDENCY_FAILURE, context=ErrorContext(operation='langfuse_span', component='langfuse', tenant=name, workspace=name))
    ...
    def update_span(self, span: Any, output_data: dict[str, Any] | None=None, error: str | None=None) -> StepResult:
        ...
        try:
            span.update(UpdateSpan(output=output_payload, level='ERROR' if error else 'DEFAULT', status_message=error))
            self.flush()
            try:
                metrics.LANGFUSE_SPANS.labels(**metrics.label_ctx(), status='completed' if not error else 'errored').inc()
            except Exception:
            ...
            return StepResult.with_context(success=False, error=f'Langfuse span update failed: {exc}', error_category=ErrorCategory.DEPENDENCY_FAILURE, context=ErrorContext(operation='langfuse_span', component='langfuse'))

    def finalize_trace(self, trace: Any, output_data: dict[str, Any] | None=None, error: str | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        if not self._client or not self.enabled or trace is None:
            return StepResult.skip(reason='langfuse_disabled')
        try:
            payload: dict[str, Any] = {'output': _sanitize(output_data or {})}
            ...
            return StepResult.with_context(success=False, error=f'Langfuse trace update failed: {exc}', error_category=ErrorCategory.DEPENDENCY_FAILURE, context=ErrorContext(operation='langfuse_trace', component='langfuse'))
