...
class WebSocketIntegrationService:
    ...
    async def notify_agent_status_change(
        ...
        try:
            result = await self.websocket_service.send_agent_status_update(
                agent_id=agent_id, status=status, details=details, tenant_id=tenant_id, workspace_id=workspace_id
            )
            if result.success:
    ...
    async def notify_analysis_progress(
        ...
        if not self.websocket_service or not self.enabled:
            return StepResult.ok(data={"notifications_sent": 0})
        try:
            result = await self.websocket_service.send_analysis_update(
    ...
    async def notify_stream_update(
        ...
        if not self.websocket_service or not self.enabled:
            return StepResult.ok(data={"notifications_sent": 0})
        try:
            result = await self.websocket_service.send_stream_update(
                stream_id=stream_id, content=content, metadata=metadata, tenant_id=tenant_id, workspace_id=workspace_id
            )
            if result.success:
    ...
    async def notify_system_alert(
        ...
        if not self.websocket_service or not self.enabled:
            return StepResult.ok(data={"notifications_sent": 0})
        try:
            result = await self.websocket_service.send_system_alert(
    ...
    async def notify_performance_metrics(
        self, metrics: dict[str, Any], tenant_id: str = "default", workspace_id: str = "main"
    ) -> StepResult:
        """Notify clients about performance metrics updates."""
        if not self.websocket_service or not self.enabled:
            return StepResult.ok(data={"notifications_sent": 0})
        try:
            result = await self.websocket_service.send_performance_metrics(
                metrics=metrics, tenant_id=tenant_id, workspace_id=workspace_id
            )
            if result.success:
    ...
    async def broadcast_custom_message(
        ...
        try:
            ...
            result = await self.websocket_service.broadcast_message(
                message=message, channel=channel, tenant_id=tenant_id, workspace_id=workspace_id
            )
            if result.success:
...
async def notify_agent_status(
    agent_id: str, status: str, details: dict[str, Any], tenant_id: str = "default", workspace_id: str = "main"
) -> StepResult:
...
async def notify_analysis_progress(
    analysis_id: str, progress: float, results: dict[str, Any], tenant_id: str = "default", workspace_id: str = "main"
) -> StepResult:
...
async def notify_stream_update(
    stream_id: str, content: str, metadata: dict[str, Any], tenant_id: str = "default", workspace_id: str = "main"
) -> StepResult:
