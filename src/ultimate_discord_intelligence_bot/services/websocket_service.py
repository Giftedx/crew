...
class WebSocketService:
    ...
    async def start_server(self) -> StepResult:
        ...
        try:
            ...
            self.server = await serve_fn(
                self._handle_client, self.host, self.port, max_size=1024 * 1024, ping_interval=20, ping_timeout=10
            )
            self.running = True
    ...
    async def _handle_client(self, websocket: Any, path: str) -> None:
        ...
        try:
            ...
            welcome_msg = WebSocketMessage(
                ...
                data={"message": "Connected to WebSocket service", "client_id": client_id},
                timestamp=time.time(),
                tenant_id=tenant_id,
                workspace_id=workspace_id,
            )
    ...
    async def _handle_subscribe(self, client_id: str, channels: list[str]) -> None:
        ...
        response = WebSocketMessage(
            message_type=MessageType.HEARTBEAT,
            data={"message": f"Subscribed to {channels}", "subscribed_channels": list(connection.subscribed_channels)},
            timestamp=time.time(),
    ...
    async def send_agent_status_update(
        ...
        message = WebSocketMessage(
            ...
            data={"agent_id": agent_id, "status": status, "details": details},
            timestamp=time.time(),
            tenant_id=tenant_id,
            workspace_id=workspace_id,
        )
        return await self.broadcast_message(
            message, channel="agent_status", tenant_id=tenant_id, workspace_id=workspace_id
        )
    ...
    async def send_analysis_update(
        ...
        message = WebSocketMessage(
            ...
            data={"analysis_id": analysis_id, "progress": progress, "results": results},
            timestamp=time.time(),
            tenant_id=tenant_id,
            workspace_id=workspace_id,
        )
    ...
    async def send_stream_update(
        ...
        message = WebSocketMessage(
            ...
            data={"stream_id": stream_id, "content": content, "metadata": metadata},
            timestamp=time.time(),
            tenant_id=tenant_id,
            workspace_id=workspace_id,
        )
    ...
    async def send_system_alert(
        ...
        message = WebSocketMessage(
            ...
            data={"alert_type": alert_type, "severity": severity, "message": message_text, "details": details},
            timestamp=time.time(),
            tenant_id=tenant_id,
            workspace_id=workspace_id,
        )
