...
class WebSocketMessage:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'message_type': self.message_type.value, 'data': self.data, 'timestamp': self.timestamp, 'tenant_id': self.tenant_id, 'workspace_id': self.workspace_id, 'message_id': self.message_id}
...
class WebSocketService:
    ...
    async def start_server(self) -> StepResult:
        ...
        if not get_settings().feature_flags.ENABLE_WEBSOCKET_UPDATES:
            return StepResult.fail('WebSocket updates disabled via feature flag')
        try:
            serve_fn = websockets.serve
            self.server = await serve_fn(self._handle_client, self.host, self.port, max_size=1024 * 1024, ping_interval=20, ping_timeout=10)
            self.running = True
            if not hasattr(self, '_background_tasks'):
                self._background_tasks = set()
            t1 = asyncio.create_task(self._heartbeat_task())
            t2 = asyncio.create_task(self._cleanup_task())
    ...
    async def _handle_client(self, websocket: Any, path: str) -> None:
        ...
        try:
            ...
            connection = ClientConnection(websocket=websocket, client_id=client_id, tenant_id=tenant_id, workspace_id=workspace_id, subscribed_channels=set(), connected_at=time.time(), last_heartbeat=time.time())
            self.clients[client_id] = connection
            log.info(f'Client {client_id} connected (tenant: {tenant_id}, workspace: {workspace_id})')
            welcome_msg = WebSocketMessage(message_type=MessageType.HEARTBEAT, data={'message': 'Connected to WebSocket service', 'client_id': client_id}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
    ...
    async def _handle_subscribe(self, client_id: str, channels: list[str]) -> None:
        ...
        response = WebSocketMessage(message_type=MessageType.HEARTBEAT, data={'message': f'Subscribed to {channels}', 'subscribed_channels': list(connection.subscribed_channels)}, timestamp=time.time(), tenant_id=connection.tenant_id, workspace_id=connection.workspace_id)
    ...
    async def _handle_unsubscribe(self, client_id: str, channels: list[str]) -> None:
        ...
        response = WebSocketMessage(message_type=MessageType.HEARTBEAT, data={'message': f'Unsubscribed from {channels}', 'subscribed_channels': list(connection.subscribed_channels)}, timestamp=time.time(), tenant_id=connection.tenant_id, workspace_id=connection.workspace_id)
        await self._send_to_client(client_id, response)
    ...
    async def _handle_ping(self, client_id: str) -> None:
        ...
        response = WebSocketMessage(message_type=MessageType.HEARTBEAT, data={'message': 'pong', 'timestamp': time.time()}, timestamp=time.time(), tenant_id=connection.tenant_id, workspace_id=connection.workspace_id)
        await self._send_to_client(client_id, response)
    ...
    async def broadcast_message(self, message: WebSocketMessage, channel: str | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
    ...
    async def send_agent_status_update(self, agent_id: str, status: str, details: dict[str, Any], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        """Send agent status update to subscribed clients."""
        message = WebSocketMessage(message_type=MessageType.AGENT_STATUS, data={'agent_id': agent_id, 'status': status, 'details': details}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
        return await self.broadcast_message(message, channel='agent_status', tenant_id=tenant_id, workspace_id=workspace_id)

    async def send_analysis_update(self, analysis_id: str, progress: float, results: dict[str, Any], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        """Send analysis progress update to subscribed clients."""
        message = WebSocketMessage(message_type=MessageType.ANALYSIS_UPDATE, data={'analysis_id': analysis_id, 'progress': progress, 'results': results}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
        return await self.broadcast_message(message, channel='analysis', tenant_id=tenant_id, workspace_id=workspace_id)

    async def send_stream_update(self, stream_id: str, content: str, metadata: dict[str, Any], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        """Send live stream update to subscribed clients."""
        message = WebSocketMessage(message_type=MessageType.STREAM_UPDATE, data={'stream_id': stream_id, 'content': content, 'metadata': metadata}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
        return await self.broadcast_message(message, channel='stream', tenant_id=tenant_id, workspace_id=workspace_id)

    async def send_system_alert(self, alert_type: str, severity: str, message_text: str, details: dict[str, Any], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        """Send system alert to subscribed clients."""
        message = WebSocketMessage(message_type=MessageType.SYSTEM_ALERT, data={'alert_type': alert_type, 'severity': severity, 'message': message_text, 'details': details}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
        return await self.broadcast_message(message, channel='alerts', tenant_id=tenant_id, workspace_id=workspace_id)

    async def send_performance_metrics(self, metrics: dict[str, Any], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        """Send performance metrics to subscribed clients."""
        message = WebSocketMessage(message_type=MessageType.PERFORMANCE_METRICS, data={'metrics': metrics}, timestamp=time.time(), tenant_id=tenant_id, workspace_id=workspace_id)
    ...
    async def _heartbeat_task(self) -> None:
        ...
        while self.running:
            try:
                ...
                for client_id, connection in list(self.clients.items()):
                    try:
                        heartbeat = WebSocketMessage(message_type=MessageType.HEARTBEAT, data={'timestamp': current_time}, timestamp=current_time, tenant_id=connection.tenant_id, workspace_id=connection.workspace_id)
    ...
    def get_connection_stats(self) -> dict[str, Any]:
        ...
        current_time = time.time()
        stats: dict[str, Any] = {'total_connections': len(self.clients), 'running': self.running, 'host': self.host, 'port': self.port, 'clients': []}
        for client_id, connection in self.clients.items():
            stats['clients'].append({'client_id': client_id, 'tenant_id': connection.tenant_id, 'workspace_id': connection.workspace_id, 'subscribed_channels': list(connection.subscribed_channels), 'connected_at': connection.connected_at, 'last_heartbeat': connection.last_heartbeat, 'uptime_seconds': current_time - connection.connected_at})
        return stats