...
class EnterpriseAuthService:
    ...
    def _initialize_default_permissions(self) -> StepResult:
        ...
        self.role_permissions = {UserRole.SUPER_ADMIN: {Permission.CREATE_TENANT, Permission.MANAGE_TENANT, Permission.DELETE_TENANT, Permission.CREATE_USER, Permission.MANAGE_USER, Permission.DELETE_USER, Permission.MANAGE_RESOURCES, Permission.VIEW_RESOURCES, Permission.CREATE_AGENT, Permission.MANAGE_AGENT, Permission.DELETE_AGENT, Permission.VIEW_ANALYTICS, Permission.MANAGE_ALERTS, Permission.API_READ, Permission.API_WRITE, Permission.API_ADMIN}, UserRole.TENANT_ADMIN: {Permission.MANAGE_TENANT, Permission.CREATE_USER, Permission.MANAGE_USER, Permission.MANAGE_RESOURCES, Permission.VIEW_RESOURCES, Permission.CREATE_AGENT, Permission.MANAGE_AGENT, Permission.DELETE_AGENT, Permission.VIEW_ANALYTICS, Permission.MANAGE_ALERTS, Permission.API_READ, Permission.API_WRITE}, UserRole.USER: {Permission.VIEW_RESOURCES, Permission.MANAGE_AGENT, Permission.VIEW_ANALYTICS, Permission.API_READ}, UserRole.VIEWER: {Permission.VIEW_RESOURCES, Permission.VIEW_ANALYTICS, Permission.API_READ}, UserRole.API_USER: {Permission.API_READ, Permission.API_WRITE}}

    def create_user(self, email: str, username: str, full_name: str, tenant_id: str, password: str | None=None, roles: set[UserRole] | None=None, auth_provider: AuthProvider=AuthProvider.LOCAL, external_id: str | None=None) -> StepResult:
        """Create a new user."""
        if not self.enabled:
        ...
        try:
            ...
            user = User(user_id=user_id, email=email, username=username, full_name=full_name, tenant_id=tenant_id, roles=roles, auth_provider=auth_provider, external_id=external_id, password_hash=password_hash)
            user.permissions = self._get_permissions_for_roles(roles)
            self.users[user_id] = user
            log.info(f'Created user {user_id} ({email}) for tenant {tenant_id}')
            return StepResult.ok(data={'user_id': user_id, 'email': email, 'username': username, 'tenant_id': tenant_id, 'roles': [role.value for role in roles], 'permissions': [perm.value for perm in user.permissions]})
    ...
    def authenticate_user(self, email: str, password: str, tenant_id: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'user_id': user.user_id, 'email': user.email, 'username': user.username, 'tenant_id': tenant_id, 'session_id': session.session_id, 'token': token, 'expires_at': session.expires_at, 'roles': [role.value for role in user.roles], 'permissions': [perm.value for perm in user.permissions]})
        except Exception as e:
    ...
    def _generate_jwt_token(self, user: User, session: Session) -> StepResult:
        ...
        payload = {'user_id': user.user_id, 'email': user.email, 'tenant_id': user.tenant_id, 'session_id': session.session_id, 'roles': [role.value for role in user.roles], 'permissions': [perm.value for perm in user.permissions], 'iat': int(time.time()), 'exp': int(session.expires_at)}
    ...
    def verify_token(self, token: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'user_id': user.user_id, 'email': user.email, 'username': user.username, 'tenant_id': user.tenant_id, 'session_id': session_id, 'roles': [role.value for role in user.roles], 'permissions': [perm.value for perm in user.permissions]})
    ...
    def create_api_key(self, user_id: str, name: str, description: str, permissions: set[Permission] | None=None, expires_at: float | None=None) -> StepResult:
        ...
        try:
            ...
            api_key = APIKey(key_id=f'key_{int(time.time() * 1000)}_{secrets.token_hex(4)}', key_hash=key_hash, user_id=user_id, tenant_id=user.tenant_id, name=name, description=description, permissions=permissions, expires_at=expires_at, created_by=user_id)
            self.api_keys[api_key.key_id] = api_key
            log.info(f'Created API key {api_key.key_id} for user {user_id}')
            return StepResult.ok(data={'key_id': api_key.key_id, 'key_value': key_value, 'name': name, 'description': description, 'permissions': [perm.value for perm in permissions], 'expires_at': expires_at})
        except Exception as e:
    ...
    def verify_api_key(self, api_key: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'user_id': user.user_id, 'email': user.email, 'username': user.username, 'tenant_id': user.tenant_id, 'key_id': api_key_record.key_id, 'permissions': [perm.value for perm in api_key_record.permissions], 'usage_count': api_key_record.usage_count})
        except Exception as e:
    ...
    def check_permission(self, user_id: str, permission: Permission) -> StepResult:
        ...
        return StepResult.ok(data={'has_permission': has_permission, 'user_id': user_id, 'permission': permission.value, 'user_roles': [role.value for role in user.roles]})
    ...
    def update_user_roles(self, user_id: str, roles: set[UserRole]) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'user_id': user_id, 'old_roles': [role.value for role in old_roles], 'new_roles': [role.value for role in roles], 'new_permissions': [perm.value for perm in user.permissions]})
        except Exception as e:
    ...
    def get_auth_stats(self) -> StepResult:
        ...
        return {'enabled': True, 'total_users': len(self.users), 'active_users': active_users, 'active_sessions': active_sessions, 'total_api_keys': len(self.api_keys), 'active_api_keys': active_api_keys}
