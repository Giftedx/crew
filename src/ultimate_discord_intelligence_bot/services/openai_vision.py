...
class OpenAIVisionService(OpenAIService):
    ...
    async def analyze_image(self, image_data: bytes, prompt: str, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            response = await self.client.chat.completions.create(model=self.model, messages=[{'role': 'user', 'content': [{'type': 'text', 'text': f'{prompt}\nTenant: {tenant}, Workspace: {workspace}'}, {'type': 'image_url', 'image_url': {'url': f'data:image/jpeg;base64,{image_base64}'}}]}], max_tokens=1000, temperature=0.7)
    ...
    async def analyze_multiple_images(self, images: list[bytes], prompt: str, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            response = await self.client.chat.completions.create(model=self.model, messages=[{'role': 'user', 'content': content}], max_tokens=1500, temperature=0.7)
            return StepResult.ok(data={'analysis': response.choices[0].message.content, 'model': self.model, 'image_count': len(images)})
        except Exception as e:
