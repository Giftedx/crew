...
class BillingConfig:
    ...
    def calculate_overage_cost(self, usage: dict[ResourceType, float], quotas: dict[ResourceType, ResourceQuota]) -> StepResult:
...
class TenantInstance:
    ...
    def get_usage_summary(self) -> StepResult:
        ...
        summary = {'tenant_id': self.config.tenant_id, 'current_usage': {}, 'quotas': {}, 'overages': {}, 'usage_percentages': {}, 'last_activity': self.last_activity}
        for resource_type in ResourceType:
            ...
            summary['usage_percentages'][resource_type.value] = current / quota.limit * 100 if quota and quota.limit > 0 else 0.0
        return summary
...
class EnterpriseTenantManager:
    ...
    def _initialize_default_tiers(self) -> StepResult:
        ...
        tier_configs = {TenantTier.FREE: {ResourceType.CPU_UNITS: 100, ResourceType.MEMORY_MB: 1024, ResourceType.GPU_HOURS: 0, ResourceType.STORAGE_GB: 1, ResourceType.API_CALLS: 1000, ResourceType.AGENT_EXECUTIONS: 100, ResourceType.STREAM_HOURS: 0}, TenantTier.BASIC: {ResourceType.CPU_UNITS: 500, ResourceType.MEMORY_MB: 4096, ResourceType.GPU_HOURS: 10, ResourceType.STORAGE_GB: 10, ResourceType.API_CALLS: 10000, ResourceType.AGENT_EXECUTIONS: 1000, ResourceType.STREAM_HOURS: 50}, TenantTier.PROFESSIONAL: {ResourceType.CPU_UNITS: 2000, ResourceType.MEMORY_MB: 16384, ResourceType.GPU_HOURS: 50, ResourceType.STORAGE_GB: 100, ResourceType.API_CALLS: 100000, ResourceType.AGENT_EXECUTIONS: 10000, ResourceType.STREAM_HOURS: 500}, TenantTier.ENTERPRISE: {ResourceType.CPU_UNITS: 10000, ResourceType.MEMORY_MB: 65536, ResourceType.GPU_HOURS: 200, ResourceType.STORAGE_GB: 1000, ResourceType.API_CALLS: 1000000, ResourceType.AGENT_EXECUTIONS: 100000, ResourceType.STREAM_HOURS: 2000}}
    ...
    def create_tenant(self, tenant_config: TenantConfig) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'tenant_id': tenant_config.tenant_id, 'tier': tenant_config.tier.value, 'quotas': {rt.value: q.limit for rt, q in tenant_config.quotas.items()}, 'created_at': tenant_config.created_at})
    ...
    def update_tenant_usage(self, tenant_id: str, resource_type: ResourceType, amount: float) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'tenant_id': tenant_id, 'resource_type': resource_type.value, 'current_usage': tenant.current_usage.get(resource_type, 0.0), 'quota_exceeded': quota_exceeded})
        except Exception as e:
    ...
    def check_tenant_quota(self, tenant_id: str, resource_type: ResourceType, required_amount: float) -> StepResult:
        ...
        return StepResult.ok(data={'sufficient': sufficient, 'available': available, 'required': required_amount, 'quota_limit': quota.limit, 'current_usage': current_usage})
        ...
        try:
            ...
            if new_tier in self.tier_configs:
                ...
                for resource_type, limit in self.tier_configs[new_tier].items():
                    new_quotas[resource_type] = ResourceQuota(resource_type=resource_type, limit=limit, current_usage=tenant.current_usage.get(resource_type, 0.0))
                tenant.config.quotas = new_quotas
            log.info(f'Upgraded tenant {tenant_id} from {old_tier.value} to {new_tier.value}')
            return StepResult.ok(data={'tenant_id': tenant_id, 'old_tier': old_tier.value, 'new_tier': new_tier.value, 'new_quotas': {rt.value: q.limit for rt, q in tenant.config.quotas.items()}})
        except Exception as e:
    ...
    def suspend_tenant(self, tenant_id: str, reason: str='') -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'tenant_id': tenant_id, 'status': 'suspended', 'reason': reason, 'suspended_at': time.time()})
        except Exception as e:
    ...
    def get_all_tenants(self) -> StepResult:
        ...
        try:
            ...
            for tenant_id, tenant in self.tenants.items():
                summary = {'tenant_id': tenant_id, 'name': tenant.config.name, 'tier': tenant.config.tier.value, 'status': tenant.config.status, 'created_at': tenant.config.created_at, 'last_accessed': tenant.last_activity, 'usage_summary': tenant.get_usage_summary()}
    ...
    def get_system_resource_usage(self) -> StepResult:
        ...
        try:
            ...
            for resource_type in ResourceType:
                ...
                system_usage[resource_type.value] = {'total_usage': total_usage, 'total_quota': total_quota, 'utilization_percentage': total_usage / total_quota * 100 if total_quota > 0 else 0.0}
