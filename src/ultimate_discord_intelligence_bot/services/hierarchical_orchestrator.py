...
class HierarchicalOrchestrator:
    ...
    async def create_orchestration_session(
        self, name: str, mission_context: str, objectives: list[dict[str, Any]], tenant: str, workspace: str
    ) -> StepResult:
        """
        ...
        try:
            ...
            session = OrchestrationSession(
                id=session_id, name=name, status=OrchestrationStatus.PLANNING, tenant=tenant, workspace=workspace
            )
    ...
    async def execute_orchestration_session(self, session_id: str) -> StepResult:
        ...
        try:
            ...
            dependency_result = await self.workflow_manager.resolve_dependencies(
                [self._task_to_dict(task) for task in session.tasks], session.tenant, session.workspace
            )
            ...
            routing_result = await self.workflow_manager.route_tasks(
                {"id": session_id, "name": session.name, "tasks": [self._task_to_dict(task) for task in session.tasks]},
                available_agents,
                session.tenant,
                session.workspace,
            )
                ...
                available_agents.append(
                    {
                        ...
                        "max_concurrent_tasks": int(agent.max_capacity * 10),
        ...
        for assignment in assignments:
            ...
            if not assigned_agent:
                execution_results.append(
                    {"task_id": task_id, "status": "failed", "error": "No agent assigned", "assigned_agent": None}
                )
                continue
            ...
            try:
                ...
                task.status = "failed"
                execution_results.append(
                    {"task_id": task_id, "status": "failed", "error": str(e), "assigned_agent": assigned_agent}
                )
