...
SCORERS = {'must_include': score_must_include, 'must_link': score_must_link, 'forbidden': score_forbidden, 'coverage': score_coverage, 'grounded': score_grounded}
...
def run_dataset(dataset_path: Path, runner: Callable[[dict[str, Any]], dict[str, Any]], _seed: int=0) -> StepResult:
    ...
    for line in dataset_path.read_text().splitlines():
        ...
        scores = {name: fn(output, record['expected']) if name != 'grounded' else fn(output, record) for name, fn in SCORERS.items()}
        quality = sum(scores.values()) / len(scores)
        samples.append(SampleResult(record=record, output=output, quality=quality, cost_usd=cost, latency_ms=latency, scores=scores))
    if samples:
...
def main(argv: list[str] | None=None) -> StepResult:
    ...
    ok = compare_to_baseline(result, Path(args.dataset).stem + '_v1', {'quality': 0.5, 'cost_usd': 0.0, 'latency_ms': 50.0})
