...
import logging
from ultimate_discord_intelligence_bot.settings import ENABLE_DISTRIBUTED_RATE_LIMITING, RATE_LIMIT_FALLBACK_TO_LOCAL, RATE_LIMIT_GLOBAL_CAPACITY, RATE_LIMIT_GLOBAL_REFILL_PER_SEC, RATE_LIMIT_REDIS_URL
...
class RateLimitingService:
    ...
    def _initialize_distributed_limiter(self) -> None:
        ...
        if not RATE_LIMIT_REDIS_URL:
            ...
            return
        try:
            self.distributed_limiter = DistributedRateLimiter(redis_url=RATE_LIMIT_REDIS_URL, default_capacity=RATE_LIMIT_GLOBAL_CAPACITY, default_refill_per_sec=RATE_LIMIT_GLOBAL_REFILL_PER_SEC, fallback_to_local=RATE_LIMIT_FALLBACK_TO_LOCAL)
            self.enabled = True
    ...
    def is_allowed(self, key: str, cost: int=1, capacity: int | None=None, refill_per_sec: float | None=None) -> StepResult:
...
def check_rate_limit(key: str, cost: int=1, capacity: int | None=None, refill_per_sec: float | None=None) -> StepResult:
