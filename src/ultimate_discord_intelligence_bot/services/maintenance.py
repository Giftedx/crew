...
class MemoryMaintenance:
    ...
    def compact(self, collections: list[str] | None=None, max_delete: int | None=None) -> StepResult:
        ...
        for name in cols:
            ...
            summaries[name] = {'status': res['status'], 'deleted': res.get('deleted', 0), 'scanned': res.get('scanned', 0), 'remaining': res.get('remaining', 0), 'tenant': f'{getattr(current_tenant(), 'tenant_id', 'default')}:{getattr(current_tenant(), 'workspace_id', 'main')}'}
        return summaries
