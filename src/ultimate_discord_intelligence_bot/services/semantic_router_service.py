...
class SemanticRouterService:
    ...
    def _get_encoder(self):
        if self.settings.OPENROUTER_API_KEY:
            return OpenAIEncoder(openai_api_key=self.settings.OPENROUTER_API_KEY, openai_api_base=self.settings.OPENROUTER_API_BASE)
...
if __name__ == '__main__':
    search_route = Route(name='search', utterances=['search for information about', 'find me details on', 'what is the capital of France?'])
    transcribe_route = Route(name='transcribe_audio', utterances=['transcribe this audio file', 'what is being said in this audio?', 'convert this speech to text'])
