...
class MemoryService:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    vec_cfg = _legacy_models.VectorParams(size=dimension, distance=getattr(_legacy_models.Distance, 'COSINE', 'Cosine'))
                except Exception:
                    vec_cfg = {'size': dimension, 'distance': 'Cosine'}
            try:
                self.qdrant_client.create_collection(collection_name=collection_name, vectors_config=vec_cfg, on_disk_payload=True)
            except TypeError:
    ...
    async def cache_lookup(self, request: str, tenant: str, workspace: str, similarity_threshold: float | None=None) -> StepResult:
        ...
        try:
            ...
            search_result = self.qdrant_client.search(collection_name=collection_name, query_vector=embedding_vector, limit=1, score_threshold=threshold, with_payload=True)
            if search_result:
                hit = search_result[0]
                ...
                if 'timestamp' in payload:
                    ...
                    if age_hours > self.cache_ttl_hours:
                        return StepResult.ok(data=None)
                return StepResult.ok(data={'content': payload.get('content'), 'metadata': payload.get('metadata', {}), 'score': hit.score, 'id': hit.id})
            return StepResult.ok(data=None)
        except Exception as e:
    ...
    async def store_artifact(self, content: str, metadata: dict[str, Any], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            try:
                from qdrant_client.http import models as _qmodels
                point_struct = _qmodels.PointStruct(id=point_id, vector=embedding_result.data, payload={'content': content, 'metadata': metadata, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace})
            except Exception:
                try:
                    from qdrant_client import models as _legacy_models
                    point_struct = _legacy_models.PointStruct(id=point_id, vector=embedding_result.data, payload={'content': content, 'metadata': metadata, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace})
                except Exception:
                    point_struct = {'id': point_id, 'vector': embedding_result.data, 'payload': {'content': content, 'metadata': metadata, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace}}
            self.qdrant_client.upsert(collection_name=collection_name, points=[point_struct])
            logging.debug('Stored artifact (tenant: %s, workspace: %s, content_length: %d)', tenant, workspace, len(content))
    ...
    async def retrieve_context(self, query: str, top_k: int, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            collection_name = self._get_tenant_collection_name(self.artifacts_collection, tenant, workspace)
            embedding_vector: list[float] = embedding_result.data if isinstance(embedding_result.data, list) else []
            search_result = self.qdrant_client.search(collection_name=collection_name, query_vector=embedding_vector, limit=min(top_k, self.max_results), score_threshold=self.similarity_threshold, with_payload=True)
            results = []
            for hit in search_result:
                payload = hit.payload or {}
                results.append({'content': payload.get('content'), 'metadata': payload.get('metadata', {}), 'score': hit.score, 'id': hit.id})
    ...
    async def store_cache_hit(self, request: str, response: dict[str, Any], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            try:
                from qdrant_client.http import models as _qmodels
                point_struct = _qmodels.PointStruct(id=point_id, vector=embedding_result.data, payload={'request': request, 'response': response, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace})
            except Exception:
                try:
                    from qdrant_client import models as _legacy_models
                    point_struct = _legacy_models.PointStruct(id=point_id, vector=embedding_result.data, payload={'request': request, 'response': response, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace})
                except Exception:
                    point_struct = {'id': point_id, 'vector': embedding_result.data, 'payload': {'request': request, 'response': response, 'timestamp': time.time(), 'tenant': tenant, 'workspace': workspace}}
            self.qdrant_client.upsert(collection_name=collection_name, points=[point_struct])
    ...
    def add(self, text: str, metadata: dict[str, Any] | None=None, namespace: str | None=None) -> StepResult:
        ...
        if self.qdrant_client and self.embedding_service:
            task = asyncio.create_task(self._async_store_text(clean_text, metadata or {}, ctx.tenant_id, ctx.workspace_id))
            _ = task
    ...
    def retrieve(self, query: str, limit: int=5, metadata: dict[str, Any] | None=None, namespace: str | None=None) -> StepResult:
