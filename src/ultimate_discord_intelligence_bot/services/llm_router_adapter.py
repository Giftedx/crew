...
class LLMRouterAdapter:
    ...
    def _to_arms(self, models: list[ModelInfo]) -> list[Arm]:
        arms: list[Arm] = []
        for mi in models:
            arms.append(Arm(arm_id=f'{mi.provider}:{mi.model}', provider=mi.provider, model=mi.model, cost_in=mi.cost_per_1k_input, cost_out=mi.cost_per_1k_output, latency_ms=mi.est_latency_ms, quality_tier=mi.quality_tier, capabilities=set(mi.capabilities)))
        return arms
    ...
    def build_arms(self, required_capabilities: set[str] | None=None) -> list[dict[str, Any]]:
        ...
        for a in shortlisted:
            self._bandit.add_arm(arm_id=a.arm_id, name=f'{a.provider}:{a.model}', metadata={'provider': a.provider, 'model': a.model, 'cost_in': a.cost_in, 'cost_out': a.cost_out, 'latency_ms': a.latency_ms, 'quality_tier': a.quality_tier, 'capabilities': sorted(a.capabilities)})
        with contextlib.suppress(Exception):
            get_metrics().histogram('router_shortlist_size', float(len(shortlisted)), labels={'policy': self.policy.value})
        return [{'arm_id': a.arm_id, 'provider': a.provider, 'model': a.model, 'capabilities': sorted(a.capabilities), 'cost_in': a.cost_in, 'cost_out': a.cost_out, 'latency_ms': a.latency_ms, 'quality_tier': a.quality_tier} for a in shortlisted]
    ...
    def select(self, required_capabilities: set[str] | None=None) -> StepResult:
        ...
        result = StepResult.ok(selection={'provider': selected['provider'], 'model': selected['model'], 'arm_id': arm_id, 'policy': self.policy.value}, arms=arms)
        result.metadata.update({'provider': selected['provider'], 'model': selected['model'], 'policy': self.policy.value})
        with contextlib.suppress(Exception):
