...
class ModelRouter:
    ...
    def _initialize_routing_rules(self) -> None:
        ...
        self.routing_rules = [{'name': 'task_complexity_routing', 'description': 'Route based on task complexity', 'enabled': True, 'rules': {'simple': ['gpt-3.5-turbo', 'claude-3-haiku'], 'medium': ['gpt-4', 'claude-3-sonnet'], 'complex': ['gpt-4-turbo', 'claude-3-opus']}}, {'name': 'cost_optimization', 'description': 'Route based on cost optimization', 'enabled': True, 'max_cost_per_request': 0.1, 'fallback_models': ['gpt-3.5-turbo', 'claude-3-haiku']}, {'name': 'performance_based_routing', 'description': 'Route based on performance metrics', 'enabled': True, 'min_success_rate': 0.95, 'max_response_time': 30.0}, {'name': 'load_balancing', 'description': 'Distribute load across available models', 'enabled': True, 'max_concurrent_requests': 10, 'load_balancing_strategy': 'round_robin'}]

    def route_request(self, task_description: str, task_type: str='general', requirements: dict[str, Any] | None=None) -> StepResult:
        ...
        """
        try:
            routing_decision: dict[str, Any] = {'selected_model': None, 'routing_reason': '', 'confidence': 0.0, 'alternatives': [], 'estimated_cost': 0.0, 'estimated_latency': 0.0}
            ...
            if selected_model:
                ...
                routing_decision['alternatives'] = [model['name'] for model in candidate_models[:3] if model['name'] != selected_model['name']]
            ...
            return StepResult.ok(data={'routing_decision': routing_decision, 'task_analysis': {'complexity': complexity, 'type': task_type, 'requirements': requirements or {}}, 'tenant': self.tenant_context.tenant, 'workspace': self.tenant_context.workspace})
    ...
    def _analyze_task_complexity(self, task_description: str, task_type: str) -> str:
        ...
        """
        complexity_indicators = {'simple': ['summarize', 'extract', 'classify', 'simple'], 'medium': ['analyze', 'compare', 'evaluate', 'generate'], 'complex': ['synthesize', 'create', 'design', 'complex', 'advanced']}
    ...
    def _get_available_models(self) -> list[dict[str, Any]]:
        ...
        """
        return [{'name': 'gpt-3.5-turbo', 'provider': 'openai', 'capabilities': ['text', 'chat'], 'max_tokens': 4096, 'cost_per_1k_tokens': 0.002, 'avg_latency': 2.0, 'success_rate': 0.98}, {'name': 'gpt-4', 'provider': 'openai', 'capabilities': ['text', 'chat', 'analysis'], 'max_tokens': 8192, 'cost_per_1k_tokens': 0.03, 'avg_latency': 5.0, 'success_rate': 0.99}, {'name': 'gpt-4-turbo', 'provider': 'openai', 'capabilities': ['text', 'chat', 'analysis', 'vision'], 'max_tokens': 128000, 'cost_per_1k_tokens': 0.01, 'avg_latency': 3.0, 'success_rate': 0.99}, {'name': 'claude-3-haiku', 'provider': 'anthropic', 'capabilities': ['text', 'chat'], 'max_tokens': 200000, 'cost_per_1k_tokens': 0.00025, 'avg_latency': 1.5, 'success_rate': 0.97}, {'name': 'claude-3-sonnet', 'provider': 'anthropic', 'capabilities': ['text', 'chat', 'analysis'], 'max_tokens': 200000, 'cost_per_1k_tokens': 0.003, 'avg_latency': 3.0, 'success_rate': 0.98}, {'name': 'claude-3-opus', 'provider': 'anthropic', 'capabilities': ['text', 'chat', 'analysis', 'reasoning'], 'max_tokens': 200000, 'cost_per_1k_tokens': 0.015, 'avg_latency': 8.0, 'success_rate': 0.99}]

    def _apply_routing_rules(self, available_models: list[dict[str, Any]], complexity: str, task_type: str, requirements: dict[str, Any] | None) -> list[dict[str, Any]]:
        ...
        if cost_rule and cost_rule['enabled']:
            ...
            candidates = [model for model in candidates if model['cost_per_1k_tokens'] * estimated_tokens / 1000 <= max_cost]
        performance_rule = next((rule for rule in self.routing_rules if rule['name'] == 'performance_based_routing'), None)
        if performance_rule and performance_rule['enabled']:
            ...
            candidates = [model for model in candidates if model['success_rate'] >= min_success_rate and model['avg_latency'] <= max_response_time]
        return candidates

    def _select_optimal_model(self, candidate_models: list[dict[str, Any]], requirements: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        for model in candidate_models:
            ...
            scored_models.append({'name': model['name'], 'score': score, 'reason': '; '.join(reasons), 'confidence': min(1.0, score / 2.0), 'estimated_cost': model['cost_per_1k_tokens'], 'estimated_latency': model['avg_latency']})
    ...
    def update_model_performance(self, model_name: str, success: bool, latency: float, cost: float) -> StepResult:
        ...
        try:
            if model_name not in self.model_performance:
                self.model_performance[model_name] = {'total_requests': 0, 'successful_requests': 0, 'total_latency': 0.0, 'total_cost': 0.0, 'avg_latency': 0.0, 'success_rate': 0.0, 'avg_cost': 0.0}
            ...
            return StepResult.ok(data={'model_name': model_name, 'updated_performance': perf, 'tenant': self.tenant_context.tenant, 'workspace': self.tenant_context.workspace})
        except Exception as e:
    ...
    def get_routing_statistics(self) -> StepResult:
        ...
        """
        try:
            stats = {'routing_rules': self.routing_rules, 'model_performance': self.model_performance, 'cost_metrics': self.cost_metrics, 'tenant': self.tenant_context.tenant, 'workspace': self.tenant_context.workspace, 'timestamp': time.time()}
...
class ModelRoutingManager:
    ...
    def route_request(self, tenant_context: TenantContext, task_description: str, task_type: str='general', requirements: dict[str, Any] | None=None) -> StepResult:
...
def route_model_request(tenant_context: TenantContext, task_description: str, task_type: str='general', requirements: dict[str, Any] | None=None) -> StepResult:
