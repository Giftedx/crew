...
class AsyncConnectionPool:
    ...
    async def post(
        self, url: str, headers: dict[str, str] | None = None, json_data: dict[str, Any] | None = None, **kwargs: Any
    ...
    async def execute_online_async(self, state: RouteState) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self._connection_pool.post(
                url="https://openrouter.ai/api/v1/chat/completions", headers=headers, json_data=payload
            )
            if response is None:
                return {
                    "status": "error",
