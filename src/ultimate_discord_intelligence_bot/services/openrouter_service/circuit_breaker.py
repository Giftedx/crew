...
class CircuitBreaker:
    ...
    def __init__(self, name: str, config: CircuitBreakerConfig | None=None) -> None:
        ...
        self._stats = {'total_requests': 0, 'successful_requests': 0, 'failed_requests': 0, 'circuit_opens': 0, 'circuit_closes': 0, 'requests_blocked': 0, 'half_open_attempts': 0}
    ...
    def execute(self, func: Callable[[], Any]) -> Any:
        ...
        if not self._can_execute():
            raise CircuitBreakerOpenException(f'Circuit breaker {self._name} is OPEN. Last failure: {self._last_failure_time}, Recovery timeout: {self._config.recovery_timeout}s')
        try:
            result = func()
            self._on_success()
            return result
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {**self._stats, 'state': self._state.value, 'failure_count': self._failure_count, 'success_count': self._success_count, 'success_rate_percent': round(success_rate, 2), 'last_failure_time': self._last_failure_time, 'last_success_time': self._last_success_time, 'config': {'failure_threshold': self._config.failure_threshold, 'recovery_timeout': self._config.recovery_timeout, 'success_threshold': self._config.success_threshold, 'timeout': self._config.timeout, 'enable_circuit_breaker': self._config.enable_circuit_breaker}}
...
class CircuitBreakerManager:
    ...
    def get_openrouter_breaker(self) -> CircuitBreaker:
        ...
        """
        config = CircuitBreakerConfig(failure_threshold=5, recovery_timeout=60, success_threshold=3, timeout=30, enable_circuit_breaker=self._feature_flags.ENABLE_OPENROUTER_CIRCUIT_BREAKER)
...
class OpenRouterCircuitBreaker:
    ...
    def route_with_circuit_breaker(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None, **kwargs: Any) -> StepResult:
    ...
    async def route_async_with_circuit_breaker(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'error', 'error': f'Service temporarily unavailable: {e!s}', 'model': model or 'unknown', 'tokens': 0, 'provider': provider_opts or {}}
        except Exception as e:
            log.error('Async routing failed: %s', e)
            return {'status': 'error', 'error': f'Routing failed: {e!s}', 'model': model or 'unknown', 'tokens': 0, 'provider': provider_opts or {}}
