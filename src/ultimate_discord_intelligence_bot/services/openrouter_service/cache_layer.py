...
def check_caches(service: OpenRouterService, state: RouteState) -> dict[str, Any] | None:
            ...
            if "error" not in holder:
                ...
                if sem_res is not None:
                        ...
                        cache_meta_sem.update(
                            {
                                ...
                                "cache_type": "semantic",
                                "similarity": float(
                                    sem_res.get("similarity", 0.0) if isinstance(sem_res, dict) else 0.0
                                ),
                            }
                        )
                    except Exception:
                    ...
                    if shadow_mode:
                        ...
                        ) and sim_val >= float(getattr(service, "semantic_cache_promotion_threshold", 0.9))
                        if promote:
    ...
    if service.cache:
        ...
        cache_key = make_key(f"{task_type}|{norm_prompt}|provider={provider_sig}", chosen)
