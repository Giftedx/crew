...
try:
    from platform.cache.semantic_cache import get_semantic_cache as _default_semantic_cache_get
...
class OpenRouterService:
    ...
    def __init__(self, models_map: dict[str, list[str]] | None=None, learning_engine: LearningEngine | None=None, api_key: str | None=None, provider_opts: dict[str, Any] | None=None, logger: AnalyticsStore | None=None, token_meter: TokenMeter | None=None, cache: Any | None=None, tenant_registry: TenantRegistry | None=None) -> None:
        config = get_config()
        settings = get_settings()
        env_general = config.get_setting('openrouter_general_model')
        env_analysis = config.get_setting('openrouter_analysis_model')
        default_map = {'general': [env_general or 'openai/gpt-4o-mini'], 'analysis': [env_analysis or env_general or 'openai/gpt-4o-mini']}
        if models_map:
                ...
                if ENABLE_CACHE_V2:
                    ...
                    if DistributedLLMCache is not None:
                        try:
                            ...
                            self.cache = cache_ctor(url=str(cfg.rate_limit_redis_url), ttl=get_unified_cache_config().get_ttl_for_domain('llm'), tenant=tenant_id, workspace=workspace_id)
                        except Exception as dist_exc:
                            log.debug('Falling back to in-memory cache (Distributed init failed): %s', dist_exc)
                            self.cache = None
                    if self.cache is None:
                        self.cache = BoundedLRUCache(max_size=int(getattr(cfg, 'cache_max_entries', 1000)), ttl=get_unified_cache_config().get_ttl_for_domain('llm'), name='openrouter')
            except Exception as cache_root_exc:
        ...
        try:
            ...
            if gptcache_flag:
                try:
                    ...
                    self.semantic_cache = TenantSemanticCache(cache_root=str(cache_root), similarity_threshold=threshold, fallback_ttl_seconds=ttl)
                    self.semantic_cache_mode = 'tenant'
                    enabled_sem = True
                except Exception as tenant_cache_exc:
                    log.warning('Tenant semantic cache init failed (%s); falling back to global cache', tenant_cache_exc)
                    self.semantic_cache = None
            if self.semantic_cache is None and enabled_sem and factory:
                try:
                    self.semantic_cache = factory()
                    self.semantic_cache_mode = 'global'
                except Exception as global_sem_exc:
                    log.debug('Global semantic cache initialisation failed: %s', global_sem_exc)
                    self.semantic_cache = None
        ...
        if ENABLE_RL_MODEL_ROUTING:
            try:
                ...
                log.warning('RL Model Router initialization failed, falling back to standard routing: %s', rl_exc)
                self.rl_router = None
        ...
        if LLMRouterSingleton.is_enabled():
            try:
                ...
                log.warning('LiteLLM Router initialization failed, falling back to standard routing: %s', lite_exc)
                self.litellm_router = None
    ...
    def route(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None, compress: bool=True) -> dict[str, Any]:
        from .workflow import route_prompt
        result = route_prompt(self, prompt, task_type=task_type, model=model, provider_opts=provider_opts, compress=compress)
        self._adaptive_flush_events()
        return result

    def _adaptive_record_outcome(self, state: RouteState, *, reward: float, status: str, metadata: dict[str, Any] | None=None) -> None:
        ...
        record_metadata: dict[str, Any] = {'status': status, 'model': state.chosen_model, 'chosen_model': state.chosen_model, 'suggested_model': suggested, 'requested_model': state.requested_model, 'provider_family': state.provider_family, 'override': bool(suggested and suggested != state.chosen_model), 'rl_router_used': state.rl_router_selection is not None}
        ...
        try:
            ...
            try:
                labels = state.labels()
                metrics.ACTIVE_BANDIT_POLICY.labels(tenant=labels.get('tenant', 'unknown'), workspace=labels.get('workspace', 'unknown'), domain=state.task_type, policy='ax_adaptive').set(0.0)
            except Exception:
                pass
