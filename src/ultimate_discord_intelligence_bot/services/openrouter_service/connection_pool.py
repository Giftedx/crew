...
class ConnectionPool:
        ...
        session.headers.update(
            {
                ...
                "Keep-Alive": f"timeout={self._keepalive}, max=1000",
            }
        )
    ...
    def _record_connection_reuse_metric(self) -> None:
        ...
        if self._metrics:
            self._metrics.counter(
                "openrouter_connection_pool_reuses_total",
    ...
    def post(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Route through central HTTP facade while preserving pooled session
            response = http_utils.retrying_post(
                url,
                json_payload=json_payload,
                headers=headers,
                timeout_seconds=timeout,
                # Force use of this pool's session for connection reuse
        ...
        **kwargs: Any,
        ...
        with self._lock:
            if service_name not in self._pools:
                # Use feature flag to determine if pooling is enabled
                if not self._feature_flags.ENABLE_OPENROUTER_CONNECTION_POOLING:
                    # Return a mock pool that doesn't actually pool connections
                    self._pools[service_name] = MockConnectionPool()  # type: ignore
                else:
