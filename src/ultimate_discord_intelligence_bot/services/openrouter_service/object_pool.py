...
class ObjectPool(Generic[T]):
    ...
    def __init__(self, factory: callable[[], T], max_size: int=10, reset_func: callable[[T], None] | None=None) -> None:
        ...
        self._stats = {'objects_created': 0, 'objects_reused': 0, 'pool_hits': 0, 'pool_misses': 0, 'current_pool_size': 0}
    ...
    def reset_stats(self) -> None:
        ...
        with self._lock:
            self._stats = {'objects_created': 0, 'objects_reused': 0, 'pool_hits': 0, 'pool_misses': 0, 'current_pool_size': len(self._pool)}
...
class RequestResponsePool:
    ...
    def __init__(self, max_size: int=50) -> None:
        ...
        """
        self._pool = ObjectPool(factory=self._create_response_dict, max_size=max_size, reset_func=self._reset_response_dict)
        self._feature_flags = FeatureFlags()
    ...
    def _reset_response_dict(self, response_dict: dict[str, Any]) -> None:
        ...
        try:
            response_dict.clear()
            response_dict.update({'status': 'success', 'model': '', 'response': '', 'tokens': 0, 'provider': {}, 'cached': False})
        except Exception as e:
...
class MetricsPool:
    ...
    def __init__(self, max_size: int=30) -> None:
        ...
        """
        self._pool = ObjectPool(factory=self._create_metrics_dict, max_size=max_size, reset_func=self._reset_metrics_dict)
        self._feature_flags = FeatureFlags()

    def _create_metrics_dict(self) -> dict[str, Any]:
        """Create a new metrics dictionary."""
        return {'timestamp': time.time(), 'latency_ms': 0.0, 'tokens_in': 0, 'tokens_out': 0, 'cost': 0.0, 'success': False, 'model': '', 'task_type': ''}

    def _reset_metrics_dict(self, metrics_dict: dict[str, Any]) -> None:
        """Reset a metrics dictionary for reuse."""
        try:
            metrics_dict.clear()
            metrics_dict.update({'timestamp': time.time(), 'latency_ms': 0.0, 'tokens_in': 0, 'tokens_out': 0, 'cost': 0.0, 'success': False, 'model': '', 'task_type': ''})
        except Exception as e:
...
class ObjectPoolManager:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        total_created = route_stats['objects_created'] + response_stats['objects_created'] + metrics_stats['objects_created']
        total_reused = route_stats['objects_reused'] + response_stats['objects_reused'] + metrics_stats['objects_reused']
        ...
        return {'overall_stats': {'total_objects_created': total_created, 'total_objects_reused': total_reused, 'overall_reuse_rate_percent': round(overall_reuse_rate, 2), 'memory_savings_kb': round(memory_savings_kb, 2), 'object_pooling_enabled': self._feature_flags.ENABLE_OPENROUTER_OBJECT_POOLING}, 'route_state_pool': route_stats, 'response_dict_pool': response_stats, 'metrics_dict_pool': metrics_stats}
