...
def execute_offline(service: OpenRouterService, state: RouteState) -> dict[str, Any]:
    ...
    service._adaptive_record_outcome(
        ...
        reward=reward,
        status="success",
        metadata={"latency_ms": latency_ms, "tokens_out": tokens_out, "execution_mode": "offline", "cache_hit": False},
    )
    ...
    def _retry_after_compression(reason: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            new_text, meta = service.prompt_engine.optimise_with_metadata(
                state.prompt, target_token_reduction=target_reduction, max_tokens=max_tokens, force_enable=True
            )
            ...
            retry_resp = _call_resilient_post(
                url, headers=headers, json_payload=retry_payload, timeout_seconds=REQUEST_TIMEOUT_SECONDS
            )
    ...
    if is_retry_enabled():
        try:
            ...
            service._adaptive_record_outcome(
                ...
                reward=0.0,
                status="error",
                metadata={"stage": "http_retry", "error": str(retry_exc), "latency_ms": latency_ms},
            )
            return error
    else:
...
def handle_failure(service: OpenRouterService, state: RouteState, exc: Exception) -> dict[str, Any]:
    ...
    state.error = error
    service._adaptive_record_outcome(
        state, reward=0.0, status="exception", metadata={"error": str(exc), "latency_ms": latency_ms}
    )
    return error
...
def _charge_tracker(state: RouteState) -> None:
    ...
    if tracker:
        try:
            ...
            log.debug(
                "request budget charge failed task=%s model=%s err=%s", state.task_type, state.chosen_model, charge_exc
            )


def _compute_reward(
    service: OpenRouterService, state: RouteState, latency_ms: float, quality_score: float | None = None
) -> float:
    settings = get_settings()
    rl: dict[str, Any] = {}
