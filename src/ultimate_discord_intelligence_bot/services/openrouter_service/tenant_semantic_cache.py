...
class TenantSemanticCache(SemanticCacheInterface):
    ...
    def __init__(self, *, cache_root: str | None=None, similarity_threshold: float | None=None, fallback_enabled: bool=True, fallback_ttl_seconds: int | None=None, factory: SemanticCacheFactory | None=None, sanitise: SanitiseNamespaceFn | None=None) -> None:
        config = get_config()
    ...
    def _get_cache(self, namespace: str | None) -> SemanticCacheInterface:
        ...
        with self._lock:
            ...
            if cache is None:
                ...
                from importlib.util import find_spec
                if find_spec('gptcache') is not None:
                    cache = self._factory(similarity_threshold=self._similarity_threshold or 0.8, cache_dir=str(cache_dir), fallback_enabled=self._fallback_enabled, fallback_ttl_seconds=self._fallback_ttl_seconds or 3600)
                else:
                    cache = self._factory(similarity_threshold=self._similarity_threshold or 0.8, max_cache_size=1000, cache_ttl_seconds=self._fallback_ttl_seconds or 3600, enable_embeddings=True, enable_tfidf=True, namespace=key)
