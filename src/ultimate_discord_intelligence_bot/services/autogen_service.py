...
class AutoGenDiscordService:
    ...
    def __init__(self, get_input_func: Callable[[str], Coroutine[Any, Any, str]], send_message_func: Callable[[str], Coroutine[Any, Any, None]]):
        """
    ...
    async def run_interactive_analysis(self, initial_task: str) -> StepResult:
        ...
        try:
            ...
            analyst_agent = AssistantAgent(name='content_analyst', system_message="You are a world-class content analyst. Ask clarifying questions to refine the task. When you have enough information, provide a detailed analysis. End your final response with 'ANALYSIS COMPLETE'.", llm_config=self.llm_config)
...
async def example_discord_command_handler(message, initial_task):
    ...
    autogen_service = AutoGenDiscordService(get_input_func=get_input_from_discord, send_message_func=send_message_to_discord)
