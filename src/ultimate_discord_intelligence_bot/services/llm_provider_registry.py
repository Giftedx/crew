...
if TYPE_CHECKING:  # typing-only imports (standard library group)
...
class ModelInfo:
    provider: str
    model: str
    # Capabilities are lower_snake_case tokens such as: text, function_calling,
    # vision, audio, json_schema, long_context, tool_use, multi_modal
    capabilities: set[str] = field(default_factory=set)
    context_window: int | None = None
    cost_per_1k_input: float | None = None
    cost_per_1k_output: float | None = None
    est_latency_ms: int | None = None
    quality_tier: str | None = None  # e.g., top, high, balanced, fast, cheap
...
class ProviderAdapter:
    ...
    # Optional hooks for later integration; return fail to indicate unimplemented
    ...
    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        return []
...
class OpenAIAdapter(ProviderAdapter):
    ...
    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        yield ModelInfo(
            ...
            cost_per_1k_input=5.0 / 1_000,  # $5 per 1M tokens expressed per 1k
            cost_per_1k_output=15.0 / 1_000,
            est_latency_ms=900,
            quality_tier="top",
        )
        yield ModelInfo(
            ...
            context_window=128000,
            cost_per_1k_input=0.3 / 1_000,
            cost_per_1k_output=1.2 / 1_000,
            est_latency_ms=600,
...
class AnthropicAdapter(ProviderAdapter):
    ...
    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        ...
        yield ModelInfo(
            ...
            capabilities={"text"},
            context_window=200_000,
            cost_per_1k_input=0.25 / 1_000,
            cost_per_1k_output=1.25 / 1_000,
            est_latency_ms=500,
            quality_tier="fast",
        )
...
class GoogleAdapter(ProviderAdapter):
    ...
    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        yield ModelInfo(
            ...
            cost_per_1k_input=3.5 / 1_000,
            cost_per_1k_output=10.5 / 1_000,
            est_latency_ms=1200,
            quality_tier="top",
        )
...
class MistralAdapter(ProviderAdapter):
    ...
    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        yield ModelInfo(
            ...
            context_window=128000,
            cost_per_1k_input=2.0 / 1_000,
            cost_per_1k_output=6.0 / 1_000,
            est_latency_ms=800,
            quality_tier="balanced",
        )
...
class OpenRouterAdapter(ProviderAdapter):
    name = "openrouter"  # meta-provider; presents best-of routes

    def _models(self) -> Iterable[ModelInfo]:  # pragma: no cover - data only
        # Treat OpenRouter as offering a curated virtual model list for routing
        yield ModelInfo(
            provider=self.name,
            model="meta-best",
            capabilities={"text", "function_calling", "json_schema"},
...
def get_provider_allowlist_from_config() -> list[str] | None:
    ...
    try:
        ...
        # When a list has been parsed by pydantic already
