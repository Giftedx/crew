...
class CreatorEvaluationHarness:
    ...
    def __init__(self):
        ...
        self.metrics_thresholds = {'wer_max': 0.12, 'der_max': 0.2, 'cost_max_usd': 2.0, 'latency_max_sec': 600, 'claim_f1_min': 0.8, 'source_r3_min': 0.8}
    ...
    def run_evaluation(self, gold_dataset_path: str | None=None) -> StepResult:
        ...
        try:
            ...
            report = EvaluationReport(report_id=f'eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}', evaluation_timestamp=datetime.now(), dataset_version='1.0', episodes_evaluated=len([r for r in episode_results if r.success]), metrics=metrics, episode_results=episode_results, trends=trends, regression_alerts=alerts, improvement_opportunities=improvements, summary=summary, recommendations=recommendations)
            report_path = self._save_evaluation_report(report)
            self._display_evaluation_results(report)
            return StepResult.ok(data={'report_id': report.report_id, 'episodes_evaluated': report.episodes_evaluated, 'overall_passed': self._check_overall_acceptance(metrics), 'metrics': self._serialize_metrics(metrics), 'report_path': report_path})
    ...
    def _evaluate_episode(self, annotation: Any) -> StepResult:
        ...
        try:
            ...
            return EvaluationEpisode(episode_id=episode_id, platform=platform, creator=creator, duration_seconds=duration, wer=wer, cer=cer, der=der, jer=jer, topic_seg_rouge=topic_seg_rouge, topic_seg_nmi=topic_seg_nmi, claim_extraction_f1=claim_extraction_f1, claim_extraction_precision=claim_extraction_precision, claim_extraction_recall=claim_extraction_recall, source_retrieval_r3=source_retrieval_r3, source_retrieval_ndcg=source_retrieval_ndcg, highlight_kendall_tau=highlight_kendall_tau, highlight_precision=highlight_precision, safety_classification_f1=safety_classification_f1, dedup_precision=dedup_precision, dedup_recall=dedup_recall, cost_usd=cost_usd, latency_seconds=latency_seconds, success=True)
        except Exception as e:
            if isinstance(annotation, dict):
            ...
            return EvaluationEpisode(episode_id=episode_id, platform=platform, creator=creator, duration_seconds=duration, wer=1.0, cer=1.0, der=1.0, jer=1.0, topic_seg_rouge=0.0, topic_seg_nmi=0.0, claim_extraction_f1=0.0, claim_extraction_precision=0.0, claim_extraction_recall=0.0, source_retrieval_r3=0.0, source_retrieval_ndcg=0.0, highlight_kendall_tau=0.0, highlight_precision=0.0, safety_classification_f1=0.0, dedup_precision=0.0, dedup_recall=0.0, cost_usd=0.0, latency_seconds=0.0, success=False, error_message=str(e))
    ...
    def _calculate_aggregate_metrics(self, episode_results: list[EvaluationEpisode]) -> StepResult:
        ...
        if not successful_results:
            return EvaluationMetrics(wer=1.0, cer=1.0, der=1.0, jer=1.0, topic_seg_rouge=0.0, topic_seg_nmi=0.0, claim_extraction_f1=0.0, claim_extraction_precision=0.0, claim_extraction_recall=0.0, source_retrieval_r3=0.0, source_retrieval_ndcg=0.0, highlight_kendall_tau=0.0, highlight_precision=0.0, safety_classification_f1=0.0, dedup_precision=0.0, dedup_recall=0.0, avg_cost_usd=0.0, avg_latency_seconds=0.0, throughput_per_hour=0.0)
        ...
        return EvaluationMetrics(wer=wer_avg, cer=cer_avg, der=der_avg, jer=jer_avg, topic_seg_rouge=topic_seg_rouge_avg, topic_seg_nmi=topic_seg_nmi_avg, claim_extraction_f1=claim_f1_avg, claim_extraction_precision=claim_precision_avg, claim_extraction_recall=claim_recall_avg, source_retrieval_r3=source_r3_avg, source_retrieval_ndcg=source_ndcg_avg, highlight_kendall_tau=highlight_tau_avg, highlight_precision=highlight_precision_avg, safety_classification_f1=safety_f1_avg, dedup_precision=dedup_precision_avg, dedup_recall=dedup_recall_avg, avg_cost_usd=cost_avg, avg_latency_seconds=latency_avg, throughput_per_hour=throughput)

    def _analyze_trends(self, episode_results: list[EvaluationEpisode]) -> StepResult:
        """Analyze trends in evaluation results."""
        trends = {'wer_trend': 'stable', 'der_trend': 'stable', 'cost_trend': 'stable', 'latency_trend': 'stable', 'claim_f1_trend': 'stable', 'source_r3_trend': 'stable'}
        successful_results = [r for r in episode_results if r.success]
    ...
    def _generate_regression_alerts(self, metrics: EvaluationMetrics, trends: dict[str, Any]) -> StepResult:
        ...
        if metrics.avg_cost_usd > self.metrics_thresholds['cost_max_usd']:
            alerts.append(f'Avg cost (${metrics.avg_cost_usd:.2f}) exceeds threshold (${self.metrics_thresholds['cost_max_usd']:.2f})')
        if metrics.avg_latency_seconds > self.metrics_thresholds['latency_max_sec']:
            alerts.append(f'Avg latency ({metrics.avg_latency_seconds:.0f}s) exceeds threshold ({self.metrics_thresholds['latency_max_sec']}s)')
    ...
    def _identify_improvement_opportunities(self, metrics: EvaluationMetrics, episode_results: list[EvaluationEpisode]) -> StepResult:
    ...
    def _generate_evaluation_summary(self, metrics: EvaluationMetrics, episode_results: list[EvaluationEpisode]) -> StepResult:
    ...
    def _check_overall_acceptance(self, metrics: EvaluationMetrics) -> StepResult:
        ...
        return metrics.wer <= self.metrics_thresholds['wer_max'] and metrics.der <= self.metrics_thresholds['der_max'] and (metrics.avg_cost_usd <= self.metrics_thresholds['cost_max_usd']) and (metrics.avg_latency_seconds <= self.metrics_thresholds['latency_max_sec']) and (metrics.claim_extraction_f1 >= self.metrics_thresholds['claim_f1_min']) and (metrics.source_retrieval_r3 >= self.metrics_thresholds['source_r3_min'])
    ...
    def _save_evaluation_report(self, report: EvaluationReport) -> StepResult:
        ...
        report_data = {'report_id': report.report_id, 'evaluation_timestamp': report.evaluation_timestamp.isoformat(), 'dataset_version': report.dataset_version, 'episodes_evaluated': report.episodes_evaluated, 'metrics': self._serialize_metrics(report.metrics), 'trends': report.trends, 'regression_alerts': report.regression_alerts, 'improvement_opportunities': report.improvement_opportunities, 'summary': report.summary, 'recommendations': report.recommendations, 'episode_results': [{'episode_id': r.episode_id, 'wer': r.wer, 'der': r.der, 'cost_usd': r.cost_usd, 'latency_seconds': r.latency_seconds, 'success': r.success, 'error_message': r.error_message} for r in report.episode_results]}
    ...
    def _serialize_metrics(self, metrics: EvaluationMetrics) -> StepResult:
        ...
        return {'wer': metrics.wer, 'cer': metrics.cer, 'der': metrics.der, 'jer': metrics.jer, 'topic_seg_rouge': metrics.topic_seg_rouge, 'topic_seg_nmi': metrics.topic_seg_nmi, 'claim_extraction_f1': metrics.claim_extraction_f1, 'claim_extraction_precision': metrics.claim_extraction_precision, 'claim_extraction_recall': metrics.claim_extraction_recall, 'source_retrieval_r3': metrics.source_retrieval_r3, 'source_retrieval_ndcg': metrics.source_retrieval_ndcg, 'highlight_kendall_tau': metrics.highlight_kendall_tau, 'highlight_precision': metrics.highlight_precision, 'safety_classification_f1': metrics.safety_classification_f1, 'dedup_precision': metrics.dedup_precision, 'dedup_recall': metrics.dedup_recall, 'avg_cost_usd': metrics.avg_cost_usd, 'avg_latency_seconds': metrics.avg_latency_seconds, 'throughput_per_hour': metrics.throughput_per_hour}
    ...
    def _display_evaluation_results(self, report: EvaluationReport) -> StepResult:
        ...
        print('.1f')
        print('\n🎯 Acceptance Criteria:')
        acceptance_checks = [(f'WER ≤ {self.metrics_thresholds['wer_max']:.1%}', metrics.wer <= self.metrics_thresholds['wer_max']), (f'DER ≤ {self.metrics_thresholds['der_max']:.1%}', metrics.der <= self.metrics_thresholds['der_max']), (f'Cost ≤ ${self.metrics_thresholds['cost_max_usd']:.2f}', metrics.avg_cost_usd <= self.metrics_thresholds['cost_max_usd']), (f'Latency ≤ {self.metrics_thresholds['latency_max_sec']}s', metrics.avg_latency_seconds <= self.metrics_thresholds['latency_max_sec']), (f'Claim F1 ≥ {self.metrics_thresholds['claim_f1_min']:.1%}', metrics.claim_extraction_f1 >= self.metrics_thresholds['claim_f1_min']), (f'Source R@3 ≥ {self.metrics_thresholds['source_r3_min']:.1%}', metrics.source_retrieval_r3 >= self.metrics_thresholds['source_r3_min'])]
