...
class EnhancedOpenRouterService:
    ...
    def __init__(self, models_map: dict[str, list[str]] | None=None, learning_engine: LearningEngine | None=None, provider_configs: list[ProviderConfig] | None=None, cache: LLMCacheProto | None=None, token_meter: TokenMeter | None=None, tenant_registry: TenantRegistry | None=None, logger: AnalyticsStore | None=None, fallback_to_openrouter: bool=True):
        ...
        try:
            self.semantic_cache = get_semantic_cache() if getattr(self.settings, 'enable_semantic_cache', False) else None
        except Exception:
        ...
        if self.fallback_to_openrouter:
            ...
            self._fallback_service: OpenRouterService | None = OpenRouterService(models_map=models_map, learning_engine=learning_engine, cache=cache, token_meter=token_meter, tenant_registry=tenant_registry, logger=logger)
        else:
    ...
    def _create_default_providers(self) -> StepResult:
        ...
        providers = [ProviderConfig(name='openai', models=['gpt-4o', 'gpt-4o-mini', 'gpt-3.5-turbo'], priority=1, cost_multiplier=1.0), ProviderConfig(name='anthropic', models=['claude-3-5-sonnet-20241022', 'claude-3-haiku-20240307'], priority=2, cost_multiplier=0.9), ProviderConfig(name='google', models=['gemini-1.5-pro', 'gemini-1.5-flash'], priority=3, cost_multiplier=0.8), ProviderConfig(name='azure', models=['azure/gpt-4o', 'azure/gpt-35-turbo'], priority=4, cost_multiplier=1.1), ProviderConfig(name='openrouter', models=['openrouter/auto'], priority=5, cost_multiplier=1.2)]
        enabled_providers = []
    ...
    def _build_enhanced_model_map(self, models_map: dict[str, list[str]] | None) -> StepResult:
        ...
        base_map = {'general': ['openai/gpt-4o-mini', 'anthropic/claude-3-haiku-20240307', 'google/gemini-1.5-flash'], 'analysis': ['openai/gpt-4o', 'anthropic/claude-3-5-sonnet-20241022', 'google/gemini-1.5-pro'], 'creative': ['anthropic/claude-3-5-sonnet-20241022', 'openai/gpt-4o', 'google/gemini-1.5-pro'], 'fast': ['openai/gpt-4o-mini', 'google/gemini-1.5-flash', 'anthropic/claude-3-haiku-20240307']}
        if models_map:
    ...
    def _setup_cache(self) -> StepResult:
        ...
        try:
            if getattr(self.config, 'enable_cache_global', True) and getattr(self.config, 'rate_limit_redis_url', None):
                return RedisLLMCache(url=str(self.config.rate_limit_redis_url), ttl=int(getattr(self.config, 'cache_ttl_llm', 3600)))
        except Exception as e:
    ...
    def _on_litellm_success(self, kwargs, completion_response, start_time, end_time):
        ...
        try:
            if _completion_cost is not None and completion_response is not None:
                ...
                metrics.LLM_ESTIMATED_COST.labels(**metrics.label_ctx(), model=model, provider=self._extract_provider_from_model(model)).observe(cost)
        except Exception as e:
    ...
    async def route_async(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None, max_tokens: int=2048, temperature: float=0.8) -> StepResult:
        """Asynchronous routing with LiteLLM integration."""
        start_time = time.perf_counter()
        self.stats.total_requests += 1
        with self.observability.trace_llm_call(model=model or 'auto-select', provider='multi-provider', task_type=task_type, metadata={'max_tokens': max_tokens, 'temperature': temperature, **{k: v for k, v in (provider_opts or {}).items() if isinstance(v, str | int | float | bool)}}) as trace_context:
            try:
                ...
                if self.semantic_cache is not None:
                    semantic_cached = await self.semantic_cache.get(prompt=prompt, model=chosen_model, temperature=temperature, max_tokens=max_tokens, namespace=ns)
                if semantic_cached is not None:
                    ...
                    try:
                        provider = self._extract_provider_from_model(chosen_model)
                        metrics.LLM_CACHE_MISSES.labels(**metrics.label_ctx(), model=chosen_model, provider=provider).inc()
                    except Exception:
                        pass
                cache_key = None
                ...
                if not self._check_budget_limits(projected_cost, task_type):
                    return {'status': 'error', 'error': 'projected cost exceeds budget limit', 'model': chosen_model, 'tokens': tokens_in}
                messages = [{'role': 'user', 'content': prompt}]
                if LITELLM_AVAILABLE:
                    result = await self._call_litellm(model=chosen_model, messages=messages, max_tokens=max_tokens, temperature=temperature, task_type=task_type)
                elif self._fallback_service:
                    result = self._fallback_service.route(prompt=prompt, task_type=task_type, model=model, provider_opts=provider_opts)
                else:
                    raise RuntimeError('No LLM routing service available')
                if result.get('status') == 'success':
                    if self.semantic_cache is not None:
                        await self.semantic_cache.set(prompt=prompt, model=chosen_model, response=result, temperature=temperature, max_tokens=max_tokens, namespace=ns)
                    if self.cache is not None and cache_key:
                        self.cache.set(cache_key, result)
                    tokens_out = result.get('tokens_out', 0)
                    actual_cost = result.get('cost', projected_cost)
                    self.observability.log_llm_interaction(run_context=trace_context, prompt=prompt, response=result.get('response', ''), tokens_input=tokens_in, tokens_output=tokens_out, cost=actual_cost)
                ...
                if self._fallback_service:
                    ...
                    return self._fallback_service.route(prompt=prompt, task_type=task_type, model=model, provider_opts=provider_opts)
                return {'status': 'error', 'error': str(e), 'model': chosen_model if 'chosen_model' in locals() else 'unknown'}
        return {'status': 'error', 'error': 'unreachable - no result from router'}

    async def _call_litellm(self, model: str, messages: list[dict], max_tokens: int, temperature: float, task_type: str) -> StepResult:
        ...
        for attempt_model in models_to_try[:3]:
            try:
                ...
                response = _completion(model=attempt_model, messages=messages, max_tokens=max_tokens, temperature=temperature, timeout=REQUEST_TIMEOUT_SECONDS, fallbacks=[m for m in models_to_try if m != attempt_model][:2])
                ...
                return {'status': 'success', 'model': attempt_model, 'response': content, 'tokens': tokens_in, 'tokens_out': tokens_out, 'cost': cost, 'provider': self._extract_provider_from_model(attempt_model)}
            except Exception as e:
