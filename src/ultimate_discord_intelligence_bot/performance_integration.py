...
class PerformanceIntegrationManager:
    ...
    async def start_interaction_tracking(self, agent_name: str, task_type: str, context: dict[str, Any]) -> str:
        ...
        self.active_interactions[interaction_id] = {'agent_name': agent_name, 'task_type': task_type, 'context': context, 'start_time': default_utc_now(), 'tools_used': [], 'tool_sequence': []}
        self.logger.debug(f'Started tracking interaction {interaction_id} for {agent_name}')
        return interaction_id

    async def record_tool_usage(self, interaction_id: str, tool_name: str, tool_action: str, success: bool=True, error_details: dict[Any, Any] | None=None):
        ...
        interaction['tool_sequence'].append({'tool': tool_name, 'action': tool_action, 'timestamp': default_utc_now().isoformat(), 'success': success, 'error_details': error_details or {}})

    async def complete_interaction_tracking(self, interaction_id: str, response: str, user_feedback: dict[Any, Any] | None=None, error_occurred: bool=False, error_details: dict[Any, Any] | None=None) -> dict[str, Any]:
        ...
        if self.enhanced_monitor:
            try:
                quality_score = await self.enhanced_monitor.real_time_quality_assessment(response, interaction['context'])
            except Exception as e:
                self.logger.warning(f'Quality assessment failed: {e}')
                quality_score = 0.5
        else:
            quality_score = self._basic_quality_assessment(response, interaction['context'])
        interaction_data = {'agent_name': interaction['agent_name'], 'task_type': interaction['task_type'], 'tools_used': interaction['tools_used'], 'tool_sequence': interaction['tool_sequence'], 'response_quality': quality_score, 'response_time': response_time, 'user_feedback': user_feedback or {}, 'error_occurred': error_occurred, 'error_details': error_details or {}}
        self.base_monitor.record_agent_interaction(**interaction_data)
        real_time_data = {}
        if self.enhanced_monitor:
            try:
                real_time_data = await self.enhanced_monitor.monitor_real_time_performance(interaction['agent_name'], interaction_data)
            except Exception as e:
                self.logger.warning(f'Enhanced monitoring failed: {e}')
        del self.active_interactions[interaction_id]
        return {'interaction_id': interaction_id, 'performance_data': interaction_data, 'real_time_analysis': real_time_data}
    ...
    async def generate_agent_dashboard_data(self, agent_name: str | None=None) -> dict[str, Any]:
        ...
        try:
            if agent_name:
                ...
                return {'agent': agent_name, 'performance_report': {'overall_score': report.overall_score, 'metrics_count': len(report.metrics), 'recommendations_count': len(report.recommendations), 'reporting_period': report.reporting_period}, 'real_time_data': dashboard_data.get('agents', {}).get(agent_name, {})}
            else:
    ...
    async def track_crew_execution(self, execution_id: str, agents_used: list[str], tools_used: list[str], quality_score: float, execution_time: float, result: Any, metadata: dict[str, Any] | None=None) -> None:
        """Persist a consolidated record for a complete crew execution run."""
        metadata = metadata or {}
        context = {'execution_id': execution_id, 'agents_used': agents_used, 'tools_used': tools_used, 'result_digest': str(result)[:512] if result is not None else '', 'user_feedback': metadata.get('user_feedback'), 'tool_sequence': metadata.get('tool_sequence', []), 'error_details': metadata.get('error_details'), 'error_occurred': metadata.get('error_occurred', False), 'task_type': metadata.get('task_type', 'crew_execution')}
        if self.enhanced_monitor:
            try:
                await self.enhanced_monitor.record_interaction_async(agent_name='crew_orchestrator', interaction_type=context['task_type'], quality_score=quality_score, response_time=execution_time, context=context, tools_used=tools_used, error_occurred=context.get('error_occurred', False))
        ...
        for agent in agents_used or ['crew_orchestrator']:
            try:
                self.base_monitor.record_agent_interaction(agent_name=agent, task_type=context['task_type'], tools_used=tools_used, tool_sequence=context.get('tool_sequence', []), response_quality=quality_score, response_time=per_agent_time, user_feedback=context.get('user_feedback'), error_occurred=context.get('error_occurred', False), error_details={'execution_id': execution_id})
            except Exception as exc:
                self.logger.debug('Base monitor failed to log interaction for %s: %s', agent, exc)
        self.logger.info('Recorded crew execution %s (agents=%d, tools=%d, quality=%.2f)', execution_id, len(agents_used), len(tools_used), quality_score)
    ...
    async def generate_weekly_report(self, agent_names: list[str] | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            if self.enhanced_monitor and agent_reports:
                comparative_data = self.enhanced_monitor.generate_comparative_analysis(list(agent_reports.keys()), days=7)
            if agent_reports:
                ...
                summary = {'reporting_period': 'Last 7 days', 'agents_analyzed': len(agent_reports), 'average_performance': sum(all_scores) / len(all_scores), 'best_performer': max(agent_reports.items(), key=lambda x: x[1].overall_score)[0], 'total_recommendations': total_recommendations, 'performance_distribution': {'excellent': len([s for s in all_scores if s >= 0.9]), 'good': len([s for s in all_scores if 0.7 <= s < 0.9]), 'needs_attention': len([s for s in all_scores if s < 0.7])}}
            else:
                summary = {'error': 'No performance data available'}
            return {'summary': summary, 'agent_reports': {name: {'overall_score': report.overall_score, 'metrics_count': len(report.metrics), 'recommendations': report.recommendations[:3], 'top_tools': [p.tool_name for p in report.tool_usage[:3]]} for name, report in agent_reports.items()}, 'comparative_analysis': comparative_data, 'generation_timestamp': default_utc_now().isoformat()}
        except Exception as e:
    ...
    def save_performance_snapshot(self) -> Path:
        ...
        snapshot_data = {'timestamp': default_utc_now().isoformat(), 'active_interactions': len(self.active_interactions), 'monitored_agents': list(self.base_monitor.performance_history.keys()), 'enhanced_monitoring_enabled': self.enable_enhanced_monitoring}
        if self.enhanced_monitor:
            snapshot_data['real_time_metrics'] = {agent: {'recent_interactions_count': len(metrics['recent_interactions']), 'session_stats': metrics['current_session_stats']} for agent, metrics in self.enhanced_monitor.real_time_metrics.items()}
...
async def complete_interaction(interaction_id: str, response: str, user_feedback: dict[Any, Any] | None=None, error_occurred: bool=False, error_details: dict[Any, Any] | None=None):
    """Convenient function to complete interaction tracking."""
    manager = get_performance_manager()
    return await manager.complete_interaction_tracking(interaction_id, response, user_feedback, error_occurred, error_details)
...
class PerformanceAwareDiscordBot:
    ...
    async def handle_command(self, ctx, agent_name: str, user_input: str):
        ...
        interaction_id = await track_agent_interaction(agent_name=agent_name, task_type='discord_command', context={'user_id': str(ctx.author.id), 'channel_type': 'discord', 'command_input': user_input})
        try:
            ...
            result = await complete_interaction(interaction_id, response, user_feedback={'user_id': str(ctx.author.id), 'channel': 'discord'})
            return (response, result)
        except Exception as e:
            await complete_interaction(interaction_id, 'Error occurred during processing', error_occurred=True, error_details={'error': str(e)})
            raise
...
async def main():
    ...
    interaction_id = await track_agent_interaction(agent_name='enhanced_fact_checker', task_type='fact_verification', context={'user_id': 'test_user', 'priority': 'high'})
    ...
    result = await complete_interaction(interaction_id, 'Claims verified with 95% confidence based on 3 reliable sources.', user_feedback={'satisfaction': 0.9})
