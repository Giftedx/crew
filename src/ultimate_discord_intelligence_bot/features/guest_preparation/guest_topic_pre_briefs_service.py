...
class AudienceReactionPrediction:
    ...
    primary_reaction: str  # excitement, controversy, agreement, confusion
    confidence: float  # 0.0 to 1.0
    likely_questions: list[str]
    engagement_potential: float  # 0.0 to 1.0
    controversy_risk: float  # 0.0 to 1.0
...
class GuestTopicPreBriefsService:
    ...
    def generate_live_fact_check_prompts(
        self,
        guest_statements: list[dict[str, Any]],
        verification_sources: list[dict[str, Any]] | None = None,
    ) -> list[str]:
    ...
    def _select_model(self, model_alias: str) -> str:
        ...
        model_configs = {
            "fast": "fast_briefing",
            "balanced": "balanced_briefing",
            "quality": "quality_briefing",
        }
    ...
    def _generate_interview_brief(
        ...
        """
        try:
            # Extract and analyze guest arguments
            arguments = self._extract_guest_arguments(guest_content)

            # Generate opponent-process summary
            ...
            # Generate live fact-check prompts
    ...
    def _extract_guest_arguments(self, guest_content: list[dict[str, Any]]) -> list[ArgumentAnalysis]:
        ...
        for content_item in guest_content:
            ...
            # Split into potential argument segments
        ...
        return arguments[:10]  # Limit to top 10 arguments
    ...
    def _assess_evidence_quality(self, argument_text: str) -> float:
        ...
        if total_evidence == 0:
            return 0.5  # Neutral if no evidence indicators
    ...
    def _assess_logical_coherence(self, argument_text: str) -> float:
        ...
        # Base coherence on logical connectors
        base_coherence = min(coherence_count * 0.2, 1.0)

        # Adjust based on argument length (longer arguments can be more coherent)
        length_factor = min(len(argument_text) / 50, 1.0)  # Optimal around 50 words
    ...
    def _generate_counterarguments(self, argument_text: str) -> list[str]:
        ...
        # Simple counterargument generation based on argument type
        text_lower = argument_text.lower()
        ...
        return counterarguments[:3]  # Limit to top 3
    ...
    def _generate_opponent_process_summary(
        ...
        key_arguments = arguments[:5]  # Top 5 arguments
    ...
    def _summarize_position(self, strong_arguments: list[ArgumentAnalysis], guest_name: str) -> str:
        ...
        # Extract key themes from strong arguments
        themes = []
    ...
    def _generate_audience_hooks(self, arguments: list[ArgumentAnalysis]) -> list[str]:
        ...
        # Generate hooks based on argument characteristics
    ...
    def _generate_fact_check_priorities(self, arguments: list[ArgumentAnalysis]) -> list[str]:
        ...
        # Prioritize strong arguments for fact-checking
    ...
    def _generate_key_questions(
        ...
        # Questions based on strong arguments
        for arg in arguments[:3]:
            if arg.strength_score > 0.7:
                questions.append(f"What evidence supports this claim: '{arg.argument_text[:50]}...'?")

        # Questions based on audience predictions
    ...
    def _calculate_brief_confidence(
        ...
        # Base confidence on argument quality
    ...
    def _calculate_topic_controversy(self, topics: list[str]) -> float:
        ...
        """
        if not topics:
            return 0.3  # Neutral controversy
    ...
    def _calculate_specificity_score(self, text: str) -> float:
        ...
        """
        # Simple specificity based on concrete vs abstract language
        concrete_words = [
            "data",
            "research",
            "study",
