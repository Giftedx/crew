...
class CommunityPulseAnalyzerService:
    ...
    def __init__(self, cache_size: int=1000) -> None:
        ...
        self._positive_words = {'love', 'great', 'awesome', 'amazing', 'good', 'helpful', 'smart', 'agree', 'win'}
        self._negative_words = {'hate', 'bad', 'terrible', 'awful', 'stupid', 'angry', 'disagree', 'fail'}
        self._risk_terms = {'controversy', 'drama', 'strike', 'piracy', 'dmca', 'lawsuit', 'unsafe', 'boycott'}

    def analyze(self, items: list[dict[str, Any]], window_hours: int=24, model: Literal['fast', 'balanced', 'quality']='fast', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            for platform, p_items in by_platform.items():
                ...
                platform_summaries.append(PlatformSignalSummary(platform=platform, total_items=len(p_items), avg_sentiment=avg_sentiment, momentum_score=momentum, top_keywords=keywords[:8], top_questions=questions[:6], risk_indicators=risks[:6]))
            ...
            result = CommunityPulseResult(platform_summaries=platform_summaries, global_sentiment=global_sentiment, global_momentum=global_momentum, trending_topics=trending_topics, audience_questions=audience_questions, risk_flags=risk_flags, cached=False, processing_time_ms=(time.time() - start) * 1000)
    ...
    def _momentum_score(self, items: list[dict[str, Any]]) -> float:
        ...
        for it in items:
            ...
            recency = max(0.0, 1.0 - (now - ts) / (72 * 3600))
    ...
    def _extract_keywords(self, texts: list[str]) -> list[str]:
        ...
        tokens: list[str] = []
        for t in texts:
            tokens.extend(re.findall('[a-zA-Z][a-zA-Z0-9_\\-]{3,}', t.lower()))
        stop = {'this', 'that', 'with', 'http', 'https', 'from', 'about', 'there', 'which', 'have', 'been', 'your', 'they', 'them', 'will', 'what', 'when', 'where', 'who'}
    ...
    def _extract_questions(self, texts: list[str]) -> list[str]:
        ...
        qs: list[str] = []
        for t in texts:
            qs.extend([q.strip() for q in re.findall('([^\\n\\r\\?]{8,}\\?)', t)])
        return qs[:20]
