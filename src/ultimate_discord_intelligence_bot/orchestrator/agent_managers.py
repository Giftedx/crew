...
def populate_agent_tool_context(agent: Any, context_data: dict[str, Any], logger_instance: logging.Logger | None=None, metrics_instance: Any | None=None) -> None:
    ...
    try:
        ...
        settings = Settings()
        if settings.enable_enhanced_memory and settings.mem0_api_key and (not any((isinstance(t, Mem0MemoryTool) for t in agent.tools))):
    ...
    for tool in agent.tools:
        if hasattr(tool, 'update_context'):
            ...
            populated_count += 1
            _logger.debug(f'✅ Populated context for {getattr(tool, 'name', tool.__class__.__name__)}: {list(context_data.keys())}')
    if populated_count > 0:
        _logger.warning(f'✅ CONTEXT POPULATED on {populated_count} tools for agent {getattr(agent, 'role', 'unknown')}')
        if metrics_instance:
            with contextlib.suppress(Exception):
                metrics_instance.counter('autointel_context_populated', labels={'agent': getattr(agent, 'role', 'unknown'), 'tools_count': populated_count, 'has_transcript': 'transcript' in context_data or 'text' in context_data}).inc()
    else:
        _logger.error(f'❌ CONTEXT POPULATION FAILED: 0 tools updated for agent {getattr(agent, 'role', 'unknown')}')

def get_or_create_agent(agent_name: str, agent_coordinators: dict[str, Any], crew_instance: Any, logger_instance: logging.Logger | None=None) -> Any:
