...
async def handle_acquisition_failure(interaction: Any, acquisition_result: StepResult, url: str, log: logging.Logger | None=None) -> None:
    ...
    try:
        ...
        if error_type == 'dependency_missing' or (isinstance(acquisition_result.error, str) and 'yt-dlp' in acquisition_result.error.lower()):
            guidance = error_data.get('guidance', "yt-dlp is missing on this system. Install it with 'pip install yt-dlp' or run 'make first-run' to set up the environment. You can also run 'make doctor' to verify binaries.")
...
async def send_error_response(interaction: Any, stage: str, error: str, log: logging.Logger | None=None) -> None:
    ...
    try:
        if not is_session_valid(interaction, _logger):
            ...
            _get_metrics().counter('discord_session_closed_total', labels={'stage': f'error_response_{stage.lower().replace(' ', '_')}'})
            return
        ...
        try:
            ...
            if 'Session is closed' in str(e):
                ...
                _get_metrics().counter('discord_session_closed_total', labels={'stage': f'error_response_send_{stage.lower().replace(' ', '_')}'})
                return
            else:
                raise
    except Exception as e:
        ...
        try:
            if is_session_valid(interaction, _logger):
                await interaction.followup.send(f'❌ Autonomous Intelligence Error\n**Stage:** {stage}\n**Error:** {error}\n\nThe autonomous workflow encountered an issue during processing.', ephemeral=False)
            else:
...
async def send_enhanced_error_response(interaction: Any, stage: str, enhanced_message: str, log: logging.Logger | None=None) -> None:
    ...
    _logger = log or logger
    try:
        await interaction.followup.send(f'❌ **Autonomous Intelligence - {stage}**\n\n{enhanced_message}', ephemeral=False)
    except Exception:
