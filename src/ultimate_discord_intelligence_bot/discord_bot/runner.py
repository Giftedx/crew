...
def create_full_bot():
    ...
    bot = commands.Bot(command_prefix='!', intents=intents, description='Ultimate Discord Intelligence Bot - Full System')
    tools = load_tools()
    ...
    try:
        ...
        background_worker = BackgroundIntelligenceWorker(orchestrator=orchestrator, storage_dir='data/background_workflows')
...
async def main():
    ...
    async def _run_headless_agent(user_on: bool, admin_on: bool) -> None:
        print(f'🧠 Running in headless agent mode (user_cmds={('on' if user_on else 'off')}, admin={('on' if admin_on else 'off')})')
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        retrying_post(webhook, json_payload={'content': f'🫀 Heartbeat: headless agent alive @ {ts}'}, headers={'Content-Type': 'application/json'}, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
                    except Exception:
                        pass
        ...
        try:
            ...
            try:
                ...
                if webhook:
                    ...
                    retrying_post(webhook, json_payload={'content': content[:1900]}, headers={'Content-Type': 'application/json'}, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
            except Exception:
                pass
            try:
                if bot is not None:
