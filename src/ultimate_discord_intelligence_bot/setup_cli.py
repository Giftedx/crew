...
def _wizard(*, non_interactive: bool=False, accept_defaults: bool=False, presets: dict[str, str] | None=None, use_example_when_missing: bool=False) -> int:
    ...
    for key, secret in [('DISCORD_BOT_TOKEN', True), ('DISCORD_WEBHOOK', True), ('DISCORD_PRIVATE_WEBHOOK', True), ('OPENROUTER_API_KEY', True), ('OPENAI_API_KEY', True), ('EXA_API_KEY', True), ('PERPLEXITY_API_KEY', True)]:
    ...
    updates['DEFAULT_DOWNLOAD_QUALITY'] = ask('DEFAULT_DOWNLOAD_QUALITY', default=os.getenv('DEFAULT_DOWNLOAD_QUALITY', '1080p'))
    ...
    updates.update({'CREWAI_BASE_DIR': base_dir, 'CREWAI_DOWNLOADS_DIR': downloads_dir, 'CREWAI_CONFIG_DIR': config_dir, 'CREWAI_LOGS_DIR': logs_dir, 'CREWAI_PROCESSING_DIR': processing_dir, 'CREWAI_YTDLP_DIR': ytdlp_dir})
    ...
    if updates.get('ENABLE_TRACING') == '1':
        updates['OTEL_EXPORTER_OTLP_ENDPOINT'] = ask('OTEL_EXPORTER_OTLP_ENDPOINT', default=os.getenv('OTEL_EXPORTER_OTLP_ENDPOINT', ''))
        updates['OTEL_EXPORTER_OTLP_HEADERS'] = ask('OTEL_EXPORTER_OTLP_HEADERS', default=os.getenv('OTEL_EXPORTER_OTLP_HEADERS', ''))
    ...
    if enable_cors == '1':
        default_origins = os.getenv('CORS_ALLOW_ORIGINS', 'http://localhost:5173,http://127.0.0.1:5173,http://localhost:5174,http://127.0.0.1:5174')
    ...
    updates['DISCORD_UPLOAD_LIMIT_BYTES'] = ask('DISCORD_UPLOAD_LIMIT_BYTES', default=os.getenv('DISCORD_UPLOAD_LIMIT_BYTES', ''))
    updates['DISCORD_UPLOAD_LIMIT_WEBHOOK_BYTES'] = ask('DISCORD_UPLOAD_LIMIT_WEBHOOK_BYTES', default=os.getenv('DISCORD_UPLOAD_LIMIT_WEBHOOK_BYTES', ''))
    ...
    _ensure_dirs([Path(downloads_dir), Path(config_dir), Path(logs_dir), Path(processing_dir), Path(ytdlp_dir) / 'config', Path(ytdlp_dir) / 'archives'])
...
def _check_ytdlp_module() -> tuple[bool, str]:
    ...
    if ok:
        return (True, path)
    try:
        out = subprocess.check_output([sys.executable, '-m', 'yt_dlp', '--version'], stderr=subprocess.STDOUT, text=True, timeout=5)
...
def main(argv: list[str] | None=None) -> int:
    ...
    if cmd in {None, 'wizard', 'setup', 'init', 'init-env'}:
        ...
        return _wizard(non_interactive=non_interactive, accept_defaults=accept_defaults, presets=presets, use_example_when_missing=use_example)
    if cmd == 'doctor':
