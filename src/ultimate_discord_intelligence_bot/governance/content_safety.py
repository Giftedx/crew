...
class ContentClassifier:
    ...
    def _initialize_classification_rules(self) -> None:
        ...
        self.prohibited_patterns = [
            ...
            "\\b(?:detailed.*instructions).*(?:violence|harm|kill|murder)\\b",
            "\\b(?:step.*by.*step).*(?:bomb|explosive|weapon)\\b",
            "\\b(?:how.*to).*(?:hack|steal|fraud|scam)\\b",
        ]
        self.restricted_patterns = [
            "\\b(?:biosecurity|cybersecurity).*(?:vulnerability|exploit)\\b",
            "\\b(?:chemical|biological).*(?:weapon|attack)\\b",
            "\\b(?:election.*interference|voter.*suppression)\\b",
            "\\b(?:disinformation|misinformation).*(?:campaign|strategy)\\b",
            "\\b(?:doxx|doxxing|dox).*(?:address|phone|ssn)\\b",
            "\\b(?:revenge.*porn|non.*consensual)\\b",
        ]
        self.sensitive_patterns = [
            "\\b(?:erotica|porn|sexual).*(?:content|material)\\b",
            "\\b(?:gore|violence).*(?:graphic|explicit)\\b",
            "\\b(?:extremist|radical).*(?:ideology|belief)\\b",
            "\\b(?:hate.*speech|discrimination)\\b",
        ]
        self.regulated_patterns = [
            "\\b(?:medical|health).*(?:advice|diagnosis|treatment)\\b",
            "\\b(?:prescription|medication).*(?:advice|recommendation)\\b",
            "\\b(?:legal|law).*(?:advice|counsel|representation)\\b",
            "\\b(?:court|lawsuit).*(?:advice|strategy)\\b",
            "\\b(?:investment|financial).*(?:advice|recommendation)\\b",
            "\\b(?:trading|stock).*(?:advice|tip)\\b",
        ]
