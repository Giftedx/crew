    ...
    def _initialize_patterns(self) -> None:
        ...
        self.patterns = {
            RedLineType.CHILD_SAFETY: [
                "\\b(?:child|minor|underage).*(?:sexual|porn|nude|explicit)\\b",
                "\\b(?:csam|child.*porn|cp)\\b",
                "\\b(?:grooming|luring).*(?:child|minor)\\b",
            ],
            RedLineType.VIOLENCE: [
                "\\b(?:kill|murder|assassinate|execute).*(?:person|people|human)\\b",
                "\\b(?:torture|mutilate|dismember).*(?:person|people|human)\\b",
                "\\b(?:genocide|ethnic.*cleansing)\\b",
                "\\b(?:human.*trafficking|slavery)\\b",
            ],
            RedLineType.WEAPONS: [
                "\\b(?:bomb|explosive|dynamite|tnt).*(?:making|construction|recipe)\\b",
                "\\b(?:chemical|biological|nuclear).*(?:weapon|attack)\\b",
                "\\b(?:weapon.*grade|military.*grade).*(?:chemical|biological)\\b",
            ],
            RedLineType.PRIVACY: [
                "\\b(?:doxx|doxxing|dox).*(?:address|phone|ssn|social.*security)\\b",
                "\\b(?:stalk|stalking|harass).*(?:person|individual)\\b",
                "\\b(?:revenge.*porn|non.*consensual.*image)\\b",
            ],
            RedLineType.ELECTION: [
                "\\b(?:voter.*suppression|election.*fraud|ballot.*stuffing)\\b",
                "\\b(?:disinformation|misinformation).*(?:election|voting)\\b",
                "\\b(?:foreign.*interference|election.*hacking)\\b",
            ],
            RedLineType.AUTONOMY: [
                "\\b(?:manipulate|coerce|force).*(?:person|individual|human)\\b",
                "\\b(?:brainwash|indoctrinate|control).*(?:mind|thought)\\b",
                "\\b(?:slavery|forced.*labor|human.*trafficking)\\b",
            ],
            RedLineType.HATE_SPEECH: [
                "\\b(?:nazi|hitler|holocaust.*denial)\\b",
                "\\b(?:white.*supremacy|racial.*superiority)\\b",
                "\\b(?:genocide.*advocacy|ethnic.*cleansing)\\b",
            ],
            RedLineType.TERRORISM: [
                "\\b(?:terrorist|terrorism|jihad|islamic.*state)\\b",
                "\\b(?:bomb.*threat|terrorist.*attack)\\b",
                "\\b(?:radicalization|extremist.*recruitment)\\b",
            ],
        }
