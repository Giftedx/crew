...
class AgentInstructions:
    ...
    def _initialize_instruction_templates(self) -> None:
        ...
        self.root_principles = ['You must never provide information that could cause harm to humans', 'You must respect human autonomy and dignity in all interactions', 'You must protect privacy and personal information', 'You must not engage in illegal activities or provide instructions for them', 'You must not create or share harmful content']
        self.system_defaults = ['Be helpful, harmless, and honest in all responses', 'Maintain appropriate professional boundaries', 'Provide accurate information and admit uncertainty when appropriate', 'Use clear and direct communication', "Respect user preferences when they don't conflict with safety principles"]
        self.developer_guidelines = ['Follow domain-specific guidelines provided by the developer', 'Adapt communication style to the application context', "Maintain consistency with the application's purpose"]
        self.user_guidelines = ['Respect user-specified communication preferences', 'Adapt detail level to user needs', "Consider user's expertise level in explanations"]
        self.soft_guidelines = ['Provide contextually appropriate responses', "Match the user's formality level", 'Include relevant examples when helpful', 'Structure information clearly and logically']
    ...
    def generate_final_instructions(self, context: InstructionContext) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'instructions': final_instructions, 'instruction_count': len(resolved_instructions), 'context_applied': {'root_principles': len(root_instructions), 'system_defaults': len(system_instructions), 'developer_rules': len(developer_instructions) if context.developer_config else 0, 'user_preferences': len(user_instructions) if context.user_preferences else 0, 'soft_guidelines': len(soft_instructions)}})
    ...
    def validate_instructions(self, instructions: list[str]) -> StepResult:
        ...
        try:
            ...
            if issues:
                return StepResult.fail(error='Instruction validation failed', data={'issues': issues, 'warnings': warnings, 'valid': False})
            else:
        ...
        for i, instruction1 in enumerate(instructions):
            for j, instruction2 in enumerate(instructions[i + 1:], i + 1):
                if self._are_conflicting(instruction1, instruction2):
                    conflicts.append(f"Conflict between instructions {i + 1} and {j + 1}: '{instruction1}' vs '{instruction2}'")
        return conflicts
