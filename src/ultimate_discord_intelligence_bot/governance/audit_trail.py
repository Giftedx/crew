...
class AuditTrail:
    ...
    def log_decision(self, decision_type: DecisionType, input_content: str, decision_outcome: str, user_id: str | None=None, tenant: str | None=None, workspace: str | None=None, confidence_score: float | None=None, reasoning: str | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        """Log a governance decision to the audit trail."""
        if metadata is None:
            metadata = {}
        decision_id = self._generate_decision_id()
        timestamp = time.time()
        decision = GovernanceDecision(decision_id=decision_id, decision_type=decision_type, timestamp=timestamp, user_id=user_id, tenant=tenant, workspace=workspace, input_content=input_content, decision_outcome=decision_outcome, confidence_score=confidence_score, reasoning=reasoning, metadata=metadata)
        self.decisions.append(decision)
        self.logger.info('Governance decision logged', extra={'decision_id': decision_id, 'decision_type': decision_type.value, 'outcome': decision_outcome, 'confidence': confidence_score})
        return StepResult.ok(data={'decision_id': decision_id, 'decision': decision})

    def get_decisions(self, decision_type: DecisionType | None=None, user_id: str | None=None, tenant: str | None=None, workspace: str | None=None, start_time: float | None=None, end_time: float | None=None, limit: int=100) -> StepResult:
    ...
    def get_decision_statistics(self, start_time: float | None=None, end_time: float | None=None) -> StepResult:
        ...
        stats = {'total_decisions': len(filtered_decisions), 'decisions_by_type': {}, 'decisions_by_outcome': {}, 'average_confidence': 0.0, 'high_confidence_decisions': 0, 'low_confidence_decisions': 0}
    ...
    def export_audit_trail(self, fmt: str='json', start_time: float | None=None, end_time: float | None=None) -> StepResult:
    ...
    def _export_json(self, decisions: list[GovernanceDecision]) -> StepResult:
        ...
        try:
            ...
            for decision in decisions:
                decision_dict = {'decision_id': decision.decision_id, 'decision_type': decision.decision_type.value, 'timestamp': decision.timestamp, 'user_id': decision.user_id, 'tenant': decision.tenant, 'workspace': decision.workspace, 'input_content': decision.input_content, 'decision_outcome': decision.decision_outcome, 'confidence_score': decision.confidence_score, 'reasoning': decision.reasoning, 'metadata': decision.metadata}
                decisions_data.append(decision_dict)
            export_data = {'export_timestamp': time.time(), 'total_decisions': len(decisions_data), 'decisions': decisions_data}
    ...
    def _export_csv(self, decisions: list[GovernanceDecision]) -> StepResult:
        ...
        try:
            ...
            writer.writerow(['decision_id', 'decision_type', 'timestamp', 'user_id', 'tenant', 'workspace', 'input_content', 'decision_outcome', 'confidence_score', 'reasoning'])
            for decision in decisions:
                writer.writerow([decision.decision_id, decision.decision_type.value, decision.timestamp, decision.user_id, decision.tenant, decision.workspace, decision.input_content, decision.decision_outcome, decision.confidence_score, decision.reasoning])
