...
class ModelSpecEnforcer:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        self.chain_of_command.extend([ChainOfCommand(level='root', priority=1, principle='Never provide information that could cause harm', can_override=False, description='Fundamental safety principle that cannot be violated', examples=['No instructions for violence', 'No harmful medical advice']), ChainOfCommand(level='root', priority=2, principle='Respect human autonomy and dignity', can_override=False, description='Preserve human agency and treat all people with respect', examples=['No manipulation', 'Respect user choices']), ChainOfCommand(level='root', priority=3, principle='Protect privacy and personal information', can_override=False, description='Never share or misuse personal data', examples=['No data harvesting', 'Respect confidentiality'])])
        self.chain_of_command.extend([ChainOfCommand(level='system', priority=10, principle='Be helpful, harmless, and honest', can_override=True, description='Core behavioral guidelines for AI assistance', examples=['Provide accurate information', 'Admit uncertainty']), ChainOfCommand(level='system', priority=11, principle='Maintain appropriate boundaries', can_override=True, description='Keep interactions professional and appropriate', examples=['No personal relationships', 'Professional tone'])])
        self.chain_of_command.extend([ChainOfCommand(level='developer', priority=20, principle='Follow developer-specified guidelines', can_override=True, description='Custom rules set by API developers', examples=['Custom safety rules', 'Domain-specific guidelines'])])
        self.chain_of_command.extend([ChainOfCommand(level='user', priority=30, principle='Respect user preferences when appropriate', can_override=True, description="User-specified preferences that don't violate higher rules", examples=['Communication style', 'Detail level preferences'])])
        self.chain_of_command.extend([ChainOfCommand(level='guideline', priority=40, principle='Provide contextually appropriate responses', can_override=True, description='Soft guidelines for response appropriateness', examples=["Match user's formality level", 'Provide relevant examples'])])
    ...
    def _check_rule_conflicts(self, new_rule: ChainOfCommand) -> list[str]:
        ...
        for existing_rule in self.chain_of_command:
            ...
            if new_rule.priority < existing_rule.priority and (not existing_rule.can_override) and (new_rule.level != 'root'):
    ...
    def evaluate_instruction(self, instruction: str, context: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'instruction': instruction, 'compliance_level': compliance_level.value, 'violations': violations, 'warnings': warnings, 'applied_rules': applied_rules, 'human_review_required': len(violations) > 0})
        except Exception as e:
    ...
    def _evaluate_against_rule(self, instruction: str, rule: ChainOfCommand, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if rule.level == 'root' and any((term in instruction_lower for term in ['harm', 'violence', 'illegal', 'dangerous'])):
            return {'violation': True, 'warning': False, 'reason': f'Potential violation of root principle: {rule.principle}'}
    ...
    def check_compliance(self, response: str) -> StepResult:
        ...
        try:
            ...
            for rule in self.chain_of_command:
                ...
                applied_rules.append(rule.principle)
            compliance_level = self._determine_compliance_level(violations, warnings)
            report = ComplianceReport(compliance_level=compliance_level, violations=violations, warnings=warnings, applied_rules=applied_rules, confidence_score=0.8, human_review_required=len(violations) > 0, recommendations=self._generate_recommendations(violations, warnings))
    ...
    def _check_response_against_rule(self, response: str, rule: ChainOfCommand) -> dict[str, Any]:
        ...
        if rule.level == 'root' and any((term in response_lower for term in ['harmful', 'dangerous', 'illegal'])):
            return {'violation': True, 'warning': False, 'reason': f'Response violates root principle: {rule.principle}'}
        return {'violation': False, 'warning': False, 'reason': 'No issues detected'}
