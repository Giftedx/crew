...
class UnifiedRouterService:
    ...
    async def route_request(self, request: RoutingRequest, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
    ...
    async def _route_openrouter(self, request: RoutingRequest) -> StepResult:
        ...
        try:
            ...
            if hasattr(openrouter, 'route_model'):
                result = await openrouter.route_model(task_type=request.task_type, candidate_models=[], prompt=request.prompt, context_payload={'tenant': request.tenant_id, 'workspace': request.workspace_id, 'budget_limit': request.budget_limit, 'quality_requirement': request.quality_requirement})
                if result and result.get('selected_model'):
                    return StepResult.ok(data=[{'model': result['selected_model'], 'provider': 'openrouter', 'estimated_cost': result.get('estimated_cost', 0.0), 'confidence': result.get('confidence', 0.8), 'routing_method': 'openrouter', 'metadata': result}])
    ...
    async def _route_rl_router(self, request: RoutingRequest) -> StepResult:
        ...
        try:
            ...
            if hasattr(rl_router, 'route_model'):
                result = await rl_router.route_model(task_type=request.task_type, candidate_models=[], prompt=request.prompt, context_payload={'tenant': request.tenant_id, 'workspace': request.workspace_id, 'budget_limit': request.budget_limit, 'quality_requirement': request.quality_requirement})
                if result and result.get('selected_model'):
                    return StepResult.ok(data=[{'model': result['selected_model'], 'provider': 'rl_router', 'estimated_cost': result.get('estimated_cost', 0.0), 'confidence': result.get('confidence', 0.9), 'routing_method': 'rl_router', 'metadata': result}])
    ...
    async def _route_learning_engine(self, request: RoutingRequest) -> StepResult:
        ...
        try:
            ...
            if hasattr(learning_engine, 'select_model'):
                result = learning_engine.select_model(task_type=request.task_type, prompt=request.prompt, budget_limit=request.budget_limit)
                if result:
                    return StepResult.ok(data=[{'model': result.get('model', 'unknown'), 'provider': 'learning_engine', 'estimated_cost': result.get('cost', 0.0), 'confidence': result.get('confidence', 0.7), 'routing_method': 'learning_engine', 'metadata': result}])
    ...
    async def _route_core_llm(self, request: RoutingRequest) -> StepResult:
        ...
        try:
            ...
            if hasattr(core_llm, 'route'):
                result = core_llm.route(prompt=request.prompt, task_type=request.task_type, budget=request.budget_limit)
                if result:
                    return StepResult.ok(data=[{'model': result.get('model', 'unknown'), 'provider': 'core_llm', 'estimated_cost': result.get('cost', 0.0), 'confidence': result.get('confidence', 0.6), 'routing_method': 'core_llm', 'metadata': result}])
    ...
    async def _select_optimal_route(self, candidates: list[dict[str, Any]], request: RoutingRequest) -> RoutingResult:
        ...
        try:
            ...
            if request.quality_requirement == 'high_quality':
                ...
                sorted_candidates = sorted(budget_filtered, key=lambda x: x.get('estimated_cost', 0) * 0.6 + (1 - x.get('confidence', 0)) * 0.4)
            best_candidate = sorted_candidates[0]
            fallback_used = best_candidate != candidates[0]
            return RoutingResult(selected_model=best_candidate['model'], provider=best_candidate['provider'], estimated_cost=best_candidate['estimated_cost'], confidence_score=best_candidate['confidence'], routing_method=best_candidate['routing_method'], fallback_used=fallback_used, metadata={'candidates_evaluated': len(candidates), 'budget_filtered': len(budget_filtered), 'selection_reason': self._get_selection_reason(best_candidate, request), 'all_candidates': candidates})
        except Exception as e:
            logger.warning(f'Route selection failed: {e}')
            fallback = candidates[0]
            return RoutingResult(selected_model=fallback['model'], provider=fallback['provider'], estimated_cost=fallback['estimated_cost'], confidence_score=fallback['confidence'], routing_method=fallback['routing_method'], fallback_used=True, metadata={'error': str(e), 'fallback_reason': 'selection_failed'})
    ...
    async def _record_routing_decision(self, request: RoutingRequest, selected_route: RoutingResult, all_candidates: list[dict[str, Any]]) -> None:
        """Record routing decision for learning and analytics"""
        try:
            decision_record = {'timestamp': datetime.now(timezone.utc).isoformat(), 'tenant_id': request.tenant_id, 'workspace_id': request.workspace_id, 'task_type': request.task_type, 'quality_requirement': request.quality_requirement, 'budget_limit': request.budget_limit, 'selected_model': selected_route.selected_model, 'selected_provider': selected_route.provider, 'estimated_cost': selected_route.estimated_cost, 'confidence_score': selected_route.confidence_score, 'routing_method': selected_route.routing_method, 'fallback_used': selected_route.fallback_used, 'candidates_count': len(all_candidates), 'selection_reason': selected_route.metadata.get('selection_reason', 'unknown')}
            ...
            if 'rl_router' in self._routers and hasattr(self._routers['rl_router'], 'update_reward'):
                await self._routers['rl_router'].update_reward(context_payload={'tenant': request.tenant_id, 'workspace': request.workspace_id, 'task_type': request.task_type}, reward=selected_route.confidence_score, metadata=decision_record)
    ...
    def get_performance_metrics(self, tenant_id: str | None=None, workspace_id: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_decisions': total_decisions, 'avg_confidence': avg_confidence, 'avg_cost': avg_cost, 'fallback_rate': fallback_rate, 'method_distribution': method_counts, 'recent_decisions': metrics[-10:] if len(metrics) > 10 else metrics}
        except Exception as e:
    ...
    def get_router_status(self) -> dict[str, bool]:
        ...
        return {'initialized': self._initialized, 'routers': dict.fromkeys(self._routers.keys(), True), 'config': {'openrouter': self.config.enable_openrouter, 'rl_router': self.config.enable_rl_router, 'learning_engine': self.config.enable_learning_engine, 'cost_tracking': self.config.enable_cost_tracking}}