...
class UnifiedRouterService:
    ...
    async def route_request(
        self, request: RoutingRequest, tenant_id: str | None = None, workspace_id: str | None = None
    ) -> StepResult:
        ...
        try:
            ...
            if hasattr(learning_engine, "select_model"):
                result = learning_engine.select_model(
                    task_type=request.task_type, prompt=request.prompt, budget_limit=request.budget_limit
                )
                if result:
                    return StepResult.ok(
                        data=[
        ...
        try:
            ...
            if request.quality_requirement == "high_quality":
                ...
                sorted_candidates = sorted(
                    budget_filtered, key=lambda x: x.get("estimated_cost", 0) * 0.6 + (1 - x.get("confidence", 0)) * 0.4
                )
    ...
    async def _record_routing_decision(
        self, request: RoutingRequest, selected_route: RoutingResult, all_candidates: list[dict[str, Any]]
    ) -> None:
