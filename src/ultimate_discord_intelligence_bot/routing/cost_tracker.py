...
class UnifiedCostTracker:
    ...
    def check_budget_compliance(self, tenant_id: str, workspace_id: str, requested_cost: float) -> StepResult:
        ...
        try:
            ...
            if daily_spent + Decimal(str(requested_cost)) > budget_config.daily_limit:
                return StepResult.fail(
                    f"Request would exceed daily budget limit. Current: ${daily_spent:.4f}, Request: ${requested_cost:.4f}, Limit: ${budget_config.daily_limit}"
                )
            if monthly_spent + Decimal(str(requested_cost)) > budget_config.monthly_limit:
                return StepResult.fail(
                    f"Request would exceed monthly budget limit. Current: ${monthly_spent:.4f}, Request: ${requested_cost:.4f}, Limit: ${budget_config.monthly_limit}"
                )
    ...
    def get_cost_analytics(
        self, tenant_id: str | None = None, workspace_id: str | None = None, days: int = 30
    ) -> StepResult:
