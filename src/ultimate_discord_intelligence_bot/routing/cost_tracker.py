...
class UnifiedCostTracker:
    ...
    def set_budget(self, tenant_id: str, workspace_id: str, daily_limit: float, monthly_limit: float, per_request_limit: float=1.0, alert_threshold: float=0.8, reset_period: str='daily') -> StepResult:
        """Set budget limits for a tenant/workspace"""
        try:
            key = f'{tenant_id}:{workspace_id}'
            budget_config = BudgetConfig(tenant_id=tenant_id, workspace_id=workspace_id, daily_limit=Decimal(str(daily_limit)), monthly_limit=Decimal(str(monthly_limit)), per_request_limit=Decimal(str(per_request_limit)), alert_threshold=alert_threshold, reset_period=reset_period)
    ...
    def get_budget_status(self, tenant_id: str, workspace_id: str) -> StepResult:
        ...
        try:
            ...
            if daily_usage_ratio >= 1.0 or monthly_usage_ratio >= 1.0:
                alert_level = 'red'
            elif daily_usage_ratio >= budget_config.alert_threshold or monthly_usage_ratio >= budget_config.alert_threshold:
                alert_level = 'yellow'
            else:
                alert_level = 'green'
            ...
            budget_status = BudgetStatus(tenant_id=tenant_id, workspace_id=workspace_id, daily_spent=daily_spent, monthly_spent=monthly_spent, daily_remaining=daily_remaining, monthly_remaining=monthly_remaining, daily_limit=budget_config.daily_limit, monthly_limit=budget_config.monthly_limit, alert_level=alert_level, last_reset=last_reset, next_reset=next_reset)
    ...
    def record_cost(self, tenant_id: str, workspace_id: str, model: str, provider: str, cost: float, tokens_used: int | None=None, request_type: str='routing', metadata: dict[str, Any] | None=None) -> StepResult:
        ...
        try:
            ...
            cost_record = CostRecord(transaction_id=transaction_id, tenant_id=tenant_id, workspace_id=workspace_id, model=model, provider=provider, cost=Decimal(str(cost)), tokens_used=tokens_used, request_type=request_type, metadata=metadata or {})
    ...
    def check_budget_compliance(self, tenant_id: str, workspace_id: str, requested_cost: float) -> StepResult:
        ...
        try:
            ...
            if Decimal(str(requested_cost)) > budget_config.per_request_limit:
                return StepResult.fail(f'Request cost ${requested_cost:.4f} exceeds per-request limit ${budget_config.per_request_limit}')
            daily_spent, monthly_spent = self._calculate_spending(tenant_id, workspace_id)
            if daily_spent + Decimal(str(requested_cost)) > budget_config.daily_limit:
                return StepResult.fail(f'Request would exceed daily budget limit. Current: ${daily_spent:.4f}, Request: ${requested_cost:.4f}, Limit: ${budget_config.daily_limit}')
            if monthly_spent + Decimal(str(requested_cost)) > budget_config.monthly_limit:
                return StepResult.fail(f'Request would exceed monthly budget limit. Current: ${monthly_spent:.4f}, Request: ${requested_cost:.4f}, Limit: ${budget_config.monthly_limit}')
            return StepResult.ok(data={'compliant': True, 'reason': 'within_budget', 'daily_remaining': float(budget_config.daily_limit - daily_spent), 'monthly_remaining': float(budget_config.monthly_limit - monthly_spent)})
        except Exception as e:
            logger.error(f'Failed to check budget compliance: {e}')
            return StepResult.fail(f'Budget compliance check failed: {e!s}')

    def get_cost_analytics(self, tenant_id: str | None=None, workspace_id: str | None=None, days: int=30) -> StepResult:
        ...
        try:
            ...
            analytics = {'total_cost': float(total_cost), 'total_requests': total_requests, 'avg_cost_per_request': float(total_cost / total_requests) if total_requests > 0 else 0.0, 'provider_breakdown': {provider: {'cost': float(data['cost']), 'requests': data['requests'], 'percentage': float(data['cost'] / total_cost * 100) if total_cost > 0 else 0.0} for provider, data in provider_costs.items()}, 'model_breakdown': {model: {'cost': float(data['cost']), 'requests': data['requests'], 'percentage': float(data['cost'] / total_cost * 100) if total_cost > 0 else 0.0} for model, data in model_costs.items()}, 'daily_spending': {str(day): float(cost) for day, cost in daily_spending.items()}, 'period_days': days, 'records_analyzed': len(filtered_records)}
