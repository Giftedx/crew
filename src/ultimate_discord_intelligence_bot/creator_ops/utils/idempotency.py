...
class IdempotencyKey:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'key': self.key, 'expires_at': self.expires_at, 'metadata': self.metadata, 'created_at': self.created_at}

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> 'IdempotencyKey':
        """Create from dictionary."""
        return cls(key=data['key'], expires_at=data.get('expires_at'), metadata=data.get('metadata', {}))
...
class IdempotencyManager:
    ...
    async def execute_with_idempotency(self, operation: Callable[..., T], idempotency_key: str | None=None, ttl: float | None=None, *args, **kwargs) -> T:
        """
        ...
        async with self._lock:
            ...
            try:
                ...
                self._store[idempotency_key] = IdempotencyKey(key=idempotency_key, expires_at=expires_at, metadata={'operation': operation.__name__, 'args_hash': hashlib.md5(str(args).encode(), usedforsecurity=False).hexdigest(), 'kwargs_hash': hashlib.md5(str(kwargs).encode(), usedforsecurity=False).hexdigest()})
    ...
    async def execute_with_result_idempotency(self, operation: Callable[..., StepResult], idempotency_key: str | None=None, ttl: float | None=None, *args, **kwargs) -> StepResult:
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'total_keys': len(self._store), 'expired_keys': expired_count, 'active_keys': len(self._store) - expired_count, 'default_ttl': self.default_ttl}
...
def with_idempotency(key: str | None=None, ttl: float | None=None, manager: IdempotencyManager | None=None):
    """
...
def with_result_idempotency(key: str | None=None, ttl: float | None=None, manager: IdempotencyManager | None=None):
    """
