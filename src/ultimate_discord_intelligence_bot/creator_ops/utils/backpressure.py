    ...
    async def acquire(self, required_slots: int = 1) -> bool:
        ...
        async with self._lock:
            if self.current_depth + required_slots > self.max_queue_depth:
                logger.warning(
                    f"Backpressure {self.name}: queue full, rejecting request. Current depth: {self.current_depth}, required: {required_slots}, max: {self.max_queue_depth}"
                )
                return False
...
class BackpressureManager:
    ...
    def get_handler(
        self, name: str, max_queue_depth: int = 100, warning_threshold: float = 0.7, critical_threshold: float = 0.9
    ) -> BackpressureHandler:
        """
    ...
    async def cleanup(self) -> None:
        ...
        for handler in self.handlers.values():
            ...
            handler.depth_history = deque(
                [(ts, depth) for ts, depth in handler.depth_history if ts > cutoff], maxlen=100
            )
...
def with_backpressure(
    handler_name: str, required_slots: int = 1, timeout: float | None = None, manager: BackpressureManager | None = None
):
    """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        async def async_wrapper(*args, **kwargs) -> T:
            ...
            if not await handler.wait_for_slots(required_slots, timeout):
                raise BackpressureError(
                    f"Backpressure limit exceeded for {handler_name}", handler.current_depth, handler.max_queue_depth
                )
            try:
