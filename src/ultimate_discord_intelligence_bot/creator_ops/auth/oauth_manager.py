...
class OAuthManager(ABC):
    ...
    def store_tokens(
        ...
        try:
            ...
            return self.token_storage.store_tokens(
    ...
    def get_stored_tokens(self, tenant: str, workspace: str, user_id: str | None = None) -> StepResult:
        ...
        try:
            return self.token_storage.get_tokens(
                platform=self.platform,
                tenant=tenant,
                workspace=workspace,
                user_id=user_id,
...
class TikTokOAuthManager(OAuthManager):
    ...
    def __init__(
        ...
        super().__init__(
            platform="tiktok",
            client_id=client_key,  # TikTok uses client_key instead of client_id
            client_secret=client_secret,
            redirect_uri=redirect_uri,
            scopes=scopes,
            token_storage=token_storage,
        ...
        try:
            ...
            return StepResult.ok(
                data={
                    ...
                    "expires_in": long_lived_token_data.get("expires_in", 5184000),  # 60 days default
                    "scope": ",".join(self.scopes),
                    "token_type": "Bearer",
                }
            )
    ...
    def validate_token(self, access_token: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(
                data={
                    "valid": True,
                    "user_id": token_info.get("data", {}).get("id"),
                    "username": token_info.get("data", {}).get("username"),
                }
            )
