...
class TokenStorage:
    ...
    def store_tokens(self, platform: str, tenant: str, workspace: str, user_id: str | None, access_token: str, refresh_token: str | None, expires_at: datetime | None, scope: str | None, handle: str | None=None, display_name: str | None=None) -> StepResult:
        ...
        try:
            ...
            account = session.query(Account).filter_by(tenant=tenant, workspace=workspace, platform=platform, platform_id=user_id or '').first()
            if account:
                ...
                account = Account(tenant=tenant, workspace=workspace, platform=platform, handle=handle or '', display_name=display_name, platform_id=user_id or '', oauth_scopes=scope, access_token_encrypted=encrypted_access_token, refresh_token_encrypted=encrypted_refresh_token, token_expires_at=expires_at, is_active=True)
                session.add(account)
            session.commit()
            logger.info(f'Stored tokens for {platform} account {user_id} in {tenant}/{workspace}')
            return StepResult.ok(data={'account_id': account.id, 'platform': platform, 'user_id': user_id, 'expires_at': expires_at})
    ...
    def get_tokens(self, platform: str, tenant: str, workspace: str, user_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'account_id': account.id, 'platform': platform, 'user_id': account.platform_id, 'handle': account.handle, 'display_name': account.display_name, 'access_token': access_token, 'refresh_token': refresh_token, 'expires_at': account.token_expires_at, 'is_expired': is_expired, 'scope': account.oauth_scopes, 'is_active': account.is_active})
        except Exception as e:
    ...
    def update_tokens(self, account_id: int, access_token: str | None=None, refresh_token: str | None=None, expires_at: datetime | None=None) -> StepResult:
    ...
    def list_accounts(self, tenant: str, workspace: str, platform: str | None=None, active_only: bool=True) -> StepResult:
        ...
        try:
            ...
            for account in accounts:
                account_data = {'id': account.id, 'platform': account.platform, 'handle': account.handle, 'display_name': account.display_name, 'platform_id': account.platform_id, 'has_tokens': bool(account.access_token_encrypted), 'expires_at': account.token_expires_at, 'is_expired': account.token_expires_at and account.token_expires_at <= datetime.utcnow(), 'is_active': account.is_active, 'created_at': account.created_at, 'updated_at': account.updated_at}
    ...
    def cleanup_expired_tokens(self, tenant: str, workspace: str) -> StepResult:
        ...
        session = self._get_session()
        try:
            expired_accounts = session.query(Account).filter_by(tenant=tenant, workspace=workspace, is_active=True).filter(Account.token_expires_at <= datetime.utcnow()).all()
            cleaned_count = 0
    ...
    def get_expiring_tokens(self, tenant: str, workspace: str, days_ahead: int=7) -> StepResult:
        ...
        try:
            ...
            expiring_accounts = session.query(Account).filter_by(tenant=tenant, workspace=workspace, is_active=True).filter(Account.token_expires_at <= cutoff_date, Account.token_expires_at > datetime.utcnow()).all()
            expiring_list = []
            for account in expiring_accounts:
                expiring_data = {'id': account.id, 'platform': account.platform, 'handle': account.handle, 'display_name': account.display_name, 'expires_at': account.token_expires_at, 'has_refresh_token': bool(account.refresh_token_encrypted)}
