...
class PlatformSecrets:
    ...
    def __init__(self, vault: SecretsVault | None=None) -> None:
        ...
        self.platform_secrets = {'youtube': {'api_key': 'YouTube Data API key', 'client_id': 'YouTube OAuth client ID', 'client_secret': 'YouTube OAuth client secret'}, 'twitch': {'client_id': 'Twitch API client ID', 'client_secret': 'Twitch API client secret', 'webhook_secret': 'Twitch webhook secret'}, 'tiktok': {'client_key': 'TikTok API client key', 'client_secret': 'TikTok API client secret'}, 'instagram': {'access_token': 'Instagram Graph API access token', 'app_id': 'Instagram app ID', 'app_secret': 'Instagram app secret'}, 'x': {'api_key': 'X API key', 'api_secret': 'X API secret', 'access_token': 'X access token', 'access_token_secret': 'X access token secret'}}
    ...
    def validate_platform_secrets(self, platform: str) -> StepResult:
        ...
        return StepResult.ok(data={'platform': platform, 'missing': missing_secrets, 'present': present_secrets, 'complete': len(missing_secrets) == 0})
...
def validate_secrets_configuration() -> StepResult:
    ...
    try:
        ...
        if not encryption_key:
            return StepResult.fail('CREATOR_OPS_ENCRYPTION_KEY not set. Generate one with: python -c "from creator_ops.auth.vault import generate_encryption_key; print(generate_encryption_key())"')
