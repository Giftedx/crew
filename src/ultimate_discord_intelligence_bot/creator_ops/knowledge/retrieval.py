...
class ContentRetriever:
    ...
    def __init__(self, config: CreatorOpsConfig | None=None, db_session: Session | None=None, embeddings_generator: EmbeddingsGenerator | None=None) -> None:
    ...
    def search_episodes(self, query_params: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            for media in results:
                episode_data = {'id': str(media.id), 'title': media.title or 'Untitled', 'creator': media.account.handle, 'platform': media.account.platform, 'published_at': media.created_at, 'duration': media.duration or 0, 'url': media.url or '', 'thumbnail_url': media.thumbnail_url, 'description': media.description, 'view_count': media.view_count, 'like_count': media.like_count, 'comment_count': media.comment_count}
    ...
    def semantic_search(self, search_params: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            search_result = self.embeddings_generator.search_similar_content(query=query, collection_name='creator_ops_transcripts', limit=limit, score_threshold=0.7)
            ...
            for result in search_result.data['results']:
                ...
                if 'clip' in metadata.get('content_type', '').lower():
                    ...
                    content_type = 'post'
                search_result_data = {'content_id': metadata.get('segment_index', 'unknown'), 'content_type': content_type, 'platform': metadata.get('platform', 'unknown'), 'title': metadata.get('title', 'Untitled'), 'relevance_score': result.score, 'url': metadata.get('url', ''), 'metadata': metadata, 'matched_terms': [query]}
    ...
    def get_clip(self, clip_id: str) -> StepResult:
        ...
        try:
            ...
            clip_data = {'id': str(unit.id), 'title': unit.title or 'Untitled Clip', 'episode_id': str(unit.media_id), 'start_time': unit.start_time or 0, 'end_time': unit.end_time or 0, 'duration': (unit.end_time or 0) - (unit.start_time or 0), 'url': unit.url, 'thumbnail_url': unit.thumbnail_url, 'transcript': unit.transcript}
    ...
    def find_similar_clips(self, reference_clip: dict[str, Any], similarity_threshold: float=0.7, limit: int=10) -> StepResult:
        """
        ...
        try:
            ...
            search_result = self.embeddings_generator.search_similar_content(query=transcript, collection_name='creator_ops_transcripts', limit=limit, score_threshold=similarity_threshold)
            if not search_result.success:
                return search_result
            similar_clips = []
            for result in search_result.data['results']:
                metadata = result.metadata
                clip_data = {'id': metadata.get('segment_index', 'unknown'), 'title': metadata.get('title', 'Similar Clip'), 'episode_id': metadata.get('episode_id', 'unknown'), 'start_time': metadata.get('start_time', 0), 'end_time': metadata.get('end_time', 0), 'duration': metadata.get('end_time', 0) - metadata.get('start_time', 0), 'url': metadata.get('url', ''), 'thumbnail_url': metadata.get('thumbnail_url'), 'transcript': result.text, 'speakers': metadata.get('speakers', []), 'topics': metadata.get('topics', []), 'sentiment': metadata.get('sentiment'), 'engagement_score': result.score}
        ...
        try:
            ...
            for interaction in interactions:
                if interaction.type == 'comment':
                    comment_data = {'id': str(interaction.id), 'text': interaction.content, 'author': interaction.author, 'timestamp': interaction.created_at, 'sentiment': interaction.sentiment, 'topics': interaction.topics or []}
                    interaction_data['comments'].append(comment_data)
                elif interaction.type == 'reaction':
                    reaction_type = interaction.content
                    interaction_data['reactions'][reaction_type] = interaction_data['reactions'].get(reaction_type, 0) + 1
                elif interaction.type == 'share':
                    share_data = {'id': str(interaction.id), 'platform': interaction.platform, 'timestamp': interaction.created_at}
    ...
    def get_audience_demographics(self, episode_id: str) -> StepResult:
        ...
        try:
            if not self.db_session:
                return StepResult.fail('Database session not available')
            interactions = self.db_session.query(Interaction).filter(Interaction.media_id == episode_id).all()
            demographics = {'total_interactions': len(interactions), 'unique_authors': len({i.author for i in interactions if i.author}), 'platform_distribution': {}, 'time_distribution': {}}
            for interaction in interactions:
                platform = interaction.platform or 'unknown'
                demographics['platform_distribution'][platform] = demographics['platform_distribution'].get(platform, 0) + 1
            for interaction in interactions:
