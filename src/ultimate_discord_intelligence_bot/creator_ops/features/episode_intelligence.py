...
from ultimate_discord_intelligence_bot.creator_ops.features.intelligence_models import AgendaItem, BrandSafetyAnalysis, ClaimStatus, DefamationRisk, EpisodeIntelligence, FactCheckableClaim, GuestInfo, IntelligenceConfig, IntelligenceResult, NotableQuotation, OutboundLink, RiskLevel, ThumbnailSuggestion
...
class EpisodeIntelligencePack:
    ...
    async def generate_intelligence_pack(self, episode_id: str, transcript: AlignedTranscript, nlp_result: NLPResult | None=None, intelligence_config: IntelligenceConfig | None=None) -> StepResult:
        """
        ...
        try:
            ...
            intelligence = EpisodeIntelligence(episode_id=episode_id, episode_title=transcript.metadata.get('title', 'Untitled Episode'), episode_duration=transcript.duration, created_at=start_time, updated_at=start_time, total_speakers=len({segment.speakers[0] for segment in transcript.segments if segment.speakers}), total_segments=len(transcript.segments))
            ...
            result = IntelligenceResult(episode_id=episode_id, success=True, intelligence_pack=intelligence, processing_time=processing_time, metadata={'agenda_items': len(intelligence.agenda), 'guests': len(intelligence.guests), 'claims': len(intelligence.claims), 'quotations': len(intelligence.quotations), 'links': len(intelligence.links), 'thumbnail_suggestions': len(intelligence.thumbnail_suggestions), 'export_formats': intelligence.export_formats})
    ...
    async def _generate_agenda(self, transcript: AlignedTranscript, config: IntelligenceConfig) -> StepResult:
        ...
        try:
            ...
            for segment in transcript.segments:
                ...
                if not current_topic or not any((topic in current_topics for topic in segment_topics)):
                    if current_topic and current_start_time is not None:
                        agenda_item = AgendaItem(title=current_topic, start_time=current_start_time, end_time=segment.start_time, duration=segment.start_time - current_start_time, description=f'Discussion about {current_topic}', speakers=list(current_speakers), topics=list(current_topics))
            ...
            if current_topic and current_start_time is not None:
                ...
                agenda_item = AgendaItem(title=current_topic, start_time=current_start_time, end_time=end_time, duration=end_time - current_start_time, description=f'Discussion about {current_topic}', speakers=list(current_speakers), topics=list(current_topics))
                agenda_items.append(agenda_item)
        ...
        try:
            ...
            for segment in transcript.segments:
                for speaker in segment.speakers:
                    if speaker not in speaker_stats:
                        speaker_stats[speaker] = {'total_time': 0, 'segments': 0, 'first_mentioned': segment.start_time, 'contributions': []}
            ...
            for speaker, stats in speaker_stats.items():
                ...
                if speaking_percentage > 0.4:
                    ...
                    role = 'Participant'
                guest = GuestInfo(name=speaker, role=role, bio=f'{role} with {stats['segments']} speaking segments', first_mentioned=stats['first_mentioned'], total_speaking_time=stats['total_time'], segments=stats['segments'], key_contributions=stats['contributions'][:5], expertise_areas=self._extract_expertise_areas(stats['contributions']))
                guests.append(guest)
    ...
    async def _generate_claims(self, transcript: AlignedTranscript, nlp_result: NLPResult | None, config: IntelligenceConfig) -> StepResult:
        """Generate fact-checkable claims from transcript."""
        try:
            claims = []
            claim_id = 0
            claim_indicators = ['according to', 'studies show', 'research indicates', 'statistics show', 'data reveals', 'experts say', 'scientists found', "it's been proven", 'the truth is', 'actually', 'in fact', 'the reality is']
            for segment in transcript.segments:
                ...
                if any((indicator in text_lower for indicator in claim_indicators)):
                    ...
                    if confidence >= config.min_claim_confidence:
                        claim = FactCheckableClaim(claim_id=f'claim_{claim_id}', text=segment.text, speaker=segment.speakers[0] if segment.speakers else 'Unknown', timestamp=segment.start_time, context=f'Segment at {segment.start_time:.1f}s', claim_type=claim_type, status=ClaimStatus.UNVERIFIED, confidence=confidence, sources_mentioned=self._extract_sources(segment.text), verification_notes='Requires manual verification')
                        claims.append(claim)
                        claim_id += 1
    ...
    async def _generate_quotations(self, transcript: AlignedTranscript, config: IntelligenceConfig) -> StepResult:
        ...
        try:
            quotations = []
            quote_id = 0
            quotation_indicators = ['quote', 'saying', 'proverb', 'wise words', 'insight', 'perspective', 'opinion', 'belief', 'philosophy']
            for segment in transcript.segments:
                ...
                if len(text) >= config.min_quotation_length:
                    ...
                    if has_indicators or significance > 0.5 or viral_potential > 0.5:
                        quotation = NotableQuotation(quote_id=f'quote_{quote_id}', text=text, speaker=segment.speakers[0] if segment.speakers else 'Unknown', timestamp=segment.start_time, context=f'Segment at {segment.start_time:.1f}s', significance=f'Significance score: {significance:.2f}', topics=segment.topics or [], sentiment=segment.sentiment_score or 0.0, viral_potential=viral_potential)
    ...
    async def _generate_links(self, transcript: AlignedTranscript) -> StepResult:
        ...
        try:
            ...
            for segment in transcript.segments:
                ...
                for url in urls:
                    link = OutboundLink(url=url, title=self._extract_link_title(url), description=f'Link mentioned by {(segment.speakers[0] if segment.speakers else 'Unknown')}', mentioned_by=segment.speakers[0] if segment.speakers else 'Unknown', timestamp=segment.start_time, context=segment.text, link_type=self._determine_link_type(url), domain=self._extract_domain(url), is_affiliate=self._is_affiliate_link(url), is_sponsored=self._is_sponsored_link(url))
                    links.append(link)
    ...
    async def _generate_thumbnail_suggestions(self, transcript: AlignedTranscript, config: IntelligenceConfig) -> StepResult:
        ...
        try:
            ...
            for i, (timestamp, score) in enumerate(engagement_scores[:config.max_thumbnail_suggestions]):
                thumbnail = ThumbnailSuggestion(timestamp=timestamp, frame_url=f'frame_at_{timestamp:.1f}s.jpg', description=f'High-engagement moment at {timestamp:.1f}s', engagement_score=score, visual_elements=['speaker', 'text_overlay'], text_overlay=self._generate_thumbnail_text(transcript, timestamp), recommended=i == 0)
    ...
    async def _generate_brand_safety_analysis(self, transcript: AlignedTranscript, nlp_result: NLPResult | None) -> StepResult:
        ...
        try:
            ...
            for segment in transcript.segments:
                ...
                if segment_toxicity < -0.5:
                    ...
                    flagged_segments.append({'timestamp': segment.start_time, 'text': segment.text, 'reason': 'negative_sentiment', 'score': abs(segment_toxicity)})
                if segment.topics:
            ...
            brand_safety = BrandSafetyAnalysis(overall_score=overall_score, toxicity_score=toxicity_score, controversy_score=controversy_score, advertiser_friendliness=advertiser_friendliness, flagged_segments=flagged_segments, content_warnings=content_warnings, brand_mentions=brand_mentions)
    ...
    async def _generate_defamation_risk(self, transcript: AlignedTranscript, claims: list[FactCheckableClaim], config: IntelligenceConfig) -> StepResult:
        ...
        try:
            ...
            for claim in claims:
                if claim.status == ClaimStatus.UNVERIFIED:
                    ...
                    if self._mentions_individual(claim.text):
                        ...
                        flagged_statements.append({'timestamp': claim.timestamp, 'text': claim.text, 'reason': 'unverified_claim_about_individual', 'risk': 'medium'})
                    if self._mentions_organization(claim.text):
                        risk_score += 0.2
                        organizations_mentioned.extend(self._extract_organizations(claim.text))
                        flagged_statements.append({'timestamp': claim.timestamp, 'text': claim.text, 'reason': 'unverified_claim_about_organization', 'risk': 'medium'})
            ...
            defamation_risk = DefamationRisk(risk_level=risk_level, risk_score=risk_score, flagged_statements=flagged_statements, individuals_mentioned=list(set(individuals_mentioned)), organizations_mentioned=list(set(organizations_mentioned)), unverified_claims=unverified_claims, recommendations=recommendations)
    ...
    async def _export_intelligence_pack(self, intelligence: EpisodeIntelligence, config: IntelligenceConfig) -> StepResult:
    ...
    def _intelligence_to_dict(self, intelligence: EpisodeIntelligence) -> dict:
        ...
        return {'episode_id': intelligence.episode_id, 'episode_title': intelligence.episode_title, 'episode_duration': intelligence.episode_duration, 'created_at': intelligence.created_at.isoformat(), 'updated_at': intelligence.updated_at.isoformat(), 'agenda': [{'title': item.title, 'start_time': item.start_time, 'end_time': item.end_time, 'duration': item.duration, 'description': item.description, 'speakers': item.speakers, 'topics': item.topics, 'key_points': item.key_points} for item in intelligence.agenda], 'guests': [{'name': guest.name, 'role': guest.role, 'bio': guest.bio, 'first_mentioned': guest.first_mentioned, 'total_speaking_time': guest.total_speaking_time, 'segments': guest.segments, 'key_contributions': guest.key_contributions, 'social_links': guest.social_links, 'expertise_areas': guest.expertise_areas} for guest in intelligence.guests], 'claims': [{'claim_id': claim.claim_id, 'text': claim.text, 'speaker': claim.speaker, 'timestamp': claim.timestamp, 'context': claim.context, 'claim_type': claim.claim_type, 'status': claim.status.value, 'confidence': claim.confidence, 'sources_mentioned': claim.sources_mentioned, 'verification_notes': claim.verification_notes, 'risk_assessment': claim.risk_assessment.value} for claim in intelligence.claims], 'quotations': [{'quote_id': quote.quote_id, 'text': quote.text, 'speaker': quote.speaker, 'timestamp': quote.timestamp, 'context': quote.context, 'significance': quote.significance, 'topics': quote.topics, 'sentiment': quote.sentiment, 'viral_potential': quote.viral_potential} for quote in intelligence.quotations], 'links': [{'url': link.url, 'title': link.title, 'description': link.description, 'mentioned_by': link.mentioned_by, 'timestamp': link.timestamp, 'context': link.context, 'link_type': link.link_type, 'domain': link.domain, 'is_affiliate': link.is_affiliate, 'is_sponsored': link.is_sponsored} for link in intelligence.links], 'brand_safety': {'overall_score': intelligence.brand_safety.overall_score, 'toxicity_score': intelligence.brand_safety.toxicity_score, 'controversy_score': intelligence.brand_safety.controversy_score, 'advertiser_friendliness': intelligence.brand_safety.advertiser_friendliness, 'flagged_segments': intelligence.brand_safety.flagged_segments, 'content_warnings': intelligence.brand_safety.content_warnings, 'brand_mentions': intelligence.brand_safety.brand_mentions} if intelligence.brand_safety else None, 'defamation_risk': {'risk_level': intelligence.defamation_risk.risk_level.value, 'risk_score': intelligence.defamation_risk.risk_score, 'flagged_statements': intelligence.defamation_risk.flagged_statements, 'individuals_mentioned': intelligence.defamation_risk.individuals_mentioned, 'organizations_mentioned': intelligence.defamation_risk.organizations_mentioned, 'recommendations': intelligence.defamation_risk.recommendations} if intelligence.defamation_risk else None, 'total_speakers': intelligence.total_speakers, 'total_segments': intelligence.total_segments, 'average_sentiment': intelligence.average_sentiment, 'top_topics': intelligence.top_topics, 'key_insights': intelligence.key_insights, 'export_formats': intelligence.export_formats, 'file_paths': intelligence.file_paths}
    ...
    def _is_controversial_topic(self, topic: str) -> bool:
        ...
        controversial_topics = ['politics', 'religion', 'controversy', 'scandal', 'breaking news', 'conspiracy', 'conspiracy theory', 'controversial', 'debate']
    ...
    def _extract_expertise_areas(self, contributions: list[str]) -> list[str]:
        ...
        expertise_areas = []
        expertise_keywords = {'technology': ['tech', 'software', 'programming', 'ai', 'machine learning'], 'business': ['business', 'entrepreneur', 'startup', 'marketing', 'sales'], 'science': ['science', 'research', 'study', 'experiment', 'data'], 'entertainment': ['entertainment', 'music', 'movie', 'tv', 'celebrity'], 'politics': ['politics', 'government', 'policy', 'election', 'democracy']}
