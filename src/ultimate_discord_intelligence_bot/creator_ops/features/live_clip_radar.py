...
class LiveClipRadar:
    ...
    async def start_monitoring(self, config: MonitoringConfig, on_moment_detected: Callable[[ViralMoment], None] | None=None, on_clip_generated: Callable[[ClipCandidate], None] | None=None) -> StepResult:
        """
    ...
    async def _monitor_youtube_stream(self, stream_key: str, config: MonitoringConfig, stream_info: StreamInfo) -> None:
        ...
        try:
            ...
            for message_data in chat_result.data.get('messages', []):
                chat_message = ChatMessage(message_id=message_data.get('id', ''), user_id=message_data.get('authorChannelId', ''), username=message_data.get('authorDisplayName', ''), message=message_data.get('displayMessage', ''), timestamp=datetime.fromisoformat(message_data.get('publishedAt', '').replace('Z', '+00:00')), platform=PlatformType.YOUTUBE, channel_id=config.channel_id, stream_id=stream_info.stream_id, emotes=self._extract_emotes(message_data.get('displayMessage', '')), metadata=message_data)
    ...
    async def _monitor_twitch_stream(self, stream_key: str, config: MonitoringConfig, stream_info: StreamInfo) -> None:
        ...
        try:
            ...
            if not chat_result.success:
                ...
                return
            for message_data in chat_result.data.get('messages', []):
                chat_message = ChatMessage(message_id=message_data.get('id', ''), user_id=message_data.get('user_id', ''), username=message_data.get('username', ''), message=message_data.get('message', ''), timestamp=datetime.fromisoformat(message_data.get('timestamp', '').replace('Z', '+00:00')), platform=PlatformType.TWITCH, channel_id=config.channel_id, stream_id=stream_info.stream_id, emotes=self._extract_emotes(message_data.get('message', '')), badges=message_data.get('badges', []), metadata=message_data)
                await self._process_chat_message(stream_key, chat_message, config)
    ...
    async def _process_chat_message(self, stream_key: str, message: ChatMessage, config: MonitoringConfig) -> None:
        ...
        try:
            ...
            self.sentiment_history[stream_key].append({'timestamp': message.timestamp, 'sentiment': sentiment.score, 'message': message.message})
            await self._check_viral_moments(stream_key, message, sentiment, config)
        except Exception as e:
            logger.error(f'Failed to process chat message: {e!s}')

    async def _check_viral_moments(self, stream_key: str, trigger_message: ChatMessage, sentiment: SentimentScore, config: MonitoringConfig) -> None:
    ...
    async def _check_chat_velocity(self, stream_key: str, trigger_message: ChatMessage, config: MonitoringConfig) -> ViralMoment | None:
        ...
        try:
            ...
            if baseline > 0 and current_velocity > baseline * config.chat_velocity_threshold:
                ...
                moment = ViralMoment(moment_id=str(uuid.uuid4()), moment_type=MomentType.CHAT_VELOCITY, timestamp=trigger_message.timestamp, platform=trigger_message.platform, channel_id=trigger_message.channel_id, stream_id=trigger_message.stream_id, confidence=min(1.0, velocity_ratio / config.chat_velocity_threshold), trigger_message=trigger_message, context_messages=list(recent_messages[-10:]), metrics={'current_velocity': current_velocity, 'baseline_velocity': baseline, 'velocity_ratio': velocity_ratio}, description=f'Chat velocity spike: {current_velocity} messages/min (baseline: {baseline:.1f})')
                return moment
            return None
        except Exception as e:
    ...
    async def _check_sentiment_flip(self, stream_key: str, trigger_message: ChatMessage, current_sentiment: SentimentScore, config: MonitoringConfig) -> ViralMoment | None:
        ...
        try:
            ...
            if sentiment_change > config.sentiment_flip_threshold:
                ...
                moment = ViralMoment(moment_id=str(uuid.uuid4()), moment_type=MomentType.SENTIMENT_FLIP, timestamp=trigger_message.timestamp, platform=trigger_message.platform, channel_id=trigger_message.channel_id, stream_id=trigger_message.stream_id, confidence=min(1.0, sentiment_change / config.sentiment_flip_threshold), trigger_message=trigger_message, context_messages=list(self.chat_queues[stream_key])[-10:], metrics={'sentiment_change': sentiment_change, 'previous_avg': avg_older, 'current_avg': avg_recent, 'flip_type': flip_type}, description=f'Sentiment flip: {flip_type} (change: {sentiment_change:.2f})')
                return moment
            return None
        except Exception as e:
    ...
    async def _check_laughter(self, stream_key: str, trigger_message: ChatMessage, config: MonitoringConfig) -> ViralMoment | None:
        ...
        try:
            ...
            repeated_patterns = re.findall('(.)\\1{2,}', message_lower)
            laughter_count += len(repeated_patterns)
            emoji_patterns = re.findall('[😂🤣😆😄😃😀]', trigger_message.message)
            ...
            if laughter_count > 0:
                ...
                confidence = min(1.0, laughter_count / 5.0)
                if confidence > config.detection_sensitivity:
                    moment = ViralMoment(moment_id=str(uuid.uuid4()), moment_type=MomentType.LAUGHTER, timestamp=trigger_message.timestamp, platform=trigger_message.platform, channel_id=trigger_message.channel_id, stream_id=trigger_message.stream_id, confidence=confidence, trigger_message=trigger_message, context_messages=list(recent_messages[-5:]), metrics={'laughter_count': laughter_count, 'total_laughter': total_laughter, 'laughter_keywords': config.laughter_keywords}, description=f'Laughter detected: {laughter_count} indicators in recent messages')
                    return moment
            return None
        except Exception as e:
    ...
    async def _generate_clip_candidate(self, moment: ViralMoment, config: MonitoringConfig) -> ClipCandidate | None:
        ...
        try:
            ...
            clip_candidate = ClipCandidate(clip_id=str(uuid.uuid4()), moment_id=moment.moment_id, title=title, description=description, start_time=start_time, end_time=end_time, duration=clip_duration, platform=moment.platform, stream_id=moment.stream_id, channel_id=moment.channel_id, status=ClipStatus.DETECTED, viral_moment=moment, metadata={'moment_type': moment.moment_type.value, 'confidence': moment.confidence, 'trigger_message': moment.trigger_message.message if moment.trigger_message else ''})
    ...
    async def _create_stream_marker(self, moment: ViralMoment, config: MonitoringConfig) -> None:
        ...
        try:
            if moment.platform == PlatformType.TWITCH:
                marker_result = await self.twitch_client.create_stream_marker(user_id=config.channel_id, description=f'Viral moment: {moment.moment_type.value} - {moment.description}')
    ...
    async def _generate_clip_title(self, moment: ViralMoment) -> str:
        ...
        try:
            moment_type_titles = {MomentType.CHAT_VELOCITY: 'Chat Goes Wild!', MomentType.SENTIMENT_FLIP: 'Mood Shift Moment', MomentType.LAUGHTER: 'Hilarious Moment', MomentType.EMOTIONAL_PEAK: 'Emotional Peak', MomentType.CONTROVERSY: 'Controversial Moment', MomentType.BREAKING_NEWS: 'Breaking News'}
    ...
    async def _generate_clip_description(self, moment: ViralMoment) -> str:
        ...
        try:
            description_parts = [f'Detected viral moment: {moment.moment_type.value}', f'Confidence: {moment.confidence:.1%}', f'Timestamp: {moment.timestamp.strftime('%H:%M:%S')}']
            if moment.trigger_message:
    ...
    async def _get_stream_info(self, platform: PlatformType, channel_id: str) -> StepResult:
        ...
        try:
            if platform == PlatformType.YOUTUBE:
                ...
                stream_info = StreamInfo(stream_id=stream_data.get('id', ''), channel_id=channel_id, platform=platform, title=stream_data.get('snippet', {}).get('title', ''), description=stream_data.get('snippet', {}).get('description', ''), status=StreamStatus.ONLINE, started_at=datetime.fromisoformat(stream_data.get('snippet', {}).get('publishedAt', '').replace('Z', '+00:00')), viewer_count=stream_data.get('liveStreamingDetails', {}).get('concurrentViewers', 0), language=stream_data.get('snippet', {}).get('defaultLanguage', 'en'), category=stream_data.get('snippet', {}).get('categoryId', ''), metadata=stream_data)
                ...
                stream_data = stream_result.data['stream']
                stream_info = StreamInfo(stream_id=stream_data.get('id', ''), channel_id=channel_id, platform=platform, title=stream_data.get('title', ''), description=stream_data.get('description', ''), status=StreamStatus.ONLINE, started_at=datetime.fromisoformat(stream_data.get('started_at', '').replace('Z', '+00:00')), viewer_count=stream_data.get('viewer_count', 0), language=stream_data.get('language', 'en'), category=stream_data.get('game_name', ''), metadata=stream_data)
            else:
    ...
    async def _analyze_sentiment(self, text: str) -> SentimentScore:
        ...
        try:
            ...
            return SentimentScore(score=score, confidence=confidence, label=label, keywords=positive_keywords + negative_keywords)
        except Exception as e:
    ...
    def _calculate_baseline_velocity(self, stream_key: str) -> float:
        ...
        try:
            ...
            if len(recent_messages) < 5:
                return 1.0
            time_span = (now - ten_minutes_ago).total_seconds() / 60.0
    ...
    async def get_monitoring_status(self) -> StepResult:
        ...
        try:
            status = {'active_streams': len(self.active_streams), 'monitoring_configs': len(self.monitoring_configs), 'viral_moments_detected': len(self.viral_moments), 'clip_candidates_generated': len(self.clip_candidates), 'streams': list(self.active_streams.keys()), 'recent_moments': [{'moment_id': moment.moment_id, 'type': moment.moment_type.value, 'timestamp': moment.timestamp.isoformat(), 'confidence': moment.confidence, 'description': moment.description} for moment in self.viral_moments[-10:]]}
