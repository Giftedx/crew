...
from ultimate_discord_intelligence_bot.creator_ops.features.clip_radar_models import ClipCandidate, ClipStatus, MonitoringConfig, ViralMoment
from platform.core.step_result import StepResult
logger = logging.getLogger(__name__)

class ViralMomentAnalysis(BaseModel):
    """Analysis result for viral moment detection."""
    moment_quality_score: float
...
class LiveClipRadarAgent:
    ...
    def _get_llm(self):
        ...
        if hasattr(self.config, 'openrouter_api_key') and self.config.openrouter_api_key:
            ...
            return ChatOpenAI(model='anthropic/claude-3-sonnet', api_key=self.config.openrouter_api_key, base_url='https://openrouter.ai/api/v1')
        else:
            from langchain_openai import ChatOpenAI
    ...
    def _create_agents(self) -> dict[str, Agent]:
        ...
        agents = {}
        agents['viral_analyst'] = Agent(role='Viral Moment Analyst', goal='Analyze viral moments for quality, potential, and optimization opportunities', backstory='You are an expert in viral content analysis with deep understanding of what makes content go viral across different platforms.\n            You specialize in identifying high-potential moments, predicting engagement, and optimizing content for maximum reach.\n            You understand audience psychology, platform algorithms, and viral content patterns.', verbose=True, allow_delegation=False, llm=self.llm)
        agents['clip_optimizer'] = Agent(role='Clip Optimization Specialist', goal='Optimize clip candidates for maximum engagement and viral potential', backstory='You are a clip optimization expert who understands how to make content perform better on different platforms.\n            You know about platform-specific requirements, optimal timing, hooks, and engagement strategies.\n            You help creators maximize the potential of their viral moments.', verbose=True, allow_delegation=False, llm=self.llm)
        agents['engagement_predictor'] = Agent(role='Engagement Prediction Specialist', goal='Predict audience engagement and viral potential for clip candidates', backstory='You are an engagement prediction expert with deep knowledge of audience behavior and viral content patterns.\n            You analyze content characteristics, audience demographics, and platform trends to predict performance.\n            You help creators understand what content will resonate with their audience.', verbose=True, allow_delegation=False, llm=self.llm)
        agents['platform_strategist'] = Agent(role='Platform Strategy Specialist', goal='Develop platform-specific strategies for clip distribution and optimization', backstory='You are a platform strategy expert who understands the unique characteristics and requirements of each social media platform.\n            You know about platform algorithms, audience preferences, and optimization strategies for YouTube, TikTok, Instagram, and Twitter.\n            You help creators adapt content for maximum impact on each platform.', verbose=True, allow_delegation=False, llm=self.llm)
        return agents

    def _create_tasks(self) -> dict[str, Task]:
        """Create tasks for viral moment analysis."""
        tasks = {}
        tasks['viral_analysis'] = Task(description='\n            Analyze the viral moment and provide insights on:\n            1. Moment quality and authenticity\n            2. Viral potential and likelihood\n            3. Audience engagement predictions\n            4. Optimization opportunities\n            5. Risk assessment and considerations\n\n            Provide specific, actionable insights that help creators understand and optimize their viral moments.\n            ', agent=self.agents['viral_analyst'], expected_output='Comprehensive viral moment analysis with quality scores and optimization recommendations')
        tasks['clip_optimization'] = Task(description='\n            Optimize the clip candidate for maximum engagement:\n            1. Title optimization for different platforms\n            2. Description and hook optimization\n            3. Timing and pacing recommendations\n            4. Platform-specific adaptations\n            5. Hashtag and metadata optimization\n\n            Provide specific optimization strategies for each platform.\n            ', agent=self.agents['clip_optimizer'], expected_output='Detailed clip optimization recommendations with platform-specific strategies')
        tasks['engagement_prediction'] = Task(description='\n            Predict audience engagement and viral potential:\n            1. Engagement metrics predictions (views, likes, shares, comments)\n            2. Viral potential assessment\n            3. Target audience identification\n            4. Performance timeline predictions\n            5. Success probability analysis\n\n            Provide data-driven predictions based on content characteristics and audience patterns.\n            ', agent=self.agents['engagement_predictor'], expected_output='Engagement predictions with specific metrics and success probability')
        tasks['platform_strategy'] = Task(description='\n            Develop platform-specific distribution strategies:\n            1. YouTube optimization (SEO, thumbnails, descriptions)\n            2. TikTok optimization (trends, sounds, hashtags)\n            3. Instagram Reels optimization (hooks, captions, timing)\n            4. Twitter/X optimization (threads, timing, engagement)\n            5. Cross-platform coordination strategies\n\n            Provide specific strategies for each platform to maximize reach and engagement.\n            ', agent=self.agents['platform_strategist'], expected_output='Platform-specific strategies with actionable recommendations')
        return tasks

    def _create_crew(self) -> Crew:
        """Create the viral moment analysis crew."""
        return Crew(agents=list(self.agents.values()), tasks=list(self.tasks.values()), verbose=True, memory=True, planning=True, cache=True, max_rpm=10)

    async def analyze_viral_moment(self, viral_moment: ViralMoment, clip_candidate: ClipCandidate | None=None, monitoring_config: MonitoringConfig | None=None) -> StepResult:
        """
        ...
        try:
            ...
            inputs = {'viral_moment': viral_moment, 'clip_candidate': clip_candidate, 'monitoring_config': monitoring_config, 'platform': viral_moment.platform.value, 'moment_type': viral_moment.moment_type.value, 'confidence': viral_moment.confidence, 'description': viral_moment.description, 'trigger_message': viral_moment.trigger_message.message if viral_moment.trigger_message else '', 'context_messages': [msg.message for msg in viral_moment.context_messages], 'metrics': viral_moment.metrics}
            result = self.crew.kickoff(inputs=inputs)
            analysis = self._parse_crew_results(result)
            viral_analysis = ViralMomentAnalysis(moment_quality_score=analysis.get('quality_score', 0.7), viral_potential=analysis.get('viral_potential', 0.6), engagement_prediction=analysis.get('engagement_prediction', {}), optimization_suggestions=analysis.get('optimization_suggestions', []), clip_recommendations=analysis.get('clip_recommendations', []), risk_assessment=analysis.get('risk_assessment', 'Low risk'), target_audience=analysis.get('target_audience', []))
    ...
    async def optimize_clip_candidate(self, clip_candidate: ClipCandidate, viral_analysis: ViralMomentAnalysis | None=None) -> StepResult:
        """
        ...
        try:
            ...
            inputs = {'clip_candidate': clip_candidate, 'viral_analysis': viral_analysis, 'title': clip_candidate.title, 'description': clip_candidate.description, 'duration': clip_candidate.duration, 'platform': clip_candidate.platform.value, 'viral_moment': clip_candidate.viral_moment, 'moment_type': clip_candidate.viral_moment.moment_type.value, 'confidence': clip_candidate.viral_moment.confidence}
            result = self.crew.kickoff(inputs=inputs)
            optimization = self._parse_crew_results(result)
            clip_optimization = ClipOptimization(title_optimization=optimization.get('title_optimization', clip_candidate.title), description_optimization=optimization.get('description_optimization', clip_candidate.description), timing_optimization=optimization.get('timing_optimization', {}), platform_specific_hooks=optimization.get('platform_hooks', {}), hashtag_recommendations=optimization.get('hashtags', []), thumbnail_suggestions=optimization.get('thumbnails', []), engagement_strategies=optimization.get('engagement_strategies', []))
    ...
    def _parse_crew_results(self, result) -> dict:
        ...
        try:
            analysis = {'quality_score': 0.7, 'viral_potential': 0.6, 'engagement_prediction': {}, 'optimization_suggestions': [], 'clip_recommendations': [], 'risk_assessment': 'Low risk', 'target_audience': [], 'title_optimization': '', 'description_optimization': '', 'timing_optimization': {}, 'platform_hooks': {}, 'hashtags': [], 'thumbnails': [], 'engagement_strategies': []}
            ...
            return {'quality_score': 0.7, 'viral_potential': 0.6, 'engagement_prediction': {}, 'optimization_suggestions': [], 'clip_recommendations': [], 'risk_assessment': 'Low risk', 'target_audience': []}
    ...
    def _extract_score(self, text: str, keyword: str) -> float:
        ...
        try:
            import re
            patterns = [f'{keyword}.*?(\\d+\\.?\\d*)', f'{keyword}.*?(\\d+)%', f'(\\d+\\.?\\d*).*?{keyword}']
    ...
    async def generate_optimized_clip(self, clip_candidate: ClipCandidate, viral_analysis: ViralMomentAnalysis, clip_optimization: ClipOptimization) -> StepResult:
        """Generate an optimized clip based on analysis and optimization."""
        try:
            optimized_clip = ClipCandidate(clip_id=clip_candidate.clip_id, moment_id=clip_candidate.moment_id, title=clip_optimization.title_optimization, description=clip_optimization.description_optimization, start_time=clip_candidate.start_time, end_time=clip_candidate.end_time, duration=clip_candidate.duration, platform=clip_candidate.platform, stream_id=clip_candidate.stream_id, channel_id=clip_candidate.channel_id, status=ClipStatus.PROCESSING, viral_moment=clip_candidate.viral_moment, metadata={**clip_candidate.metadata, 'optimized': True, 'quality_score': viral_analysis.moment_quality_score, 'viral_potential': viral_analysis.viral_potential, 'platform_hooks': clip_optimization.platform_specific_hooks, 'hashtags': clip_optimization.hashtag_recommendations, 'engagement_strategies': clip_optimization.engagement_strategies}, created_at=clip_candidate.created_at, updated_at=datetime.now())
    ...
    async def get_analysis_summary(self, viral_analysis: ViralMomentAnalysis, clip_optimization: ClipOptimization) -> StepResult:
        """Get a summary of the analysis and optimization."""
        try:
            summary = {'viral_moment_quality': viral_analysis.moment_quality_score, 'viral_potential': viral_analysis.viral_potential, 'engagement_predictions': viral_analysis.engagement_prediction, 'optimization_highlights': {'title': clip_optimization.title_optimization, 'description': clip_optimization.description_optimization, 'hashtags': clip_optimization.hashtag_recommendations[:3], 'platforms': list(clip_optimization.platform_specific_hooks.keys())}, 'recommendations': {'optimization': viral_analysis.optimization_suggestions[:3], 'engagement': clip_optimization.engagement_strategies[:3], 'clips': viral_analysis.clip_recommendations[:3]}, 'risk_assessment': viral_analysis.risk_assessment, 'target_audience': viral_analysis.target_audience}
