...
from ultimate_discord_intelligence_bot.creator_ops.features.repurposing_models import CaptionStyle, ClipCandidate, ClipStatus, FFmpegConfig, PlatformHook, PlatformType, RepurposingConfig, RepurposingJob, RepurposingResult, VideoClip
...
class RepurposingStudio:
    ...
    async def repurpose_episode(self, episode_id: str, video_file_path: str, transcript: AlignedTranscript, target_platforms: list[PlatformType], repurposing_config: RepurposingConfig | None=None) -> StepResult:
        """
        ...
        try:
            ...
            job = RepurposingJob(job_id=f'repurpose_{episode_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}', episode_id=episode_id, episode_title=transcript.metadata.get('title', 'Untitled Episode'), episode_duration=transcript.duration, target_platforms=target_platforms, clip_count=repurposing_config.target_clip_count, status=ClipStatus.PENDING, created_at=datetime.now(), updated_at=datetime.now())
            ...
            result = RepurposingResult(job_id=job.job_id, success=len(clips) > 0, clips_created=len(clips), total_duration=sum((clip.duration for clip in clips)), platforms_targeted=target_platforms, output_files=[clip.file_path for clip in clips], metadata={'episode_id': episode_id, 'episode_title': job.episode_title, 'episode_duration': job.episode_duration, 'clip_candidates_found': len(candidates), 'clips_generated': len(clips), 'platforms_targeted': [p.value for p in target_platforms]})
    ...
    async def _identify_clip_candidates(self, transcript: AlignedTranscript, config: RepurposingConfig) -> StepResult:
        ...
        try:
            ...
            for segment in transcript.segments:
                ...
                candidate = ClipCandidate(start_time=segment.start_time, end_time=segment.end_time, duration=segment.end_time - segment.start_time, transcript_segment=segment.text, speakers=segment.speakers, topics=segment.topics or [], sentiment_score=segment.sentiment_score or 0.0, engagement_score=engagement_score, viral_potential=viral_potential, reason=reason)
    ...
    async def _generate_clip(self, job: RepurposingJob, candidate: ClipCandidate, video_file_path: str, config: RepurposingConfig) -> StepResult:
        ...
        try:
            ...
            for platform in job.target_platforms:
                ...
                ffmpeg_config = FFmpegConfig(input_file=video_file_path, output_file=str(clip_dir / f'{clip_id}.mp4'), start_time=candidate.start_time, end_time=candidate.end_time, aspect_ratio=aspect_ratio, resolution=resolution, metadata={'title': candidate.transcript_segment[:100], 'description': candidate.transcript_segment, 'platform': platform.value})
                ...
                clip = VideoClip(clip_id=clip_id, episode_id=job.episode_id, title=candidate.transcript_segment[:100], description=candidate.transcript_segment, start_time=candidate.start_time, end_time=candidate.end_time, duration=candidate.duration, aspect_ratio=aspect_ratio, resolution=resolution, file_path=ffmpeg_config.output_file, thumbnail_path=str(clip_dir / f'{clip_id}_thumb.jpg'), captions_path=captions_path, status=ClipStatus.COMPLETED, created_at=datetime.now(), updated_at=datetime.now(), metadata={'platform': platform.value, 'engagement_score': candidate.engagement_score, 'viral_potential': candidate.viral_potential, 'speakers': candidate.speakers, 'topics': candidate.topics, 'selection_reason': candidate.reason})
                clips.append(clip)
    ...
    async def _process_video_with_ffmpeg(self, ffmpeg_config: FFmpegConfig, config: RepurposingConfig) -> StepResult:
        ...
        try:
            cmd = ['ffmpeg', '-i', ffmpeg_config.input_file, '-ss', str(ffmpeg_config.start_time), '-t', str(ffmpeg_config.end_time - ffmpeg_config.start_time), '-vf', f'scale={ffmpeg_config.resolution.split('x')[0]}:{ffmpeg_config.resolution.split('x')[1]}', '-c:v', 'libx264', '-b:v', ffmpeg_config.bitrate, '-c:a', 'aac', '-b:a', ffmpeg_config.audio_bitrate, '-r', str(ffmpeg_config.fps), '-y', ffmpeg_config.output_file]
            ...
            thumbnail_cmd = ['ffmpeg', '-i', ffmpeg_config.output_file, '-ss', '00:00:01', '-vframes', '1', '-y', ffmpeg_config.output_file.replace('.mp4', '_thumb.jpg')]
            thumbnail_process = await asyncio.create_subprocess_exec(*thumbnail_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    ...
    async def _generate_captions(self, candidate: ClipCandidate, clip_dir: Path, clip_id: str, caption_style: CaptionStyle) -> StepResult:
    ...
    async def _generate_platform_hooks(self, job: RepurposingJob, transcript: AlignedTranscript) -> StepResult:
        ...
        try:
            ...
            for platform in job.target_platforms:
                ...
                hook = PlatformHook(platform=platform, hook_text=hook_text, hashtags=hashtags, mentions=[], call_to_action=call_to_action, trending_topics=[], engagement_tips=self._get_engagement_tips(platform))
                hooks.append(hook)
    ...
    def _get_engagement_tips(self, platform: PlatformType) -> list[str]:
        ...
        if platform == PlatformType.YOUTUBE_SHORTS:
            return ['Post during peak hours (6-9 PM)', 'Use trending audio', 'Keep it under 60 seconds', 'Add captions for accessibility']
        elif platform == PlatformType.TIKTOK:
            return ['Post during peak hours (6-9 PM)', 'Use trending hashtags', 'Keep it under 60 seconds', 'Add captions for accessibility']
        elif platform == PlatformType.INSTAGRAM_REELS:
            return ['Post during peak hours (6-9 PM)', 'Use trending audio', 'Keep it under 90 seconds', 'Add captions for accessibility']
        elif platform == PlatformType.X:
            return ['Post during peak hours (9-11 AM, 1-3 PM)', 'Use trending hashtags', 'Keep it concise', 'Add images/videos for engagement']
        else:
