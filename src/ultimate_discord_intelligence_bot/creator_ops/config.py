...
class CreatorOpsConfig:
    ...
    def _initialize_platform_configs(self) -> None:
        ...
        self.platforms['youtube'] = PlatformConfig(name='youtube', api_base_url='https://www.googleapis.com/youtube/v3', rate_limit_requests_per_minute=100, rate_limit_requests_per_hour=10000, rate_limit_requests_per_day=100000, quota_units_per_request={'search': 100, 'videos': 1, 'channels': 1, 'comments': 1, 'live_chat': 1, 'captions': 1}, timeout_seconds=30, retry_attempts=3, retry_backoff_factor=1.0, enabled=os.getenv('ENABLE_YOUTUBE', 'true').lower() == 'true')
        self.platforms['twitch'] = PlatformConfig(name='twitch', api_base_url='https://api.twitch.tv/helix', rate_limit_requests_per_minute=800, rate_limit_requests_per_hour=8000, rate_limit_requests_per_day=80000, timeout_seconds=30, retry_attempts=3, retry_backoff_factor=1.0, enabled=os.getenv('ENABLE_TWITCH', 'true').lower() == 'true')
        self.platforms['tiktok'] = PlatformConfig(name='tiktok', api_base_url='https://open-api.tiktok.com', rate_limit_requests_per_minute=60, rate_limit_requests_per_hour=1000, rate_limit_requests_per_day=10000, timeout_seconds=30, retry_attempts=3, retry_backoff_factor=2.0, enabled=os.getenv('ENABLE_TIKTOK', 'true').lower() == 'true')
        self.platforms['instagram'] = PlatformConfig(name='instagram', api_base_url='https://graph.facebook.com/v18.0', rate_limit_requests_per_minute=200, rate_limit_requests_per_hour=4800, rate_limit_requests_per_day=50000, timeout_seconds=30, retry_attempts=3, retry_backoff_factor=1.0, enabled=os.getenv('ENABLE_INSTAGRAM', 'true').lower() == 'true')
        self.platforms['x'] = PlatformConfig(name='x', api_base_url='https://api.twitter.com/2', rate_limit_requests_per_minute=15, rate_limit_requests_per_hour=300, rate_limit_requests_per_day=30000, timeout_seconds=30, retry_attempts=3, retry_backoff_factor=1.0, enabled=os.getenv('ENABLE_X', 'true').lower() == 'true')
    ...
    def validate_configuration(self) -> StepResult:
        ...
        try:
            ...
            if self.real_apis:
                required_env_vars = {'youtube': ['YOUTUBE_API_KEY'], 'twitch': ['TWITCH_CLIENT_ID', 'TWITCH_CLIENT_SECRET'], 'tiktok': ['TIKTOK_CLIENT_KEY', 'TIKTOK_CLIENT_SECRET'], 'instagram': ['INSTAGRAM_ACCESS_TOKEN'], 'x': ['X_API_KEY', 'X_API_SECRET']}
            ...
            return StepResult.ok(data={'status': 'valid', 'enabled_platforms': enabled_platforms, 'warnings': warnings, 'fixture_mode': self.fixture_mode})
    ...
    def get_rate_limit_config(self, platform: str) -> dict[str, int]:
        ...
        if not config:
            return {}
        return {'requests_per_minute': config.rate_limit_requests_per_minute, 'requests_per_hour': config.rate_limit_requests_per_hour, 'requests_per_day': config.rate_limit_requests_per_day, 'timeout_seconds': config.timeout_seconds, 'retry_attempts': config.retry_attempts, 'retry_backoff_factor': config.retry_backoff_factor}
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        """
        return {'enabled': self.enabled, 'real_apis': self.real_apis, 'fixture_mode': self.fixture_mode, 'platforms': {name: {'name': config.name, 'api_base_url': config.api_base_url, 'rate_limit_requests_per_minute': config.rate_limit_requests_per_minute, 'rate_limit_requests_per_hour': config.rate_limit_requests_per_hour, 'rate_limit_requests_per_day': config.rate_limit_requests_per_day, 'enabled': config.enabled} for name, config in self.platforms.items()}, 'processing': {'max_workers': self.max_workers, 'batch_size': self.batch_size, 'processing_timeout_seconds': self.processing_timeout_seconds}, 'ml': {'whisper_model': self.whisper_model, 'use_gpu': self.use_gpu, 'embedding_model': self.embedding_model}, 'security': {'data_retention_days': self.data_retention_days, 'enable_audit_logging': self.enable_audit_logging}}
