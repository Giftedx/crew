...
class NLPPipeline:
    ...
    def __init__(self, config: CreatorOpsConfig | None=None, device: str | None=None) -> None:
    ...
    def _initialize_models(self) -> None:
        ...
        if pipeline is None:
            raise ImportError('transformers not available. Install ML dependencies: pip install transformers torch')
        try:
            self.sentiment_pipeline = pipeline('sentiment-analysis', model='cardiffnlp/twitter-roberta-base-sentiment-latest', device=0 if self.device == 'cuda' else -1)
            self.toxicity_pipeline = pipeline('text-classification', model='unitary/toxic-bert', device=0 if self.device == 'cuda' else -1)
    ...
    def analyze_transcript(self, aligned_transcript: AlignedTranscript, analysis_options: dict[str, bool] | None=None) -> StepResult:
        """
        ...
        try:
            options = analysis_options or {'topic_segmentation': True, 'keyphrase_extraction': True, 'entity_extraction': True, 'claim_detection': True, 'sentiment_analysis': True, 'content_safety': True}
            topic_segments = []
            ...
            result = NLPResult(topic_segments=topic_segments, keyphrases=keyphrases, entities=entities, claims=claims, sentiment_analysis=sentiment_analysis, content_safety=content_safety, processing_time=processing_time, model_versions={'sentiment': 'cardiffnlp/twitter-roberta-base-sentiment-latest', 'toxicity': 'unitary/toxic-bert', 'ner': 'dbmdz/bert-large-cased-finetuned-conll03-english'}, created_at=start_time)
    ...
    def _segment_topics(self, segments: list[AlignedSegment]) -> list[TopicSegment]:
        ...
        for i, segment in enumerate(segments):
            ...
            if is_topic_boundary and i > current_topic_start:
                topic_segment = TopicSegment(start_time=segments[current_topic_start].start_time, end_time=segments[i - 1].end_time, topic=current_topic, confidence=0.8, keywords=topic_keywords, segment_indices=list(range(current_topic_start, i)))
        ...
        if current_topic_start < len(segments):
            topic_segment = TopicSegment(start_time=segments[current_topic_start].start_time, end_time=segments[-1].end_time, topic=current_topic, confidence=0.8, keywords=topic_keywords, segment_indices=list(range(current_topic_start, len(segments))))
            topic_segments.append(topic_segment)
            ...
            if freq > 2:
                ...
                for segment in segments:
                    if word in segment.text.lower():
                        keyphrase = Keyphrase(text=word, score=score, start_time=segment.start_time, end_time=segment.end_time, speaker=segment.speaker)
                        keyphrases.append(keyphrase)
                        break
        keyphrases.sort(key=lambda x: x.score, reverse=True)
        return keyphrases[:50]
        ...
        for segment in segments:
            ...
            try:
                ...
                for entity in ner_results:
                    if entity['score'] > 0.5:
                        entity_obj = Entity(text=entity['word'], label=entity['entity'], confidence=entity['score'], start_time=segment.start_time, end_time=segment.end_time, speaker=segment.speaker)
    ...
    def _detect_claims(self, segments: list[AlignedSegment]) -> list[Claim]:
        ...
        claims = []
        claim_indicators = ['according to', 'studies show', 'research indicates', "it's a fact that", 'the truth is', 'scientists say', 'experts believe', 'data shows', 'statistics prove']
        for segment in segments:
            ...
            for indicator in claim_indicators:
                if indicator in text_lower:
                    ...
                    if any((word in text_lower for word in ['study', 'research', 'data', 'statistics'])):
                        ...
                        verifiability = 'low'
                    claim = Claim(text=segment.text, confidence=0.7, start_time=segment.start_time, end_time=segment.end_time, speaker=segment.speaker, claim_type='factual', verifiability=verifiability)
                    claims.append(claim)
                    break
        return claims
    ...
    def _analyze_sentiment(self, segments: list[AlignedSegment]) -> list[SentimentAnalysis]:
        ...
        for segment in segments:
            ...
            try:
                ...
                sentiment = SentimentAnalysis(label=label_mapping.get(sentiment_result[0]['label'], 'neutral'), score=sentiment_result[0]['score'], start_time=segment.start_time, end_time=segment.end_time, speaker=segment.speaker)
    ...
    def _analyze_content_safety(self, segments: list[AlignedSegment]) -> list[ContentSafety]:
        ...
        for segment in segments:
            ...
            try:
                ...
                controversy_flags = []
                text_lower = segment.text.lower()
                controversial_topics = ['politics', 'religion', 'race', 'gender', 'sexuality', 'conspiracy', 'fake news', 'misinformation']
                ...
                if toxicity_score > 0.7 or len(controversy_flags) > 2:
                    ...
                    risk_level = 'medium'
                safety = ContentSafety(toxicity_score=toxicity_score, brand_suitability_score=brand_suitability_score, controversy_flags=controversy_flags, risk_level=risk_level, start_time=segment.start_time, end_time=segment.end_time, speaker=segment.speaker)
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        return {'device': self.device, 'has_gpu': torch.cuda.is_available() if torch is not None else False, 'models_loaded': {'sentiment': self.sentiment_pipeline is not None, 'toxicity': self.toxicity_pipeline is not None, 'ner': self.ner_pipeline is not None}}
