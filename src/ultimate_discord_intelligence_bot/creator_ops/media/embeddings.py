...
class EmbeddingsGenerator:
    ...
    def _initialize_model(self) -> None:
        ...
        if SentenceTransformer is None:
            raise ImportError(
                "sentence-transformers not available. Install ML dependencies: pip install sentence-transformers torch"
            )
        try:
    ...
    def generate_embeddings(
        self, texts: list[str], metadata_list: list[dict[str, Any]] | None = None, batch_size: int = 32
    ) -> StepResult:
        """
            ...
            for i in range(0, len(texts), batch_size):
                ...
                for _j, (text, metadata, embedding) in enumerate(
                    ...
                    embedding_obj = Embedding(
                        vector=embedding.tolist(), text=text, metadata=metadata, created_at=datetime.utcnow()
                    )
    ...
    def generate_transcript_embeddings(
        ...
        try:
            ...
            return StepResult.ok(
                data={"stored_ids": stored_ids, "collection_name": collection_name, "embedding_count": len(stored_ids)}
            )
        except Exception as e:
