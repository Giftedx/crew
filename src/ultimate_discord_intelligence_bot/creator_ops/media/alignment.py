...
class TranscriptAlignment:
    ...
    def _initialize_cleanup_patterns(self) -> dict[str, re.Pattern]:
        ...
        return {
            "filler_words": re.compile(
                "\\b(um|uh|er|ah|like|you know|basically|literally|actually|so|well)\\b", re.IGNORECASE
            ),
            "stutters": re.compile("\\b(\\w+)\\s+\\1\\b", re.IGNORECASE),
            "repeated_words": re.compile("\\b(\\w+)\\s+\\1\\s+\\1\\b", re.IGNORECASE),
            "extra_spaces": re.compile("\\s+"),
            "punctuation_spacing": re.compile("\\s+([,.!?;:])"),
            "quotes": re.compile('["""]'),
            "dashes": re.compile("[\\u2014\\u2013-]+"),
        }
    ...
    def _align_segments(
        self, asr_segments: list[ASRSegment], diarization_segments: list[DiarizationSegment]
    ) -> list[AlignedSegment]:
        ...
        for asr_segment in asr_segments:
            overlapping_speakers = self._find_overlapping_speakers(
                asr_segment.start_time, asr_segment.end_time, speaker_map
            )
            primary_speaker = self._determine_primary_speaker(
                asr_segment.start_time, asr_segment.end_time, overlapping_speakers
            )
    ...
    def _find_overlapping_speakers(
        self, start_time: float, end_time: float, speaker_map: list[dict[str, Any]]
    ) -> list[str]:
    ...
    def _determine_primary_speaker(
        self, start_time: float, end_time: float, overlapping_speakers: list[str]
    ) -> str | None:
