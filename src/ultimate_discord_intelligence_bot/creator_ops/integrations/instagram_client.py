...
from ultimate_discord_intelligence_bot.creator_ops.integrations.instagram_models import InstagramAccountInsight, InstagramComment, InstagramError, InstagramHashtag, InstagramInsight, InstagramLongLivedToken, InstagramMedia, InstagramMediaInsight, InstagramStory, InstagramUser, InstagramWebhookEvent, InstagramWebhookSubscription
...
class InstagramClient:
    ...
    async def _get_access_token(self, creator_id: str) -> StepResult[str]:
        ...
        token_result = await self.oauth_manager.get_access_token(platform='instagram', creator_id=creator_id, scopes=self.scope.split(','))
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10), before_sleep=before_sleep_log(logger, logging.WARNING), retry_error_callback=lambda retry_state: StepResult.fail(f'Instagram API request failed after multiple retries: {retry_state.outcome.exception()}'), reraise=True)
    async def _make_api_request(self, method: str, url: str, creator_id: str, json: dict[str, Any] | None=None, params: dict[str, Any] | None=None, headers: dict[str, Any] | None=None, base_url: str=BASE_URL) -> StepResult[dict[str, Any]]:
    ...
    async def get_user_media(self, creator_id: str, limit: int=25, after: str | None=None, fields: list[str] | None=None) -> StepResult[tuple[list[InstagramMedia], str | None]]:
        """
        Fetches media for an Instagram user.
        Requires 'instagram_basic' scope. Supports pagination.
        """
        url = f'/{creator_id}/media'
        params = {'limit': limit, 'after': after, 'fields': ','.join(fields) if fields else None}
    ...
    async def get_media_comments(self, creator_id: str, media_id: str, limit: int=25, after: str | None=None) -> StepResult[tuple[list[InstagramComment], str | None]]:
        """
    ...
    async def get_user_stories(self, creator_id: str, fields: list[str] | None=None) -> StepResult[list[InstagramStory]]:
        """
    ...
    async def get_media_insights(self, creator_id: str, media_id: str, metrics: list[str]) -> StepResult[InstagramMediaInsight]:
        """
        ...
        return StepResult.success(InstagramMediaInsight(media_id=media_id, insights=insights, timestamp=datetime.utcnow()))

    async def get_account_insights(self, creator_id: str, metrics: list[str], period: str='day', since: datetime | None=None, until: datetime | None=None) -> StepResult[InstagramAccountInsight]:
        """
        ...
        for insight_data in insights_data:
            try:
                ...
                logger.warning(f'Failed to parse Instagram insight data: {e} - {insight_data}')
        return StepResult.success(InstagramAccountInsight(account_id=creator_id, insights=insights, timestamp=datetime.utcnow(), period=period))

    async def search_hashtags(self, creator_id: str, query: str, limit: int=25) -> StepResult[list[InstagramHashtag]]:
        """
    ...
    async def get_hashtag_media(self, creator_id: str, hashtag_id: str, limit: int=25, after: str | None=None) -> StepResult[tuple[list[InstagramMedia], str | None]]:
        """
    ...
    async def create_webhook_subscription(self, creator_id: str, callback_url: str, fields: list[str], verify_token: str) -> StepResult[InstagramWebhookSubscription]:
        """
        Creates a webhook subscription for Instagram events.
        Requires appropriate permissions.
        """
        url = f'/{self.app_id}/subscriptions'
        payload = {'object': 'instagram', 'callback_url': callback_url, 'fields': ','.join(fields), 'verify_token': verify_token}
    ...
    async def get_long_lived_token(self, creator_id: str, short_lived_token: str) -> StepResult[InstagramLongLivedToken]:
        """
        Exchanges a short-lived token for a long-lived token.
        """
        url = '/access_token'
        params = {'grant_type': 'ig_exchange_token', 'client_secret': self.app_secret, 'access_token': short_lived_token}
        result = await self._make_api_request('GET', url, creator_id, params=params)
        if not result.success:
            return result.as_failure()
        try:
    ...
    async def refresh_long_lived_token(self, creator_id: str, long_lived_token: str) -> StepResult[InstagramLongLivedToken]:
        """
    ...
    async def verify_webhook(self, mode: str, token: str, challenge: str) -> StepResult[str]:
        """
    ...
    async def process_webhook_event(self, event_data: dict[str, Any]) -> StepResult[InstagramWebhookEvent]:
        """
    ...
    async def get_rate_limit_info(self, creator_id: str) -> StepResult[dict[str, Any]]:
        ...
        """
        return StepResult.success({'platform': 'instagram', 'rate_limit_type': 'app_based', 'note': 'Instagram Graph API uses app-based rate limiting'})
