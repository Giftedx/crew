...
from ultimate_discord_intelligence_bot.creator_ops.integrations.x_models import XCostGuard, XError, XMedia, XPlace, XPoll, XRateLimit, XTweet, XUser, XWebhookEvent, XWebhookSubscription
...
class XClient:
    ...
    def _get_monthly_tweet_cap(self) -> int:
        ...
        caps = {'free': 0, 'basic': 10000, 'pro': 100000, 'enterprise': 1000000}
        return caps.get(self.api_tier, 0)

    def _get_cost_per_tweet(self) -> float:
        """Get cost per tweet based on API tier."""
        costs = {'free': 0.0, 'basic': 0.001, 'pro': 0.0005, 'enterprise': 0.0001}
    ...
    async def _get_access_token(self, creator_id: str) -> StepResult[str]:
        ...
        token_result = await self.oauth_manager.get_access_token(platform='x', creator_id=creator_id, scopes=self.scope.split(','))
    ...
    def _check_rate_limit(self, response: httpx.Response) -> XRateLimit:
        ...
        return XRateLimit(limit=int(headers.get('x-rate-limit-limit', 0)), remaining=int(headers.get('x-rate-limit-remaining', 0)), reset=datetime.fromtimestamp(int(headers.get('x-rate-limit-reset', 0))), retry_after=int(headers.get('retry-after', 0)) if headers.get('retry-after') else None)

    def _check_cost_guard(self) -> StepResult[XCostGuard]:
        """Check if we're within cost limits."""
        if self.api_tier == 'free':
            return StepResult.success(XCostGuard(tier=self.api_tier, monthly_tweet_cap=0, tweets_used=0, tweets_remaining=0, reset_date=datetime.utcnow() + timedelta(days=30), cost_per_tweet=0.0, estimated_monthly_cost=0.0))
        if self.tweets_used >= self.monthly_tweet_cap:
            return StepResult.fail('Monthly tweet cap exceeded')
        return StepResult.success(XCostGuard(tier=self.api_tier, monthly_tweet_cap=self.monthly_tweet_cap, tweets_used=self.tweets_used, tweets_remaining=self.monthly_tweet_cap - self.tweets_used, reset_date=datetime.utcnow() + timedelta(days=30), cost_per_tweet=self.cost_per_tweet, estimated_monthly_cost=self.tweets_used * self.cost_per_tweet))

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10), before_sleep=before_sleep_log(logger, logging.WARNING), retry_error_callback=lambda retry_state: StepResult.fail(f'X API request failed after multiple retries: {retry_state.outcome.exception()}'), reraise=True)
    async def _make_api_request(self, method: str, url: str, creator_id: str, json: dict[str, Any] | None=None, params: dict[str, Any] | None=None, headers: dict[str, Any] | None=None, base_url: str=BASE_URL) -> StepResult[tuple[dict[str, Any], XRateLimit]]:
        ...
        _headers = {'Authorization': f'Bearer {token}', 'Content-Type': 'application/json', **headers} if headers else {'Authorization': f'Bearer {token}', 'Content-Type': 'application/json'}
        try:
    ...
    async def get_user_by_username(self, creator_id: str, username: str, fields: list[str] | None=None) -> StepResult[XUser]:
        """
    ...
    async def get_user_by_id(self, creator_id: str, user_id: str, fields: list[str] | None=None) -> StepResult[XUser]:
        """
    ...
    async def get_user_timeline(self, creator_id: str, user_id: str, max_results: int=10, pagination_token: str | None=None, fields: list[str] | None=None) -> StepResult[tuple[list[XTweet], str | None, XRateLimit]]:
        """
        Fetches tweets from a user's timeline.
        Requires 'tweet.read' scope. Supports pagination.
        """
        url = f'/users/{user_id}/tweets'
        params = {'max_results': max_results, 'pagination_token': pagination_token, 'tweet.fields': ','.join(fields) if fields else None}
    ...
    async def get_user_mentions(self, creator_id: str, user_id: str, max_results: int=10, pagination_token: str | None=None, fields: list[str] | None=None) -> StepResult[tuple[list[XTweet], str | None, XRateLimit]]:
        """
    ...
    async def search_tweets(self, creator_id: str, query: str, max_results: int=10, next_token: str | None=None, fields: list[str] | None=None) -> StepResult[tuple[list[XTweet], str | None, XRateLimit]]:
        """
        Searches for tweets.
        Requires 'tweet.read' scope. Supports pagination.
        """
        url = '/tweets/search/recent'
        params = {'query': query, 'max_results': max_results, 'next_token': next_token, 'tweet.fields': ','.join(fields) if fields else None}
        params = {k: v for k, v in params.items() if v is not None}
        result = await self._make_api_request('GET', url, creator_id, params=params)
        if not result.success:
            return result.as_failure()
    ...
    async def get_tweet(self, creator_id: str, tweet_id: str, fields: list[str] | None=None) -> StepResult[XTweet]:
        """
    ...
    async def create_tweet(self, creator_id: str, text: str, reply_to_tweet_id: str | None=None, media_ids: list[str] | None=None) -> StepResult[XTweet]:
        """
        Creates a new tweet.
    ...
    async def delete_tweet(self, creator_id: str, tweet_id: str) -> StepResult[bool]:
        """
        Deletes a tweet.
        Requires 'tweet.write' scope.
        """
    ...
    async def get_tweet_media(self, creator_id: str, tweet_id: str) -> StepResult[list[XMedia]]:
        """
    ...
    async def get_tweet_poll(self, creator_id: str, tweet_id: str) -> StepResult[XPoll | None]:
        """
    ...
    async def get_tweet_place(self, creator_id: str, tweet_id: str) -> StepResult[XPlace | None]:
        """
    ...
    async def get_rate_limit_info(self, creator_id: str) -> StepResult[dict[str, Any]]:
        ...
        """
        return StepResult.success({'platform': 'x', 'api_tier': self.api_tier, 'monthly_tweet_cap': self.monthly_tweet_cap, 'tweets_used': self.tweets_used, 'tweets_remaining': self.monthly_tweet_cap - self.tweets_used, 'cost_per_tweet': self.cost_per_tweet, 'estimated_monthly_cost': self.tweets_used * self.cost_per_tweet})
    ...
    async def create_webhook_subscription(self, creator_id: str, webhook_url: str, events: list[str]) -> StepResult[XWebhookSubscription]:
        """
        Creates a webhook subscription for X events.
        Requires appropriate permissions.
        """
        return StepResult.success(XWebhookSubscription(id='webhook_123', url=webhook_url, valid=True, created_timestamp=datetime.utcnow()))

    async def process_webhook_event(self, event_data: dict[str, Any]) -> StepResult[XWebhookEvent]:
        """
