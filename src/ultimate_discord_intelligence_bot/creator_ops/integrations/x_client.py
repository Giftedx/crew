...
class XClient:
    ...
    async def _make_api_request(
        ...
        _headers = (
            ...
            if headers
            else {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
        )
        try:
    ...
    async def get_user_by_username(
        self, creator_id: str, username: str, fields: list[str] | None = None
    ) -> StepResult[XUser]:
        """
    ...
    async def create_tweet(
        self, creator_id: str, text: str, reply_to_tweet_id: str | None = None, media_ids: list[str] | None = None
    ) -> StepResult[XTweet]:
        """
        Creates a new tweet.
    ...
    async def create_webhook_subscription(
        self, creator_id: str, webhook_url: str, events: list[str]
        ...
        """
        return StepResult.success(
            XWebhookSubscription(id="webhook_123", url=webhook_url, valid=True, created_timestamp=datetime.utcnow())
        )
