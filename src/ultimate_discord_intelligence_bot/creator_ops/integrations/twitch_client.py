...
from ultimate_discord_intelligence_bot.creator_ops.integrations.twitch_models import TwitchChatMessage, TwitchClip, TwitchEventSubEvent, TwitchRateLimit, TwitchStream, TwitchStreamMarker, TwitchUser, TwitchVideo, TwitchWebSocketMessage
...
class TwitchClient:
    ...
    def __init__(self, client_id: str | None=None, client_secret: str | None=None, oauth_manager: TwitchOAuthManager | None=None, config: CreatorOpsConfig | None=None, fixture_mode: bool=False) -> None:
    ...
    def get_streams(self, user_id: str | None=None, user_login: str | None=None, game_id: str | None=None, type: str | None=None, language: str | None=None, first: int=20) -> StepResult:
        """Get live streams."""
    ...
    def get_videos(self, user_id: str | None=None, game_id: str | None=None, id: str | None=None, language: str | None=None, period: str='all', sort: str='time', type: str='all', first: int=20) -> StepResult:
        """Get videos (VODs)."""
    ...
    def get_clips(self, broadcaster_id: str | None=None, game_id: str | None=None, id: str | None=None, started_at: datetime | None=None, ended_at: datetime | None=None, first: int=20) -> StepResult:
        """Get clips."""
        params = {'first': min(first, 100)}
        if broadcaster_id:
    ...
    def create_stream_marker(self, user_id: str, description: str | None=None) -> StepResult:
        ...
        if self.fixture_mode:
            mock_marker = {'id': 'test_marker_id', 'created_at': datetime.utcnow().isoformat() + 'Z', 'description': description or 'Test marker', 'position_seconds': 300, 'url': 'https://www.twitch.tv/videos/test_video?t=5m'}
    ...
    def get_eventsub_subscriptions(self) -> StepResult:
        ...
        return StepResult.ok(data={'subscriptions': subscriptions, 'total': data.get('total'), 'pagination': data.get('pagination')})

    def create_eventsub_subscription(self, type: str, version: str, condition: dict[str, Any], transport: dict[str, Any]) -> StepResult:
        """Create EventSub subscription."""
        if self.fixture_mode:
            mock_subscription = {'id': 'test_subscription_id', 'type': type, 'version': version, 'status': 'enabled', 'created_at': datetime.utcnow().isoformat() + 'Z', 'condition': condition, 'transport': transport, 'cost': 0}
    ...
    async def connect_eventsub_websocket(self, callback: Callable[[TwitchWebSocketMessage], None], max_duration_minutes: int=60) -> StepResult:
        """Connect to EventSub WebSocket and listen for events."""
        if self.fixture_mode:
            await asyncio.sleep(1)
            mock_message = TwitchWebSocketMessage(metadata={'message_id': 'test_message_id', 'message_type': 'notification', 'message_timestamp': datetime.utcnow().isoformat() + 'Z'}, payload={'subscription': {'id': 'test_subscription_id', 'type': 'stream.online', 'version': '1'}, 'event': {'id': 'test_stream_id', 'broadcaster_user_id': 'test_user_id', 'broadcaster_user_login': 'testuser', 'broadcaster_user_name': 'TestUser', 'type': 'live', 'started_at': datetime.utcnow().isoformat() + 'Z'}})
    ...
    def get_rate_limit_info(self) -> dict[str, Any]:
        ...
        return {'limit': self.rate_limit.limit, 'remaining': self.rate_limit.remaining, 'reset_time': self.rate_limit.reset_time.isoformat() if self.rate_limit.reset_time else None}
