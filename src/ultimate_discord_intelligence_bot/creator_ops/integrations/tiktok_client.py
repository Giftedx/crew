        ...
        try:
            with httpx.Client(timeout=30.0) as client:
                response = client.request(
                    method=method, url=url, params=params, json=json_data, headers=headers or self._get_headers()
                )
    ...
    def get_video_list(self, open_id: str | None = None, cursor: str | None = None, max_count: int = 20) -> StepResult:
        ...
        try:
            ...
            video_list = TikTokVideoList(
                videos=videos, cursor=video_data.get("cursor"), has_more=video_data.get("has_more", False)
            )
    ...
    def get_video_comments(self, video_id: str, cursor: str | None = None, max_count: int = 20) -> StepResult:
        ...
        try:
            ...
            comment_list = TikTokCommentList(
                comments=comments, cursor=comment_data.get("cursor"), has_more=comment_data.get("has_more", False)
            )
    ...
    def upload_video(
        ...
        try:
            ...
            json_data = {
                ...
                "source_info": {"source": "FILE_UPLOAD", "video_size": 0, "chunk_size": 0},
            }
            if hashtags:
