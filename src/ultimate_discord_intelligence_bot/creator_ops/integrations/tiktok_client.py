...
from ultimate_discord_intelligence_bot.creator_ops.integrations.tiktok_models import TikTokComment, TikTokCommentList, TikTokInsights, TikTokPublishStatus, TikTokUploadResponse, TikTokUser, TikTokVideo, TikTokVideoList
...
class TikTokClient:
    ...
    def _get_headers(self, include_auth: bool=True) -> dict[str, str]:
        """Get request headers."""
        headers = {'Content-Type': 'application/json', 'User-Agent': 'CreatorOps/1.0'}
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10), retry_error_callback=lambda retry_state: None)
    def _make_request(self, method: str, url: str, params: dict[str, Any] | None=None, json_data: dict[str, Any] | None=None, headers: dict[str, str] | None=None) -> httpx.Response:
        """Make HTTP request with retry logic for 5xx errors."""
        try:
            with httpx.Client(timeout=30.0) as client:
                response = client.request(method=method, url=url, params=params, json=json_data, headers=headers or self._get_headers())
    ...
    def get_video_list(self, open_id: str | None=None, cursor: str | None=None, max_count: int=20) -> StepResult:
        """
        ...
        try:
            ...
            video_list = TikTokVideoList(videos=videos, cursor=video_data.get('cursor'), has_more=video_data.get('has_more', False))
    ...
    def get_video_comments(self, video_id: str, cursor: str | None=None, max_count: int=20) -> StepResult:
        """
        Get video comments.
        ...
        try:
            ...
            params = {'video_id': video_id, 'max_count': min(max_count, 20)}
            if cursor:
                params['cursor'] = cursor
            response = self._make_request('GET', url, params=params)
            response.raise_for_status()
            ...
            comment_list = TikTokCommentList(comments=comments, cursor=comment_data.get('cursor'), has_more=comment_data.get('has_more', False))
    ...
    def get_video_insights(self, video_id: str, date_range: dict[str, str] | None=None) -> StepResult:
        """
        Get video insights.
        ...
        try:
            ...
            insights = TikTokInsights(video_id=video_id, view_count=insights_data.get('view_count', 0), like_count=insights_data.get('like_count', 0), comment_count=insights_data.get('comment_count', 0), share_count=insights_data.get('share_count', 0), profile_views=insights_data.get('profile_views', 0), video_views_by_region=insights_data.get('video_views_by_region', {}), video_views_by_age_group=insights_data.get('video_views_by_age_group', {}), video_views_by_gender=insights_data.get('video_views_by_gender', {}), date_range=date_range or {'start_date': '', 'end_date': ''})
    ...
    def upload_video(self, video_file_path: str, title: str, description: str | None=None, privacy_level: str='PUBLIC_TO_EVERYONE', hashtags: list[str] | None=None) -> StepResult:
        """
        ...
        try:
            ...
            json_data = {'post_info': {'title': title, 'description': description or '', 'privacy_level': privacy_level, 'disable_duet': False, 'disable_comment': False, 'disable_stitch': False, 'video_cover_timestamp_ms': 1000}, 'source_info': {'source': 'FILE_UPLOAD', 'video_size': 0, 'chunk_size': 0}}
            if hashtags:
            ...
            upload_response_data = TikTokUploadResponse(publish_id=publish_id, upload_url=upload_url, status='UPLOADED')
    ...
    def publish_video(self, publish_id: str) -> StepResult:
        ...
        try:
            ...
            status = TikTokPublishStatus(publish_id=publish_id, status=publish_data.get('status', 'UNKNOWN'), video_id=publish_data.get('video_id'), error_message=publish_data.get('error_message'), created_time=publish_data.get('created_time'))
    ...
    def get_publish_status(self, publish_id: str) -> StepResult:
        ...
        try:
            ...
            status = TikTokPublishStatus(publish_id=publish_id, status=status_data.get('status', 'UNKNOWN'), video_id=status_data.get('video_id'), error_message=status_data.get('error_message'), created_time=status_data.get('created_time'))
            return StepResult.ok(data=status)
        except Exception as e:
    ...
    def get_rate_limit_info(self) -> dict[str, Any]:
        ...
        return {'remaining': self.rate_limit_remaining, 'reset': self.rate_limit_reset, 'supported_regions': self.supported_regions}

    def _get_fixture_response(self, endpoint: str) -> dict[str, Any]:
        """Get fixture response for testing."""
        try:
            from pathlib import Path
            fixture_file = Path(__file__).parent.parent.parent.parent / 'fixtures' / 'creator_ops' / f'tiktok_{endpoint}.json'
