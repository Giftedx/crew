    ...
    def _run(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            try:
                ...
                if tool_cls.endswith("DownloadTool"):
                    ...
                    if not url_param:
                        validation_errors.append(
                            f"{tool_cls} requires url/video_url parameter but received None. LLM must provide URL in tool call."
                        )
                    ...
                    if not text_param or (isinstance(text_param, str) and len(text_param.strip()) < 10):
                        validation_errors.append(
                            f"{tool_cls} requires non-empty 'text' parameter (min 10 chars). Received: {(len(str(text_param)) if text_param else 0)} chars. This comes from transcription task output."
                        )
                    ...
                    if not text_param or (isinstance(text_param, str) and len(text_param.strip()) < 50):
                        validation_errors.append(
                            f"{tool_cls} requires substantial 'text' parameter (min 50 chars) to extract claims. Received: {(len(str(text_param)) if text_param else 0)} chars."
                        )
                    ...
                    if not claim_param or (isinstance(claim_param, str) and len(claim_param.strip()) < 5):
                        validation_errors.append(
                            f"{tool_cls} requires non-empty 'claim' parameter. Received: {(len(str(claim_param)) if claim_param else 0)} chars."
                        )
                elif "MemoryStorage" in tool_cls or "GraphMemory" in tool_cls:
                    text_param = final_kwargs.get("text") or final_kwargs.get("content")
                    if not text_param or (isinstance(text_param, str) and len(text_param.strip()) < 10):
                        validation_errors.append(
                            f"{tool_cls} requires non-empty text to store. Received: {(len(str(text_param)) if text_param else 0)} chars."
                        )
                ...
                return StepResult.fail(
                    error=f"{tool_cls} execution failed: {tool_error!s}",
                    tool=tool_cls,
                    args_provided=list(final_kwargs.keys()),
                )
            ...
            return StepResult.fail(
                error=msg,
                step="tool_execution",
                data={"tool_class": tool_cls, "provided_args": list(final_kwargs.keys()), "error_details": str(e)},
            )
        except Exception as e:
            import traceback
            ...
            return StepResult.fail(
                error=f"Tool {tool_cls} execution failed: {e!s}",
                step="tool_execution",
                data={
                    "tool_class": tool_cls,
...
class RAGIngestToolWrapper(BaseTool):
    ...
    def _run(
        self, texts: list | None = None, index: str = "memory", chunk_size: int = 400, overlap: int = 50, **kwargs
    ) -> Any:
...
class MemoryStorageToolWrapper(BaseTool):
    ...
    def _run(
        self, text: str = "sample memory", namespace: str = "default", metadata: dict | None = None, **kwargs
    ) -> Any:
