...
Usage:
  python -m ultimate_discord_intelligence_bot.scripts.google_oauth_setup     [--client-secrets /path/to/client.json] [--token-path /path/to/token.json]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--client-secrets', type=Path, default=CONFIG_DIR / 'google-oauth-client.json', help='Path to OAuth client secrets JSON (Desktop app)')
    parser.add_argument('--token-path', type=Path, default=Path(os.getenv('GOOGLE_OAUTH_TOKEN_PATH', str(CONFIG_DIR / 'google-oauth-token.json'))), help='Where to write the OAuth token JSON')
    parser.add_argument('--console', action='store_true', help='Use console-based OAuth flow (for headless servers)')
