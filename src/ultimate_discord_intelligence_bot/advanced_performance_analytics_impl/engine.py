...
from .health import calculate_system_health_score, get_health_status, get_key_health_indicators, perform_comparative_analysis
...
class AdvancedPerformanceAnalytics:
    ...
    def __init__(self, enhanced_monitor: EnhancedPerformanceMonitor | None=None, integration_manager: PerformanceIntegrationManager | None=None):
    ...
    async def analyze_comprehensive_performance(self, lookback_hours: int=24) -> dict[str, Any]:
        ...
        try:
            ...
            return {'analysis_timestamp': default_utc_now().isoformat(), 'lookback_hours': lookback_hours, 'system_health': {'overall_score': health_score, 'status': get_health_status(health_score), 'key_indicators': get_key_health_indicators(self)}, 'performance_trends': trends, 'anomalies': {'detected': len(anomalies), 'critical': len([a for a in anomalies if a.severity == 'critical']), 'recent': anomalies[-5:] if anomalies else []}, 'forecasts': forecasts, 'optimization_recommendations': {'critical': [r for r in recommendations if r.priority == 'critical'], 'high': [r for r in recommendations if r.priority == 'high'], 'medium': [r for r in recommendations if r.priority == 'medium'], 'total_recommendations': len(recommendations)}, 'comparative_insights': comparative_insights, 'actionable_insights': self._generate_actionable_insights(trends, anomalies, recommendations)}
        except Exception as e:
            logger.error(f'Comprehensive performance analysis failed: {e}')
            return {'error': str(e), 'timestamp': default_utc_now().isoformat()}

    def _generate_actionable_insights(self, trends: list[dict[str, Any]], anomalies: list[PerformanceAnomaly], recommendations: list[OptimizationRecommendation]) -> list[str]:
        insights: list[str] = []
        try:
            declining_quality_trends = [t for t in trends if 'quality' in t.get('metric_name', '') and t.get('trend_direction') == 'declining']
            if declining_quality_trends:
                insights.append(f'🔍 Quality declining in {len(declining_quality_trends)} agents - immediate attention needed')
            recent_critical = [a for a in anomalies if a.severity == 'critical']
            if recent_critical:
                insights.append(f'🚨 {len(recent_critical)} critical performance anomalies detected - investigate root causes')
