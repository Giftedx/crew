...
class CrossAgentLearningService:
    ...
    async def _extract_patterns(self, agent_id: str, agent_type: str) -> int:
        ...
        try:
            ...
            if (
                ...
                and len(self._success_tracker[agent_id]) >= self.config.pattern_extraction_threshold
            ):
            ...
            if (
                ...
                and len(self._failure_tracker[agent_id]) >= self.config.pattern_extraction_threshold
            ):
            ...
            for group in success_groups:
                if len(group) >= self.config.pattern_extraction_threshold:
                    pattern = await self._create_pattern_from_group(
                        group=group, learning_type=LearningType.SUCCESS_PATTERN, agent_type=agent_type
                    )
                    if pattern:
    ...
    async def _analyze_failure_patterns(self, agent_id: str, agent_type: str) -> list[LearningPattern]:
        ...
        try:
            ...
            for group in failure_groups:
                if len(group) >= self.config.pattern_extraction_threshold:
                    pattern = await self._create_pattern_from_group(
                        group=group, learning_type=LearningType.FAILURE_PATTERN, agent_type=agent_type
                    )
                    if pattern:
                        patterns.append(pattern)
            return patterns
    ...
    async def _create_pattern_from_group(
        ...
        try:
            ...
            pattern = LearningPattern(
                ...
                metadata={"created_from": "experience_analysis", "source_experiences": len(group)},
            )
            self._patterns[pattern_id] = pattern
    ...
    def _calculate_pattern_relevance(
        self, pattern: LearningPattern, context: dict[str, Any], requesting_agent_type: str
    ) -> float:
    ...
    def _calculate_learning_trends(self, experiences: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "overall_success_rate": len([exp for exp in experiences if exp["success"]]) / len(experiences),
            }
        except Exception as e:
