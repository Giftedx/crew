...
class AgentKnowledgeBridge:
    ...
    async def share_insight(self, agent_id: str, agent_type: str, insight_type: InsightType, title: str, description: str, context: dict[str, Any], tags: list[str], confidence_score: float, priority: InsightPriority=InsightPriority.NORMAL, expires_at: datetime | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            insight = AgentInsight(insight_id=insight_id, agent_id=agent_id, agent_type=agent_type, insight_type=insight_type, priority=priority, title=title, description=description, context=context, tags=tags, confidence_score=confidence_score, expires_at=expires_at, metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, 'shared_at': datetime.now(timezone.utc).isoformat()})
            ...
            return StepResult.ok(data={'insight_id': insight_id, 'shared': True, 'tags_indexed': len(tags), 'confidence': confidence_score})
        except Exception as e:
    ...
    async def request_insights(self, requesting_agent_id: str, agent_type: str, query: str, context: dict[str, Any], insight_types: list[InsightType], max_results: int=10, min_confidence: float=0.5, tags: list[str] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            relevant_insights = await self._search_insights(query=query, context=context, insight_types=insight_types, min_confidence=min_confidence, tags=tags, requesting_agent_id=requesting_agent_id, max_results=max_results)
            query_time = (datetime.now(timezone.utc) - start_time).total_seconds() * 1000
            response = InsightResponse(insights=relevant_insights, total_found=len(relevant_insights), query_time_ms=query_time, confidence_threshold=min_confidence, metadata={'query': query, 'requesting_agent': requesting_agent_id, 'agent_type': agent_type, 'tenant_id': tenant_id, 'workspace_id': workspace_id})
    ...
    async def validate_insight(self, insight_id: str, validating_agent_id: str, is_successful: bool, feedback: str | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            validation_record = {'validating_agent': validating_agent_id, 'is_successful': is_successful, 'feedback': feedback, 'validated_at': datetime.now(timezone.utc).isoformat(), 'tenant_id': tenant_id, 'workspace_id': workspace_id}
            ...
            return StepResult.ok(data={'validated': True, 'insight_id': insight_id, 'validation_count': insight.validation_count, 'success_rate': success_rate, 'updated_confidence': insight.confidence_score})
        except Exception as e:
            logger.error(f'Error validating insight: {e}', exc_info=True)
            return StepResult.fail(f'Insight validation failed: {e!s}')

    async def get_agent_insights(self, agent_id: str, limit: int=50, insight_types: list[InsightType] | None=None, min_confidence: float=0.0) -> StepResult:
    ...
    async def get_insight_statistics(self, tenant_id: str | None=None, workspace_id: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_insights': total_insights, 'total_validations': total_validations, 'total_successes': total_successes, 'success_rate': total_successes / max(total_validations, 1), 'average_confidence': avg_confidence, 'insight_types': type_counts, 'agent_types': agent_type_counts, 'top_tags': self._get_top_tags(filtered_insights), 'recent_insights': len([i for i in filtered_insights if (datetime.now(timezone.utc) - i.created_at).days < 7])}
        except Exception as e:
            logger.error(f'Error getting insight statistics: {e}')
            return {'error': str(e)}

    async def _search_insights(self, query: str, context: dict[str, Any], insight_types: list[InsightType], min_confidence: float, tags: list[str] | None, requesting_agent_id: str, max_results: int) -> list[AgentInsight]:
    ...
    def _calculate_relevance_score(self, insight: AgentInsight, query: str, context: dict[str, Any]) -> float:
        ...
        try:
            ...
            priority_bonus = {InsightPriority.LOW: 0.0, InsightPriority.NORMAL: 0.05, InsightPriority.HIGH: 0.1, InsightPriority.CRITICAL: 0.15}
    ...
    def get_bridge_status(self) -> dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'total_insights': len(self._insights), 'total_agents': len(self._agent_insights), 'total_tags': len(self._insight_index), 'total_validations': sum((len(validations) for validations in self._validation_history.values())), 'config': {'enable_insight_sharing': self.config.enable_insight_sharing, 'enable_cross_agent_learning': self.config.enable_cross_agent_learning, 'enable_insight_validation': self.config.enable_insight_validation, 'max_insights_per_agent': self.config.max_insights_per_agent}}