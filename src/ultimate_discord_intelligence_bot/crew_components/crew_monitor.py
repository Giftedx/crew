...
class CrewMonitor:
    ...
    def end_execution(self, result: Any, success: bool=True) -> None:
        ...
        if self.execution_history:
            self.execution_history[-1].update({'end_time': datetime.now(UTC).isoformat(), 'execution_time': execution_time, 'success': success, 'status': 'completed' if success else 'failed', 'result': str(result) if result else None})
        self.performance_metrics.update({'last_execution_time': execution_time, 'total_executions': len(self.execution_history), 'success_rate': self._calculate_success_rate(), 'average_execution_time': self._calculate_average_execution_time()})
    ...
    def get_execution_summary(self) -> dict[str, Any]:
        ...
        return {'total_executions': len(self.execution_history), 'recent_executions': recent_executions, 'performance_metrics': self.performance_metrics, 'current_status': self.execution_history[-1]['status'] if self.execution_history else 'idle'}
    ...
    def _calculate_average_execution_time(self) -> float:
        ...
        if not self.execution_history:
            return 0.0
        execution_times = [record.get('execution_time', 0) for record in self.execution_history if 'execution_time' in record]
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        recent_times = [record.get('execution_time', 0) for record in self.execution_history[-5:] if 'execution_time' in record]
        avg_recent_time = sum(recent_times) / len(recent_times) if recent_times else 0.0
        return {'status': 'healthy' if len(recent_failures) < 2 else 'degraded', 'recent_failures': len(recent_failures), 'average_execution_time': avg_recent_time, 'total_executions': len(self.execution_history), 'last_execution': self.execution_history[-1] if self.execution_history else None}