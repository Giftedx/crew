...
from ..tools import AudioTranscriptionTool, DiscordPostTool, GraphMemoryTool, HippoRagContinualMemoryTool, LogicalFallacyTool, MemoryStorageTool, PerspectiveSynthesizerTool, TextAnalysisTool
try:
    from ..tools.integration.drive_upload_tool import DriveUploadTool
except Exception:
    DriveUploadTool = None
...
class PipelineBase:
    ...
    def __init__(self, webhook_url: str | None=None, downloader: MultiPlatformDownloadTool | None=None, transcriber: AudioTranscriptionTool | None=None, analyzer: TextAnalysisTool | None=None, drive: DriveUploadTool | None=None, discord: DiscordPostTool | None=None, fallacy_detector: LogicalFallacyTool | None=None, perspective: PerspectiveSynthesizerTool | None=None, memory: MemoryStorageTool | None=None, graph_memory: GraphMemoryTool | None=None, hipporag_memory: HippoRagContinualMemoryTool | None=None, transcript_cache: TranscriptCache | None=None, pipeline_rate_limit: float=120.0, tool_rate_limit: float=120.0, step_middlewares: Iterable[PipelineStepMiddleware] | None=None) -> None:
        ...
        if analyzer is None:
            try:
                ...
                self.logger.warning('TextAnalysisTool unavailable (%s) - using degraded stub analyzer', exc)

                class _DegradedAnalyzer:
                    name = 'DegradedTextAnalysisTool'

                    def run(self, text: str) -> StepResult:
                        return StepResult.skip(reason='text_analysis_unavailable', details={'tokens': len(text.split()), 'note': 'NLTK missing'})
        ...
        hipporag_enabled = bool(getattr(settings, 'enable_hipporag_memory', False) or getattr(settings, 'enable_hipporag_continual_memory', False))
        ...
        self.pipeline_bucket = TokenBucket(rate=pipeline_rate_limit / 60.0, capacity=max(int(pipeline_rate_limit), 1))
        self.pipeline_limiter = self.pipeline_bucket
        self.tool_bucket = TokenBucket(rate=tool_rate_limit / 60.0, capacity=max(int(tool_rate_limit), 3))
    ...
    def _apply_pii_filtering(self, text: str, step: str) -> str:
        try:
            ...
            if report.found:
                self.logger.info('PII detected in %s: %s patterns found', step, sum(report.redacted_by_type.values()))
    ...
    def _maybe_compress_transcript(self, transcript: str) -> tuple[str, dict[str, Any] | None]:
        ...
        meta: dict[str, Any] = {'enabled': True, 'original_tokens': original_tokens, 'threshold': threshold}
        if threshold > 0 and original_tokens < threshold:
            meta.update({'applied': False, 'final_tokens': original_tokens, 'reason': 'below_threshold'})
        ...
        try:
            compressed, details = engine.optimise_with_metadata(transcript, target_token_reduction=ratio, max_tokens=max_tokens, llmlingua_ratio=ratio if ratio > 0 else None, force_enable=True)
        except Exception as exc:
            self.logger.warning('Transcript compression failed: %s', exc)
            meta.update({'applied': False, 'final_tokens': original_tokens, 'reason': 'compression_error'})
            return (transcript, meta)
        final_tokens: int
        ...
        meta.update({'applied': applied, 'compressed': compressed_changed, 'final_tokens': final_tokens, 'details': details})
        return (compressed, meta)

    async def _run_with_retries(self, func: Any, *args: Any, step: str, attempts: int=3, delay: float=2.0, check_tool_rate_limit: bool=True, **kwargs: Any) -> StepResult:
        import inspect
        ...
        for attempt in range(attempts):
            ...
            self.logger.warning('%s attempt %s failed (%s): %s', step, attempt + 1, classification, result.error)
            ...
            with contextlib.suppress(Exception):
                metrics.PIPELINE_RETRIES.labels(**metrics.label_ctx(), step=step, reason=classification).inc()
    ...
    async def _transcription_step(self, local_path: str, video_id: str | None, model_name: str | None) -> StepResult:
        ...
        if cached_payload is not None:
            return StepResult.ok(transcript=cached_payload['transcript'], segments=cached_payload.get('segments', []), cache_hit=True, cached_at=cached_payload.get('cached_at'), video_id=cached_payload.get('video_id', video_id), model=model_name)
        import inspect
