...
class ContentPipeline(PipelineExecutionMixin, PipelineBase):
    ...
    def _langfuse_start_span(self, ctx: _PipelineContext, name: str, input_data: dict[str, Any], metadata: dict[str, Any] | None=None) -> None:
        service = getattr(ctx, 'langfuse_service', None)
        trace = getattr(ctx, 'langfuse_trace', None)
        if not service or not trace:
            return
        result = service.create_span(trace, name, self._langfuse_prepare_payload(input_data), metadata=self._langfuse_prepare_payload(metadata or {}))
        if result.success:
            span = result.data.get('span')
            if span is not None:
                ctx.langfuse_spans[name] = span

    def _langfuse_finish_span(self, ctx: _PipelineContext, name: str, output_data: dict[str, Any] | None=None, *, error: str | None=None) -> None:
        service = getattr(ctx, 'langfuse_service', None)
        if not service:
            return
    ...
    def _langfuse_finalize_trace(self, ctx: _PipelineContext | None, status: str, payload: dict[str, Any], duration: float) -> None:
        ...
        if ctx.langfuse_pipeline_span:
            service.update_span(ctx.langfuse_pipeline_span, sanitized, error=error_message)
            ctx.langfuse_pipeline_span = None
        service.finalize_trace(ctx.langfuse_trace, sanitized, error=error_message, metadata=metadata)
        ctx.langfuse_trace_finalized = True
        ctx.langfuse_trace_error = error_message

    def _finalize_pipeline(self, start_time: float, status: str, payload: dict[str, Any], *, duration: float | None=None) -> dict[str, Any]:
    ...
    @contextmanager
    def _pipeline_context(self, url: str, quality: str, start_time: float, total_limit: float | None, per_task_limits: dict[str, float]) -> Iterator[_PipelineContext]:
        with ExitStack() as stack:
            tracker = stack.enter_context(self._pipeline_pkg.track_request_budget(total_limit=total_limit, per_task_limits=per_task_limits))
            ...
            if langfuse_service and getattr(langfuse_service, 'enabled', False):
                ...
                trace_result = langfuse_service.create_trace(name='pipeline.process_video', user_id=user_id, metadata={'orchestrator': self._orchestrator, 'workspace': workspace_id}, input_data={'url': url, 'quality': quality}, tags=[self._orchestrator])
                if trace_result.success:
                    langfuse_trace = trace_result.data.get('trace')
                    span_result = langfuse_service.create_span(langfuse_trace, 'pipeline_execution', {'url': url, 'quality': quality}, metadata={'phase': 'pipeline_start'})
                    if span_result.success:
                        langfuse_pipeline_span = span_result.data.get('span')
            ctx = _PipelineContext(span=span, start_time=start_time, tracker=tracker, langfuse_service=langfuse_service, langfuse_trace=langfuse_trace, langfuse_pipeline_span=langfuse_pipeline_span)
            ...
            try:
                ...
                if langfuse_service and langfuse_pipeline_span:
                    langfuse_service.update_span(langfuse_pipeline_span, {'status': 'exception', 'exception': str(exc)}, error=str(exc))
                    ctx.langfuse_pipeline_span = None
                if langfuse_service and langfuse_trace and (not ctx.langfuse_trace_finalized):
                    langfuse_service.finalize_trace(langfuse_trace, {'status': 'exception'}, error=str(exc))
                    ctx.langfuse_trace_finalized = True
                raise
            finally:
                self._active_pipeline_ctx = None
                if langfuse_service and ctx.langfuse_pipeline_span and (not ctx.langfuse_trace_finalized):
                    langfuse_service.update_span(ctx.langfuse_pipeline_span, ctx.langfuse_trace_output or {'status': 'unknown'}, error=ctx.langfuse_trace_error)
                    ctx.langfuse_pipeline_span = None
                if langfuse_service and langfuse_trace and (not ctx.langfuse_trace_finalized):
                    langfuse_service.finalize_trace(langfuse_trace, ctx.langfuse_trace_output or {'status': 'unknown'}, error=ctx.langfuse_trace_error)
                    ctx.langfuse_trace_finalized = True
    ...
    async def _run_pipeline(self, ctx: _PipelineContext, url: str, quality: str) -> PipelineRunResult:
        ...
        should_exit, exit_reason, exit_confidence = await self._check_early_exit_condition(ctx, 'post_download', {'duration': download_info.data.get('duration', 0), 'view_count': download_info.data.get('view_count', 0), 'age_days': download_info.data.get('age_days', 0), 'title_spam_score': self._calculate_spam_score(download_info.data.get('title', ''))})
        if should_exit:
        ...
        should_exit, exit_reason, exit_confidence = await self._check_early_exit_condition(ctx, 'post_transcription', {'transcript_length': len(transcription_bundle.filtered_transcript), 'transcription_confidence': transcription_bundle.transcription.data.get('confidence', 1.0), 'word_error_rate': self._calculate_wer(transcription_bundle.filtered_transcript), 'repetition_ratio': self._calculate_repetition_ratio(transcription_bundle.filtered_transcript), 'unique_word_ratio': self._calculate_vocabulary_diversity(transcription_bundle.filtered_transcript)}, routing_result)
        if should_exit:
            return await self._early_exit_processing(ctx, download_info, transcription_bundle, exit_reason, exit_confidence)
        quality_result, should_skip_analysis = await self._quality_filtering_phase(ctx, transcription_bundle.filtered_transcript, routing_result)
        if quality_result.success:
            qr_data = quality_result.data
            if 'result' in qr_data and isinstance(qr_data['result'], dict):
                qr_data = qr_data['result']
            should_exit, exit_reason, exit_confidence = await self._check_early_exit_condition(ctx, 'post_quality_filtering', {'overall_quality': qr_data.get('overall_score', 0.0), 'assessment_confidence': qr_data.get('confidence', 0.0), 'coherence_score': qr_data.get('quality_metrics', {}).get('coherence', 0.0), 'completeness_score': qr_data.get('quality_metrics', {}).get('completeness', 0.0), 'informativeness_score': qr_data.get('quality_metrics', {}).get('informativeness', 0.0)}, routing_result)
            if should_exit:
                return await self._early_exit_processing(ctx, download_info, transcription_bundle, exit_reason, exit_confidence, quality_result)
    ...
    async def _content_routing_phase(self, ctx: _PipelineContext, download_info: StepResult, transcription_bundle: _TranscriptionArtifacts) -> StepResult:
        """Route content based on type classification (Week 4 Phase 2)."""
        import os
        self._langfuse_start_span(ctx, 'content_routing', {'title': download_info.data.get('title'), 'video_id': download_info.data.get('video_id')}, metadata={'phase': 'content_routing'})
        ...
        try:
            if not routing_enabled:
                ...
                routing_input = {'transcript': transcription_bundle.filtered_transcript, 'title': download_info.data.get('title', ''), 'description': download_info.data.get('description', ''), 'metadata': download_info.data.get('metadata', {})}
            ...
            self.logger.warning('Content routing failed with exception, using default thresholds: %s', error_message)
            result = StepResult.ok(result={'routing_enabled': False, 'content_type': 'general', 'error': error_message})
        ...
        if isinstance(data, dict):
            ...
            routing_payload.update({'success': result.success, 'content_type': classification.get('primary_type', 'general'), 'confidence': classification.get('confidence'), 'pipeline': data.get('routing', {}).get('pipeline') if isinstance(data.get('routing'), dict) else None})
        ...
        if result.success:
            ...
            self.logger.info("Content routed as '%s' (confidence: %.2f, pipeline: %s)", content_type, confidence, pipeline)
    ...
    def _load_content_type_thresholds(self, routing_result: StepResult | None) -> dict[str, float]:
        ...
        from pathlib import Path
        import yaml
        default_thresholds = {'quality_threshold': float(os.getenv('QUALITY_MIN_OVERALL', '0.65')), 'coherence_threshold': float(os.getenv('QUALITY_MIN_COHERENCE', '0.60')), 'completeness_threshold': float(os.getenv('QUALITY_MIN_COMPLETENESS', '0.60')), 'informativeness_threshold': float(os.getenv('QUALITY_MIN_INFORMATIVENESS', '0.65'))}
        ...
        try:
            ...
            thresholds = {'quality_threshold': type_config.get('quality_threshold', default_thresholds['quality_threshold']), 'coherence_threshold': type_config.get('coherence_threshold', default_thresholds['coherence_threshold']), 'completeness_threshold': type_config.get('completeness_threshold', default_thresholds['completeness_threshold']), 'informativeness_threshold': type_config.get('informativeness_threshold', default_thresholds['informativeness_threshold'])}
            self.logger.info(f"Loaded thresholds for content type '{content_type}': quality={thresholds['quality_threshold']:.2f}")
            return thresholds
        except Exception as e:
            self.logger.warning(f'Failed to load content type config: {e}, using defaults')
            return default_thresholds

    async def _quality_filtering_phase(self, ctx: _PipelineContext, transcript: str, routing_result: StepResult | None=None) -> tuple[StepResult, bool]:
        """Assess transcript quality and determine processing path (now with content-type aware thresholds)."""
        import os
        self._langfuse_start_span(ctx, 'quality_filtering', {'transcript_length': len(transcript)}, metadata={'phase': 'quality_filtering'})
        quality_enabled = os.getenv('ENABLE_QUALITY_FILTERING', '1') == '1'
        if not quality_enabled:
            result = StepResult.ok(result={'should_process': True, 'bypass_reason': 'quality_filtering_disabled'})
            skip_analysis = False
        else:
            ...
            try:
                ...
                quality_input = {'transcript': transcript, 'thresholds': content_type_thresholds}
                result = quality_tool.run(quality_input)
                if not result.success:
                    self.logger.warning('Quality assessment failed, proceeding with full analysis: %s', result.error)
                    ...
                    if not should_process:
                        self.logger.info('Quality filtering bypass: %s (score: %.2f)', bypass_reason, quality_score)
                ...
                self.logger.warning('Quality filtering failed with exception, proceeding with full analysis: %s', str(exc))
        ...
        if result.success:
            ...
            if 'result' in data and isinstance(data['result'], dict):
                data = data['result']
            if isinstance(data, dict):
                qr_output.update({'should_process': data.get('should_process', True), 'overall_score': data.get('overall_score'), 'bypass_reason': data.get('bypass_reason')})
    ...
    def _load_early_exit_config(self) -> dict[str, Any]:
        ...
        import yaml
        default_config = {'enabled': True, 'global': {'min_exit_confidence': 0.8, 'continue_on_error': True}, 'checkpoints': {}}
        try:
    ...
    async def _check_early_exit_condition(self, ctx: _PipelineContext, checkpoint_name: str, checkpoint_data: dict[str, Any], routing_result: StepResult | None=None) -> tuple[bool, str, float]:
        """
        ...
        try:
            ...
            min_confidence = global_config.get('min_exit_confidence', 0.8)
            for condition in exit_conditions:
                ...
                try:
                    ...
                    if self._evaluate_condition(condition_expr, eval_context):
                        if checkpoint_name == 'post_transcription' and condition_name == 'very_short_transcript' and quality_filtering_enabled:
                            self.logger.debug("Early exit condition '%s' suppressed to allow quality filtering", condition_name)
                            continue
                        self.logger.info(f'Early exit triggered at {checkpoint_name}: {condition_name} (confidence: {condition_confidence:.2f})')
                        ...
                        try:
                            if hasattr(metrics, 'PIPELINE_EARLY_EXITS'):
                                metrics.PIPELINE_EARLY_EXITS.labels(**metrics.label_ctx(), checkpoint=checkpoint_name).inc()
                        except Exception:
                            pass
    ...
    def _evaluate_condition(self, condition_expr: str, context: dict[str, Any]) -> bool:
        ...
        """
        try:
            safe_namespace = {'__builtins__': {}, **context}
    ...
    async def _early_exit_processing(self, ctx: _PipelineContext, download_info: StepResult, transcription_bundle: _TranscriptionArtifacts | None, exit_reason: str, exit_confidence: float, quality_result: StepResult | None=None) -> PipelineRunResult:
        """
        ...
        if quality_result and quality_result.success:
            ...
            if 'result' in qr_data and isinstance(qr_data['result'], dict):
                qr_data = qr_data['result']
            quality_score = qr_data.get('overall_score', 0.0)
        memory_payload = {'source_url': source_url, 'title': title, 'summary': basic_summary, 'exit_reason': exit_reason, 'exit_confidence': exit_confidence, 'quality_score': quality_score, 'processing_type': 'early_exit', 'duration': duration, 'transcript_preview': transcript_preview, 'processed_at': time.time()}
        ...
        try:
            if hasattr(metrics, 'PIPELINE_PROCESSING_TIME'):
                metrics.PIPELINE_PROCESSING_TIME.labels(**metrics.label_ctx(), processing_type='early_exit').observe(processing_duration)
        except Exception:
            pass
        ...
        self.logger.info(f'Early exit processing complete: {exit_reason} (confidence: {exit_confidence:.2f}, duration: {processing_duration:.2f}s)')
        return self._finalize_pipeline(ctx.start_time, 'success', {'status': 'success', 'processing_type': 'early_exit', 'exit_reason': exit_reason, 'exit_confidence': exit_confidence, 'quality_score': quality_score, 'summary': basic_summary, 'title': title, 'memory_stored': memory_result.success if memory_result else False, 'processing_duration': processing_duration, 'time_saved_estimate': '75-90%'})

    async def _lightweight_processing_phase(self, ctx: _PipelineContext, download_info: StepResult, transcription_bundle: _TranscriptionArtifacts, quality_result: StepResult) -> PipelineRunResult:
        """Lightweight processing for low-quality content."""
        start_time = time.monotonic()
        span_input = {'video_id': download_info.data.get('video_id'), 'transcript_length': len(transcription_bundle.filtered_transcript), 'quality_result_success': quality_result.success}
        self._langfuse_start_span(ctx, 'lightweight_processing', span_input, metadata={'phase': 'lightweight_processing'})
        ...
        try:
            ...
            if isinstance(qm, dict):
                raw_metrics = {k: v for k, v in qm.items() if k in {'word_count', 'sentence_count', 'avg_sentence_length', 'coherence_score', 'topic_clarity_score', 'language_quality_score', 'overall_quality_score'}}
            title = download_info.data.get('title', '')
            source_url = download_info.data.get('source_url', '')
            duration = download_info.data.get('duration', 0)
            memory_payload = {'source_url': source_url, 'title': title, 'summary': basic_summary, 'quality_score': quality_score, 'processing_type': 'lightweight', 'bypass_reason': bypass_reason, 'duration': duration, 'quality_metrics': raw_metrics, 'transcript_preview': transcription_bundle.filtered_transcript[:200] + '...' if len(transcription_bundle.filtered_transcript) > 200 else transcription_bundle.filtered_transcript, 'processed_at': time.time()}
            ...
            try:
                if hasattr(metrics, 'PIPELINE_STEP_DURATION'):
                    metrics.PIPELINE_STEP_DURATION.labels(**metrics.label_ctx(), step='lightweight_processing').observe(processing_duration)
            except Exception:
                pass
            ...
            ctx.span.set_attribute('bypass_reason', bypass_reason)
            ctx.span.set_attribute('processing_duration_seconds', processing_duration)
            span_output.update({'status': 'success', 'quality_score': quality_score, 'bypass_reason': bypass_reason, 'memory_stored': bool(memory_result and memory_result.success), 'processing_duration': processing_duration})
            result = self._finalize_pipeline(ctx.start_time, 'success', {'status': 'success', 'processing_type': 'lightweight', 'quality_score': quality_score, 'summary': basic_summary, 'title': title, 'bypass_reason': bypass_reason, 'memory_stored': memory_result.success if memory_result else False, 'processing_duration': processing_duration, 'time_saved_estimate': '60-75%', 'quality_metrics': raw_metrics})
    ...
    async def _download_phase(self, ctx: _PipelineContext, url: str, quality: str) -> tuple[StepResult | None, PipelineRunResult | None]:
        self._langfuse_start_span(ctx, 'download', {'url': url, 'quality': quality}, metadata={'phase': 'download'})
        ...
        span_output = {'status': 'success', 'platform': download_info.data.get('platform'), 'video_id': download_info.data.get('video_id'), 'duration': download_info.data.get('duration')}
        self._langfuse_finish_span(ctx, 'download', span_output)
        return (download_info, None)

    async def _transcription_phase(self, ctx: _PipelineContext, download_info: StepResult) -> tuple[_TranscriptionArtifacts | None, PipelineRunResult | None]:
        local_path = download_info.data['local_path']
        video_id = download_info.data.get('video_id')
        self._langfuse_start_span(ctx, 'transcription', {'video_id': video_id, 'local_path': local_path}, metadata={'phase': 'transcription'})
        ...
        if not transcription.success:
            ...
            if fallback is not None:
                self.logger.warning('Transcription unavailable (%s); using %s-derived fallback', transcription.error or 'unknown error', fallback.data.get('transcript_source', 'metadata'))
        ...
        filtered_transcript = self._apply_pii_filtering(transcription.data.get('transcript', ''), 'transcript')
        transcript_task: asyncio.Task[StepResult] | None = None
        span_output = {'status': 'success', 'transcription_confidence': transcription.data.get('confidence'), 'drive_outcome': drive_artifacts.outcome, 'transcript_length': len(filtered_transcript)}
        self._langfuse_finish_span(ctx, 'transcription', span_output)
        return (_TranscriptionArtifacts(transcription=transcription, drive=drive_artifacts, filtered_transcript=filtered_transcript, transcript_task=transcript_task), None)

    async def _analysis_phase(self, ctx: _PipelineContext, download_info: StepResult, transcription_bundle: _TranscriptionArtifacts) -> tuple[_AnalysisArtifacts | None, PipelineRunResult | None]:
        ...
        analysis_span_output: dict[str, Any] = {'video_id': download_info.data.get('video_id'), 'platform': download_info.data.get('platform'), 'transcript_length': len(filtered_transcript)}
        self._langfuse_start_span(ctx, 'analysis', analysis_span_output, metadata={'phase': 'analysis'})
        try:
        ...
        if isinstance(analysis, Exception) or not analysis.success:
            ...
            error_payload = {'status': 'error', 'error': error_msg, 'step': 'analysis'}
            return (None, self._fail(ctx.span, ctx.start_time, 'analysis', error_payload))
        analysis_span_output.update({'analysis_keys': list(analysis.data.keys())[:10], 'analysis_success': True})
        ...
        perspective_task = asyncio.create_task(self._run_perspective(compressed_transcript, analysis.data), name='perspective')
        ...
        if not fallacy.success:
            ...
            return (None, self._fail(ctx.span, ctx.start_time, 'fallacy', {'status': 'error', 'error': fallacy_error, 'step': 'fallacy'}))
        try:
            ...
            self._langfuse_finish_span(ctx, 'analysis', analysis_span_output, error=error_msg)
            return (None, self._fail(ctx.span, ctx.start_time, 'perspective', {'status': 'error', 'error': error_msg, 'step': 'perspective'}))
        ...
        analysis_span_output.update({'status': 'success', 'summary_length': len(summary), 'sentiment': analysis.data.get('sentiment')})
        memory_payload = {'video_id': download_info.data['video_id'], 'title': download_info.data['title'], 'platform': download_info.data.get('platform', 'unknown'), 'sentiment': analysis.data.get('sentiment'), 'keywords': analysis.data.get('keywords'), 'summary': summary}
        if compression_meta:
            memory_payload['transcript_compression'] = compression_meta
        analysis_memory_task = asyncio.create_task(self._run_analysis_memory(summary, memory_payload), name='analysis_memory')
        if transcript_task is None:
            transcript_task = self._schedule_transcript_storage(filtered_transcript, download_info)
        ...
        if getattr(self, 'graph_memory', None) is not None and getattr(self, '_graph_memory_enabled', False):
            ...
            graph_task = asyncio.create_task(self._execute_step('graph_memory', self.graph_memory.run, text=summary, metadata=memory_payload, index='analysis', tags=sorted(tags)), name='graph_memory')
        ...
        if getattr(self, 'hipporag_memory', None) is not None and getattr(self, '_hipporag_memory_enabled', False):
            ...
            hipporag_task = asyncio.create_task(self._execute_step('hipporag_memory', self.hipporag_memory.run, text=summary, metadata=memory_payload, index='continual_memory', tags=hippo_tags, consolidate=True), name='hipporag_memory')
            hipporag_placeholder = StepResult.skip(data={'state': 'pending'})
        else:
            self._record_step_skip('hipporag_memory')
        discord_task, discord_placeholder = self._schedule_discord_post(transcription_bundle.drive, download_info, analysis, fallacy, perspective)
        analysis_span_output.update({'memory_payload_keys': list(memory_payload.keys())[:10], 'graph_memory_enabled': graph_task is not None, 'hipporag_memory_enabled': hipporag_task is not None})
        self._langfuse_finish_span(ctx, 'analysis', analysis_span_output)
        return (_AnalysisArtifacts(analysis=analysis, fallacy=fallacy, perspective=perspective, summary=summary, memory_payload=memory_payload, analysis_memory_task=analysis_memory_task, discord_task=discord_task, discord_placeholder=discord_placeholder, graph_task=graph_task, graph_placeholder=graph_placeholder, hipporag_task=hipporag_task, hipporag_placeholder=hipporag_placeholder, compression=compression_meta), None)

    async def _finalize_phase(self, ctx: _PipelineContext, download_info: StepResult, transcription_bundle: _TranscriptionArtifacts, analysis_bundle: _AnalysisArtifacts) -> PipelineRunResult:
        memory_result, graph_result, hipporag_result = await self._await_storage_tasks(analysis_bundle.analysis_memory_task, transcription_bundle.transcript_task, analysis_bundle.graph_task, analysis_bundle.hipporag_task)
        if isinstance(memory_result, Exception):
            if analysis_bundle.discord_task:
                analysis_bundle.discord_task.cancel()
            return self._fail(ctx.span, ctx.start_time, 'memory', {'status': 'error', 'step': 'memory', 'error': str(memory_result)})
        ...
        return self._finalize_pipeline(ctx.start_time, 'success', {'status': 'success', 'download': download_info.to_dict(), 'drive': transcription_bundle.drive.result.to_dict(), 'transcription': transcription_bundle.transcription.to_dict(), 'analysis': analysis_bundle.analysis.to_dict(), 'fallacy': analysis_bundle.fallacy.to_dict(), 'perspective': analysis_bundle.perspective.to_dict(), 'memory': memory_step.to_dict(), 'graph_memory': graph_step.to_dict() if graph_step is not None else {}, 'hipporag_memory': hipporag_step.to_dict() if hipporag_step is not None else {}, 'discord': discord_result.to_dict(), 'transcript_compression': analysis_bundle.compression}, duration=duration)
    ...
    def _rate_limit_failure(self) -> PipelineRunResult | None:
        ...
        return {'status': 'error', 'step': 'rate_limit', 'error': f'Pipeline rate limit exceeded for tenant {tenant_key}', 'rate_limit_exceeded': True, 'status_code': 429}

    def _start_drive_upload(self, download_info: StepResult, local_path: str) -> tuple[asyncio.Task[StepResult] | None, StepResult | None]:
        if not self.drive:
            self.logger.info('Drive upload skipped (disabled)')
            skip_result = StepResult.skip(data={'state': 'skipped', 'message': 'Google Drive disabled', 'reason': 'disabled'})
            self._record_step_skip('drive')
            return (None, skip_result)
        drive_platform = download_info.data.get('platform', 'unknown').lower()
        task = asyncio.create_task(self._execute_step('drive', self.drive.run, local_path, drive_platform), name='drive')
        return (task, None)

    async def _await_drive_result(self, drive_task: asyncio.Task[StepResult] | None, existing_info: StepResult | None, span, pipeline_start: float) -> PipelineRunResult | _DriveArtifacts:
    ...
    async def _attempt_transcription_fallback(self, download_info: StepResult, transcription: StepResult) -> StepResult | None:
        error_text = (transcription.error or '').lower()
        degrade_markers = ('whisper package is not installed', "no module named 'whisper'", 'no module named "whisper"', 'failed to load whisper')
        ...
        data.update({'transcript': transcript_text, 'segments': segments, 'cache_hit': False, 'model': 'metadata-fallback', 'video_id': download_info.data.get('video_id'), 'transcript_source': transcript_source, 'fallback_reason': transcription.error, 'tool': 'transcription-fallback'})
        return StepResult(success=True, data=data, custom_status='degraded')

    async def _fetch_provider_transcript(self, download_info: StepResult, source_url: str | None) -> tuple[str | None, str]:
        if not source_url:
            return (None, '')
        platform = str(download_info.data.get('platform', '')).lower()
        try:
            if 'youtube' in platform or 'youtu' in source_url:
                from domains.ingestion.providers.youtube import fetch_transcript
    ...
    def _compose_metadata_transcript(self, download_info: StepResult, metadata: dict[str, Any], source_url: str | None) -> str | None:
        lines: list[str] = []
        title = download_info.data.get('title') or metadata.get('title')
        if title:
            lines.append(f'Title: {title}')
        presenter = download_info.data.get('uploader') or download_info.data.get('channel') or metadata.get('uploader') or metadata.get('channel')
        if presenter:
    ...
    def _build_segments_from_text(self, transcript_text: str) -> list[dict[str, Any]]:
        ...
        for idx, line in enumerate(transcript_text.splitlines()):
            ...
            if not stripped:
                continue
            segments.append({'id': idx, 'start': float(idx), 'end': float(idx + 1), 'text': stripped})
        return segments

    def _schedule_transcript_storage(self, transcript: str, download_info: StepResult) -> asyncio.Task[StepResult] | None:
        ...
        if disable_transcript:
            ...
            return None
        return asyncio.create_task(self._execute_step('transcript_memory', self.memory.run, transcript, {'video_id': download_info.data['video_id'], 'title': download_info.data['title'], 'platform': download_info.data.get('platform', 'unknown')}, collection='transcripts'), name='transcript_storage')

    async def _await_transcript_best_effort(self, task: asyncio.Task[StepResult]) -> None:
        with suppress(Exception):
            await asyncio.wait_for(task, timeout=2)

    def _schedule_discord_post(self, drive_artifacts: _DriveArtifacts, download_info: StepResult, analysis: StepResult, fallacy: StepResult, perspective: StepResult) -> tuple[asyncio.Task[StepResult] | None, StepResult]:
        content_data = {**download_info.to_dict(), **analysis.to_dict(), **fallacy.data, **perspective.data}
        if not self.discord:
        ...
        if drive_artifacts.outcome == 'skipped':
            drive_links = {'status': 'skipped', 'reason': drive_artifacts.result.data.get('reason', 'quota'), 'message': drive_artifacts.result.data.get('message', 'Drive upload skipped')}
        else:
            drive_links = drive_artifacts.result.data.get('links', {}) if drive_artifacts.result.success else {}
        return (asyncio.create_task(self._execute_step('discord', self.discord.run, content_data, drive_links), name='discord'), StepResult.skip(data={'state': 'pending'}))

    async def _await_storage_tasks(self, analysis_memory_task: asyncio.Task[StepResult], transcript_task: asyncio.Task[StepResult] | None, graph_task: asyncio.Task[StepResult] | None, hipporag_task: asyncio.Task[StepResult] | None) -> tuple[StepResult | Exception, StepResult | Exception | None, StepResult | Exception | None]:
    ...
    def _fail(self, span, pipeline_start: float, step: str, payload: dict[str, Any]) -> PipelineRunResult:
    ...
    async def _run_transcription(self, local_path: str, video_id: str | None) -> StepResult:
        ...
        return await self._execute_step('transcription', self._transcription_step, local_path, video_id, model_name)
    ...
    async def _run_analysis_memory(self, summary: str, payload: dict[str, Any]) -> StepResult:
        return await self._execute_step('analysis_memory', self.memory.run, summary, payload, collection='analysis')
    ...
    def _calculate_spam_score(self, title: str) -> float:
        ...
        spam_indicators = ['buy now', 'click here', 'limited time', 'discount', 'sale', 'free', 'win', 'prize', 'cash', 'money', 'earn', 'profit', 'guaranteed', 'no risk', 'act now', 'hurry', 'expires', 'subscribe', 'follow', 'like', 'share', 'comment', 'link in bio', 'swipe up', 'tap here', 'download now']
