...
class ContentPipeline(PipelineExecutionMixin, PipelineBase):
    ...
    def _langfuse_start_span(
        self, ctx: _PipelineContext, name: str, input_data: dict[str, Any], metadata: dict[str, Any] | None = None
    ) -> None:
    ...
    def _langfuse_finish_span(
        self, ctx: _PipelineContext, name: str, output_data: dict[str, Any] | None = None, *, error: str | None = None
    ) -> None:
        service = getattr(ctx, "langfuse_service", None)
        if not service:
            return
    ...
    def _langfuse_finalize_trace(
        self, ctx: _PipelineContext | None, status: str, payload: dict[str, Any], duration: float
    ) -> None:
    ...
    def _finalize_pipeline(
        self, start_time: float, status: str, payload: dict[str, Any], *, duration: float | None = None
    ) -> dict[str, Any]:
    ...
    def _pipeline_context(
        self, url: str, quality: str, start_time: float, total_limit: float | None, per_task_limits: dict[str, float]
    ) -> Iterator[_PipelineContext]:
        with ExitStack() as stack:
            tracker = stack.enter_context(
                self._pipeline_pkg.track_request_budget(total_limit=total_limit, per_task_limits=per_task_limits)
            )
            ...
            try:
                ...
                if langfuse_service and langfuse_pipeline_span:
                    langfuse_service.update_span(
                        langfuse_pipeline_span, {"status": "exception", "exception": str(exc)}, error=str(exc)
                    )
                    ctx.langfuse_pipeline_span = None
    ...
    async def _content_routing_phase(
        ...
        self._langfuse_start_span(
            ctx,
            "content_routing",
            {"title": download_info.data.get("title"), "video_id": download_info.data.get("video_id")},
        ...
        if result.success:
            ...
            self.logger.info(
                "Content routed as '%s' (confidence: %.2f, pipeline: %s)", content_type, confidence, pipeline
            )
    ...
    async def _quality_filtering_phase(
        ...
        self._langfuse_start_span(
            ctx, "quality_filtering", {"transcript_length": len(transcript)}, metadata={"phase": "quality_filtering"}
        )
        ...
        if not quality_enabled:
            ...
            try:
                ...
                self.logger.warning(
                    "Quality filtering failed with exception, proceeding with full analysis: %s", str(exc)
                )
    ...
    def _load_early_exit_config(self) -> dict[str, Any]:
        ...
        default_config = {
            "enabled": True,
            "global": {"min_exit_confidence": 0.8, "continue_on_error": True},
            "checkpoints": {},
        }
        try:
                ...
                try:
                    ...
                    if self._evaluate_condition(condition_expr, eval_context):
                        if (
                            ...
                            continue
                        self.logger.info(
                            f"Early exit triggered at {checkpoint_name}: {condition_name} (confidence: {condition_confidence:.2f})"
                        )
    ...
    async def _early_exit_processing(
        ...
        self.logger.info(
            f"Early exit processing complete: {exit_reason} (confidence: {exit_confidence:.2f}, duration: {processing_duration:.2f}s)"
        )
    ...
    async def _lightweight_processing_phase(
        ...
        self._langfuse_start_span(
            ctx, "lightweight_processing", span_input, metadata={"phase": "lightweight_processing"}
        )
    ...
    async def _transcription_phase(
        self, ctx: _PipelineContext, download_info: StepResult
    ) -> tuple[_TranscriptionArtifacts | None, PipelineRunResult | None]:
        local_path = download_info.data["local_path"]
        video_id = download_info.data.get("video_id")
        self._langfuse_start_span(
            ctx, "transcription", {"video_id": video_id, "local_path": local_path}, metadata={"phase": "transcription"}
        )
    ...
    async def _analysis_phase(
        ...
        analysis_memory_task = asyncio.create_task(
            self._run_analysis_memory(summary, memory_payload), name="analysis_memory"
        )
        if transcript_task is None:
            transcript_task = self._schedule_transcript_storage(filtered_transcript, download_info)
        ...
        discord_task, discord_placeholder = self._schedule_discord_post(
            transcription_bundle.drive, download_info, analysis, fallacy, perspective
        )
        analysis_span_output.update(
            {
    ...
    async def _finalize_phase(
        ...
        if isinstance(memory_result, Exception):
            ...
            return self._fail(
                ctx.span, ctx.start_time, "memory", {"status": "error", "step": "memory", "error": str(memory_result)}
            )
    ...
    def _start_drive_upload(
        self, download_info: StepResult, local_path: str
        ...
        if not self.drive:
            ...
            skip_result = StepResult.skip(
                data={"state": "skipped", "message": "Google Drive disabled", "reason": "disabled"}
            )
            self._record_step_skip("drive")
            return (None, skip_result)
        drive_platform = download_info.data.get("platform", "unknown").lower()
        task = asyncio.create_task(
            self._execute_step("drive", self.drive.run, local_path, drive_platform), name="drive"
        )
        return (task, None)

    async def _await_drive_result(
        self, drive_task: asyncio.Task[StepResult] | None, existing_info: StepResult | None, span, pipeline_start: float
    ...
    async def _attempt_transcription_fallback(
        self, download_info: StepResult, transcription: StepResult
    ) -> StepResult | None:
    ...
    async def _fetch_provider_transcript(
        self, download_info: StepResult, source_url: str | None
    ) -> tuple[str | None, str]:
        if not source_url:
            return (None, "")
    ...
    def _compose_metadata_transcript(
        self, download_info: StepResult, metadata: dict[str, Any], source_url: str | None
    ) -> str | None:
        lines: list[str] = []
    ...
    def _schedule_transcript_storage(
        self, transcript: str, download_info: StepResult
    ...
    def _schedule_discord_post(
        ...
        return (
            asyncio.create_task(
                self._execute_step("discord", self.discord.run, content_data, drive_links), name="discord"
            ),
