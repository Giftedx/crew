...
class PipelineMetricsMixin:
    ...
    def _record_step_metrics(self, step: str, outcome: str, duration: float | None) -> None:
        ...
        try:
            name = {'success': 'pipeline_steps_completed', 'skipped': 'pipeline_steps_skipped'}.get(outcome, 'pipeline_steps_failed')
    ...
    def _finalize_pipeline(self, start_time: float, status: str, payload: PipelineRunResult, *, duration: float | None=None) -> PipelineRunResult:
        if duration is None:
        ...
        if isinstance(step_observability, dict) and step_observability:
            ...
            obs['log_patterns'] = {'summary': merge_log_pattern_summaries(step_observability), 'per_step': deepcopy(step_observability)}
        if isinstance(step_observability, dict):
            step_observability.clear()
        self._record_dashboard_metrics_background(payload, duration)
        return payload

    def _record_dashboard_metrics_background(self, payload: PipelineRunResult, duration: float) -> None:
        """
        ...
        try:
            ...
            if loop.is_running():
                ...
                task = loop.create_task(record_pipeline_metrics(processing_type=processing_type, content_type=content_type, quality_score=quality_score, processing_time=duration, exit_checkpoint=exit_checkpoint, exit_reason=exit_reason, exit_confidence=exit_confidence, time_saved_pct=time_saved_pct))
...
class PipelineExecutionMixin(PipelineMetricsMixin):
    ...
    async def _execute_step(self, step: str, func: Callable[..., Any], *args: Any, retries: int | None=None, retry_delay: float=2.0, check_tool_rate_limit: bool=True, **kwargs: Any) -> StepResult:
        ...
        try:
            ...
            result = await self._run_with_retries(func, *args, step=step, attempts=attempts, delay=retry_delay, check_tool_rate_limit=check_tool_rate_limit, **kwargs)
    ...
    async def _dispatch_before_step(self, context: StepContext, middlewares: Iterable[PipelineStepMiddleware]) -> None:
        await self._run_middlewares(middlewares, 'before_step', context)

    async def _dispatch_after_step(self, context: StepContext, middlewares: Iterable[PipelineStepMiddleware]) -> None:
        await self._run_middlewares(middlewares, 'after_step', context)

    async def _dispatch_on_error(self, context: StepContext, middlewares: Iterable[PipelineStepMiddleware]) -> None:
        await self._run_middlewares(middlewares, 'on_error', context)

    async def _run_middlewares(self, middlewares: Iterable[PipelineStepMiddleware], hook_name: str, context: StepContext) -> None:
        for middleware in middlewares:
            ...
            try:
                ...
                logger = getattr(self, 'logger', None)
                if logger is not None:
                    logger.warning('Middleware %s.%s failed: %s', middleware.__class__.__name__, hook_name, exc)

    def _capture_step_observability(self, step: str, context: StepContext, summary: dict[str, Any] | None=None) -> None:
        if summary is None:
