...
class PipelineMetricsMixin:
    ...
    def _record_step_metrics(self, step: str, outcome: str, duration: float | None) -> None:
        ...
        try:
            name = {"success": "pipeline_steps_completed", "skipped": "pipeline_steps_skipped"}.get(
                outcome, "pipeline_steps_failed"
            )
    ...
    def _finalize_pipeline(
        self, start_time: float, status: str, payload: PipelineRunResult, *, duration: float | None = None
    ) -> PipelineRunResult:
        if duration is None:
...
class PipelineExecutionMixin(PipelineMetricsMixin):
    ...
    async def _run_middlewares(
        self, middlewares: Iterable[PipelineStepMiddleware], hook_name: str, context: StepContext
    ) -> None:
    ...
    def _capture_step_observability(
        self, step: str, context: StepContext, summary: dict[str, Any] | None = None
    ) -> None:
        if summary is None:
