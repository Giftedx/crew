...
class PipelineStepMiddleware(Protocol):
    ...
    async def before_step(self, context: StepContext) -> None: ...  # pragma: no cover - interface only

    async def after_step(self, context: StepContext) -> None: ...  # pragma: no cover - interface only

    async def on_error(self, context: StepContext) -> None: ...  # pragma: no cover - interface only


class BasePipelineStepMiddleware:
    """Convenience base with optional async hooks."""

    async def before_step(self, context: StepContext) -> None:  # pragma: no cover - default no-op
        return None

    async def after_step(self, context: StepContext) -> None:  # pragma: no cover - default no-op
        return None

    async def on_error(self, context: StepContext) -> None:  # pragma: no cover - default no-op
        return None
