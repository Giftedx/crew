...
class EnhancedPerformanceMonitor:
    ...
    def __init__(self, base_monitor: AgentPerformanceMonitor=None):
        ...
        self.performance_thresholds = {'critical_accuracy_drop': 0.15, 'response_time_spike': 2.0, 'error_rate_spike': 0.2, 'tool_failure_rate': 0.3}
    ...
    def _assess_content_quality(self, response: str, context: dict[str, Any]) -> float:
        ...
        if target_length[0] <= response_length <= target_length[1]:
            score += 0.3
        elif response_length > target_length[1] * 0.8:
            score += 0.2
    ...
    def _assess_reasoning_quality(self, response: str) -> float:
        ...
        score = 0.0
        connectors = ['because', 'therefore', 'however', 'moreover', 'furthermore', 'consequently', 'as a result', 'due to', 'given that']
    ...
    def _assess_user_experience(self, response: str, context: dict[str, Any]) -> float:
        ...
        if '\n\n' in response or response.count('\n') >= 2:
            score += 0.3
        if context.get('channel_type') == 'professional' and (not any((phrase in response.lower() for phrase in ['lol', 'omg', 'wtf']))):
            score += 0.2
        action_phrases = ['you can', 'try', 'consider', 'recommend', 'suggest']
        if any((phrase in response.lower() for phrase in action_phrases)):
            score += 0.3
        if not any((phrase in response.lower() for phrase in ['error', 'failed', 'unable to', "couldn't", "can't process"])):
            score += 0.2
        return min(1.0, score)
    ...
    async def monitor_real_time_performance(self, agent_name: str, interaction_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if agent_name not in self.real_time_metrics:
            self.real_time_metrics[agent_name] = {'recent_interactions': [], 'current_session_stats': {'total_requests': 0, 'avg_quality': 0.0, 'avg_response_time': 0.0, 'error_count': 0, 'session_start': default_utc_now().isoformat()}}
        ...
        recent = agent_metrics['recent_interactions'][-10:]
        if recent:
        ...
        return {'agent_name': agent_name, 'current_performance': session_stats, 'alerts': alerts, 'recent_trend': self._calculate_recent_trend(recent)}

    async def record_interaction_async(self, agent_name: str, interaction_type: str, quality_score: float, response_time: float, context: dict[str, Any] | None=None, tools_used: list[str] | None=None, error_occurred: bool=False) -> dict[str, Any]:
        """Asynchronously persist interaction details and refresh monitoring state."""
        context = context or {}
        interaction_payload = {'task_type': interaction_type, 'tools_used': tools_used or context.get('tools_used', []), 'tool_sequence': context.get('tool_sequence', []), 'response_quality': quality_score, 'response_time': response_time, 'user_feedback': context.get('user_feedback', {}), 'error_occurred': error_occurred or context.get('error_occurred', False), 'error_details': context.get('error_details', {})}
        ...
        try:
            self.base_monitor.record_agent_interaction(agent_name=agent_name, task_type=interaction_type, tools_used=interaction_payload.get('tools_used', []), tool_sequence=interaction_payload.get('tool_sequence', []), response_quality=quality_score, response_time=response_time, user_feedback=interaction_payload.get('user_feedback'), error_occurred=interaction_payload.get('error_occurred', False), error_details=interaction_payload.get('error_details'))
    ...
    async def _check_performance_alerts(self, agent_name: str, recent_interactions: list[dict]) -> list[dict[str, Any]]:
        ...
        if recent_quality and earlier_quality:
            ...
            if earlier_avg - recent_avg > self.performance_thresholds['critical_accuracy_drop']:
                alerts.append({'type': 'quality_degradation', 'severity': 'high', 'message': f'Agent {agent_name} quality dropped by {(earlier_avg - recent_avg) * 100:.1f}%', 'current_avg': recent_avg, 'previous_avg': earlier_avg, 'threshold': self.performance_thresholds['critical_accuracy_drop']})
        ...
        if recent_times and earlier_times:
            ...
            if recent_avg_time > earlier_avg_time * self.performance_thresholds['response_time_spike']:
                alerts.append({'type': 'response_time_spike', 'severity': 'medium', 'message': f'Agent {agent_name} response time increased significantly', 'current_avg': recent_avg_time, 'previous_avg': earlier_avg_time, 'spike_factor': recent_avg_time / earlier_avg_time if earlier_avg_time > 0 else 'undefined'})
        ...
        if error_rate > self.performance_thresholds['error_rate_spike']:
            alerts.append({'type': 'error_rate_spike', 'severity': 'high', 'message': f'Agent {agent_name} error rate: {error_rate * 100:.1f}%', 'error_rate': error_rate, 'threshold': self.performance_thresholds['error_rate_spike'], 'recent_errors': recent_errors})
        return alerts
    ...
    def generate_comparative_analysis(self, agent_names: list[str], days: int=7) -> dict[str, Any]:
        ...
        comparative_analysis: dict[str, Any] = {'reporting_period': {'days_analyzed': days, 'end_date': default_utc_now().isoformat()}, 'agent_rankings': {}, 'performance_summary': {}, 'recommendations': {'top_performers': [], 'needs_attention': [], 'improvement_opportunities': []}}
        ...
        for rank, (agent_name, report) in enumerate(agents_by_score, 1):
            comparative_analysis['agent_rankings'][agent_name] = {'rank': rank, 'overall_score': report.overall_score, 'score_percentile': 100 * (len(agents_by_score) - rank) / len(agents_by_score)}
        all_scores = [report.overall_score for report in agent_reports.values()]
        comparative_analysis['performance_summary'] = {'average_score': sum(all_scores) / len(all_scores), 'best_score': max(all_scores), 'worst_score': min(all_scores), 'score_variance': self._calculate_variance(all_scores), 'total_agents_analyzed': len(agent_reports)}
        ...
        comparative_analysis['recommendations']['top_performers'] = [f'{agent}: {agent_reports[agent].overall_score:.2f} score' for agent in top_performers]
        comparative_analysis['recommendations']['needs_attention'] = [f'{agent}: {agent_reports[agent].overall_score:.2f} score - {len(agent_reports[agent].recommendations)} issues identified' for agent in needs_attention]
        all_recommendations = []
    ...
    def _find_common_patterns(self, recommendations: list[str]) -> list[str]:
        ...
        patterns = {'tool efficiency': ['tool', 'efficiency', 'selection'], 'response time': ['response time', 'timeout', 'optimization'], 'accuracy': ['accuracy', 'quality', 'verification'], 'error handling': ['error', 'failure', 'exception']}
        pattern_counts = {}
    ...
    async def generate_real_time_dashboard_data(self) -> dict[str, Any]:
        ...
        dashboard_data: dict[str, Any] = {'timestamp': default_utc_now().isoformat(), 'agents': {}, 'system_overview': {'total_agents_monitored': len(self.real_time_metrics), 'alerts_count': 0, 'avg_system_performance': 0.0}}
        ...
        for agent_name, metrics in self.real_time_metrics.items():
            ...
            agent_dashboard_data = {'current_performance': session_stats, 'recent_trend': self._calculate_recent_trend(recent_interactions), 'active_alerts': alerts, 'interaction_count_last_hour': len([i for i in recent_interactions if datetime.fromisoformat(i['timestamp']) > default_utc_now() - timedelta(hours=1)]), 'performance_status': self._get_performance_status(session_stats, alerts)}
        ...
        if dashboard_data['agents']:
            dashboard_data['system_overview']['avg_system_performance'] = total_performance / len(dashboard_data['agents'])
    ...
    def _get_performance_status(self, session_stats: dict, alerts: list[dict]) -> str:
        ...
        if any((alert['severity'] == 'high' for alert in alerts)):
            ...
            return 'excellent'
        elif session_stats.get('avg_quality', 0) > 0.7:
            return 'good'
        else:
            return 'needs_attention'

    def save_enhanced_report(self, agent_name: str, comparative_data: dict[str, Any] | None=None, dashboard_data: dict[str, Any] | None=None) -> Path:
        ...
        enhanced_report = {'base_performance_report': {'agent_name': base_report.agent_name, 'reporting_period': base_report.reporting_period, 'overall_score': base_report.overall_score, 'metrics': [{'metric_name': m.metric_name, 'target_value': m.target_value, 'actual_value': m.actual_value, 'trend': m.trend, 'confidence': m.confidence, 'last_updated': m.last_updated} for m in base_report.metrics], 'tool_usage': [{'tool_name': p.tool_name, 'usage_frequency': p.usage_frequency, 'success_rate': p.success_rate, 'average_quality_score': p.average_quality_score, 'common_sequences': p.common_sequences, 'error_patterns': p.error_patterns} for p in base_report.tool_usage], 'quality_trends': base_report.quality_trends, 'recommendations': base_report.recommendations, 'training_suggestions': base_report.training_suggestions}, 'enhanced_analytics': {'real_time_performance': self.real_time_metrics.get(agent_name, {}), 'comparative_analysis': comparative_data, 'dashboard_snapshot': dashboard_data, 'enhancement_timestamp': default_utc_now().isoformat()}}
...
async def main():
    ...
    interaction_data = {'task_type': 'fact_verification', 'tools_used': ['claim_extractor_tool', 'fact_check_tool'], 'response_quality': 0.87, 'response_time': 8.5, 'error_occurred': False}
