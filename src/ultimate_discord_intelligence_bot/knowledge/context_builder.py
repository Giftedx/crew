...
class UnifiedContextBuilder:
    ...
    async def _build_primary_context(
        ...
        try:
            ...
            retrieval_query = RetrievalQuery(
                text=request.query, intent=request.task_type, context=request.current_context, limit=5
            )
            ...
            return ContextSegment(
                ...
                metadata={"query": request.query, "task_type": request.task_type, "results_count": len(results)},
            )
        except Exception as e:
        ...
        try:
            ...
            if agent_history:
                ...
                for i, context in enumerate(recent_contexts):
                    segment = ContextSegment(
                        ...
                        metadata={"task_type": context.task_type, "agent_id": request.agent_id},
                    )
            ...
            if request.query:
                ...
                historical_query = RetrievalQuery(
                    text=f"historical context {request.query}", intent="historical_reference", limit=3
                )
                    ...
                    for ranked_result in results:
                        if ranked_result.metadata.get("timestamp"):
                            segment = ContextSegment(
                                ...
                                relevance_score=ranked_result.confidence * 0.8,
                                segment_type="historical",
                ...
                if result.success:
                    ...
                    for ranked_result in results:
                        segment = ContextSegment(
                            content=ranked_result.content,
                            source=ranked_result.source,
                            relevance_score=ranked_result.confidence * 0.6,
        ...
        try:
            ...
            if request.query:
                ...
                fact_query = RetrievalQuery(
                    text=request.query, intent="fact_checking", filters={"content_type": "fact_check"}, limit=2
                )
    ...
    async def _build_unified_context(
        self, request: ContextRequest, segments: list[ContextSegment], tenant_id: str, workspace_id: str
    ) -> UnifiedContext:
