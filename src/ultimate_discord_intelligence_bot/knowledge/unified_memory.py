...
class UnifiedMemoryService:
    ...
    async def store(self, content: str, metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None, namespace: str | None=None) -> StepResult:
        ...
        try:
            ...
            metadata.update({'tenant_id': tenant_id, 'workspace_id': workspace_id, 'namespace': namespace, 'timestamp': asyncio.get_event_loop().time()})
            store_tasks = []
            backend_results = {}
            ...
            if successful_backends:
                return StepResult.ok(data={'stored_backends': successful_backends, 'total_backends': len(self._backends), 'namespace': namespace, 'metadata': metadata})
            else:
    ...
    async def retrieve(self, query: str, tenant_id: str | None=None, workspace_id: str | None=None, filters: dict[str, Any] | None=None, limit: int=10) -> StepResult:
                    ...
                    if result and hasattr(result, 'success') and result.success:
                        ...
                        for item in backend_results:
                            if isinstance(item, dict):
                                all_results.append(MemoryResult(content=item.get('content', ''), metadata=item.get('metadata', {}), source=backend_name, confidence=item.get('confidence', 1.0), timestamp=item.get('timestamp'), tenant_id=tenant_id, workspace_id=workspace_id))
            if self.config.deduplication_enabled:
                all_results = self._deduplicate_results(all_results)
            all_results = sorted(all_results, key=lambda x: x.confidence, reverse=True)
            final_results = all_results[:limit]
            return StepResult.ok(data={'results': final_results, 'total_found': len(all_results), 'returned': len(final_results), 'backends_queried': len(retrieve_tasks), 'namespace': namespace})
        except Exception as e:
    ...
    async def _store_vector(self, content: str, metadata: dict[str, Any], namespace: str) -> bool:
        ...
        try:
            if not memory_store:
                return False
            result = await memory_store(store=self._backends.get('sqlite'), vstore=self._backends['vector'], tenant=metadata.get('tenant_id', 'default'), workspace=metadata.get('workspace_id', 'main'), text=content, item_type='long', policy='default')
    ...
    async def _retrieve_vector(self, query: str, namespace: str, filters: dict[str, Any], limit: int) -> StepResult:
        ...
        try:
            ...
            result = await memory_retrieve(store=self._backends.get('sqlite'), vstore=self._backends['vector'], tenant=namespace.split(':')[0] if ':' in namespace else 'default', workspace=namespace.split(':')[1] if ':' in namespace else 'main', query=query)
        ...
        try:
            ...
            for memory in store.memories:
                if query.lower() in memory.get('text', '').lower():
                    results.append({'content': memory.get('text', ''), 'metadata': memory.get('metadata', {}), 'confidence': 0.8})
    ...
    async def _retrieve_semantic(self, query: str, namespace: str, filters: dict[str, Any], limit: int) -> StepResult:
        ...
        try:
            ...
            if hasattr(cache, 'get'):
                ...
                if result:
                    return StepResult.ok(data=[{'content': result.get('response', ''), 'metadata': result.get('metadata', {}), 'confidence': result.get('similarity', 0.9)}])
    ...
    def get_backend_status(self) -> dict[str, bool]:
        ...
        return {'initialized': self._initialized, 'backends': dict.fromkeys(self._backends.keys(), True), 'config': {'vector_store': self.config.enable_vector_store, 'sqlite_store': self.config.enable_sqlite_store, 'semantic_cache': self.config.enable_semantic_cache, 'mem0': self.config.enable_mem0}}