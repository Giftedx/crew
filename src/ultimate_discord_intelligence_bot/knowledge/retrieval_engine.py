...
class UnifiedRetrievalEngine:
    ...
    async def retrieve(self, query: RetrievalQuery, unified_memory_service, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            memory_result = await unified_memory_service.retrieve(query=query.text, tenant_id=tenant_id, workspace_id=workspace_id, filters=query.filters, limit=query.limit * 3)
            ...
            for _i, result in enumerate(raw_results):
                ranked_result = RankedResult(content=result.content, metadata=result.metadata, source=result.source, confidence=result.confidence, tenant_id=tenant_id, workspace_id=workspace_id)
            ...
            response_data = {'results': final_results, 'query': {'text': query.text, 'intent': query.intent, 'filters': query.filters}, 'metadata': {'total_sources': len({r.source for r in final_results}), 'avg_confidence': sum((r.confidence for r in final_results)) / len(final_results) if final_results else 0, 'avg_relevance': sum((r.relevance_score for r in final_results)) / len(final_results) if final_results else 0, 'tenant_id': tenant_id, 'workspace_id': workspace_id}}
    ...
    async def _rank_and_fuse_results(self, results: list[RankedResult], query: RetrievalQuery, tenant_id: str, workspace_id: str) -> list[RankedResult]:
            ...
            for key, value in query_context.items():
                if key in result_metadata:
                    ...
                    if result_metadata[key] == value:
                        overlap_score += 1.0
                    elif isinstance(value, str) and isinstance(result_metadata[key], str) and (value.lower() in result_metadata[key].lower()):
                        overlap_score += 0.5
