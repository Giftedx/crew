...
class EnhancedAutonomousOrchestrator:
    ...
    def _validate_system_health(self) -> None:
        ...
        tool_checks = [('yt-dlp', 'YouTube download capability'), ('openai', 'OpenAI API for transcription/analysis'), ('discord webhook', 'Discord posting capability')]
        ...
        self.system_health = {'healthy': len(capabilities) >= 2, 'errors': errors, 'available_capabilities': capabilities}
    ...
    async def _execute_crewai_workflow(self, interaction: Any, url: str, depth: str, tenant_ctx: Any, workflow_id: str) -> None:
        ...
        try:
            ...
            inputs = {'url': url, 'depth': depth, 'workflow_id': workflow_id, 'quality': '1080p' if depth in ('comprehensive', 'experimental') else '720p', 'tenant_id': tenant_ctx.tenant_id if tenant_ctx else 'default', 'workspace_id': tenant_ctx.workspace_id if tenant_ctx else 'default'}
            ...
            await self._send_progress_update(interaction, '⏰ Multi-agent workflow timed out, switching to fallback...', 5, 10)
            await self._execute_pipeline_workflow(interaction, url, depth, tenant_ctx, workflow_id)
        except Exception as e:
            logger.warning(f'CrewAI workflow {workflow_id} failed: {e}, falling back to pipeline')
            await self._send_progress_update(interaction, '⚠️ Multi-agent workflow failed, switching to fallback...', 5, 10)
            await self._execute_pipeline_workflow(interaction, url, depth, tenant_ctx, workflow_id)

    async def _execute_pipeline_workflow(self, interaction: Any, url: str, depth: str, tenant_ctx: Any, workflow_id: str) -> None:
    ...
    async def _format_and_send_crewai_results(self, interaction: Any, result: Any, url: str, depth: str) -> None:
        ...
        try:
            await interaction.followup.send(f'🤖 **Multi-Agent Analysis Complete**\n**URL:** {url}\n**Depth:** {depth}\n**Method:** CrewAI Multi-Agent Workflow\n\n**Result:** {str(result)[:500]}...')
        except Exception as e:
    ...
    async def _format_and_send_pipeline_results(self, interaction: Any, result: Any, url: str, depth: str, workflow_id: str) -> None:
        ...
        try:
            ...
            if isinstance(result, StepResult):
                if result.success:
                    ...
                    response = f'✅ **Content Analysis Complete**\n\n**📺 Content Info:**\n• Title: {title[:100]}{('...' if len(title) > 100 else '')}\n• Uploader: {uploader}\n• Platform: {platform}\n\n**⚙️ Processing Info:**\n• Method: Direct Pipeline (Reliable)\n• Depth: {depth}\n• Workflow ID: {workflow_id}\n\n**📊 Processing Stages:**\n'
                    ...
                    await interaction.followup.send(f'❌ **Pipeline Processing Failed**\n**Error:** {result.error}\n**URL:** {url}\n**Workflow ID:** {workflow_id}')
            else:
    ...
    async def _send_system_unavailable_response(self, interaction: Any) -> None:
        ...
        error_msg += f'\n**Available capabilities:**\n• {(', '.join(self.system_health['available_capabilities']) if self.system_health['available_capabilities'] else 'None')}\n\nPlease check the system configuration and try again.'
    ...
    async def _handle_workflow_failure(self, interaction: Any, error: str, workflow_id: str) -> None:
        ...
        await interaction.followup.send(f'❌ **Autonomous Workflow Failed**\n\n**Error:** {error}\n**Workflow ID:** {workflow_id}\n\n**System Status:**\n• Healthy: {self.system_health['healthy']}\n• Available capabilities: {len(self.system_health['available_capabilities'])}\n• Known issues: {len(self.system_health['errors'])}\n\nThis may be due to missing dependencies or configuration issues. Please check the system logs for more details.')