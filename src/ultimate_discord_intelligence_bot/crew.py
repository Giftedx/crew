...
# Import MCP tools
from .tools.mcp_tool_wrapper import (
    CodeReviewTool,
    DataAnalysisTool,
    ImageAnalysisTool,
    MCPToolWrapper,
...
class UltimateDiscordIntelligenceBotCrew:
    ...
    def _get_mcp_tools(self):
        ...
        return [
            MCPToolWrapper(self.mcp_client),
            WebSearchTool(self.mcp_client),
            ImageAnalysisTool(self.mcp_client),
            DataAnalysisTool(self.mcp_client),
            CodeReviewTool(self.mcp_client),
        ]

    def setup_discord_integration(self, bot=None, webhook_url: str | None = None):
        """Setup Discord integration for artifact publishing.

        Args:
            bot: Discord bot instance (optional)
            webhook_url: Discord webhook URL for publishing
        """
        if webhook_url:
    ...
    async def publish_crew_results(self, crew_result: dict[str, Any], tenant: str = "", workspace: str = ""):
        """Publish crew execution results as artifacts.
        ...
        if not self.artifact_handler:
            print("No Discord integration configured, skipping artifact publishing")
            return

        try:
            # Extract and publish artifacts from crew result
            artifacts = self._extract_artifacts_from_crew_result(crew_result)

            if not artifacts:
                print("No artifacts found in crew result")
                return

            # Publish artifacts
            for artifact_data, metadata in artifacts:
                result = await self.artifact_publisher.publish_artifact(
                ...
                if result.success:
                    print(f"Published artifact: {metadata.artifact_id}")
                else:
                    print(f"Failed to publish artifact {metadata.artifact_id}: {result.error}")

        except Exception as e:
            print(f"Error publishing crew results: {e}")

    def _extract_artifacts_from_crew_result(self, crew_result: dict[str, Any]):
        ...
        from .services.artifact_publisher import ArtifactMetadata
        ...
        try:
            ...
            print(f"Error extracting artifacts from crew result: {e}")
    ...
    def mission_orchestrator(self) -> Agent:  # type: ignore[misc]
        """Mission Orchestrator Agent."""
        agent_class = get_agent("mission_orchestrator")
        if agent_class:
            agent_instance = agent_class()
            # Add MCP tools to the agent
            for tool in self._get_mcp_tools():
                agent_instance.add_tool(tool)
            return agent_instance.create()
        else:
    ...
    @agent  # type: ignore[misc]
    def acquisition_specialist(self) -> Agent:  # type: ignore[misc]
