        ...
        try:
            with with_tenant(tenant_ctx) if tenant_ctx is not None else with_tenant(None):
                ...
                result = await _maybe_evaluate_and_annotate(
                    result, task_name="url_analysis", inputs=inputs, tenant_ctx=tenant_ctx, pipeline_path="url"
                )
                return result
    ...
    try:
        with with_tenant(tenant_ctx) if tenant_ctx is not None else with_tenant(None):
            ...
            result = await pipeline.process_content(
                content=content_text, content_type=content_type, tenant=tenant, workspace=workspace
            )
...
async def _maybe_evaluate_and_annotate(
    ...
    try:
        ...
        eval_res = await evaluator.evaluate_output(
            task_name=task_name, input_data=inputs, output_data=output_data, context={}
        )
