...
def recent_interactions(history: dict[str, list[dict[str, Any]]], agent_name: str, days: int) -> list[dict[str, Any]]:
    ...
    return [interaction for interaction in history.get(agent_name, []) if ensure_utc(datetime.fromisoformat(interaction['timestamp'])) > cutoff_date]
...
def analyze_tool_usage(interactions: list[dict[str, Any]]) -> list[ToolUsagePattern]:
    tool_stats: dict[str, dict[str, Any]] = defaultdict(lambda: {'usage_count': 0, 'success_count': 0, 'quality_scores': [], 'sequences': [], 'errors': []})
    for interaction in interactions:
        tools_used = interaction.get('tools_used', [])
        quality = interaction.get('response_quality', 0.0)
    ...
    for tool_name, stats in tool_stats.items():
        if stats['usage_count']:
            ...
            patterns.append(ToolUsagePattern(tool_name=tool_name, usage_frequency=stats['usage_count'], success_rate=success_rate, average_quality_score=avg_quality, common_sequences=common_sequences, error_patterns=list(set(stats['errors']))))
    return sorted(patterns, key=lambda p: p.usage_frequency, reverse=True)

def calculate_performance_metrics_for_interactions(interactions: list[dict[str, Any]], thresholds: dict[str, float]) -> list[PerformanceMetric]:
    if not interactions:
        return []
    quality_scores = [i.get('response_quality', 0.0) for i in interactions]
    avg_quality = sum(quality_scores) / len(quality_scores)
    metrics: list[PerformanceMetric] = []
    metrics.append(PerformanceMetric(metric_name='accuracy_target', target_value=thresholds.get('accuracy_target', 0.9), actual_value=avg_quality, trend='stable', confidence=0.9 if len(quality_scores) > 10 else 0.7, last_updated=default_utc_now().isoformat()))
    response_times = [i.get('response_time', 0.0) for i in interactions]
    avg_response_time = sum(response_times) / len(response_times)
    metrics.append(PerformanceMetric(metric_name='response_time', target_value=thresholds.get('response_time', 30.0), actual_value=avg_response_time, trend='stable', confidence=0.9, last_updated=default_utc_now().isoformat()))
    return metrics
__all__ = ['analyze_tool_usage', 'calculate_performance_metrics_for_interactions', 'calculate_tool_efficiency', 'calculate_trend', 'recent_interactions']