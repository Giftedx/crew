...
class DashboardIntegrationService:
    ...
    async def register_prometheus_metric(self, metric_name: str, metric_type: str, help_text: str, labels: list[str] | None=None, buckets: list[float] | None=None, quantiles: list[float] | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        """Register a metric with Prometheus"""
        try:
            if not self._initialized:
                return StepResult.fail('Dashboard Integration Service not initialized')
            prometheus_metric = PrometheusMetrics(metric_name=metric_name, metric_type=metric_type, help_text=help_text, labels=labels or [], buckets=buckets, quantiles=quantiles, metadata=metadata or {})
            self._prometheus_metrics[metric_name] = prometheus_metric
            logger.info(f'Prometheus metric registered: {metric_name} ({metric_type})')
            return StepResult.ok(data={'registered': True, 'metric_name': metric_name, 'metric_type': metric_type, 'labels': labels or [], 'help_text': help_text})
        except Exception as e:
            logger.error(f'Error registering Prometheus metric: {e}')
            return StepResult.fail(f'Prometheus metric registration failed: {e!s}')

    async def create_grafana_dashboard(self, dashboard_id: str, title: str, description: str, dashboard_type: DashboardType, widgets: list[DashboardWidget], tags: list[str] | None=None, time_range: str | None=None, refresh_interval: str | None=None, metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            dashboard = GrafanaDashboard(dashboard_id=dashboard_id, title=title, description=description, dashboard_type=dashboard_type, widgets=widgets, tags=tags or [], time_range=time_range or self.config.default_time_range, refresh_interval=refresh_interval or f'{self.config.auto_refresh_interval}s', metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, 'created_at': datetime.now(timezone.utc).isoformat(), **(metadata or {})})
            self._grafana_dashboards[dashboard_id] = dashboard
            logger.info(f'Grafana dashboard created: {dashboard_id} - {title}')
            return StepResult.ok(data={'created': True, 'dashboard_id': dashboard_id, 'title': title, 'dashboard_type': dashboard_type.value, 'widgets_count': len(widgets), 'tags': tags or []})
        except Exception as e:
            logger.error(f'Error creating Grafana dashboard: {e}')
            return StepResult.fail(f'Grafana dashboard creation failed: {e!s}')

    async def execute_metrics_query(self, query: str, range_time: str='1h', step: str='15s', timeout: int | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'query': query, 'range': range_time, 'step': step, 'result': query_result, 'cached': False, 'timestamp': datetime.now(timezone.utc).isoformat()})
        except Exception as e:
            logger.error(f'Error executing metrics query: {e}')
            return StepResult.fail(f'Metrics query execution failed: {e!s}')

    async def get_dashboard_data(self, dashboard_id: str, time_range: str | None=None, refresh_cache: bool=False, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            dashboard_data = {'dashboard_id': dashboard_id, 'title': dashboard.title, 'description': dashboard.description, 'dashboard_type': dashboard.dashboard_type.value, 'time_range': time_range, 'widgets': [], 'metadata': dashboard.metadata}
    ...
    async def export_dashboard_config(self, dashboard_id: str, format_type: str='json', tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'dashboard_id': dashboard_id, 'format': format_type, 'exported_at': datetime.now(timezone.utc).isoformat(), 'config': exported_config})
        except Exception as e:
            logger.error(f'Error exporting dashboard config: {e}')
            return StepResult.fail(f'Dashboard config export failed: {e!s}')

    async def _execute_prometheus_query(self, query: str, range_time: str, step: str, timeout: int | None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'success', 'data': {'resultType': 'matrix', 'result': [{'metric': {'__name__': query}, 'values': data_points}]}}
        except Exception as e:
    ...
    async def _get_widget_data(self, widget: DashboardWidget, time_range: str) -> dict[str, Any]:
        ...
        try:
            widget_data = {'widget_id': widget.widget_id, 'title': widget.title, 'widget_type': widget.widget_type.value, 'position': widget.position, 'queries': [], 'options': widget.options, 'thresholds': widget.thresholds}
            for query in widget.queries:
                query_result = await self._execute_prometheus_query(query.query, query.range or time_range, query.step, None)
                widget_data['queries'].append({'ref_id': query.ref_id, 'legend': query.legend, 'query': query.query, 'result': query_result})
            return widget_data
        except Exception as e:
    ...
    def _export_dashboard_json(self, dashboard: GrafanaDashboard) -> dict[str, Any]:
        ...
        try:
            return {'dashboard': {'id': dashboard.dashboard_id, 'title': dashboard.title, 'description': dashboard.description, 'tags': dashboard.tags, 'time': {'from': f'now-{dashboard.time_range}', 'to': 'now'}, 'refresh': dashboard.refresh_interval, 'panels': [{'id': widget.widget_id, 'title': widget.title, 'type': widget.widget_type.value, 'gridPos': widget.position, 'targets': [{'expr': query.query, 'legendFormat': query.legend, 'refId': query.ref_id} for query in widget.queries], 'options': widget.options, 'thresholds': widget.thresholds} for widget in dashboard.widgets]}, 'meta': dashboard.metadata}
        except Exception as e:
    ...
    def _export_dashboard_grafana(self, dashboard: GrafanaDashboard) -> dict[str, Any]:
        ...
        try:
            return {'dashboard': {'title': dashboard.title, 'description': dashboard.description, 'tags': dashboard.tags, 'timezone': 'utc', 'panels': [{'title': widget.title, 'type': widget.widget_type.value, 'gridPos': widget.position, 'targets': [{'expr': query.query, 'legendFormat': query.legend, 'refId': query.ref_id} for query in widget.queries], 'fieldConfig': {'defaults': widget.options, 'thresholds': {'steps': widget.thresholds}}} for widget in dashboard.widgets], 'time': {'from': f'now-{dashboard.time_range}', 'to': 'now'}, 'refresh': dashboard.refresh_interval}, 'overwrite': True}
        except Exception as e:
    ...
    def get_dashboard_integration_status(self) -> dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'prometheus_metrics': len(self._prometheus_metrics), 'grafana_dashboards': len(self._grafana_dashboards), 'cached_queries': len(self._query_cache), 'cached_dashboards': len(self._dashboard_cache), 'config': {'enable_prometheus': self.config.enable_prometheus, 'enable_grafana': self.config.enable_grafana, 'enable_auto_dashboards': self.config.enable_auto_dashboards, 'prometheus_endpoint': self.config.prometheus_endpoint, 'grafana_endpoint': self.config.grafana_endpoint, 'cache_ttl': self.config.cache_ttl}}