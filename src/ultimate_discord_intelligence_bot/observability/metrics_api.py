...
class MetricsAPI:
    ...
    def setup_routes(self):
        ...
        def health_check():
            ...
            return jsonify({'status': 'healthy', 'timestamp': datetime.now(timezone.utc).isoformat(), 'service': 'metrics-api'})
        ...
        def get_all_tool_metrics():
            ...
            try:
                ...
                return jsonify({'status': 'success', 'data': {'tool_count': len(metrics), 'tools': {name: {'tool_name': m.tool_name, 'total_calls': m.total_calls, 'successful_calls': m.successful_calls, 'failed_calls': m.failed_calls, 'success_rate': m.success_rate, 'average_execution_time': m.average_execution_time, 'min_execution_time': m.min_execution_time, 'max_execution_time': m.max_execution_time, 'last_called': m.last_called.isoformat() if m.last_called else None, 'first_called': m.first_called.isoformat() if m.first_called else None} for name, m in metrics.items()}}})
            except Exception as e:
                return (jsonify({'status': 'error', 'error': str(e)}), 500)
        ...
        def get_tool_metrics(tool_name: str):
            ...
            try:
                ...
                return jsonify({'status': 'success', 'data': {'tool_name': metrics.tool_name, 'total_calls': metrics.total_calls, 'successful_calls': metrics.successful_calls, 'failed_calls': metrics.failed_calls, 'success_rate': metrics.success_rate, 'average_execution_time': metrics.average_execution_time, 'min_execution_time': metrics.min_execution_time, 'max_execution_time': metrics.max_execution_time, 'total_execution_time': metrics.total_execution_time, 'last_called': metrics.last_called.isoformat() if metrics.last_called else None, 'first_called': metrics.first_called.isoformat() if metrics.first_called else None, 'error_count': metrics.error_count}})
            except Exception as e:
                return (jsonify({'status': 'error', 'error': str(e)}), 500)
        ...
        def get_top_tools():
            ...
            try:
                ...
                return jsonify({'status': 'success', 'data': {'limit': limit, 'tools': [{'tool_name': tool.tool_name, 'total_calls': tool.total_calls, 'success_rate': tool.success_rate, 'average_execution_time': tool.average_execution_time} for tool in top_tools]}})
            except Exception as e:
                return (jsonify({'status': 'error', 'error': str(e)}), 500)
        ...
        def get_slowest_tools():
            ...
            try:
                ...
                return jsonify({'status': 'success', 'data': {'limit': limit, 'tools': [{'tool_name': tool.tool_name, 'average_execution_time': tool.average_execution_time, 'total_calls': tool.total_calls, 'max_execution_time': tool.max_execution_time} for tool in slowest_tools]}})
            except Exception as e:
                return (jsonify({'status': 'error', 'error': str(e)}), 500)
        ...
        def get_error_prone_tools():
            ...
            try:
                ...
                return jsonify({'status': 'success', 'data': {'limit': limit, 'tools': [{'tool_name': tool.tool_name, 'error_rate': tool.failed_calls / tool.total_calls if tool.total_calls > 0 else 0, 'failed_calls': tool.failed_calls, 'total_calls': tool.total_calls, 'success_rate': tool.success_rate} for tool in error_prone_tools]}})
            except Exception as e:
                return (jsonify({'status': 'error', 'error': str(e)}), 500)
        ...
        def export_metrics():
            ...
            try:
                ...
                if collector.export_metrics(export_file):
                    return jsonify({'status': 'success', 'message': f'Metrics exported to {export_file}', 'export_file': export_file})
                else:
