...
class UnifiedMetricsConfig:
    ...
    export_formats: List[str] = field(
        default_factory=lambda: ["prometheus", "json", "csv"]
    )
...
class UnifiedMetricsCollector:
    ...
    def __init__(self, config: Optional[UnifiedMetricsConfig] = None):
    ...
    async def collect_system_metric(
        ...
        try:
            ...
            return StepResult.fail(f"System metric collection failed: {str(e)}")
    ...
    async def collect_agent_metric(
        ...
        try:
            ...
            return StepResult.fail(f"Agent metric collection failed: {str(e)}")
    ...
    async def collect_performance_metric(
        ...
        try:
            ...
            logger.debug(
                f"Performance metric collected: {component}:{operation} = {duration_ms}ms"
            )
            ...
            return StepResult.fail(f"Performance metric collection failed: {str(e)}")
    ...
    async def collect_quality_metric(
        ...
        try:
            ...
            logger.debug(
                f"Quality metric collected: {component}:{quality_type} = {score}"
            )
            ...
            return StepResult.fail(f"Quality metric collection failed: {str(e)}")
        ...
        try:
            ...
            for metric_key, metrics_list in self._metrics_store.items():
                for metric in metrics_list:
                    ...
                    if (
                        category
                        and hasattr(metric, "category")
                        and metric.category != category
                    ):
                        continue
                    ...
                    if (
                        component
                        and hasattr(metric, "component")
                        and metric.component != component
                    ):
                        continue
                    ...
                    if (
                        agent_type
                        and hasattr(metric, "agent_type")
                        and metric.agent_type != agent_type
                    ):
                        continue
                    ...
                    if (
                        workspace_id
                        and metric.metadata.get("workspace_id") != workspace_id
                    ):
                        continue
            ...
            for metric in filtered_metrics:
                metric_name = getattr(
                    metric, "name", getattr(metric, "metric_name", "unknown")
                )
            ...
            return StepResult.fail(f"Metrics summary retrieval failed: {str(e)}")
    ...
    async def export_metrics(
        ...
        try:
            ...
            if format_type == "prometheus":
                exported_data = await self._export_prometheus_format(
                    category, hours, tenant_id, workspace_id
                )
            elif format_type == "json":
                exported_data = await self._export_json_format(
                    category, hours, tenant_id, workspace_id
                )
            elif format_type == "csv":
                exported_data = await self._export_csv_format(
                    category, hours, tenant_id, workspace_id
                )
            else:
            ...
            return StepResult.fail(f"Metrics export failed: {str(e)}")
    ...
    async def _cleanup_old_metrics(self, metric_key: str) -> None:
        ...
        try:
            ...
            self._metrics_store[metric_key] = [
                metric
                for metric in self._metrics_store[metric_key]
    ...
    async def _export_csv_format(
        ...
        try:
            csv_lines = [
                "timestamp,metric_name,value,metric_type,category,labels,metadata"
            ]
    ...
    def get_collector_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_metrics": sum(
                len(metrics) for metrics in self._metrics_store.values()
            ),
            "config": {
            ...
            "last_collection_times": {
                show_key: time.isoformat()
                for show_key, time in self._last_collection_time.items()
            },
        }
