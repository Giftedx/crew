...
class HealthCheckService:
    ...
    def get_system_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': overall_status, 'timestamp': time.time(), 'uptime_seconds': uptime_seconds, 'version': self._get_version(), 'environment': self._get_environment(), 'services': service_checks, 'performance': performance_summary, 'resources': resource_usage}
        except Exception as e:
            return {'status': 'error', 'timestamp': time.time(), 'error': str(e), 'services': {'health_check_service': {'status': 'error', 'error': str(e)}}}
    ...
    def _check_qdrant(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'status': 'healthy', 'type': 'remote', 'url': qdrant_url, 'collections_count': len(collections.collections), 'response_time_ms': 100}
        except Exception as e:
    ...
    def _check_openrouter(self) -> dict[str, Any]:
        ...
        try:
            ...
            response = resilient_get('https://openrouter.ai/api/v1/auth/key', headers={'Authorization': 'Bearer test'}, timeout_seconds=5)
            if response.status_code == 401:
                return {'status': 'healthy', 'response_time_ms': 200, 'message': 'API reachable (authentication required)'}
            else:
                return {'status': 'healthy', 'response_time_ms': 200, 'message': f'API responded with status {response.status_code}'}
        except Exception as e:
    ...
    def _check_memory_service(self) -> dict[str, Any]:
        ...
        try:
            ...
            test_result = memory_service.add(text='health_check_test', metadata={'source': 'health_check'}, namespace=None)
    ...
    def _assess_overall_health(self, service_checks: dict[str, dict[str, Any]], performance_summary: dict[str, Any], resource_usage: dict[str, Any]) -> str:
        """Assess overall system health based on all checks."""
        critical_failures = [service_name for service_name, check in service_checks.items() if check.get('status') == 'unhealthy']
...
def check_circuit_breakers() -> dict[str, Any]:
    ...
    try:
        ...
        if open_breakers:
            return {'status': 'degraded', 'open_circuit_breakers': open_breakers, 'total_breakers': len(breaker_status), 'message': f'{len(open_breakers)} circuit breakers are open'}
        else:
            return {'status': 'healthy', 'total_breakers': len(breaker_status), 'message': 'All circuit breakers are operational'}
    except Exception as e:
...
def check_performance_health() -> dict[str, Any]:
    ...
    try:
        ...
        if critical_alerts > 0:
            return {'status': 'unhealthy', 'critical_alerts': critical_alerts, 'warning_alerts': warning_alerts, 'message': f'{critical_alerts} critical performance issues detected'}
        elif warning_alerts > 3:
            return {'status': 'degraded', 'critical_alerts': critical_alerts, 'warning_alerts': warning_alerts, 'message': f'{warning_alerts} performance warnings'}
        else:
            return {'status': 'healthy', 'critical_alerts': critical_alerts, 'warning_alerts': warning_alerts, 'message': 'Performance within acceptable ranges'}
    except Exception as e:
...
def get_comprehensive_health() -> dict[str, Any]:
    ...
    all_checks = {'system': system_health, 'circuit_breakers': circuit_breaker_health, 'performance': performance_health, 'resources': resource_health}
...
__all__ = ['HealthCheckService', 'check_circuit_breakers', 'check_performance_health', 'check_resource_health', 'get_comprehensive_health', 'get_health_check_service', 'get_system_health', 'health_check_endpoint']