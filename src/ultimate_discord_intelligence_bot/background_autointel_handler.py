...
async def handle_autointel_background(interaction: Interaction, orchestrator: AutonomousIntelligenceOrchestrator, background_worker: BackgroundIntelligenceWorker, url: str, depth: str='standard') -> None:
    ...
    try:
        ...
        if not webhook_url:
            await interaction.followup.send('❌ **Configuration Error**\n\nBackground processing requires `DISCORD_WEBHOOK` to be configured.\nPlease set the webhook URL in your `.env` file.', ephemeral=True)
            return
        budget_limits: dict[str, Any] = {}
        with contextlib.suppress(Exception):
            budget_limits = orchestrator._get_budget_limits(depth)
        workflow_id = await background_worker.start_background_workflow(url=url, depth=depth, webhook_url=webhook_url, user_id=str(interaction.user.id) if hasattr(interaction, 'user') else None, channel_id=str(interaction.channel_id) if hasattr(interaction, 'channel_id') else None, metadata={'guild_id': str(interaction.guild_id) if hasattr(interaction, 'guild_id') else None, 'command': '/autointel', 'budget_limits': budget_limits})
        acknowledgment = f"# 🚀 Intelligence Analysis Started\n\n**Workflow ID:** `{workflow_id}`\n**URL:** {url}\n**Analysis Depth:** `{depth}`\n**Processing Mode:** Background (no time limits)\n\n---\n\n## What's Happening Now\n\nYour request has been accepted and is now processing in the background. This analysis will run for as long as needed to ensure comprehensive fact-checking and rigorous validation of all claims.\n\n**Key Features:**\n• ✅ **No Time Constraints** - Analysis proceeds at the pace required for accuracy\n• ✅ **Comprehensive Research** - Every claim is independently verified\n• ✅ **Automatic Delivery** - Results will be posted here when complete\n• ✅ **Retrievable** - Use `/retrieve_results workflow_id:{{workflow_id}}` if needed\n\n---\n\n## Expected Timeline\n\n- **Quick analysis:** ~2-5 minutes\n- **Standard analysis:** ~5-15 minutes\n- **Deep analysis:** ~15-45 minutes\n- **Comprehensive/Experimental:** ~45+ minutes (thorough fact-finding)\n\n"
        if budget_limits:
            budget_summary = '\n'.join((f'• {name.replace('_', ' ').title()}: {value}' for name, value in budget_limits.items()))
            acknowledgment += f'## Budget Guardrails\n\n{budget_summary}\n\nThese guardrails are sourced from the orchestrator and help ensure background runs stay within policy.\n\n'
        ...
        try:
            await interaction.followup.send(f'❌ **Failed to Start Analysis**\n\nAn error occurred while initiating background processing:\n```{e!s}```\n\nPlease try again or contact an administrator.', ephemeral=True)
        except Exception:
            with contextlib.suppress(Exception):
                await interaction.response.send_message(f'❌ Error: {e!s}', ephemeral=True)

async def handle_retrieve_results(interaction: Interaction, background_worker: BackgroundIntelligenceWorker, workflow_id: str) -> None:
    ...
    try:
        ...
        if not status:
            await interaction.followup.send(f'❌ **Workflow Not Found**\n\nNo workflow found with ID: `{workflow_id}`\n\nPossible reasons:\n• Invalid workflow ID\n• Workflow was cleaned up (results expire after 7 days)\n• Workflow was started in a different environment', ephemeral=True)
            return
        ...
        if workflow_status == 'initiated' or workflow_status == 'in_progress':
            ...
            response = f'# ❌ Analysis Failed\n\n**Workflow ID:** `{workflow_id}`\n**Status:** Failed\n**Error:** {error}\n\nPlease try running the analysis again or contact an administrator.'
            await interaction.followup.send(response)
        else:
            await interaction.followup.send(f'⚠️ **Unknown Status**\n\nWorkflow `{workflow_id}` has status: `{workflow_status}`\n\nRaw status data:\n```json\n{status}\n```', ephemeral=True)
    except Exception as e:
