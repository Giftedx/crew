...
class RLCacheOptimizer:
    ...
    def _get_pattern_types(self) -> list[str]:
        ...
        return ['frequent_small', 'frequent_large', 'infrequent_small', 'infrequent_large', 'burst_pattern', 'steady_pattern', 'declining_pattern', 'growing_pattern']

    async def optimize_ttl(self, key_pattern: str, current_ttl: int, access_history: list[dict[str, Any]], tenant_id: str='default', workspace_id: str='main') -> StepResult:
        ...
        try:
            ...
            optimization_result = TTLOptimizationResult(original_ttl=current_ttl, optimized_ttl=self._bucket_to_ttl(ttl_bucket), confidence=min(1.0, abs(expected_reward) * 2), expected_hit_rate_improvement=expected_reward * 0.1, expected_cost_savings=expected_reward * 0.05, optimization_method='rl_epsilon_greedy', metadata={'pattern_type': pattern_type, 'ttl_bucket': ttl_bucket, 'expected_reward': expected_reward, 'pattern_analysis': pattern_analysis})
    ...
    async def update_reward(self, key_pattern: str, pattern_type: str, ttl_bucket: int, actual_reward: float, metadata: dict[str, Any] | None=None) -> StepResult:
        """Update Q-table with actual reward"""
        try:
            current_q = self._q_table.get(pattern_type, {}).get(ttl_bucket, 0.0)
            new_q = current_q + self.config.learning_rate * (actual_reward + self.config.reward_discount * max(self._q_table.get(pattern_type, {}).values(), default=0.0) - current_q)
            if pattern_type not in self._q_table:
                self._q_table[pattern_type] = {}
            self._q_table[pattern_type][ttl_bucket] = new_q
            self._record_performance({'timestamp': datetime.now(timezone.utc), 'pattern_type': pattern_type, 'ttl_bucket': ttl_bucket, 'actual_reward': actual_reward, 'new_q_value': new_q, 'metadata': metadata or {}})
        ...
        try:
            ...
            for pattern_key, pattern in self._cache_patterns.items():
                ...
                if pattern_type in self._q_table:
                    ...
                    if improvement > 0.1:
                        recommendations.append({'pattern': pattern_key, 'current_ttl': pattern.ttl_preference, 'recommended_ttl': best_ttl, 'expected_improvement': improvement, 'confidence': min(1.0, improvement * 2), 'pattern_type': pattern_type})
            recommendations.sort(key=lambda x: x['expected_improvement'], reverse=True)
            return StepResult.ok(data={'recommendations': recommendations, 'total_patterns': len(self._cache_patterns), 'optimizable_patterns': len(recommendations), 'q_table_size': sum((len(buckets) for buckets in self._q_table.values()))})
        except Exception as e:
    ...
    def get_optimization_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_updates': len(self._performance_history), 'avg_reward': avg_reward, 'avg_q_value': avg_q_value, 'learning_progress': learning_progress, 'q_table_size': sum((len(buckets) for buckets in self._q_table.values())), 'pattern_types': len(self._q_table), 'last_update': self._performance_history[-1]['timestamp'].isoformat() if self._performance_history else None}
        except Exception as e:
    ...
    def _analyze_access_pattern(self, access_history: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(access_history) >= 10:
            ...
            trend = 'stable'
        return {'frequency': frequency, 'burstiness': burstiness, 'trend': trend, 'total_accesses': len(access_history), 'time_span_hours': total_time.total_seconds() / 3600}
