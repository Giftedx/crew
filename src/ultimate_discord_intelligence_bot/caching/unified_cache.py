...
class UnifiedCacheService:
    ...
    def __init__(self, config: UnifiedCacheConfig | None=None, use_new_config: bool=True):
        ...
        if use_new_config:
            ...
            self.config = UnifiedCacheConfig(enable_l1_cache=new_config.memory.enabled, enable_l2_cache=new_config.redis.enabled, enable_l3_cache=new_config.semantic.enabled, enable_rl_optimization=new_config.auto_tier_selection, enable_metrics=new_config.metrics.enabled, l1_max_size=new_config.memory.max_size, l1_default_ttl=new_config.domain.tool.ttl if new_config.domain.tool.enabled else 300, l2_host=new_config.redis.host, l2_port=new_config.redis.port, l2_db=new_config.redis.db, l2_default_ttl=new_config.redis.default_ttl, l3_default_ttl=new_config.semantic.ttl, l3_similarity_threshold=new_config.semantic.similarity_threshold, optimization_interval=300, metrics_reset_interval=3600)
        else:
    ...
    def _initialize_cache_layers(self) -> None:
        ...
        try:
            ...
            if self.config.enable_l2_cache and Redis:
                try:
                    self._l2_client = redis.Redis(host=self.config.l2_host, port=self.config.l2_port, db=self.config.l2_db, decode_responses=False)
        ...
        try:
            ...
            if self.config.enable_l1_cache and self._l1_cache is not None:
                ...
                if l1_result.success and l1_result.data is not None:
                    ...
                    return StepResult.ok(data=CacheResult(hit=True, value=l1_result.data, cache_level='l1', latency_ms=latency, metadata={'source': 'l1_memory'}))
            if self.config.enable_l2_cache and self._l2_client is not None:
                ...
                if l2_result.success and l2_result.data is not None:
                    ...
                    return StepResult.ok(data=CacheResult(hit=True, value=l2_result.data, cache_level='l2', latency_ms=latency, cost=0.001, metadata={'source': 'l2_redis'}))
            if self.config.enable_l3_cache and self._l3_service is not None:
                ...
                if l3_result.success and l3_result.data is not None:
                    ...
                    return StepResult.ok(data=CacheResult(hit=True, value=l3_result.data, cache_level='l3', latency_ms=latency, cost=0.01, metadata={'source': 'l3_semantic'}))
            if self._core_cache is not None:
                ...
                if core_result.success and core_result.data is not None:
                    ...
                    return StepResult.ok(data=CacheResult(hit=True, value=core_result.data, cache_level='core', latency_ms=latency, cost=0.005, metadata={'source': 'core_cache'}))
            latency = (time.time() - start_time) * 1000
            self._record_metrics(metrics_key, False, 'none', latency, 0.0)
            return StepResult.ok(data=CacheResult(hit=False, cache_level='none', latency_ms=latency, metadata={'source': 'cache_miss'}))
        except Exception as e:
            logger.error(f'Error in cache get operation: {e}', exc_info=True)
            return StepResult.fail(f'Cache get failed: {e!s}')

    async def set(self, key: str, value: Any, ttl: int | None=None, cache_level: str='auto', tenant_id: str | None=None, workspace_id: str | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'set_in_layers': [level for level, success in results if success], 'total_layers': len(results), 'success_count': success_count, 'metadata': metadata or {}})
        except Exception as e:
    ...
    async def delete(self, key: str, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            if self._core_cache is not None:
                ...
                results.append(('core', core_result.success))
            success_count = sum((1 for _, success in results if success))
            return StepResult.ok(data={'deleted_from_layers': [level for level, success in results if success], 'total_layers': len(results), 'success_count': success_count})
        except Exception as e:
    ...
    def get_metrics(self, tenant_id: str | None=None, workspace_id: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            if metrics.total_requests > 0:
                metrics.avg_latency_ms = sum((getattr(metrics, f'{level}_hits', 0) * (1.0 if level == 'l1' else 2.0 if level == 'l2' else 5.0) for level in ['l1', 'l2', 'l3'])) / metrics.total_requests
            return {'total_requests': metrics.total_requests, 'total_hits': metrics.total_hits, 'total_misses': metrics.total_misses, 'hit_rate': metrics.hit_rate, 'l1_hits': metrics.l1_hits, 'l2_hits': metrics.l2_hits, 'l3_hits': metrics.l3_hits, 'avg_latency_ms': metrics.avg_latency_ms, 'total_cost': metrics.total_cost, 'last_reset': metrics.last_reset.isoformat(), 'active_layers': self._get_active_layers()}
        except Exception as e:
    ...
    async def _cleanup_l1(self) -> None:
        ...
        try:
            ...
            expired_keys = [key for key, data in self._l1_cache.items() if isinstance(data, dict) and data.get('expire_time', 0) < current_time]
    ...
    async def _set_l3(self, key: str, value: Any, ttl: int, tenant_id: str, workspace_id: str, metadata: dict[str, Any] | None) -> StepResult:
