...
class TaskScheduler:
    ...
    def __init__(self, config: TaskManagerConfig | None=None):
        ...
        self._priority_queue: list[tuple[int, datetime, str]] = []
    ...
    def schedule_task(self, task_id: str, priority: int, estimated_duration: int=0, resource_requirements: dict[str, Any] | None=None, scheduled_time: datetime | None=None) -> StepResult:
        ...
        try:
            ...
            task_schedule = TaskSchedule(task_id=task_id, scheduled_time=scheduled_time, priority=priority, estimated_duration=estimated_duration, resource_requirements=resource_requirements or {})
    ...
    def get_scheduler_status(self) -> dict[str, Any]:
        ...
        try:
            return {'queued_tasks': len(self._priority_queue), 'scheduled_tasks': len(self._scheduled_tasks), 'resource_usage': self._resource_usage.copy(), 'max_resources': self._max_resources.copy(), 'next_task': self._priority_queue[0][2] if self._priority_queue else None}
        except Exception as e:
...
class TaskManager:
    ...
    def add_task(self, task_id: str, task_type: str, priority: int=2, dependencies: list[str] | None=None, estimated_duration: int=0, resource_requirements: dict[str, Any] | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        """Add a task to the manager"""
        try:
            task_node = TaskNode(task_id=task_id, task_type=task_type, priority=priority, dependencies=set(dependencies or []), dependents=set(), metadata=metadata or {})
            if dependencies:
                dep_result = self._dependency_resolver.add_task_dependencies(task_id, dependencies)
                if not dep_result.success:
                    return dep_result
            schedule_result = self._task_scheduler.schedule_task(task_id=task_id, priority=priority, estimated_duration=estimated_duration, resource_requirements=resource_requirements)
    ...
    def get_ready_tasks(self) -> list[str]:
        ...
        try:
            ...
            ready_tasks.sort(key=lambda tid: self._task_nodes.get(tid, TaskNode('', '', 0)).priority, reverse=True)
            return ready_tasks
        except Exception as e:
            logger.error(f'Error getting ready tasks: {e}')
            return []
    ...
    def get_task_status(self, task_id: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'task_id': task_id, 'task_type': task_node.task_type, 'status': task_node.status, 'priority': task_node.priority, 'dependencies': list(task_node.dependencies), 'dependents': list(task_node.dependents), 'created_at': task_node.created_at.isoformat(), 'metadata': task_node.metadata})
        except Exception as e:
    ...
    def get_manager_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_tasks': len(self._task_nodes), 'running_tasks': len(self._running_tasks), 'completed_tasks': len(self._completed_tasks), 'ready_tasks': len(self.get_ready_tasks()), 'scheduler_status': scheduler_status, 'dependency_graph_size': len(self._dependency_resolver._dependency_graph), 'reverse_dependency_graph_size': len(self._dependency_resolver._reverse_dependency_graph)}
        except Exception as e:
