...
class FallbackStrategy:
    ...
    async def execute_workflow(self, url: str, depth: str='standard', tenant: str='default', workspace: str='main', **kwargs: Any) -> StepResult:
        ...
        try:
            ...
            if interaction:
                context = OrchestrationContext(tenant_id=tenant, request_id=str(uuid.uuid4()), metadata={'url': url, 'depth': depth, 'workspace': workspace})
                facade = get_orchestration_facade()
                result = await facade.orchestrate('fallback_autonomous', context, interaction=interaction, url=url, depth=depth)
                metrics.counter('orchestration_strategy_executions_total', labels={'strategy': self.name, 'outcome': 'success' if result.success else 'failure'})
                if result.success:
                    return StepResult.ok(message='Fallback workflow completed', mode='fallback', url=url, tenant=tenant, workspace=workspace)
                else:
                    return result
            else:
            ...
            return StepResult.fail(f'Fallback orchestration failed: {exc}', step='fallback_workflow')
    ...
    async def _execute_pipeline_only(self, url: str) -> StepResult:
        ...
        try:
            ...
            if not pipeline_result.success:
                return StepResult.fail(f'Pipeline failed: {pipeline_result.error}', step='fallback_pipeline')
            ...
            return StepResult.ok(pipeline=pipeline_data, analysis=analysis_result.data if analysis_result.success else {}, fact_check=fact_result.data if fact_result.success else {}, mode='fallback', message='Fallback workflow completed without Discord interaction')
        except Exception as exc:
