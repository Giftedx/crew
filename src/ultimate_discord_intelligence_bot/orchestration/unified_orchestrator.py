...
class UnifiedOrchestrationService:
    ...
    async def execute_task(self, task_request: TaskRequest, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            if not await self._check_dependencies(task_request):
                ...
                return StepResult.ok(data={'status': 'queued', 'reason': 'dependencies_not_satisfied', 'task_id': task_request.task_id})
            if len(self._running_tasks) >= self.config.max_concurrent_tasks:
                self._task_queue[task_request.task_id] = task_request
                return StepResult.ok(data={'status': 'queued', 'reason': 'concurrent_task_limit_reached', 'task_id': task_request.task_id})
    ...
    async def _execute_task_internal(self, task_request: TaskRequest) -> TaskResult:
        ...
        try:
            ...
            task_result = TaskResult(task_id=task_id, status=TaskStatus.COMPLETED if result.success else TaskStatus.FAILED, result_data=result.data if result.success else None, error_message=result.error if not result.success else None, execution_time_ms=execution_time, started_at=start_time, completed_at=end_time, retry_count=task_request.retry_count, metadata={'task_type': task_request.task_type, 'tenant_id': task_request.tenant_id, 'workspace_id': task_request.workspace_id, 'orchestrator_used': self._get_orchestrator_used(task_request)})
            if result.success:
            ...
            end_time = datetime.now(timezone.utc)
            execution_time = (end_time - start_time).total_seconds() * 1000
            task_result = TaskResult(task_id=task_id, status=TaskStatus.FAILED, error_message=str(e), execution_time_ms=execution_time, started_at=start_time, completed_at=end_time, retry_count=task_request.retry_count, metadata={'error_type': type(e).__name__, 'task_type': task_request.task_type})
            self._failed_tasks[task_id] = task_result
            if task_request.retry_count < task_request.max_retries:
                await self._schedule_retry(task_request)
            return task_result
    ...
    async def _execute_via_crew_executor(self, task_request: TaskRequest) -> StepResult:
        ...
        try:
            ...
            result = await self._enhanced_crew_executor.execute_with_comprehensive_monitoring(inputs=task_request.payload, tenant_id=task_request.tenant_id, workspace_id=task_request.workspace_id)
            return result
        except Exception as e:
    ...
    async def _execute_via_hierarchical_orchestrator(self, task_request: TaskRequest) -> StepResult:
        ...
        try:
            ...
            result = await self._hierarchical_orchestrator.execute_task(task_type=task_request.task_type, payload=task_request.payload, tenant_id=task_request.tenant_id, workspace_id=task_request.workspace_id)
            return result
        except Exception as e:
    ...
    async def _execute_generic_task(self, task_request: TaskRequest) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'task_id': task_request.task_id, 'task_type': task_request.task_type, 'executed_at': datetime.now(timezone.utc).isoformat(), 'payload': task_request.payload})
        except Exception as e:
    ...
    async def _process_queued_tasks(self, tenant_id: str, workspace_id: str) -> None:
        ...
        try:
            ...
            for task_id, task_request in list(self._task_queue.items()):
                if await self._check_dependencies(task_request) and len(self._running_tasks) < self.config.max_concurrent_tasks:
    ...
    def get_task_status(self, task_id: str) -> StepResult:
        ...
        try:
            if task_id in self._running_tasks:
                return StepResult.ok(data={'task_id': task_id, 'status': TaskStatus.RUNNING.value, 'started_at': self._running_tasks[task_id].metadata.get('started_at')})
            if task_id in self._completed_tasks:
                result = self._completed_tasks[task_id]
                return StepResult.ok(data={'task_id': task_id, 'status': result.status.value, 'result_data': result.result_data, 'execution_time_ms': result.execution_time_ms, 'started_at': result.started_at.isoformat() if result.started_at else None, 'completed_at': result.completed_at.isoformat() if result.completed_at else None, 'metadata': result.metadata})
            if task_id in self._failed_tasks:
                result = self._failed_tasks[task_id]
                return StepResult.ok(data={'task_id': task_id, 'status': result.status.value, 'error_message': result.error_message, 'execution_time_ms': result.execution_time_ms, 'started_at': result.started_at.isoformat() if result.started_at else None, 'completed_at': result.completed_at.isoformat() if result.completed_at else None, 'retry_count': result.retry_count, 'metadata': result.metadata})
            if task_id in self._task_queue:
                return StepResult.ok(data={'task_id': task_id, 'status': TaskStatus.PENDING.value, 'dependencies': self._task_queue[task_id].dependencies, 'priority': self._task_queue[task_id].priority.value})
            return StepResult.fail(f'Task {task_id} not found')
        except Exception as e:
            logger.error(f'Error getting task status: {e}')
            return StepResult.fail(f'Task status retrieval failed: {e!s}')

    def get_orchestration_metrics(self, tenant_id: str | None=None, workspace_id: str | None=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_tasks': metrics.total_tasks, 'completed_tasks': metrics.completed_tasks, 'failed_tasks': metrics.failed_tasks, 'success_rate': metrics.success_rate, 'avg_execution_time_ms': metrics.avg_execution_time_ms, 'active_tasks': metrics.active_tasks, 'queued_tasks': metrics.queued_tasks, 'last_reset': metrics.last_reset.isoformat(), 'active_components': self._get_active_components()}
        except Exception as e:
    ...
    def cancel_task(self, task_id: str) -> StepResult:
        ...
        try:
            if task_id in self._running_tasks:
                task_result = TaskResult(task_id=task_id, status=TaskStatus.CANCELLED, completed_at=datetime.now(timezone.utc))
                self._failed_tasks[task_id] = task_result
                del self._running_tasks[task_id]
    ...
    def get_orchestration_status(self) -> dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'active_components': self._get_active_components(), 'running_tasks': len(self._running_tasks), 'queued_tasks': len(self._task_queue), 'completed_tasks': len(self._completed_tasks), 'failed_tasks': len(self._failed_tasks), 'max_concurrent_tasks': self.config.max_concurrent_tasks, 'hierarchical_orchestrator': self._hierarchical_orchestrator is not None, 'enhanced_crew_executor': self._enhanced_crew_executor is not None}