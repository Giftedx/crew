...
from .advanced_performance_analytics_alert_engine import AdvancedPerformanceAnalyticsAlertEngine, AlertSeverity, PerformanceAlert
...
class AdvancedPerformanceAnalyticsDiscordIntegration:
    ...
    def __init__(self, alert_engine: AdvancedPerformanceAnalyticsAlertEngine | None=None, config: DiscordNotificationConfig | None=None):
    ...
    def _format_alert_for_discord(self, alert: PerformanceAlert) -> dict[str, Any]:
        ...
        discord_data['analytics_integration'] = {'alert_id': alert.alert_id, 'severity': alert.severity.value, 'category': alert.category.value, 'timestamp': alert.timestamp.isoformat()}
        return discord_data
    ...
    async def send_alert_notification(self, alert: PerformanceAlert) -> dict[str, Any]:
        ...
        try:
            ...
            if self._is_notification_in_cooldown(cooldown_key):
                return {'status': 'cooldown', 'message': f'Notification type {cooldown_key} is in cooldown period', 'alert_id': alert.alert_id}
            alert_tool = self._get_alert_tool_for_severity(alert.severity)
            if not alert_tool:
                return {'status': 'no_tool', 'message': f'No Discord tool configured for severity {alert.severity.value}', 'alert_id': alert.alert_id}
            discord_data = self._format_alert_for_discord(alert)
            result = alert_tool._run(message=discord_data['content'], metrics=alert.metrics, thresholds=alert.thresholds)
            notification_record = {'alert_id': alert.alert_id, 'severity': alert.severity.value, 'category': alert.category.value, 'timestamp': default_utc_now(), 'status': 'success' if result.success else 'failed', 'webhook_used': alert_tool.webhook_url if hasattr(alert_tool, 'webhook_url') else 'unknown', 'message_length': len(discord_data['content']), 'result': result.to_dict() if hasattr(result, 'to_dict') else str(result)}
            ...
            if result.success:
                ...
                return {'status': 'success', 'alert_id': alert.alert_id, 'notification_id': notification_record['timestamp'].isoformat(), 'webhook_used': notification_record['webhook_used']}
            else:
    ...
    async def send_batch_notifications(self, alerts: list[PerformanceAlert]) -> dict[str, Any]:
        ...
        try:
            ...
            if not self.config.batch_notifications:
                ...
                return {'status': 'individual_sent', 'total_alerts': len(alerts), 'successful': success_count, 'failed': len(alerts) - success_count, 'results': results}
            ...
            return {'status': 'batch_sent', 'total_alerts': len(alerts), 'batch_groups': len(batch_results), 'successful_batches': success_count, 'failed_batches': len(batch_results) - success_count, 'results': batch_results}
        except Exception as e:
    ...
    async def _send_batched_alerts(self, severity: AlertSeverity, alerts: list[PerformanceAlert]) -> dict[str, Any]:
        ...
        try:
            ...
            if not alert_tool:
                return {'status': 'no_tool', 'message': f'No Discord tool configured for severity {severity.value}', 'alert_count': len(alerts)}
            ...
            batch_record = {'batch_id': f'batch_{timestamp.replace(' ', '_').replace(':', '_')}', 'severity': severity.value, 'alert_count': len(alerts), 'timestamp': default_utc_now(), 'status': 'success' if result.success else 'failed', 'webhook_used': alert_tool.webhook_url if hasattr(alert_tool, 'webhook_url') else 'unknown', 'message_length': len(batch_content), 'alert_ids': [alert.alert_id for alert in alerts]}
            ...
            if result.success:
                ...
                return {'status': 'success', 'batch_id': batch_record['batch_id'], 'alert_count': len(alerts), 'severity': severity.value}
            else:
    ...
    async def send_executive_summary(self, hours: int=24) -> dict[str, Any]:
        ...
        try:
            ...
            analytics_results = await self.alert_engine.analytics_system.run_comprehensive_performance_analysis(lookback_hours=hours, include_optimization=False)
            ...
            if result.success:
                ...
                return {'status': 'success', 'summary_period_hours': hours, 'total_alerts_summarized': alert_summary.get('total_alerts', 0), 'message_length': len(summary_content)}
            else:
    ...
    async def get_notification_statistics(self, hours: int=24) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_notifications': len(recent_notifications), 'time_period_hours': hours, 'status_breakdown': status_counts, 'severity_breakdown': severity_counts, 'average_message_length': round(avg_message_length, 1), 'success_rate': round(status_counts.get('success', 0) / len(recent_notifications) * 100 if recent_notifications else 0, 1), 'configuration': {'enabled': self.config.enabled, 'batch_notifications': self.config.batch_notifications, 'cooldown_minutes': self.config.notification_cooldown_minutes, 'tools_configured': {'primary': self.primary_alert_tool is not None, 'private': self.private_alert_tool is not None, 'executive': self.executive_alert_tool is not None}}}
        except Exception as e:
...
async def send_analytics_monitoring_batch(lookback_hours: int=2, webhook_url: str | None=None) -> dict[str, Any]:
    ...
    return {'status': 'monitoring_complete', 'lookback_hours': lookback_hours, 'alerts_generated': len(alerts), 'notification_result': notification_result}