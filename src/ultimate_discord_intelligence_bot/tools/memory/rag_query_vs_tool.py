    ...
    def run(
        ...
        *,
        ...
        if hits:
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "rag_query_vs", "outcome": "success"},
                ).inc()
        ...
        if isinstance(documents, list) and documents:
            try:
                ...
                with contextlib.suppress(Exception):
                    self._metrics.counter(
                        "tool_runs_total",
                        labels={"tool": "rag_query_vs", "outcome": "fallback_offline"},
                    ).inc()
                ...
                with contextlib.suppress(Exception):
                    self._metrics.counter(
                        "tool_runs_total",
                        labels={"tool": "rag_query_vs", "outcome": "error_offline"},
                    ).inc()
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "rag_query_vs", "outcome": "no_results"},
            ).inc()
