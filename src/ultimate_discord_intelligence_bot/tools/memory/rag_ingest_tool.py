        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "rag_ingest", "outcome": "success", "tenant_scoped": str(tenant_scoped).lower()},
                ).inc()
            return StepResult.ok(
                data={"inserted": inserted, "chunks": chunks_total, "index": namespace, "tenant_scoped": tenant_scoped}
            )
        except Exception as exc:
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "rag_ingest", "outcome": "error", "tenant_scoped": str(tenant_scoped).lower()},
                ).inc()
            return StepResult.fail(
                str(exc),
                data={"inserted": inserted, "chunks": chunks_total, "index": index, "tenant_scoped": tenant_scoped},
            )
