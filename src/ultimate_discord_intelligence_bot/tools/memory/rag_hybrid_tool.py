...
class RagHybridTool(BaseTool[StepResult]):
        ...
        try:
            ...
            for p in getattr(pts, 'points', pts) or []:
                ...
                if isinstance(text, str):
                    out.append({'text': text, 'score': float(score) if isinstance(score, (int, float)) else 0.0, 'source': 'vector'})
            return out
        except Exception:
            return []
    ...
    def _merge(self, vec_hits: list[dict[str, Any]], tfidf_hits: list[dict[str, Any]], alpha: float, top_k: int) -> list[dict[str, Any]]:
        ...
        for v in by_text.values():
            ...
            src = v['source']
            combined.append({'text': v['text'], 'score': round(float(score), 6), 'source': 'both' if len(src) > 1 else next(iter(src))})
        combined.sort(key=lambda d: d.get('score', 0.0), reverse=True)
        return combined[:max(1, top_k)]

    def _maybe_rerank(self, query_text: str, hits: list[dict[str, Any]], enable_rerank: bool | None) -> tuple[list[dict[str, Any]], bool]:
    ...
    def run(self, *, query: str, index: str='memory', candidate_docs: list[str] | None=None, top_k: int=5, alpha: float=0.7, enable_rerank: bool | None=None) -> StepResult:
        try:
