...
class ResearchAndBriefMultiTool(BaseTool[StepResult]):
    ...
    description: str = 'Synthesizes a brief from sources with an offline core, optionally augmented by a multi-agent run.'
        ...
        try:
            ...
            def worker() -> None:
                ...
                async def _runner() -> dict[str, Any]:
                    ...
                    return await executor.execute_with_comprehensive_monitoring(inputs=inputs, enable_real_time_alerts=False, quality_threshold=0.7, max_execution_time=timeout)
                try:
    ...
    def run(self, *, query: str, sources_text: list[str] | None=None, max_items: int=5, max_time: float=60.0, enable_alerts: bool=False) -> StepResult:
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter('tool_runs_total', labels={'tool': 'research_and_brief_multi', 'outcome': 'success'}).inc()
