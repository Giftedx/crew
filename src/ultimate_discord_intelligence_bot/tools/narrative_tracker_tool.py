...
class NarrativeTrackerTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            # Step 1: Query KG for related content
            ...
            # Step 2: Build temporal timeline
            timeline = self._build_timeline(events, max_events)

            # Step 3: Identify contradictions and clarifications
            ...
            logger.error(f"Narrative tracking failed: {str(e)}")
            return StepResult.fail(f"Narrative tracking failed: {str(e)}")
    ...
    def _query_narrative_events(
        ...
        try:
            # For now, return mock data that simulates KG queries
            # In a real implementation, this would query the actual KG store
            ...
            logger.error(f"Failed to query narrative events: {str(e)}")
            return []
    ...
    def _build_timeline(self, events: list[dict[str, Any]], max_events: int) -> list[NarrativeEvent]:
        ...
        for event_data in sorted_events:
            event = NarrativeEvent(
                ...
                contradictions=[],  # Will be populated later
                clarifications=[],  # Will be populated later
                provenance={
                    "url": event_data["url"],
    ...
    def _identify_contradictions(self, timeline: list[NarrativeEvent]) -> list[dict[str, Any]]:
        ...
        # Simple contradiction detection based on claim analysis
        # In a real implementation, this would use more sophisticated NLP

        for i, event1 in enumerate(timeline):
            for j, event2 in enumerate(timeline[i + 1 :], i + 1):
    ...
    def _claims_contradict(self, claims1: list[str], claims2: list[str]) -> bool:
        ...
        # Simple heuristic: if claims contain opposite sentiment words
        ...
        # Simple contradiction: one set has positive sentiment, other has negative
    ...
    def _generate_insights(self, timeline: list[NarrativeEvent], contradictions: list[dict[str, Any]]) -> list[str]:
        ...
        # Insight 2: Creator participation
        creators = set(event.creator for event in timeline)
        insights.append(f"Active creators: {', '.join(creators)}")

        # Insight 3: Platform distribution
        platforms = set(event.platform for event in timeline)
        insights.append(f"Platforms involved: {', '.join(platforms)}")

        # Insight 4: Contradictions
        if contradictions:
        ...
        # Insight 5: Quote attribution accuracy
    ...
    def _calculate_statistics(self, timeline: list[NarrativeEvent], creators: list[str]) -> dict[str, Any]:
        ...
        platforms = list(set(event.platform for event in timeline))
