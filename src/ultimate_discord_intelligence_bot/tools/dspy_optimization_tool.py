...
class DSPyOptimizationSchema(BaseModel):
    action: Literal["optimize", "load", "compare", "metric"] = Field(
        ..., description="Operation to perform"
    )
    signature: Optional[SignatureName] = Field(
        None, description="DSPy signature name to optimize/evaluate"
    )
    ...
    training_data_path: Optional[str] = Field(
        None, description="Path to JSON training set (from generator script)"
    )
    test_data_path: Optional[str] = Field(
        None, description="Path to JSON test set for evaluation/compare"
    )
    data_limit: Optional[int] = Field(
        50, description="Limit number of examples loaded from dataset"
    )
    agent_name: Optional[str] = Field(
        None, description="Logical agent name for saving/loading modules"
    )
    version: Optional[str] = Field(
        None, description="Version tag to save or load an optimized module"
    )
    baseline_version: Optional[str] = Field(
        None, description="Baseline version to compare against"
    )
...
class DSPyOptimizationTool(BaseTool[dict]):
    ...
    description: str = (
        "Compile and evaluate DSPy-optimized modules for specific agent signatures."
    )
        ...
        for rec in raw if isinstance(raw, list) else []:
            ...
            try:
                ...
                if sig_name == "DebateAnalysisSignature":
                    ...
                    ex = dspy.Example(transcript=str(transcript)).with_inputs(
                        "transcript"
                    )
                elif sig_name in (
                    "SentimentAnalysisSignature",
                    ...
                    ex = dspy.Example(claim=str(claim), context=str(ctx)).with_inputs(
                        "claim", "context"
                    )
                    ...
                    ex = dspy.Example(content=str(content), max_length=150).with_inputs(
                        "content", "max_length"
                    )
                examples.append(ex)
            except Exception:
    ...
    def _run(
        ...
        signature: Optional[SignatureName] = None,
        optimization_level: Optional[Literal["light", "medium", "heavy"]] = "medium",
        training_data_path: Optional[str] = None,
        test_data_path: Optional[str] = None,
        data_limit: Optional[int] = 50,
        ...
        if not getattr(self.settings, "enable_dspy_optimization", False):
            return StepResult.skip(
                step=self.name, reason="DSPy optimization disabled in settings"
            )
        ...
        if sig_cls is None:
            return StepResult.fail(
                "Unable to resolve DSPy signature; ensure dspy is installed"
            )
        ...
        try:
            with get_metrics().tool_timer(self.name):
                if action == "optimize":
                    if not training_data_path:
                        return StepResult.fail(
                            "training_data_path is required for optimize"
                        )
                    train = self._load_examples(
                        training_data_path, sig_cls, data_limit or 50
                    )
                    if not train:
                        return StepResult.fail(
                            "No training examples loaded from training_data_path"
                        )
                    ...
                    if agent_name and version:
                        sv = optimizer.save_optimized_module(
                            res.data, agent_name=agent_name, version=version
                        )
                        if not sv.success:
                            return sv
                        saved_to = (
                            sv.data.get("saved_to")
                            if isinstance(sv.data, dict)
                            else None
                        )
                    get_metrics().counter(
                        "tool_runs_total",
                ...
                if action == "load":
                    if not agent_name or not version:
                        return StepResult.fail(
                            "agent_name and version are required for load"
                        )
                    optimizer = AgentOptimizer()
                    lr = optimizer.load_optimized_module(
                    ...
                    return StepResult.ok(
                        data={"loaded": True, "agent": agent_name, "version": version}
                    )
                ...
                if action == "compare":
                    ...
                    optimizer = AgentOptimizer()
                    lr_opt = optimizer.load_optimized_module(
                        agent_name=agent_name, version=version, signature=sig_cls
                    )
                    ...
                    if not lr_base.success:
                        return lr_base
                    test = self._load_examples(
                        test_data_path, sig_cls, data_limit or 50
                    )
                    if not test:
                        return StepResult.fail(
                            "No test examples loaded from test_data_path"
                        )
                ...
                if action == "metric":
                    ...
                    if agent_name and version:
                        ...
                        return StepResult.fail(
                            "agent_name and version are required for metric"
                        )
                    test = self._load_examples(
                        test_data_path, sig_cls, data_limit or 50
                    )
            ...
            get_metrics().counter(
                "tool_runs_total", labels={"tool": self.name, "outcome": "error"}
            ).inc()
