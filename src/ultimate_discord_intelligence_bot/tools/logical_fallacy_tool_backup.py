...
class LogicalFallacyTool(BaseTool):
    ...
    PATTERN_FALLACIES: ClassVar[Dict[str, List[str]]] = {
        ...
        "circular reasoning": [
            ...
            r'\b(\w+\s+is\s+\w+\s+because\s+\1\s+is\s+\w+)\b'  # X is Y because X is Y
        ],
    ...
    STRUCTURAL_PATTERNS: ClassVar[Dict[str, List[str]]] = {
        "begging the question": [
            # Look for circular definitions
            r'\b(\w+)\s+is\s+(\w+)\s+because\s+\1\s+is\s+\2\b',
...
class LogicalFallacyTool(BaseTool):
            ...
            for pattern in patterns:
                if re.search(pattern, text_lower, re.IGNORECASE):
                    if fallacy not in findings:
                        findings.append(fallacy)
                        confidence_scores[fallacy] = 0.8  # High confidence for regex matches
                    break
        ...
        for fallacy, patterns in self.STRUCTURAL_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, text_lower, re.IGNORECASE):
                    if fallacy not in findings:
                        findings.append(fallacy)
                        confidence_scores[fallacy] = 0.7  # Medium confidence for structural patterns
                    break
    ...
    def _check_heuristic_fallacies(self, text: str) -> List[Tuple[str, float]]:
        ...
        # Check for excessive use of absolutes (hasty generalization indicator)
        ...
        # Check for excessive emotional language (appeal to emotion indicator)
        ...
        # Check for question patterns that might indicate loaded questions
        if '?' in text:
            ...
            if any(pattern in text_lower for pattern in loaded_question_patterns):
                fallacies.append(("loaded question", 0.6))
        
        # Check for comparison fallacies
        if any(phrase in text_lower for phrase in ['like comparing', 'apples to oranges', 'different as']):
            fallacies.append(("false analogy", 0.7))
