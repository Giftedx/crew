...
class CheckpointArgs(BaseModel):
    action: Literal["list", "load", "delete"] = Field(
        ..., description="Operation to perform"
    )
    thread_id: Optional[str] = Field(
        None, description="Thread ID to operate on (required for load/delete)"
    )
    store_path: Optional[str] = Field(
        "crew_data/langgraph/checkpoints.meta.json",
        description="Path to metadata store tracking checkpoints",
    )
...
class CheckpointManagementTool(BaseTool[dict]):
    ...
    def _run(
        self,
        action: Literal["list", "load", "delete"],
        ...
        if action == "load":
            ...
            return StepResult.ok(
                data={"checkpoint": {k: v for k, v in rec.items() if k != "events"}}
            )
