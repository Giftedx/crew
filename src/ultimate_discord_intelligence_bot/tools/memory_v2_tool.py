...
class MemoryV2OperationInput(BaseModel):
    ...
    operation: str = Field(
        ..., description="Operation: upsert, query, delete, get_stats"
    )
    content: str | None = Field(
        default=None, description="Content for upsert operations"
    )
    vector: list[float] | None = Field(
        default=None, description="Vector for query operations"
    )
    ...
    creator: str | None = Field(
        default=None, description="Creator identifier for namespace isolation"
    )
        ...
        try:
            ...
            if operation == "upsert":
                if not content:
                    ...
                    return json.dumps(
                        StepResult.fail(
                            "Content required for upsert operation"
                        ).to_dict()
                    )
                ...
                result = asyncio.run(
                    memory.upsert(namespace, records, creator or "default")
                )
                import json
                ...
                if not vector:
                    ...
                    return json.dumps(
                        StepResult.fail("Vector required for query operation").to_dict()
                    )

                import asyncio

                result = asyncio.run(
                    memory.query(namespace, vector, top_k, creator or "default")
                )
                import json
                ...
                return json.dumps(
                    StepResult.fail(
                        "Delete operation not supported in current facade"
                    ).to_dict()
                )
            ...
            return json.dumps(
                StepResult.fail(f"Memory operation failed: {exc}").to_dict()
            )
