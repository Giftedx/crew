...
class OpenAIEnhancedAnalysisTool(BaseTool):
    ...
    def _run(self, content: str, analysis_type: str, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            result = self.openai_service.process_with_enhancements_sync(content=content, enhancements=enhancements, tenant=tenant, workspace=workspace, analysis_type=analysis_type)
            if not result.success:
                return result
            return StepResult.ok(data={'analysis': result.data, 'enhanced': True, 'openai_powered': True, 'enhancements_used': enhancements})
        except Exception as e:
            return StepResult.fail(f'Enhanced analysis failed: {e!s}')

    async def stream_analysis(self, content: str, analysis_type: str, tenant: str, workspace: str) -> AsyncGenerator[StepResult, None]:
        """Stream analysis results in real-time."""
        try:
            async for result in self.openai_service.stream_enhanced_analysis(content=content, analysis_type=analysis_type, tenant=tenant, workspace=workspace):
                yield result
        except Exception as e:
            yield StepResult.fail(f'Streaming analysis failed: {e!s}')

    async def analyze_with_images(self, content: str, images: list[bytes], analysis_type: str, tenant: str, workspace: str) -> StepResult:
        """Analyze content with images using multimodal capabilities."""
        try:
            enhancements = ['structured_outputs', 'vision', 'multimodal']
            result = await self.openai_service.process_with_enhancements(content=content, enhancements=enhancements, tenant=tenant, workspace=workspace, analysis_type=analysis_type, images=images)
            if not result.success:
                return result
            return StepResult.ok(data={'multimodal_analysis': result.data, 'enhanced': True, 'openai_powered': True, 'images_analyzed': len(images)})
        except Exception as e:
    ...
    async def analyze_voice_content(self, audio_data: bytes, analysis_type: str, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            result = await self.openai_service.analyze_voice_content(audio_data=audio_data, analysis_type=analysis_type, tenant=tenant, workspace=workspace)
            if not result.success:
                return result
    ...
    async def fact_check_multimodal(self, text: str, images: list[bytes], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            result = await self.openai_service.fact_check_multimodal(text=text, images=images, tenant=tenant, workspace=workspace)
            if not result.success:
                return result
    ...
    async def detect_bias_multimodal(self, text: str, images: list[bytes], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            result = await self.openai_service.detect_bias_multimodal(text=text, images=images, tenant=tenant, workspace=workspace)
            if not result.success:
                return result
    ...
    async def generate_summary(self, text: str, images: list[bytes], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            result = await self.openai_service.generate_multimodal_summary(text=text, images=images, tenant=tenant, workspace=workspace)
            if not result.success:
                return result
