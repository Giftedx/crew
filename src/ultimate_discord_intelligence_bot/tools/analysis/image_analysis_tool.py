...
class ImageAnalysisTool(BaseTool[StepResult]):
    ...
    def __init__(self, enable_moderation: bool=True, enable_brand_recognition: bool=True, enable_accessibility: bool=True, enable_reverse_search: bool=False, vision_model: str='gpt-4-vision-preview', max_image_size: int=2048):
        super().__init__()
    ...
    def _run(self, image_path: str, tenant: str='default', workspace: str='default', analysis_type: str='comprehensive') -> StepResult:
        """
        ...
        try:
            ...
            if tenant and workspace:
                self.note(f'Starting image analysis for {(os.path.basename(image_path) if not image_path.startswith('http') else 'URL')}')
    ...
    def _load_image(self, image_path: str) -> dict[str, Any] | None:
        ...
        try:
            if image_path.startswith(('http://', 'https://')):
                ...
                image_source = 'file'
            if PIL is None:
                return {'raw_data': image_data, 'base64': base64.b64encode(image_data).decode('utf-8'), 'source': image_source, 'properties': {}}
            ...
            properties = {'width': image.size[0], 'height': image.size[1], 'mode': image.mode, 'format': getattr(image, 'format', 'JPEG'), 'size_bytes': len(image_data), 'aspect_ratio': image.size[0] / image.size[1] if image.size[1] > 0 else 1.0}
            if image.mode in ('RGB', 'RGBA'):
                stat = ImageStat.Stat(image)
                properties.update({'average_brightness': sum(stat.mean) / len(stat.mean), 'color_variance': sum(stat.var) / len(stat.var), 'is_grayscale': len(set(stat.mean)) == 1 if len(stat.mean) >= 3 else False})
            return {'image': image, 'raw_data': image_data, 'base64': base64.b64encode(image_data).decode('utf-8'), 'source': image_source, 'properties': properties}
        except Exception as e:
    ...
    def _perform_analysis(self, image_data: dict[str, Any], analysis_type: str) -> ImageAnalysisResult:
        """Perform comprehensive image analysis."""
        result: ImageAnalysisResult = {'image_properties': image_data.get('properties', {}), 'objects_detected': [], 'text_content': '', 'scene_classification': 'unknown', 'visual_sentiment': 'neutral', 'moderation': {'is_safe': True, 'confidence': 0.0}, 'brand_recognition': {'brands_detected': [], 'logos_detected': []}, 'accessibility': {'alt_text': '', 'detailed_description': ''}, 'quality_score': 0.0, 'metadata': {'analysis_type': analysis_type}}
    ...
    def _analyze_with_ai(self, image_data: dict[str, Any], analysis_type: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not base64_image:
                return None
            response = self.openai_client.chat.completions.create(model=self._vision_model, messages=[{'role': 'user', 'content': [{'type': 'text', 'text': prompt}, {'type': 'image_url', 'image_url': {'url': f'data:image/jpeg;base64,{base64_image}'}}]}], max_tokens=1500)
            ...
            try:
                ...
                return {'objects_detected': [], 'text_content': '', 'scene_classification': 'unknown', 'visual_sentiment': 'neutral', 'raw_response': response_text}
        except Exception as e:
            logging.error(f'AI image analysis failed: {e}')
            return None

    def _analyze_content_moderation(self, image_data: dict[str, Any], ai_analysis: dict[str, Any] | None) -> ModerationResult:
        ...
        try:
            if ai_analysis:
                ...
                confidence = 0.7
                return {'is_safe': is_safe, 'nsfw_score': min(1.0, nsfw_score), 'violence_score': min(1.0, violence_score), 'hate_score': min(1.0, hate_score), 'moderation_flags': flags, 'confidence': confidence}
            return {'is_safe': True, 'nsfw_score': 0.0, 'violence_score': 0.0, 'hate_score': 0.0, 'moderation_flags': [], 'confidence': 0.5}
        except Exception as e:
            logging.error(f'Content moderation analysis failed: {e}')
            return {'is_safe': True, 'nsfw_score': 0.0, 'violence_score': 0.0, 'hate_score': 0.0, 'moderation_flags': [], 'confidence': 0.0}

    def _analyze_brand_recognition(self, image_data: dict[str, Any], ai_analysis: dict[str, Any] | None) -> BrandRecognitionResult:
                ...
                for obj in objects:
                    if isinstance(obj, dict):
                        ...
                        if any((brand_word in obj_name for brand_word in brand_objects)):
                            brands_detected.append({'name': obj_name, 'confidence': obj.get('confidence', 0.5), 'location': obj.get('location', {})})
                common_brands = ['apple', 'google', 'microsoft', 'amazon', 'facebook', 'meta', 'twitter', 'instagram', 'youtube', 'netflix', 'uber', 'tesla']
                text_lower = text_content.lower()
                for brand in common_brands:
                    if brand in text_lower:
                        brands_detected.append({'name': brand, 'confidence': 0.8, 'detected_via': 'text'})
                        confidence_scores[brand] = 0.8
            return {'brands_detected': brands_detected, 'logos_detected': logos_detected, 'confidence_scores': confidence_scores}
        except Exception as e:
            logging.error(f'Brand recognition analysis failed: {e}')
            return {'brands_detected': [], 'logos_detected': [], 'confidence_scores': {}}

    def _generate_accessibility_features(self, image_data: dict[str, Any], ai_analysis: dict[str, Any] | None) -> AccessibilityResult:
        ...
        try:
            if ai_analysis:
                ...
                return {'alt_text': alt_text[:125], 'detailed_description': detailed_description, 'accessibility_issues': issues, 'color_contrast_score': contrast_score}
            return {'alt_text': 'Image content available', 'detailed_description': 'No detailed description available', 'accessibility_issues': ['AI analysis not available'], 'color_contrast_score': 0.5}
        except Exception as e:
            logging.error(f'Accessibility features generation failed: {e}')
            return {'alt_text': 'Image', 'detailed_description': '', 'accessibility_issues': ['Analysis failed'], 'color_contrast_score': 0.0}
    ...
    def run(self, image_path: str, tenant: str='default', workspace: str='default', analysis_type: str='comprehensive') -> StepResult:
