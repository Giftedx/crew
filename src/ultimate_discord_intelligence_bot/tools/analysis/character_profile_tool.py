...
class CharacterProfileTool(BaseTool[StepResult]):
    ...
    def __init__(self, storage_path: Path | None=None, leaderboard: LeaderboardTool | None=None, trust_tracker: TrustworthinessTrackerTool | None=None) -> None:
        super().__init__()
    ...
    def get_profile(self, person: str) -> _ProfileResult:
        ...
        trust: _ProfileTrustSection = {'truths': int(raw_trust.get('truths', 0)), 'lies': int(raw_trust.get('lies', 0)), 'score': float(raw_trust.get('score', 0.0))}
        return {'person': person, 'events': profile.get('events', []), 'leaderboard': {'lies': board.get('lies', 0), 'misquotes': board.get('misquotes', 0), 'misinfo': board.get('misinfo', 0)}, 'trust': trust}
