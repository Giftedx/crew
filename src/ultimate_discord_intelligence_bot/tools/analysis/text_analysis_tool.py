...
# Lightweight runtime placeholders (overridden lazily in _load_runtime)
nltk: Any | None = None
...
class TextAnalysisTool(BaseTool[StepResult]):
    ...
    def __init__(self) -> None:
        ...
        if not self._nltk_available:
            ...
            allow_degraded = os.getenv("ALLOW_NLTK_DEGRADED_MODE", "0").lower() in {
                "1",
                "true",
                "yes",
                "on",
    ...
    # @observe_tool_execution(tool_name="text_analysis")  # TODO: Fix decorator usage
        ...
        try:  # full analysis path (or degraded neutral path)
            ...
            try:
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "text_analysis", "outcome": "success"},
                ).inc()
            ...
            try:
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "text_analysis", "outcome": "error"},
                ).inc()
            except Exception as exc:  # pragma: no cover
