...
class TextAnalysisTool(BaseTool[StepResult]):
    ...
    def __init__(self) -> None:
        ...
        self._nltk_available = bool(nltk is not None and stopwords is not None and (SentimentIntensityAnalyzer is not object))
        self.sia = None
        ...
        if not self._nltk_available:
            ...
            if allow_degraded:
                logging.info('NLTK not available; TextAnalysisTool using heuristic fallback mode due to ALLOW_NLTK_DEGRADED_MODE=1. Install nltk for richer analysis.')
            else:
                logging.warning('NLTK not available and ALLOW_NLTK_DEGRADED_MODE not enabled; refusing to run in degraded mode.')
                raise RuntimeError('NLTK not available; set ALLOW_NLTK_DEGRADED_MODE=1 to use heuristic fallback or install nltk.')
    ...
    def _ensure_nltk_data(self) -> None:
        if os.getenv('NLTK_OFFLINE'):
            ...
            return
        resources = [('sentiment/vader_lexicon', 'vader_lexicon'), ('tokenizers/punkt', 'punkt'), ('tokenizers/punkt_tab', 'punkt_tab'), ('corpora/stopwords', 'stopwords')]
    ...
    def _run(self, text: str) -> StepResult:
        try:
            from ultimate_discord_intelligence_bot.models.structured_responses import AnalysisStatus, SentimentScore, TextAnalysisResult
            ...
            structured = TextAnalysisResult(status=AnalysisStatus.SUCCESS, sentiment=structured_sentiment, key_phrases=keywords[:10], word_count=word_count, language_detected='en', readability_score=readability).model_dump()
            out = {'sentiment': sentiment_label, 'sentiment_score': sentiment_score, 'keywords': keywords[:10], 'key_phrases': keywords[:10], 'word_count': word_count, 'language_detected': 'en', 'readability_score': readability, 'sentiment_details': sentiment_raw, 'structured': structured, 'emotions': enhanced_analysis['emotions'], 'topics': enhanced_analysis['topics'], 'enhanced_insights': enhanced_analysis['enhanced_insights'], 'dominant_emotion': enhanced_analysis['emotions']['dominant_emotion'], 'emotion_intensity': enhanced_analysis['emotions']['emotion_intensity'], 'primary_topics': enhanced_analysis['topics']['primary_topics'], 'content_themes': enhanced_analysis['topics']['content_themes']}
            try:
    ...
    def get_sentiment(self, text: str) -> _SentimentResult:
        if not self._nltk_available or self.sia is None:
            pos_words = {'good', 'great', 'excellent', 'amazing', 'positive', 'love', 'like', 'enjoy', 'success', 'benefit'}
            neg_words = {'bad', 'terrible', 'awful', 'horrible', 'negative', 'hate', 'dislike', 'fail', 'problem', 'issue'}
            try:
            ...
            return {'neg': max(0.0, float(neg_hits / total)), 'neu': float(max(0.0, 1.0 - (pos_hits + neg_hits) / max(1, len(toks)))) if toks else 1.0, 'pos': max(0.0, float(pos_hits / total)), 'compound': float(compound)}
    ...
    def get_emotions(self, text: str) -> _EmotionResult:
        ...
        emotion_lexicons = {'joy': {'happy', 'excited', 'pleased', 'delighted', 'thrilled', 'ecstatic', 'cheerful', 'jubilant', 'elated', 'overjoyed', 'blissful', 'radiant', 'laughing', 'smiling', 'grinning', 'beaming', 'glowing', 'exuberant'}, 'anger': {'angry', 'furious', 'irate', 'enraged', 'livid', 'incensed', 'outraged', 'infuriated', 'irritated', 'annoyed', 'frustrated', 'aggravated', 'exasperated', 'provoked', 'hostile', 'aggressive'}, 'fear': {'scared', 'afraid', 'frightened', 'terrified', 'panicked', 'alarmed', 'anxious', 'worried', 'nervous', 'uneasy', 'apprehensive', 'dread', 'terror', 'horror', 'panic', 'alarm', 'fearful', 'timid'}, 'sadness': {'sad', 'depressed', 'unhappy', 'miserable', 'despairing', 'sorrowful', 'grieving', 'melancholy', 'heartbroken', 'devastated', 'disappointed', 'downhearted', 'dejected', 'despondent', 'hopeless', 'forlorn'}, 'surprise': {'surprised', 'shocked', 'astonished', 'amazed', 'stunned', 'astounded', 'dumbfounded', 'flabbergasted', 'speechless', 'incredulous', 'unexpected', 'sudden', 'startling', 'shocking', 'amazing'}, 'disgust': {'disgusted', 'repulsed', 'revolted', 'nauseated', 'sickened', 'appalled', 'horrified', 'outraged', 'repugnant', 'vile', 'loathsome', 'abhorrent', 'detestable', 'repulsive', 'nauseating', 'sickening'}, 'anticipation': {'anticipating', 'expecting', 'awaiting', 'looking forward', 'hopeful', 'eager', 'excited', 'impatient', 'anxious', 'expectant', 'pending', 'upcoming', 'forthcoming', 'imminent', 'prospective', 'awaited'}, 'trust': {'trust', 'believe', 'confident', 'reliable', 'dependable', 'faithful', 'loyal', 'honest', 'sincere', 'genuine', 'authentic', 'credible', 'trustworthy', 'reputable', 'honorable', 'integrity'}}
        try:
            words = word_tokenize(text.lower())
        except Exception:
            words = text.lower().split()
        ...
        return {'joy': emotion_scores.get('joy', 0.0), 'anger': emotion_scores.get('anger', 0.0), 'fear': emotion_scores.get('fear', 0.0), 'sadness': emotion_scores.get('sadness', 0.0), 'surprise': emotion_scores.get('surprise', 0.0), 'disgust': emotion_scores.get('disgust', 0.0), 'anticipation': emotion_scores.get('anticipation', 0.0), 'trust': emotion_scores.get('trust', 0.0), 'dominant_emotion': dominant_emotion, 'emotion_intensity': emotion_intensity}

    def get_topics(self, text: str) -> _TopicResult:
        """Extract topics and themes from text using enhanced analysis."""
        topic_categories = {'politics': {'government', 'policy', 'election', 'president', 'congress', 'senate', 'democrat', 'republican', 'legislation', 'law', 'regulation', 'vote', 'campaign', 'candidate', 'political', 'partisan', 'ideology'}, 'technology': {'technology', 'tech', 'software', 'hardware', 'computer', 'internet', 'digital', 'innovation', 'artificial intelligence', 'AI', 'machine learning', 'automation', 'cybersecurity', 'programming', 'development', 'app'}, 'business': {'business', 'company', 'corporate', 'industry', 'market', 'economy', 'finance', 'investment', 'stock', 'revenue', 'profit', 'growth', 'strategy', 'management', 'entrepreneurship', 'startup'}, 'science': {'science', 'research', 'study', 'experiment', 'discovery', 'theory', 'scientific', 'biology', 'chemistry', 'physics', 'mathematics', 'data', 'analysis', 'evidence', 'hypothesis', 'conclusion'}, 'health': {'health', 'medical', 'medicine', 'doctor', 'patient', 'treatment', 'disease', 'illness', 'wellness', 'fitness', 'nutrition', 'mental health', 'therapy', 'diagnosis', 'prevention', 'care', 'hospital'}, 'environment': {'environment', 'climate', 'sustainability', 'green', 'eco', 'nature', 'pollution', 'conservation', 'renewable', 'energy', 'carbon', 'emissions', 'ecosystem', 'biodiversity', 'wildlife', 'planet'}, 'education': {'education', 'school', 'university', 'college', 'student', 'teacher', 'learning', 'curriculum', 'academic', 'degree', 'knowledge', 'skill', 'training', 'development', 'instruction', 'pedagogy'}, 'entertainment': {'entertainment', 'movie', 'film', 'music', 'song', 'artist', 'celebrity', 'game', 'gaming', 'sport', 'athlete', 'performance', 'show', 'concert', 'festival', 'media', 'streaming'}}
        try:
            words = word_tokenize(text.lower())
        except Exception:
            words = text.lower().split()
        ...
        return {'primary_topics': primary_topics, 'topic_categories': list(topic_categories.keys()), 'topic_confidence': topic_confidence, 'content_themes': content_themes}
    ...
    def get_enhanced_sentiment_analysis(self, text: str) -> dict[str, Any]:
        ...
        enhanced_analysis = {'basic_sentiment': basic_sentiment, 'emotions': emotions, 'topics': topics, 'enhanced_insights': self._generate_enhanced_insights(basic_sentiment, emotions, topics)}
        return enhanced_analysis

    def _generate_enhanced_insights(self, sentiment: _SentimentResult, emotions: _EmotionResult, topics: _TopicResult) -> dict[str, Any]:
