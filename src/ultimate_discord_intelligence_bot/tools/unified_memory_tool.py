...
class UnifiedMemoryStoreInput(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional metadata for the content"
    )
    namespace: Optional[str] = Field(
        default=None, description="Optional namespace for the content"
    )
...
class UnifiedMemoryRetrieveInput(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional filters for the search"
    )
    limit: int = Field(default=10, description="Maximum number of results to return")
    min_confidence: float = Field(
        default=0.5, description="Minimum confidence threshold for results"
    )
...
class UnifiedContextBuildInput(BaseModel):
    ...
    query: Optional[str] = Field(default=None, description="Query for context building")
    current_context: Optional[Dict[str, Any]] = Field(
        default=None, description="Current context to build upon"
    )
    max_context_length: int = Field(
        default=4000, description="Maximum context length in tokens"
    )
    include_history: bool = Field(
        default=True, description="Include historical context"
    )
    include_related_content: bool = Field(
        default=True, description="Include related content"
    )
    include_creator_intelligence: bool = Field(
        default=True, description="Include creator intelligence"
    )
    include_fact_checking: bool = Field(
        default=True, description="Include fact-checking information"
    )
        ...
        **kwargs: Any,
        ...
        try:
            if not self._memory_service or not self._retrieval_engine:
                return "Error: Unified memory services not initialized"
            ...
            result = loop.run_until_complete(
                self._retrieval_engine.retrieve(retrieval_query, self._memory_service)
            )
            ...
            if metadata:
                ...
                response_parts.append(
                    f"- Sources queried: {metadata.get('total_sources', 'unknown')}"
                )
                response_parts.append(
                    f"- Average confidence: {metadata.get('avg_confidence', 0):.2f}"
                )
                response_parts.append(
                    f"- Average relevance: {metadata.get('avg_relevance', 0):.2f}"
                )
        ...
        **kwargs: Any,
        ...
        try:
            if not self._memory_service:
                return "Error: Unified memory service not initialized"
            ...
            if not result.success:
                return f"Storage failed: {result.error}"
            ...
            response = f"Successfully stored information in {len(stored_backends)}/{total_backends} backends:"
            response += f"\n- Backends: {', '.join(stored_backends)}"
            ...
            return f"Error during storage: {str(e)}"
        ...
        **kwargs: Any,
        ...
        try:
            if not self._memory_service or not self._context_builder:
                return "Error: Unified context services not initialized"
            ...
            if not result.success:
                return f"Context building failed: {result.error}"
            ...
            response_parts = []
            response_parts.append(
                f"Comprehensive context built for {agent_id} (Task: {task_type})"
            )
            response_parts.append("")
            ...
            response_parts.append(
                f"- Total segments: {metadata.get('segments_count', 0)}"
            )
            response_parts.append(f"- Total tokens: {unified_context.total_tokens}")
            response_parts.append(
                f"- Overall relevance: {unified_context.relevance_score:.2f}"
            )
            ...
            return f"Error during context building: {str(e)}"
