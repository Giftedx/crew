...
class SteelmanArgumentTool:
    ...
    def run(self, claim: str, evidence: list[dict] | None = None, context: str | None = None) -> StepResult:
        ...
        if not claim:
            self._metrics.counter("tool_runs_total", labels={"tool": "steelman_argument", "outcome": "skipped"}).inc()
            return StepResult.skip(
                message="No claim provided", argument="", evidence=[], platform="internal", command="steelman"
            )
        ...
        try:
            if not evidence:
                ...
                self._metrics.counter(
                    "tool_runs_total", labels={"tool": "steelman_argument", "outcome": "success"}
                ).inc()
            ...
            self._metrics.counter("tool_runs_total", labels={"tool": "steelman_argument", "outcome": "success"}).inc()
            return StepResult.ok(argument=argument, evidence=evidence, claim=claim)
        except Exception as e:  # pragma: no cover - defensive
            self._metrics.counter("tool_runs_total", labels={"tool": "steelman_argument", "outcome": "error"}).inc()
    ...
    def _generate_steelman(self, position: str, context: str | None) -> dict[str, Any]:
        ...
        steelman = {
            ...
            "assumptions": ["Assumes rational actors", "Assumes available information is accurate"],
            "confidence": 0.75,
        }
