...
class EscalationManagementTool(BaseTool):
    ...
    def _run(
        ...
        try:
            logger.info(
                f"Managing escalation for tenant '{tenant}', workspace '{workspace}'"
            )
    ...
    def _determine_escalation_path(self, incident: Incident) -> List[str]:
        ...
        return escalation_paths.get(incident.severity, {}).get(
            incident.category, ["SRE Team"]
        )

    def _generate_response_actions(
        self, incident: Incident, escalation_path: List[str]
    ) -> List[ResponseAction]:
    ...
    def _create_escalation_timeline(
        self, incident: Incident, response_actions: List[ResponseAction]
    ) -> Dict[str, Any]:
        ...
        timeline = {
            ...
            "incident_closed": {
                "timestamp": current_time
                + ((total_timeline_minutes + 60) * 60),  # +1 hour for closure
