...
from platform.http.http_utils import DEFAULT_RATE_LIMIT_RETRY, HTTP_RATE_LIMITED, HTTP_SUCCESS_NO_CONTENT, REQUEST_TIMEOUT_SECONDS, resilient_post, validate_public_https_url
...
class DiscordPostTool(BaseTool[StepResult]):
    ...
    def __init__(self, webhook_url: str | None=None):
        ...
        if not self.webhook_url:
            raise ValueError('Discord webhook URL is not configured. Please set DISCORD_WEBHOOK in your environment or config.')
    ...
    def _run(self, content_data: dict, drive_links: dict) -> StepResult:
        ...
        if file_size_bytes is None:
            ...
            try:
                ...
                file_size_bytes = None
        if not getattr(self, '_force_embeds', False) and file_size_bytes is not None and (file_size_bytes / (1024 * 1024) <= DISCORD_FILE_LIMIT_MB) and local_file_path:
    ...
    def _post_with_embed_links(self, content_data: dict, drive_links: dict) -> StepResult:
        ...
        skip_message = (str(drive_links.get('message')) if drive_skipped else '') if isinstance(drive_links, dict) else ''
        embed = {'title': content_data.get('title', 'New Content Available'), 'description': f'📹 **Platform**: {content_data.get('platform', 'Unknown')}\n👤 **Creator**: {content_data.get('uploader', 'Unknown')}\n⏱️ **Duration**: {content_data.get('duration', 'Unknown')}\n📊 **Size**: {content_data.get('file_size', 'Unknown')}', 'color': 65280, 'thumbnail': {'url': '' if drive_skipped else drive_links.get('thumbnail', '')}, 'fields': [{'name': 'i Drive Upload', 'value': skip_message or 'Drive upload skipped', 'inline': False}] if drive_skipped else [{'name': '🎥 Watch Online', 'value': f'[Google Drive Preview]({drive_links.get('preview_link', '')})', 'inline': True}, {'name': '💾 Download', 'value': f'[Direct Download]({drive_links.get('direct_link', '')})', 'inline': True}], 'footer': {'text': 'CrewAI Content Monitor', 'icon_url': 'https://example.com/crewai-icon.png'}, 'timestamp': default_utc_now().isoformat()}
    ...
    def _post_with_file_upload(self, content_data: dict, drive_links: dict) -> StepResult:
        ...
        with open(local_file_path, 'rb') as fh:
            files = {'file': (Path(local_file_path).name, fh, 'video/mp4'), 'payload_json': (None, json.dumps({'content': f'🎥 **{content_data.get('title', 'New Video')}**\nFrom: {content_data.get('uploader', 'Unknown')}\nPlatform: {content_data.get('platform', 'Unknown')}'}))}
            try:
    ...
    def _send_webhook(self, payload: dict) -> StepResult:
        ...
        try:
            response = resilient_post(self.webhook_url, json_payload=payload, headers={'Content-Type': 'application/json'}, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
    ...
    def _handle_response(self, response) -> StepResult:
        ...
        if response.status_code == HTTP_SUCCESS_NO_CONTENT:
            ...
            self._metrics.counter('tool_runs_total', labels={'tool': 'discord_post', 'outcome': 'error'}).inc()
            return StepResult.fail(error=f'HTTP error: {response.status_code}', data={'status_code': int(response.status_code)})
