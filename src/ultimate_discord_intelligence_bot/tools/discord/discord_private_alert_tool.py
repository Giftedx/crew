...
class DiscordPrivateAlertTool:
    ...
    def _run(self, message: str, metrics: dict | None=None, thresholds: dict | None=None) -> StepResult:
        if not self.webhook_url:
            self._metrics.counter('tool_runs_total', labels={'tool': 'discord_private_alert', 'outcome': 'skipped'}).inc()
        ...
        if metrics:
            ...
            content = f'{content}\n``{metrics_str}``'
        try:
            response = resilient_post(self.webhook_url, json_payload={'content': content, 'username': 'Alert Bot', 'allowed_mentions': {'users': []}})
            if getattr(response, 'status_code', 0) in (200, 204):
                self._metrics.counter('tool_runs_total', labels={'tool': 'discord_private_alert', 'outcome': 'success'}).inc()
            ...
            return StepResult.fail(error=f'Discord API returned status {getattr(response, 'status_code', 'unknown')}', status_code=getattr(response, 'status_code', None), platform='Discord', command='POST webhook')
