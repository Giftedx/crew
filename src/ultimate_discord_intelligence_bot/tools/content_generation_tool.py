...
class ContentGenerationTool(BaseTool[StepResult]):
    ...
    def _run(
        ...
        try:
            # Input validation
            if not any([multimodal_analysis_data, video_analysis_data, audio_analysis_data, text_analysis_data]):
            ...
            combined_data = self._merge_analysis_data(
                multimodal_analysis_data, video_analysis_data, audio_analysis_data, text_analysis_data
            )
            ...
            return StepResult.fail(f"Content generation failed: {str(e)}")
    ...
    def _generate_perspective_analysis(
        ...
        if not self._enable_perspective_analysis:
            return {"perspectives": [], "viewpoint_diversity": 0.0, "balanced_summary": "Perspective analysis disabled"}
        ...
        viewpoint_diversity = len(set(p["viewpoint"] for p in perspectives)) / 3.0
    ...
    def _generate_accessibility_content(
        ...
        if not self._enable_accessibility_generation:
            return {"content_type": "accessibility", "content": "Accessibility content disabled"}
        ...
        return {
            ...
            "metadata": {"accessibility_compliant": True, "sections": len(content_parts)},
