...
_SAFE_REGISTRY: dict[str, tuple[str, list[str]]] = {
    "obs": ("mcp_server.obs_server", ["summarize_health", "get_counters", "recent_degradations"]),
    "http": ("mcp_server.http_server", ["http_get", "http_json_get"]),
    "ingest": (
        "mcp_server.ingest_server",
        ["extract_metadata", "list_channel_videos", "fetch_transcript_local", "summarize_subtitles"],
    ),
    "kg": ("mcp_server.kg_server", ["kg_query", "kg_timeline", "policy_keys"]),
    "router": ("mcp_server.routing_server", ["estimate_cost", "route_completion", "choose_embedding_model"]),
    "multimodal": (
    ...
    "memory": ("mcp_server.memory_server", ["vs_search", "vs_list_namespaces", "vs_samples"]),
    "a2a": ("mcp_server.a2a_bridge_server", ["a2a_call"]),
    "crewai": (
