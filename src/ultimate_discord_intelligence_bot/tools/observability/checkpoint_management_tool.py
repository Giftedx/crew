...
class CheckpointArgs(BaseModel):
    ...
    store_path: str | None = Field('crew_data/langgraph/checkpoints.meta.json', description='Path to metadata store tracking checkpoints')
...
class CheckpointManagementTool(BaseTool[dict]):
    ...
    def _run(self, action: Literal['list', 'load', 'delete'], thread_id: str | None=None, store_path: str | None=None) -> StepResult:
        meta_path = Path(store_path or 'crew_data/langgraph/checkpoints.meta.json')
        store = self._read_store(meta_path)
        if action == 'list':
            runs = [{'thread_id': tid, **{k: v for k, v in rec.items() if k != 'events'}} for tid, rec in store.items() if isinstance(rec, dict)]
