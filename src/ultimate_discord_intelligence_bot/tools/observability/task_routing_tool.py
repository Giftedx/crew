...
class TaskRoutingTool(BaseTool):
    ...
    def _run(self, workflow_execution: dict[str, Any], available_agents: list[dict[str, Any]], tenant: str, workspace: str) -> StepResult:
        """
        ...
        try:
            ...
            routing_report = {'routing_id': f'routing_{int(time.time())}_{tenant}_{workspace}', 'workflow_id': workflow_execution.get('id', 'unknown'), 'assignments': [assignment.__dict__ for assignment in assignments], 'routing_metrics': routing_metrics, 'agent_utilization': self._calculate_agent_utilization(assignments, agent_capabilities), 'load_balancing_score': self._calculate_load_balancing_score(assignments, agent_capabilities), 'created_at': self._get_current_timestamp(), 'tenant': tenant, 'workspace': workspace, 'status': 'routed', 'version': '1.0'}
    ...
    def _parse_workflow_tasks(self, workflow_execution: dict[str, Any]) -> list[TaskRequirement]:
        ...
        for task_data in workflow_tasks:
            task = TaskRequirement(task_id=task_data.get('id', f'task_{len(tasks)}'), name=task_data.get('name', 'Untitled Task'), required_capabilities=task_data.get('required_capabilities', []), priority=task_data.get('priority', 1), estimated_duration_minutes=task_data.get('estimated_duration_minutes', 60), complexity_score=task_data.get('complexity_score', 0.5), deadline_minutes=task_data.get('deadline_minutes'), resource_requirements=task_data.get('resource_requirements', {}))
            tasks.append(task)
        return tasks
    ...
    def _create_agent_capabilities(self, available_agents: list[dict[str, Any]]) -> list[AgentCapability]:
        ...
        for agent_data in available_agents:
            capability = AgentCapability(agent_id=agent_data.get('id', f'agent_{len(capabilities)}'), name=agent_data.get('name', 'Unknown Agent'), capabilities=agent_data.get('capabilities', []), current_load=agent_data.get('current_load', 0.0), max_concurrent_tasks=agent_data.get('max_concurrent_tasks', 5), average_task_duration_minutes=agent_data.get('average_task_duration_minutes', 60), success_rate=agent_data.get('success_rate', 0.9), cost_per_task=agent_data.get('cost_per_task', 0.1), latency_ms=agent_data.get('latency_ms', 1000), last_updated=current_time)
            capabilities.append(capability)
        return capabilities

    def _route_tasks_intelligently(self, tasks: list[TaskRequirement], agent_capabilities: list[AgentCapability], execution_order: list[str]) -> list[TaskAssignment]:
        ...
        for task in tasks:
            ...
            if suitable_agents:
                ...
                assignment = TaskAssignment(assignment_id=f'assign_{task.task_id}_{int(current_time)}', task_id=task.task_id, agent_id=best_agent.agent_id, assigned_at=current_time, estimated_start_time=current_time + best_agent.current_load * 60, estimated_completion_time=current_time + best_agent.current_load * 60 + task.estimated_duration_minutes * 60, priority=task.priority, routing_confidence=self._calculate_routing_confidence(task, best_agent), routing_reason=self._generate_routing_reason(task, best_agent))
    ...
    def _find_suitable_agents(self, task: TaskRequirement, agent_capabilities: list[AgentCapability]) -> list[AgentCapability]:
    ...
    def _calculate_agent_score(self, task: TaskRequirement, agent: AgentCapability) -> float:
        ...
        capability_factor = len(set(task.required_capabilities) & set(agent.capabilities)) / len(task.required_capabilities)
        priority_factor = 0.5 + task.priority * 0.1
        score = (load_factor * 0.3 + success_factor * 0.25 + latency_factor * 0.2 + cost_factor * 0.15 + capability_factor * 0.1) * priority_factor
        return score
    ...
    def _calculate_routing_metrics(self, assignments: list[TaskAssignment], agent_capabilities: list[AgentCapability]) -> dict[str, Any]:
        ...
        return {'total_assignments': total_assignments, 'average_confidence': average_confidence, 'routing_efficiency': routing_efficiency, 'high_confidence_assignments': sum((1 for a in assignments if a.routing_confidence > 0.8)), 'low_confidence_assignments': sum((1 for a in assignments if a.routing_confidence < 0.6))}

    def _calculate_agent_utilization(self, assignments: list[TaskAssignment], agent_capabilities: list[AgentCapability]) -> dict[str, float]:
    ...
    def _calculate_load_balancing_score(self, assignments: list[TaskAssignment], agent_capabilities: list[AgentCapability]) -> float:
