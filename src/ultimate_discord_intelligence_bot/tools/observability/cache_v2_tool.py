...
class CacheV2OperationInput(BaseModel):
    ...
    cache_name: str = Field(..., description="Logical cache name (e.g., 'llm', 'analysis', 'transcription')")
        ...
        **kwargs: Any,
        ...
        try:
            if not ENABLE_CACHE_V2:
                return StepResult.fail("ENABLE_CACHE_V2 is disabled. Use legacy cache tools.")
            ...
            if operation == "get":
                ...
                if value is None:
                    return StepResult.fail("Value required for set operation")

                deps_set = set(dependencies) if dependencies else None
                import asyncio

                result = asyncio.run(cache.set(namespace, cache_name, key, value, dependencies=deps_set))
                return result
            ...
            return StepResult.fail(f"Cache operation failed: {exc}")
