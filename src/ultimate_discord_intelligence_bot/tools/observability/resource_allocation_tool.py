...
class ResourceAllocationTool(BaseTool):
    ...
    def _run(self, plan: dict[str, Any], available_resources: dict[str, Any], tenant: str, workspace: str) -> StepResult:
        """
        ...
        try:
            ...
            allocation_report = {'allocation_id': f'alloc_{int(time.time())}_{tenant}_{workspace}', 'plan_id': plan.get('plan_id', 'unknown'), 'allocations': [alloc.__dict__ for alloc in allocations], 'allocation_summary': allocation_summary, 'load_balancing_metrics': [metric.__dict__ for metric in load_metrics], 'resource_utilization': self._calculate_resource_utilization(allocations, resource_pool), 'created_at': self._get_current_timestamp(), 'tenant': tenant, 'workspace': workspace, 'status': 'allocated', 'version': '1.0'}
    ...
    def _create_resource_pool(self, available_resources: dict[str, Any]) -> ResourcePool:
        ...
        return ResourcePool(cpu_units=available_resources.get('cpu_units', 100), memory_mb=available_resources.get('memory_mb', 8192), gpu_hours=available_resources.get('gpu_hours', 10.0), storage_gb=available_resources.get('storage_gb', 1000), network_bandwidth_mbps=available_resources.get('network_bandwidth_mbps', 1000))
    ...
    def _estimate_objective_requirements(self, objective: dict[str, Any]) -> dict[str, Any]:
        ...
        return {'objective_id': objective.get('id', 'unknown'), 'title': objective.get('title', ''), 'cpu_units': cpu_units, 'memory_mb': memory_mb, 'gpu_hours': gpu_hours, 'storage_gb': storage_gb, 'network_bandwidth_mbps': network_bandwidth_mbps, 'priority': objective.get('priority', 1), 'estimated_duration_hours': 1.0}
    ...
    def _analyze_load_balancing(self, requirements: list[dict[str, Any]]) -> list[LoadBalancingMetrics]:
        ...
        for i, agent_id in enumerate(agents):
            base_load = 0.2 + i * 0.1
            metrics.append(LoadBalancingMetrics(agent_id=agent_id, current_load=min(base_load, 0.9), cpu_utilization=base_load * 0.8, memory_utilization=base_load * 0.7, active_tasks=max(1, int(base_load * 10)), queued_tasks=max(0, int(base_load * 5)), average_task_duration_hours=1.0 + i * 0.2))
        return metrics

    def _allocate_resources_with_balancing(self, requirements: list[dict[str, Any]], resource_pool: ResourcePool, load_metrics: list[LoadBalancingMetrics]) -> list[ResourceAllocation]:
        ...
        for req in sorted_requirements:
            ...
            if self._check_resource_availability(req, resource_pool):
                allocation = ResourceAllocation(allocation_id=f'alloc_{req['objective_id']}_{int(current_timestamp)}', task_id=req['objective_id'], agent_id=best_agent, cpu_units=req['cpu_units'], memory_mb=req['memory_mb'], gpu_hours=req['gpu_hours'], storage_gb=req['storage_gb'], network_bandwidth_mbps=req['network_bandwidth_mbps'], priority=req['priority'], estimated_duration_hours=req['estimated_duration_hours'], allocated_at=current_timestamp)
    ...
    def _check_resource_availability(self, requirement: dict[str, Any], resource_pool: ResourcePool) -> bool:
        ...
        return requirement['cpu_units'] <= resource_pool.cpu_units and requirement['memory_mb'] <= resource_pool.memory_mb and (requirement['gpu_hours'] <= resource_pool.gpu_hours) and (requirement['storage_gb'] <= resource_pool.storage_gb) and (requirement['network_bandwidth_mbps'] <= resource_pool.network_bandwidth_mbps)
    ...
    def _update_load_metrics(self, allocation: ResourceAllocation, load_metrics: list[LoadBalancingMetrics]):
        ...
        for metric in load_metrics:
            if metric.agent_id == allocation.agent_id:
                load_increase = allocation.cpu_units / 100.0 * 0.3 + allocation.memory_mb / 8192.0 * 0.2 + allocation.gpu_hours / 10.0 * 0.5
                metric.current_load = min(1.0, metric.current_load + load_increase)
                metric.active_tasks += 1
                break

    def _calculate_allocation_summary(self, allocations: list[ResourceAllocation], resource_pool: ResourcePool) -> dict[str, Any]:
        """Calculate summary of resource allocations."""
        total_allocated = {'cpu_units': sum((alloc.cpu_units for alloc in allocations)), 'memory_mb': sum((alloc.memory_mb for alloc in allocations)), 'gpu_hours': sum((alloc.gpu_hours for alloc in allocations)), 'storage_gb': sum((alloc.storage_gb for alloc in allocations)), 'network_bandwidth_mbps': sum((alloc.network_bandwidth_mbps for alloc in allocations))}
        return {'total_allocations': len(allocations), 'total_allocated_resources': total_allocated, 'remaining_resources': {'cpu_units': resource_pool.cpu_units, 'memory_mb': resource_pool.memory_mb, 'gpu_hours': resource_pool.gpu_hours, 'storage_gb': resource_pool.storage_gb, 'network_bandwidth_mbps': resource_pool.network_bandwidth_mbps}, 'utilization_percentage': {'cpu_units': total_allocated['cpu_units'] / (total_allocated['cpu_units'] + resource_pool.cpu_units) * 100, 'memory_mb': total_allocated['memory_mb'] / (total_allocated['memory_mb'] + resource_pool.memory_mb) * 100, 'gpu_hours': total_allocated['gpu_hours'] / (total_allocated['gpu_hours'] + resource_pool.gpu_hours) * 100}}

    def _calculate_resource_utilization(self, allocations: list[ResourceAllocation], resource_pool: ResourcePool) -> dict[str, Any]:
        """Calculate detailed resource utilization metrics."""
        agent_utilization = {}
        for alloc in allocations:
            if alloc.agent_id not in agent_utilization:
                agent_utilization[alloc.agent_id] = {'cpu_units': 0, 'memory_mb': 0, 'gpu_hours': 0, 'storage_gb': 0, 'network_bandwidth_mbps': 0, 'task_count': 0}
        ...
        return {'by_agent': agent_utilization, 'overall_efficiency': self._calculate_overall_efficiency(allocations), 'load_distribution': self._calculate_load_distribution(allocations)}
