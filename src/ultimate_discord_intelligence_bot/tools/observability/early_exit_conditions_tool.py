...
class EarlyExitConditionsTool(BaseTool[dict]):
    ...
    description: str = 'Assesses processing confidence to enable early exit when sufficient information has been gathered'
    ...
    def run(self, input_data: dict) -> StepResult:
        ...
        try:
            ...
            confidence_metrics = self._calculate_confidence_metrics(transcript, title, partial_analysis, processing_stage)
            result = {'confidence_metrics': {'content_clarity': confidence_metrics.content_clarity, 'information_density': confidence_metrics.information_density, 'topic_coherence': confidence_metrics.topic_coherence, 'completeness_score': confidence_metrics.completeness_score, 'overall_confidence': confidence_metrics.overall_confidence}, 'exit_decision': {'should_exit_early': confidence_metrics.should_exit_early, 'exit_reason': confidence_metrics.exit_reason, 'confidence_threshold': self._get_confidence_threshold(processing_stage), 'stage': processing_stage}, 'recommendations': self._generate_exit_recommendations(confidence_metrics), 'estimated_savings': self._estimate_processing_savings(confidence_metrics, processing_stage)}
            return StepResult.ok(result=result)
        except Exception as e:
            return StepResult.fail(error=f'Early exit evaluation failed: {e!s}')

    def _calculate_confidence_metrics(self, transcript: str, title: str, partial_analysis: dict, processing_stage: str) -> ConfidenceMetrics:
        ...
        overall_confidence = self._calculate_overall_confidence(content_clarity, information_density, topic_coherence, completeness_score)
        should_exit_early, exit_reason = self._evaluate_exit_conditions(overall_confidence, processing_stage, partial_analysis)
        return ConfidenceMetrics(content_clarity=content_clarity, information_density=information_density, topic_coherence=topic_coherence, completeness_score=completeness_score, overall_confidence=overall_confidence, should_exit_early=should_exit_early, exit_reason=exit_reason)
    ...
    def _assess_content_clarity(self, transcript: str, title: str) -> float:
        ...
        if len(words) < 10:
            return 0.2
        clarity_score = 0.5
        clear_indicators = ['\\b(explain|definition|example|specifically|clearly|obviously)\\b', '\\b(first|second|third|next|then|finally|in conclusion)\\b', '\\b(because|therefore|however|although|for instance)\\b']
        for pattern in clear_indicators:
            matches = len(re.findall(pattern, combined_text))
            clarity_score += min(matches * 0.1, 0.2)
        unclear_indicators = ['\\b(um|uh|like|you know|I guess|maybe|sort of)\\b', "\\b(confusing|unclear|I don\\'t know|not sure)\\b"]
    ...
    def _assess_information_density(self, transcript: str) -> float:
        ...
        if len(words) < 10:
            return 0.1
        info_patterns = ['\\b(data|research|study|analysis|evidence|statistics)\\b', '\\b(important|significant|crucial|key|essential|critical)\\b', '\\b(process|method|technique|approach|strategy|solution)\\b', '\\b(result|conclusion|finding|discovery|insight|outcome)\\b']
        info_word_count = 0
        ...
        tech_patterns = ['\\b(algorithm|system|framework|architecture|implementation)\\b', '\\b(optimize|efficient|performance|scalable|robust)\\b']
        tech_count = 0
    ...
    def _assess_topic_coherence(self, transcript: str, partial_analysis: dict) -> float:
        ...
        transition_patterns = ['\\b(also|additionally|furthermore|moreover|in addition)\\b', '\\b(however|but|although|on the other hand|nevertheless)\\b', '\\b(moving on|next topic|speaking of|regarding|concerning)\\b']
        transition_count = 0
    ...
    def _assess_topic_repetition(self, transcript: str) -> float:
        ...
        words = transcript.lower().split()
        word_counts = {}
        common_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'this', 'that', 'these', 'those', 'i', 'you', 'he', 'she', 'it', 'we', 'they'}
        for word in words:
    ...
    def _assess_completeness(self, transcript: str, partial_analysis: dict, stage: str) -> float:
        ...
        completion_patterns = ['\\b(in conclusion|to summarize|in summary|finally|lastly)\\b', "\\b(that\\'s all|that\\'s it|end of|wrapping up|to conclude)\\b"]
    ...
    def _calculate_overall_confidence(self, clarity: float, density: float, coherence: float, completeness: float) -> float:
        """Calculate weighted overall confidence score."""
        weights = {'clarity': 0.3, 'density': 0.2, 'coherence': 0.2, 'completeness': 0.3}
        overall = clarity * weights['clarity'] + density * weights['density'] + coherence * weights['coherence'] + completeness * weights['completeness']
        return max(0.0, min(1.0, overall))

    def _evaluate_exit_conditions(self, confidence: float, stage: str, partial_analysis: dict) -> tuple[bool, str | None]:
    ...
    def _get_confidence_threshold(self, stage: str) -> float:
        ...
        thresholds = {'transcription': 0.8, 'initial': 0.75, 'analysis': 0.7, 'intermediate': 0.65, 'final': 0.6, 'complete': 0.5}
    ...
    def _estimate_processing_savings(self, metrics: ConfidenceMetrics, stage: str) -> dict:
        ...
        stage_savings = {'transcription': {'time': 15, 'cost': 10}, 'initial': {'time': 25, 'cost': 20}, 'analysis': {'time': 40, 'cost': 35}, 'intermediate': {'time': 60, 'cost': 50}, 'final': {'time': 15, 'cost': 10}}
        ...
        return {'time_saved_percent': time_saved, 'cost_saved_percent': cost_saved, 'estimated_speedup': round(speedup, 2)}