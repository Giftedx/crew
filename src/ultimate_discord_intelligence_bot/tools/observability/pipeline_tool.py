...
class PipelineTool(BaseTool[StepResult]):
    ...
    def __init__(self, pipeline: Any | None = None):
        ...
        if PipelineTool._circuit_breaker is None:
            ...
            logger.info(
                f"Initialized pipeline circuit breaker: failure_threshold={failure_threshold}, recovery_timeout={recovery_timeout}s, success_threshold={success_threshold}"
            )

    async def _run(
        self, url: str, quality: str = "1080p", *, tenant_id: str | None = None, workspace_id: str | None = None
    ) -> StepResult:
        ...
        if self._circuit_breaker:
            ...
            if not can_execute:
                ...
                self._metrics.counter(
                    "tool_runs_total", labels={"tool": "pipeline", "outcome": "circuit_breaker_open"}
                ).inc()
            ...
            for step_name in steps:
                ...
                if not step_result.success:
                    return StepResult.fail(
                        ...
                        data={"failed_step": step_name, "completed_steps": results, "error": step_result.error},
                    )
