...
class ContentTypeRoutingTool(BaseTool[dict]):
    ...
    def run(self, input_data: dict) -> StepResult:
        ...
        try:
            ...
            if not transcript:
                return StepResult.fail(error='No transcript provided for content classification', error_category=ErrorCategory.MISSING_REQUIRED_FIELD)
            llm_result = self._try_llm_classification(transcript, title, description)
            if llm_result is not None:
                self._metrics.counter('tool_runs_total', labels={'tool': self.name, 'method': 'llm_instructor', 'outcome': 'success'}).inc()
                return llm_result
            ...
            result = {'classification': {'primary_type': classification.primary_type, 'confidence': classification.confidence, 'secondary_types': classification.secondary_types}, 'routing': {'pipeline': classification.routing_decision, 'processing_flags': classification.processing_flags, 'estimated_speedup': routing_config.get('estimated_speedup', 1.0)}, 'recommendations': self._generate_recommendations(classification), 'analysis_method': 'pattern_matching'}
            self._metrics.counter('tool_runs_total', labels={'tool': self.name, 'method': 'pattern_matching', 'outcome': 'success'}).inc()
            return StepResult.ok(result=result)
        except Exception as e:
            self._metrics.counter('tool_runs_total', labels={'tool': self.name, 'method': 'unknown', 'outcome': 'error'}).inc()
    ...
    def _try_llm_classification(self, transcript: str, title: str, description: str) -> StepResult | None:
        ...
        try:
            ...
            response = client.chat.completions.create(model=self._config.openrouter_llm_model, response_model=ContentTypeClassification, messages=[{'role': 'system', 'content': 'You are an expert content classifier. Analyze content and provide structured classification results.'}, {'role': 'user', 'content': prompt}], max_tokens=1000, temperature=0.3)
            decision = response.parsed if hasattr(response, 'parsed') else response
            result = {'classification': {'primary_type': decision.primary_type.value, 'confidence': decision.confidence, 'secondary_types': [t.value for t in decision.secondary_types]}, 'routing': {'pipeline': decision.recommended_pipeline, 'processing_flags': decision.processing_flags, 'estimated_speedup': decision.estimated_processing_time / 100.0}, 'recommendations': decision.recommendations, 'analysis_method': 'llm_instructor', 'quality_score': decision.quality_score}
            return StepResult.ok(result=result)
        except Exception as e:
            logger.warning(f'LLM classification failed, falling back to pattern matching: {e}')
            self._metrics.counter('tool_runs_total', labels={'tool': self.name, 'method': 'llm_instructor', 'outcome': 'error'}).inc()
            return None
    ...
    def _classify_content(self, transcript: str, title: str, description: str, metadata: dict) -> ContentClassification:
        ...
        educational_indicators = ['\\b(learn|tutorial|explain|how\\s+to|lesson|course|teach|study)\\b', '\\b(university|school|professor|student|lecture|class)\\b', '\\b(definition|theory|concept|principle|method|technique)\\b']
        entertainment_indicators = ['\\b(funny|comedy|humor|joke|laugh|entertainment|fun)\\b', '\\b(movie|film|show|series|episode|actor|actress)\\b', '\\b(game|gaming|play|player|level|score)\\b']
        news_indicators = ['\\b(news|report|breaking|update|journalist|reporter)\\b', '\\b(politics|government|election|vote|policy|law)\\b', '\\b(economy|market|business|company|industry)\\b']
        tech_indicators = ['\\b(technology|software|programming|code|developer|engineer)\\b', '\\b(computer|internet|digital|cyber|online|web)\\b', '\\b(AI|artificial\\s+intelligence|machine\\s+learning|data)\\b']
        discussion_indicators = ['\\b(debate|discuss|opinion|argument|perspective|viewpoint)\\b', '\\b(interview|conversation|talk|dialogue|panel)\\b', '\\b(think|believe|feel|agree|disagree)\\b']
        ...
        if confidence < 0.1:
            ...
            confidence = 0.5
        secondary_types = [content_type for content_type, score in scores.items() if score > 0.5 and content_type != primary_type]
        ...
        return ContentClassification(primary_type=primary_type, confidence=confidence, secondary_types=secondary_types, routing_decision=routing_decision, processing_flags=processing_flags)

    def _map_type_to_pipeline(self, content_type: str) -> str:
        """Map content type to processing pipeline."""
        pipeline_mapping = {'educational': 'deep_analysis', 'technology': 'deep_analysis', 'news': 'fast_summary', 'entertainment': 'light_analysis', 'discussion': 'deep_analysis', 'general': 'standard_pipeline'}
    ...
    def _get_processing_flags(self, primary_type: str, secondary_types: list[str]) -> dict[str, bool]:
        ...
        flags = {'enable_deep_analysis': False, 'enable_fallacy_detection': False, 'enable_sentiment_analysis': False, 'enable_topic_extraction': False, 'skip_perspective_api': False, 'use_fast_transcription': False}
