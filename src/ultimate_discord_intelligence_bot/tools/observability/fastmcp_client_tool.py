...
class FastMCPClientTool(BaseTool[StepResult]):
    ...
    async def _call_mcp_tool_async(self, server_url: str, tool_name: str, arguments: dict[str, Any]) -> StepResult:
        ...
        if not _CLIENT_AVAILABLE:
            return StepResult.fail('FastMCP client not available. Install with: pip install fastmcp', step='client_check')
        ...
        try:
            async with Client(server_url) as client:
                ...
                if available_tools is not None and tool_name not in available_tools:
                    return StepResult.fail(f"Tool '{tool_name}' not available on server", step='tool_validation', metadata={'available_tools': available_tools})
            ...
            return StepResult.fail(f'MCP tool call failed: {e}', step='mcp_call', metadata={'server_url': server_url, 'tool_name': tool_name, 'arguments': arguments})
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(lambda: asyncio.run(self._call_mcp_tool_async(server_url, tool_name, arguments)))
                    result = future.result(timeout=30)
            else:
                result = loop.run_until_complete(self._call_mcp_tool_async(server_url, tool_name, arguments))
                with contextlib.suppress(Exception):
                    outcome = 'success' if result.success else 'error'
                    self._metrics.counter('tool_runs_total', labels={'tool': 'fastmcp_client', 'server': server_url.split('://')[0] if '://' in server_url else 'unknown', 'outcome': outcome}).inc()
            return result
        except Exception as e:
...
class MCPResourceTool(BaseTool[StepResult]):
    ...
    async def _read_mcp_resource_async(self, server_url: str, resource_uri: str) -> StepResult:
        ...
        try:
            async with Client(server_url) as client:
                try:
                    ...
                    if resource_uri not in available_uris:
                        return StepResult.fail(f"Resource '{resource_uri}' not available", step='resource_validation', metadata={'available_resources': available_uris})
                except Exception:
                    pass
            ...
            return StepResult.fail(f'MCP resource read failed: {e}', step='resource_read', metadata={'uri': resource_uri, 'server_url': server_url})
    ...
    def run(self, server_url: str, resource_uri: str) -> StepResult:
        ...
        try:
            ...
            if loop.is_running():
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(lambda: asyncio.run(self._read_mcp_resource_async(server_url, resource_uri)))
                    result = future.result(timeout=30)
            else:
                result = loop.run_until_complete(self._read_mcp_resource_async(server_url, resource_uri))
            with contextlib.suppress(Exception):
                outcome = 'success' if result.success else 'error'
                self._metrics.counter('tool_runs_total', labels={'tool': 'mcp_resource', 'server': server_url.split('://')[0] if '://' in server_url else 'unknown', 'outcome': outcome}).inc()
            return result
        except Exception as e:
