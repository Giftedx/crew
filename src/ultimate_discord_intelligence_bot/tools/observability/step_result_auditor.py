...
class StepResultAuditor:
    ...
    def _check_tool_file(self, file_path: Path) -> dict:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ImportFrom) and node.module and "step_result" in node.module:
                ...
                if "Tool" in node.name or node.name.endswith("Agent"):
                    ...
                    for item in node.body:
                        if (
                            ...
                            and not self._returns_stepresult(item)
                        ):
        ...
        for node in ast.walk(func_node):
            if isinstance(node, ast.Return) and node.value and isinstance(node.value, ast.Dict):
                ...
                for key in node.value.keys:
                    if isinstance(key, ast.Constant) and key.value in ["success", "error", "data"]:
                        return True
        return False
