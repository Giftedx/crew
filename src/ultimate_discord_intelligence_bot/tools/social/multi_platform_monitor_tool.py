...
class MultiPlatformMonitorTool(BaseTool[StepResult]):
    ...
    def _run(self, items: Iterable[dict[str, str]]) -> StepResult:
        ...
        try:
            ...
            self._metrics.counter('tool_runs_total', labels={'tool': 'multi_platform_monitor', 'outcome': 'success'}).inc()
            return StepResult.ok(new_items=new_items, data={'new_items': new_items})
        except Exception as e:
            self._metrics.counter('tool_runs_total', labels={'tool': 'multi_platform_monitor', 'outcome': 'error'}).inc()
