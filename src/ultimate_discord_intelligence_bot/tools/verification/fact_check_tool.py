...
class FactCheckTool:
    ...
    def run(self, claim: str, tenant: str='global', workspace: str='global') -> StepResult:
        ...
        backends: list[tuple[str, Callable[[str], list[dict]]]] = [('duckduckgo', self._search_duckduckgo), ('serply', self._search_serply), ('exa', self._search_exa), ('perplexity', self._search_perplexity), ('wolfram', self._search_wolfram)]
        for name, fn in backends:
            try:
                ...
                _logger.warning(f"FactCheckTool: Backend '{name}' failed (RequestException): {e} - possibly rate limited or unavailable")
                continue
                ...
                return StepResult.network_error(error=f"Backend '{name}' failed: {e!s}", context=context, claim=claim, failed_backend=name)
        _logger.info(f'FactCheckTool: Completed - {len(evidence)} total evidence items from {len(successful_backends)} backends. Successful: {successful_backends}, Failed: {failed_backends or 'None'}')
        self._metrics.counter('tool_runs_total', labels={'tool': 'fact_check', 'outcome': 'success'}).inc()
        return StepResult.ok(claim=claim, evidence=evidence, backends_used=successful_backends, backends_failed=failed_backends, evidence_count=len(evidence))
    ...
    def _search_duckduckgo(self, query: str) -> list[dict]:
        ...
        try:
            ...
            if data.get('Abstract'):
                evidence.append({'title': data.get('Heading', 'DuckDuckGo Answer'), 'url': data.get('AbstractURL', ''), 'snippet': data.get('Abstract', '')})
            for topic in data.get('RelatedTopics', [])[:3]:
                if isinstance(topic, dict) and topic.get('Text'):
                    evidence.append({'title': topic.get('Text', '')[:100], 'url': topic.get('FirstURL', ''), 'snippet': topic.get('Text', '')})
            return evidence
        except Exception:
    ...
    def _search_serply(self, query: str) -> list[dict]:
        ...
        try:
            ...
            for result in data.get('organic_results', [])[:5]:
                evidence.append({'title': result.get('title', ''), 'url': result.get('link', ''), 'snippet': result.get('snippet', '')})
            return evidence
        except Exception:
    ...
    def _search_exa(self, query: str) -> list[dict]:
        ...
        try:
            ...
            for result in data.get('results', [])[:5]:
                evidence.append({'title': result.get('title', ''), 'url': result.get('url', ''), 'snippet': result.get('text', '')[:300]})
            return evidence
        except Exception:
    ...
    def _search_perplexity(self, query: str) -> list[dict]:
        ...
        try:
            ...
            payload = {'model': 'llama-3.1-sonar-small-128k-online', 'messages': [{'role': 'user', 'content': f'Find evidence for or against this claim: {query}'}], 'max_tokens': 500, 'temperature': 0.2, 'return_citations': True}
        ...
        try:
            ...
            for pod in pods[:3]:
                ...
                for subpod in subpods[:1]:
                    ...
                    if text:
                        evidence.append({'title': title, 'url': f'https://www.wolframalpha.com/input/?i={query.replace(' ', '+')}', 'snippet': text[:300]})
            return evidence
        except Exception:
