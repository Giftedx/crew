...
class ContextVerificationTool(BaseTool[StepResult]):
    ...
    def _run(self, video_id: str, ts: float, clip_text: str | None=None) -> StepResult:
        try:
            ...
            if not context:
                self._metrics.counter('tool_runs_total', labels={'tool': 'context_verification', 'outcome': 'skipped'}).inc()
            ...
            self._metrics.counter('tool_runs_total', labels={'tool': 'context_verification', 'outcome': 'success'}).inc()
