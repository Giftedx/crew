...
class StrategicPlanningTool(BaseTool):
    ...
    description: str = (
        "Creates strategic plans for intelligence missions with execution phases, "
        "risk assessment, and success metrics. Handles complex multi-objective "
        "planning with dependency management and resource allocation."
    )
    ...
    def _run(
        ...
        try:
            logger.info(
                f"Creating strategic plan for tenant '{tenant}', workspace '{workspace}'"
            )
    ...
    def _create_strategic_objectives(
        self, objectives_data: List[Dict[str, Any]]
    ...
    def _create_execution_plan(
        ...
        # Sort objectives by priority first
        ...
        return {
            ...
            "estimated_duration_hours": len(execution_phases) * 2,  # Rough estimate
            "status": "planned",
            "created_at": self._get_current_timestamp(),
        }
    ...
    def _assess_risks(
        ...
        for objective in objectives:
            # Identify potential risks based on objective characteristics
            ...
            if (
                "real-time" in objective.title.lower()
                or "live" in objective.title.lower()
            ):
                risks.append(
                    RiskAssessment(
    ...
    def _define_success_metrics(
        ...
        metrics = {
            "overall_success_rate": 0.95,  # Target 95% completion rate
            "average_completion_time_hours": 0,  # Will be calculated
            "cost_efficiency_score": 0.90,  # Target 90% cost efficiency
            "quality_score": 0.92,  # Target 92% quality
        }

        # Add objective-specific metrics
        for obj in objectives:
            metrics[f"{obj.id}_completion_rate"] = 1.0  # Target 100% completion
            metrics[f"{obj.id}_time_to_completion_hours"] = 1.0  # Target 1 hour
            metrics[f"{obj.id}_cost_usd"] = 0.10  # Target $0.10 per objective
            metrics[f"{obj.id}_quality_score"] = 0.90  # Target 90% quality
