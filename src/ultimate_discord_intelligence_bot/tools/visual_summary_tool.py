...
class VisualSummaryTool(BaseTool[StepResult]):
    ...
    def _run(
        ...
        try:
            ...
            return StepResult.fail(f"Visual summary generation failed: {str(e)}")
    ...
    def _generate_visual_summary(self, frame_analysis_data: dict[str, Any], summary_type: str) -> VisualSummary:
        ...
        executive_summary = self._generate_executive_summary(
            key_visual_elements, dominant_themes, emotional_arc, duration, len(key_frames)
        )
    ...
    def _generate_thumbnail_recommendations(self, frame_analysis_data: dict[str, Any]) -> list[ThumbnailRecommendation]:
        ...
        for i, scored_frame in enumerate(scored_frames[: self._thumbnail_count]):
            ...
            recommendation: ThumbnailRecommendation = {
                ...
                "reasons": reasons,
                "alternative_frames": [
                    {"frame_id": alt["frame_id"], "timestamp": alt["timestamp"], "score": alt["overall_score"]}
    ...
    def _analyze_pacing(
        self, key_frames: list[dict[str, Any]], scene_transitions: list[dict[str, Any]], duration: float
    ) -> dict[str, Any]:
    ...
    def _generate_executive_summary(
        ...
        if emotional_arc:
            dominant_emotion = max(
                set(point["emotion"] for point in emotional_arc),
    ...
    def _identify_engagement_factors(
        ...
        # Visual variety
        unique_scenes = set(frame.get("scene_type", "unknown") for frame in key_frames)
