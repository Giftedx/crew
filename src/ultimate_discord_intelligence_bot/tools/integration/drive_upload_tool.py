...
class DriveUploadTool(BaseTool[StepResult]):
        ...
        if prefer_oauth and OAuthCredentials is not None and (build is not None):
            try:
                ...
                if os.path.exists(token_path):
                    ...
                    credentials = OAuthCredentials.from_authorized_user_info(token_data, scopes=['https://www.googleapis.com/auth/drive'])
                    try:
                        if credentials and credentials.expired and getattr(credentials, 'refresh_token', None) and GoogleRequest:
    ...
    def _setup_folder_structure(self) -> tuple[str, dict[str, str]]:
        ...
        subfolders = {'youtube': self._get_or_create_folder('YouTube_Videos', base_folder), 'instagram': self._get_or_create_folder('Instagram_Content', base_folder), 'processed': self._get_or_create_folder('Processed_Content', base_folder)}
    ...
    def _run(self, file_path: str, platform: str) -> StepResult:
        ...
        if not self.service or not self.base_folder_id:
            ...
            return StepResult.skip(message='Google Drive uploads disabled (no credentials configured)', file_path=file_path, platform=platform)
        try:
            ...
            request = self.service.files().create(body=file_metadata, media_body=media, fields='id,name,size,webViewLink')
            ...
            return StepResult.ok(data={'file_id': file_id, 'file_name': response.get('name'), 'file_size': response.get('size'), 'links': links})
            ...
            try:
                ...
                if is_quota and (status_code == 403 or status_code is None):
                    logging.warning('Google Drive quota unavailable for service account; skipping upload (status=%s)', status_code or 'unknown')
                    self._metrics.counter('tool_runs_total', labels={'tool': 'drive_upload', 'outcome': 'skipped'}).inc()
                    return StepResult.skip(message='Drive upload skipped: service account has no storage quota. Use a shared drive or OAuth domain-wide delegation.', status_code=403, reason=reason or 'quota', platform='GoogleDrive', command=f'upload {os.path.basename(file_path)}', file_path=file_path)
            except Exception:
                pass
            logging.exception('Drive upload failed')
            self._metrics.counter('tool_runs_total', labels={'tool': 'drive_upload', 'outcome': 'error'}).inc()
            return StepResult.fail(error=str(e), platform='GoogleDrive', command=f'upload {os.path.basename(file_path)}')
    ...
    def _generate_discord_links(self, file_id: str) -> dict[str, str]:
        ...
        return {'preview_link': f'https://drive.google.com/file/d/{file_id}/preview', 'direct_link': f'https://drive.google.com/uc?id={file_id}', 'view_link': f'https://drive.google.com/file/d/{file_id}/view', 'thumbnail': f'https://drive.google.com/thumbnail?id={file_id}&sz=w1920-h1080'}
