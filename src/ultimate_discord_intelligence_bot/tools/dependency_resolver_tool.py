...
class DependencyResolverTool(BaseTool):
    ...
    def _run(
        ...
        try:
            logger.info(
                f"Resolving dependencies for tenant '{tenant}', workspace '{workspace}'"
            )
            ...
            if task_deps.issubset(completed_tasks):
                ...
                if current_phase:
                    phase = ExecutionPhase(
                        ...
                        tasks=current_phase,
                        estimated_duration_minutes=self._estimate_phase_duration(
    ...
    def _estimate_phase_duration(
        self, tasks: List[str], dependencies: List[TaskDependency]
    ) -> int:
    ...
    def _identify_critical_path(
        self, phases: List[ExecutionPhase], tasks: List[Dict[str, Any]]
    ) -> List[str]:
            ...
            if phase.critical_path or len(phase.tasks) == 1:
                ...
                if phase_tasks:
                    highest_priority_task = max(
                        phase_tasks, key=lambda t: t.get("priority", 1)
                    )
    ...
    def _find_missing_dependencies(
        self, dependencies: List[TaskDependency], tasks: List[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _calculate_resolution_metrics(
        self, resolution: DependencyResolution
    ) -> Dict[str, Any]:
        ...
        parallel_phases = sum(
            1
            for phase in resolution.execution_phases
        ...
        return {
            ...
            "parallelization_ratio": parallel_phases / total_phases
            if total_phases > 0
            else 0.0,
    ...
    def _calculate_efficiency_metric(self, resolution: DependencyResolution) -> float:
        ...
        parallelization_score = len(resolution.parallelization_opportunities) / len(
            resolution.execution_phases
        )
        critical_path_score = 1.0 - (
            len(resolution.critical_path) / len(resolution.execution_order)
        )
