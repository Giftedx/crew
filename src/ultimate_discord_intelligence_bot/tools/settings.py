...
def get_max_file_size() -> int:
    ...
    return int(get_tool_setting('MAX_FILE_SIZE', '104857600'))
...
def get_cache_ttl() -> int:
    ...
    """
    warnings.warn("tools.settings.get_cache_ttl() is deprecated. Use core.cache.unified_config.get_cache_ttl('tool') instead.", DeprecationWarning, stacklevel=2)
    if _unified_get_cache_ttl:
        with contextlib.suppress(Exception):
            return int(_unified_get_cache_ttl('tool'))
    return int(get_tool_setting('CACHE_TTL', '3600'))
TOOL_CONFIG = {'download_timeout': get_download_timeout(), 'transcription_timeout': get_transcription_timeout(), 'analysis_timeout': get_analysis_timeout(), 'max_file_size': get_max_file_size(), 'enable_caching': get_enable_caching(), 'cache_ttl': get_cache_ttl()}