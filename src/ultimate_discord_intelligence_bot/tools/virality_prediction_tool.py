...
class ViralityPredictionTool(BaseTool[StepResult]):
    ...
    def _run(
        ...
        try:
            ...
            return StepResult.fail(f"Virality prediction failed: {str(e)}")
    ...
    def _assess_platform_optimization(self, content: dict[str, Any]) -> float:
        ...
        if platform in platform_optimizations:
            if content_type in platform_optimizations[platform]:
                platform_score += 0.3
    ...
    def _create_performance_trajectory(self, prediction: ViralPrediction) -> PerformanceTrajectory:
        ...
        time_points = [float(hour) for hour in range(0, self._prediction_horizon_hours + 1)]
