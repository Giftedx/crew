...
class TwitterAPITool(BaseTool[StepResult]):
    ...
    def _run(
        self, query: str, tenant: str, workspace: str, action: str = "search_tweets", max_results: int = 10
    ) -> StepResult:
        ...
        try:
            ...
            return self._handle_error(
                e, context=f"Twitter API operation failed: {action}", error_category=ErrorCategory.EXTERNAL_SERVICE
            )
    ...
    def _search_tweets(self, query: str, max_results: int) -> StepResult:
        ...
        try:
            tweets = self.api.search_recent_tweets(
                query=query, max_results=max_results, tweet_fields=["created_at", "author_id", "public_metrics"]
            )
            results = []
            if tweets.data:
            ...
            return self._handle_error(
                e, context=f"Failed to search tweets: {query}", error_category=ErrorCategory.EXTERNAL_SERVICE
            )
    ...
    def _get_tweet(self, tweet_id: str) -> StepResult:
        ...
        try:
            ...
            return self._handle_error(
                e, context=f"Failed to get tweet: {tweet_id}", error_category=ErrorCategory.EXTERNAL_SERVICE
            )
    ...
    def _get_user(self, username: str) -> StepResult:
        ...
        try:
            ...
            return self._handle_error(
                e, context=f"Failed to get user: {username}", error_category=ErrorCategory.EXTERNAL_SERVICE
            )
