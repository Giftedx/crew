    ...
    def _run(self, url: str, quality: str = "1080p", **kwargs: Any) -> StepResult:
        ...
        if not url:
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "multi_platform_download", "outcome": "skipped"},
            ).inc()
        ...
        try:
            ...
            if not dispatcher:
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "multi_platform_download", "outcome": "error"},
                ).inc()
            ...
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "multi_platform_download", "outcome": label},
            ).inc()
            ...
            if isinstance(step, StepResult):
                ...
                step.data.setdefault(
                    ...
                    step.data.get(
                        "platform",
                        dispatcher.__class__.__name__.replace("DownloadTool", ""),
                    ),
                )
            return step
