...
class YtDlpDownloadTool(BaseTool[StepResult]):
    ...
    # @observe_tool_execution(tool_name="yt_dlp_download")  # TODO: Fix decorator usage
    def _run(self, video_url: str, quality: str = "1080p") -> StepResult:
        ...
        def _success(**fields: Any) -> StepResult:
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "yt_dlp_download", "outcome": "success"},
            ).inc()
        ...
        def _error(message: str) -> StepResult:
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "yt_dlp_download", "outcome": "error"},
            ).inc()
        ...
        try:
            ...
            if not local_path:
                return _success(
                    **base_fields,
                    local_path="",
                    note="Metadata only (no file path in output)",
                )
