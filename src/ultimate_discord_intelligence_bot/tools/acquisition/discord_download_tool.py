...
def _resolve_download_dir(user_dir: Path | None) -> Path:
    ...
    if _resolve_download_dir.__dict__.get('_cached') is None:
        _resolve_download_dir._cached = Path(tempfile.mkdtemp(prefix='discord_dl_'))
    return _resolve_download_dir._cached
...
class DiscordDownloadTool(BaseTool[StepResult]):
    ...
    def run(self, attachment_url: str, filename: str | None=None, **kwargs) -> StepResult:
        if hasattr(attachment_url, 'attachment_url'):
            try:
                url_val = attachment_url.attachment_url
    ...
    def _run(self, attachment_url: str, filename: str | None=None) -> StepResult:
        ...
        try:
            ...
            command_str = f'requests.get {attachment_url}'
            ...
            if status_code != HTTP_OK:
                self._metrics.counter('tool_runs_total', labels={'tool': 'discord_download', 'outcome': 'error'}).inc()
                return StepResult.fail(error=f'Failed to download: HTTP {status_code}', data={'status_code': status_code}, command=command_str)
            if not filename:
            ...
            return StepResult.ok(file_path=str(file_path), local_path=str(file_path), size=file_path.stat().st_size, success=True, platform=self.platform, command=command_str)
