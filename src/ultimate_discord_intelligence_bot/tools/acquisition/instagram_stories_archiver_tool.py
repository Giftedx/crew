...
class InstagramStoriesArchiverTool(BaseTool[StepResult]):
    ...
    def _run(self, instagram_handles: list[str], tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            result = InstagramStoriesArchiveResult(timestamp=time.time(), handles_checked=instagram_handles, new_stories_found=len(new_stories), stories_archived=archive_results['archived'], stories_failed=archive_results['failed'], archive_urls=archive_results['archive_urls'], processing_time_seconds=processing_time, next_check_recommended=time.time() + 4 * 60 * 60, errors=archive_results['errors'])
    ...
    def _get_stories_for_handle(self, handle: str, tenant: str, workspace: str) -> list[StoryPost]:
        ...
        mock_stories = [StoryPost(story_id=f'{handle}_story_{int(current_time)}_1', creator_handle=handle, media_type='image', media_url=f'https://instagram.com/stories/{handle}/mock_image_1.jpg', caption='Check out this new product! #sponsored', timestamp=current_time - 3600, expires_at=current_time + 23 * 3600, view_count=1500, engagement_data={'likes': 50, 'replies': 10}), StoryPost(story_id=f'{handle}_story_{int(current_time)}_2', creator_handle=handle, media_type='video', media_url=f'https://instagram.com/stories/{handle}/mock_video_1.mp4', caption='Behind the scenes of our latest podcast recording', timestamp=current_time - 1800, expires_at=current_time + 23.5 * 3600, view_count=800, engagement_data={'likes': 30, 'replies': 5})]
        return mock_stories
    ...
    def _store_story_metadata(self, story: StoryPost, archive_url: str, tenant: str, workspace: str) -> None:
        ...
        try:
            {'story_id': story['story_id'], 'creator_handle': story['creator_handle'], 'media_type': story['media_type'], 'original_url': story['media_url'], 'archive_url': archive_url, 'caption': story['caption'], 'timestamp': story['timestamp'], 'expires_at': story['expires_at'], 'view_count': story['view_count'], 'engagement_data': story['engagement_data'], 'tenant': tenant, 'workspace': workspace, 'archived_at': time.time()}
    ...
    def _generate_visual_summary(self, story: StoryPost, archive_url: str, tenant: str, workspace: str) -> None:
        ...
        try:
            if story['media_type'] == 'image':
                visual_summary = {'detected_text': story['caption'] or '', 'objects': ['person', 'product', 'text_overlay'], 'scene_type': 'indoor', 'dominant_colors': ['#FF5733', '#33FF57'], 'face_count': 1}
            elif story['media_type'] == 'video':
                visual_summary = {'duration_seconds': 15, 'scene_changes': 3, 'detected_audio': True, 'dominant_objects': ['person', 'microphone', 'background'], 'motion_level': 'medium'}
            else:
                return
