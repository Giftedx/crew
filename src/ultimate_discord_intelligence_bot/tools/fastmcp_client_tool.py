...
class FastMCPClientTool(BaseTool[StepResult]):
    ...
    async def _call_mcp_tool_async(self, server_url: str, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        """Asynchronously call an MCP tool on an external server."""
        if not _CLIENT_AVAILABLE:
            raise RuntimeError("FastMCP client not available. Install with: pip install fastmcp")

        if not self._validate_server_url(server_url):
            raise ValueError(f"Server URL not allowed: {server_url}")
        ...
        try:
            async with Client(server_url) as client:
                ...
                return {"success": True, "result": result, "server_url": server_url, "tool_name": tool_name}
        except Exception as e:
            return {"error": str(e), "server_url": server_url, "tool_name": tool_name, "arguments": arguments}
    ...
    def run(self, server_url: str, tool_name: str, arguments: dict[str, Any] | None = None) -> StepResult:
        ...
        try:
            ...
            if "error" in result:
                return StepResult.fail(result["error"], step="mcp_call")
...
class MCPResourceTool(BaseTool[StepResult]):
    ...
    async def _read_mcp_resource_async(self, server_url: str, resource_uri: str) -> dict[str, Any]:
        ...
        try:
            async with Client(server_url) as client:
                ...
                return {"success": True, "resource": resource, "uri": resource_uri, "server_url": server_url}
        except Exception as e:
            return {"error": str(e), "uri": resource_uri, "server_url": server_url}
    ...
    def run(self, server_url: str, resource_uri: str) -> StepResult:
        ...
        try:
            ...
            if "error" in result:
                return StepResult.fail(result["error"], step="resource_read")
