...
class AdvancedPerformanceAnalyticsSystem:
    ...
    def _synthesize_results(
        ...
        try:
            ...
            # Priority recommendations across all components
            priority_recommendations = []

            # Critical analytics recommendations
    ...
    <div class="grid">
        <div class="metric-card">
            ...
            <p><strong>Health Score:</strong> {
                insights.get("component_results", {}).get("analytics", {}).get("health_score", 0):.2f}</p>
            <p><strong>Trends:</strong> {
                insights.get("component_results", {}).get("analytics", {}).get("trends_analyzed", 0)
            }</p>
        </div>
        <div class="metric-card">
            <h3>🔮 Predictive</h3>
            <p><strong>Reliability:</strong> {
                insights.get("component_results", {}).get("predictive", {}).get("reliability_score", 0):.2f}</p>
            <p><strong>Warnings:</strong> {
                insights.get("component_results", {}).get("predictive", {}).get("early_warnings", 0)
            }</p>
        </div>
        <div class="metric-card">
            <h3>⚙️ Optimization</h3>
            <p><strong>Success Rate:</strong> {
                insights.get("component_results", {}).get("optimization", {}).get("success_rate", 0):.1%}</p>
            <p><strong>Status:</strong> {
                insights.get("component_results", {}).get("optimization", {}).get("engine_status", "inactive").title()
            }</p>
        </div>
    </div>
