...
def _load_pricing() -> dict[str, float]:
    try:
        from platform import token_meter
...
def _estimate_cost_impl(model: str, input_tokens: int, output_tokens: int) -> dict:
    ...
    try:
        ...
        return {'usd': usd, 'breakdown': {'input_tokens': int(input_tokens), 'output_tokens': int(output_tokens), 'price_per_token': price}}
...
def _route_completion_impl(task: str, tokens_hint: dict | None=None) -> dict:
    ...
    try:
        ...
        return {'model': preferred, 'reason': reason, 'est_cost_usd': est, 'latency_class': latency_class, 'tokens': {'input': tokens_in, 'output': tokens_out}}
    except Exception as exc:
...
@routing_mcp.tool
def estimate_cost_tool(model: str, input_tokens: int, output_tokens: int) -> dict:
    return _estimate_cost_impl(model, input_tokens, output_tokens)

@routing_mcp.tool
def route_completion_tool(task: str, tokens_hint: dict | None=None) -> dict:
    return _route_completion_impl(task, tokens_hint)

@routing_mcp.tool
def choose_embedding_model_tool(dimensions_required: int | None=None) -> dict:
    return _choose_embedding_model_impl(dimensions_required)
__all__ = ['choose_embedding_model', 'choose_embedding_model_tool', 'estimate_cost', 'estimate_cost_tool', 'route_completion', 'route_completion_tool', 'routing_mcp']