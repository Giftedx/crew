...
_ALLOWED_HOSTS = {'youtube.com', 'www.youtube.com', 'm.youtube.com', 'youtu.be', 'twitch.tv', 'www.twitch.tv'}
...
def _extract_metadata_impl(url: str) -> dict:
    ...
    try:
        from ultimate_discord_intelligence_bot.tools.yt_dlp_download_tool import twitch_fetch_metadata, youtube_fetch_metadata
    except Exception as exc:
...
def _summarize_subtitles_impl(url: str, lang: str | None=None, max_chars: int=1000) -> dict:
    ...
    if host not in _ALLOWED_HOSTS:
        return {'error': f'unsupported_provider:{host}'}
    try:
        from ultimate_discord_intelligence_bot.tools.yt_dlp_download_tool import youtube_fetch_metadata
    except Exception as exc:
        return {'error': f'metadata_helpers_unavailable:{exc}'}
    try:
...
@ingest_mcp.tool
def summarize_subtitles_tool(url: str, lang: str | None=None, max_chars: int=1000) -> dict:
    return _summarize_subtitles_impl(url, lang=lang, max_chars=max_chars)

@ingest_mcp.tool
def list_channel_videos_tool(channel_url: str, limit: int=50) -> dict:
    return _list_channel_videos_impl(channel_url, limit=limit)

@ingest_mcp.tool
def fetch_transcript_local_tool(path: str, model: str='tiny', max_chars: int=10000) -> dict:
    return _fetch_transcript_local_impl(path, model=model, max_chars=max_chars)

@ingest_mcp.resource('ingest://providers')
def providers() -> dict:
    return {'youtube': {'metadata': True, 'channel_list': True}, 'twitch': {'metadata': True}, 'transcript': {'local_file_only': True}}
__all__ = ['extract_metadata', 'extract_metadata_tool', 'fetch_transcript_local', 'fetch_transcript_local_tool', 'ingest_mcp', 'list_channel_videos', 'list_channel_videos_tool', 'providers', 'summarize_subtitles', 'summarize_subtitles_tool']