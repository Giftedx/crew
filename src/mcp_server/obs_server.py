...
def _summarize_health_impl() -> dict:
    ...
    try:
        ...
        return {'status': status, 'prometheus_available': prom_ok, 'exposition_bytes': len(data) if isinstance(data, (bytes, bytearray)) else 0}
...
def _get_counters_impl(sample_bytes: int=1000) -> dict:
    ...
    try:
        ...
        return {'prometheus_available': prom_ok, 'scrape_bytes': len(text), 'sample': sample}
    except Exception as exc:
...
def _recent_degradations_impl(limit: int=50) -> dict:
    ...
    try:
        ...
        for e in events:
            try:
                out.append({'ts': float(getattr(e, 'ts_epoch', 0.0)), 'component': str(getattr(e, 'component', '')), 'event': str(getattr(e, 'event_type', '')), 'severity': str(getattr(e, 'severity', '')), 'detail': getattr(e, 'detail', None), 'added_latency_ms': getattr(e, 'added_latency_ms', None)})
            except Exception:
                continue
        return {'events': out}
    except Exception as exc:
...
@obs_mcp.tool
def summarize_health_tool() -> dict:
    return _summarize_health_impl()

@obs_mcp.tool
def get_counters_tool(sample_bytes: int=1000) -> dict:
    return _get_counters_impl(sample_bytes)

@obs_mcp.tool
def recent_degradations_tool(limit: int=50) -> dict:
    return _recent_degradations_impl(limit)
...
def degradations_recent() -> dict:
    ...
    try:
        ...
        return {'events': [], 'error': str(exc)}
__all__ = ['degradations_recent', 'get_counters', 'get_counters_tool', 'metrics_prom', 'obs_mcp', 'recent_degradations', 'recent_degradations_tool', 'summarize_health', 'summarize_health_tool']