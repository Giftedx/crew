                ...
                if not attr_name.startswith('_') and callable(getattr(crew, attr_name, None)):
                    try:
                        ...
                        if hasattr(agent_method, '__annotations__') or 'agent' in attr_name.lower():
                            agents_info.append({'name': attr_name, 'type': 'agent_method' if 'agent' in attr_name else 'method', 'available': True})
                    except Exception:
                        continue
        return {'crews': [{'name': 'UltimateDiscordIntelligenceBotCrew', 'type': 'intelligence_analysis', 'description': 'Multi-agent crew for content intelligence and analysis', 'agents_count': len([a for a in agents_info if a['type'] == 'agent_method']), 'execution_enabled': _is_execution_enabled()}], 'agents': agents_info[:10], 'total_methods': len(agents_info)}
...
def get_crew_status() -> dict[str, Any]:
    ...
    try:
        ...
        try:
            import os
            config_status = {'execution_enabled': _is_execution_enabled(), 'crew_max_rpm': os.getenv('CREW_MAX_RPM', '10'), 'crew_embedder_provider': os.getenv('CREW_EMBEDDER_PROVIDER', 'openai'), 'validation_enabled': os.getenv('ENABLE_CREW_CONFIG_VALIDATION', '0') == '1', 'step_verbose': os.getenv('ENABLE_CREW_STEP_VERBOSE', '0') == '1'}
        except Exception:
            config_status = {'error': 'Could not read configuration'}
        return {'crew_available': crew_available, 'active_executions': active_count, 'execution_ids': list(_active_executions.keys()), 'configuration': config_status, 'timestamp': time.time()}
    except Exception as e:
...
def execute_crew(inputs: dict[str, Any], crew_type: str='default') -> dict[str, Any]:
    ...
    if not _is_execution_enabled():
        return {'error': 'Crew execution disabled', 'hint': 'Set ENABLE_MCP_CREWAI_EXECUTION=1 to enable execution', 'status': 'disabled'}
    execution_id = _generate_execution_id()
    try:
        crew = _get_crew_instance()
        if crew is None:
            return {'error': 'CrewAI crew not available', 'execution_id': execution_id}
        _active_executions[execution_id] = {'start_time': time.time(), 'status': 'running', 'inputs': inputs, 'crew_type': crew_type}
        start_time = time.time()
        try:
            result = crew.crew().kickoff(inputs=inputs)
            execution_time = time.time() - start_time
            _active_executions[execution_id].update({'status': 'completed', 'end_time': time.time(), 'execution_time': execution_time, 'result_type': type(result).__name__})
            if hasattr(result, '__dict__'):
                result_data = {'type': type(result).__name__, 'attributes': {k: str(v) for k, v in result.__dict__.items() if not k.startswith('_')}, 'success': True}
            else:
                result_data = {'type': type(result).__name__, 'value': str(result), 'success': True}
            return {'execution_id': execution_id, 'status': 'completed', 'execution_time': execution_time, 'result': result_data, 'inputs': inputs, 'crew_type': crew_type}
        except Exception as exec_error:
            execution_time = time.time() - start_time
            _active_executions[execution_id].update({'status': 'failed', 'end_time': time.time(), 'execution_time': execution_time, 'error': str(exec_error)})
            return {'execution_id': execution_id, 'status': 'failed', 'error': str(exec_error), 'execution_time': execution_time, 'inputs': inputs, 'crew_type': crew_type}
    except Exception as e:
...
def get_agent_performance(agent_name: str | None=None) -> dict[str, Any]:
    ...
    try:
        if agent_name:
            return {'agent': agent_name, 'metrics': {'total_executions': 0, 'success_rate': 0.0, 'average_execution_time': 0.0, 'last_execution': None}, 'status': 'metrics_not_available', 'note': 'Performance tracking requires enhanced monitoring integration'}
        else:
            return {'summary': {'total_agents': len(list(_active_executions)), 'active_executions': len([e for e in _active_executions.values() if e.get('status') == 'running']), 'completed_executions': len([e for e in _active_executions.values() if e.get('status') == 'completed']), 'failed_executions': len([e for e in _active_executions.values() if e.get('status') == 'failed'])}, 'recent_executions': list(_active_executions.values())[-5:], 'note': 'Enhanced performance tracking requires monitoring integration'}
    except Exception as e:
...
def abort_crew_execution(execution_id: str) -> dict[str, Any]:
    ...
    if execution.get('status') != 'running':
        return {'error': 'Execution not running', 'execution_id': execution_id, 'current_status': execution.get('status')}
    execution.update({'status': 'aborted', 'end_time': time.time(), 'aborted_by': 'mcp_tool'})
    return {'execution_id': execution_id, 'status': 'aborted', 'message': 'Execution marked as aborted (note: actual cancellation requires crew integration)'}
...
def list_agents_resource() -> str:
    ...
    try:
        ...
        return json.dumps({'agents': agents, 'total': len(agents), 'crew_type': 'UltimateDiscordIntelligenceBotCrew'}, indent=2)
    except Exception as e:
...
def list_tasks_resource() -> str:
    ...
    try:
        ...
        return json.dumps({'tasks': tasks, 'total': len(tasks), 'crew_type': 'UltimateDiscordIntelligenceBotCrew'}, indent=2)
    except Exception as e:
