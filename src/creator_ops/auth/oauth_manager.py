...
class OAuthCredentials:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'access_token': self.access_token, 'refresh_token': self.refresh_token, 'expires_at': self.expires_at, 'token_type': self.token_type, 'scope': self.scope, 'platform': self.platform, 'user_id': self.user_id, 'metadata': self.metadata}
...
class YouTubeOAuthProvider:
    ...
    def get_auth_url(self, state: str, redirect_uri: str) -> str:
        ...
        params = {'client_id': self.client_id, 'redirect_uri': redirect_uri, 'response_type': 'code', 'scope': 'https://www.googleapis.com/auth/youtube.readonly', 'state': state, 'access_type': 'offline', 'prompt': 'consent'}
        query_string = '&'.join([f'{k}={v}' for k, v in params.items()])
        return f'{self.base_url}?{query_string}'

    def exchange_code(self, code: str, redirect_uri: str) -> OAuthCredentials:
        """Exchange authorization code for YouTube tokens."""
        data = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': redirect_uri}
        response = httpx.post(self.token_url, data=data)
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=token_data.get('refresh_token'), expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), scope=token_data.get('scope'), platform='youtube')
    ...
    def refresh_token(self, refresh_token: str) -> OAuthCredentials:
        ...
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=refresh_token, expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), platform='youtube')
...
class TwitchOAuthProvider:
    ...
    def get_auth_url(self, state: str, redirect_uri: str) -> str:
        ...
        params = {'client_id': self.client_id, 'redirect_uri': redirect_uri, 'response_type': 'code', 'scope': 'user:read:email', 'state': state}
        query_string = '&'.join([f'{k}={v}' for k, v in params.items()])
        return f'{self.base_url}?{query_string}'
    ...
    def exchange_code(self, code: str, redirect_uri: str) -> OAuthCredentials:
        ...
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=token_data.get('refresh_token'), expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), scope=token_data.get('scope'), platform='twitch')
    ...
    def refresh_token(self, refresh_token: str) -> OAuthCredentials:
        ...
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=refresh_token, expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), platform='twitch')
...
class TikTokOAuthProvider:
    ...
    def get_auth_url(self, state: str, redirect_uri: str) -> str:
        ...
        params = {'client_key': self.client_id, 'scope': 'user.info.basic', 'response_type': 'code', 'redirect_uri': redirect_uri, 'state': state, 'code_challenge': code_challenge, 'code_challenge_method': 'S256'}
        query_string = '&'.join([f'{k}={v}' for k, v in params.items()])
        return f'{self.base_url}?{query_string}'

    def exchange_code(self, code: str, redirect_uri: str) -> OAuthCredentials:
        """Exchange authorization code for TikTok tokens."""
        data = {'client_key': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': redirect_uri}
        response = httpx.post(self.token_url, data=data)
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=token_data.get('refresh_token'), expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), scope=token_data.get('scope'), platform='tiktok')
    ...
    def refresh_token(self, refresh_token: str) -> OAuthCredentials:
        ...
        response.raise_for_status()
        token_data = response.json()
        return OAuthCredentials(access_token=token_data['access_token'], refresh_token=refresh_token, expires_at=int(time.time()) + token_data.get('expires_in', 3600), token_type=token_data.get('token_type', 'Bearer'), platform='tiktok')
...
class OAuthManager:
    ...
    def get_platform_status(self, platform: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'platform': platform, 'has_credentials': has_credentials, 'is_expired': is_expired, 'expires_in': expires_in, 'provider_registered': True})
        except Exception as e:
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            status = {'registered_providers': len(self.providers), 'stored_credentials': len(self.credentials), 'platforms': list(self.providers.keys()), 'has_storage_backend': self._storage_backend is not None, 'timestamp': time.time()}
