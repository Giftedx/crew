...
class TikTokClient:
    ...
    def get_video_list(self, max_count: int=20, cursor: str | None=None, start_date: str | None=None, end_date: str | None=None) -> StepResult:
    ...
    def get_video_analytics(self, video_ids: list[str], start_date: str, end_date: str) -> StepResult:
        ...
        data = {'video_ids': video_ids, 'start_date': start_date, 'end_date': end_date}
    ...
    def search_videos(self, query: str, max_count: int=20, cursor: str | None=None) -> StepResult:
        ...
        data = {'query': query, 'max_count': min(max_count, 20)}
        if cursor:
            data['cursor'] = cursor
    ...
    def get_hashtag_info(self, hashtag_name: str, fields: list[str] | None=None) -> StepResult:
        ...
        data = {'hashtag_name': hashtag_name}
        if fields:
            data['fields'] = fields
    ...
    def get_hashtag_videos(self, hashtag_name: str, max_count: int=20, cursor: str | None=None) -> StepResult:
        ...
        data = {'hashtag_name': hashtag_name, 'max_count': min(max_count, 20)}
        if cursor:
            data['cursor'] = cursor
    ...
    def get_trending_hashtags(self, count: int=20, region: str='US') -> StepResult:
        ...
        data = {'count': min(count, 100), 'region': region}
    ...
    def get_comment_list(self, video_id: str, max_count: int=20, cursor: str | None=None) -> StepResult:
        ...
        data = {'video_id': video_id, 'max_count': min(max_count, 20)}
        if cursor:
            data['cursor'] = cursor
    ...
    def extract_video_id(self, url: str) -> str | None:
        ...
        import re
        patterns = ['tiktok\\.com/@[^/]+/video/(\\d+)', 'vm\\.tiktok\\.com/([^/?]+)', 'vt\\.tiktok\\.com/([^/?]+)']
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            return StepResult.ok(data={'service': 'tiktok', 'status': 'healthy', 'client_key_configured': bool(self.client_key), 'access_token_configured': bool(self.access_token)})
        except Exception as e:
