...
class TwitchClient:
    ...
    def __init__(self, client_id: str, access_token: str | None=None):
        ...
        self.headers = {'Client-ID': client_id, 'Content-Type': 'application/json'}
        if access_token:
    ...
    def get_videos(self, user_id: str, video_type: str='archive', max_results: int=20) -> StepResult:
        ...
        params = {'user_id': user_id, 'type': video_type, 'first': min(max_results, 100)}
    ...
    def get_top_games(self, max_results: int=20) -> StepResult:
        """Get top games."""
        params = {'first': min(max_results, 100)}
    ...
    def get_followed_channels(self, user_id: str, max_results: int=20) -> StepResult:
        ...
        params = {'user_id': user_id, 'first': min(max_results, 100)}
    ...
    def get_channel_followers(self, broadcaster_id: str, max_results: int=20) -> StepResult:
        ...
        params = {'broadcaster_id': broadcaster_id, 'first': min(max_results, 100)}
    ...
    def get_channel_analytics(self, broadcaster_id: str, started_at: str, ended_at: str) -> StepResult:
        ...
        params = {'broadcaster_id': broadcaster_id, 'started_at': started_at, 'ended_at': ended_at, 'type': 'overview_v2'}
    ...
    def extract_user_id_from_url(self, url: str) -> str | None:
        ...
        import re
        patterns = ['twitch\\.tv/([^/?]+)', 'twitch\\.tv/user/([^/?]+)']
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            ...
            if result.success:
                return StepResult.ok(data={'service': 'twitch', 'status': 'healthy', 'client_id_configured': bool(self.client_id), 'access_token_configured': bool(self.access_token)})
            else:
