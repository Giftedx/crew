...
class MetricsCollector:
    ...
    def _save_metrics(self):
        ...
        try:
            data = {'tool_metrics': {name: asdict(metrics) for name, metrics in self.tool_metrics.items()}, 'system_metrics': asdict(self.system_metrics), 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    def get_slowest_tools(self, limit: int=10) -> list[ToolMetrics]:
        ...
        return sorted([m for m in self.tool_metrics.values() if m.total_calls > 0], key=lambda x: x.average_execution_time, reverse=True)[:limit]

    def get_most_error_prone_tools(self, limit: int=10) -> list[ToolMetrics]:
        """Get tools with highest error rates."""
        return sorted([m for m in self.tool_metrics.values() if m.total_calls > 0], key=lambda x: x.failed_calls / x.total_calls, reverse=True)[:limit]

    def generate_metrics_report(self) -> dict[str, Any]:
        """Generate comprehensive metrics report."""
        self._update_system_metrics()
        return {'timestamp': datetime.now(timezone.utc).isoformat(), 'system_metrics': asdict(self.system_metrics), 'tool_count': len(self.tool_metrics), 'active_tools': len([m for m in self.tool_metrics.values() if m.total_calls > 0]), 'total_tool_calls': sum(m.total_calls for m in self.tool_metrics.values()), 'average_execution_time': sum(m.average_execution_time for m in self.tool_metrics.values()) / len(self.tool_metrics) if self.tool_metrics else 0, 'top_tools': [asdict(tool) for tool in self.get_top_tools(5)], 'slowest_tools': [asdict(tool) for tool in self.get_slowest_tools(5)], 'error_prone_tools': [asdict(tool) for tool in self.get_most_error_prone_tools(5)], 'tool_metrics': {name: asdict(metrics) for name, metrics in self.tool_metrics.items()}}
