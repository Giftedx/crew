...
class AnalyticsService:
    ...
    def get_system_health(self) -> StepResult:
        ...
        try:
            ...
            health = SystemHealth(overall_score=overall_score, status=status, components_healthy=healthy_count, total_components=len(components), recommendations=recommendations)
    ...
    def get_performance_metrics(self) -> StepResult:
        ...
        try:
            ...
            metrics = PerformanceMetrics(cache_hit_rate=cache_data.get('hit_rate', 0.0), avg_latency_ms=router_data.get('avg_latency_ms', 0.0), error_rate=router_data.get('error_rate', 0.0), total_requests=router_data.get('total_requests', 0), cost_savings=cache_data.get('cost_savings', 0.0))
    ...
    def record_agent_performance(self, agent_name: str, task_type: str, quality_score: float, response_time: float, tools_used: list[str] | None=None, error_occurred: bool=False, **context) -> StepResult:
        ...
        try:
            monitor.record_agent_interaction(agent_name=agent_name, task_type=task_type, tools_used=tools_used or [], tool_sequence=context.get('tool_sequence', []), response_quality=quality_score, response_time=response_time, user_feedback=context.get('user_feedback'), error_occurred=error_occurred, error_details=context.get('error_details'))
    ...
    def get_agent_performance_report(self, agent_name: str, days: int=30) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok({'agent_name': report.agent_name, 'overall_score': report.overall_score, 'reporting_period': report.reporting_period, 'metrics': [{'metric_name': m.metric_name, 'target_value': m.target_value, 'actual_value': m.actual_value, 'trend': m.trend, 'confidence': m.confidence} for m in report.metrics], 'recommendations': report.recommendations, 'training_suggestions': report.training_suggestions}, step='get_agent_performance_report')
        except Exception as exc:
    ...
    def get_comparative_agent_analysis(self, agent_names: list[str], days: int=30) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok({'total_agents': len(reports), 'average_score': avg_score, 'best_agent': {'name': best_agent, 'score': agent_scores[best_agent]}, 'worst_agent': {'name': worst_agent, 'score': agent_scores[worst_agent]}, 'agent_scores': agent_scores}, step='get_comparative_agent_analysis')
        except Exception as exc:
