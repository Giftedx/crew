    ...
    def evaluate(self, metrics: SystemHealthMetrics) -> bool:
        ...
        try:
            ...
            if not isinstance(value, int | float):
                ...
                return False
            comparisons = {'>': lambda v, t: v > t, '<': lambda v, t: v < t, '>=': lambda v, t: v >= t, '<=': lambda v, t: v <= t, '==': lambda v, t: v == t, '!=': lambda v, t: v != t}
...
class AlertManager:
    ...
    def _load_default_rules(self):
        ...
        self.alert_rules = [AlertRule(name='high_error_rate', metric_path='error_rate_percentage', comparison='>', threshold=5.0, level=AlertLevel.CRITICAL, description='Error rate exceeds 5%'), AlertRule(name='high_latency_p95', metric_path='response_latency_p95', comparison='>', threshold=2000.0, level=AlertLevel.WARNING, description='P95 latency exceeds 2 seconds'), AlertRule(name='critical_latency_p95', metric_path='response_latency_p95', comparison='>', threshold=5000.0, level=AlertLevel.CRITICAL, description='P95 latency exceeds 5 seconds'), AlertRule(name='low_success_rate', metric_path='success_rate_percentage', comparison='<', threshold=95.0, level=AlertLevel.WARNING, description='Success rate below 95%'), AlertRule(name='high_daily_cost', metric_path='daily_cost_total', comparison='>', threshold=100.0, level=AlertLevel.WARNING, description='Daily cost exceeds $100'), AlertRule(name='critical_daily_cost', metric_path='daily_cost_total', comparison='>', threshold=500.0, level=AlertLevel.CRITICAL, description='Daily cost exceeds $500'), AlertRule(name='low_cache_hit_rate', metric_path='cache_hit_rate_semantic', comparison='<', threshold=30.0, level=AlertLevel.INFO, description='Semantic cache hit rate below 30%'), AlertRule(name='high_memory_usage', metric_path='memory_usage_mb', comparison='>', threshold=8192.0, level=AlertLevel.WARNING, description='Memory usage exceeds 8GB'), AlertRule(name='high_db_connection_wait', metric_path='db_connection_wait_time', comparison='>', threshold=100.0, level=AlertLevel.WARNING, description='Database connection wait time exceeds 100ms'), AlertRule(name='high_queue_depth', metric_path='queue_depth', comparison='>', threshold=100, level=AlertLevel.WARNING, description='Queue depth exceeds 100 items'), AlertRule(name='high_api_retry_rate', metric_path='api_retry_rate', comparison='>', threshold=10.0, level=AlertLevel.WARNING, description='API retry rate exceeds 10%'), AlertRule(name='critical_db_query_latency', metric_path='db_query_latency_p95', comparison='>', threshold=1000.0, level=AlertLevel.CRITICAL, description='Database query P95 latency exceeds 1 second')]
    ...
    def evaluate_alerts(self, metrics: SystemHealthMetrics) -> list[Alert]:
        ...
        for rule in self.alert_rules:
            ...
            if rule.evaluate(metrics):
                alert = Alert(rule_name=rule.name, level=rule.level, message=rule.description, metrics_snapshot=asdict(metrics))
...
class EnhancedMonitoringSystem:
    ...
    def _setup_quality_gates(self):
        ...
        self.quality_gates = [PerformanceQualityGate('response_latency', 1000.0), CostQualityGate('cost_per_interaction', 0.02), ReliabilityQualityGate('success_rate', 98.0), DatabaseQualityGate('db_query_latency_p95', 500.0), QueueQualityGate('queue_depth', 50)]
    ...
    async def collect_metrics(self) -> SystemHealthMetrics:
        ...
        try:
            ...
            unique_users = 85
            metrics_data = SystemHealthMetrics(timestamp=current_time, response_latency_p50=response_latency_p50, response_latency_p95=response_latency_p95, response_latency_p99=response_latency_p99, throughput_requests_per_second=throughput_requests_per_second, error_rate_percentage=error_rate_percentage, success_rate_percentage=success_rate_percentage, cost_per_interaction=cost_per_interaction, daily_cost_total=daily_cost_total, cost_per_token=cost_per_token, api_call_efficiency=api_call_efficiency, cache_hit_rate_semantic=cache_hit_rate_semantic, cache_hit_rate_traditional=cache_hit_rate_traditional, cache_cost_savings=cache_cost_savings, db_connection_pool_size=db_connection_pool_size, db_active_connections=db_active_connections, db_query_latency_p95=db_query_latency_p95, db_connection_wait_time=db_connection_wait_time, queue_depth=queue_depth, queue_processing_rate=queue_processing_rate, queue_error_rate=queue_error_rate, api_rate_limit_usage=api_rate_limit_usage, api_throttling_events=api_throttling_events, api_retry_rate=api_retry_rate, user_satisfaction_score=user_satisfaction_score, content_relevance_score=content_relevance_score, hallucination_detection_rate=hallucination_detection_rate, memory_usage_mb=memory_usage_mb, cpu_usage_percentage=cpu_usage_percentage, active_connections=active_connections, active_tenants=active_tenants, messages_processed=messages_processed, unique_users=unique_users)
            return metrics_data
        except Exception as e:
            logger.error(f'Failed to collect real metrics: {e}')
            return SystemHealthMetrics(timestamp=time.time())

    def process_rl_feedback_once(self, batch_size: int | None=None, labels: dict[str, str] | None=None) -> dict[str, Any]:
        ...
        summary: dict[str, Any] = {'enabled': enabled, 'batch_size': resolved_batch_size, 'processed': 0, 'failed': 0, 'queue_depth': 0, 'status': 'disabled' if not enabled else 'idle'}
        try:
        ...
        logger.debug('RL feedback batch processed', extra={'rl_feedback': {'processed': processed, 'failed': failed, 'queue_depth': remaining_queue, 'latency_ms': summary.get('latency_ms'), 'status': summary['status']}})
        return summary
    ...
    async def evaluate_quality_gates(self, metrics: SystemHealthMetrics) -> dict[str, dict[str, Any]]:
        ...
        for gate in self.quality_gates:
            ...
            try:
                ...
                results[gate.name] = {'passed': False, 'score': 0.0, 'threshold': gate.threshold, 'message': f'Evaluation failed: {e}'}
        return results

    def determine_system_status(self, metrics: SystemHealthMetrics, quality_gates: dict) -> MonitoringStatus:
        """Determine overall system health status."""
        if metrics.error_rate_percentage > ALERT_ERROR_RATE_THRESHOLD or metrics.response_latency_p95 > ALERT_LATENCY_THRESHOLD_MS or metrics.success_rate_percentage < ALERT_SUCCESS_RATE_THRESHOLD:
        ...
        if metrics.error_rate_percentage > WARNING_ERROR_RATE_THRESHOLD or metrics.response_latency_p95 > WARNING_LATENCY_THRESHOLD_MS or len(failed_gates) > 0:
            return MonitoringStatus.DEGRADED
                ...
                try:
                    ...
                    try:
                        ...
                        if system_health_metric is not None:
                            health_score = 1.0 if self.monitoring_status == MonitoringStatus.HEALTHY else 0.5 if self.monitoring_status == MonitoringStatus.DEGRADED else 0.0
                            try:
    ...
    def get_current_status(self) -> dict[str, Any]:
        ...
        return {'status': self.monitoring_status.value, 'timestamp': latest_metrics.timestamp, 'metrics': asdict(latest_metrics), 'active_alerts': [{'rule': alert.rule_name, 'level': alert.level.value, 'message': alert.message, 'timestamp': alert.timestamp} for alert in active_alerts], 'alerts_count': len(active_alerts)}
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            active_alerts = self.alert_manager.get_active_alerts()
            return {'healthy': is_healthy, 'status': self.monitoring_status.value, 'timestamp': default_utc_now().isoformat(), 'metrics': asdict(current_metrics), 'quality_gates': quality_results, 'active_alerts_count': len(active_alerts), 'uptime_seconds': time.time() - (self.metrics_history[0].timestamp if self.metrics_history else time.time())}
        except Exception as e:
...
__all__ = ['Alert', 'AlertLevel', 'AlertRule', 'EnhancedMonitoringSystem', 'MonitoringStatus', 'QualityGate', 'SystemHealthMetrics', 'get_enhanced_monitoring', 'start_monitoring_system', 'stop_monitoring_system']
