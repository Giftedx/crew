...
class UnifiedMetricsCollector:
    ...
    async def collect_system_metric(self, name: str, value: int | float, metric_type: MetricType, category: MetricCategory, labels: dict[str, str] | None=None, description: str='', metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            metric = SystemMetric(name=name, value=value, metric_type=metric_type, category=category, labels=labels or {}, timestamp=datetime.now(timezone.utc), description=description, metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, **(metadata or {})})
            ...
            return StepResult.ok(data={'collected': True, 'metric_name': name, 'value': value, 'timestamp': metric.timestamp.isoformat()})
        except Exception as e:
            logger.error(f'Error collecting system metric: {e}')
            return StepResult.fail(f'System metric collection failed: {e!s}')

    async def collect_agent_metric(self, agent_id: str, agent_type: str, metric_name: str, value: int | float, metric_type: MetricType, category: MetricCategory, labels: dict[str, str] | None=None, description: str='', metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            if workspace_id is None:
                workspace_id = getattr(ctx, 'workspace_id', 'main') if ctx else 'main'
            metric = AgentMetric(agent_id=agent_id, agent_type=agent_type, metric_name=metric_name, value=value, metric_type=metric_type, category=category, labels=labels or {}, timestamp=datetime.now(timezone.utc), description=description, metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, **(metadata or {})})
            ...
            return StepResult.ok(data={'collected': True, 'agent_id': agent_id, 'metric_name': metric_name, 'value': value, 'timestamp': metric.timestamp.isoformat()})
        except Exception as e:
            logger.error(f'Error collecting agent metric: {e}')
            return StepResult.fail(f'Agent metric collection failed: {e!s}')

    async def collect_performance_metric(self, component: str, operation: str, duration_ms: float, success: bool, error_message: str | None=None, metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            if workspace_id is None:
                workspace_id = getattr(ctx, 'workspace_id', 'main') if ctx else 'main'
            perf_metric = PerformanceMetric(component=component, operation=operation, duration_ms=duration_ms, success=success, error_message=error_message, metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, **(metadata or {})})
            metric_key = f'performance:{component}:{operation}'
            if metric_key not in self._metrics_store:
                self._metrics_store[metric_key] = []
            self._metrics_store[metric_key].append(perf_metric)
            await self.collect_system_metric(name=f'{component}_{operation}_duration_ms', value=duration_ms, metric_type=MetricType.HISTOGRAM, category=MetricCategory.PERFORMANCE, labels={'component': component, 'operation': operation, 'success': str(success)}, description=f'Duration of {operation} in {component}', metadata={'error_message': error_message} if error_message else None, tenant_id=tenant_id, workspace_id=workspace_id)
            await self.collect_system_metric(name=f'{component}_{operation}_success_rate', value=1.0 if success else 0.0, metric_type=MetricType.GAUGE, category=MetricCategory.PERFORMANCE, labels={'component': component, 'operation': operation}, description=f'Success rate of {operation} in {component}', tenant_id=tenant_id, workspace_id=workspace_id)
            logger.debug(f'Performance metric collected: {component}:{operation} = {duration_ms}ms')
            return StepResult.ok(data={'collected': True, 'component': component, 'operation': operation, 'duration_ms': duration_ms, 'success': success, 'timestamp': perf_metric.timestamp.isoformat()})
        except Exception as e:
            logger.error(f'Error collecting performance metric: {e}')
            return StepResult.fail(f'Performance metric collection failed: {e!s}')

    async def collect_quality_metric(self, component: str, quality_type: str, score: float, threshold: float, passed: bool, metadata: dict[str, Any] | None=None, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            if workspace_id is None:
                workspace_id = getattr(ctx, 'workspace_id', 'main') if ctx else 'main'
            quality_metric = QualityMetric(component=component, quality_type=quality_type, score=score, threshold=threshold, passed=passed, metadata={'tenant_id': tenant_id, 'workspace_id': workspace_id, **(metadata or {})})
            metric_key = f'quality:{component}:{quality_type}'
            if metric_key not in self._metrics_store:
                self._metrics_store[metric_key] = []
            self._metrics_store[metric_key].append(quality_metric)
            await self.collect_system_metric(name=f'{component}_{quality_type}_score', value=score, metric_type=MetricType.GAUGE, category=MetricCategory.QUALITY, labels={'component': component, 'quality_type': quality_type, 'passed': str(passed)}, description=f'Quality score for {quality_type} in {component}', metadata={'threshold': threshold}, tenant_id=tenant_id, workspace_id=workspace_id)
            logger.debug(f'Quality metric collected: {component}:{quality_type} = {score}')
            return StepResult.ok(data={'collected': True, 'component': component, 'quality_type': quality_type, 'score': score, 'threshold': threshold, 'passed': passed, 'timestamp': quality_metric.timestamp.isoformat()})
        except Exception as e:
            logger.error(f'Error collecting quality metric: {e}')
            return StepResult.fail(f'Quality metric collection failed: {e!s}')

    async def get_metrics_summary(self, category: MetricCategory | None=None, component: str | None=None, agent_type: str | None=None, hours: int=24, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        ...
        try:
            ...
            summary = {'total_metrics': len(filtered_metrics), 'time_range': {'start': start_time.isoformat(), 'end': end_time.isoformat(), 'hours': hours}, 'categories': {}, 'components': {}, 'agent_types': {}, 'top_metrics': []}
    ...
    async def export_metrics(self, format_type: str='prometheus', category: MetricCategory | None=None, hours: int=1, tenant_id: str | None=None, workspace_id: str | None=None) -> StepResult:
        """Export metrics in specified format"""
        try:
            if not self._initialized:
                return StepResult.fail('Unified Metrics Collector not initialized')
            summary_result = await self.get_metrics_summary(category=category, hours=hours, tenant_id=tenant_id, workspace_id=workspace_id)
            ...
            return StepResult.ok(data={'format': format_type, 'exported_at': datetime.now(timezone.utc).isoformat(), 'data': exported_data, 'summary': summary_result.data})
        except Exception as e:
    ...
    async def _cleanup_old_metrics(self, metric_key: str) -> None:
        ...
        try:
            ...
            self._metrics_store[metric_key] = [metric for metric in self._metrics_store[metric_key] if metric.timestamp > cutoff_time]
        except Exception as e:
            logger.error(f'Error cleaning up old metrics: {e}')

    async def _export_prometheus_format(self, category: MetricCategory | None, hours: int, tenant_id: str | None, workspace_id: str | None) -> str:
    ...
    async def _export_json_format(self, category: MetricCategory | None, hours: int, tenant_id: str | None, workspace_id: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            for metric_key, metrics_list in self._metrics_store.items():
                for metric in metrics_list:
                    ...
                    if workspace_id and metric.metadata.get('workspace_id') != workspace_id:
                        continue
                    metric_dict = {'key': metric_key, 'timestamp': metric.timestamp.isoformat(), 'metadata': metric.metadata}
    ...
    async def _export_csv_format(self, category: MetricCategory | None, hours: int, tenant_id: str | None, workspace_id: str | None) -> str:
    ...
    def get_collector_status(self) -> dict[str, Any]:
        ...
        return {'initialized': self._initialized, 'total_metric_keys': len(self._metrics_store), 'total_metrics': sum(len(metrics) for metrics in self._metrics_store.values()), 'config': {'enable_collection': self.config.enable_collection, 'enable_real_time': self.config.enable_real_time, 'collection_interval': self.config.collection_interval, 'retention_days': self.config.retention_days, 'export_formats': self.config.export_formats}, 'last_collection_times': {show_key: time.isoformat() for show_key, time in self._last_collection_time.items()}}
