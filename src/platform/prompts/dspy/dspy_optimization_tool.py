...
SignatureName = Literal['DebateAnalysisSignature', 'FactCheckingSignature', 'ClaimExtractionSignature', 'SentimentAnalysisSignature', 'SummaryGenerationSignature']
...
class DSPyOptimizationSchema(BaseModel):
    ...
    optimization_level: Literal['light', 'medium', 'heavy'] | None = Field('medium', description='Optimization intensity for DSPy')
...
class DSPyOptimizationTool(BaseTool[dict]):
    ...
    def _import_signatures() -> dict[str, Any]:
        try:
            from ..services.dspy_components.signature import ClaimExtractionSignature, DebateAnalysisSignature, FactCheckingSignature, SentimentAnalysisSignature, SummaryGenerationSignature
            return {'DebateAnalysisSignature': DebateAnalysisSignature, 'FactCheckingSignature': FactCheckingSignature, 'ClaimExtractionSignature': ClaimExtractionSignature, 'SentimentAnalysisSignature': SentimentAnalysisSignature, 'SummaryGenerationSignature': SummaryGenerationSignature}
        except Exception:
            return {}
        ...
        for rec in raw if isinstance(raw, list) else []:
            try:
                ...
                if sig_name == 'DebateAnalysisSignature':
                    transcript = rec.get('transcript') or rec.get('context') or rec.get('content') or rec.get('text') or rec.get('question') or ''
    ...
    def _run(self, action: Literal['optimize', 'load', 'compare', 'metric'], signature: SignatureName | None=None, optimization_level: Literal['light', 'medium', 'heavy'] | None='medium', training_data_path: str | None=None, test_data_path: str | None=None, data_limit: int | None=50, agent_name: str | None=None, version: str | None=None, baseline_version: str | None=None) -> StepResult:
        ...
        try:
            with get_metrics().tool_timer(self.name):
                if action == 'optimize':
                    ...
                    res = optimizer.optimize_agent_prompt(agent_signature=sig_cls, training_examples=train, metric=metric, optimization_level=optimization_level or 'medium')
                    if not res.success:
                        return res
                    ...
                    return StepResult.ok(data={'optimized': True, 'saved_to': saved_to, 'agent': agent_name, 'version': version})
                if action == 'load':
                ...
                if action == 'compare':
                    if not all([agent_name, version, baseline_version, test_data_path]):
                        return StepResult.fail('agent_name, version, baseline_version, and test_data_path are required for compare')
                    optimizer = AgentOptimizer()
                    lr_opt = optimizer.load_optimized_module(agent_name=agent_name, version=version, signature=sig_cls)
                    lr_base = optimizer.load_optimized_module(agent_name=agent_name, version=baseline_version, signature=sig_cls)
                    ...
                    cmp_res = optimizer.compare_optimized_vs_baseline(optimized_module=lr_opt.data, baseline_module=lr_base.data, test_examples=test, metric=self._default_metric)
                    return cmp_res
                if action == 'metric':
                    ...
                    if agent_name and version:
                        module_res = optimizer.load_optimized_module(agent_name=agent_name, version=version, signature=sig_cls)
                    ...
                    if not test:
                        return StepResult.fail('No test examples loaded from test_data_path')
                    return optimizer.calculate_metric(module=module_res.data, examples=test, metric=self._default_metric)
