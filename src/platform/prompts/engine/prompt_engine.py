...
class PromptEngine:
    ...
    def enable_optimization(self, compression_config: CompressionConfig | None=None, optimization_config: OptimizationConfig | None=None) -> None:
        ...
        self._optimization_pipeline = OptimizationPipeline(compressor=PromptCompressor(compression_config), config=optimization_config)
    ...
    async def generate_optimized(self, template: str, variables: dict[str, Any], prompt_id: str | None=None, tenant: str='', workspace: str='') -> StepResult:
        ...
        try:
            ...
            if not self._compression_enabled or self._optimization_pipeline is None:
                return StepResult.ok(data={'prompt': base_prompt, 'optimized': False, 'metrics': {'optimization_disabled': True}})
            optimization_result = await self._optimization_pipeline.optimize_prompt(prompt=base_prompt, prompt_id=prompt_id, tenant=tenant, workspace=workspace)
            if optimization_result.success:
                return StepResult.ok(data={'prompt': optimization_result.data['optimized_prompt'], 'optimized': True, 'metrics': optimization_result.data['metrics']})
            else:
                logger = logging.getLogger(__name__)
                logger.warning('Prompt optimization failed, using original: %s', optimization_result.error)
                return StepResult.ok(data={'prompt': base_prompt, 'optimized': False, 'metrics': {'optimization_failed': True, 'error': optimization_result.error}})
    ...
    def optimise(self, prompt: str, target_token_reduction: float=0.0, max_tokens: int | None=None, *, model: str | None=None, force_enable: bool=False) -> StepResult:
        text, _ = self.optimise_with_metadata(prompt, model=model, target_token_reduction=target_token_reduction, max_tokens=max_tokens, force_enable=force_enable)
        return text

    def optimise_with_metadata(self, prompt: str, *, model: str | None=None, target_token_reduction: float=0.0, max_tokens: int | None=None, llmlingua_ratio: float | None=None, force_enable: bool=False) -> StepResult:
        ...
        with tracer.start_as_current_span('prompt.optimise') as span:
            ...
            settings_enabled = bool(getattr(settings, 'enable_prompt_compression', False)) or bool(getattr(settings, 'enable_prompt_compression_flag', False))
            ...
            metadata.update({'source_policy': source_policy, 'env_present': bool(env_present), 'env_enabled': bool(env_enabled), 'settings_enabled': bool(settings_enabled), 'enabled': bool(effective_enabled), 'forced': bool(force_enable), 'target_token_reduction': float(target_token_reduction), 'max_tokens': int(max_tokens) if max_tokens is not None else None})
            if span:
                try:
            ...
            metadata['stages'].append({'stage': 'basic', 'before_tokens': original_tokens, 'after_tokens': basic_tokens})
            ...
            if target_token_reduction > 0 and original_tokens > 0:
                ...
                if current_tokens > target_tokens:
                    ...
                    metadata['stages'].append({'stage': 'context_trim', 'before_tokens': current_tokens, 'after_tokens': trimmed_tokens, 'target_tokens': target_tokens})
            ...
            if max_tokens is not None:
                ...
                if current_tokens > max_tokens:
                    ...
                    metadata['stages'].append({'stage': 'emergency_truncation', 'before_tokens': current_tokens, 'after_tokens': emergency_tokens, 'max_tokens': max_tokens})
            ...
            llmlingua_shadow_enabled = bool(getattr(settings, 'enable_llmlingua_shadow', False)) or (bool(llmlingua_shadow_env) and llmlingua_shadow_env.lower() in truthy)
            ...
            if llmlingua_mode != 'disabled':
                ...
                if before_tokens >= max(1, min_tokens):
                    ratio_effective = llmlingua_ratio if llmlingua_ratio is not None else float(getattr(settings, 'llmlingua_target_ratio', target_token_reduction or 0.0) or 0.0)
                    ...
                    llmlingua_target_tokens = int(target_budget) if target_budget not in (None, '', 0) else int(before_tokens * (1.0 - ratio_effective))
                    if max_tokens is not None:
                    ...
                    compressed, details = compress_prompt_with_details(text, enabled=True, target_tokens=llmlingua_target_tokens, ratio=ratio_effective, extra_kwargs=extra_kwargs)
                    after_tokens_llm = self.count_tokens(compressed, model)
                    llmlingua_stage_meta: dict[str, Any] = {'stage': 'llmlingua', 'mode': llmlingua_mode, 'before_tokens': before_tokens, 'after_tokens': after_tokens_llm if llmlingua_mode == 'active' else before_tokens, 'shadow_after_tokens': after_tokens_llm if llmlingua_mode == 'shadow' else None, 'details': details}
                    metadata['stages'].append(llmlingua_stage_meta)
                    llmlingua_info.update({'applied': bool(details.get('applied')), 'details': details, 'target_tokens': llmlingua_target_tokens, 'ratio': ratio_effective, 'after_tokens': after_tokens_llm})
    ...
    def _apply_context_trimming(self, text: str, target_tokens: int, current_tokens: int) -> StepResult:
        ...
        for i, line in enumerate(lines):
            ...
            if len(stripped) > 50:
                value_score += 1
            if any(marker in stripped.lower() for marker in ['error', 'warning', 'important', 'note:', 'question:', 'answer:', 'context:', 'summary:', 'conclusion:', 'result:', 'output:']):
                value_score += 2
            if len(stripped) < 5:
                value_score -= 1
    ...
    def _apply_emergency_truncation(self, text: str, max_tokens: int) -> StepResult:
        ...
        while keep_start + keep_end > 0:
            ...
            if start_lines and end_lines:
                candidate_lines = [*start_lines, f'...[omitted {total_lines - keep_start - keep_end} lines]...', *end_lines]
            elif start_lines:
    ...
    def build_with_context(self, instruction: str, query: str, k: int=3, metadata: dict[str, Any] | None=None) -> StepResult:
