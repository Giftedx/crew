...
def _is_retry_enabled() -> bool:
    ...
    if legacy_flag and str(legacy_flag).lower() in ("1", "true", "yes", "on"):
        if date.today() > _HTTP_RETRY_LEGACY_REMOVAL:
            raise RuntimeError(
                "ENABLE_HTTP_RETRY exceeded deprecation window (removal after 2025-12-31). Remove this variable and set ENABLE_HTTP_RETRY if retries are desired."
            )
        warnings.warn(
    ...
    if _is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_post(
                u, json_payload=json_payload, headers=headers, files=files, timeout_seconds=timeout_seconds
            )
    ...
    if _is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_get(
                u, params=params, headers=headers, timeout_seconds=timeout_seconds, stream=stream
            )
        return http_request_with_retry(
