...
def _is_retry_enabled() -> bool:
    ...
    if legacy_flag and str(legacy_flag).lower() in ('1', 'true', 'yes', 'on'):
        if date.today() > _HTTP_RETRY_LEGACY_REMOVAL:
            raise RuntimeError('ENABLE_HTTP_RETRY exceeded deprecation window (removal after 2025-12-31). Remove this variable and set ENABLE_HTTP_RETRY if retries are desired.')
        warnings.warn('ENABLE_ANALYSIS_HTTP_RETRY is deprecated; use ENABLE_HTTP_RETRY instead (grace until 2025-12-31).', DeprecationWarning, stacklevel=2)
        if not _DEPRECATION_LOG_EMITTED.get('ENABLE_HTTP_RETRY'):
            logging.getLogger('deprecations').info(json.dumps({'event': 'deprecated_flag_used', 'flag': 'ENABLE_ANALYSIS_HTTP_RETRY', 'replacement': 'ENABLE_HTTP_RETRY', 'removal_date': str(_HTTP_RETRY_LEGACY_REMOVAL)}))
...
class _CircuitBreaker:
    ...
    def __init__(self, name: str, *, failure_threshold: int=5, recovery_timeout: float=60.0, expected_exception: type[BaseException]=requests.RequestException, success_threshold: int=3, half_open_max_calls: int=3) -> None:
        self.name = name
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        with self.lock:
            return {'name': self.name, 'state': self.state, 'failure_count': self.failure_count, 'success_count': self.success_count, 'half_open_call_count': self.half_open_call_count, 'last_failure_time': self.last_failure_time, 'failure_threshold': self.failure_threshold, 'recovery_timeout': self.recovery_timeout}
...
def _breaker_for(url: str) -> tuple[str, _CircuitBreaker]:
    ...
    if br is None:
        if 'openrouter' in host.lower():
            br = _CircuitBreaker(name=host, failure_threshold=10, recovery_timeout=120.0, success_threshold=5, half_open_max_calls=5)
        elif 'qdrant' in host.lower():
            br = _CircuitBreaker(name=host, failure_threshold=3, recovery_timeout=30.0, success_threshold=3, half_open_max_calls=2)
        else:
            br = _CircuitBreaker(name=host, failure_threshold=5, recovery_timeout=60.0, success_threshold=3, half_open_max_calls=3)
        _BREAKERS[host] = br
    return (host, br)

def http_request_with_retry(method: str, url: str, *, request_callable: Callable[..., Any], statuses_to_retry: tuple[int, ...]=(500, 502, 503, 504, HTTP_RATE_LIMITED), max_attempts: int=DEFAULT_HTTP_RETRY_ATTEMPTS, base_backoff: float=0.5, jitter: float=0.05, on_give_up: Callable[[Exception | None, int], None] | None=None, **call_kwargs: Any) -> Any:
    ...
    if is_circuit_breaker_enabled():
        ...
        if br_inst and (not br_inst._can_attempt()):
            _metrics.CIRCUIT_BREAKER_REQUESTS.labels(**_metrics.label_ctx(), circuit=br_host, result='short_circuit').inc()
        ...
        with tracer.start_as_current_span('http.retry_attempt') as span:
            ...
            try:
                ...
                if br_inst is not None:
                    ...
                    _metrics.CIRCUIT_BREAKER_REQUESTS.labels(**_metrics.label_ctx(), circuit=br_host, result='failure').inc()
            ...
            if is_circuit_breaker_enabled() and br_inst is not None:
                if status is None or int(status) >= 500:
                    ...
                    _metrics.CIRCUIT_BREAKER_REQUESTS.labels(**_metrics.label_ctx(), circuit=br_host, result='success').inc()
                state_val = 1.0 if br_inst.state == 'open' else 0.5 if br_inst.state == 'half_open' else 0.0
                _metrics.CIRCUIT_BREAKER_STATE.labels(**_metrics.label_ctx(), circuit=br_host).set(state_val)
...
def retrying_post(url: str, *, json_payload: Any | None=None, headers: dict[str, str] | None=None, files: dict[str, Any] | None=None, timeout_seconds: int=REQUEST_TIMEOUT_SECONDS, max_attempts: int | None=None, request_callable: Callable[..., Any] | None=None, **kwargs: Any):
    ...
    if _is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_post(u, json_payload=json_payload, headers=headers, files=files, timeout_seconds=timeout_seconds)
        return http_request_with_retry('POST', url, request_callable=request_callable, max_attempts=effective_max, **kwargs)
    return resilient_post(url, json_payload=json_payload, headers=headers, files=files, timeout_seconds=timeout_seconds)

def retrying_get(url: str, *, params: dict[str, Any] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int=REQUEST_TIMEOUT_SECONDS, stream: bool | None=None, max_attempts: int | None=None, request_callable: Callable[..., Any] | None=None, **kwargs: Any):
    effective_max = max_attempts if max_attempts is not None else resolve_retry_attempts()
    if _is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_get(u, params=params, headers=headers, timeout_seconds=timeout_seconds, stream=stream)
        return http_request_with_retry('GET', url, request_callable=request_callable, max_attempts=effective_max, **kwargs)
    return resilient_get(url, params=params, headers=headers, timeout_seconds=timeout_seconds, stream=stream)
...
__all__ = ['get_circuit_breaker_status', 'http_request_with_retry', 'is_circuit_breaker_enabled', 'is_retry_enabled', 'reset_circuit_breakers', 'retrying_get', 'retrying_post']
