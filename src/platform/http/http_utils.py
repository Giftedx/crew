...
from .config import DEFAULT_HTTP_RETRY_ATTEMPTS, DEFAULT_RATE_LIMIT_RETRY, HTTP_RATE_LIMITED, HTTP_SUCCESS_NO_CONTENT, REQUEST_TIMEOUT_SECONDS, get_request_timeout
from .validators import validate_public_https_url
from .retry import is_circuit_breaker_enabled, is_retry_enabled, _is_retry_enabled
...
def resilient_post(url: str, *, json_payload: object | None=None, headers: dict[str, str] | None=None, files: dict[str, object] | None=None, timeout_seconds: int | None=None, allow_legacy_timeout_fallback: bool=True, request_fn: object | None=None):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    return _rq.resilient_post(url, json_payload=json_payload, headers=headers, files=files, timeout_seconds=eff_timeout, allow_legacy_timeout_fallback=allow_legacy_timeout_fallback, request_fn=request_fn)

def resilient_get(url: str, *, params: dict[str, object] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int | None=None, allow_legacy_timeout_fallback: bool=True, request_fn: object | None=None, stream: bool | None=None):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    return _rq.resilient_get(url, params=params or None, headers=headers, timeout_seconds=eff_timeout, allow_legacy_timeout_fallback=allow_legacy_timeout_fallback, request_fn=request_fn, stream=stream)

def resilient_delete(url: str, *, params: dict[str, object] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int | None=None, allow_legacy_timeout_fallback: bool=True, request_fn: object | None=None):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    return _rq.resilient_delete(url, params=params or None, headers=headers, timeout_seconds=eff_timeout, allow_legacy_timeout_fallback=allow_legacy_timeout_fallback, request_fn=request_fn)

def http_request_with_retry(method: str, url: str, *, request_callable, statuses_to_retry: tuple[int, ...]=(500, 502, 503, 504, HTTP_RATE_LIMITED), max_attempts: int=DEFAULT_HTTP_RETRY_ATTEMPTS, base_backoff: float=0.5, jitter: float=0.05, on_give_up=None, **call_kwargs):
    ...
    if is_circuit_breaker_enabled():
        ...
        return _cb_retry(method, url, request_callable=request_callable, statuses_to_retry=statuses_to_retry, max_attempts=max_attempts, base_backoff=base_backoff, jitter=jitter, on_give_up=on_give_up, **call_kwargs)
    attempts = 0
    while True:
        attempts += 1
        try:
...
def retrying_post(url: str, *, json_payload: object | None=None, headers: dict[str, str] | None=None, files: dict[str, object] | None=None, timeout_seconds: int | None=None, max_attempts: int | None=None, request_callable=None, **kwargs):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    effective_max = max_attempts if max_attempts is not None else resolve_retry_attempts()
    if is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_post(u, json_payload=json_payload, headers=headers, files=files, timeout_seconds=eff_timeout)
        return http_request_with_retry('POST', url, request_callable=request_callable, max_attempts=effective_max, **kwargs)
    return resilient_post(url, json_payload=json_payload, headers=headers, files=files, timeout_seconds=eff_timeout)

def retrying_get(url: str, *, params: dict[str, object] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int | None=None, stream: bool | None=None, max_attempts: int | None=None, request_callable=None, **kwargs):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    effective_max = max_attempts if max_attempts is not None else resolve_retry_attempts()
    if is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_get(u, params=params, headers=headers, timeout_seconds=eff_timeout, stream=stream)
        return http_request_with_retry('GET', url, request_callable=request_callable, max_attempts=effective_max, **kwargs)
    return resilient_get(url, params=params, headers=headers, timeout_seconds=eff_timeout, stream=stream)

def retrying_delete(url: str, *, params: dict[str, object] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int | None=None, max_attempts: int | None=None, request_callable=None, **kwargs):
    eff_timeout = int(timeout_seconds if timeout_seconds is not None else get_request_timeout())
    effective_max = max_attempts if max_attempts is not None else resolve_retry_attempts()
    if is_retry_enabled():
        if request_callable is None:
            request_callable = lambda u, **__: resilient_delete(u, params=params, headers=headers, timeout_seconds=eff_timeout)
        return http_request_with_retry('DELETE', url, request_callable=request_callable, max_attempts=effective_max, **kwargs)
    return resilient_delete(url, params=params, headers=headers, timeout_seconds=eff_timeout)

def cached_get(url: str, *, params: dict[str, object] | None=None, headers: dict[str, str] | None=None, timeout_seconds: int | None=None, ttl_seconds: int | None=None):
    ...
    if not getattr(settings, 'enable_http_cache', False):
        return resilient_get(url, params=params, headers=headers, timeout_seconds=timeout_seconds or get_request_timeout(), stream=False)
...
__all__ = ['DEFAULT_HTTP_RETRY_ATTEMPTS', 'DEFAULT_RATE_LIMIT_RETRY', 'HTTP_RATE_LIMITED', 'HTTP_SUCCESS_NO_CONTENT', 'REQUEST_TIMEOUT_SECONDS', '_RedisCache', '_load_retry_config', 'cached_get', 'get_request_timeout', 'get_settings', 'http_request_with_retry', 'is_circuit_breaker_enabled', 'is_retry_enabled', 'resilient_delete', 'resilient_get', 'resilient_post', 'resolve_retry_attempts', 'retrying_delete', 'retrying_get', 'retrying_post', 'time', 'validate_public_https_url']