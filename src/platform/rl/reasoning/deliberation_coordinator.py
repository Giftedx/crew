...
class DeliberationCoordinator:
    ...
    def __init__(self, message_bus: AgentMessageBus, *, default_timeout: int=30, default_strategy: VoteAggregationStrategy=VoteAggregationStrategy.CONFIDENCE_WEIGHTED):
    ...
    async def request_deliberation(self, request: DeliberationRequest, *, tenant_id: str='default') -> ConsensusResult:
        ...
        logger.info(f"Starting deliberation {deliberation_id}: '{request.question}' (min_agents={request.min_agents}, timeout={request.timeout_seconds}s)")
        try:
            message = AgentMessage(type=MessageType.REQUEST_REVIEW, content=request.question, sender_agent_id=request.requesting_agent_id, target_agent_id=None, priority=MessagePriority.HIGH, metadata={'deliberation_id': deliberation_id, 'options': request.options, 'context': request.context, 'min_agents': request.min_agents, 'timeout_seconds': request.timeout_seconds})
            ...
            while True:
                ...
                if elapsed >= timeout:
                    logger.warning(f'Deliberation {deliberation_id}: timeout after {elapsed:.1f}s (received {current_votes}/{request.min_agents} votes)')
                    break
                await asyncio.sleep(0.5)
            votes = self._collected_votes.get(deliberation_id, [])
            if not votes:
                logger.warning(f'Deliberation {deliberation_id}: no votes received')
                return ConsensusResult(decision=None, confidence=0.0, votes=[], strategy=request.strategy, is_consensus=False, reasoning='No agent responses received within timeout')
            protocol = ConsensusProtocol(strategy=request.strategy)
            result = protocol.aggregate_votes(votes)
            logger.info(f'Deliberation {deliberation_id}: consensus={result.is_consensus}, decision={result.decision}, confidence={result.confidence:.2f}')
            self._metrics.counter('deliberations_completed_total', labels={'strategy': request.strategy.value, 'consensus': str(result.is_consensus).lower()}).inc()
            return result
        finally:
            self._active_deliberations.pop(deliberation_id, None)
            self._collected_votes.pop(deliberation_id, None)

    async def submit_vote(self, deliberation_id: str, agent_id: str, decision: Any, confidence: float, reasoning: str='', evidence: list[str] | None=None, metadata: dict[str, Any] | None=None) -> bool:
        ...
        if deliberation_id not in self._active_deliberations:
            ...
            return False
        vote = AgentVote(agent_id=agent_id, decision=decision, confidence=confidence, reasoning=reasoning, evidence=evidence or [], metadata=metadata or {})
        if deliberation_id not in self._collected_votes:
            self._collected_votes[deliberation_id] = []
        self._collected_votes[deliberation_id].append(vote)
        logger.debug(f"Vote recorded for deliberation {deliberation_id}: {agent_id} voted '{decision}' (confidence={confidence:.2f})")
    ...
    async def handle_vote_message(self, message: AgentMessage) -> None:
        ...
        await self.submit_vote(deliberation_id=deliberation_id, agent_id=message.sender_agent_id or 'unknown', decision=decision, confidence=confidence, reasoning=reasoning, evidence=evidence, metadata=metadata)
