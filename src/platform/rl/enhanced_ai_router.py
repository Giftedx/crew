...
class EnhancedAIRouter:
    ...
    def __init__(self, enhanced_openrouter_service: Any=None, performance_monitor: AgentPerformanceMonitor | None=None, learning_engine: LearningEngine | None=None):
    ...
    async def intelligent_route(self, prompt: str, task_type: str='general', optimization_target: str='balanced', model: str | None=None, provider_opts: dict[str, Any] | None=None, max_tokens: int=2048, temperature: float=0.8) -> dict[str, Any]:
        """Route request using performance-based intelligence."""
        start_time = asyncio.get_event_loop().time()
        try:
            available_models = self._get_task_models(task_type)
            if model is None:
                routing_decision = self.performance_router.select_optimal_model(task_type=task_type, available_models=available_models, optimization_target=optimization_target)
                ...
                logger.info(f'AI Router decision: {selected_model} for {task_type} (target: {optimization_target}, confidence: {routing_decision.confidence:.2f})')
            else:
            ...
            if self.openrouter_service:
                result = await self.openrouter_service.route_async(prompt=prompt, task_type=task_type, model=selected_model, provider_opts=provider_opts, max_tokens=max_tokens, temperature=temperature)
            else:
                result = {'status': 'success', 'model': selected_model, 'response': f'[Demo response from {selected_model} for {task_type} task]', 'tokens': 50, 'tokens_out': 30, 'cost': expected_cost}
            ...
            if success:
                ...
                self.performance_router.update_model_performance(model=selected_model, latency_ms=actual_latency_ms, cost=actual_cost, success=success, quality_score=quality_score)
            ...
            enhanced_result.update({'routing_decision': decision_reasoning, 'optimization_target': optimization_target, 'expected_latency_ms': expected_latency, 'actual_latency_ms': actual_latency_ms, 'performance_delta': actual_latency_ms - expected_latency, 'intelligence_used': True})
            self.route_history.append({'timestamp': start_time, 'task_type': task_type, 'optimization_target': optimization_target, 'selected_model': selected_model, 'reasoning': decision_reasoning, 'success': success, 'actual_latency_ms': actual_latency_ms, 'actual_cost': actual_cost})
            return enhanced_result
        except Exception as e:
            logger.error(f'Enhanced AI routing failed: {e}')
            if self.openrouter_service:
                fallback_result = cast('dict[str, Any]', await self.openrouter_service.route_async(prompt=prompt, task_type=task_type, model=model, provider_opts=provider_opts))
    ...
    def _assess_response_quality(self, response: str, task_type: str) -> float:
        ...
        coherence_score = 0.8
        task_bonus = 0.0
        if task_type == 'analysis' and any((word in response.lower() for word in ['analyze', 'evidence', 'conclusion'])) or (task_type == 'creative' and len(response) > 100):
            task_bonus = 0.1
        return min(1.0, length_score * 0.3 + coherence_score * 0.6 + task_bonus)

    def _calculate_reward(self, cost: float, latency_ms: float, quality_score: float, optimization_target: str) -> float:
        """Calculate reward for learning engine based on optimization target."""
        cost_score = max(0, (0.01 - cost) / 0.01)
        speed_score = max(0, (2000 - latency_ms) / 2000)
    ...
    def get_routing_analytics(self) -> dict[str, Any]:
        ...
        return {'total_routes': total_routes, 'success_rate': success_rate, 'avg_latency_ms': avg_latency, 'avg_cost': avg_cost, 'model_usage': model_usage, 'optimization_targets': target_usage, 'router_confidence': router_stats.get('recent_avg_confidence', 0), 'learning_data_points': router_stats.get('performance_data_points', 0)}

def create_enhanced_ai_router(enhanced_openrouter_service: Any=None, performance_monitor: AgentPerformanceMonitor | None=None, learning_engine: LearningEngine | None=None) -> EnhancedAIRouter:
...
if __name__ == '__main__':
    ...
    async def demo_enhanced_ai_routing() -> None:
        ...
        test_scenarios = [('Analyze this complex dataset for trends', 'analysis', 'quality'), ('Write a creative story', 'creative', 'quality'), ('Quick summary of this text', 'general', 'speed'), ('Cost-effective content analysis', 'general', 'cost')]
