...
class MetaLearningAggregator:
    ...
    def __init__(self, *, redis_client: Any=None, epsilon: float=1.0, enable_differential_privacy: bool=True, sync_interval_seconds: int=300):
    ...
    def aggregate_agent_routing_params(self, tenant_ids: Sequence[str] | None=None) -> GlobalParameters:
        ...
        try:
            ...
            avg_context_weights = {feature: total_weight / context_weights_count[feature] for feature, total_weight in context_weights_sum.items() if context_weights_count.get(feature, 0) > 0}
            avg_agent_priors = {agent_id: (agent_performance_sum[agent_id][0] / agent_performance_count[agent_id], agent_performance_sum[agent_id][1] / agent_performance_count[agent_id]) for agent_id in agent_performance_sum if agent_performance_count.get(agent_id, 0) > 0}
            global_params = GlobalParameters(agent_context_weights=avg_context_weights, agent_performance_priors=avg_agent_priors, aggregated_from_tenants=aggregated_tenants, last_sync=datetime.now(timezone.utc), total_observations=total_obs)
            self._store_global_params(global_params)
            self._metrics.counter('meta_learning_aggregations_total', labels={'type': 'agent_routing', 'tenants': str(len(aggregated_tenants))}).inc()
            logger.info(f'Aggregated agent routing params from {len(aggregated_tenants)} tenants ({total_obs} observations, {len(avg_context_weights)} context features, {len(avg_agent_priors)} agents)')
    ...
    def aggregate_model_routing_params(self, tenant_ids: Sequence[str] | None=None) -> GlobalParameters:
        ...
        try:
            ...
            avg_posteriors = {model_id: (model_posteriors_sum[model_id][0] / model_posteriors_count[model_id], model_posteriors_sum[model_id][1] / model_posteriors_count[model_id]) for model_id in model_posteriors_sum if model_posteriors_count.get(model_id, 0) > 0}
            global_params = GlobalParameters(model_posteriors=avg_posteriors, aggregated_from_tenants=aggregated_tenants, last_sync=datetime.now(timezone.utc))
            ...
            self._metrics.counter('meta_learning_aggregations_total', labels={'type': 'model_routing', 'tenants': str(len(aggregated_tenants))}).inc()
            logger.info(f'Aggregated model routing params from {len(aggregated_tenants)} tenants ({len(avg_posteriors)} models)')
    ...
    async def sync_all_params(self) -> GlobalParameters:
        ...
        try:
            ...
            global_params = GlobalParameters(agent_context_weights=agent_params.agent_context_weights, agent_performance_priors=agent_params.agent_performance_priors, model_posteriors=model_params.model_posteriors, aggregated_from_tenants=list(set((agent_params.aggregated_from_tenants or []) + (model_params.aggregated_from_tenants or []))), last_sync=datetime.now(timezone.utc), total_observations=agent_params.total_observations)
            self._store_global_params(global_params)
            self._metrics.counter('meta_learning_full_syncs_total').inc()
            logger.info(f'Full meta-learning sync complete: {len(global_params.aggregated_from_tenants or [])} tenants, {global_params.total_observations} observations')
            return global_params
        except Exception:
