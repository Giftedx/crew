...
class PerformanceBasedRouter:
    ...
    def _initialize_baseline_metrics(self) -> None:
        ...
        baseline_models = {'openai/gpt-4o-mini': ModelPerformanceMetrics(model_name='openai/gpt-4o-mini', avg_latency_ms=800.0, avg_cost=0.0015, success_rate=0.98, quality_score=0.85, total_requests=100), 'openai/gpt-4o': ModelPerformanceMetrics(model_name='openai/gpt-4o', avg_latency_ms=1500.0, avg_cost=0.005, success_rate=0.99, quality_score=0.95, total_requests=100), 'anthropic/claude-3-5-sonnet-20241022': ModelPerformanceMetrics(model_name='anthropic/claude-3-5-sonnet-20241022', avg_latency_ms=1200.0, avg_cost=0.003, success_rate=0.97, quality_score=0.92, total_requests=100), 'google/gemini-1.5-flash': ModelPerformanceMetrics(model_name='google/gemini-1.5-flash', avg_latency_ms=600.0, avg_cost=0.001, success_rate=0.95, quality_score=0.8, total_requests=100)}
        self.model_metrics.update(baseline_models)
        logger.info(f'Initialized baseline metrics for {len(baseline_models)} models')

    def update_model_performance(self, model: str, latency_ms: float, cost: float, success: bool, quality_score: float | None=None) -> None:
    ...
    def select_optimal_model(self, task_type: str, available_models: list[str], optimization_target: str='balanced') -> RoutingDecision:
        ...
        if not candidate_models:
            return RoutingDecision(selected_model=available_models[0], confidence=0.5, reasoning='No performance data available, using fallback', alternatives=available_models[1:], expected_latency_ms=1000.0, expected_cost=0.003, quality_prediction=0.8)
        model_scores = {}
        ...
        alternatives = sorted([m for m in candidate_models if m != best_model], key=lambda m: model_scores[m], reverse=True)[:3]
        decision = RoutingDecision(selected_model=best_model, confidence=confidence, reasoning=reasoning, alternatives=alternatives, expected_latency_ms=best_metrics.avg_latency_ms, expected_cost=best_metrics.avg_cost, quality_prediction=best_metrics.quality_score)
        self.routing_history.append(decision)
        logger.info(f'Selected {best_model} for {task_type} (confidence: {confidence:.2f})')
        return decision

    def _calculate_model_score(self, metrics: ModelPerformanceMetrics, optimization_target: str, task_type: str) -> float:
        ...
        score = speed_score * weights['speed'] * task_multipliers.get('speed', 1.0) + cost_score * weights['cost'] * task_multipliers.get('cost', 1.0) + quality_score * weights['quality'] * task_multipliers.get('quality', 1.0) + reliability_score * weights['reliability'] * task_multipliers.get('reliability', 1.0)
    ...
    def _get_task_multipliers(self, task_type: str) -> dict[str, float]:
        ...
        multipliers = {'analysis': {'quality': 1.2, 'speed': 0.8}, 'creative': {'quality': 1.3, 'cost': 0.9}, 'general': {'speed': 1.0, 'cost': 1.0, 'quality': 1.0, 'reliability': 1.0}, 'fast': {'speed': 1.5, 'cost': 0.8}, 'summary': {'speed': 1.2, 'cost': 1.1}}
        return multipliers.get(task_type, {})

    def _build_reasoning(self, model: str, metrics: ModelPerformanceMetrics, optimization_target: str, task_type: str) -> str:
    ...
    def get_routing_stats(self) -> dict[str, Any]:
        ...
        return {'total_decisions': total_decisions, 'recent_avg_confidence': avg_confidence, 'model_usage': model_usage, 'available_models': len(self.model_metrics), 'performance_data_points': sum((m.total_requests for m in self.model_metrics.values()))}
...
if __name__ == '__main__':
    ...
    async def demo_performance_routing() -> None:
        ...
        scenarios = [('analysis', ['openai/gpt-4o', 'anthropic/claude-3-5-sonnet-20241022'], 'quality'), ('general', ['openai/gpt-4o-mini', 'google/gemini-1.5-flash'], 'cost'), ('fast', ['google/gemini-1.5-flash', 'openai/gpt-4o-mini'], 'speed')]
