...
class VariantStats:
    ...
    def as_dict(self) -> dict[str, float]:
        return {'pulls': float(self.pulls), 'reward_sum': float(self.reward_sum), 'reward_mean': float(self.reward_mean), 'last_reward': float(self.last_reward), 'regret_sum': float(self.regret_sum)}
...
class Experiment:
    ...
    def allocate(self, allocation_key: str) -> str:
        ...
        for arm, boundary in cumulative:
            ...
            if r <= threshold:
                chosen = arm
                break
        logger.debug('Experiment allocation: experiment_id=%s, allocation_key=%s, phase=%s, chosen=%s, r=%f, variants=%s', self.experiment_id, allocation_key, self.phase, chosen, r, self.variants)
        if METRICS_AVAILABLE:
            try:
                ...
                ctx = current_tenant()
                if ctx:
                    labels = {'tenant': ctx.tenant_id, 'workspace': ctx.workspace_id, 'experiment_id': self.experiment_id, 'variant': chosen, 'phase': self.phase}
                    metrics.EXPERIMENT_VARIANT_ALLOCATIONS.labels(**labels).inc()
                    phase_labels = {'tenant': ctx.tenant_id, 'workspace': ctx.workspace_id, 'experiment_id': self.experiment_id}
    ...
    def record(self, arm: str, reward: float) -> None:
        ...
        logger.debug('Experiment reward recorded: experiment_id=%s, arm=%s, reward=%f, pulls=%d, reward_sum=%f, reward_mean=%f, regret_sum=%f', self.experiment_id, arm, reward, vs.pulls, vs.reward_sum, vs.reward_mean, vs.regret_sum)
        if METRICS_AVAILABLE:
            try:
                from platform.observability import metrics
                ...
                ctx = current_tenant()
                if ctx:
                    reward_labels = {'tenant': ctx.tenant_id, 'workspace': ctx.workspace_id, 'experiment_id': self.experiment_id, 'variant': arm}
        ...
        if self.phase == 'shadow' and self.auto_activate_after is not None and (self.stats[self.control].pulls >= self.auto_activate_after):
            old_phase = self.phase
            self.phase = 'active'
            logger.info('Experiment auto-activated: experiment_id=%s, phase=%s->%s, control_pulls=%d, threshold=%d', self.experiment_id, old_phase, self.phase, self.stats[self.control].pulls, self.auto_activate_after)

    def snapshot(self) -> dict[str, Any]:
        return {'experiment_id': self.experiment_id, 'control': self.control, 'variants': dict(self.variants), 'phase': self.phase, 'stats': {k: v.as_dict() for k, v in self.stats.items()}}
...
class ExperimentManager:
    ...
    def register(self, exp: Experiment) -> None:
        ...
        logger.info('Experiment registered: experiment_id=%s, control=%s, variants=%s, phase=%s, auto_activate_after=%s', exp.experiment_id, exp.control, exp.variants, exp.phase, exp.auto_activate_after)

    def exists(self, experiment_id: str) -> bool:
        return experiment_id in self._experiments

    def recommend(self, experiment_id: str, context: Mapping[str, Any], candidates: list[str], allocation_key_fn: Callable[[Mapping[str, Any]], str] | None=None) -> str:
        ...
        logger.debug('Experiment recommendation: experiment_id=%s, key=%s, chosen=%s, candidates=%s, context_keys=%s', experiment_id, key, chosen, candidates, list(context.keys()))
        return chosen
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        dashboard_summary = {'total_experiments': len(self._experiments), 'active_experiments': sum(1 for exp in self._experiments.values() if exp.phase == 'active'), 'shadow_experiments': sum(1 for exp in self._experiments.values() if exp.phase == 'shadow'), 'experiments': snapshot_data, 'timestamp': __import__('time').time()}
        logger.debug('Experiment snapshot generated: total=%d, active=%d, shadow=%d', dashboard_summary['total_experiments'], dashboard_summary['active_experiments'], dashboard_summary['shadow_experiments'])
