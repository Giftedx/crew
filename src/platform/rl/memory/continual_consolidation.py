...
class ContinualMemoryConsolidation:
    ...
    def __init__(self, min_consolidation_interval_s: float=3600.0, prune_batch_size: int=50, merge_similarity_threshold: float=0.95):
        ...
        logger.info(f'ContinualMemoryConsolidation initialized: interval={min_consolidation_interval_s}s, batch_size={prune_batch_size}')
    ...
    async def consolidate_memory(self, tenant: str, workspace: str, trigger_reason: str='scheduled') -> StepResult:
        ...
        try:
            if not self._can_consolidate(tenant):
                return StepResult.skip(message='Skipping consolidation: too soon since last run', data={'last_consolidation': self.stats.last_consolidation})
            ...
            result = ConsolidationResult(chunks_pruned=pruned_count, chunks_merged=merged_count, chunks_reindexed=reindexed_count, embeddings_recomputed=recomputed_count, quality_improvement=quality_improvement, processing_time_s=processing_time)
            self._update_stats(tenant, result)
            logger.info(f'Consolidation complete: pruned={pruned_count}, merged={merged_count}, recomputed={recomputed_count}, quality_improvement={quality_improvement:.2%}, time={processing_time:.2f}s')
    ...
    async def _merge_similar_chunks(self, tenant: str, workspace: str, rag_feedback, memory_service) -> int:
        ...
        """
        chunks_with_metrics = [(chunk_id, metrics) for chunk_id, metrics in rag_feedback.chunk_metrics.items() if metrics.retrieval_count >= 3]
    ...
    def _calculate_quality_improvement(self, rag_feedback) -> float:
        ...
        if not rag_feedback.chunk_metrics:
            return 0.0
        current_avg = sum((m.quality_score for m in rag_feedback.chunk_metrics.values())) / len(rag_feedback.chunk_metrics)
    ...
    def _update_stats(self, tenant: str, result: ConsolidationResult) -> None:
        ...
        alpha = 0.2
        self.stats.avg_quality_improvement = alpha * result.quality_improvement + (1 - alpha) * self.stats.avg_quality_improvement
        ...
        tenant_stats.avg_quality_improvement = alpha * result.quality_improvement + (1 - alpha) * tenant_stats.avg_quality_improvement
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        """
        return {'global_stats': {'total_operations': self.stats.total_operations, 'total_chunks_pruned': self.stats.total_chunks_pruned, 'total_chunks_merged': self.stats.total_chunks_merged, 'total_embeddings_recomputed': self.stats.total_embeddings_recomputed, 'avg_quality_improvement': self.stats.avg_quality_improvement, 'last_consolidation': self.stats.last_consolidation}, 'tenant_stats': {tenant: {'total_operations': stats.total_operations, 'total_chunks_pruned': stats.total_chunks_pruned, 'total_chunks_merged': stats.total_chunks_merged, 'avg_quality_improvement': stats.avg_quality_improvement, 'last_consolidation': stats.last_consolidation} for tenant, stats in self.tenant_stats.items()}, 'recent_consolidations': [{'chunks_pruned': r.chunks_pruned, 'chunks_merged': r.chunks_merged, 'quality_improvement': r.quality_improvement, 'processing_time_s': r.processing_time_s, 'timestamp': r.timestamp} for r in self.consolidation_history[-10:]]}
