...
class LangSmithEvaluator:
    ...
    def __init__(self, shadow_mode: bool | None=None, regression_threshold: float | None=None) -> None:
        ...
        self.regression_threshold = regression_threshold if regression_threshold is not None else config.self_eval_regression_threshold
    ...
    async def evaluate_output(self, task_name: str, input_data: dict[str, Any], output_data: dict[str, Any], context: dict[str, Any] | None=None) -> EvalResult:
        ...
        """
        if not self.enabled:
            return EvalResult(passed=True, metrics=EvalMetrics(quality_score=1.0, safety_score=1.0, relevance_score=1.0, coherence_score=1.0, overall_score=1.0, latency_ms=0.0), feedback='Evaluation disabled', should_rollback=False, regression_detected=False)
        ...
        try:
            ...
            if task_key not in self._baselines or metrics_result.overall_score > self._baselines[task_key].overall_score:
            ...
            return EvalResult(passed=not should_rollback, metrics=metrics_result, feedback=feedback, should_rollback=should_rollback, regression_detected=regression_detected)
        except Exception as e:
            logger.exception('Evaluation failed for task %s: %s', task_name, e)
            return EvalResult(passed=self.shadow_mode, metrics=EvalMetrics(quality_score=0.0, safety_score=0.0, relevance_score=0.0, coherence_score=0.0, overall_score=0.0, latency_ms=(time.perf_counter() - start) * 1000, metadata={'error': str(e)}), feedback=f'Evaluation error: {e}', should_rollback=not self.shadow_mode, regression_detected=False)

    async def _compute_metrics(self, task_name: str, input_data: dict[str, Any], output_data: dict[str, Any], context: dict[str, Any]) -> EvalMetrics:
        ...
        return EvalMetrics(quality_score=quality_score, safety_score=safety_score, relevance_score=relevance_score, coherence_score=coherence_score, overall_score=overall_score, latency_ms=0.0, metadata={'task': task_name, 'evaluator': 'langsmith', 'version': '1.0'})
