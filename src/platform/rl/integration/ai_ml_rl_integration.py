...
class AIMLRLConfig:
    ...
    def from_env(cls) -> AIMLRLConfig:
        ...
        return cls(enable_unified_feedback=os.getenv('ENABLE_UNIFIED_FEEDBACK', '1') == '1', enable_tool_routing_bandit=os.getenv('ENABLE_TOOL_ROUTING_BANDIT', '1') == '1', enable_agent_routing_bandit=os.getenv('ENABLE_AGENT_ROUTING_BANDIT', '1') == '1', enable_rag_quality_feedback=os.getenv('ENABLE_RAG_QUALITY_FEEDBACK', '1') == '1', enable_prompt_ab_testing=os.getenv('ENABLE_PROMPT_AB_TESTING', '1') == '1', exploration_rate=float(os.getenv('BANDIT_EXPLORATION_RATE', '0.1')))
...
class AIMLRLIntegration:
    ...
    async def start(self) -> StepResult:
        ...
        try:
            ...
            self._running = True
            self._tasks = [asyncio.create_task(self._feedback_processing_loop()), asyncio.create_task(self._metrics_collection_loop())]
    ...
    async def _initialize_components(self) -> None:
        ...
        if self.config.enable_unified_feedback:
            from platform.orchestration.application import get_orchestrator
            self._orchestrator = get_orchestrator(auto_create=True)
            if self.config.enable_tool_routing_bandit:
                from platform.rl.tool_routing_bandit import get_tool_router
            ...
            if self.config.enable_agent_routing_bandit:
                from platform.rl.agent_routing_bandit import get_agent_router
            ...
            if self.config.enable_prompt_ab_testing:
                from platform.rl.prompts.prompt_library_ab import get_prompt_library
            ...
            if self.config.enable_threshold_tuning:
                from platform.rl.threshold_tuning_bandit import get_threshold_bandit
    ...
    async def get_aggregate_metrics(self) -> dict[str, Any]:
        ...
        metrics = {'orchestrator': None, 'tool_router': None, 'agent_router': None, 'rag_feedback': None, 'prompt_library': None, 'threshold_tuner': None}
        try:
            if self._orchestrator:
    ...
    async def route_tool(self, task_description: str, context: dict[str, Any], task_type: str='general') -> StepResult:
    ...
    async def route_agent(self, task_description: str, context: dict[str, Any], task_type: str='general') -> StepResult:
    ...
    async def select_prompt(self, prompt_type: str, context: dict[str, Any], optimization_target: str='quality') -> StepResult:
        """Select best prompt variant"""
        if not self._prompt_library:
            return StepResult.fail('Prompt A/B testing not enabled')
        from platform.rl.prompts.prompt_library_ab import PromptType
        try:
    ...
    def submit_rag_feedback(self, query_id: str, query_text: str, retrieved_chunks: list[dict[str, Any]], relevance_scores: list[float]) -> StepResult:
    ...
    async def select_thresholds(self, content_type: str='general', context: dict[str, Any] | None=None) -> StepResult:
    ...
    def submit_threshold_feedback(self, config_id: str, content_type: str, bypass_decision: bool, cost_saved_usd: float, quality_score: float, processing_time_saved_s: float=0.0) -> StepResult:
        """Submit feedback for threshold decision"""
        if not self._threshold_tuner:
            return StepResult.fail('Threshold tuning not enabled')
        self._threshold_tuner.submit_threshold_feedback(config_id, content_type, bypass_decision, cost_saved_usd, quality_score, processing_time_saved_s)
...
__all__ = ['AIMLRLConfig', 'AIMLRLIntegration', 'get_ai_integration', 'set_ai_integration']