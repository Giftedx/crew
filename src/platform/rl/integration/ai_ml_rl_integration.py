...
class AIMLRLIntegration:
    ...
    def submit_rag_feedback(
        self, query_id: str, query_text: str, retrieved_chunks: list[dict[str, Any]], relevance_scores: list[float]
    ) -> StepResult:
    ...
    async def select_thresholds(
        self, content_type: str = "general", context: dict[str, Any] | None = None
    ) -> StepResult:
    ...
    def submit_threshold_feedback(
        ...
        self._threshold_tuner.submit_threshold_feedback(
            config_id, content_type, bypass_decision, cost_saved_usd, quality_score, processing_time_saved_s
        )
