...
class LangSmithTrajectoryEvaluator:
    ...
    def __init__(self, orchestrator: UnifiedFeedbackOrchestrator | None=None, *, settings: SecureConfig | None=None, adapter: LangSmithEvaluationAdapter | None=None) -> None:
    ...
    def evaluate(self, trajectory: AgentTrajectory) -> StepResult:
        ...
        metrics.TRAJECTORY_EVALUATIONS.labels(**metrics.label_ctx({'success': str(result.success and (not result.skipped)).lower()})).inc()
        return result
    ...
    def submit_feedback(self, trajectory: AgentTrajectory, evaluation: StepResult) -> StepResult:
        ...
        try:
            ...
            return evaluation
        metadata.update({'feedback_submitted': feedback_result.success and (not feedback_result.skipped), 'orchestrator_status': feedback_result.custom_status or ('skipped' if feedback_result.skipped else 'success' if feedback_result.success else 'error'), 'feedback_payload': payload})
    ...
    def _build_feedback_payload(evaluation: StepResult) -> dict[str, Any] | None:
        ...
        if accuracy == 0.0 and efficiency == 0.0 and (error_handling == 0.0):
            return None
        overall = LangSmithTrajectoryEvaluator._coerce_float(data.get('overall_score'), default=0.5 * accuracy + 0.3 * efficiency + 0.2 * error_handling)
        ...
        payload: dict[str, Any] = {'score': bool(data.get('score', True)), 'overall_score': overall, 'accuracy_score': accuracy, 'efficiency_score': efficiency, 'error_handling_score': error_handling, 'confidence': confidence, 'reasoning': reasoning}
        if evaluation_id:
