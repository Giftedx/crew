...
class EnhancedSemanticCache:
    ...
    def __init__(self, *, similarity_threshold: float=0.8, max_cache_size: int=1000, cache_ttl_seconds: int=3600, enable_embeddings: bool=True, enable_tfidf: bool=True, namespace: str | None=None):
        ...
        self.stats = {'total_requests': 0, 'exact_hits': 0, 'semantic_hits': 0, 'fuzzy_hits': 0, 'misses': 0, 'cache_stores': 0, 'evictions': 0}
    ...
    def _preprocess_prompt(self, prompt: str) -> str:
        ...
        prompt = re.sub('\\b[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}\\b', '[UUID]', prompt)
    ...
    def _tfidf_similarity(self, text1: str, text2: str) -> float:
        ...
        try:
            if self._tfidf_matrix is None:
                all_prompts = list(self._prompt_embeddings.keys()) + [text1, text2]
    ...
    def set(self, prompt: str, model: str, response: dict[str, Any], **kwargs) -> None:
        ...
        entry = CacheEntry(key=cache_key, prompt=prompt, response=response, model=model, namespace=kwargs.get('namespace'), stored_at=time.time(), expires_at=time.time() + self.cache_ttl_seconds)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        hit_rate = (self.stats['exact_hits'] + self.stats['semantic_hits']) / total_requests if total_requests > 0 else 0.0
        return {**self.stats, 'total_requests': total_requests, 'hit_rate': hit_rate, 'cache_size': len(self._cache), 'embedding_model_available': self.enable_embeddings, 'tfidf_model_available': self.enable_tfidf}
...
def create_enhanced_semantic_cache(similarity_threshold: float=0.8, max_cache_size: int=1000, cache_ttl_seconds: int=3600, enable_embeddings: bool=True, enable_tfidf: bool=True, namespace: str | None=None) -> EnhancedSemanticCache:
    """Factory function for creating enhanced semantic cache instances."""
    return EnhancedSemanticCache(similarity_threshold=similarity_threshold, max_cache_size=max_cache_size, cache_ttl_seconds=cache_ttl_seconds, enable_embeddings=enable_embeddings, enable_tfidf=enable_tfidf, namespace=namespace)
