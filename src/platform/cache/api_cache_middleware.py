...
class APICacheConfig:
    ...
    def __init__(self, cache_ttl: int=300, exclude_paths: set[str] | None=None, exclude_methods: set[str] | None=None, include_headers: list[str] | None=None, cache_private: bool=False):
...
class APICacheMiddleware:
    ...
    def __init__(self, cache_service: Any | None=None, cache_ttl: int=300, exclude_paths: set[str] | None=None, exclude_methods: set[str] | None=None, include_headers: list[str] | None=None, cache_private: bool=False):
    ...
    def _get_user_identifier(self, request: Request) -> str | None:
        ...
        api_key = getattr(request.headers, 'get', lambda x: None)('x-api-key') or getattr(request.headers, 'get', lambda x: None)('authorization')
        if api_key:
    ...
    def _create_response_from_cache(self, cached_data: dict[str, Any]) -> Response:
        ...
        return Response(content=cached_data.get('content', b''), status_code=cached_data.get('status_code', 200), headers=cached_data.get('headers', {}), media_type=cached_data.get('media_type'))
    ...
    async def _cache_response(self, cache_key: str, response: Response) -> None:
        ...
        try:
            ...
            return
        cache_data = {'content': content, 'status_code': response.status_code, 'headers': dict(getattr(response, 'headers', {})), 'media_type': getattr(response, 'media_type', None), 'cached_at': 'now'}
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'total_requests': total_requests, 'cache_hits': self.stats['hits'], 'cache_misses': self.stats['misses'], 'cache_bypassed': self.stats['bypassed'], 'cache_errors': self.stats['errors'], 'hit_rate': hit_rate, 'cache_ttl': self.cache_ttl}
...
def create_api_cache_middleware(cache_ttl: int=300, exclude_paths: set[str] | None=None, exclude_methods: set[str] | None=None, include_headers: list[str] | None=None, cache_private: bool=False) -> APICacheMiddleware:
    """Factory function to create API cache middleware."""
    return APICacheMiddleware(cache_ttl=cache_ttl, exclude_paths=exclude_paths, exclude_methods=exclude_methods, include_headers=include_headers, cache_private=cache_private)
