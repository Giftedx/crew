...
class InvalidationEngine:
    ...
    async def invalidate_key(
        self, key: str, cascade: bool = True, reason: str = "manual_invalidation"
    ) -> InvalidationResult:
        ...
        async with self._semaphore:
            if self._circuit_open and (not await self._should_attempt_reset()):
                return InvalidationResult(
                    skipped_keys={key},
                    errors=[f"Circuit breaker open, skipping invalidation of {key}"],
            ...
            try:
                ...
                return InvalidationResult(
                    skipped_keys={key},
                    errors=[f"Invalidation failed: {e!s}"],
                    start_time=start_time,
                    end_time=time.time(),
                )

    async def invalidate_keys(
        self, keys: set[str], cascade: bool = True, reason: str = "batch_invalidation"
    ) -> InvalidationResult:
        ...
        if len(keys) <= self.max_batch_size:
            ...
            return InvalidationResult(
                skipped_keys=keys, errors=["Queued for batch processing"], start_time=time.time(), end_time=time.time()
            )
    ...
    async def _invalidate_keys_batch(self, keys: set[str], cascade: bool, reason: str) -> InvalidationResult:
        ...
        async with self._semaphore:
            if self._circuit_open and (not await self._should_attempt_reset()):
                return InvalidationResult(
                    skipped_keys=keys, errors=["Circuit breaker open"], start_time=start_time, end_time=time.time()
                )
            try:
                ...
                return InvalidationResult(
                    skipped_keys=keys,
                    errors=[f"Batch invalidation failed: {e!s}"],
                    start_time=start_time,
                    end_time=time.time(),
                )
    ...
    async def _invalidate_keys(self, keys: set[str], reason: str) -> InvalidationResult:
        ...
        return InvalidationResult(
            invalidated_keys=invalidated_keys, errors=errors, start_time=start_time, end_time=time.time()
        )
    ...
    async def _process_batch_queue(self) -> None:
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Processed invalidation batch: {batch.reason}, keys: {len(batch.keys)}, invalidated: {len(result.invalidated_keys)}, errors: {len(result.errors)}"
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "successful_invalidations": self._stats["successful_invalidations"],
                "failed_invalidations": self._stats["failed_invalidations"],
                "success_rate": self._stats["successful_invalidations"] / self._stats["total_invalidations"]
                if self._stats["total_invalidations"] > 0
                else 1.0,
                "average_batch_size": self._stats["average_batch_size"],
                "max_batch_size_processed": self._stats["max_batch_size_processed"],
            },
            "queue": {"size": self._batch_queue.qsize(), "max_concurrent": self.max_concurrent_invalidations},
        }
...
__all__ = [
    ...
    "start_invalidation_engine",
    "stop_invalidation_engine",
]
