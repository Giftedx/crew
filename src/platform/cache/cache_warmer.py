...
class AccessPatternTracker:
    ...
    async def get_top_patterns(self, cache_name: str, limit: int=50) -> list[AccessPattern]:
        ...
        async with self._lock:
            cache_patterns = [pattern for pattern_key, pattern in self.patterns.items() if pattern_key.startswith(f'{cache_name}:')]
...
class CacheWarmer:
    ...
    async def warm_specific_keys(self, keys: list[str], data_fetcher: Callable[[str], dict[str, Any] | None], batch_size: int=10) -> int:
    ...
    def get_warming_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {'cache_efficiency': efficiency, 'frequent_keys_count': len(frequent_keys), 'top_keys': frequent_keys[:5], 'cache_stats': cache_stats, 'recommendations': self._generate_recommendations(efficiency, frequent_keys)}
        except Exception as e:
    ...
    def _generate_recommendations(self, efficiency: float, frequent_keys: list[tuple[str, int]]) -> list[str]:
        ...
        if len(frequent_keys) > HIGH_ACCESS_DIVERSITY_THRESHOLD:
            recommendations.append(f'High access pattern diversity detected ({len(frequent_keys)} keys). Consider cache warming for top keys.')
...
class PredictiveCacheWarmer(CacheWarmer):
    ...
    def __init__(self, cache: EnhancedRedisCache, warm_interval_hours: int=4, prediction_window_hours: int=24) -> None:
    ...
    async def warm_all_caches(self, max_concurrent: int=3) -> dict[str, int]:
        ...
        try:
            ...
            for i in range(0, len(warmer_items), max_concurrent):
                ...
                for (cache_name, _), result in zip(batch, batch_results):
