...
HIT_RATE_TARGET = 0.6
HIT_RATE_DECLINE_TOLERANCE = 0.05
...
class CachePerformanceMetrics:
    ...
    def total_requests(self) -> int:
        """Convenience alias for total requests handled."""
        return self.get_total_requests()

    @property
    def cache_hits(self) -> int:
        """Alias for number of cache hits."""
        return self.hit_count

    @property
    def cache_misses(self) -> int:
        """Alias for number of cache misses."""
        return self.miss_count

    @property
    def hit_rate(self) -> float:
        """Alias for current hit rate."""
        return self.get_hit_rate()

    @property
    def cost_efficiency(self) -> float:
        """Alias for cost savings ratio."""
        return self.get_cost_savings_ratio()
...
class AdaptiveSemanticCache:
    ...
    async def _generate_optimization_recommendation(self) -> OptimizationRecommendation | None:
        ...
        hit_rate_trending_down = (
            recent_total >= self.evaluation_window / 2
            and current_hit_rate - recent_hit_rate > HIT_RATE_DECLINE_TOLERANCE
        )
        needs_improvement = (
            current_hit_rate < HIT_RATE_TARGET or current_cost_savings < COST_SAVINGS_TARGET or hit_rate_trending_down
        )
        ...
        if current_hit_rate < HIT_RATE_TARGET:
            if self.current_threshold > MIN_THRESHOLD:
                ...
                return OptimizationRecommendation(
                    ...
                    current_threshold=self.current_threshold,
                    recommended_threshold=self.current_threshold,
                    reason="Already at minimum threshold",
                    expected_hit_rate_improvement=0.0,
                    expected_cost_savings_improvement=0.0,
                    confidence=0.5,
                )
            ...
            return OptimizationRecommendation(
                ...
                current_threshold=self.current_threshold,
                recommended_threshold=self.current_threshold,
                reason="Already at maximum threshold",
                expected_hit_rate_improvement=0.0,
                expected_cost_savings_improvement=0.0,
                confidence=0.5,
            )
        ...
        return OptimizationRecommendation(
            ...
            expected_cost_savings_improvement=expected_improvement * 0.5,
    ...
    async def _apply_optimization(self, recommendation: OptimizationRecommendation) -> None:
        ...
        logger.info(
            f"Applied optimization: {recommendation.action} (threshold: {old_threshold:.3f} -> {new_threshold:.3f}) reason: {recommendation.reason} confidence: {recommendation.confidence:.3f}"
        )
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "hit_rate": self.metrics.get_hit_rate(),
            "total_requests": self.metrics.get_total_requests(),
            "cost_savings_ratio": self.metrics.get_cost_savings_ratio(),
            "avg_latency_savings_ms": self.metrics.get_avg_latency_savings_ms(),
            ...
            "targets": {"hit_rate_target": HIT_RATE_TARGET, "cost_savings_target": COST_SAVINGS_TARGET},
            "threshold_limits": {"min_threshold": MIN_THRESHOLD, "max_threshold": MAX_THRESHOLD},
        }
        ...
        return [
            {
                ...
                "reason": rec.reason,
                "expected_hit_rate_improvement": rec.expected_hit_rate_improvement,
                "expected_cost_savings_improvement": rec.expected_cost_savings_improvement,
...
async def optimize_all_caches() -> dict[str, Any]:
    ...
    return {
        "cache_name": _adaptive_cache.name,
        "recommendation": recommendation.__dict__ if recommendation else None,
        "performance_summary": _adaptive_cache.get_performance_summary(),
    }


def create_adaptive_semantic_cache(**kwargs: Any) -> AdaptiveSemanticCache:
    """Factory helper mirroring legacy synchronous API."""

    return AdaptiveSemanticCache(**kwargs)


def reset_adaptive_cache() -> None:
    """Reset the cached singleton instance used by get_adaptive_semantic_cache."""
