...
class LLMRouterSingleton:
    ...
    def get(cls) -> Any | None:
        ...
        cfg = get_config()
        try:
            model_list = [{'model_name': getattr(cfg, 'openrouter_general_model', None) or getattr(cfg, 'openrouter_analysis_model', None) or 'openai/gpt-4o-mini', 'litellm_params': {'api_key': getattr(cfg, 'openrouter_api_key', None) or getattr(cfg, 'openai_api_key', None), 'api_base': 'https://openrouter.ai/api/v1'}}]
            router = _Router(model_list=model_list, routing_strategy=getattr(cfg, 'litellm_routing_strategy', 'usage-based-routing'), cache=getattr(cfg, 'litellm_cache_enabled', True), fallbacks_enabled=getattr(cfg, 'litellm_fallback_enabled', True))
            cls._instance = router
            logger.info('LiteLLM router initialised (strategy=%s)', getattr(cfg, 'litellm_routing_strategy', 'usage-based-routing'))
            return cls._instance
