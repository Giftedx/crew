...
class ContextualBandit:
    ...
    def __init__(self, arms: list[ModelCapability], context_dim: int=10, exploration_rate: float=0.1):
        """
    ...
    def update(self, arm_id: str, context: np.ndarray, reward: float, trajectory_feedback: TrajectoryFeedback | None=None) -> None:
        """
        ...
        if trajectory_feedback is not None:
            trajectory_quality = 0.5 * trajectory_feedback.accuracy_score + 0.3 * trajectory_feedback.efficiency_score + 0.2 * trajectory_feedback.error_handling_score
    ...
    def get_arm_statistics(self) -> StepResult:
        ...
        for arm_id in self.arms:
            ...
            stats[arm_id] = {'count': self.arm_counts[arm_id], 'average_reward': np.mean(rewards) if rewards else 0.0, 'reward_std': np.std(rewards) if rewards else 0.0, 'total_rewards': len(rewards)}
        return stats
...
class RLModelRouter:
    ...
    def __init__(self):
        ...
        self.performance_metrics = {'total_routes': 0, 'successful_routes': 0, 'average_latency': 0.0, 'average_cost': 0.0, 'average_quality': 0.0}
    ...
    def _initialize_default_models(self):
        ...
        default_models = [ModelCapability(model_id='gpt-4', provider=ModelProvider.OPENAI, max_tokens=8192, cost_per_1k_tokens=0.03, average_latency_ms=2000, accuracy_score=0.95, reliability_score=0.98, capabilities=['text_generation', 'reasoning', 'analysis'], specializations=['complex_reasoning', 'creative_writing']), ModelCapability(model_id='gpt-3.5-turbo', provider=ModelProvider.OPENAI, max_tokens=4096, cost_per_1k_tokens=0.002, average_latency_ms=1000, accuracy_score=0.85, reliability_score=0.95, capabilities=['text_generation', 'summarization'], specializations=['fast_generation', 'cost_effective']), ModelCapability(model_id='claude-3-opus', provider=ModelProvider.ANTHROPIC, max_tokens=200000, cost_per_1k_tokens=0.015, average_latency_ms=3000, accuracy_score=0.97, reliability_score=0.99, capabilities=['text_generation', 'analysis', 'reasoning'], specializations=['long_context', 'high_accuracy']), ModelCapability(model_id='claude-3-sonnet', provider=ModelProvider.ANTHROPIC, max_tokens=200000, cost_per_1k_tokens=0.003, average_latency_ms=1500, accuracy_score=0.92, reliability_score=0.97, capabilities=['text_generation', 'analysis'], specializations=['balanced_performance'])]
    ...
    async def route_request(self, context: RoutingContext) -> StepResult:
        ...
        try:
            ...
            selection = ModelSelection(model_id=selected_model_id, provider=selected_model.provider, confidence=confidence, expected_reward=self._calculate_expected_reward(selected_model, context), reasoning=self._generate_routing_reason(selected_model, context), fallback_models=self._get_fallback_models(selected_model_id, context), estimated_cost=self._estimate_cost(selected_model, context.token_estimate), estimated_latency_ms=selected_model.average_latency_ms)
            self.performance_metrics['total_routes'] += 1
            return StepResult.ok(data={'model_selection': selection, 'context': context, 'routing_metadata': {'bandit_confidence': confidence, 'available_models': len(self.model_capabilities), 'routing_timestamp': default_utc_now().isoformat()}})
    ...
    def process_trajectory_feedback(self, batch_size: int=10) -> StepResult:
        ...
        for _ in range(batch_count):
            ...
            if routing_entry is None:
                failed += 1
                metrics.TRAJECTORY_FEEDBACK_PROCESSED.labels(**metrics.label_ctx(), model_id=feedback.model_id, result='missing_history').inc()
            ...
            try:
                ...
                processed += 1
                metrics.TRAJECTORY_FEEDBACK_PROCESSED.labels(**metrics.label_ctx(), model_id=feedback.model_id, result='success').inc()
                ...
                metrics.TRAJECTORY_FEEDBACK_PROCESSED.labels(**metrics.label_ctx(), model_id=feedback.model_id, result='failure').inc()
        ...
        return StepResult.ok(processed=processed, failed=failed, remaining_queue_size=len(self.trajectory_feedback_queue))
    ...
    def _extract_context_vector(self, context: dict[str, Any] | None) -> np.ndarray:
        ...
        context = context or {}
        complexity_map: dict[str, float] = {TaskComplexity.SIMPLE.value: 0.25, TaskComplexity.MODERATE.value: 0.5, TaskComplexity.COMPLEX.value: 0.75, TaskComplexity.CRITICAL.value: 1.0}
        ...
        if isinstance(token_estimate, (int, float)) and token_estimate > 0:
            features[1] = float(np.log1p(token_estimate) / np.log1p(100000))
        ...
        if isinstance(latency_requirement, (int, float)) and latency_requirement > 0:
            features[3] = 1.0 - min(1.0, float(latency_requirement) / 10000.0)
        else:
            features[3] = 0.5
    ...
    async def update_reward(self, model_id: str, task_id: str, reward_data: dict[str, Any]) -> StepResult:
        ...
        """
        try:
            reward = RoutingReward(model_id=model_id, task_id=task_id, reward=reward_data.get('reward', 0.0), latency_ms=reward_data.get('latency_ms', 0.0), cost_usd=reward_data.get('cost_usd', 0.0), quality_score=reward_data.get('quality_score', 0.0), success=reward_data.get('success', False), context=reward_data.get('context', {}))
            ...
            return StepResult.ok(data={'model_id': model_id, 'task_id': task_id, 'reward': reward.reward, 'updated_metrics': self.performance_metrics, 'bandit_statistics': self.bandit.get_arm_statistics() if self.bandit else {}})
        except Exception as e:
    ...
    def get_routing_statistics(self) -> StepResult:
        ...
        """
        try:
            stats = {'performance_metrics': self.performance_metrics, 'model_capabilities': {model_id: {'provider': model.provider.value, 'max_tokens': model.max_tokens, 'cost_per_1k_tokens': model.cost_per_1k_tokens, 'average_latency_ms': model.average_latency_ms, 'accuracy_score': model.accuracy_score, 'reliability_score': model.reliability_score, 'capabilities': model.capabilities, 'specializations': model.specializations} for model_id, model in self.model_capabilities.items()}, 'bandit_statistics': self.bandit.get_arm_statistics() if self.bandit else {}, 'routing_history_size': len(self.routing_history), 'recent_performance': self._get_recent_performance()}
    ...
    def _context_to_features(self, context: RoutingContext) -> StepResult:
        ...
        features = np.zeros(10)
        task_types = ['analysis', 'generation', 'summarization', 'translation', 'reasoning']
        if context.task_type in task_types:
            features[task_types.index(context.task_type)] = 1.0
        complexity_values = {TaskComplexity.SIMPLE: 0.25, TaskComplexity.MODERATE: 0.5, TaskComplexity.COMPLEX: 0.75, TaskComplexity.CRITICAL: 1.0}
    ...
    def _update_performance_metrics(self, reward: RoutingReward):
        ...
        alpha = 0.1
        self.performance_metrics['average_latency'] = (1 - alpha) * self.performance_metrics['average_latency'] + alpha * reward.latency_ms
        self.performance_metrics['average_cost'] = (1 - alpha) * self.performance_metrics['average_cost'] + alpha * reward.cost_usd
        self.performance_metrics['average_quality'] = (1 - alpha) * self.performance_metrics['average_quality'] + alpha * reward.quality_score
    ...
    def _get_recent_performance(self) -> StepResult:
        ...
        return {'recent_success_rate': sum((1 for r in recent_rewards if r.success)) / len(recent_rewards), 'recent_average_latency': np.mean([r.latency_ms for r in recent_rewards]), 'recent_average_cost': np.mean([r.cost_usd for r in recent_rewards]), 'recent_average_quality': np.mean([r.quality_score for r in recent_rewards]), 'recent_reward_count': len(recent_rewards)}

    def save_state(self, filepath: str) -> StepResult:
        """Save router state to file."""
        try:
            state = {'model_capabilities': self.model_capabilities, 'routing_history': self.routing_history, 'performance_metrics': self.performance_metrics, 'bandit_parameters': {k: v.tolist() if hasattr(v, 'tolist') else v for k, v in (self.bandit.arm_parameters if self.bandit else {}).items()}, 'bandit_counts': {k: v.tolist() if hasattr(v, 'tolist') else v for k, v in (self.bandit.arm_counts if self.bandit else {}).items()}, 'bandit_rewards': {k: v.tolist() if hasattr(v, 'tolist') else v for k, v in (self.bandit.arm_rewards if self.bandit else {}).items()}}
