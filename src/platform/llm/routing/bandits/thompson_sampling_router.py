...
class BetaPosterior:
    ...
    def from_dict(cls, data: dict[str, Any]) -> BetaPosterior:
        ...
        return cls(alpha=data.get('alpha', 1.0), beta=data.get('beta', 1.0), trials=data.get('trials', 0), last_updated=data.get('last_updated', time.time()))
...
class ThompsonSamplingRouter:
    ...
    def __init__(self, persistence_path: str | Path | None=None, cold_start_trials: int | None=None) -> None:
        ...
        self.cold_start_trials = cold_start_trials if cold_start_trials is not None else config.ts_routing_cold_start_trials
        ...
        if self.enabled:
            self._load_state()
            logger.info('Thompson Sampling router initialized (cold_start=%d, state=%s)', self.cold_start_trials, self.persistence_path)
    ...
    def select_model(self, task_name: str, available_models: list[str], deterministic: bool=False) -> str:
        ...
        if in_cold_start:
            ...
            logger.debug('Cold-start exploration: task=%s, selected=%s, trials=%d/%d', task_key, selected, total_trials, self.cold_start_trials)
        elif deterministic:
            selected = max(available_models, key=lambda m: posteriors[m].mean())
            logger.debug('Deterministic selection: task=%s, selected=%s, mean=%.3f', task_key, selected, posteriors[selected].mean())
        else:
        ...
        metrics.get_metrics().counter('ts_router_selections_total', labels={'task': task_name, 'model': selected, 'cold_start': str(in_cold_start)})
        return selected
    ...
    def update_reward(self, task_name: str, model_name: str, reward: float) -> None:
        ...
        logger.debug('Updated posterior: task=%s, model=%s, reward=%.3f, alpha=%.2f, beta=%.2f, mean=%.3f', task_key, model_name, reward, posterior.alpha, posterior.beta, posterior.mean())
        metrics.get_metrics().histogram('ts_router_reward', reward, labels={'task': task_name, 'model': model_name})
        metrics.get_metrics().histogram('ts_router_posterior_mean', posterior.mean(), labels={'task': task_name, 'model': model_name})
    ...
    def get_statistics(self, task_name: str | None=None) -> dict[str, Any]:
        ...
        for task_key, posteriors in self._posteriors.items():
            if task_name and (not task_key.endswith(f':{task_name}')):
                continue
            stats[task_key] = {model: {'mean': posterior.mean(), 'alpha': posterior.alpha, 'beta': posterior.beta, 'trials': posterior.trials, 'last_updated': posterior.last_updated} for model, posterior in posteriors.items()}
        return stats

    def _save_state(self) -> None:
        """Persist posteriors to JSON."""
        try:
            state = {task_key: {model: posterior.to_dict() for model, posterior in posteriors.items()} for task_key, posteriors in self._posteriors.items()}
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            for task_key, posteriors_dict in state.items():
                self._posteriors[task_key] = {model: BetaPosterior.from_dict(posterior_dict) for model, posterior_dict in posteriors_dict.items()}
