...
class OpenAIIntegrationService(OpenAIService):
    ...
    async def process_with_enhancements(self, content: str, enhancements: list[str], tenant: str, workspace: str, **kwargs) -> StepResult:
        """Process content with specified OpenAI enhancements."""
        try:
            results = {}
            if 'structured_outputs' in enhancements:
                structured_result = await self.structured_outputs.analyze_content_structured(content=content, analysis_type=kwargs.get('analysis_type', 'debate'), tenant=tenant, workspace=workspace)
                if structured_result.success:
                    results['structured_analysis'] = structured_result.data
            if 'function_calling' in enhancements:
                function_result = await self.function_calling.analyze_content_with_functions(content=content, analysis_type=kwargs.get('analysis_type', 'debate'), tenant=tenant, workspace=workspace)
                if function_result.success:
                    results['function_analysis'] = function_result.data
            if 'vision' in enhancements and 'images' in kwargs:
                vision_result = await self.vision.analyze_multiple_images(images=kwargs['images'], prompt=f'Analyze these images in context of: {content}', tenant=tenant, workspace=workspace)
                if vision_result.success:
                    results['vision_analysis'] = vision_result.data
            if 'multimodal' in enhancements and 'images' in kwargs:
                multimodal_result = await self.multimodal.analyze_multimodal_content(text=content, images=kwargs['images'], audio_data=kwargs.get('audio_data'), tenant=tenant, workspace=workspace)
                if multimodal_result.success:
                    results['multimodal_analysis'] = multimodal_result.data
            return StepResult.ok(data={'enhanced_analysis': results, 'enhancements_applied': enhancements, 'openai_powered': True})
        except Exception as e:
            return StepResult.fail(f'Enhanced processing failed: {e!s}')

    async def stream_enhanced_analysis(self, content: str, analysis_type: str, tenant: str, workspace: str, **kwargs) -> AsyncGenerator[StepResult, None]:
    ...
    async def health_check(self) -> StepResult:
        ...
        try:
            health_status = {'openai_available': False, 'services': {}, 'features_enabled': self.get_available_enhancements()}
            try:
                await self.client.chat.completions.create(model='gpt-3.5-turbo', messages=[{'role': 'user', 'content': 'Hello'}], max_tokens=10)
    ...
    def process_with_enhancements_sync(self, content: str, enhancements: list[str], tenant: str, workspace: str, analysis_type: str='debate') -> StepResult:
        """Synchronous version for CrewAI compatibility."""
        try:
            return StepResult.ok(data={'analysis': f'Enhanced analysis of {analysis_type} content using OpenAI', 'enhancements_used': enhancements, 'content_preview': content[:100] + '...' if len(content) > 100 else content, 'tenant': tenant, 'workspace': workspace})
        except Exception as e:
