...
class ConnectionPool:
    ...
    def __init__(self, pool_size: int=10, max_retries: int=3, backoff_factor: float=0.3, status_forcelist: tuple[int, ...]=(500, 502, 504), keepalive: int=30) -> None:
        ...
        self._stats = {'total_requests': 0, 'successful_requests': 0, 'failed_requests': 0, 'retry_attempts': 0, 'connection_reuses': 0}
    ...
    def _create_session(self) -> http_utils.requests.Session:
        ...
        retry_strategy = http_utils.requests.packages.urllib3.util.retry.Retry(total=self._max_retries, backoff_factor=self._backoff_factor, status_forcelist=self._status_forcelist, allowed_methods=['HEAD', 'GET', 'POST', 'PUT', 'DELETE', 'OPTIONS', 'TRACE'])
        adapter = http_utils.requests.adapters.HTTPAdapter(pool_connections=self._pool_size, pool_maxsize=self._pool_size, pool_block=False, max_retries=retry_strategy)
    ...
    def _record_request_metric(self, method: str, status: str) -> None:
        ...
        """
        if self._metrics:
            self._metrics.counter('openrouter_connection_pool_requests_total', labels={'method': method, 'pool_size': str(self._pool_size), 'status': status}).inc()

    def _record_connection_reuse_metric(self) -> None:
        """Record connection reuse metric to Prometheus."""
        if self._metrics:
            self._metrics.counter('openrouter_connection_pool_reuses_total', labels={'pool_size': str(self._pool_size)}).inc()
    ...
    def post(self, url: str, headers: dict[str, str] | None=None, json_payload: dict[str, Any] | None=None, timeout: int=30, **kwargs: Any) -> http_utils.requests.Response | None:
        ...
        try:
            ...
            response = http_utils.retrying_post(url, json_payload=json_payload, headers=headers, timeout_seconds=timeout, request_callable=lambda u, **_: http_utils.resilient_post(u, json_payload=json_payload, headers=headers, timeout_seconds=timeout, request_fn=session.post))
    ...
    def get(self, url: str, headers: dict[str, str] | None=None, timeout: int=30, **kwargs: Any) -> requests.Response | None:
        ...
        try:
            self._stats['total_requests'] += 1
            response = http_utils.retrying_get(url, headers=headers, timeout_seconds=timeout, request_callable=lambda u, **_: http_utils.resilient_get(u, headers=headers, timeout_seconds=timeout, request_fn=session.get))
            if response and getattr(response, 'status_code', 500) < 400:
                self._stats['successful_requests'] += 1
                log.debug('Request successful: %s', response.status_code)
            else:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {**self._stats, 'success_rate_percent': round(success_rate, 2), 'pool_size': self._pool_size, 'max_retries': self._max_retries}
...
class MockConnectionPool:
    ...
    def post(self, url: str, headers: dict[str, str] | None=None, json_payload: dict[str, Any] | None=None, timeout: int=30, **kwargs: Any) -> requests.Response | None:
        """Make a POST request without connection pooling."""
        try:
            self._stats['total_requests'] += 1
            response = http_utils.retrying_post(url, json_payload=json_payload, headers=headers, timeout_seconds=timeout)
