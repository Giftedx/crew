...
class TenantResolver:
    ...
    def get_effective_models(self, models_map: dict[str, list[str]], tenant_context: TenantContext | None) -> dict[str, list[str]]:
...
class CostCalculator:
    ...
    def calculate_costs(self, prompt: str, chosen_model: str, effective_models: dict[str, list[str]], task_type: str, tenant_context: TenantContext | None) -> tuple[int, float, str | None]:
        ...
        affordable = self.token_meter.affordable_model(tokens_in, effective_models.get(task_type, effective_models.get('general', [])), prices=effective_prices)
        return (tokens_in, projected_cost, affordable)

    def enforce_budget_limits(self, projected_cost: float, task_type: str, chosen_model: str, affordable: str | None, tokens_in: int, effective_max: float, tenant_context: TenantContext | None) -> tuple[bool, str | None, str | None]:
...
class CacheManager:
    ...
    def get_semantic_cache_result(self, prompt: str, chosen_model: str, namespace: str | None, shadow_mode: bool, promotion_enabled: bool, promotion_threshold: float) -> RouteResult | None:
        ...
        try:
            ...
            if 'error' not in _holder:
                ...
                if sem_res is not None:
                    return self._process_semantic_cache_result(sem_res, chosen_model, shadow_mode, promotion_enabled, promotion_threshold)
        except Exception:
            pass
        return None

    def _process_semantic_cache_result(self, sem_res: dict[str, Any], chosen_model: str, shadow_mode: bool, promotion_enabled: bool, promotion_threshold: float) -> RouteResult | None:
        ...
        if shadow_mode:
            ...
            if promotion_enabled and sim_val >= promotion_threshold:
                ...
                return RouteResult(status='success', model=chosen_model, response=result.get('response', ''), cached=True, cache_type='semantic')
            return None
        else:
            result = dict(sem_res)
            result['cached'] = True
    ...
    def get_traditional_cache_result(self, cache_key: str, chosen_model: str) -> RouteResult | None:
        ...
        if cached:
            result = dict(cached)
            result['cached'] = True
            return RouteResult(status='success', model=result.get('model', chosen_model), response=result.get('response', ''), cached=True, cache_type='traditional')
        return None

    def set_cache_results(self, prompt: str, chosen_model: str, result: dict[str, Any], namespace: str | None, cache_key: str | None) -> None:
...
class RewardCalculator:
    ...
    def calculate_reward(self, projected_cost: float, latency_ms: float, effective_max: float, task_type: str, tenant_context: TenantContext | None) -> float:
...
class MetricsRecorder:
    ...
    def get_labels(self) -> dict[str, str]:
        ...
        if self.tenant_context is not None:
            return {'tenant': getattr(self.tenant_context, 'tenant_id', 'unknown'), 'workspace': getattr(self.tenant_context, 'workspace_id', 'unknown')}
        try:
...
class OfflineExecutor:
    ...
    def __init__(self, prompt_engine, learning_engine: LearningEngine, reward_calculator: RewardCalculator, metrics_recorder: MetricsRecorder):
        self.prompt_engine = prompt_engine
        self.learning_engine = learning_engine
        self.reward_calculator = reward_calculator
        self.metrics_recorder = metrics_recorder

    def execute(self, context: RouteContext, projected_cost: float, effective_max: float, tenant_context: TenantContext | None) -> RouteResult:
        """Execute offline path."""
        start = time.perf_counter()
        response = context.prompt.upper()
        latency_ms = (time.perf_counter() - start) * 1000
        reward = self.reward_calculator.calculate_reward(projected_cost, latency_ms, effective_max, context.task_type, tenant_context)
        if context.model:
        ...
        return RouteResult(status='success', model=context.model or 'unknown', response=response, tokens=context.tokens_in, provider=context.provider, cached=False)

class NetworkExecutor:
    """Handles network execution path."""

    def __init__(self, api_key: str, prompt_engine, learning_engine: LearningEngine, reward_calculator: RewardCalculator, metrics_recorder: MetricsRecorder):
    ...
    def execute(self, context: RouteContext, projected_cost: float, effective_max: float, tenant_context: TenantContext | None) -> RouteResult:
        ...
        start = time.perf_counter()
        try:
            payload: dict[str, Any] = {'model': context.model, 'messages': [{'role': 'user', 'content': context.prompt}]}
            if context.provider:
            ...
            if is_retry_enabled():
                try:
                    resp = http_request_with_retry('POST', url, request_callable=lambda u, **_: resilient_post(u, headers=headers, json_payload=payload, timeout_seconds=REQUEST_TIMEOUT_SECONDS), max_attempts=3)
                except Exception as retry_exc:
                    latency_ms = (time.perf_counter() - start) * 1000
                    return RouteResult(status='error', model=context.model or 'unknown', error=str(retry_exc), tokens=context.tokens_in, provider=context.provider)
            else:
                resp = resilient_post(url, headers=headers, json_payload=payload, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
            ...
            if context.model:
                ...
                self.metrics_recorder.record_cost_metrics(context.model, context.provider_family, projected_cost)
            self.metrics_recorder.record_latency_metrics(latency_ms)
            return RouteResult(status='success', model=context.model or 'unknown', response=message, tokens=context.tokens_in, provider=context.provider, cached=False)
        except Exception as exc:
            latency_ms = (time.perf_counter() - start) * 1000
            return RouteResult(status='error', model=context.model or 'unknown', error=str(exc), tokens=context.tokens_in, provider=context.provider)