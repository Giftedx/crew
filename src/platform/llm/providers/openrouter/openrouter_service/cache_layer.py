...
def check_caches(service: OpenRouterService, state: RouteState) -> dict[str, Any] | None:
    ...
    if service.semantic_cache is not None:
        ...
        try:
            log.debug('semantic_cache_get attempting (ns=%s, model=%s, type=%s)', ns, chosen, type(service.semantic_cache))
            ...
            if sem_res is not None:
                ...
                try:
                    cache_meta_sem.update({'status': 'hit', 'cache_type': 'semantic', 'similarity': float(sem_res.get('similarity', 0.0) if isinstance(sem_res, dict) else 0.0)})
                except Exception:
                ...
                if shadow_mode:
                    ...
                    try:
                        ...
                        sim_val = 0.0
                    promote = bool(getattr(service, 'semantic_cache_promotion_enabled', False)) and sim_val >= float(getattr(service, 'semantic_cache_promotion_threshold', 0.9))
                    if promote:
                        ...
                        service._adaptive_record_outcome(state, reward=0.0, status='cache_hit', metadata={'cache_type': 'semantic_shadow', 'similarity': sim_val, 'cache_info': cache_meta, 'promotion': True})
                        state.result = result
                        return result
                    log.debug('semantic_cache SHADOW HIT (no promotion) tracked for model=%s ns=%s sim=%.3f', chosen, ns, sim_val)
                else:
                    result = dict(sem_res)
                    result['cached'] = True
                    result['cache_type'] = 'semantic'
                    ...
                    try:
                        ...
                        similarity_val = 0.0
                    service._adaptive_record_outcome(state, reward=0.0, status='cache_hit', metadata={'cache_type': 'semantic', 'similarity': similarity_val, 'cache_info': cache_meta})
                    state.result = result
                    return result
            else:
    ...
    if service.cache:
        ...
        cache_key = combine_keys(generate_key_from_params(task_type=task_type, prompt=norm_prompt, provider=provider_sig, namespace=state.namespace or 'default:main'), generate_key_from_params(system_prompt=getattr(service, 'system_prompt', '') or ''))
        ...
        if cached:
            ...
            service._adaptive_record_outcome(state, reward=0.0, status='cache_hit', metadata={'cache_type': 'redis', 'cache_info': cache_meta})
            state.result = result
            return result
...
def _record_cache_hit(labels: dict[str, str], model: str, provider_family: str, *, shadow_promoted: bool=False) -> None:
    try:
        if shadow_promoted:
