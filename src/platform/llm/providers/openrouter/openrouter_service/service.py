...
class OpenRouterService:
            ...
            if gptcache_flag:
                try:
                    ...
                    self.semantic_cache = TenantSemanticCache(
                        cache_root=str(cache_root), similarity_threshold=threshold, fallback_ttl_seconds=ttl
                    )
                    self.semantic_cache_mode = "tenant"
                    enabled_sem = True
                except Exception as tenant_cache_exc:
                    log.warning(
                        "Tenant semantic cache init failed (%s); falling back to global cache", tenant_cache_exc
                    )
                    self.semantic_cache = None
    ...
    def route(
        ...
        result = route_prompt(
            self, prompt, task_type=task_type, model=model, provider_opts=provider_opts, compress=compress
        )
        self._adaptive_flush_events()
        return result

    def _adaptive_record_outcome(
        self, state: RouteState, *, reward: float, status: str, metadata: dict[str, Any] | None = None
    ) -> None:
