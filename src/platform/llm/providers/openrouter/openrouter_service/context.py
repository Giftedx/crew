...
def prepare_route_state(service: OpenRouterService, prompt: str, task_type: str, model: str | None, provider_opts: dict[str, Any] | None) -> RouteState:
    ...
    def _labels() -> dict[str, str]:
        if ctx_effective is not None:
            return {'tenant': getattr(ctx_effective, 'tenant_id', 'unknown'), 'workspace': getattr(ctx_effective, 'workspace_id', 'unknown')}
    ...
    if rl_router is not None and model is None and bool(candidate_models):
        try:
            context_features = {'task_type': task_type, 'tenant': labels_initial.get('tenant', 'unknown'), 'workspace': labels_initial.get('workspace', 'unknown'), 'offline_mode': bool(effective_offline), 'candidate_count': len(candidate_models), 'default_model': fallback_choice}
    ...
    adaptive_enabled = model is None and manager is not None and getattr(manager, 'enabled', False) and bool(candidate_models) and (rl_router_selection is None)
    if adaptive_enabled:
        context_payload = {'tenant': labels_initial.get('tenant', 'unknown'), 'workspace': labels_initial.get('workspace', 'unknown'), 'requested_model': model, 'default_model': fallback_choice, 'candidate_count': len(candidate_models), 'offline_mode': bool(effective_offline)}
    ...
    if manager is not None and getattr(manager, 'enabled', False):
        with contextlib.suppress(Exception):
            metrics.ACTIVE_BANDIT_POLICY.labels(tenant=labels_initial.get('tenant', 'unknown'), workspace=labels_initial.get('workspace', 'unknown'), domain=task_type, policy='ax_adaptive').set(1.0 if adaptive_trial_index is not None else 0.0)
    ...
    affordable = service.token_meter.affordable_model(tokens_in, effective_models.get(task_type, effective_models.get('general', [])), prices=effective_prices)
    tracker = _crt()
    return RouteState(prompt=prompt, task_type=task_type, requested_model=model, chosen_model=chosen, ctx_effective=ctx_effective, labels_factory=_labels, effective_models=effective_models, provider=provider, provider_family=provider_family, tokens_in=tokens_in, projected_cost=projected_cost, affordable_alternative=affordable, effective_prices=effective_prices, tracker=tracker, offline_mode=effective_offline, provider_overrides=provider_opts, compression_metadata=compression_meta, adaptive_trial_index=adaptive_trial_index, adaptive_suggested_model=adaptive_suggested_model, adaptive_candidates=adaptive_candidates, rl_router_selection=rl_router_selection)
