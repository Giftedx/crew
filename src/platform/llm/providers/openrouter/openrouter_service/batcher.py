...
class RequestBatcher:
    ...
    def __init__(self, service: OpenRouterService, config: BatchConfig | None=None) -> None:
        ...
        self._stats = {'total_requests': 0, 'batched_requests': 0, 'individual_requests': 0, 'batches_processed': 0, 'average_batch_size': 0.0, 'total_savings_ms': 0.0}
    ...
    async def _process_batch(self, batch_key: str, requests: list[BatchedRequest]) -> None:
        ...
        try:
            ...
            batch_payload = {'model': model, 'messages': [{'role': 'user', 'content': req.prompt} for req in requests], 'batch_mode': True}
            ...
            if result.get('status') == 'success':
                ...
                for i, request in enumerate(requests):
                    if i < len(responses):
                        ...
                        request.future.set_result({'status': 'success', 'model': model, 'response': response, 'tokens': self._service.prompt_engine.count_tokens(response, model), 'provider': requests[0].provider_opts, 'batched': True})
                    else:
                        request.future.set_result({'status': 'error', 'error': 'No response in batch', 'model': model, 'tokens': 0, 'provider': requests[0].provider_opts})
            else:
                error_response = {'status': 'error', 'error': result.get('error', 'Batch request failed'), 'model': model, 'tokens': 0, 'provider': requests[0].provider_opts}
            ...
            log.debug('Processed batch of %d requests in %.2fms (estimated savings: %.2fms)', len(requests), actual_time, savings)
        except Exception as e:
            log.error('Batch processing failed: %s', e)
            error_response = {'status': 'error', 'error': f'Batch processing failed: {e!s}', 'model': requests[0].model if requests else 'unknown', 'tokens': 0, 'provider': requests[0].provider_opts if requests else {}}
            for request in requests:
    ...
    async def _make_batch_request(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for message in messages:
                ...
                result = self._service.route(prompt=message['content'], task_type='general', model=payload['model'], provider_opts=payload.get('provider'))
                if result.success:
    ...
    async def add_request(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None) -> dict[str, Any]:
        ...
        request = BatchedRequest(prompt=prompt, task_type=task_type, model=model, provider_opts=provider_opts, future=future, request_id=f'{batch_key}_{int(time.time() * 1000)}', timestamp=time.time())
        async with self._batch_locks[batch_key]:
            self._batches[batch_key].append(request)
            self._stats['total_requests'] += 1
            should_process = len(self._batches[batch_key]) >= self._config.batch_size or (self._batches[batch_key] and time.time() - self._batches[batch_key][0].timestamp >= self._config.max_wait_time_ms / 1000)
            if should_process:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {**self._stats, 'batching_rate_percent': round(batching_rate, 2), 'active_batches': len(self._batches), 'active_processing_tasks': len(self._processing_tasks), 'config': {'batch_size': self._config.batch_size, 'wait_time_ms': self._config.wait_time_ms, 'max_wait_time_ms': self._config.max_wait_time_ms, 'enable_batching': self._config.enable_batching}}
...
class BatchManager:
    ...
    async def route_batched(self, prompt: str, task_type: str='general', model: str | None=None, provider_opts: dict[str, Any] | None=None) -> dict[str, Any]:
