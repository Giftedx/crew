...
class TaskComplexityMetrics:
    ...
    def analyze_content(cls, messages: Sequence[dict[str, Any]]) -> TaskComplexityMetrics:
        ...
        complexity_factors = [
            min(1.0, estimated_tokens / 2000),
            0.3 if has_reasoning else 0.0,
            0.2 if has_creativity else 0.0,
            0.2 if has_facts else 0.0,
            0.1 if has_multiple_languages else 0.0,
        ]
...
class LLMRouter:
    ...
    def __init__(self, clients: dict[str, LLMClient]):
        ...
        self._content_type_weights = {
            ...
            "image": {"accuracy": 0.9, "latency": 0.5, "cost": 0.3},
            "video": {"accuracy": 0.9, "latency": 0.4, "cost": 0.2},
            "audio": {"accuracy": 0.8, "latency": 0.7, "cost": 0.5},
            "multimodal": {"accuracy": 0.95, "latency": 0.3, "cost": 0.1},
        }
    ...
    def _generate_routing_cache_key(
        self, messages: Sequence[dict[str, Any]], context: dict[str, Any] | None = None, content_type: str | None = None
    ) -> str:
    ...
    def select_model_for_content_type(
        ...
        logger.info(
            f"Selected model '{best_model}' for {content_type} content (complexity: {task_complexity}, quality: {effective_quality_threshold:.2f})"
        )
