...
MCP_SERVERS = {
    ...
    "a2a": {"tools": ["a2a_call_tool"], "resources": ["a2a://skills", "a2a://skills_full"], "enabled": False},
    "crewai": {
        "tools": [
...
class MCPToolsValidator:
    ...
    async def validate_tool(
        ...
        try:
            ...
            return ValidationResult(
                tool_name=tool_name, server_name=server_name, success=False, latency_ms=latency, error_message=str(e)
            )
        ...
        try:
            if tool_name == "kg_query_tool":
                ...
                return StepResult.ok(
                    data={
                        "timeline": [
                            {"timestamp": "2024-01-01T00:00:00Z", "event": f"Event 1 for {entity}", "confidence": 0.9},
                            {"timestamp": "2024-01-02T00:00:00Z", "event": f"Event 2 for {entity}", "confidence": 0.8},
                        ],
                        "entity": entity,
                    }
                )
    ...
    async def _test_ingest_server_tool(self, tool_name: str, params: dict[str, Any]) -> StepResult:
        ...
        try:
            if tool_name == "extract_metadata_tool":
                ...
                return StepResult.ok(
                    data={"channel_url": channel_url, "videos": videos, "total_found": len(videos), "limit": limit}
                )
    ...
    async def _test_http_server_tool(self, tool_name: str, params: dict[str, Any]) -> StepResult:
        ...
        try:
            if tool_name == "http_get_tool":
                ...
                return StepResult.ok(
                    data={"url": url, "status_code": 200, "json_data": {"message": "JSON response", "success": True}}
                )
            else:
    ...
    async def validate_all_servers(self) -> MCPValidationReport:
        ...
        for server_name in enabled_servers:
            ...
            if server_result.tools_failed > 0:
                recommendations.append(
                    f"Server {server_name} has {server_result.tools_failed} failed tools. Check configuration and dependencies."
                )
            if server_result.total_latency_ms > 1000:
                recommendations.append(
                    f"Server {server_name} has high latency ({server_result.total_latency_ms:.1f}ms). Consider performance optimization."
                )
        ...
        if overall_success_rate < 80:
            recommendations.append(
                f"Overall success rate is {overall_success_rate:.1f}%. Consider enabling more MCP servers or fixing failed tools."
            )
        ...
        logger.info(
            f"MCP validation completed: {validated_tools}/{total_tools} tools validated ({overall_success_rate:.1f}% success rate)"
        )
        return report
    ...
    async def validate_research_workflows(self) -> StepResult:
        ...
        logger.info(
            f"Research workflows validation completed: {successful_workflows}/{total_workflows} workflows successful ({workflow_success_rate:.1f}% success rate)"
        )
        return StepResult.ok(
            data={
