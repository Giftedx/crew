...
class GraphRAGMemory:
    ...
    async def build_knowledge_graph(self, content: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(result={'entities_count': len(entities), 'relationships_count': len(relationships), 'communities_count': len(communities), 'community_summaries': summaries}, metadata={'graph_key': graph_key})
    ...
    async def query_graph(self, query: str, max_hops: int=2) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(result={'entities': relevant_entities, 'graph_context': context, 'community_context': community_context})
        except Exception as e:
            return StepResult.fail(error=str(e), error_category=ErrorCategory.MEMORY_ERROR)
