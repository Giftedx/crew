...
class RateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        self._stats = {'total_requests': 0, 'rate_limited_requests': 0, 'warnings_issued': 0, 'actions_taken': 0, 'cooldowns_activated': 0, 'avg_check_time_ms': 0.0}
        self._cleanup_task = asyncio.create_task(self._cleanup_loop())
        if config.enable_default_rules:
            self._setup_default_rules()

    async def check_rate_limit(self, user_id: str, guild_id: str | None=None, channel_id: str | None=None, request_type: str='message') -> StepResult:
        ...
        try:
            ...
            for rule in sorted(self._rules, key=lambda r: r.priority, reverse=True):
                ...
                if not rule_result.allowed:
                    ...
                    return StepResult.fail(f'Rate limit exceeded for {rule.name}', data={'rate_limit_result': rule_result})
    ...
    async def _check_rule(self, rule: RateLimitRule, user_id: str, guild_id: str | None, channel_id: str | None) -> RateLimitResult:
        ...
        return RateLimitResult(allowed=allowed, remaining_requests=remaining_requests, reset_time=reset_time, action_required=action_required, warning_issued=warning_issued, metadata={'rule_name': rule.name, 'scope': rule.scope.value, 'current_requests': current_requests, 'max_requests': rule.max_requests, 'window_seconds': rule.window_seconds})
    ...
    async def _take_action(self, action: RateLimitAction, user_id: str, guild_id: str | None, channel_id: str | None):
        ...
        {'type': 'rate_limit_cooldown', 'user_id': user_id, 'guild_id': guild_id, 'channel_id': channel_id, 'action_type': action.value, 'duration': cooldown_duration, 'reason': f'Rate limit exceeded: {action.value}', 'timestamp': time.time()}
    ...
    async def _issue_warning(self, user_id: str, guild_id: str | None, channel_id: str | None, rule: RateLimitRule):
        ...
        """
        {'type': 'rate_limit_warning', 'user_id': user_id, 'guild_id': guild_id, 'channel_id': channel_id, 'content': f'⚠️ Warning: You are approaching the rate limit for {rule.name}. Please slow down.', 'timestamp': time.time()}
    ...
    def _cleanup_old_actions(self, current_time: float):
        ...
        for user_id in list(self._user_actions.keys()):
            ...
            self._user_actions[user_id] = [(timestamp, action) for timestamp, action in actions if timestamp > cutoff_time]
        ...
        for guild_id in list(self._guild_actions.keys()):
            ...
            self._guild_actions[guild_id] = [(timestamp, action) for timestamp, action in actions if timestamp > cutoff_time]
        ...
        for channel_id in list(self._channel_actions.keys()):
            ...
            self._channel_actions[channel_id] = [(timestamp, action) for timestamp, action in actions if timestamp > cutoff_time]
    ...
    def _setup_default_rules(self):
        ...
        default_rules = [RateLimitRule(name='user_messages_per_minute', scope=RateLimitScope.USER, max_requests=self.config.user_messages_per_minute, window_seconds=60, action=RateLimitAction.TEMP_MUTE, action_duration_seconds=300, priority=100), RateLimitRule(name='user_messages_per_hour', scope=RateLimitScope.USER, max_requests=self.config.user_messages_per_hour, window_seconds=3600, action=RateLimitAction.TEMP_MUTE, action_duration_seconds=1800, priority=90), RateLimitRule(name='guild_messages_per_minute', scope=RateLimitScope.GUILD, max_requests=self.config.guild_messages_per_minute, window_seconds=60, action=RateLimitAction.WARN, priority=80), RateLimitRule(name='channel_messages_per_minute', scope=RateLimitScope.CHANNEL, max_requests=self.config.channel_messages_per_minute, window_seconds=60, action=RateLimitAction.WARN, priority=70)]
    ...
    def get_user_status(self, user_id: str) -> dict[str, Any]:
        ...
        return {'user_id': user_id, 'requests_last_hour': len(user_requests), 'in_cooldown': user_id in self._user_cooldowns, 'cooldown_until': self._user_cooldowns.get(user_id, None), 'recent_actions': [{'timestamp': timestamp, 'action': action.value} for timestamp, action in self._user_actions.get(user_id, []) if timestamp > current_time - 3600]}
