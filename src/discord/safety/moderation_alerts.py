...
class ModerationAlertManager:
    ...
    def __init__(self, config: AlertConfig):
        ...
        self._stats = {'total_alerts_generated': 0, 'alerts_by_type': defaultdict(int), 'alerts_by_severity': defaultdict(int), 'alerts_resolved': 0, 'alerts_escalated': 0, 'auto_actions_taken': 0, 'avg_resolution_time_minutes': 0.0}
        self._cleanup_task = asyncio.create_task(self._cleanup_loop())

    async def create_alert(self, alert_type: AlertType, severity: AlertSeverity, user_id: str, content: str, guild_id: str | None=None, channel_id: str | None=None, message_id: str | None=None, metadata: dict[str, Any] | None=None) -> StepResult:
        ...
        try:
            ...
            alert = ModerationAlert(alert_id=alert_id, alert_type=alert_type, severity=severity, user_id=user_id, guild_id=guild_id, channel_id=channel_id, message_id=message_id, content=content, timestamp=time.time(), metadata=metadata or {}, status=AlertStatus.PENDING)
    ...
    async def _execute_auto_mute(self, alert: ModerationAlert) -> StepResult:
        ...
        """
        action = {'type': 'mute', 'user_id': alert.user_id, 'guild_id': alert.guild_id, 'reason': f'Auto-mute: {alert.alert_type.value} (Alert {alert.alert_id})', 'duration_seconds': 3600, 'timestamp': time.time()}
    ...
    async def _execute_auto_kick(self, alert: ModerationAlert) -> StepResult:
        ...
        """
        action = {'type': 'kick', 'user_id': alert.user_id, 'guild_id': alert.guild_id, 'reason': f'Auto-kick: {alert.alert_type.value} (Alert {alert.alert_id})', 'timestamp': time.time()}
        if 'pending_actions' not in alert.metadata:
            alert.metadata['pending_actions'] = []
        alert.metadata['pending_actions'].append(action)
        return StepResult.ok(data={'action': action, 'status': 'queued'})
    ...
    async def _execute_auto_ban(self, alert: ModerationAlert) -> StepResult:
        ...
        """
        action = {'type': 'ban', 'user_id': alert.user_id, 'guild_id': alert.guild_id, 'reason': f'Auto-ban: Repeat offenses ({len(self._user_alerts.get(alert.user_id, []))} alerts)', 'delete_message_days': 1, 'timestamp': time.time()}
        if 'pending_actions' not in alert.metadata:
            alert.metadata['pending_actions'] = []
        alert.metadata['pending_actions'].append(action)
        return StepResult.ok(data={'action': action, 'status': 'queued'})
    ...
    async def _send_notifications(self, alert: ModerationAlert):
        ...
        if self.config.notify_moderators and alert.guild_id:
            notifications.append({'type': 'moderator_channel', 'guild_id': alert.guild_id, 'embed': self._create_alert_embed(alert)})
        if self.config.notify_admins and alert.severity in [AlertSeverity.CRITICAL, AlertSeverity.HIGH]:
            notifications.append({'type': 'admin_channel', 'guild_id': alert.guild_id, 'embed': self._create_alert_embed(alert), 'mention_admins': True})
        if self.config.notify_webhook and self.config.webhook_url:
            notifications.append({'type': 'webhook', 'url': self.config.webhook_url, 'payload': self._serialize_alert(alert)})
    ...
    def _create_alert_embed(self, alert: ModerationAlert) -> dict[str, Any]:
        ...
        """
        severity_colors = {AlertSeverity.LOW: 9807270, AlertSeverity.MEDIUM: 15965202, AlertSeverity.HIGH: 15158332, AlertSeverity.CRITICAL: 10038562}
        return {'title': f'🚨 Moderation Alert: {alert.alert_type.value.replace('_', ' ').title()}', 'description': alert.content[:1000], 'color': severity_colors.get(alert.severity, 9807270), 'fields': [{'name': 'Severity', 'value': alert.severity.value.upper(), 'inline': True}, {'name': 'User', 'value': f'<@{alert.user_id}>', 'inline': True}, {'name': 'Alert ID', 'value': alert.alert_id, 'inline': True}, {'name': 'Status', 'value': alert.status.value, 'inline': True}], 'timestamp': alert.timestamp, 'footer': {'text': 'Moderation Alert System'}}

    def _serialize_alert(self, alert: ModerationAlert) -> dict[str, Any]:
        """Serialize alert for API responses."""
        return {'alert_id': alert.alert_id, 'alert_type': alert.alert_type.value, 'severity': alert.severity.value, 'user_id': alert.user_id, 'guild_id': alert.guild_id, 'channel_id': alert.channel_id, 'message_id': alert.message_id, 'content': alert.content[:500], 'timestamp': alert.timestamp, 'status': alert.status.value, 'assigned_moderator': alert.assigned_moderator, 'resolution_notes': alert.resolution_notes, 'resolution_timestamp': alert.resolution_timestamp, 'metadata': alert.metadata}

    def _severity_priority(self, severity: str) -> int:
        """Get priority number for severity (lower = higher priority)."""
        priorities = {AlertSeverity.CRITICAL.value: 0, AlertSeverity.HIGH.value: 1, AlertSeverity.MEDIUM.value: 2, AlertSeverity.LOW.value: 3}
    ...
    def _update_avg_resolution_time(self, resolution_time: float):
        ...
        if resolved_count > 0:
            self._stats['avg_resolution_time_minutes'] = (current_avg * (resolved_count - 1) + resolution_minutes) / resolved_count
        else:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        stats['total_active_alerts'] = len([alert for alert in self._alerts.values() if alert.status == AlertStatus.PENDING])
        stats['total_resolved_alerts'] = len([alert for alert in self._alerts.values() if alert.status == AlertStatus.RESOLVED])
        return stats
