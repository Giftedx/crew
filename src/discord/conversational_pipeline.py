...
class ConversationalPipeline:
    ...
    def __init__(self, routing_manager: AdaptiveRoutingManager, prompt_engine: PromptEngine, memory_service: MemoryService, learning_engine: Any, config: PipelineConfig | None=None):
    ...
    async def process_message(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]]) -> StepResult[PipelineResult]:
        ...
        try:
            ...
            if not user_opt_in_status and (not self._is_direct_mention(message_data)):
                return StepResult.ok(data=PipelineResult(should_respond=False, reasoning='User has not opted in to AI interactions'))
            personality_context = await self._build_personality_context(message_data, recent_messages)
            pipeline_context = PipelineContext(message_data=message_data, guild_id=guild_id, channel_id=channel_id, user_id=user_id, recent_messages=recent_messages, user_opt_in_status=user_opt_in_status, personality_context=personality_context)
            evaluation_result = await self.message_evaluator.evaluate_discord_message(message_data, recent_messages, user_opt_in_status)
            ...
            if not evaluation.should_respond:
                return StepResult.ok(data=PipelineResult(should_respond=False, confidence=evaluation.confidence, reasoning=evaluation.reasoning, processing_time=time.time() - start_time))
            ...
            if cost_estimate > self.config.max_cost_per_interaction:
                ...
                return StepResult.ok(data=PipelineResult(should_respond=False, reasoning='Interaction cost exceeds threshold', cost_actual=cost_estimate, processing_time=time.time() - start_time))
            ...
            response_result = await self._generate_response(pipeline_context, evaluation, memory_context, personality_traits)
            ...
            return StepResult.ok(data=PipelineResult(should_respond=True, response_content=final_response, confidence=evaluation.confidence, reasoning=evaluation.reasoning, personality_traits=personality_traits, memory_retrieved=memory_context, cost_actual=cost_estimate, processing_time=processing_time))
    ...
    def _is_direct_mention(self, message_data: dict[str, Any]) -> bool:
        ...
        return '@bot' in content or 'hey bot' in content or 'bot,' in content or (message_data.get('referenced_message') is not None)

    async def _build_personality_context(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]]) -> PersonalityContext:
        ...
        try:
            ...
            return PersonalityContext(channel_type=channel_type, time_of_day=time_of_day, user_history=user_history, message_sentiment=message_sentiment, conversation_length=conversation_length, guild_culture=guild_culture)
        except Exception as e:
            logger.error(f'Failed to build personality context: {e}')
            return PersonalityContext(channel_type='general', time_of_day='afternoon', user_history=np.zeros(10), message_sentiment=0.0, conversation_length=1, guild_culture='casual')
    ...
    async def _estimate_interaction_cost(self, context: PipelineContext, evaluation: EvaluationResult) -> float:
        ...
        try:
            ...
            cost_result = await self.routing_manager.estimate_cost(model=evaluation.suggested_personality_traits.get('model', 'gpt-4o-mini'), input_tokens=int(input_tokens), output_tokens=int(output_tokens))
    ...
    async def _retrieve_memory_context(self, context: PipelineContext) -> list[dict[str, Any]]:
        ...
        try:
            if not self.config.memory_retrieval_enabled:
                return []
            message_content = context.message_data.get('content', '')
            result = await self.memory_service.search_memories(query=message_content, tenant=context.guild_id, workspace='general', limit=5)
    ...
    async def _get_personality_traits(self, guild_id: str, personality_context: PersonalityContext) -> dict[str, float]:
        ...
        try:
            if guild_id not in self._guild_personalities:
                ...
                await self._guild_personalities[guild_id].load_personality(tenant=guild_id, workspace='discord_personality')
            ...
            return {'humor': 0.5, 'formality': 0.5, 'enthusiasm': 0.7, 'knowledge_confidence': 0.8, 'debate_tolerance': 0.6, 'empathy': 0.7, 'creativity': 0.6, 'directness': 0.5}
    ...
    async def _generate_response(self, context: PipelineContext, evaluation: EvaluationResult, memory_context: list[dict[str, Any]], personality_traits: dict[str, float]) -> StepResult[str]:
        ...
        try:
            ...
            model_suggestion = await self.routing_manager.suggest_model(task_type='response_generation', context={'prompt_length': len(prompt), 'personality_traits': personality_traits, 'complexity': 'medium'})
            if not model_suggestion.success:
                return StepResult.fail(f'Failed to get model suggestion: {model_suggestion.error}')
            response_result = await self.prompt_engine.generate_response(prompt=prompt, model=model_suggestion.data.get('model'), max_tokens=self.config.max_response_length, temperature=0.7)
    ...
    async def _build_response_prompt(self, context: PipelineContext, evaluation: EvaluationResult, memory_context: list[dict[str, Any]], personality_traits: dict[str, float]) -> str:
        ...
        if context.recent_messages:
            recent_text = '\n'.join([f'{msg.get('author', {}).get('username', 'User')}: {msg.get('content', '')}' for msg in context.recent_messages[-5:]])
    ...
    async def _store_interaction(self, context: PipelineContext, evaluation: EvaluationResult, response: str) -> None:
        ...
        try:
            interaction_data = {'message_id': context.message_data.get('id'), 'user_id': context.user_id, 'guild_id': context.guild_id, 'channel_id': context.channel_id, 'message_content': context.message_data.get('content'), 'bot_response': response, 'evaluation': evaluation.reasoning, 'personality_traits': evaluation.suggested_personality_traits, 'timestamp': time.time()}
            await self.memory_service.store_memory(content=f'Discord interaction: {context.message_data.get('content', '')}', metadata=interaction_data, tenant=context.guild_id, workspace='discord_interactions')
            await self.opt_in_manager.record_interaction(user_id=context.user_id, guild_id=context.guild_id, message_id=context.message_data.get('id'), interaction_type='ai_response', content=context.message_data.get('content'), bot_response=response)
        except Exception as e:
            logger.error(f'Failed to store interaction: {e}')

    async def _monitor_interaction_rewards(self, context: PipelineContext, evaluation: EvaluationResult, response: str) -> None:
        """Monitor interaction for reward signals and update personality."""
        try:
            if not self.config.personality_adaptation_enabled:
                return
            metrics = InteractionMetrics(message_id=context.message_data.get('id', ''), user_id=context.user_id, guild_id=context.guild_id, timestamp=time.time())
            ...
            if reward_result.success:
                ...
                if personality_manager:
                    await personality_manager.adapt_personality(context=context.personality_context, reward=reward.total_reward, tenant=context.guild_id, workspace='discord_personality')
        except Exception as e:
    ...
    async def get_pipeline_status(self, guild_id: str) -> StepResult[dict[str, Any]]:
        ...
        try:
            status = {'guild_id': guild_id, 'config': self.config.__dict__, 'personality_loaded': guild_id in self._guild_personalities}
            ...
            if guild_id in self._guild_personalities:
                personality_summary = await self._guild_personalities[guild_id].get_personality_summary(tenant=guild_id, workspace='discord_personality')
