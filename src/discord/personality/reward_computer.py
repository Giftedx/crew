...
class InteractionMetrics:
    ...
    time_to_first_reply: float | None = None
...
class RewardComputer:
    ...
    def __init__(self):
        ...
        self.thresholds = {'quick_reply': 60.0, 'good_reaction': 0.6, 'long_conversation': 300.0, 'min_message_length': 10, 'good_response_length': 50}
        self.positive_reactions = {'👍', '❤️', '😊', '😂', '🔥', '✨', '💯', '🎉', 'thumbsup', 'heart', 'smile', 'joy', 'fire', 'sparkles', '100', 'tada'}
        self.negative_reactions = {'👎', '😞', '😠', '💔', 'thumbsdown', 'disappointed', 'angry', 'broken_heart'}

    async def compute_reward(self, metrics: InteractionMetrics, context: dict[str, Any] | None=None) -> StepResult[RewardSignal]:
        ...
        try:
            ...
            total_reward = engagement_reward * self.weights['engagement'] + reaction_reward * self.weights['reaction'] + continuation_reward * self.weights['continuation'] + temporal_reward * self.weights['temporal'] + content_reward * self.weights['content']
            confidence = self._compute_confidence(metrics)
            reasoning = self._generate_reasoning(metrics, engagement_reward, reaction_reward, continuation_reward, temporal_reward, content_reward)
            reward_signal = RewardSignal(total_reward=total_reward, engagement_reward=engagement_reward, reaction_reward=reaction_reward, continuation_reward=continuation_reward, temporal_reward=temporal_reward, content_reward=content_reward, confidence=confidence, reasoning=reasoning, metrics=metrics)
    ...
    def _compute_continuation_reward(self, metrics: InteractionMetrics) -> float:
        ...
        try:
            reward = 0.0
            if metrics.time_to_first_reply is not None and metrics.time_to_first_reply <= self.thresholds['quick_reply']:
                reward += 0.6
            if metrics.user_replies > 1:
                reward += min(0.4, 0.2 * (metrics.user_replies - 1))
            if metrics.conversation_duration is not None and metrics.conversation_duration >= self.thresholds['long_conversation']:
                reward += 0.3
            return min(1.0, reward)
        except Exception as e:
    ...
    def _compute_temporal_reward(self, metrics: InteractionMetrics) -> float:
        ...
        try:
            ...
            if time_since_interaction < 3600:
                reward += 0.5
            elif time_since_interaction < 86400:
                reward += 0.3
            else:
                reward += 0.1
            import datetime
    ...
    def _compute_content_reward(self, metrics: InteractionMetrics) -> float:
        ...
        try:
            ...
            if metrics.message_length > 0 and metrics.response_length > 0:
                length_ratio = min(metrics.response_length / metrics.message_length, metrics.message_length / metrics.response_length)
    ...
    def _generate_reasoning(self, metrics: InteractionMetrics, engagement_reward: float, reaction_reward: float, continuation_reward: float, temporal_reward: float, content_reward: float) -> str:
        ...
        try:
            ...
            for metrics in metrics_list:
                ...
                if reward_result.success:
                    ...
                    neutral_reward = RewardSignal(total_reward=0.0, engagement_reward=0.0, reaction_reward=0.0, continuation_reward=0.0, temporal_reward=0.0, content_reward=0.0, confidence=0.0, reasoning='Failed to compute reward', metrics=metrics)
    ...
    def get_reward_statistics(self, rewards: list[RewardSignal]) -> dict[str, Any]:
        ...
        try:
            ...
            return {'total_interactions': len(rewards), 'average_total_reward': sum(total_rewards) / len(total_rewards), 'average_engagement_reward': sum(engagement_rewards) / len(engagement_rewards), 'average_reaction_reward': sum(reaction_rewards) / len(reaction_rewards), 'average_continuation_reward': sum(continuation_rewards) / len(continuation_rewards), 'high_reward_interactions': sum((1 for r in total_rewards if r > 0.7)), 'low_reward_interactions': sum((1 for r in total_rewards if r < 0.3)), 'average_confidence': sum((r.confidence for r in rewards)) / len(rewards)}
        except Exception as e:
            logger.error(f'Failed to compute reward statistics: {e}')
            return {}

    async def monitor_ongoing_interaction(self, message_id: str, user_id: str, guild_id: str) -> StepResult[InteractionMetrics]:
        """Monitor an ongoing interaction for reward computation."""
        try:
            current_time = time.time()
            metrics = InteractionMetrics(message_id=message_id, user_id=user_id, guild_id=guild_id, timestamp=current_time)
