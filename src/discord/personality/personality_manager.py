...
class PersonalityTraits:
    ...
    def to_vector(self) -> np.ndarray:
        ...
        return np.array([self.humor, self.formality, self.enthusiasm, self.knowledge_confidence, self.debate_tolerance, self.empathy, self.creativity, self.directness])
    ...
    def from_vector(cls, vector: np.ndarray) -> PersonalityTraits:
        ...
        return cls(humor=float(clamped[0]), formality=float(clamped[1]), enthusiasm=float(clamped[2]), knowledge_confidence=float(clamped[3]), debate_tolerance=float(clamped[4]), empathy=float(clamped[5]), creativity=float(clamped[6]), directness=float(clamped[7]))

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> PersonalityTraits:
        """Create from dictionary."""
        return cls(humor=float(data.get('humor', 0.5)), formality=float(data.get('formality', 0.5)), enthusiasm=float(data.get('enthusiasm', 0.7)), knowledge_confidence=float(data.get('knowledge_confidence', 0.8)), debate_tolerance=float(data.get('debate_tolerance', 0.6)), empathy=float(data.get('empathy', 0.7)), creativity=float(data.get('creativity', 0.6)), directness=float(data.get('directness', 0.5)))
...
class PersonalityContext:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'channel_type': self.channel_type, 'time_of_day': self.time_of_day, 'user_history': self.user_history.tolist(), 'message_sentiment': self.message_sentiment, 'conversation_length': self.conversation_length, 'guild_culture': self.guild_culture}
...
class PersonalityStateManager:
    ...
    async def load_personality(self, tenant: str, workspace: str) -> StepResult[PersonalityTraits]:
        ...
        try:
            result = await self.memory_service.search_memories(query='personality_traits latest', tenant=tenant, workspace=workspace, limit=1)
    ...
    async def _load_personality_history(self, tenant: str, workspace: str) -> None:
        ...
        try:
            result = await self.memory_service.search_memories(query='personality_evolution', tenant=tenant, workspace=workspace, limit=50)
            if result.success and result.data.get('memories'):
    ...
    async def _save_personality(self, tenant: str, workspace: str, traits: PersonalityTraits) -> None:
        ...
        try:
            import time
            timestamp = time.time()
            await self.memory_service.store_memory(content=f'Personality traits: {traits.to_dict()}', metadata={'traits': traits.to_dict(), 'timestamp': timestamp, 'type': 'personality_state'}, tenant=tenant, workspace=workspace)
            await self.memory_service.store_memory(content=f'Personality evolution at {timestamp}', metadata={'traits': traits.to_dict(), 'timestamp': timestamp, 'type': 'personality_evolution'}, tenant=tenant, workspace='personality_history')
            self._personality_history.append((timestamp, traits))
    ...
    async def adapt_personality(self, context: PersonalityContext, reward: float, tenant: str, workspace: str) -> StepResult[PersonalityTraits]:
    ...
    async def _get_rl_recommendation(self, context: PersonalityContext) -> StepResult[dict[str, Any]]:
        ...
        try:
            rl_context = {'channel_type': context.channel_type, 'time_of_day': context.time_of_day, 'message_sentiment': context.message_sentiment, 'conversation_length': context.conversation_length, 'guild_culture': context.guild_culture}
            ...
            recommendation = self.learning_engine.recommend(domain=self.rl_domain, context=rl_context, candidates=['adjust_humor', 'adjust_formality', 'adjust_enthusiasm', 'adjust_confidence', 'adjust_empathy', 'maintain_current'])
    ...
    def _apply_personality_adjustment(self, current_traits: PersonalityTraits, action: dict[str, Any], context: PersonalityContext) -> PersonalityTraits:
        """Apply personality adjustment based on RL action."""
        try:
            new_traits = PersonalityTraits(humor=current_traits.humor, formality=current_traits.formality, enthusiasm=current_traits.enthusiasm, knowledge_confidence=current_traits.knowledge_confidence, debate_tolerance=current_traits.debate_tolerance, empathy=current_traits.empathy, creativity=current_traits.creativity, directness=current_traits.directness)
    ...
    async def get_personality_summary(self, tenant: str, workspace: str) -> StepResult[dict[str, Any]]:
        ...
        try:
            ...
            summary = {'current_traits': self.current_traits.to_dict(), 'trait_statistics': trait_stats, 'personality_drift': drift_analysis, 'recent_adaptations': recent_adaptations, 'total_adaptations': len(self._personality_history), 'rl_domain': self.rl_domain}
    ...
    def _analyze_personality_drift(self) -> dict[str, Any]:
        ...
        try:
            ...
            trait_changes = {}
            trait_names = ['humor', 'formality', 'enthusiasm', 'knowledge_confidence', 'debate_tolerance', 'empathy', 'creativity', 'directness']
            for i, trait_name in enumerate(trait_names):
                trait_changes[trait_name] = float(drift_vector[i])
            max_change_trait = max(trait_changes.items(), key=lambda x: abs(x[1]))
            return {'drift_magnitude': drift_magnitude, 'drift_direction': direction, 'trait_changes': trait_changes, 'max_change_trait': max_change_trait[0], 'max_change_value': max_change_trait[1]}
        except Exception as e:
    ...
    def _calculate_trait_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            for i, trait_name in enumerate(trait_names):
                ...
                stats[trait_name] = {'mean': float(np.mean(trait_values)), 'std': float(np.std(trait_values)), 'min': float(np.min(trait_values)), 'max': float(np.max(trait_values)), 'current': float(self.current_traits.to_vector()[i])}
            return stats
        except Exception as e:
