...
class HierarchicalReasoningEngine:
    ...
    def __init__(self, routing_manager: AdaptiveRoutingManager, prompt_engine: PromptEngine, memory_service: MemoryService, retrieval_engine: UnifiedRetrievalEngine | None=None, context_builder: UnifiedContextBuilder | None=None):
    ...
    async def reason(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]], user_opt_in_status: bool) -> StepResult[ReasoningResult]:
        ...
        try:
            ...
            if reasoning_ctx.interpreted_tokens and reasoning_ctx.interpreted_tokens.action.action_type == 'ignore':
                return StepResult.ok(data=ReasoningResult(should_respond=False, confidence=1.0, reasoning_steps=reasoning_ctx.reasoning_trace, recommended_action='ignore', priority=0, context_summary='Message marked for ignoring'))
            ...
            if reasoning_ctx.interpreted_tokens:
                decision_tree_result = await self.decision_tree.evaluate(interpreted_tokens=reasoning_ctx.interpreted_tokens, message_metadata={'mentions': message_data.get('mentions', []), 'user_opt_in_status': user_opt_in_status})
                if decision_tree_result.success:
                    tree_path = decision_tree_result.data
                    decision = {'should_respond': tree_path.final_decision.get('action') != 'skip_response', 'confidence': tree_path.confidence, 'reasoning': tree_path.reasoning, 'used_llm': tree_path.used_llm}
                    reasoning_ctx.reasoning_trace.append({'layer': 'decision_making', 'method': 'adaptive_decision_tree', 'path': tree_path.nodes_visited, 'decision': decision})
                else:
            ...
            reasoning_result = ReasoningResult(should_respond=decision.get('should_respond', False), confidence=decision.get('confidence', 0.0), reasoning_steps=reasoning_ctx.reasoning_trace, recommended_action=reasoning_ctx.interpreted_tokens.action.action_type if reasoning_ctx.interpreted_tokens else 'respond', priority=reasoning_ctx.interpreted_tokens.action.priority if reasoning_ctx.interpreted_tokens else 0, context_summary=self._build_context_summary(reasoning_ctx), requires_crewmai=reasoning_ctx.interpreted_tokens.action.requires_crewmai if reasoning_ctx.interpreted_tokens else False, suggested_agents=reasoning_ctx.interpreted_tokens.action.suggested_agents if reasoning_ctx.interpreted_tokens else [])
    ...
    async def _preprocess_layer(self, message_data: dict[str, Any], ctx: ReasoningContext) -> StepResult:
        ...
        try:
            ...
            ctx.reasoning_trace.append({'layer': 'preprocessing', 'action': 'token_interpretation', 'intent': interpreted.intent.primary_intent, 'confidence': interpreted.confidence_score})
    ...
    async def _context_analysis_layer(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]], ctx: ReasoningContext, user_opt_in_status: bool) -> StepResult:
        ...
        try:
            ...
            if self.retrieval_engine:
                ...
                query = RetrievalQuery(text=content, intent=ctx.interpreted_tokens.intent.primary_intent, limit=10, min_confidence=0.5)
                retrieval_result = await self.retrieval_engine.retrieve(query=query, unified_memory_service=self.memory_service, tenant_id=guild_id, workspace_id='discord')
                if retrieval_result.success:
                    results = retrieval_result.data.get('results', [])
                    ctx.retrieved_context = [{'content': r.content if hasattr(r, 'content') else str(r), 'confidence': getattr(r, 'confidence', 0.5), 'source': getattr(r, 'source', 'memory')} for r in results]
            else:
                search_result = await self.memory_service.search_memories(query=content, tenant=guild_id, workspace='discord_interactions', limit=10)
                if search_result.success:
                    memories = search_result.data.get('memories', [])
                    ctx.retrieved_context = [{'content': mem.get('content', ''), 'confidence': mem.get('confidence', 0.5), 'source': 'memory'} for mem in memories]
            if self.context_builder:
                from ultimate_discord_intelligence_bot.knowledge.context_builder import ContextRequest
                context_request = ContextRequest(agent_id='discord_bot', task_type='conversation', query=content, current_context={'recent_messages': recent_messages, 'user_opt_in': user_opt_in_status}, max_context_length=4000)
                build_result = await self.context_builder.build_context(request=context_request, unified_memory_service=self.memory_service, retrieval_engine=self.retrieval_engine, tenant_id=guild_id, workspace_id='discord')
                if build_result.success:
                    ctx.unified_context = build_result.data
            ctx.reasoning_trace.append({'layer': 'context_analysis', 'action': 'memory_retrieval', 'contexts_retrieved': len(ctx.retrieved_context), 'has_unified_context': ctx.unified_context is not None})
    ...
    async def _decision_making_layer(self, message_data: dict[str, Any], ctx: ReasoningContext, user_opt_in_status: bool) -> StepResult[dict[str, Any]]:
        ...
        try:
            ...
            if not user_opt_in_status and tokens.context.urgency != 'critical':
                if not message_data.get('mentions') and tokens.action.priority < 7:
                    return StepResult.ok(data={'should_respond': False, 'confidence': 0.8, 'reasoning': 'User not opted in and message not high priority'})
            should_respond = True
            ...
            if tokens.intent.primary_intent in ['question', 'command', 'request']:
                ...
                confidence = 0.6
            ctx.reasoning_trace.append({'layer': 'decision_making', 'action': 'response_decision', 'should_respond': should_respond, 'confidence': confidence, 'factors': {'intent': tokens.intent.primary_intent, 'urgency': tokens.context.urgency, 'priority': tokens.action.priority, 'opt_in': user_opt_in_status}})
            ctx.confidence_scores['decision'] = confidence
            return StepResult.ok(data={'should_respond': should_respond, 'confidence': confidence, 'reasoning': f'Intent: {tokens.intent.primary_intent}, Urgency: {tokens.context.urgency}, Priority: {tokens.action.priority}'})
        except Exception as e:
    ...
    async def _response_generation_layer_planning(self, ctx: ReasoningContext, decision: dict[str, Any]) -> StepResult:
        ...
        try:
            ...
            ctx.reasoning_trace.append({'layer': 'response_generation_planning', 'action': 'strategy_selection', 'strategy': strategy, 'requires_crewmai': tokens.action.requires_crewmai, 'requires_knowledge': tokens.action.requires_knowledge})
