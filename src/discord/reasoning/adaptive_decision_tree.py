...
class AdaptiveDecisionTree:
    ...
    def _build_tree(self) -> None:
        ...
        self.nodes['root'] = DecisionNode(node_id='root', condition='always', next_nodes=['check_ignore', 'check_priority'])
        self.nodes['check_ignore'] = DecisionNode(node_id='check_ignore', condition="action_type == 'ignore'", action='skip_response', next_nodes=[])
        self.nodes['check_priority'] = DecisionNode(node_id='check_priority', condition='priority >= 8 AND is_direct_mention', action='respond_immediately', confidence=0.95, next_nodes=['llm_evaluate'])
        self.nodes['check_greeting'] = DecisionNode(node_id='check_greeting', condition="intent == 'greeting'", action='respond_friendly', confidence=0.9, next_nodes=[])
        self.nodes['check_low_priority'] = DecisionNode(node_id='check_low_priority', condition="intent == 'statement' AND priority < 3 AND NOT is_direct_mention", action='skip_response', confidence=0.8, next_nodes=[])
        self.nodes['llm_evaluate'] = DecisionNode(node_id='llm_evaluate', condition=None, requires_llm=True, next_nodes=[])

    async def evaluate(self, interpreted_tokens: InterpretedTokens, message_metadata: dict[str, Any]) -> StepResult[DecisionPath]:
        ...
        try:
            ...
            while current_node_id and current_node_id not in visited:
                ...
                if node.action and (not node.next_nodes):
                    path.final_decision = {'action': node.action, 'confidence': node.confidence, 'node_id': node.node_id}
                ...
                if node.requires_llm:
                    ...
                    if llm_result.success:
                        ...
                        path.final_decision = {'action': 'respond', 'confidence': 0.6, 'node_id': 'llm_evaluate_fallback'}
                        path.confidence = 0.6
    ...
    async def _llm_reasoning(self, tokens: InterpretedTokens, metadata: dict[str, Any]) -> StepResult[dict[str, Any]]:
        ...
        try:
            ...
            model_suggestion = await self.routing_manager.suggest_model(task_type='decision_making', context={'complexity': 'high', 'requires_reasoning': True})
            ...
            if hasattr(self.prompt_engine, 'generate_response'):
                result = await self.prompt_engine.generate_response(prompt=prompt, model=model, max_tokens=300, temperature=0.3)
                if result.success:
                    import json
                    ...
                    try:
                        ...
                        if json_start >= 0 and json_end > json_start:
                            ...
                            return StepResult.ok(data={'action': 'respond' if decision_data.get('should_respond') else 'skip', 'confidence': float(decision_data.get('confidence', 0.7)), 'reasoning': str(decision_data.get('reasoning', 'LLM reasoning')), 'priority': str(decision_data.get('recommended_priority', 'medium'))})
                    except json.JSONDecodeError as e:
                        logger.warning(f'Failed to parse LLM JSON response: {e}')
                        return StepResult.ok(data={'action': 'respond', 'confidence': 0.7, 'reasoning': 'LLM reasoning (parse error, using defaults)', 'priority': 'medium'})
                return StepResult.fail('LLM reasoning generation failed')
            else:
                return StepResult.ok(data={'action': 'respond', 'confidence': 0.7, 'reasoning': 'LLM reasoning unavailable, using default', 'priority': 'medium'})
        except Exception as e:
