...
class MessageContextBuilder:
    ...
    async def build_context(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]], user_opt_in_status: bool) -> MessageContext:
        ...
        try:
            ...
            return MessageContext(message_id=message_id, channel_id=channel_id, guild_id=guild_id, user_id=user_id, username=username, content=content, timestamp=timestamp, is_direct_mention=is_direct_mention, is_reply_to_bot=is_reply_to_bot, channel_type=channel_type, user_opt_in_status=user_opt_in_status, recent_messages=recent_messages, user_interaction_history=user_interaction_history)
    ...
    async def _get_user_interaction_history(self, user_id: str, guild_id: str) -> list[dict[str, Any]]:
        ...
        try:
            result = await self.memory_service.search_memories(query=f'user:{user_id} guild:{guild_id}', tenant=guild_id, workspace='discord_interactions', limit=10)
            if result.success:
...
class ResponseDecisionAgent:
    ...
    def __init__(self, routing_manager: AdaptiveRoutingManager, prompt_engine: PromptEngine, memory_service: MemoryService):
    ...
    async def evaluate_message(self, context: MessageContext) -> StepResult[EvaluationResult]:
        ...
        try:
            ...
            model_suggestion = await self.routing_manager.suggest_model(task_type='decision_making', context={'prompt_length': len(evaluation_prompt), 'complexity': 'medium', 'requires_reasoning': True})
    ...
    async def _get_relevant_context(self, context: MessageContext) -> str:
        ...
        try:
            result = await self.memory_service.search_memories(query=context.content, tenant=context.guild_id, workspace='general', limit=5)
    ...
    async def _generate_evaluation(self, prompt: str, model_info: dict[str, Any]) -> StepResult[str]:
        ...
        try:
            result = await self.prompt_engine.generate_response(prompt=prompt, model=model_info.get('model'), max_tokens=500, temperature=0.3)
            if result.success:
    ...
    def _parse_evaluation_result(self, llm_response: str, context: MessageContext) -> EvaluationResult:
        ...
        try:
            ...
            default_traits = {'humor': 0.5, 'formality': 0.5, 'enthusiasm': 0.7, 'knowledge_confidence': 0.8, 'debate_tolerance': 0.6}
            ...
            return EvaluationResult(should_respond=should_respond, confidence=confidence, reasoning=reasoning, priority=priority, suggested_personality_traits=personality_traits, context_relevance_score=context_relevance_score, estimated_cost=estimated_cost)
        except Exception as e:
            logger.error(f'Error parsing evaluation result: {e}')
            return EvaluationResult(should_respond=False, confidence=0.0, reasoning=f'Parse error: {e!s}', priority='low', suggested_personality_traits={'humor': 0.5, 'formality': 0.5, 'enthusiasm': 0.7, 'knowledge_confidence': 0.8, 'debate_tolerance': 0.6}, context_relevance_score=0.0, estimated_cost=0.0)
...
class MessageEvaluator:
    ...
    def __init__(self, routing_manager: AdaptiveRoutingManager, prompt_engine: PromptEngine, memory_service: MemoryService):
        ...
        try:
            ...
            self.hierarchical_reasoner = HierarchicalReasoningEngine(routing_manager=routing_manager, prompt_engine=prompt_engine, memory_service=memory_service, retrieval_engine=retrieval_engine, context_builder=context_builder)
    ...
    async def evaluate_discord_message(self, message_data: dict[str, Any], recent_messages: list[dict[str, Any]], user_opt_in_status: bool) -> StepResult[EvaluationResult]:
        """Evaluate whether bot should respond to Discord message."""
        try:
            if self._use_hierarchical_reasoning and self.hierarchical_reasoner:
                reasoning_result = await self.hierarchical_reasoner.reason(message_data=message_data, recent_messages=recent_messages, user_opt_in_status=user_opt_in_status)
                if reasoning_result.success:
                    reasoning = reasoning_result.data
                    personality_traits = {'humor': 0.5, 'formality': 0.5, 'enthusiasm': 0.7, 'knowledge_confidence': min(0.9, reasoning.confidence + 0.2), 'debate_tolerance': 0.6}
                    estimated_cost = 0.001 if reasoning.requires_crewmai else 0.0005
                    evaluation_result = EvaluationResult(should_respond=reasoning.should_respond, confidence=reasoning.confidence, reasoning=reasoning.context_summary, priority='high' if reasoning.priority >= 7 else 'medium' if reasoning.priority >= 4 else 'low', suggested_personality_traits=personality_traits, context_relevance_score=reasoning.confidence, estimated_cost=estimated_cost)
                    from discord.message_evaluator import MessageContext
                    context = MessageContext(message_id=str(message_data.get('id', '')), channel_id=str(message_data.get('channel_id', '')), guild_id=str(message_data.get('guild_id', '')), user_id=str(message_data.get('author', {}).get('id', '')), username=message_data.get('author', {}).get('username', 'unknown'), content=message_data.get('content', ''), timestamp=float(message_data.get('timestamp', 0)), is_direct_mention=bool(message_data.get('mentions')), is_reply_to_bot=False, channel_type='text', user_opt_in_status=user_opt_in_status, recent_messages=recent_messages, user_interaction_history=[])
    ...
    async def _store_evaluation(self, context: MessageContext, result: EvaluationResult) -> None:
        ...
        try:
            evaluation_data = {'message_id': context.message_id, 'user_id': context.user_id, 'guild_id': context.guild_id, 'content': context.content, 'should_respond': result.should_respond, 'confidence': result.confidence, 'reasoning': result.reasoning, 'priority': result.priority, 'personality_traits': result.suggested_personality_traits, 'context_relevance_score': result.context_relevance_score, 'estimated_cost': result.estimated_cost, 'timestamp': context.timestamp}
            await self.memory_service.store_memory(content=f'Message evaluation: {context.content}', metadata=evaluation_data, tenant=context.guild_id, workspace='message_evaluations')
        except Exception as e:
