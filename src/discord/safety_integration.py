...
from .safety import AlertConfig, AlertSeverity, AlertType, ContentFilterConfig, RateLimitConfig, SafetyManager, create_safety_manager
...
class DiscordSafetyIntegration:
    ...
    def __init__(self, safety_manager: SafetyManager | None=None):
        ...
        self._stats = {'total_messages_processed': 0, 'messages_blocked': 0, 'alerts_generated': 0, 'auto_actions_taken': 0, 'avg_processing_time_ms': 0.0}

    def _create_default_safety_manager(self) -> SafetyManager:
        """Create a safety manager with default configuration."""
        content_filter_config = ContentFilterConfig(low_risk_threshold=0.3, medium_risk_threshold=0.6, high_risk_threshold=0.8, critical_threshold=0.9, enable_regex_filtering=True, enable_ai_classification=True, enable_similarity_filtering=True, enable_rate_limiting=True, max_messages_per_minute=10, max_messages_per_hour=100, enable_caching=True, cache_size=1000, cache_ttl_seconds=3600, auto_delete_high_risk=False, auto_warn_users=True, log_moderation_actions=True)
        rate_limit_config = RateLimitConfig(enable_default_rules=True, user_messages_per_minute=10, user_messages_per_hour=100, user_messages_per_day=500, guild_messages_per_minute=1000, guild_messages_per_hour=10000, channel_messages_per_minute=100, channel_messages_per_hour=1000, global_messages_per_minute=10000, global_messages_per_hour=100000, warn_threshold=0.8, action_threshold=1.0, cooldown_seconds=60, escalation_cooldown_seconds=300, cleanup_interval_seconds=300, max_storage_time_seconds=86400)
        alert_config = AlertConfig(enable_alerts=True, auto_escalate_threshold=3, escalation_time_window=3600, notify_moderators=True, notify_admins=True, notify_webhook=True, max_alerts_per_user=100, max_alerts_per_guild=1000, alert_retention_days=30, max_alerts_per_minute=10, max_alerts_per_hour=100, auto_mute_on_escalation=True, auto_kick_on_critical=False, auto_ban_on_repeat_offenses=False, webhook_url=None, webhook_timeout=10)
    ...
    async def process_discord_message(self, content: str, user_id: str, guild_id: str | None=None, channel_id: str | None=None, message_id: str | None=None, message_author: dict[str, Any] | None=None, message_mentions: list[dict[str, Any]] | None=None) -> StepResult:
        """Process a Discord message through the safety pipeline."""
        try:
            if not self._initialized:
                return StepResult.fail('Safety integration not initialized')
            start_time = time.time()
            safety_result = await self.safety_manager.process_message(content=content, user_id=user_id, guild_id=guild_id, channel_id=channel_id, message_id=message_id)
            ...
            if not safety_result.success:
                ...
                return StepResult.fail(safety_result.error, data={'safety_check': 'failed', 'reason': safety_result.data.get('reason', 'unknown'), 'processing_time_ms': processing_time, 'safety_data': safety_result.data})
            processing_time = (time.time() - start_time) * 1000
            self._update_avg_processing_time(processing_time)
            return StepResult.ok(data={'safety_check': 'passed', 'processing_time_ms': processing_time, 'safety_data': safety_result.data})
        except Exception as e:
    ...
    async def get_user_safety_profile(self, user_id: str) -> StepResult:
        ...
        try:
            ...
            profile = {'user_id': user_id, 'safety_score': safety_status['safety_score'], 'rate_limit_status': safety_status['rate_limit_status'], 'recent_alerts': safety_status['recent_alerts'], 'content_filter_stats': safety_status['content_filter_stats'], 'recommendations': self._generate_user_recommendations(safety_status), 'risk_level': self._determine_risk_level(safety_status['safety_score'])}
    ...
    async def get_guild_moderation_dashboard(self, guild_id: str) -> StepResult:
        ...
        try:
            ...
            dashboard = {'guild_id': guild_id, 'safety_metrics': safety_overview['safety_metrics'], 'total_alerts': safety_overview['total_alerts'], 'pending_alerts': safety_overview['pending_alerts'], 'recent_alerts': safety_overview['recent_alerts'], 'pending_alerts_list': safety_overview['pending_alerts_list'], 'moderation_actions': await self._get_recent_moderation_actions(guild_id), 'top_offenders': await self._get_top_offenders(guild_id), 'alert_trends': await self._get_alert_trends(guild_id), 'recommendations': self._generate_guild_recommendations(safety_overview)}
    ...
    async def handle_moderation_action(self, action_type: str, user_id: str, moderator_id: str, guild_id: str | None=None, reason: str | None=None, duration: int | None=None) -> StepResult:
        ...
        try:
            ...
            alert_result = await self.safety_manager.alert_manager.create_alert(alert_type=alert_type, severity=severity, user_id=user_id, content=f'Moderation action: {action_type}', guild_id=guild_id, metadata={'action_type': action_type, 'moderator_id': moderator_id, 'reason': reason, 'duration': duration, 'manual_action': True})
            if alert_result.success:
                self._stats['auto_actions_taken'] += 1
                return StepResult.ok(data={'moderation_action_recorded': True, 'alert_id': alert_result.data.get('alert_id')})
            else:
    ...
    def _map_action_to_alert_type(self, action_type: str) -> AlertType:
        ...
        action_mapping = {'warn': AlertType.CONTENT_VIOLATION, 'mute': AlertType.RATE_LIMIT_EXCEEDED, 'kick': AlertType.HARASSMENT, 'ban': AlertType.HATE_SPEECH, 'timeout': AlertType.SPAM_DETECTED}
