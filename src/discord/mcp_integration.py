...
class MCPIntegrationLayer:
    ...
    async def search_memories(self, tenant: str, workspace: str, name: str, query: str, k: int=5, min_score: float | None=None) -> StepResult[dict[str, Any]]:
    ...
    async def get_memory_samples(self, tenant: str, workspace: str, name: str, probe: str='', n: int=3) -> StepResult[list[dict[str, Any]]]:
    ...
    async def ingest_youtube_video(self, url: str, tenant: str, workspace: str, fetch_transcript: bool=True) -> StepResult[dict[str, Any]]:
    ...
    async def query_creator_content(self, query_text: str, collection_type: str, tenant: str, workspace: str, limit: int=10, score_threshold: float=0.7) -> StepResult[list[dict[str, Any]]]:
        ...
        if not creator_server:
            return StepResult.fail('Creator Intelligence MCP server not initialized')
        try:
            result = await creator_server.query_creator_content(query_text, collection_type, tenant, workspace, limit, score_threshold)
            return result
        except Exception as e:
            logger.error(f'Creator content query failed: {e}')
            return StepResult.fail(f'Creator content query failed: {e!s}')

    async def get_collection_stats(self, collection_type: str, tenant: str, workspace: str) -> StepResult[dict[str, Any]]:
    ...
    async def get_integration_status(self) -> dict[str, Any]:
        ...
        server_checks = {'memory': self.feature_flags.ENABLE_MCP_MEMORY, 'kg': self.feature_flags.ENABLE_MCP_KG, 'crewai': self.feature_flags.ENABLE_MCP_CREWAI, 'routing': self.feature_flags.ENABLE_MCP_ROUTER, 'creator_intelligence': self.feature_flags.ENABLE_MCP_CREATOR_INTELLIGENCE}
        for server_name, enabled in server_checks.items():
            status['servers'][server_name] = {'enabled': enabled, 'initialized': server_name in self._mcp_servers, 'available': enabled and server_name in self._mcp_servers}
        return status