...
class PersonalityMetrics:
    ...
    trait_name: str
    current_value: float
    evolution_history: list[tuple[float, float]] = field(default_factory=list)  # (timestamp, value)
...
class DiscordMetricsCollector:
    ...
    async def record_personality_feedback(
        self,
        trait_name: str,
        feedback_value: float,  # -1.0 to 1.0 (negative to positive)
        user_id: str,
    ) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                active_users_24h = sum(
                    1
                    for user_metrics in self._user_metrics.values()
                    if current_time - user_metrics.last_active < 86400  # 24 hours
                )
