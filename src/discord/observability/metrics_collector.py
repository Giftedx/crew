...
class DiscordMetricsCollector:
    ...
    def __init__(self, max_history_size: int=10000):
        ...
        self._stats = {'total_conversations': 0, 'total_messages': 0, 'total_bot_responses': 0, 'avg_response_time_ms': 0.0, 'user_satisfaction_avg': 0.0, 'active_users_24h': 0, 'conversation_success_rate': 0.0}

    async def start_conversation(self, conversation_id: str, user_id: str, guild_id: str, channel_id: str, metadata: dict[str, Any] | None=None) -> StepResult:
        """Start tracking a new conversation."""
        try:
            async with self._lock:
                conversation = ConversationMetrics(conversation_id=conversation_id, user_id=user_id, guild_id=guild_id, channel_id=channel_id, start_time=time.time(), metadata=metadata or {})
    ...
    async def end_conversation(self, conversation_id: str, satisfaction_score: float | None=None, topics_discussed: list[str] | None=None) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                if conversation.bot_responses > 0:
                    conversation.avg_response_time_ms = (conversation.avg_response_time_ms * (conversation.bot_responses - 1) + (conversation.end_time - conversation.start_time) * 1000) / conversation.bot_responses
    ...
    async def record_message(self, conversation_id: str, user_id: str, message_type: str, response_time_ms: float | None=None, topic: str | None=None) -> StepResult:
                ...
                if message_type == 'bot':
                    ...
                    if response_time_ms is not None:
                        if conversation.avg_response_time_ms == 0:
                            ...
                            conversation.avg_response_time_ms = (conversation.avg_response_time_ms * (conversation.bot_responses - 1) + response_time_ms) / conversation.bot_responses
        ...
        try:
            async with self._lock:
                if trait_name not in self._personality_metrics:
                    self._personality_metrics[trait_name] = PersonalityMetrics(trait_name=trait_name, current_value=0.0, last_updated=time.time())
                ...
                return StepResult.ok(data={'trait_name': trait_name, 'feedback_recorded': True, 'total_feedback': trait_metrics.user_feedback_count})
        except Exception as e:
    ...
    async def update_personality_trait(self, trait_name: str, new_value: float) -> StepResult:
        ...
        try:
            async with self._lock:
                if trait_name not in self._personality_metrics:
                    self._personality_metrics[trait_name] = PersonalityMetrics(trait_name=trait_name, current_value=new_value, last_updated=time.time())
                else:
                    trait_metrics = self._personality_metrics[trait_name]
    ...
    async def _update_conversation_stats(self, conversation: ConversationMetrics):
        ...
        if conversation.avg_response_time_ms > 0:
            ...
            if total_responses > 0:
                self._stats['avg_response_time_ms'] = (self._stats['avg_response_time_ms'] * (total_responses - 1) + conversation.avg_response_time_ms) / total_responses
        if conversation.user_satisfaction_score is not None:
            satisfaction_scores = [c.user_satisfaction_score for c in self._conversation_history if c.user_satisfaction_score is not None]
    ...
    async def _update_user_engagement(self, conversation: ConversationMetrics):
        ...
        if user_id in self._user_metrics:
            ...
            if user_metrics.total_conversations > 0:
                ...
                user_metrics.avg_conversation_length = (user_metrics.avg_conversation_length * (user_metrics.total_conversations - 1) + conversation_length) / user_metrics.total_conversations
            if conversation.avg_response_time_ms > 0:
                if user_metrics.avg_response_time == 0:
                    user_metrics.avg_response_time = conversation.avg_response_time_ms
                else:
                    user_metrics.avg_response_time = (user_metrics.avg_response_time + conversation.avg_response_time_ms) / 2
            if conversation.user_satisfaction_score is not None:
        ...
        try:
            async with self._lock:
                ...
                if guild_metrics.total_conversations > 0:
                    ...
                    guild_metrics.peak_activity_hours = sorted(hour_activity.keys(), key=lambda h: hour_activity[h], reverse=True)[:5]
    ...
    async def get_comprehensive_stats(self) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                active_users_24h = sum((1 for user_metrics in self._user_metrics.values() if current_time - user_metrics.last_active < 86400))
                self._stats['active_users_24h'] = active_users_24h
                total_conversations = len(self._conversation_history)
                successful_conversations = sum((1 for conv in self._conversation_history if conv.user_satisfaction_score is not None and conv.user_satisfaction_score >= 0.5))
                if total_conversations > 0:
                    self._stats['conversation_success_rate'] = successful_conversations / total_conversations
                return StepResult.ok(data={'system_stats': self._stats, 'active_conversations': len(self._active_conversations), 'total_users': len(self._user_metrics), 'total_guilds': len(self._guild_metrics), 'personality_traits': len(self._personality_metrics), 'conversation_history_size': len(self._conversation_history)})
        except Exception as e:
    ...
    async def cleanup_old_data(self, max_age_hours: int=168) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                old_conversations = [conv for conv in self._conversation_history if current_time - conv.start_time > max_age_seconds]
                for conv in old_conversations:
                    self._conversation_history.remove(conv)
                inactive_users = [user_id for user_id, user_metrics in self._user_metrics.items() if current_time - user_metrics.last_active > max_age_seconds]
                for user_id in inactive_users:
                    del self._user_metrics[user_id]
                return StepResult.ok(data={'conversations_cleaned': len(old_conversations), 'users_cleaned': len(inactive_users)})
        except Exception as e:
