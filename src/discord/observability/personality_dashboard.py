...
class PersonalityDashboard:
    ...
    def __init__(self, max_history_size: int=10000):
        ...
        self._stats = {'total_traits_tracked': 0, 'total_snapshots': 0, 'active_users': 0, 'avg_user_satisfaction': 0.0, 'personality_evolution_rate': 0.0}

    async def record_trait_snapshot(self, trait_name: str, value: float, confidence: float=1.0, user_feedback: dict[str, Any] | None=None, context: dict[str, Any] | None=None) -> StepResult:
        """Record a snapshot of a personality trait."""
        try:
            async with self._lock:
                snapshot = PersonalityTraitSnapshot(trait_name=trait_name, value=value, timestamp=time.time(), confidence=confidence, context=context or {})
                if user_feedback:
    ...
    async def record_user_feedback(self, user_id: str, trait_name: str, feedback_value: float, context: dict[str, Any] | None=None) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                feedback_record = {'trait_name': trait_name, 'feedback_value': feedback_value, 'timestamp': time.time(), 'context': context or {}}
                    ...
                    if snapshots:
                        ...
                        for snapshot in snapshots:
                            if snapshot.user_feedback_count > 0:
                                satisfaction = (snapshot.positive_feedback - snapshot.negative_feedback) / snapshot.user_feedback_count
        ...
        for trait_name, satisfaction_scores in analytics.user_satisfaction_by_trait.items():
            if satisfaction_scores:
                ...
                if avg_satisfaction < -0.2:
                    recommendations.append(f'Consider reducing {trait_name} intensity (low satisfaction: {avg_satisfaction:.2f})')
                elif avg_satisfaction > 0.2:
                    recommendations.append(f'Consider increasing {trait_name} intensity (high satisfaction: {avg_satisfaction:.2f})')
        for trait_name, evolution in self._trait_evolutions.items():
    ...
    async def get_dashboard_summary(self) -> StepResult:
        ...
        try:
            async with self._lock:
                ...
                return StepResult.ok(data={'dashboard_stats': self._stats, 'active_traits': len(self._trait_snapshots), 'tracked_users': len(self._user_profiles), 'cache_status': 'valid' if time.time() - self._cache_timestamp < self._cache_ttl else 'expired'})
        except Exception as e:
    ...
    async def export_personality_data(self, fmt: str='json') -> StepResult:
        ...
        try:
            async with self._lock:
                if fmt.lower() == 'json':
                    export_data = {'trait_snapshots': {trait_name: [{'value': snapshot.value, 'timestamp': snapshot.timestamp, 'confidence': snapshot.confidence, 'user_feedback_count': snapshot.user_feedback_count, 'positive_feedback': snapshot.positive_feedback, 'negative_feedback': snapshot.negative_feedback, 'context': snapshot.context} for snapshot in snapshots] for trait_name, snapshots in self._trait_snapshots.items()}, 'user_profiles': {user_id: {'preferred_traits': profile.preferred_traits, 'interaction_count': profile.interaction_count, 'satisfaction_scores': profile.satisfaction_scores, 'personality_preferences': profile.personality_preferences} for user_id, profile in self._user_profiles.items()}, 'trait_evolutions': {trait_name: {'trend_direction': evolution.trend_direction, 'trend_strength': evolution.trend_strength, 'volatility': evolution.volatility, 'user_satisfaction_correlation': evolution.user_satisfaction_correlation} for trait_name, evolution in self._trait_evolutions.items()}, 'export_timestamp': time.time()}
