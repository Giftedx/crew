...
class ObservabilityManager:
    ...
    def __init__(self, max_traces: int=1000, max_history_size: int=10000, max_steps_per_trace: int=100):
        ...
        self._stats = {'total_observations': 0, 'active_conversations': 0, 'traces_created': 0, 'metrics_recorded': 0, 'personality_snapshots': 0}
    ...
    async def start_conversation_observation(self, conversation_id: str, user_id: str, guild_id: str, channel_id: str, initial_context: dict[str, Any] | None=None) -> StepResult:
        """Start observing a new conversation."""
        try:
            if not self._initialized:
                return StepResult.fail('Observability system not initialized')
            metrics_result = await self.metrics_collector.start_conversation(conversation_id, user_id, guild_id, channel_id, initial_context)
            if not metrics_result.success:
                return metrics_result
            trace_result = await self.conversation_tracer.start_trace(conversation_id, user_id, guild_id, channel_id, initial_context)
            if not trace_result.success:
                return trace_result
            async with self._lock:
                self._stats['active_conversations'] += 1
                self._stats['traces_created'] += 1
            return StepResult.ok(data={'conversation_id': conversation_id, 'trace_id': trace_result.data['trace_id'], 'action': 'conversation_observation_started'})
        except Exception as e:
            return StepResult.fail(f'Failed to start conversation observation: {e!s}')

    async def end_conversation_observation(self, conversation_id: str, satisfaction_score: float | None=None, topics_discussed: list[str] | None=None, summary: str | None=None) -> StepResult:
        """End observing a conversation."""
        try:
            if not self._initialized:
                return StepResult.fail('Observability system not initialized')
            await self.metrics_collector.end_conversation(conversation_id, satisfaction_score, topics_discussed)
    ...
    async def record_conversation_step(self, conversation_id: str, step_type: str, content: str, user_id: str | None=None, metadata: dict[str, Any] | None=None, start_time: float | None=None) -> StepResult:
        ...
        try:
            ...
            latest_trace = traces_result.data['traces'][-1]
            trace_id = latest_trace['trace'].trace_id
            step_result = await self.conversation_tracer.add_step(trace_id, step_type, content, metadata, None, start_time)
            ...
            return StepResult.ok(data={'conversation_id': conversation_id, 'step_id': step_result.data['step_id'], 'step_recorded': True})
        except Exception as e:
            return StepResult.fail(f'Failed to record conversation step: {e!s}')

    async def end_conversation_step(self, conversation_id: str, step_id: str, success: bool=True, final_content: str | None=None, error_message: str | None=None) -> StepResult:
        ...
        try:
            ...
            latest_trace = traces_result.data['traces'][-1]
            trace_id = latest_trace['trace'].trace_id
            step_result = await self.conversation_tracer.end_step(trace_id, step_id, success, final_content, error_message)
            return step_result
        except Exception as e:
            return StepResult.fail(f'Failed to end conversation step: {e!s}')

    async def record_personality_snapshot(self, trait_name: str, value: float, confidence: float=1.0, user_feedback: dict[str, Any] | None=None, context: dict[str, Any] | None=None) -> StepResult:
        """Record a personality trait snapshot."""
        try:
            if not self._initialized:
                return StepResult.fail('Observability system not initialized')
            result = await self.personality_dashboard.record_trait_snapshot(trait_name, value, confidence, user_feedback, context)
            if result.success:
                async with self._lock:
    ...
    async def record_user_feedback(self, user_id: str, trait_name: str, feedback_value: float, context: dict[str, Any] | None=None) -> StepResult:
    ...
    async def get_conversation_insights(self, conversation_id: str) -> StepResult:
        ...
        try:
            ...
            insights = {'conversation_metrics': metrics_result.data if metrics_result.success else None, 'conversation_traces': traces_result.data if traces_result.success else None, 'flow_analysis': flow_analysis}
    ...
    async def get_user_insights(self, user_id: str) -> StepResult:
        ...
        try:
            ...
            insights = {'user_metrics': metrics_result.data if metrics_result.success else None, 'personality_profile': profile_result.data if profile_result.success else None}
    ...
    async def get_system_insights(self) -> StepResult:
        ...
        try:
            ...
            system_insights = {'metrics_stats': metrics_stats.data if metrics_stats.success else {}, 'tracer_stats': tracer_stats.data if tracer_stats.success else {}, 'dashboard_stats': dashboard_stats.data if dashboard_stats.success else {}, 'performance_insights': performance_insights.data if performance_insights.success else {}, 'personality_analytics': personality_analytics.data if personality_analytics.success else {}, 'observability_stats': self._stats}
    ...
    async def export_observability_data(self, fmt: str='json') -> StepResult:
        ...
        try:
            ...
            export_data = {'export_timestamp': time.time(), 'personality_data': personality_export.data if personality_export.success else {}, 'system_insights': system_insights.data if system_insights.success else {}, 'format': fmt}
    ...
    async def cleanup_old_data(self, max_age_hours: int=168) -> StepResult:
        ...
        try:
            ...
            cleanup_results = {'metrics_cleanup': metrics_cleanup.data if metrics_cleanup.success else {}, 'cleanup_timestamp': time.time()}
    ...
    async def get_observability_stats(self) -> StepResult:
        ...
        try:
            async with self._lock:
                return StepResult.ok(data={'observability_stats': self._stats, 'initialized': self._initialized, 'components': {'metrics_collector': 'active', 'conversation_tracer': 'active', 'personality_dashboard': 'active'}})
        except Exception as e:
            return StepResult.fail(f'Failed to get observability stats: {e!s}')

def create_observability_manager(max_traces: int=1000, max_history_size: int=10000, max_steps_per_trace: int=100) -> ObservabilityManager:
