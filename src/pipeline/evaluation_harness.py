...
class EvaluationMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'word_error_rate': self.word_error_rate, 'diarization_error_rate': self.diarization_error_rate, 'total_cost': self.total_cost, 'total_latency': self.total_latency, 'transcription_quality': self.transcription_quality, 'diarization_quality': self.diarization_quality, 'content_analysis_quality': self.content_analysis_quality, 'claim_extraction_quality': self.claim_extraction_quality, 'stage_metrics': self.stage_metrics, 'stages_completed': self.stages_completed, 'stages_failed': self.stages_failed}
...
class TestEpisode:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'url': self.url, 'title': self.title, 'creator': self.creator, 'platform': self.platform, 'duration_seconds': self.duration_seconds, 'expected_speakers': self.expected_speakers, 'expected_topics': self.expected_topics, 'expected_claims': self.expected_claims, 'ground_truth_transcript': self.ground_truth_transcript, 'ground_truth_diarization': self.ground_truth_diarization}
...
class PipelineEvaluationHarness:
    ...
    def create_test_episodes(self) -> list[TestEpisode]:
        ...
        return [TestEpisode(url='https://www.youtube.com/watch?v=dQw4w9WgXcQ', title='H3 Podcast #123 - Tech Talk with Guest', creator='H3 Podcast', platform='youtube', duration_seconds=3600, expected_speakers=3, expected_topics=['technology', 'podcasting', 'entertainment'], expected_claims=['AI will revolutionize content creation', 'Podcasting is the future of media'], ground_truth_transcript="Welcome to the H3 Podcast. Today we're discussing technology trends...", ground_truth_diarization={'Ethan': [(0.0, 300.0), (600.0, 900.0)], 'Hila': [(300.0, 600.0), (900.0, 1200.0)], 'Guest': [(1200.0, 1800.0)]}), TestEpisode(url='https://www.youtube.com/watch?v=example2', title='H3 Podcast #124 - Gaming Discussion', creator='H3 Podcast', platform='youtube', duration_seconds=2700, expected_speakers=2, expected_topics=['gaming', 'streaming', 'entertainment'], expected_claims=['Streaming platforms are changing gaming', 'Mobile gaming is the future'], ground_truth_transcript="Today we're talking about the gaming industry and how streaming has changed everything...", ground_truth_diarization={'Ethan': [(0.0, 1350.0)], 'Hila': [(1350.0, 2700.0)]}), TestEpisode(url='https://www.twitch.tv/videos/example1', title='Hasan Reacts to Political News', creator='Hasan Piker', platform='twitch', duration_seconds=7200, expected_speakers=1, expected_topics=['politics', 'news', 'reaction'], expected_claims=['Current political system needs reform', 'Media bias affects public opinion'], ground_truth_transcript="Welcome back to the stream. Today we're reacting to the latest political news...", ground_truth_diarization={'Hasan': [(0.0, 7200.0)]}), TestEpisode(url='https://www.twitch.tv/videos/example2', title='Hasan Gaming Stream - Valorant', creator='Hasan Piker', platform='twitch', duration_seconds=5400, expected_speakers=1, expected_topics=['gaming', 'valorant', 'streaming'], expected_claims=['Valorant is the best tactical shooter', 'Streaming improves gaming skills'], ground_truth_transcript="Alright, let's jump into some Valorant. I've been practicing my aim...", ground_truth_diarization={'Hasan': [(0.0, 5400.0)]}), TestEpisode(url='https://www.youtube.com/watch?v=example3', title='Hasan Reacts to Drama', creator='Hasan Piker', platform='youtube', duration_seconds=1800, expected_speakers=1, expected_topics=['drama', 'reaction', 'entertainment'], expected_claims=['Drama content is harmful to creators', 'Reaction content adds value'], ground_truth_transcript="Today we're reacting to some drama that's been going around...", ground_truth_diarization={'Hasan': [(0.0, 1800.0)]})]
    ...
    def calculate_diarization_error_rate(self, predicted: dict[str, list[tuple[float, float]]], ground_truth: dict[str, list[tuple[float, float]]]) -> float:
    ...
    def calculate_cost_estimate(self, stage_metrics: dict[str, dict[str, Any]]) -> float:
        ...
        total_cost = 0.0
        cost_estimates = {'download': 0.0, 'diarization': 0.05, 'transcription': 0.1, 'visual_analysis': 0.15, 'content_analysis': 0.2, 'claim_extraction': 0.1, 'kg_ingestion': 0.05}
    ...
    async def evaluate_episode(self, episode: TestEpisode) -> EvaluationMetrics:
        ...
        try:
            ...
            for stage_name, stage_data in stage_results.items():
                ...
                metrics.stage_metrics[stage_name] = {'duration_seconds': stage_duration, 'success': stage_data.get('success', False), 'data_size': len(str(stage_data.get('data', {})))}
            ...
            if episode.ground_truth_transcript and 'transcription' in stage_results:
                ...
                metrics.word_error_rate = self.calculate_word_error_rate(predicted_transcript, episode.ground_truth_transcript)
            if episode.ground_truth_diarization and 'diarization' in stage_results:
                ...
                metrics.diarization_error_rate = self.calculate_diarization_error_rate(predicted_diarization, episode.ground_truth_diarization)
    ...
    async def run_evaluation(self, episodes: list[TestEpisode] | None=None) -> dict[str, Any]:
        ...
        return {'episodes': [ep.to_dict() for ep, _ in results], 'metrics': [m.to_dict() for _, m in results], 'aggregate_metrics': aggregate_metrics, 'summary': self.generate_summary(results, aggregate_metrics)}
    ...
    def calculate_aggregate_metrics(self, results: list[tuple[TestEpisode, EvaluationMetrics]]) -> dict[str, Any]:
        ...
        return {'total_episodes': total_episodes, 'successful_episodes': successful_episodes, 'success_rate': successful_episodes / total_episodes, 'average_wer': avg_wer, 'average_der': avg_der, 'average_cost': avg_cost, 'average_latency': avg_latency, 'stage_success_rates': stage_success_rates}

    def generate_summary(self, results: list[tuple[TestEpisode, EvaluationMetrics]], aggregate_metrics: dict[str, Any]) -> dict[str, Any]:
        ...
        if not wer_criteria:
            summary['criteria_failed']['wer'] = f'WER {aggregate_metrics.get('average_wer', 0):.1%} exceeds 12% threshold'
            summary['overall_status'] = 'FAIL'
        der_criteria = aggregate_metrics.get('average_der', 1.0) <= 0.2
        summary['criteria_met']['der'] = der_criteria
        if not der_criteria:
            summary['criteria_failed']['der'] = f'DER {aggregate_metrics.get('average_der', 0):.1%} exceeds 20% threshold'
            summary['overall_status'] = 'FAIL'
        cost_criteria = aggregate_metrics.get('average_cost', 100.0) <= 2.0
        summary['criteria_met']['cost'] = cost_criteria
        if not cost_criteria:
            summary['criteria_failed']['cost'] = f'Cost ${aggregate_metrics.get('average_cost', 0):.2f} exceeds $2.00 threshold'
            summary['overall_status'] = 'FAIL'
        latency_criteria = aggregate_metrics.get('average_latency', 1000.0) <= 600.0
        summary['criteria_met']['latency'] = latency_criteria
        if not latency_criteria:
            summary['criteria_failed']['latency'] = f'Latency {aggregate_metrics.get('average_latency', 0):.1f}s exceeds 10min threshold'
            summary['overall_status'] = 'FAIL'
        if not wer_criteria:
    ...
    def save_results(self, filepath: str) -> StepResult:
        ...
        try:
            results_data = {'episodes': [ep.to_dict() for ep, _ in self.results], 'metrics': [m.to_dict() for _, m in self.results], 'aggregate_metrics': self.calculate_aggregate_metrics(self.results), 'summary': self.generate_summary(self.results, self.calculate_aggregate_metrics(self.results))}
