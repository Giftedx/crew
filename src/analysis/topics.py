...
def extract(text: str) -> TopicResult:
    ...
    for i in range(len(words) - 1):
        ...
        if i < len(words) - 1:
            ...
            if (
                ...
                and len(words[i]) >= MIN_KEYWORD_LEN
                and len(words[i + 1]) >= MIN_KEYWORD_LEN
            ):
    ...
    for keyword in top_keywords[:10]:
        if len(keyword) >= MIN_TOPIC_KEYWORD_LEN:  # Only longer, more meaningful words
