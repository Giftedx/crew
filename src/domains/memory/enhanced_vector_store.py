...
class EnhancedVectorStore(VectorStore):
    ...
    def create_collection_with_hybrid_config(self, namespace: str, dimension: int, enable_sparse: bool=True, quantization: bool=True) -> bool:
        ...
        try:
            ...
            if quantization:
                quantization_config = qmodels.ScalarQuantization(scalar=qmodels.ScalarQuantizationConfig(type=qmodels.ScalarType.INT8, quantile=0.99, always_ram=True))
            ...
            hnsw_config = qmodels.HnswConfigDiff(m=16, ef_construct=200, full_scan_threshold=10000, max_indexing_threads=0)
            optimizer_config = qmodels.OptimizersConfigDiff(deleted_threshold=0.2, vacuum_min_vector_number=1000, default_segment_number=0)
            self.client.create_collection(collection_name=collection_name, vectors_config=vectors_config, sparse_vectors_config=sparse_vectors_config, quantization_config=quantization_config, hnsw_config=hnsw_config, optimizers_config=optimizer_config, replication_factor=1, write_consistency_factor=1)
            self.client.create_payload_index(collection_name=collection_name, field_name='tenant', field_type=qmodels.PayloadSchemaType.KEYWORD)
            self.client.create_payload_index(collection_name=collection_name, field_name='workspace', field_type=qmodels.PayloadSchemaType.KEYWORD)
    ...
    def hybrid_search(self, namespace: str, query_vector: list[float], query_text: str='', limit: int=10, score_threshold: float=0.7, filter_conditions: dict[str, Any] | None=None) -> list[SearchResult]:
        ...
        try:
            search_requests = []
            from qdrant_client.http import models as qmodels
            dense_request = qmodels.SearchRequest(vector=qmodels.NamedVector(name='', vector=query_vector), limit=limit * 2, score_threshold=score_threshold * 0.8, with_payload=True, with_vector=False)
            ...
            if self._sparse_vectors_supported and query_text.strip():
                try:
                    ...
                    sparse_request = qmodels.SearchRequest(vector=qmodels.NamedSparseVector(name='text', vector=sparse_vector), limit=limit * 2, score_threshold=score_threshold * 0.6, with_payload=True, with_vector=False)
                    if filter_conditions:
            ...
            hybrid_results = self._combine_search_results(dense_results=dense_results, sparse_results=sparse_results, limit=limit)
    ...
    def _build_filter(self, conditions: dict[str, Any]) -> Any:
        ...
        for field, value in conditions.items():
            if isinstance(value, list):
                should_conditions: list[Any] = [qmodels.FieldCondition(key=field, match=qmodels.MatchValue(value=v)) for v in value]
    ...
    def _combine_search_results(self, dense_results: list[Any], sparse_results: list[Any], limit: int) -> list[SearchResult]:
        """Combine and re-rank dense and sparse search results."""
        results_map = {}
        for result in dense_results:
            result_id = str(result.id)
            results_map[result_id] = SearchResult(id=result_id, payload=result.payload or {}, dense_score=result.score, sparse_score=None, hybrid_score=result.score * self.hybrid_config.dense_weight)
        for result in sparse_results:
            result_id = str(result.id)
            if result_id in results_map:
                results_map[result_id].sparse_score = result.score
                results_map[result_id].hybrid_score = results_map[result_id].dense_score * self.hybrid_config.dense_weight + result.score * self.hybrid_config.sparse_weight
            else:
                results_map[result_id] = SearchResult(id=result_id, payload=result.payload or {}, dense_score=0.0, sparse_score=result.score, hybrid_score=result.score * self.hybrid_config.sparse_weight)
    ...
    def get_collection_stats(self, namespace: str) -> dict[str, Any]:
        ...
        try:
            ...
            stats = {'collection_name': collection_name, 'vectors_count': getattr(info, 'vectors_count', None), 'segments_count': getattr(info, 'segments_count', None), 'disk_data_size': getattr(info, 'disk_data_size', None), 'ram_data_size': getattr(info, 'ram_data_size', None), 'config': {'distance': getattr(getattr(vectors, 'distance', None), 'value', None), 'dimension': getattr(vectors, 'size', None)}, 'quantization_enabled': getattr(cfg, 'quantization_config', None) is not None, 'sparse_vectors_enabled': bool(getattr(cfg, 'sparse_vectors_config', None))}
            try:
