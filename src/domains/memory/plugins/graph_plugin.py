    ...
    async def store(self, namespace: str, records: Sequence[dict[str, Any]]) -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(
                stored=stored_count, graph_ids=graph_ids, namespace=namespace, index=index, backend="graph"
            )
        except Exception as exc:
        ...
        try:
            ...
            if not result.success:
                return StepResult.fail(
                    f"Graph search failed: {result.error}", namespace=namespace, step="graph_retrieve"
                )
                ...
                formatted_results.append(
                    {
                        ...
                        "metadata": {"tenant_scoped": graph_meta.get("tenant_scoped", False), "namespace": namespace},
                    }
                )
            return StepResult.ok(
