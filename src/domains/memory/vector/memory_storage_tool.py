    ...
    def _run(
        ...
        if self.embedding_fn is None:
            self._metrics.counter(
                "tool_runs_total", labels={"tool": "memory_storage", "outcome": "skipped", "reason": "no_embedding_fn"}
            ).inc()
        ...
        try:
            ...
            if len(vector) == 1:
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "memory_storage", "outcome": "skipped", "reason": "invalid_embedding_dimension"},
                ).inc()
                return StepResult.skip(
