...
class PromptCompressionTool(BaseTool[dict]):
    ...
    def __init__(self):
        ...
        if self._enabled:
            try:
                self._compressor = PromptCompressor(model_name='microsoft/llmlingua-2-xlm-roberta-large-meetingbank', device_map='auto')
    ...
    def _run(self, contexts: list[str], instruction: str='', question: str='', target_token: int=2000, compression_ratio: float=0.5) -> StepResult:
        """
        ...
        if not self._enabled or not self._compressor:
            return StepResult.skip(step='prompt_compression', reason='ENABLE_PROMPT_COMPRESSION not set or initialization failed')
        try:
            compressed = self._compressor.compress_prompt(contexts, instruction=instruction, question=question, target_token=target_token, ratio=compression_ratio, condition_compare=True, condition_in_question='after', rank_method='longllmlingua', use_sentence_level_filter=False, context_budget='+100', dynamic_context_compression_ratio=0.4, reorder_context='sort')
            ...
            get_metrics().histogram('prompt_compression_ratio', actual_ratio, labels={'tool': self.name, 'outcome': 'success'})
            ...
            return StepResult.ok(result={'compressed_prompt': compressed['compressed_prompt'], 'origin_tokens': origin_tokens, 'compressed_tokens': compressed_tokens, 'tokens_saved': tokens_saved, 'compression_ratio': actual_ratio})
        except Exception as e:
