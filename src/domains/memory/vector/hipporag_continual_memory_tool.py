        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "hipporag_init_failures",
                    labels={"namespace": namespace, "error": type(exc).__name__},
                ).inc()
            return None
        ...
        *,
        ...
        if not isinstance(text, str) or not text.strip():
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "hipporag_continual_memory", "outcome": "skipped"},
                ).inc()
        ...
        if not _is_feature_enabled():
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "hipporag_continual_memory", "outcome": "disabled"},
                ).inc()
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "tool_runs_total",
                    labels={"tool": "hipporag_continual_memory", "outcome": "error"},
                ).inc()

            return StepResult.fail(
                f"HippoRAG continual memory failed: {exc}",
                namespace=namespace,
                step="hipporag_index",
            )
        ...
        *,
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter(
                    "hipporag_memory_operations",
                    labels={"operation": "retrieve", "namespace": namespace},
                ).inc()
