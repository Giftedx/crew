...
class RagIngestUrlTool(BaseTool[StepResult]):
    ...
    def run(self, *, urls: list[str], index: str='memory', chunk_size: int=400, overlap: int=50, max_bytes: int=500000) -> StepResult:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter('tool_runs_total', labels={'tool': 'rag_ingest_url', 'outcome': 'success', 'tenant_scoped': str(tenant_scoped).lower()}).inc()
            return StepResult.ok(data={'fetched': fetched, 'inserted': inserted, 'chunks': chunks_total, 'index': namespace, 'tenant_scoped': tenant_scoped})
        except Exception as exc:
            with contextlib.suppress(Exception):
                self._metrics.counter('tool_runs_total', labels={'tool': 'rag_ingest_url', 'outcome': 'error', 'tenant_scoped': str(tenant_scoped).lower()}).inc()
            return StepResult.fail(str(exc), data={'fetched': fetched, 'inserted': inserted, 'chunks': chunks_total, 'index': index, 'tenant_scoped': tenant_scoped})
