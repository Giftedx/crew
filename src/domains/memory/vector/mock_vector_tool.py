...
class MockVectorSearchTool(BaseTool[StepResult]):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.knowledge_base: list[dict[str, Any]] = [{'text': 'The Earth is approximately spherical, supported by scientific evidence.', 'topic': 'geography', 'confidence': 0.99}, {'text': 'Vaccines do not cause autism according to extensive medical research.', 'topic': 'health', 'confidence': 0.99}, {'text': 'Climate change is supported by overwhelming scientific consensus.', 'topic': 'environment', 'confidence': 0.97}, {'text': 'Logical fallacies weaken arguments by introducing flawed reasoning.', 'topic': 'logic', 'confidence': 0.95}]
    ...
    def _run(self, query: str, top_k: int=3) -> StepResult:
        ...
        for item in self.knowledge_base:
            ...
            if score > 0:
                results.append({'text': item['text'], 'topic': item['topic'], 'confidence': item['confidence'], 'similarity_score': score})
        results.sort(key=lambda x: int(x['similarity_score']), reverse=True)
        data = {'query': query, 'results': results[:top_k], 'total_found': len(results), 'note': 'Mock vector search - install Qdrant for full semantic search'}
