...
class UnifiedMemoryRetrieveInput(BaseModel):
    ...
    intent: str = Field(
        default="general", description="Intent of the query (general, fact_check, debate_analysis, creator_intel)"
    )
...
class UnifiedMemoryTool(BaseTool):
    ...
    def __init__(self, **kwargs):
        ...
        memory_config = UnifiedMemoryConfig(
            enable_vector_store=True, enable_sqlite_store=True, enable_semantic_cache=True, enable_mem0=False
        )
        retrieval_config = RetrievalConfig(
            enable_semantic_ranking=True, enable_content_deduplication=True, enable_metadata_fusion=True
        )
        context_config = ContextConfig(
            enable_semantic_grouping=True, enable_temporal_ordering=True, enable_relevance_filtering=True
        )
        try:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            retrieval_query = RetrievalQuery(
                text=query, intent=intent, filters=filters, limit=limit, min_confidence=min_confidence
            )
            import asyncio
...
class UnifiedMemoryStoreTool(BaseTool):
    ...
    def _run(
        self, content: str, metadata: dict[str, Any] | None = None, namespace: str | None = None, **kwargs: Any
    ) -> StepResult:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            try:
                ...
                asyncio.set_event_loop(loop)
            result = loop.run_until_complete(
                self._context_builder.build_context(context_request, self._memory_service, None)
            )
            if not result.success:
