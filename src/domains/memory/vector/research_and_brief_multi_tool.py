...
class ResearchAndBriefMultiTool(BaseTool[StepResult]):
    ...
    def _run_multi_agent(self, inputs: dict[str, Any], timeout: float) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Thread worker to isolate an event loop
            ...
            if not t.is_alive():
                ...
                if isinstance(item, BaseException):
                    return None
                return item
            # thread still alive after join window
            return None
        except Exception:
            return None
    ...
    def run(
        ...
        *,
        ...
        max_items: int = 5,
        max_time: float = 60.0,
        enable_alerts: bool = False,  # reserved, currently not used
    ) -> StepResult:
        # First, produce deterministic synthesis via offline tool
        ...
        # Attempt multi-agent execution if allowed; ignore failures
        ma_inputs = {
            "query": query,
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter(
                "tool_runs_total",
                labels={"tool": "research_and_brief_multi", "outcome": "success"},
            ).inc()

        # Preserve uncertain vs ok semantics from offline result
