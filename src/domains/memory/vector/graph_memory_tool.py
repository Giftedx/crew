...
class GraphMemoryTool(BaseTool[StepResult]):
    ...
    def list_graphs(self, *, namespace: str = "graph") -> StepResult:
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter(
                "graph_memory_lists_total", labels={"namespace": namespace, "tenant_scoped": str(tenant_scoped).lower()}
            ).inc()
        return StepResult.ok(
            graph_ids=graph_ids, count=len(graph_ids), namespace=namespace, tenant_scoped=tenant_scoped
        )

    def search_graphs(
        self, *, query: str | None = None, namespace: str = "graph", tags: list[str] | None = None, limit: int = 10
    ) -> StepResult:
    ...
    def traverse_graph(
        ...
        *,
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter(
                "graph_memory_traversals_total", labels={"namespace": graph_result.data["namespace"]}
            ).inc()
        return StepResult.ok(
            graph_id=graph_id,
