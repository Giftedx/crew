...
def _split_sentences(text: str) -> list[str]:
    sentences = [segment.strip() for segment in re.split('(?<=[.!?])\\s+', text) if segment.strip()]
...
def _build_graph(text: str) -> dict[str, Any]:
    ...
    if nx is None:
        ...
        for idx, sentence in enumerate(sentences):
            ...
            if idx > 0:
                edges.append({'source': f'sentence_{idx}', 'target': node_id, 'relation': 'sequence'})
        for kw in keywords:
            kw_id = f'keyword_{kw}'
            nodes.append({'id': kw_id, 'label': kw, 'type': 'keyword'})
            if sentences:
                edges.append({'source': kw_id, 'target': 'sentence_1', 'relation': 'mentions'})
    ...
    return {'nodes': [{'id': node, **(graph.nodes[node] or {})} for node in graph.nodes], 'edges': [{'source': src, 'target': dst, **(graph.edges[src, dst] or {})} for src, dst in graph.edges], 'keywords': keywords}
...
class GraphMemoryTool(BaseTool[StepResult]):
    ...
    def _physical_namespace(namespace: str) -> str:
        ...
        return re.sub('[^A-Za-z0-9_.-]', '_', safe)
    ...
    def run(self, *, text: str, index: str='graph', metadata: dict[str, Any] | None=None, tags: Iterable[str] | None=None) -> StepResult:
        ...
        try:
            ...
            graph_payload['metadata'] = {'tenant_scoped': tenant_scoped, 'namespace': namespace, 'keywords': keywords, 'tags': list(tags or []), 'source_metadata': _normalise_metadata(metadata)}
            ...
            if self._use_unified_store and self._unified_store is not None:
                try:
                    for node in graph_payload['nodes']:
                        ...
                        self._unified_store.add_node(node_id, labels=labels or ['Node'], properties=props, namespace=namespace)
                    ...
                    with contextlib.suppress(Exception):
                        self._metrics.counter('graph_memory_store_errors_total', labels={'namespace': namespace}).inc()
            ...
            with contextlib.suppress(Exception):
                self._metrics.counter('tool_runs_total', labels={'tool': 'graph_memory', 'outcome': 'success', 'tenant_scoped': str(tenant_scoped).lower()}).inc()
            return StepResult.ok(graph_id=graph_id, namespace=namespace, tenant_scoped=tenant_scoped, storage_path=str(file_path), node_count=len(graph_payload['nodes']), edge_count=len(graph_payload['edges']), keywords=graph_payload['metadata'].get('keywords', []))
        except Exception as exc:
            with contextlib.suppress(Exception):
                self._metrics.counter('tool_runs_total', labels={'tool': 'graph_memory', 'outcome': 'error', 'tenant_scoped': str(tenant_scoped).lower()}).inc()
    ...
    def list_graphs(self, *, namespace: str='graph') -> StepResult:
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter('graph_memory_lists_total', labels={'namespace': namespace, 'tenant_scoped': str(tenant_scoped).lower()}).inc()
        return StepResult.ok(graph_ids=graph_ids, count=len(graph_ids), namespace=namespace, tenant_scoped=tenant_scoped)

    def search_graphs(self, *, query: str | None=None, namespace: str='graph', tags: list[str] | None=None, limit: int=10) -> StepResult:
        ...
        for json_file in ns_path.glob('*.json'):
            try:
                ...
                if query_keywords:
                    ...
                    score = 1.0
                matches.append({'graph_id': json_file.stem, 'score': score, 'keywords': metadata.get('keywords', []), 'tags': metadata.get('tags', []), 'node_count': metadata.get('node_count', 0), 'edge_count': metadata.get('edge_count', 0), 'tenant_scoped': metadata.get('tenant_scoped', False)})
            except Exception:
                continue
        matches.sort(key=lambda x: x['score'], reverse=True)
        results = matches[:limit]
        with contextlib.suppress(Exception):
            self._metrics.counter('graph_memory_searches_total', labels={'namespace': namespace, 'tenant_scoped': str(tenant_scoped).lower()}).inc()
        return StepResult.ok(graphs=results, count=len(results), namespace=namespace, tenant_scoped=tenant_scoped)
    ...
    def get_graph(self, *, graph_id: str, namespace: str='graph') -> StepResult:
        ...
        if not file_path.exists():
            return StepResult.fail(f'Graph not found: {graph_id}', namespace=namespace, graph_id=graph_id)
        try:
            with file_path.open('r', encoding='utf-8') as f:
                graph_data = json.load(f)
            with contextlib.suppress(Exception):
                self._metrics.counter('graph_memory_retrievals_total', labels={'namespace': namespace, 'tenant_scoped': str(tenant_scoped).lower()}).inc()
            return StepResult.ok(graph_id=graph_id, nodes=graph_data.get('nodes', []), edges=graph_data.get('edges', []), keywords=graph_data.get('keywords', []), metadata=graph_data.get('metadata', {}), namespace=namespace, tenant_scoped=tenant_scoped)
        except Exception as exc:
            return StepResult.fail(f'Failed to load graph: {exc}', namespace=namespace, graph_id=graph_id)

    def traverse_graph(self, *, graph_id: str, start_node: str, max_depth: int=3, relation_filter: list[str] | None=None, namespace: str='graph') -> StepResult:
        ...
        with contextlib.suppress(Exception):
            self._metrics.counter('graph_memory_traversals_total', labels={'namespace': graph_result.data['namespace']}).inc()
        return StepResult.ok(graph_id=graph_id, start_node=start_node, visited_nodes=visited_nodes, paths=paths, subgraph={'nodes': visited_nodes, 'edges': visited_edges}, node_count=len(visited_nodes), edge_count=len(visited_edges), max_depth=max_depth, namespace=graph_result.data['namespace'], tenant_scoped=graph_result.data.get('tenant_scoped', False))
