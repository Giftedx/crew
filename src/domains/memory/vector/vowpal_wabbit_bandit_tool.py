...
class VowpalWabbitBanditTool(BaseTool):
    ...
    def _run(self, action: str, context: dict[str, str] | None=None) -> StepResult:
        ...
        try:
            ...
            result = {'action': selected_action, 'context': context, 'action_scores': action_scores, 'confidence': confidence, 'expected_reward': expected_reward, 'status': 'optimized', 'message': 'Bandit optimization completed successfully'}
    ...
    def _calculate_action_scores(self, action: str, context: dict[str, str]) -> dict[str, float]:
        ...
        if 'urgency' in context:
            if context['urgency'] == 'high':
                ...
                base_score -= 0.1
        return {f'{action}_standard': base_score, f'{action}_optimized': base_score + 0.1, f'{action}_conservative': base_score - 0.1}
