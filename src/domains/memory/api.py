...
def store(store: MemoryStore, vstore: vector_store.VectorStore, *, tenant: str, workspace: str, text: str, item_type: str='long', policy: str='default') -> int:
    ...
    item = MemoryItem(id=None, tenant=tenant, workspace=workspace, type=item_type, content_json=json.dumps({'text': clean}), embedding_json=json.dumps(vec), ts_created=now, ts_last_used=now, retention_policy=policy, decay_score=1.0, pinned=0, archived=0)
...
def retrieve(store: MemoryStore, vstore: vector_store.VectorStore, *, tenant: str, workspace: str, query: str, k: int=5, strategies: Sequence[str] | None=None, engine: LearningEngine | None=None) -> list[MemoryHit]:
