...
EMBEDDING_DIMENSIONS = {'sentence-transformers/all-MiniLM-L6-v2': 384, 'sentence-transformers/all-mpnet-base-v2': 768, 'openai/text-embedding-3-small': 1536, 'openai/text-embedding-3-large': 3072}
...
COLLECTION_CONFIGS = {'episodes': CollectionConfig(name='creator_episodes', description='Full episodes/videos with platform metadata', embedding_model='sentence-transformers/all-mpnet-base-v2', enable_quantization=True, enable_sparse_vectors=True, payload_schema={'content_type': 'string', 'platform': 'string', 'creator_id': 'string', 'episode_id': 'string', 'title': 'string', 'published_at': 'datetime', 'duration_seconds': 'integer', 'url': 'string', 'thumbnail_url': 'string', 'view_count': 'integer', 'tenant': 'string', 'workspace': 'string'}), 'segments': CollectionConfig(name='creator_segments', description='Timestamped segments within episodes', embedding_model='sentence-transformers/all-MiniLM-L6-v2', enable_quantization=True, payload_schema={'content_type': 'string', 'episode_id': 'string', 'segment_id': 'string', 'start_time_seconds': 'integer', 'end_time_seconds': 'integer', 'title': 'string', 'segment_type': 'string', 'speaker': 'string', 'text': 'string', 'tenant': 'string', 'workspace': 'string'}), 'claims': CollectionConfig(name='creator_claims', description='Factual claims with verification status', embedding_model='sentence-transformers/all-mpnet-base-v2', enable_quantization=False, enable_sparse_vectors=True, payload_schema={'content_type': 'string', 'claim_id': 'string', 'text': 'string', 'speaker': 'string', 'episode_id': 'string', 'timestamp_seconds': 'integer', 'verification_status': 'string', 'confidence_score': 'float', 'sources': 'json', 'tenant': 'string', 'workspace': 'string'}), 'quotes': CollectionConfig(name='creator_quotes', description='Notable quotes with speaker attribution', embedding_model='sentence-transformers/all-MiniLM-L6-v2', enable_quantization=True, payload_schema={'content_type': 'string', 'quote_id': 'string', 'text': 'string', 'speaker': 'string', 'episode_id': 'string', 'timestamp_seconds': 'integer', 'context': 'string', 'tenant': 'string', 'workspace': 'string'}), 'topics': CollectionConfig(name='creator_topics', description='Topic embeddings for narrative tracking', embedding_model='sentence-transformers/all-mpnet-base-v2', enable_quantization=True, enable_sparse_vectors=True, payload_schema={'content_type': 'string', 'topic_id': 'string', 'name': 'string', 'category': 'string', 'first_mention': 'datetime', 'last_mention': 'datetime', 'mention_count': 'integer', 'episodes': 'json', 'tenant': 'string', 'workspace': 'string'})}
...
class CreatorIntelligenceCollectionManager:
        ...
        try:
            ...
            for _collection_type, config in COLLECTION_CONFIGS.items():
                ...
                if namespace in self._initialized_collections:
                    ...
                    continue
                success = self.vector_store.create_collection_with_hybrid_config(namespace=namespace, dimension=config.dimension, enable_sparse=config.enable_sparse_vectors, quantization=config.enable_quantization)
                if success:
            ...
            if failed:
                return StepResult.fail(f'Some collections failed to initialize: {', '.join(failed)}', metadata={'initialized': initialized, 'failed': failed})
            return StepResult.ok(data={'initialized': initialized, 'total': len(initialized), 'collections': list(COLLECTION_CONFIGS.keys())})
        except Exception as e:
            logger.error(f'Collection initialization failed: {e}')
            return StepResult.fail(f'Collection initialization failed: {e!s}')

    def query_with_cache(self, collection_type: Literal['episodes', 'segments', 'claims', 'quotes', 'topics'], query_embedding: list[float], query_text: str, tenant: str, workspace: str, limit: int=10, score_threshold: float=0.7, filter_conditions: dict[str, Any] | None=None, bypass_cache: bool=False) -> StepResult:
        ...
        try:
            ...
            if not bypass_cache and self.enable_semantic_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'results': [r.__dict__ for r in cache_result.results], 'cache_hit': True, 'cache_age_seconds': time.time() - cache_result.cache_time})
            results = self.vector_store.hybrid_search(namespace=namespace, query_vector=query_embedding, query_text=query_text, limit=limit, score_threshold=score_threshold, filter_conditions=filter_conditions)
            if self.enable_semantic_cache and results:
                self._cache_query_results(query_embedding, namespace, results, limit)
            return StepResult.ok(data={'results': [r.__dict__ for r in results], 'cache_hit': False, 'result_count': len(results)})
        except Exception as e:
    ...
    def _cache_query_results(self, query_embedding: list[float], namespace: str, results: list[SearchResult], limit: int) -> None:
        ...
        """
        query_hash = self._compute_query_hash(query_embedding, namespace, limit)
        cached_query = CachedQuery(query_hash=query_hash, query_embedding=query_embedding, results=results, cache_time=time.time())
    ...
    def get_collection_stats(self, collection_type: Literal['episodes', 'segments', 'claims', 'quotes', 'topics'], tenant: str, workspace: str) -> StepResult:
