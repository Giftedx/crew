...
class LRUCache:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'size': len(self._cache), 'max_size': self.max_size, 'hits': self._hits, 'misses': self._misses, 'hit_rate': hit_rate, 'default_ttl': self.default_ttl}
...
class MemoryAnalytics:
    ...
    performance_metrics: deque[MemoryOperationMetrics] = field(default_factory=lambda: deque(maxlen=PERFORMANCE_MONITORING_WINDOW))
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {'total_operations': len(self.performance_metrics), 'avg_duration_ms': statistics.mean(durations), 'median_duration_ms': statistics.median(durations), 'avg_throughput_vectors_per_sec': statistics.mean(throughputs), 'p95_duration_ms': sorted(durations)[int(len(durations) * 0.95)], 'p99_duration_ms': sorted(durations)[int(len(durations) * 0.99)], 'operations_per_minute': len(self.performance_metrics) / max(1, (time.time() - self.performance_metrics[0].timestamp) / 60)}
...
class OptimizedSearchResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        result = {'vector_id': self.vector_id, 'score': self.score, 'payload': self.payload, 'search_time_ms': self.search_time_ms, 'search_method': self.search_method}
        if self.vector:
...
class VectorStore:
    ...
    def __init__(self, url: str | None=None, api_key: str | None=None):
        ...
        self._similarity_cache: dict[str, list[tuple[str, float, float]]] = {}
        ...
        try:
            ...
            _ttl = int(os.getenv('VECTOR_CACHE_TTL', '300'))
    ...
    def upsert(self, namespace: str, records: Sequence[VectorRecord]) -> None:
        ...
        for offset in range(0, len(records), effective_batch_size):
            ...
            if not QDRANT_AVAILABLE:
                ...
                from qdrant_client.http import models as qmodels
                points = [qmodels.PointStruct(id=base + offset + i, vector=r.vector, payload=dict(r.payload)) for i, r in enumerate(chunk)]
    ...
    def query(self, namespace: str, vector: Sequence[float], top_k: int=3) -> list[Any]:
        ...
        """
        physical = self._physical_names.get(namespace, namespace)
        res = self.client.query_points(collection_name=physical, query=list(vector) if hasattr(vector, '__iter__') and (not isinstance(vector, dict)) else vector, limit=top_k, with_payload=True)
    ...
    def _get_adaptive_batch_size(self, namespace: str, dim: int) -> int:
        ...
        if len(self._performance_history) > 10:
            recent_avg_duration = statistics.mean((m.duration_ms for m in list(self._performance_history)[-10:] if m.operation in {'upsert', 'query'}))
            if recent_avg_duration < 100:
                batch_size = int(batch_size * ADAPTIVE_BATCH_FACTOR)
            elif recent_avg_duration > 1000:
                batch_size = max(1, int(batch_size / ADAPTIVE_BATCH_FACTOR))
        return max(1, min(batch_size, self._batch_size * 2))

    async def batch_upsert(self, vectors: list[list[float]], payloads: list[dict[str, Any]], tenant: str, workspace: str, batch_size: int | None=None) -> StepResult:
        ...
        if len(vectors) != len(payloads):
            return StepResult(success=False, error='Mismatch between vectors and payloads count', custom_status='bad_request')
        ...
        try:
            ...
            return StepResult(success=True, data={'total_upserted': total_vectors, 'batches_processed': batches_processed, 'effective_batch_size': effective_batch_size, 'vector_dimension': vector_dim, 'duration_ms': (time.time() - start_time) * 1000, 'namespace': namespace})
        except Exception as e:
            logger.error(f'Batch upsert failed: {e}')
            return StepResult(success=False, error=f'Batch upsert failed: {e!s}', custom_status='retryable')

    async def batch_search(self, query_vectors: list[list[float]], limit: int=10, tenant: str='default', workspace: str='main', filters: dict[str, Any] | None=None, score_threshold: float=0.0) -> StepResult:
        ...
        try:
            ...
            for i, query_vector in enumerate(query_vectors):
                task = asyncio.create_task(self._single_vector_search(query_vector, limit, namespace, filters, score_threshold, i))
            ...
            return StepResult(success=True, data={'query_count': len(query_vectors), 'successful_searches': len(successful_results), 'failed_searches': failed_searches, 'results': successful_results, 'duration_ms': (time.time() - start_time) * 1000, 'namespace': namespace})
        except Exception as e:
            logger.error(f'Batch search failed: {e}')
            return StepResult(success=False, error=f'Batch search failed: {e!s}', custom_status='retryable')

    async def _single_vector_search(self, query_vector: list[float], limit: int, namespace: str, filters: dict[str, Any] | None, score_threshold: float, query_id: int) -> dict[str, Any]:
        """Perform single vector search (used by batch_search)."""
        try:
            physical = self._physical_names.get(namespace, namespace)
            results = self._search_with_filter(collection_name=physical, search_vector=query_vector, limit=limit, score_threshold=score_threshold, filter_conditions=filters)
            return {'query_id': query_id, 'results': results, 'result_count': len(results), 'query_vector_hash': hash(tuple(query_vector[:10]))}
        except Exception as e:
            logger.error(f'Single vector search {query_id} failed: {e}')
            raise

    async def batch_delete(self, vector_ids: list[str | int], tenant: str, workspace: str, batch_size: int | None=None) -> StepResult:
        ...
        try:
            if batch_size is None:
                effective_batch_size = min(self._get_adaptive_batch_size(namespace, 384) // 2, 50)
            else:
            ...
            for offset in range(0, total_ids, effective_batch_size):
                ...
                try:
                    if QDRANT_AVAILABLE:
                        from qdrant_client.http import models as qmodels
                        points_filter = qmodels.Filter(must=[qmodels.FieldCondition(key='id', match=qmodels.MatchAny(any=batch_ids))])
                        scroll_result = self.client.scroll(collection_name=physical, scroll_filter=points_filter, limit=effective_batch_size, with_vectors=False, with_payload=False)
                        if scroll_result[0]:
                            point_ids = [point.id for point in scroll_result[0]]
                            self.client.delete(collection_name=physical, points_selector=qmodels.PointIdsList(points=point_ids))
            ...
            return StepResult(success=True, data={'total_requested': total_ids, 'successful_deletes': successful_deletes, 'batches_processed': batches_processed, 'effective_batch_size': effective_batch_size, 'duration_ms': (time.time() - start_time) * 1000, 'namespace': namespace})
        except Exception as e:
            logger.error(f'Batch delete failed: {e}')
            return StepResult(success=False, error=f'Batch delete failed: {e!s}', custom_status='retryable')
    ...
    async def get_batch_operation_stats(self, tenant: str, workspace: str) -> StepResult:
        ...
        namespace = f'{tenant}:{workspace}'
        try:
            recent_operations = list(self._performance_history)[-50:]
            batch_operations = [op for op in recent_operations if op.operation.startswith('batch_')]
            if not batch_operations:
                return StepResult(success=True, data={'namespace': namespace, 'message': 'No batch operations recorded yet', 'total_operations': 0})
            ...
            return StepResult(success=True, data={'namespace': namespace, 'total_batch_operations': len(batch_operations), 'total_vectors_processed': total_vectors, 'average_duration_ms': avg_duration, 'average_vectors_per_operation': avg_vectors_per_op, 'operation_breakdown': operation_types, 'adaptive_batch_size': self._adaptive_batch_sizes.get(namespace, self._batch_size), 'performance_history_size': len(self._performance_history)})
        except Exception as e:
            logger.error(f'Failed to get batch operation stats: {e}')
            return StepResult(success=False, error=f'Failed to get batch operation stats: {e!s}', custom_status='retryable')

    async def compact_and_deduplicate(self, tenant: str, workspace: str, similarity_threshold: float | None=None, batch_size: int | None=None) -> StepResult:
        ...
        try:
            ...
            if not all_vectors:
                return StepResult(success=True, data={'message': 'No vectors found for compaction', 'namespace': namespace, 'duration_ms': (time.time() - start_time) * 1000})
            ...
            if not duplicates:
                return StepResult(success=True, data={'message': 'No duplicates found', 'namespace': namespace, 'vectors_analyzed': len(all_vectors), 'duplicates_found': 0, 'duration_ms': (time.time() - start_time) * 1000})
            ...
            if not deletion_result.success:
                return StepResult(success=False, error=f'Failed to delete duplicate vectors: {deletion_result.error}', custom_status='retryable')
            ...
            logger.info(f'Memory compaction completed: {len(duplicates)} duplicates found, {len(ids_to_remove)} vectors removed ({space_saved_percent:.1f}% space saved)')
            return StepResult(success=True, data={'namespace': namespace, 'vectors_analyzed': len(all_vectors), 'duplicates_found': len(duplicates), 'vectors_removed': len(ids_to_remove), 'space_saved_percent': space_saved_percent, 'similarity_threshold': similarity_threshold, 'duration_ms': (time.time() - start_time) * 1000, 'deletion_result': deletion_result.data})
        except Exception as e:
            logger.error(f'Memory compaction failed: {e}')
            return StepResult(success=False, error=f'Memory compaction failed: {e!s}', custom_status='retryable')
    ...
    async def _fetch_all_vectors_for_compaction(self, collection_name: str, batch_size: int) -> list[dict[str, Any]]:
        ...
        try:
            while True:
                if QDRANT_AVAILABLE:
                    scroll_result = self.client.scroll(collection_name=collection_name, limit=batch_size, offset=offset, with_vectors=True, with_payload=True)
                    points, next_offset = scroll_result
                    if not points:
                        break
                    for point in points:
                        vector_data = {'id': point.id, 'vector': list(point.vector) if point.vector else [], 'payload': dict(point.payload) if point.payload else {}}
    ...
    async def _find_duplicate_vectors(self, vectors: list[dict[str, Any]], similarity_threshold: float) -> list[dict[str, Any]]:
        ...
        batch_size = min(1000, total_vectors // 10)
        for i in range(0, total_vectors, batch_size):
            ...
            if i > 0:
                cross_batch_duplicates = self._find_cross_batch_duplicates(batch_vectors, vectors[:i], similarity_threshold)
    ...
    def _find_duplicates_in_batch(self, vectors: list[dict[str, Any]], similarity_threshold: float) -> list[dict[str, Any]]:
        ...
        for i, vec1 in enumerate(vectors):
            for _j, vec2 in enumerate(vectors[i + 1:], start=i + 1):
                ...
                if similarity > similarity_threshold:
                    duplicates.append({'original_id': vec1['id'], 'duplicate_id': vec2['id'], 'similarity': similarity, 'original_payload': vec1['payload'], 'duplicate_payload': vec2['payload']})
        return duplicates

    def _find_cross_batch_duplicates(self, current_batch: list[dict[str, Any]], previous_vectors: list[dict[str, Any]], similarity_threshold: float) -> list[dict[str, Any]]:
        ...
        for current_vec in current_batch:
            for prev_vec in previous_vectors:
                ...
                if similarity > similarity_threshold:
                    duplicates.append({'original_id': prev_vec['id'], 'duplicate_id': current_vec['id'], 'similarity': similarity, 'original_payload': prev_vec['payload'], 'duplicate_payload': current_vec['payload']})
        return duplicates
    ...
    async def get_memory_compaction_stats(self, tenant: str, workspace: str) -> StepResult:
        ...
        namespace = f'{tenant}:{workspace}'
        try:
            recent_operations = list(self._performance_history)[-100:]
            compaction_operations = [op for op in recent_operations if op.operation == 'memory_compaction']
            if not compaction_operations:
                return StepResult(success=True, data={'namespace': namespace, 'message': 'No compaction operations recorded yet', 'total_operations': 0})
            ...
            return StepResult(success=True, data={'namespace': namespace, 'total_compaction_operations': len(compaction_operations), 'total_vectors_processed': total_vectors_processed, 'average_vectors_per_compaction': avg_vectors_per_compaction, 'average_duration_ms': avg_duration, 'last_compaction': compaction_operations[-1].timestamp if compaction_operations else None, 'collection_info': collection_info})
        except Exception as e:
            logger.error(f'Failed to get memory compaction stats: {e}')
            return StepResult(success=False, error=f'Failed to get memory compaction stats: {e!s}', custom_status='retryable')
    ...
    async def _get_collection_info(self, collection_name: str) -> dict[str, Any]:
        ...
        try:
            if QDRANT_AVAILABLE:
                ...
                return {'points_count': collection_info.points_count, 'vectors_count': collection_info.vectors_count, 'indexed_vectors_count': collection_info.indexed_vectors_count, 'status': collection_info.status}
            else:
    ...
    def _track_operation_performance(self, operation: str, start_time: float, vectors_processed: int=0, bytes_transferred: int=0) -> None:
        """Track operation performance for adaptive optimization."""
        duration_ms = (time.time() - start_time) * 1000
        metric = MemoryOperationMetrics(operation=operation, duration_ms=duration_ms, vectors_processed=vectors_processed, bytes_transferred=bytes_transferred)
        self._performance_history.append(metric)
        self._analytics.add_operation_metric(metric)

    def enhanced_query(self, namespace: str, vector: Sequence[float], top_k: int=10, threshold: float=SIMILARITY_THRESHOLD_MEDIUM, include_vectors: bool=False) -> list[OptimizedSearchResult]:
        ...
        try:
            ...
            physical = self._physical_names.get(namespace, namespace)
            res = self.client.query_points(collection_name=physical, query=list(vector), limit=top_k, with_payload=True)
            vectors_processed = len(res.points)
            results = []
            for point in res.points:
                result = OptimizedSearchResult(vector_id=str(point.id), score=point.score, payload=dict(point.payload) if point.payload else {}, vector=list(point.vector) if include_vectors and point.vector else None, search_time_ms=(time.time() - start_time) * 1000, search_method='exact')
                results.append(result)
            return results
        finally:
            self._track_operation_performance('query', start_time, vectors_processed)

    def _optimized_similarity_search(self, namespace: str, vector: Sequence[float], top_k: int, threshold: float, include_vectors: bool) -> list[OptimizedSearchResult]:
        ...
        if cached_results is not None:
            ...
            for point in cached_results:
                result = OptimizedSearchResult(vector_id=str(point.id), score=point.score, payload=dict(point.payload) if point.payload else {}, vector=list(point.vector) if include_vectors and point.vector else None, search_time_ms=(time.time() - start_time) * 1000, search_method=search_method)
                results.append(result)
            return results
        try:
            ...
            if total_vectors > 1000 and threshold > SIMILARITY_THRESHOLD_HIGH:
                ...
                res = self.client.query_points(collection_name=physical, query=list(vector), limit=min(top_k * 2, MAX_SIMILARITY_SEARCH_RESULTS), with_payload=True, search_params={'hnsw_ef': 128, 'exact': False} if hasattr(self.client, 'query_points') else None)
        ...
        if results:
            ...
            for result in results[:top_k]:
                ...
                class CachedPoint:
                    ...
                    def __init__(self, vector_id: str, score: float, payload: dict[str, Any], vector: list[float] | None=None):
                        self.id = vector_id
                        self.score = score
                        self.payload = payload
                        self.vector = vector
                cache_results.append(CachedPoint(vector_id=result.vector_id, score=result.score, payload=result.payload, vector=result.vector))
            self._search_result_cache.set(cache_key, cache_results)
        return results

    def _convert_points_to_results(self, point_data: list[tuple[str, float]], collection_name: str, include_vectors: bool, search_time_ms: float, method: str) -> list[OptimizedSearchResult]:
        """Convert point data to OptimizedSearchResult objects."""
        results = []
        if point_data:
            point_ids = [pid for pid, _ in point_data]
            try:
                res = self.client.retrieve(collection_name=collection_name, ids=point_ids, with_payload=True, with_vectors=include_vectors)
                for i, point in enumerate(res):
                    if i < len(point_data):
                        vector_id, score = point_data[i]
                        result = OptimizedSearchResult(vector_id=vector_id, score=score, payload=dict(point.payload) if point.payload else {}, vector=list(point.vector) if include_vectors and point.vector else None, search_time_ms=search_time_ms, search_method=method)
                        results.append(result)
            except Exception as e:
                logger.warning(f'Failed to retrieve point details: {e}')
                for vector_id, score in point_data:
                    result = OptimizedSearchResult(vector_id=vector_id, score=score, payload={}, search_time_ms=search_time_ms, search_method=method)
                    results.append(result)
        return results
    ...
    def analyze_memory_usage(self) -> dict[str, Any]:
        ...
        try:
            ...
            for collection in all_collections:
                ...
                try:
                    ...
                    collections_info[collection_name] = {'points_count': info.points_count or 0, 'vectors_count': info.points_count or 0, 'status': 'active'}
                except Exception as e:
        ...
        return {'total_collections': len(collections_info), 'total_vectors': total_vectors, 'estimated_memory_mb': estimated_memory_mb, 'collections': collections_info, 'similarity_analysis': similarity_analysis, 'performance_metrics': self._analytics.get_performance_summary(), 'optimization_suggestions': self._generate_memory_optimization_suggestions(total_vectors, estimated_memory_mb, similarity_analysis)}
    ...
    def _analyze_similarity_patterns(self) -> dict[str, Any]:
        ...
        compaction_opportunities = high_similarity_count
        return {'avg_similarity': avg_similarity, 'high_similarity_vectors': high_similarity_count, 'compaction_opportunities': compaction_opportunities, 'similarity_distribution': {'very_high': len([s for s in all_scores if s > 0.95]), 'high': len([s for s in all_scores if 0.85 <= s <= 0.95]), 'medium': len([s for s in all_scores if 0.75 <= s < 0.85]), 'low': len([s for s in all_scores if s < 0.75])}}

    def _generate_memory_optimization_suggestions(self, total_vectors: int, memory_mb: float, similarity_analysis: dict[str, Any]) -> list[str]:
        ...
        if self._analytics.get_avg_throughput() < 100:
        ...
        if len(self._performance_history) > 50:
            ...
            if slow_operations > 10:
                suggestions.append('Detected slow operations. Consider optimizing query parameters or collection structure')
        if not suggestions:
    ...
    def compact_memory(self, namespace: str, similarity_threshold: float=DEDUPLICATION_THRESHOLD) -> dict[str, Any]:
        ...
        try:
            ...
            sample_points = self.client.query_points(collection_name=physical, query=[0.0] * self._dims.get(namespace, 384), limit=sample_size, with_payload=False, with_vectors=True)
            duplicates_found = 0
            ...
            for group in similar_groups:
                if group.avg_similarity >= similarity_threshold and len(group.vectors) > 1:
                    vectors_to_remove = group.vectors[1:]
            ...
            return {'compacted': duplicates_found, 'duplicates_removed': duplicates_found, 'compaction_time_ms': compaction_time * 1000, 'estimated_space_saved_mb': duplicates_found * 0.001}
        except Exception as e:
    ...
    def optimize_collection_structure(self, namespace: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {'collection_name': physical, 'current_structure': {'vectors': collection_info.points_count, 'dimensions': self._dims.get(namespace, 'unknown')}, 'optimizations': optimizations, 'applied': False}
        except Exception as e:
            return {'error': str(e)}
    ...
    def store_multimodal_embedding(self, content_id: str, content_type: str, text_embedding: list[float] | None=None, visual_embedding: list[float] | None=None, audio_embedding: list[float] | None=None, combined_embedding: list[float] | None=None, metadata: dict[str, Any] | None=None, tenant: str='default', workspace: str='default') -> StepResult:
        ...
        try:
            ...
            content_metadata.update({'content_type': content_type, 'content_id': content_id, 'tenant': tenant, 'workspace': workspace, 'timestamp': time.time()})
            vectors = {}
            ...
            return StepResult.ok(data={'content_id': content_id, 'content_type': content_type, 'stored_vectors': list(vectors.keys()), 'collection': collection_name, 'tenant': tenant, 'workspace': workspace})
        except Exception as e:
            logger.error(f'Failed to store multi-modal embedding: {e}')
            return StepResult.fail(f'Storage failed: {e!s}')

    def search_multimodal_content(self, query_embedding: list[float], content_types: list[str] | None=None, limit: int=10, threshold: float=0.7, tenant: str='default', workspace: str='default') -> StepResult:
        ...
        try:
            ...
            results = self._search_with_filter(collection_name, search_vector, limit=limit, score_threshold=threshold, filter_conditions=filter_conditions or None)
            formatted_results = []
            for result in results:
                formatted_results.append({'content_id': result.payload.get('content_id'), 'content_type': result.payload.get('content_type'), 'similarity_score': result.score, 'metadata': result.payload})
            logger.info(f'Found {len(formatted_results)} multi-modal content matches')
            return StepResult.ok(data={'results': formatted_results, 'total_found': len(formatted_results), 'content_types': content_types or ['all'], 'tenant': tenant, 'workspace': workspace})
        except Exception as e:
            logger.error(f'Multi-modal search failed: {e}')
            return StepResult.fail(f'Search failed: {e!s}')

    def get_content_themes(self, content_ids: list[str] | None=None, content_types: list[str] | None=None, tenant: str='default', workspace: str='default') -> StepResult:
        ...
        try:
            ...
            if content_types:
                filter_conditions['content_type'] = {'$in': content_types}
            all_themes = []
            scroll_results = self.client.scroll(collection_name=collection_name, limit=1000, scroll_filter=filter_conditions or None)
            ...
            return StepResult.ok(data={'dominant_themes': dominant_themes, 'theme_distribution': dict(theme_counts.most_common(20)), 'total_content_analyzed': len(scroll_results[0]), 'content_types': content_types or ['all'], 'tenant': tenant, 'workspace': workspace})
        except Exception as e:
    ...
    def _search_with_filter(self, collection_name: str, search_vector: list[float], limit: int=10, score_threshold: float=0.0, filter_conditions: dict[str, Any] | None=None) -> list[Any]:
        """Search with filter conditions and score threshold."""
        try:
            cache_key = self._generate_search_cache_key(collection_name, search_vector, limit, score_threshold, filter_conditions)
                ...
                if filter_conditions:
                    ...
                    for field, condition in filter_conditions.items():
                        if isinstance(condition, dict) and '$in' in condition:
                            must_conditions.append(qmodels.FieldCondition(key=field, match=qmodels.MatchAny(any=condition['$in'])))
                    if must_conditions:
                        query_filter = qmodels.Filter(must=must_conditions)
                search_result = self.client.search(collection_name=collection_name, query_vector=search_vector, limit=limit, score_threshold=score_threshold, query_filter=query_filter, with_payload=True, with_vectors=False)
    ...
    def _generate_search_cache_key(self, collection_name: str, search_vector: list[float], limit: int, score_threshold: float, filter_conditions: dict[str, Any] | None=None) -> str:
        """Generate cache key for search parameters."""
        vector_hash = hash(tuple(search_vector[:10]))
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        return {'search_result_cache': self._search_result_cache.get_stats(), 'similarity_cache_size': len(self._similarity_cache)}
