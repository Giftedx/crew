...
class VectorStore:
    ...
    async def _single_vector_search(
        ...
        try:
            ...
            return {
                ...
                "query_vector_hash": hash(tuple(query_vector[:10])),
            }
        except Exception as e:
    ...
    async def batch_delete(
        self, vector_ids: list[str | int], tenant: str, workspace: str, batch_size: int | None = None
    ) -> StepResult:
    ...
    async def get_batch_operation_stats(self, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            return StepResult(
                success=False, error=f"Failed to get batch operation stats: {e!s}", custom_status="retryable"
            )

    async def compact_and_deduplicate(
        self, tenant: str, workspace: str, similarity_threshold: float | None = None, batch_size: int | None = None
    ) -> StepResult:
        ...
        try:
            ...
            logger.info(
                f"Memory compaction completed: {len(duplicates)} duplicates found, {len(ids_to_remove)} vectors removed ({space_saved_percent:.1f}% space saved)"
            )
            return StepResult(
                success=True,
                data={
    ...
    def _find_cross_batch_duplicates(
        self, current_batch: list[dict[str, Any]], previous_vectors: list[dict[str, Any]], similarity_threshold: float
    ) -> list[dict[str, Any]]:
        ...
        for current_vec in current_batch:
            for prev_vec in previous_vectors:
                ...
                if similarity > similarity_threshold:
                    duplicates.append(
                        {
                            "original_id": prev_vec["id"],
                            "duplicate_id": current_vec["id"],
                            "similarity": similarity,
    ...
    async def get_memory_compaction_stats(self, tenant: str, workspace: str) -> StepResult:
        ...
        try:
            ...
            return StepResult(
                success=False, error=f"Failed to get memory compaction stats: {e!s}", custom_status="retryable"
            )
        ...
        try:
            ...
            if total_vectors > 1000 and threshold > SIMILARITY_THRESHOLD_HIGH:
                ...
                res = self.client.query_points(
                    collection_name=physical,
                    query=list(vector),
                    limit=min(top_k * 2, MAX_SIMILARITY_SEARCH_RESULTS),
                    with_payload=True,
    ...
    def compact_memory(self, namespace: str, similarity_threshold: float = DEDUPLICATION_THRESHOLD) -> dict[str, Any]:
        ...
        try:
            ...
            sample_points = self.client.query_points(
                collection_name=physical,
                query=[0.0] * self._dims.get(namespace, 384),
                limit=sample_size,
                with_payload=False,
                with_vectors=True,
            )
            ...
            return {
                ...
                "estimated_space_saved_mb": duplicates_found * 0.001,
            }
        except Exception as e:
    ...
    def get_content_themes(
        ...
        try:
            ...
            scroll_results = self.client.scroll(
                collection_name=collection_name, limit=1000, scroll_filter=filter_conditions or None
            )
