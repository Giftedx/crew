...
class ResilienceOrchestrator(BaseOrchestrator):
    ...
    def __init__(self, config: ResilienceConfig | None = None) -> None:
        ...
        logger.info(
            "resilience_orchestrator_initialized",
            config={
    ...
    async def orchestrate(
        ...
        **kwargs: Any,
        ...
        if not primary_func:
            result = StepResult.fail(
                "Missing required parameter: primary_func",
                error_category=ErrorCategory.PROCESSING,
            )
            self._log_orchestration_end(context, result)
            return result

        # Execute with selected resilience strategy
        ...
        try:
            if strategy == ResilienceStrategy.FAIL_FAST:
                ...
                result = StepResult.fail(
                    f"Unknown resilience strategy: {strategy}",
                    error_category=ErrorCategory.PROCESSING,
                )
                self._log_orchestration_end(context, result)
                return result
            ...
            result = StepResult.fail(
                f"Resilience execution failed: {e}",
                error_category=ErrorCategory.PROCESSING,
                metadata={
                    "service": service_name,
                    "strategy": strategy.value,
    ...
    def _start_health_monitoring(self) -> None:
        ...
        try:
            ...
            # No event loop available - will start on first async call
    ...
    async def _execute_with_adaptive_routing(
        self, service_name: str, func: Callable[..., Awaitable[T]], **kwargs: Any
    ) -> T:
        """Execute with adaptive routing based on health."""
        # Check if service is healthy
        if not self._is_service_healthy(service_name):
            logger.warning(
                "service_unhealthy_enabling_degraded_mode",
                service=service_name,
            )
            # Could route to alternative service or enable degraded mode
            # For now, just proceed with warning

        # Apply routing weight (could be used for load balancing)
    ...
    def _get_or_create_circuit_breaker(
        self, service_name: str, fallback_func: Callable[..., Awaitable[T]] | None
    ) -> CircuitBreaker:
    ...
    def _update_routing_weight(self, service_name: str, success: bool, latency: float) -> None:
        ...
        if success and latency < self.config.health_latency_threshold:
            ...
            new_weight = min(
                current_weight * 1.1,
                self.config.routing_weight_max,
            )
        else:
            # Decrease weight for failures or slow requests
            new_weight = max(
                current_weight * 0.9,
                self.config.routing_weight_min,
            )
        ...
        for service_name, health in self.service_health.items():
            ...
            if was_healthy != is_healthy:
                logger.info(
                    ...
                    service=service_name,
                    old_status="healthy" if was_healthy else "unhealthy",
                    new_status="healthy" if is_healthy else "unhealthy",
                    success_rate=(health.success_count / health.total_requests if health.total_requests > 0 else 0),
                )
        ...
        return {
            ...
            "service_health": {
                name: {
                    ...
                    "success_rate": (health.success_count / health.total_requests if health.total_requests > 0 else 0),
                    "avg_response_time": (
                        sum(health.response_times) / len(health.response_times) if health.response_times else 0
                    ),
                }
...
def resilient_execute(
    service_name: str,
    strategy: ResilienceStrategy = ResilienceStrategy.RETRY,
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            # Create minimal context for decorator usage
            from core.orchestration import OrchestrationContext

            context = OrchestrationContext(
                tenant_id="default",
                request_id=f"decorator-{id(func)}",
            )
