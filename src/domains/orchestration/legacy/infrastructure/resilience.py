...
class ResilienceOrchestrator(BaseOrchestrator):
    ...
    def __init__(self, config: ResilienceConfig | None = None) -> None:
        ...
        """
        super().__init__(
            layer=OrchestrationLayer.INFRASTRUCTURE, name="resilience", orchestration_type=OrchestrationType.ADAPTIVE
        )
    ...
    async def orchestrate(self, context: OrchestrationContext, **kwargs: Any) -> StepResult:
        ...
        if not service_name:
            result = StepResult.fail(
                "Missing required parameter: service_name", error_category=ErrorCategory.PROCESSING
            )
            self._log_orchestration_end(context, result)
            return result
        if not primary_func:
            result = StepResult.fail(
                "Missing required parameter: primary_func", error_category=ErrorCategory.PROCESSING
            )
            self._log_orchestration_end(context, result)
            return result
        ...
        try:
            if strategy == ResilienceStrategy.FAIL_FAST:
                ...
                result = StepResult.fail(
                    f"Unknown resilience strategy: {strategy}", error_category=ErrorCategory.PROCESSING
                )
                self._log_orchestration_end(context, result)
                return result
    ...
    def _get_or_create_circuit_breaker(
        ...
        if service_name not in self.circuit_breakers:
            ...
            self.circuit_breakers[service_name] = CircuitBreaker(
                name=service_name, config=config, fallback_func=fallback_func
            )
...
def resilient_execute(
    service_name: str, strategy: ResilienceStrategy = ResilienceStrategy.RETRY
