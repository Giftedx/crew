...
def create_transcription_task(agent: Any, acquisition_task: Task, callback: Any | None = None) -> Task:
    ...
    """
    return Task(
        description="STEP 1: Extract the 'file_path' field from the previous task's JSON output. STEP 2: YOU MUST CALL AudioTranscriptionTool(file_path=<extracted_path>). DO NOT generate placeholder text like 'Your transcribed text goes here'. DO NOT respond until the tool returns actual transcript data. STEP 3: Create timeline anchors for key moments using the real transcript. \n\nCRITICAL: Return your results as JSON with these exact keys: transcript, timeline_anchors, transcript_length, quality_score. Wrap the JSON in ```json``` code blocks.",
        expected_output="JSON with transcript (min 1000 chars from AudioTranscriptionTool), timeline_anchors (list of dicts), transcript_length (int), quality_score (0.0-1.0). ❌ REJECT: Placeholder text. ❌ REJECT: transcript_length < 100. ✅ ACCEPT: Real transcript with actual spoken words from the media file.",
        agent=agent,
...
def create_analysis_tasks(
    ...
    if enable_parallel:
        ...
        text_analysis_task = Task(
            description="STEP 1: Extract 'transcript' field from previous task's JSON output. STEP 2: Perform comprehensive text analysis including: - Linguistic patterns and readability analysis - Theme extraction and topic modeling - Sentiment analysis and emotional indicators - Keyword extraction and frequency analysis - Content structure and organization analysis \n\nCRITICAL: Return your results as JSON with these exact keys: linguistic_patterns, themes, sentiment_analysis, keywords, content_structure. Wrap the JSON in ```json``` code blocks.",
        ...
        fact_checking_task = Task(
            description="STEP 1: Extract 'transcript' field from previous task's JSON output. STEP 2: Perform comprehensive fact-checking analysis including: - Claim identification and extraction - Source verification and credibility assessment - Logical fallacy detection - Evidence quality evaluation - Truth assessment and confidence scoring \n\nCRITICAL: Return your results as JSON with these exact keys: claims, source_verification, logical_analysis, evidence_quality, truth_assessment. Wrap the JSON in ```json``` code blocks.",
        ...
        bias_analysis_task = Task(
            description="STEP 1: Extract 'transcript' field from previous task's JSON output. STEP 2: Perform comprehensive bias and manipulation analysis including: - Bias indicators and type classification - Deception detection and manipulation techniques - Narrative integrity assessment - Psychological threat evaluation - Trustworthiness scoring \n\nCRITICAL: Return your results as JSON with these exact keys: bias_indicators, deception_analysis, narrative_integrity, psychological_threats, trustworthiness. Wrap the JSON in ```json``` code blocks.",
        ...
        analysis_task = Task(
            description=f"STEP 1: Extract 'transcript' field from previous task's JSON output. STEP 2: Perform comprehensive content analysis including: - Linguistic patterns and readability analysis - Theme extraction and topic modeling - Sentiment analysis and emotional indicators - Fact-checking and claim verification - Bias detection and manipulation analysis - Source credibility assessment \n\nDepth level: {depth} - adjust analysis thoroughness accordingly. \n\nCRITICAL: Return your results as JSON with these exact keys: linguistic_patterns, themes, sentiment_analysis, fact_checking, bias_analysis, credibility. Wrap the JSON in ```json``` code blocks.",
...
def create_knowledge_integration_task(
    agent: Any, previous_tasks: list[Task], depth: str, callback: Any | None = None
) -> Task:
    ...
    """
    return Task(
        description=f"STEP 1: Extract and synthesize data from ALL previous task outputs. STEP 2: Perform comprehensive knowledge integration including: - Cross-reference findings from all analysis stages - Identify patterns and correlations across data sources - Generate actionable insights and recommendations - Create comprehensive summary with key findings - Assess overall content quality and reliability \n\nDepth level: {depth} - adjust integration thoroughness accordingly. \n\nCRITICAL: Return your results as JSON with these exact keys: synthesis_analysis, key_insights, recommendations, quality_assessment, summary. Wrap the JSON in ```json``` code blocks.",
...
def build_crew_with_tasks(
    tasks: list[Task], process_type: Process = Process.sequential, logger_instance: logging.Logger | None = None
) -> Crew:
...
def build_intelligence_crew(
    ...
    _logger.info(
        f"⚡ Parallel execution: analysis={parallel_config['enable_parallel_analysis']}, fact_checking={parallel_config['enable_parallel_fact_checking']}, memory={parallel_config['enable_parallel_memory_ops']}"
    )
