...
from platform.realtime.live_monitor import AlertLevel, LiveContentMetrics, LiveMonitor, MonitoringAlert, MonitoringRule, MonitorType, TrendData
from platform.realtime.stream_processor import ProcessingPriority, ProcessingResult, StreamChunk, StreamProcessor, StreamType
...
class LiveMonitoringAgent:
    ...
    def __init__(self):
        ...
        self.performance_metrics = {'total_sessions': 0, 'active_sessions': 0, 'trends_detected': 0, 'fact_checks_triggered': 0, 'average_response_time': 0.0}
        self.agent = None
        logger.info('Live Monitoring Agent initialized')

    async def start_monitoring_session(self, stream_id: str, stream_type: StreamType, monitoring_mode: MonitoringMode=MonitoringMode.INTELLIGENT, configuration: dict[str, Any] | None=None, tenant: str='default', workspace: str='default') -> StepResult:
        """
        ...
        try:
            ...
            session = LiveMonitoringSession(session_id=session_id, stream_id=stream_id, stream_type=stream_type, monitoring_mode=monitoring_mode, start_time=time.time(), configuration=configuration or {}, tenant=tenant, workspace=workspace)
            stream_metadata = await self.stream_processor.start_stream(stream_id=stream_id, stream_type=stream_type, source_url=configuration.get('source_url', ''), title=configuration.get('title', f'Stream {stream_id}'), description=configuration.get('description', ''), priority=ProcessingPriority.HIGH if monitoring_mode == MonitoringMode.INTELLIGENT else ProcessingPriority.NORMAL)
            ...
            return StepResult.success({'session_id': session_id, 'stream_id': stream_id, 'monitoring_mode': monitoring_mode.value, 'status': 'active', 'stream_metadata': stream_metadata.__dict__, 'tenant': tenant, 'workspace': workspace})
    ...
    async def stop_monitoring_session(self, session_id: str) -> StepResult:
        ...
        try:
            ...
            return StepResult.success({'session_id': session_id, 'status': 'completed', 'duration_seconds': session.duration_seconds, 'summary': summary})
        except Exception as e:
    ...
    async def process_stream_chunk(self, session_id: str, chunk: StreamChunk) -> StepResult:
        ...
        try:
            ...
            return StepResult.success({'session_id': session_id, 'chunk_processed': True, 'processing_result': latest_result.__dict__ if latest_result else None})
        except Exception as e:
        ...
        try:
            ...
            if metric_name:
                ...
                for metric in ['viewer_count', 'engagement_rate', 'sentiment_score', 'content_quality']:
                    ...
                    if trend:
                        trends.append(trend)
            return StepResult.success({'session_id': session_id, 'trends': [trend.__dict__ for trend in trends], 'analysis_timestamp': time.time()})
        except Exception as e:
    ...
    async def _setup_monitoring_rules(self, session: LiveMonitoringSession) -> None:
        ...
        rules = []
        rules.append(MonitoringRule(rule_id=f'{session.session_id}_sentiment', monitor_type=MonitorType.SENTIMENT_MONITORING, name='Sentiment Monitoring', description='Monitor sentiment trends', condition='sentiment_score < 0.3', threshold=0.3, alert_level=AlertLevel.WARNING))
        rules.append(MonitoringRule(rule_id=f'{session.session_id}_quality', monitor_type=MonitorType.QUALITY_MONITORING, name='Content Quality Monitoring', description='Monitor content quality', condition='content_quality < 0.6', threshold=0.6, alert_level=AlertLevel.WARNING))
        rules.append(MonitoringRule(rule_id=f'{session.session_id}_engagement', monitor_type=MonitorType.AUDIENCE_ENGAGEMENT, name='Engagement Monitoring', description='Monitor audience engagement', condition='engagement_rate < 0.2', threshold=0.2, alert_level=AlertLevel.INFO))
        for rule in rules:
            self.live_monitor.add_monitoring_rule(rule)

    def _create_metrics_from_result(self, result: ProcessingResult, session: LiveMonitoringSession) -> LiveContentMetrics:
        """Create metrics from processing result."""
        result_data = result.result_data
        return LiveContentMetrics(stream_id=session.stream_id, timestamp=time.time(), viewer_count=result_data.get('viewer_count', 0), engagement_rate=result_data.get('engagement_rate', 0.0), sentiment_score=result_data.get('sentiment_score', 0.5), content_quality=result_data.get('content_quality', 0.8), fact_check_accuracy=result_data.get('fact_check_accuracy', 0.9), moderation_flags=result_data.get('moderation_flags', 0), performance_score=result_data.get('performance_score', 0.8), metadata=result_data.get('metadata', {}))
    ...
    async def _check_for_trends_and_alerts(self, session: LiveMonitoringSession, result: ProcessingResult) -> None:
        ...
        if result.result_data.get('engagement_rate', 0) > 0.8:
            trend_alert = TrendAlert(alert_id=str(uuid.uuid4()), trend_category=TrendCategory.VIRAL_CONTENT, confidence=0.9, description='High engagement detected - potential viral content', timestamp=time.time(), stream_id=session.stream_id, impact_score=0.8, recommended_actions=['Monitor closely', 'Prepare response strategy'])
        ...
        if result.result_data.get('fact_check_opportunity', False):
            fact_alert = FactCheckAlert(alert_id=str(uuid.uuid4()), claim=result.result_data.get('claim', ''), confidence=0.7, source_context=result.result_data.get('context', ''), timestamp=time.time(), stream_id=session.stream_id, verification_status='pending', suggested_response='Verify claim with reliable sources', priority='high')
    ...
    async def _generate_session_summary(self, session: LiveMonitoringSession) -> dict[str, Any]:
        ...
        return {'session_id': session.session_id, 'stream_id': session.stream_id, 'duration_seconds': session.duration_seconds, 'total_metrics': len(session.metrics), 'total_alerts': len(session.alerts), 'trends_detected': len([a for a in self.trend_alerts if a.stream_id == session.stream_id]), 'fact_checks_triggered': len([a for a in self.fact_check_alerts if a.stream_id == session.stream_id]), 'monitoring_mode': session.monitoring_mode.value, 'performance_summary': {'average_engagement': sum((m.engagement_rate for m in session.metrics)) / max(1, len(session.metrics)), 'average_sentiment': sum((m.sentiment_score for m in session.metrics)) / max(1, len(session.metrics)), 'average_quality': sum((m.content_quality for m in session.metrics)) / max(1, len(session.metrics))}}

    async def get_agent_status(self) -> StepResult:
        """Get the current status of the live monitoring agent."""
        try:
            return StepResult.success({'agent_type': 'LiveMonitoringAgent', 'status': 'active', 'active_sessions': len(self.active_sessions), 'performance_metrics': self.performance_metrics, 'monitoring_statistics': await self.live_monitor.get_monitoring_statistics(), 'stream_statistics': await self.stream_processor.get_stream_statistics()})
        except Exception as e:
