...
class CrewInsightGenerator:
    ...
    def generate_insights(self, result: CrewExecutionResult) -> dict[str, Any]:
        ...
        """
        insights: dict[str, Any] = {'task_id': result.task_id, 'success': result.step_result.success, 'execution_time_seconds': result.execution_time_seconds, 'performance': self._analyze_performance(result), 'resource_usage': self._analyze_resource_usage(result), 'recommendations': self._generate_recommendations(result)}
        logger.debug('crew_insights_generated', tenant_id=self.tenant_id, task_id=result.task_id, insight_count=len(insights))
        return insights
    ...
    def _analyze_performance(self, result: CrewExecutionResult) -> dict[str, Any]:
        ...
        if result.execution_time_seconds < 5:
            ...
            speed = 'very_slow'
        return {'speed_category': speed, 'execution_time_seconds': result.execution_time_seconds, 'cache_efficiency': result.cache_hits / (result.cache_hits + 1) if result.cache_hits > 0 else 0.0, 'retry_rate': result.retry_count}
    ...
    def _analyze_resource_usage(self, result: CrewExecutionResult) -> dict[str, Any]:
        ...
        """
        return {'agents_count': len(result.agents_used), 'agents_used': result.agents_used, 'tools_count': len(result.tools_used), 'tools_used': result.tools_used, 'cache_hits': result.cache_hits}
