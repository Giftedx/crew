...
class TrustworthinessTrackerTool(BaseTool[StepResult]):
    ...
    def _load(self) -> dict[str, _PersonRecord]:
        ...
        for k, v in raw.items():
            if isinstance(v, dict) and {'truths', 'lies', 'score'}.issubset(v.keys()):
                try:
                    out[k] = {'truths': int(v.get('truths', 0)), 'lies': int(v.get('lies', 0)), 'score': float(v.get('score', 0.0))}
                except Exception as exc:
    ...
    def _run(self, person: str, verdict: bool) -> StepResult:
        ...
        """
        if not person:
            self.metrics.counter('tool_runs_total', labels={'tool': 'trustworthiness_tracker', 'outcome': 'skipped'}).inc()
        ...
        try:
            with self._lock:
                ...
                data[person] = record
                self._save(data)
            self.metrics.histogram('trustworthiness_score', score, labels={'source_type': self._classify_source(person)})
            self.metrics.counter('tool_runs_total', labels={'tool': 'trustworthiness_tracker', 'outcome': 'success'}).inc()
            return StepResult.ok(person=person, truths=record['truths'], lies=record['lies'], score=record['score'])
        except Exception as e:
            self.metrics.counter('tool_runs_total', labels={'tool': 'trustworthiness_tracker', 'outcome': 'error'}).inc()
    ...
    def get_report(self) -> StepResult:
        ...
        try:
            ...
            for person, record in data.items():
                report.append({'source': person, 'truths': record['truths'], 'lies': record['lies'], 'trustworthiness': record['score'], 'classification': self._classify_trustworthiness(record['score'])})
            timestamp = default_utc_now()
            self.metrics.counter('tool_runs_total', labels={'tool': 'trustworthiness_tracker_report', 'outcome': 'success'}).inc()
            return StepResult.ok(report=report, total_sources=len(report), timestamp=timestamp.isoformat())
        except Exception as e:
            self.metrics.counter('tool_runs_total', labels={'tool': 'trustworthiness_tracker_report', 'outcome': 'error'}).inc()
            return StepResult.fail(error=str(e))
