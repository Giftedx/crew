...
class CrossPlatformDeduplicationService:
    ...
    def __init__(self, cache_size: int=1000):
        ...
        self._text_hashes: dict[str, list[float]] = {}

    def find_duplicates(self, image_paths: list[str] | None=None, text_items: list[dict[str, Any]] | None=None, platforms: list[str] | None=None, similarity_threshold: float=0.8, model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if not image_paths and (not text_items):
                return StepResult.fail('Either image_paths or text_items must be provided', status='bad_request')
            if use_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'duplicate_clusters': [c.__dict__ for c in cache_result.duplicate_clusters], 'total_items_processed': cache_result.total_items_processed, 'duplicates_found': cache_result.duplicates_found, 'unique_items': cache_result.unique_items, 'deduplication_method': cache_result.deduplication_method, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            model_name = self._select_model(model)
            deduplication_result = self._find_duplicates(image_paths, text_items, platforms, similarity_threshold, model_name)
            if deduplication_result:
                if use_cache:
                    self._cache_result(image_paths, text_items, platforms, similarity_threshold, model, deduplication_result)
                processing_time = (time.time() - start_time) * 1000
                return StepResult.ok(data={'duplicate_clusters': [c.__dict__ for c in deduplication_result.duplicate_clusters], 'total_items_processed': deduplication_result.total_items_processed, 'duplicates_found': deduplication_result.duplicates_found, 'unique_items': deduplication_result.unique_items, 'deduplication_method': deduplication_result.deduplication_method, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def deduplicate_content_stream(self, content_items: list[dict[str, Any]], similarity_threshold: float=0.8, model: Literal['fast', 'balanced', 'quality']='balanced') -> StepResult:
        ...
        try:
            ...
            return StepResult.ok(data={'processed_items': processed_items, 'total_items_processed': total_items, 'duplicates_found': duplicates, 'unique_items': unique_items, 'deduplication_method': 'streaming', 'similarity_threshold': similarity_threshold})
        except Exception as e:
    ...
    def _select_model(self, model_alias: str) -> str:
        ...
        """
        model_configs = {'fast': 'fast_deduplication', 'balanced': 'balanced_deduplication', 'quality': 'quality_deduplication'}
        return model_configs.get(model_alias, 'balanced_deduplication')

    def _find_duplicates(self, image_paths: list[str] | None, text_items: list[dict[str, Any]] | None, platforms: list[str] | None, similarity_threshold: float, model_name: str) -> DeduplicationResult | None:
        ...
        try:
            ...
            return DeduplicationResult(duplicate_clusters=all_clusters, total_items_processed=total_items, duplicates_found=duplicates_found, unique_items=total_items - duplicates_found, deduplication_method=model_name)
        except Exception as e:
            logger.error(f'Deduplication failed: {e}')
            return None
            ...
            try:
                ...
                for cluster_id, existing_hash in processed_hashes.items():
                    ...
                    if similarity >= threshold:
                        clusters[cluster_id].platform_items['images'].append({'path': image_path, 'hash': image_hash, 'similarity': similarity})
                        clusters[cluster_id].similarity_scores[image_path] = similarity
                        found_cluster = True
                        break
                if not found_cluster:
                    cluster_id = f'image_cluster_{len(clusters)}'
                    new_cluster = DuplicateCluster(cluster_id=cluster_id, platform_items={'images': [{'path': image_path, 'hash': image_hash}]}, similarity_scores={image_path: 1.0}, representative_item={'path': image_path, 'hash': image_hash}, confidence=1.0)
        ...
        for i, item1 in enumerate(text_items):
            ...
            for j, item2 in enumerate(text_items[i + 1:], i + 1):
                ...
                if similarity >= threshold:
                    ...
                    cluster = DuplicateCluster(cluster_id=cluster_id, platform_items={'text': [item1, item2]}, similarity_scores={item1_id: similarity, item2_id: similarity}, representative_item=item1, confidence=similarity)
    ...
    def _get_text_hash(self, text: str) -> list[float]:
        ...
        for i in range(768):
            ...
            embedding.append(float(bit_value) * 2 - 1)
        return embedding
    ...
    def _check_cache(self, image_paths: list[str] | None, text_items: list[dict[str, Any]] | None, platforms: list[str] | None, similarity_threshold: float, model: str) -> DeduplicationResult | None:
    ...
    def _cache_result(self, image_paths: list[str] | None, text_items: list[dict[str, Any]] | None, platforms: list[str] | None, similarity_threshold: float, model: str, result: DeduplicationResult) -> None:
    ...
    def get_cache_stats(self) -> StepResult:
        ...
        """
        try:
            stats = {'total_cached': len(self._deduplication_cache), 'cache_size_limit': self.cache_size, 'utilization': len(self._deduplication_cache) / self.cache_size if self.cache_size > 0 else 0.0, 'models_cached': {}}
