...
class SafetyBrandSuitabilityService:
    ...
    def __init__(self, cache_size: int=1000):
        ...
        self._policy_categories = {'hate_speech': 'Content that promotes hatred or discrimination', 'violence': 'Content depicting or promoting violence', 'adult_content': 'Adult or sexual content', 'profanity': 'Profane or inappropriate language', 'misinformation': 'False or misleading information', 'spam': 'Spam or promotional content', 'harassment': 'Harassment or bullying content', 'illegal_activity': 'Content promoting illegal activities'}

    def analyze_content(self, content: str, brand_guidelines: dict[str, Any] | None=None, speaker: str | None=None, timestamp: float | None=None, model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if use_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'safety': cache_result.safety.__dict__, 'brand_suitability': cache_result.brand_suitability.__dict__, 'policy_compliance': cache_result.policy_compliance.__dict__, 'content_segment': cache_result.content_segment, 'speaker': cache_result.speaker, 'timestamp': cache_result.timestamp, 'analysis_confidence': cache_result.analysis_confidence, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            ...
            if analysis_result:
                ...
                return StepResult.ok(data={'safety': analysis_result.safety.__dict__, 'brand_suitability': analysis_result.brand_suitability.__dict__, 'policy_compliance': analysis_result.policy_compliance.__dict__, 'content_segment': analysis_result.content_segment, 'speaker': analysis_result.speaker, 'timestamp': analysis_result.timestamp, 'analysis_confidence': analysis_result.analysis_confidence, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def analyze_segments(self, segments: list[dict[str, Any]], brand_guidelines: dict[str, Any] | None=None, model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            for segment in segments:
                ...
                if not segment_content:
                    continue
                segment_result = self.analyze_content(content=segment_content, brand_guidelines=brand_guidelines, speaker=speaker, timestamp=timestamp, model=model, use_cache=False)
            ...
            return StepResult.ok(data={'results': results, 'total_segments': len(segments), 'analyzed_segments': len(results), 'average_safety_score': avg_safety, 'average_suitability_score': avg_suitability, 'average_compliance_score': avg_compliance, 'overall_compliance_score': (avg_safety + avg_suitability + avg_compliance) / 3, 'model': model})
        except Exception as e:
    ...
    def _analyze_content(self, content: str, brand_guidelines: dict[str, Any] | None, speaker: str | None, timestamp: float | None, model_name: str) -> SafetyBrandSuitabilityResult | None:
    ...
    def _analyze_with_transformers(self, content: str, brand_guidelines: dict[str, Any] | None, speaker: str | None, timestamp: float | None, model_name: str) -> SafetyBrandSuitabilityResult:
        ...
        if self._safety_classifier is None:
            ...
            try:
                self._safety_classifier = pipeline('text-classification', model='unitary/toxic-bert', return_all_scores=True)
            except Exception:
        ...
        return SafetyBrandSuitabilityResult(safety=safety, brand_suitability=brand_suitability, policy_compliance=policy_compliance, content_segment=content, speaker=speaker, timestamp=timestamp, analysis_confidence=avg_confidence)
        ...
        if self._safety_classifier:
            try:
                ...
                if results and isinstance(results[0], list):
                    ...
                    return SafetyClassification(safety_level=safety_level, confidence=confidence, risk_factors=risk_factors, compliance_score=compliance_score)
            except Exception as e:
    ...
    def _analyze_brand_suitability(self, content: str, brand_guidelines: dict[str, Any] | None) -> BrandSuitability:
        ...
        brand_friendly_score = sum((1 for word in brand_friendly_words if word in content_lower)) / len(brand_friendly_words)
        ...
        return BrandSuitability(suitability_score=suitability_score, brand_alignment=brand_alignment, target_audience=target_audience, content_warnings=content_warnings, sponsorship_readiness=sponsorship_readiness)
    ...
    def _analyze_policy_compliance(self, content: str) -> PolicyCompliance:
        ...
        return PolicyCompliance(compliance_score=compliance_score, violated_policies=violated_policies, compliance_flags=compliance_flags, recommendations=recommendations)
    ...
    def _check_policy_violation(self, content_lower: str, policy: str) -> bool:
        ...
        """
        policy_indicators = {'hate_speech': ['hate', 'racist', 'sexist', 'homophobic', 'discrimination'], 'violence': ['kill', 'murder', 'assault', 'attack', 'fight', 'violent'], 'adult_content': ['sex', 'sexual', 'porn', 'adult', 'explicit'], 'profanity': ['fuck', 'shit', 'damn', 'hell', 'ass'], 'misinformation': ['fake', 'false', 'lie', 'wrong', 'incorrect'], 'spam': ['buy now', 'click here', 'limited time', 'discount'], 'harassment': ['bully', 'harass', 'threaten', 'intimidate'], 'illegal_activity': ['drugs', 'steal', 'illegal', 'crime']}
    ...
    def _analyze_safety_rules(self, content: str) -> SafetyClassification:
        ...
        """
        content_lower = content.lower()
        unsafe_indicators = {'hate_speech': ['hate', 'racist', 'sexist', 'homophobic'], 'violence': ['kill', 'murder', 'assault', 'attack'], 'profanity': ['fuck', 'shit', 'damn', 'hell']}
        risk_factors = []
        max_risk_score = 0.0
        ...
        if max_risk_score > 0.6:
            ...
            safety_level = 'safe'
        return SafetyClassification(safety_level=safety_level, confidence=min(max_risk_score + 0.5, 1.0), risk_factors=risk_factors, compliance_score=max(0.0, 1.0 - max_risk_score))

    def _check_cache(self, content: str, brand_guidelines: dict[str, Any] | None, model: str) -> SafetyBrandSuitabilityResult | None:
    ...
    def _cache_result(self, content: str, brand_guidelines: dict[str, Any] | None, model: str, result: SafetyBrandSuitabilityResult) -> None:
