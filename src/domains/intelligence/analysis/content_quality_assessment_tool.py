    ...
    def run(self, input_data: dict) -> StepResult:
        ...
        try:
            ...
            result = {
                ...
                "should_process": should_process_fully,
                "overall_score": metrics.overall_quality_score,
                "recommendation": self._get_processing_recommendation(metrics),
                "recommendation_details": self._get_processing_recommendation(metrics),
                "bypass_reason": None if should_process_fully else self._get_bypass_reason(metrics),
            }
            try:
                self._metrics.counter(
                    "tool_runs_total", labels={"tool": self.name or "content_quality_assessment", "outcome": "success"}
                ).inc()
            ...
            try:
                self._metrics.counter(
                    "tool_runs_total", labels={"tool": self.name or "content_quality_assessment", "outcome": "error"}
                ).inc()
            except Exception as exc:
