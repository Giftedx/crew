...
def _cohere_rerank(query: str, docs: list[str], top_n: int) -> RerankResult:
    ...
    payload = {'query': query, 'documents': docs, 'top_n': min(max(1, top_n), len(docs)), 'model': 'rerank-english-v3.0'}
    headers = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
    resp = resilient_post('https://api.cohere.ai/v1/rerank', json_payload=payload, headers=headers, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
    data_any: Any
    try:
        data_any = resp.json()
    except Exception:
    ...
    if not isinstance(results, list):
        record_degradation(component='rerank', event_type='cohere_identity_fallback', severity='warn', detail='cohere response missing results; using identity order')
    ...
    if not idxs:
        ...
        scores = [0.0] * len(docs)
        record_degradation(component='rerank', event_type='cohere_empty_results', severity='warn', detail='cohere returned no usable indexes; identity order applied')
...
def rerank(query: str, docs: list[str], *, provider: str, top_n: int) -> RerankResult:
    ...
    if prov == 'jina':
        ...
        payload = {'model': 'jina-reranker-v2-base-en', 'query': query, 'documents': docs, 'top_n': min(max(1, top_n), len(docs))}
        headers = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
        resp = resilient_post('https://api.jina.ai/v1/rerank', json_payload=payload, headers=headers, timeout_seconds=REQUEST_TIMEOUT_SECONDS)
        try:
            data = resp.json()
        except Exception:
            data = {}
        ...
        if not idxs:
            idxs = list(range(len(docs)))
            scores = [0.0] * len(docs)
            record_degradation(component='rerank', event_type='jina_empty_results', severity='warn', detail='jina returned no usable indexes; identity order applied')
        return RerankResult(indexes=idxs, scores=scores)
