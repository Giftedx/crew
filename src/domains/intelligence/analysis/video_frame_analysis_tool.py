...
class VideoFrameAnalysisTool(BaseTool[StepResult]):
    ...
    def __init__(self, max_frames: int=10, scene_threshold: float=0.3, enable_ocr: bool=True, enable_face_detection: bool=True, vision_model: str='gpt-4-vision-preview'):
        super().__init__()
    ...
    def _run(self, video_path: str, tenant: str='default', workspace: str='default', analysis_depth: str='standard') -> StepResult:
        """
        ...
        try:
            ...
            result: VideoAnalysisResult = {'total_frames': len(key_frames), 'analyzed_frames': len(frame_analyses), 'duration_seconds': self._get_video_duration(video_path), 'key_frames': frame_analyses, 'scene_transitions': self._detect_scene_transitions(frame_analyses), 'overall_sentiment': overall_analysis.get('overall_sentiment', 'neutral'), 'dominant_objects': overall_analysis.get('dominant_objects', []), 'text_timeline': overall_analysis.get('text_timeline', []), 'processing_time': processing_time}
    ...
    def _extract_key_frames(self, video_path: str, analysis_depth: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            max_frames = {'standard': min(self._max_frames, 5), 'deep': min(self._max_frames, 10), 'comprehensive': self._max_frames}.get(analysis_depth, self._max_frames)
            ...
            while True:
                ...
                if frame_id % interval == 0:
                    ...
                    frame_data = {'frame_id': frame_id, 'timestamp': timestamp, 'frame': frame, 'frame_base64': self._frame_to_base64(frame)}
    ...
    def _analyze_frame(self, frame_data: dict[str, Any], analysis_depth: str) -> FrameAnalysisResult | None:
        ...
        try:
            ...
            result: FrameAnalysisResult = {'timestamp': frame_data.get('timestamp', 0.0), 'frame_id': frame_data.get('frame_id', 0), 'objects': analysis.get('objects', []), 'faces': analysis.get('faces', []), 'text_content': analysis.get('text_content', ''), 'scene_type': analysis.get('scene_type', 'unknown'), 'visual_sentiment': analysis.get('visual_sentiment', 'neutral'), 'confidence_scores': analysis.get('confidence_scores', {}), 'raw_analysis': analysis}
            return result
        except Exception as e:
    ...
    def _analyze_frame_with_gpt4v(self, frame_base64: str, analysis_depth: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(model=self._vision_model, messages=[{'role': 'user', 'content': [{'type': 'text', 'text': prompt}, {'type': 'image_url', 'image_url': {'url': f'data:image/jpeg;base64,{frame_base64}'}}]}], max_tokens=1000)
            ...
            try:
                ...
                return {'objects': [], 'text_content': '', 'scene_type': 'unknown', 'visual_sentiment': 'neutral', 'raw_response': response_text}
        except Exception as e:
    ...
    def _analyze_frame_fallback(self) -> dict[str, Any]:
        ...
        return {'objects': [], 'faces': [], 'text_content': '', 'scene_type': 'unknown', 'visual_sentiment': 'neutral', 'confidence_scores': {'overall': 0.0}, 'error': 'AI vision analysis not available'}
    ...
    def _detect_scene_transitions(self, frame_analyses: list[FrameAnalysisResult]) -> list[dict[str, Any]]:
        ...
        for i in range(1, len(frame_analyses)):
            ...
            if current.get('scene_type') != previous.get('scene_type'):
                transitions.append({'timestamp': current.get('timestamp', 0.0), 'transition_type': 'scene_change', 'from_scene': previous.get('scene_type', 'unknown'), 'to_scene': current.get('scene_type', 'unknown')})
        return transitions
    ...
    def _generate_overall_analysis(self, frame_analyses: list[FrameAnalysisResult], video_path: str) -> dict[str, Any]:
        ...
        return {'overall_sentiment': overall_sentiment, 'dominant_objects': dominant_objects, 'text_timeline': text_timeline, 'total_scenes': len({frame.get('scene_type', 'unknown') for frame in frame_analyses})}
    ...
    def run(self, video_path: str, tenant: str='default', workspace: str='default', analysis_depth: str='standard') -> StepResult:
