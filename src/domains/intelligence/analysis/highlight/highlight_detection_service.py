...
class HighlightDetectionService:
    ...
    def detect_highlights(self, audio_path: str | None=None, transcript_segments: list[dict[str, Any]] | None=None, chat_data: list[dict[str, Any]] | None=None, model: Literal['fast', 'balanced', 'quality']='balanced', min_highlight_duration: float=5.0, max_highlights: int=20, use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if not audio_path and (not transcript_segments):
                return StepResult.fail('Either audio_path or transcript_segments must be provided', status='bad_request')
            if use_cache:
                cache_result = self._check_cache(audio_path, transcript_segments, chat_data, model)
                if cache_result:
                    logger.info('Highlight detection cache hit')
                    return StepResult.ok(data={'highlights': [h.__dict__ for h in cache_result.highlights], 'total_duration': cache_result.total_duration, 'detection_method': cache_result.detection_method, 'signal_weights': cache_result.signal_weights, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            self._select_model(model)
            detection_result = self._detect_highlights(audio_path, transcript_segments, chat_data, min_highlight_duration)
            if detection_result:
                filtered_highlights = self._filter_and_rank_highlights(detection_result.highlights, max_highlights, min_highlight_duration)
                if use_cache:
                    self._cache_result(audio_path, transcript_segments, chat_data, model, detection_result)
                processing_time = (time.time() - start_time) * 1000
                return StepResult.ok(data={'highlights': [h.__dict__ for h in filtered_highlights], 'total_duration': detection_result.total_duration, 'detection_method': detection_result.detection_method, 'signal_weights': detection_result.signal_weights, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def _detect_highlights(self, audio_path: str | None, transcript_segments: list[dict[str, Any]] | None, chat_data: list[dict[str, Any]] | None, min_duration: float) -> HighlightDetectionResult | None:
        ...
        try:
            ...
            return HighlightDetectionResult(highlights=highlights, total_duration=total_duration, detection_method='multi_signal', signal_weights=self._signal_weights)
        except Exception as e:
            logger.error(f'Highlight detection failed: {e}')
            return None
    ...
    def _analyze_semantic_novelty(self, transcript_segments: list[dict[str, Any]] | None, duration: float) -> list[float]:
    ...
    def _combine_signals(self, audio_scores: list[float], chat_scores: list[float], semantic_scores: list[float], duration: float) -> list[float]:
        ...
        for i in range(num_segments):
            combined_score = audio_scores[i] * self._signal_weights['audio_energy'] + chat_scores[i] * self._signal_weights['chat_spikes'] + semantic_scores[i] * self._signal_weights['semantic_novelty']
    ...
    def _detect_highlight_segments(self, scores: list[float], min_duration: float) -> list[HighlightSegment]:
        ...
        for i, score in enumerate(scores):
            if score > 0.6:
                ...
                if highlight_duration >= min_duration:
                    highlight = HighlightSegment(start_time=current_start, end_time=i * segment_duration, duration=highlight_duration, highlight_score=current_score, confidence=min(current_score, 1.0), audio_energy_score=current_score * self._signal_weights['audio_energy'], chat_spike_score=current_score * self._signal_weights['chat_spikes'], semantic_novelty_score=current_score * self._signal_weights['semantic_novelty'])
        ...
        if current_start is not None:
            ...
            if highlight_duration >= min_duration:
                highlight = HighlightSegment(start_time=current_start, end_time=len(scores) * segment_duration, duration=highlight_duration, highlight_score=current_score, confidence=min(current_score, 1.0), audio_energy_score=current_score * self._signal_weights['audio_energy'], chat_spike_score=current_score * self._signal_weights['chat_spikes'], semantic_novelty_score=current_score * self._signal_weights['semantic_novelty'])
                highlights.append(highlight)
        return highlights

    def _filter_and_rank_highlights(self, highlights: list[HighlightSegment], max_highlights: int, min_duration: float) -> list[HighlightSegment]:
    ...
    def _check_cache(self, audio_path: str | None, transcript_segments: list[dict[str, Any]] | None, chat_data: list[dict[str, Any]] | None, model: str) -> HighlightDetectionResult | None:
    ...
    def _cache_result(self, audio_path: str | None, transcript_segments: list[dict[str, Any]] | None, chat_data: list[dict[str, Any]] | None, model: str, result: HighlightDetectionResult) -> None:
    ...
    def get_cache_stats(self) -> StepResult:
        ...
        """
        try:
            stats = {'total_cached': len(self._detection_cache), 'cache_size_limit': self.cache_size, 'utilization': len(self._detection_cache) / self.cache_size if self.cache_size > 0 else 0.0, 'models_cached': {}}
