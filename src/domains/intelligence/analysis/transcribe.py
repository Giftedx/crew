...
def run_whisper(path: str, model: str='tiny') -> Transcript:
    ...
    if getattr(cfg, 'enable_faster_whisper', False):
        try:
            ...
            return Transcript(segments=[Segment(start=float(s.start), end=float(s.end), text=str(s.text).strip()) for s in segs])
        except Exception:
            record_degradation(component='transcribe', event_type='faster_whisper_fallback', severity='warn', detail='faster-whisper path failed; falling back to whisper/text')
    try:
        import whisper
        ...
        record_degradation(component='transcribe', event_type='whisper_fallback_text', severity='warn', detail='whisper unavailable; treating path as plaintext transcript')
