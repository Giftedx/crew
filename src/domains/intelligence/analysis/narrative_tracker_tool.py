...
class NarrativeTrackerInput(BaseModel):
    ...
    time_range: dict = Field(default={'start': '2024-01-01', 'end': '2024-12-31'}, description='Time range for narrative search')
...
class NarrativeTrackerTool(BaseTool):
    ...
    def _run(self, narrative_query: str, creators: list[str], time_range: dict, include_cross_references: bool=True, max_events: int=50, tenant: str='default', workspace: str='default') -> StepResult:
        """
        ...
        try:
            ...
            events = self._query_narrative_events(narrative_query=narrative_query, creators=creators, time_range=time_range, tenant=tenant, workspace=workspace)
            if not events:
                return StepResult.ok(data={'narrative_id': f'narrative_{narrative_query.replace(' ', '_').lower()}', 'timeline': [], 'statistics': {'total_events': 0, 'platforms': [], 'date_range': {'first': None, 'last': None}, 'creators': creators}, 'contradictions': [], 'key_insights': ['No events found for this narrative query']})
            ...
            narrative_timeline = NarrativeTimeline(narrative_id=f'narrative_{narrative_query.replace(' ', '_').lower()}', narrative_query=narrative_query, timeline=timeline, statistics=statistics, contradictions=contradictions, key_insights=insights)
    ...
    def _query_narrative_events(self, narrative_query: str, creators: list[str], time_range: dict, tenant: str, workspace: str) -> list[dict[str, Any]]:
        """Query the knowledge graph for events related to the narrative."""
        try:
            mock_events = [{'event_id': 'evt_001', 'timestamp': datetime(2024, 2, 15, 10, 30, 0), 'platform': 'youtube', 'content_id': 'video_xyz_001', 'creator': 'h3podcast', 'episode_title': 'H3 Podcast #123 - Triller Lawsuit Update', 'summary': 'Ethan discusses the latest developments in the Triller lawsuit', 'key_claims': ['claim_triller_001', 'claim_triller_002'], 'quotes': [{'text': "We will win this lawsuit, I'm confident about that", 'speaker': 'Ethan', 'timestamp': 150.3, 'confidence': 0.95}], 'topics': ['legal', 'lawsuit', 'triller'], 'url': 'https://youtube.com/watch?v=xyz001'}, {'event_id': 'evt_002', 'timestamp': datetime(2024, 2, 16, 14, 20, 0), 'platform': 'twitch', 'content_id': 'vod_abc_002', 'creator': 'hasanabi', 'episode_title': 'Reacting to H3 Triller Lawsuit Discussion', 'summary': "Hasan reacts to Ethan's comments about the Triller lawsuit", 'key_claims': ['claim_triller_003'], 'quotes': [{'text': 'I think Ethan is being too optimistic about this case', 'speaker': 'Hasan', 'timestamp': 245.7, 'confidence': 0.88}], 'topics': ['legal', 'lawsuit', 'triller', 'reaction'], 'url': 'https://twitch.tv/videos/abc002'}, {'event_id': 'evt_003', 'timestamp': datetime(2024, 2, 20, 9, 15, 0), 'platform': 'youtube', 'content_id': 'video_xyz_003', 'creator': 'h3podcast', 'episode_title': 'H3 Podcast #124 - Triller Response', 'summary': 'Ethan responds to criticism about his lawsuit confidence', 'key_claims': ['claim_triller_004'], 'quotes': [{'text': 'I stand by what I said, but I understand the concerns', 'speaker': 'Ethan', 'timestamp': 89.2, 'confidence': 0.92}], 'topics': ['legal', 'lawsuit', 'triller', 'response'], 'url': 'https://youtube.com/watch?v=xyz003'}]
            ...
            filtered_events = [event for event in mock_events if start_date <= event['timestamp'] <= end_date and event['creator'] in creators]
    ...
    def _build_timeline(self, events: list[dict[str, Any]], max_events: int) -> list[NarrativeEvent]:
        ...
        for event_data in sorted_events:
            event = NarrativeEvent(event_id=event_data['event_id'], timestamp=event_data['timestamp'], platform=event_data['platform'], content_id=event_data['content_id'], creator=event_data['creator'], summary=event_data['summary'], key_claims=event_data['key_claims'], quotes=event_data['quotes'], contradictions=[], clarifications=[], provenance={'url': event_data['url'], 'episode_title': event_data['episode_title'], 'topics': event_data['topics']}, confidence_score=0.85)
        ...
        for i, event1 in enumerate(timeline):
            for _j, event2 in enumerate(timeline[i + 1:], i + 1):
                if self._claims_contradict(event1.key_claims, event2.key_claims):
                    contradiction = {'event1_id': event1.event_id, 'event2_id': event2.event_id, 'event1_timestamp': event1.timestamp.isoformat(), 'event2_timestamp': event2.timestamp.isoformat(), 'event1_creator': event1.creator, 'event2_creator': event2.creator, 'description': f'Contradictory claims between {event1.creator} and {event2.creator}', 'confidence': 0.75}
    ...
    def _generate_insights(self, timeline: list[NarrativeEvent], contradictions: list[dict[str, Any]]) -> list[str]:
        ...
        insights.append(f'Narrative spans {span_days} days from {first_event.timestamp.date()} to {last_event.timestamp.date()}')
    ...
    def _calculate_statistics(self, timeline: list[NarrativeEvent], creators: list[str]) -> dict[str, Any]:
        ...
        if not timeline:
            return {'total_events': 0, 'platforms': [], 'date_range': {'first': None, 'last': None}, 'creators': creators}
        platforms = list({event.platform for event in timeline})
        timestamps = [event.timestamp for event in timeline]
        return {'total_events': len(timeline), 'platforms': platforms, 'date_range': {'first': min(timestamps).isoformat(), 'last': max(timestamps).isoformat()}, 'creators': creators, 'events_per_creator': {creator: sum((1 for event in timeline if event.creator == creator)) for creator in creators}, 'events_per_platform': {platform: sum((1 for event in timeline if event.platform == platform)) for platform in platforms}}

    def _serialize_timeline(self, timeline: NarrativeTimeline) -> dict[str, Any]:
        """Serialize the narrative timeline to a dictionary."""
        return {'narrative_id': timeline.narrative_id, 'narrative_query': timeline.narrative_query, 'timeline': [{'event_id': event.event_id, 'timestamp': event.timestamp.isoformat(), 'platform': event.platform, 'content_id': event.content_id, 'creator': event.creator, 'summary': event.summary, 'key_claims': event.key_claims, 'quotes': event.quotes, 'contradictions': event.contradictions, 'clarifications': event.clarifications, 'provenance': event.provenance, 'confidence_score': event.confidence_score} for event in timeline.timeline], 'statistics': timeline.statistics, 'contradictions': timeline.contradictions, 'key_insights': timeline.key_insights}