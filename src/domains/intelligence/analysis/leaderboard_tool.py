...
class LeaderboardTool(BaseTool[StepResult]):
    ...
    def get_top(self, n: int=10) -> list[_LeaderboardRecord]:
        ...
        for person, scores in data.items():
            rec: _LeaderboardRecord = {'person': person, 'lies': int(scores.get('lies', 0)), 'misquotes': int(scores.get('misquotes', 0)), 'misinfo': int(scores.get('misinfo', 0))}
            items.append(rec)
    ...
    def get_person(self, person: str) -> _LeaderboardRecord | None:
        ...
        if record is None:
            return None
        return {'person': person, 'lies': int(record.get('lies', 0)), 'misquotes': int(record.get('misquotes', 0)), 'misinfo': int(record.get('misinfo', 0))}
