...
from ..tools.platform_resolver import resolve_podcast_query, resolve_social_handle, resolve_twitch_login, resolve_youtube_handle
...
class DebateCommandTool(BaseTool[StepResult]):
    ...
    def __init__(self, pipeline: DebateAnalysisPipeline | None=None, ethan_defender: Callable[[str], str] | None=None, hasan_defender: Callable[[str], str] | None=None, profile_store: ProfileStore | None=None) -> None:
        super().__init__()
        if pipeline is None:
    ...
    def _run(self, command: str, **kwargs: Any) -> StepResult:
        ...
        def analyze() -> _DebateCommandResult:
            analysis = self.pipeline.analyze(kwargs['url'], kwargs.get('ts', 0.0), kwargs.get('clip_text', ''), kwargs.get('person', 'unknown'), transcript=kwargs.get('transcript'))
        ...
        def claim() -> StepResult:
            ...
            if person:
                ...
                self.profile_tool.record_event(person, {'video_id': 'claim', 'ts': 0, 'clip': kwargs['claim'], 'fact_verdict': verdict, 'context_verdict': 'n/a', 'evidence': result.get('evidence', [])})
        ...
        def compare() -> StepResult:
            a_key = kwargs['a']
            b_key = kwargs['b']
            return StepResult.ok(data={'compare': {a_key: self.profile_store.get_collaborators(a_key), b_key: self.profile_store.get_collaborators(b_key)}})
        ...
        def verify_profiles() -> StepResult:
            ...
            for seed in seeds:
                ...
                profile_obj = CreatorProfile(name=seed.name, type=seed.type, roles=seed.roles, shows=seed.shows, content_tags=seed.content_tags, platforms=platforms)
                self.profile_store.upsert_profile(profile_obj)
                verified.append(seed.name)
            return StepResult.ok(data={'verified': verified})
        registry: dict[str, Callable[[], Any]] = {'analyze': analyze, 'context': context, 'claim': claim, 'leaderboard': leaderboard, 'timeline': timeline, 'profile': profile, 'creator': profile, 'latest': latest, 'collabs': collabs, 'trends': trends, 'compare': compare, 'verify_profiles': verify_profiles}
