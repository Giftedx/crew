...
class ClaimQuoteExtractionService:
    ...
    def extract_claims_and_quotes(self, text: str, speakers: list[dict[str, Any]] | None=None, model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            if use_cache:
                ...
                if cache_result:
                    ...
                    return StepResult.ok(data={'claims': [c.__dict__ for c in cache_result.claims], 'quotes': [q.__dict__ for q in cache_result.quotes], 'total_segments_processed': cache_result.total_segments_processed, 'extraction_confidence': cache_result.extraction_confidence, 'model': cache_result.model, 'cache_hit': True, 'processing_time_ms': (time.time() - start_time) * 1000})
            ...
            if extraction_result:
                ...
                return StepResult.ok(data={'claims': [c.__dict__ for c in extraction_result.claims], 'quotes': [q.__dict__ for q in extraction_result.quotes], 'total_segments_processed': extraction_result.total_segments_processed, 'extraction_confidence': extraction_result.extraction_confidence, 'model': extraction_result.model, 'cache_hit': False, 'processing_time_ms': processing_time})
            else:
    ...
    def extract_from_segments(self, segments: list[dict[str, Any]], model: Literal['fast', 'balanced', 'quality']='balanced', use_cache: bool=True) -> StepResult:
        ...
        try:
            ...
            for segment in segments:
                ...
                if not segment_text:
                    continue
                segment_result = self.extract_claims_and_quotes(text=segment_text, speakers=[{'speaker': speaker, 'start': start_time, 'end': start_time + 30}], model=model, use_cache=False)
            ...
            avg_confidence = (sum((c['confidence'] for c in all_claims)) + sum((q['confidence'] for q in all_quotes))) / total_items if total_items > 0 else 0.0
            return StepResult.ok(data={'claims': all_claims, 'quotes': all_quotes, 'total_segments_processed': len(segments), 'extraction_confidence': avg_confidence, 'model': model})
        except Exception as e:
    ...
    def _extract_claims_and_quotes(self, text: str, speakers: list[dict[str, Any]] | None, model_name: str) -> ClaimQuoteExtractionResult | None:
    ...
    def _extract_with_nlp(self, text: str, speakers: list[dict[str, Any]] | None, model_name: str) -> ClaimQuoteExtractionResult:
        ...
        if self._claim_classifier is None and TRANSFORMERS_AVAILABLE:
            ...
            try:
                self._claim_classifier = pipeline('text-classification', model='microsoft/DialoGPT-medium', return_all_scores=True)
            except Exception:
        ...
        avg_confidence = (sum((c.confidence for c in claims)) + sum((q.confidence for q in quotes))) / total_items if total_items > 0 else 0.0
        return ClaimQuoteExtractionResult(claims=claims, quotes=quotes, total_segments_processed=len(sentences), extraction_confidence=avg_confidence, model=model_name)

    def _extract_with_rules(self, text: str, speakers: list[dict[str, Any]] | None, model_name: str) -> ClaimQuoteExtractionResult:
        ...
        for i, sentence in enumerate(sentences):
            timestamp = self._estimate_timestamp(i, len(sentences), speakers)
            claim_patterns = ['\\b(claim|state|assert|believe|think)\\b.*\\b(that|because|since)\\b', '\\b(according to|research shows|studies indicate)\\b', '\\b(in fact|actually|clearly|obviously)\\b', '\\b(statistics|data|evidence)\\b.*\\b(show|prove|demonstrate)\\b']
            quote_patterns = ['"([^"]*)"', "'([^']*)'", '\\b(said|stated|mentioned|explained)\\b.*["\\\']([^"\\\']*)["\\\']']
            for pattern in claim_patterns:
                if re.search(pattern, sentence, re.IGNORECASE):
                    claim = ExtractedClaim(text=sentence, speaker=self._find_speaker_at_time(timestamp, speakers), timestamp_seconds=timestamp, confidence=0.7, claim_type='statement')
                    claims.append(claim)
                    break
            for pattern in quote_patterns:
                ...
                if match:
                    ...
                    if quote_text and len(quote_text) > 10:
                        quote = ExtractedQuote(text=quote_text, speaker=self._find_speaker_at_time(timestamp, speakers), timestamp_seconds=timestamp, confidence=0.8, quote_type='notable', context=sentence, significance_score=0.6)
                        quotes.append(quote)
                        break
        total_items = len(claims) + len(quotes)
        avg_confidence = (sum((c.confidence for c in claims)) + sum((q.confidence for q in quotes))) / total_items if total_items > 0 else 0.0
    ...
    def _extract_claim(self, sentence: str, timestamp: float | None, model_name: str) -> ExtractedClaim | None:
        ...
        """
        factual_indicators = ['according to', 'research shows', 'studies indicate', 'data shows', 'statistics reveal', 'evidence suggests', 'in fact', 'actually', 'clearly', 'obviously']
        ...
        for pattern in quote_patterns:
            ...
            if match:
                ...
                if len(quote_text) > 10:
                    return ExtractedQuote(text=quote_text, timestamp_seconds=timestamp, confidence=0.8, quote_type='notable', context=sentence, significance_score=0.6)
        return None

    def _estimate_timestamp(self, sentence_index: int, total_sentences: int, speakers: list[dict[str, Any]] | None) -> float | None:
    ...
    def _check_cache(self, text: str, speakers: list[dict[str, Any]] | None, model: str) -> ClaimQuoteExtractionResult | None:
    ...
    def _cache_result(self, text: str, speakers: list[dict[str, Any]] | None, model: str, result: ClaimQuoteExtractionResult) -> None:
    ...
    def get_cache_stats(self) -> StepResult:
        ...
        """
        try:
            stats = {'total_cached': len(self._extraction_cache), 'cache_size_limit': self.cache_size, 'utilization': len(self._extraction_cache) / self.cache_size if self.cache_size > 0 else 0.0, 'models_cached': {}}
