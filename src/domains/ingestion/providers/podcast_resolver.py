...
def resolve_podcast_query(query: str) -> CanonicalFeed:
    ...
    known_podcasts = {'joe rogan': 'https://feeds.redcircle.com/0eccc737-7d67-4fea-b3de-37faf0e5c9a1', 'tim ferriss': 'https://feeds.feedburner.com/thetimferrissshow', 'lex fridman': 'https://lexfridman.com/feed/podcast/', 'ben shapiro': 'https://feeds.dailywire.com/rss/the-ben-shapiro-show', 'jordan peterson': 'https://feeds.feedburner.com/JordanPetersonPodcast', 'sam harris': 'https://feeds.feedburner.com/samharrisorg', 'dan carlin': 'https://feeds.feedburner.com/dancarlin/history', 'hardcore history': 'https://feeds.feedburner.com/dancarlin/history'}
    for show_name, rss_url in known_podcasts.items():
        if show_name in query_lower or query_lower in show_name:
            return CanonicalFeed(rss_url=rss_url, directory_urls=[f'https://podcasts.apple.com/search?term={query.replace(' ', '+')}', f'https://open.spotify.com/search/{query.replace(' ', '%20')}'])
    slug = query.replace(' ', '-').lower()
    search_term = query.replace(' ', '+')
    return CanonicalFeed(rss_url=f'https://feeds.example.com/{slug}.rss', directory_urls=[f'https://podcasts.apple.com/search?term={search_term}', f'https://open.spotify.com/search/{query.replace(' ', '%20')}', f'https://podcastindex.org/search?q={search_term}'])