...
class YtDlpDownloadTool(BaseTool[StepResult]):
    ...
    def _run(self, video_url: str, quality: str='1080p') -> StepResult:
        ...
        def _iter_candidate_commands() -> list[list[str]]:
            ...
            potential_bins = ('.venv/bin/yt-dlp', '.venv/Scripts/yt-dlp.exe', 'venv/bin/yt-dlp', 'venv/Scripts/yt-dlp.exe')
            potential_pythons = ('.venv/bin/python', '.venv/Scripts/python.exe', 'venv/bin/python', 'venv/Scripts/python.exe')
            if repo_root:
        ...
        command = [*base_cmd, '-o', output_template, '-f', format_selector, '--restrict-filenames', '--write-info-json', '--print-json', '--retries', '3', '--no-cache-dir', video_url]
        display_cmd = ['yt-dlp', '-o', output_template, '-f', format_selector, '--restrict-filenames', '--write-info-json', '--print-json', '--retries', '3', '--no-cache-dir', video_url]
        command_str = ' '.join(display_cmd)
        env = os.environ.copy()
        env.update({'CREWAI_DOWNLOADS_DIR': str(DOWNLOADS_DIR), 'CREWAI_YTDLP_ARCHIVE': str(YTDLP_ARCHIVE), 'CREWAI_TEMP_DIR': str(TEMP_DIR)})
        ...
        try:
            ...
            if any(('[download]' in line and 'has already been downloaded' in line for line in output_lines)):
                try:
                    download_line = next((line for line in output_lines if '[download]' in line and 'has already been downloaded' in line))
                    ...
                    if path_match:
                        ...
                        return _success(video_id=video_id, title=title, uploader='', duration='', file_size='', local_path=existing_path, note='File was already downloaded')
            ...
            base_fields = {'video_id': str(info.get('id', 'unknown')), 'title': info.get('title', 'unknown'), 'uploader': info.get('uploader', 'unknown'), 'duration': str(info.get('duration', 'unknown')), 'file_size': str(info.get('filesize_approx', 'unknown'))}
            with contextlib.suppress(Exception):
                base_fields.update({'channel_id': info.get('channel_id') or info.get('uploader_id'), 'channel_url': info.get('channel_url') or info.get('uploader_url'), 'uploader_url': info.get('uploader_url'), 'uploader_id': info.get('uploader_id')})
            if not local_path:
...
def youtube_fetch_metadata(url: str) -> dict[str, Any]:
    ...
    return {'id': info.get('id', ''), 'title': info.get('title', ''), 'channel': info.get('uploader', ''), 'channel_id': info.get('channel_id') or info.get('uploader_id', ''), 'published_at': info.get('upload_date'), 'duration': info.get('duration'), 'url': info.get('webpage_url', url), 'thumbnails': [t.get('url', '') for t in info.get('thumbnails', []) if isinstance(t, dict)], 'subtitles': info.get('subtitles', {})}

def twitch_fetch_metadata(url: str) -> dict[str, Any]:
    info = _extract_info_with_yt_dlp(url, {'skip_download': True})
    return {'id': info.get('id', ''), 'title': info.get('title', ''), 'streamer': info.get('uploader', ''), 'published_at': info.get('upload_date'), 'duration': info.get('duration'), 'url': info.get('webpage_url', url)}
...
def youtube_list_channel_videos(channel_url: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        playlistend = 250
    info = _extract_info_with_yt_dlp(channel_url, {'skip_download': True, 'extract_flat': True, 'playlistend': playlistend})
