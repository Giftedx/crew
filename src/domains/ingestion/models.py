...
def upsert_creator_by_youtube_channel(conn: sqlite3.Connection, *, tenant: str, workspace: str, channel_id: str) -> int:
    ...
    if row:
        cid = int(row[0])
        conn.execute("UPDATE creator_profile SET youtube_id=?, last_checked_at=datetime('now') WHERE id=?", (channel_id, cid))
        conn.commit()
        return cid
    cur2 = conn.execute("INSERT INTO creator_profile (slug, youtube_id, verified, last_checked_at) VALUES (?,?,?,datetime('now'))", (slug, channel_id, 0))
    conn.commit()
    rid = cur2.lastrowid
    return int(rid) if rid is not None else 0

def ensure_watchlist(conn: sqlite3.Connection, *, tenant: str, workspace: str, source_type: str, handle: str, label: str | None=None) -> int:
    """Ensure a watchlist entry exists; returns its id.

    Adds a corresponding ingest_state row if inserting.
    """
    cur = conn.execute('SELECT id FROM watchlist WHERE tenant=? AND workspace=? AND source_type=? AND handle=?', (tenant, workspace, source_type, handle))
    row = cur.fetchone()
    if row:
        return int(row[0])
    now = default_utc_now().isoformat()
    cur2 = conn.execute('INSERT INTO watchlist (tenant, workspace, source_type, handle, label, enabled, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)', (tenant, workspace, source_type, handle, label, 1, now, now))
    wid = int(cur2.lastrowid) if cur2.lastrowid is not None else 0
    conn.execute('INSERT INTO ingest_state (watchlist_id, cursor, last_seen_at, etag, failure_count, backoff_until) VALUES (?,?,?,?,?,?)', (wid, None, None, None, 0, None))
    conn.commit()
    return wid

def record_provenance(conn: sqlite3.Connection, prov: Provenance) -> None:
    conn.execute('INSERT INTO provenance (content_id, source_url, source_type, retrieved_at, license, terms_url, consent_flags, checksum_sha256, creator_id, episode_id) VALUES (?,?,?,?,?,?,?,?,?,?)', (prov.content_id, prov.source_url, prov.source_type, prov.retrieved_at, prov.license, prov.terms_url, prov.consent_flags, prov.checksum_sha256, prov.creator_id, prov.episode_id))
    conn.commit()

def record_usage(conn: sqlite3.Connection, usage: UsageLog) -> None:
    conn.execute('INSERT INTO usage_log (call_id, content_ids, policy_version, decisions, redactions, output_hash, user_cmd, channel_id, ts) VALUES (?,?,?,?,?,?,?,?,?)', (usage.call_id, usage.content_ids, usage.policy_version, usage.decisions, usage.redactions, usage.output_hash, usage.user_cmd, usage.channel_id, usage.ts))
    conn.commit()
...
def get_episodes_by_creator(conn: sqlite3.Connection, creator_id: int, limit: int=50) -> list[Episode]:
    ...
    cursor = conn.execute('SELECT * FROM episode WHERE creator_id = ? ORDER BY published_at DESC LIMIT ?', (creator_id, limit))
...
def get_watchlist_by_tenant_workspace(conn: sqlite3.Connection, tenant: str, workspace: str) -> list[Watchlist]:
    ...
    cursor = conn.execute('SELECT * FROM watchlist WHERE tenant = ? AND workspace = ? AND enabled = 1', (tenant, workspace))
    return [Watchlist(*row) for row in cursor.fetchall()]

def get_pending_ingest_jobs(conn: sqlite3.Connection, tenant: str, workspace: str, limit: int=10) -> list[IngestJobRecord]:
...
def get_ingest_jobs_by_status(conn: sqlite3.Connection, status: str, limit: int=100) -> list[IngestJobRecord]:
    ...
    cursor = conn.execute('SELECT * FROM ingest_job WHERE status = ? ORDER BY scheduled_at DESC LIMIT ?', (status, limit))
    return [IngestJobRecord(*row) for row in cursor.fetchall()]
...
def bulk_insert_provenance(conn: sqlite3.Connection, provenance_records: list[Provenance]) -> None:
    ...
    if not provenance_records:
        return
    with conn:
        conn.executemany('\n            INSERT INTO provenance (\n                content_id, source_url, source_type, retrieved_at, license,\n                terms_url, consent_flags, checksum_sha256, creator_id, episode_id\n            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n            ', [(p.content_id, p.source_url, p.source_type, p.retrieved_at, p.license, p.terms_url, p.consent_flags, p.checksum_sha256, p.creator_id, p.episode_id) for p in provenance_records])
...
def bulk_insert_usage_logs(conn: sqlite3.Connection, usage_records: list[UsageLog]) -> None:
    ...
    if not usage_records:
        return
    with conn:
        conn.executemany('\n            INSERT INTO usage_log (\n                call_id, content_ids, policy_version, decisions, redactions,\n                output_hash, user_cmd, channel_id, ts\n            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)\n            ', [(u.call_id, u.content_ids, u.policy_version, u.decisions, u.redactions, u.output_hash, u.user_cmd, u.channel_id, u.ts) for u in usage_records])