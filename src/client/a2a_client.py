...
class A2AClient:
    ...
    def get_agent_card(self) -> Json:
        if self.config.enable_retry:
            resp = http_utils.retrying_get(self._agent_card, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        else:
            resp = http_utils.resilient_get(self._agent_card, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        self._ensure_ok(resp)
        return resp.json()

    def get_skills(self) -> Json:
        if self.config.enable_retry:
            resp = http_utils.retrying_get(self._skills, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        else:
            resp = http_utils.resilient_get(self._skills, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        self._ensure_ok(resp)
        return resp.json()
    ...
    def call(self, method: str, params: Json | None=None, id_value: str | int=1) -> Json:
        ...
        if self.config.enable_retry:
            resp = http_utils.retrying_post(self._rpc_endpoint, json_payload=payload, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        else:
            resp = http_utils.resilient_post(self._rpc_endpoint, json_payload=payload, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        self._ensure_ok(resp)
        data = resp.json()
    ...
    def call_batch(self, calls: list[tuple[str, Json | None, str | int]]) -> list[Json]:
        ...
        for method, params, id_value in calls:
            ...
            req.append(item)
        if self.config.enable_retry:
            resp = http_utils.retrying_post(self._rpc_endpoint, json_payload=req, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        else:
            resp = http_utils.resilient_post(self._rpc_endpoint, json_payload=req, headers=self._headers(), timeout_seconds=self.config.timeout_seconds)
        self._ensure_ok(resp)
        out = resp.json()
