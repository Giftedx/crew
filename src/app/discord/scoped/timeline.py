...
class TimelineManager:
    ...
    def add_timeline_event(self, subject: str, event: dict[str, Any]) -> None:
        ...
        event_entry = {'timestamp': default_utc_now().isoformat(), 'type': event.get('type', 'general'), 'title': event.get('title', ''), 'description': event.get('description', ''), 'source_url': event.get('source_url'), 'confidence': event.get('confidence', 0.5), 'evidence_refs': event.get('evidence_refs', []), 'analytical_framing': event.get('analytical_framing', '')}
    ...
    def get_timeline_summary(self, subject: str, days: int=30) -> str:
        ...
        for event in recent_events[-10:]:
            ...
            framing = f' • {event['analytical_framing'][:100]}...' if event['analytical_framing'] else ''
