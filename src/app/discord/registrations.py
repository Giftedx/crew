...
def _register_events(bot: Any) -> None:
    ...
    async def on_ready():
        ...
        try:
            if os.getenv('ENABLE_INGEST_WORKER', '0') in {'1', 'true', 'True'} and (not getattr(bot, '_ingest_workers_started', False)):
...
def _register_prefix_commands(bot: Any) -> None:
    ...
    async def status_cmd(ctx):
        start = time.time()
        tools_status = {'Pipeline Tool': bool(getattr(bot, 'pipeline_tool', None)), 'Debate Tool': bool(getattr(bot, 'debate_tool', None)), 'Fact Check Tool': bool(getattr(bot, 'fact_check_tool', None)), 'Fallacy Tool': bool(getattr(bot, 'fallacy_tool', None))}
        try:
            embed = discord.Embed(title='🟢 Bot Status', description='Operational overview', color=65280)
            embed.add_field(name='🔧 Core Tools', value='\n'.join([f'• {k}: {('✅' if v else '❌')}' for k, v in tools_status.items()]) or 'No tools', inline=False)
    ...
    async def help_full(ctx):
        try:
            embed = discord.Embed(title='🤖 Ultimate Discord Intelligence Bot', description='Debate analysis and fact-checking system', color=65280)
            embed.add_field(name='Usage', value='Use slash commands from the / menu (if enabled).\nCommon toggles: ENABLE_DISCORD_USER_COMMANDS=1, ENABLE_AUTO_URL_ANALYSIS=1', inline=False)
            embed.timestamp = default_utc_now()
    ...
    async def autointel_cmd(ctx, *, args: str=''):
        ...
        depth = _normalize_depth(_extract_depth(text))
        if not url:
            await ctx.send('❌ Could not parse URL. Usage examples:\n• /autointel url:https://youtu.be/VIDEO depth:Experimental - Cutting-Edge AI\n• !autointel https://youtu.be/VIDEO depth:experimental')
            return
...
async def _execute_autointel(interaction: Any, url: str, depth: str='standard') -> None:
    ...
    try:
        if not url or not url.startswith(('http://', 'https://')):
            with contextlib.suppress(Exception):
                await interaction.followup.send('❌ Invalid URL provided. Please provide a valid HTTP/HTTPS URL.', ephemeral=True)
            return
        ...
        if background_worker and orchestrator:
            ...
            try:
                ...
                await handle_autointel_background(interaction=interaction, orchestrator=orchestrator, background_worker=background_worker, url=url, depth=depth)
                return
        ...
        try:
            ...
            if result.success:
                data = result.data or {}
                summary = str(data.get('briefing') or data.get('summary') or data.get('analysis') or '✅ Mission completed.')
        ...
        import_attempts = [('direct', '..autonomous_orchestrator', 'AutonomousIntelligenceOrchestrator'), ('crew', '..crew', 'UltimateDiscordIntelligenceBotCrew')]
        last_error = None
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                await interaction.followup.send(f'⚠️ Tenant context setup failed: {tenant_error}\nProceeding with basic execution.', ephemeral=True)
            tenant_ctx = None
        if orchestrator_type in ('fallback', 'enhanced', 'direct', 'crew'):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                error_context = {'orchestrator_type': orchestrator_type, 'url': url, 'depth': depth, 'execution_time': execution_time, 'error': str(orchestrator_error), 'error_type': type(orchestrator_error).__name__}
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                await interaction.followup.send(f'⚠️ Tenancy system import failed: {tenancy_error}\nRunning without tenant isolation.', ephemeral=True)
            with_tenant = None
        ...
        try:
            ...
            traceback.print_exc()
            with contextlib.suppress(Exception):
                await interaction.followup.send(f'❌ Autonomous intelligence workflow failed: {orchestrator_error!s}\n\n**Error Details:**\n- URL: {url}\n- Analysis Depth: {depth}\n- Error Type: {type(orchestrator_error).__name__}\n\nThe system encountered an error during autonomous processing. This may be due to:\n• Missing CrewAI dependencies\n• Network connectivity issues\n• Content access restrictions\n• System resource limitations\n\nPlease try again or contact support if the issue persists.', ephemeral=True)
    except Exception as e:
        print(f'❌ Critical command failure: {e}')
        import traceback
        traceback.print_exc()
        with contextlib.suppress(Exception):
            await interaction.followup.send(f'❌ Critical system error: {e!s}\nThe /autointel command encountered an unexpected error. Please try again or contact support.', ephemeral=True)
...
def _register_slash_commands(bot: Any) -> None:
    ...
    try:
        ...
        @bot.tree.command(name='autointel', description='Enhanced autonomous URL intelligence analysis with full AI agentic workflow')
        @app_commands.describe(url='URL to analyze (YouTube, Twitter, TikTok, Instagram, Reddit, etc.)', depth='Analysis depth: standard (fast), deep (comprehensive), comprehensive (all features), experimental (cutting-edge AI)')
        @app_commands.choices(depth=[app_commands.Choice(name='Standard - Fast Analysis', value='standard'), app_commands.Choice(name='Deep - Comprehensive Analysis', value='deep'), app_commands.Choice(name='Comprehensive - All Features', value='comprehensive'), app_commands.Choice(name='Experimental - Cutting-Edge AI', value='experimental')])
        ...
        async def _retrieve_results(interaction, workflow_id: str):
            ...
            if not background_worker:
                with contextlib.suppress(Exception):
                    await interaction.followup.send('❌ **Background Worker Not Available**\n\nThe background processing system is not enabled. Results retrieval is only available when background processing is active.', ephemeral=True)
                return
            try:
                from ..background_autointel_handler import handle_retrieve_results
                await handle_retrieve_results(interaction=interaction, background_worker=background_worker, workflow_id=workflow_id)
            except Exception as e:
                print(f'❌ Result retrieval failed: {e}')
                with contextlib.suppress(Exception):
                    await interaction.followup.send(f'❌ **Retrieval Error**\n\nFailed to retrieve results: {e!s}', ephemeral=True)
