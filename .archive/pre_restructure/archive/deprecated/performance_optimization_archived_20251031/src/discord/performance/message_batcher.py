...
class MessageBatcher:
    ...
    async def _find_or_create_batch(self, guild_key: str, message: BatchedMessage) -> MessageBatch:
        ...
        for batch in batches:
            if not batch.is_full and not batch.is_expired and abs(batch.messages[0].priority - message.priority) <= 2:
                return batch
    ...
    async def _find_or_create_batch(self, guild_key: str, message: BatchedMessage) -> MessageBatch:
        ...
        for batch in batches:
            if (
                ...
                and self._is_content_similar(message.content, batch.messages[0].content)
            ):
                return batch
