...
if TYPE_CHECKING:
    from ai.agent_messaging.redis_bus import AgentMessage, AgentMessageBus
...
class DeliberationRequest:
    ...
    question: str  # The decision/question to deliberate on
    options: list[Any]  # Available choices/options
    context: dict[str, Any] = field(default_factory=dict)  # Additional context for agents
    requesting_agent_id: str | None = None
    min_agents: int = 3  # Minimum agents required to participate
...
class DeliberationCoordinator:
    ...
    async def request_deliberation(
        ...
        *,
        ...
        logger.info(
            f"Starting deliberation {deliberation_id}: '{request.question}' "
            f"(min_agents={request.min_agents}, timeout={request.timeout_seconds}s)"
        )

        try:
            # Broadcast deliberation request to all agents
            ...
            self._metrics.counter(
                "deliberations_requested_total",
                labels={"strategy": request.strategy.value},
            ).inc()
            ...
            while True:
                ...
                if elapsed >= timeout:
                    logger.warning(
                        f"Deliberation {deliberation_id}: timeout after {elapsed:.1f}s "
                        f"(received {current_votes}/{request.min_agents} votes)"
                    )
                    break
            ...
            logger.info(
                f"Deliberation {deliberation_id}: consensus={result.is_consensus}, "
            ...
            self._metrics.counter(
                ...
                labels={
                    ...
                    "consensus": str(result.is_consensus).lower(),
                },
            ).inc()
    ...
    async def submit_vote(
        ...
        logger.debug(
            f"Vote recorded for deliberation {deliberation_id}: {agent_id} voted '{decision}' "
    ...
    async def handle_vote_message(self, message: AgentMessage) -> None:
        ...
        """
        from ai.agent_messaging.redis_bus import MessageType
