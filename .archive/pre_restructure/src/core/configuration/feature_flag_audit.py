...
class FeatureFlagAuditor:
    ...
    def discover_feature_flags(self) -> list[FeatureFlag]:
        ...
        # Scan settings files for feature flags
        settings_files = [
            "src/ultimate_discord_intelligence_bot/settings.py",
        ...
        # Scan for feature flag usage in code
    ...
    def _parse_settings_file(self, file_path: Path) -> list[FeatureFlag]:
        ...
        try:
            ...
            # Find ENABLE_* flags
            enable_pattern = r'ENABLE_(\w+)\s*=\s*str\(_get_setting\("([^"]+)",\s*"([^"]+)"\)\)'
    ...
    def _extract_flag_description(self, content: str, flag_name: str) -> str | None:
        ...
        for i, line in enumerate(lines):
            if flag_name in line:
                # Look for comments before the flag
    ...
    def identify_deprecated_flags(self) -> list[FeatureFlag]:
        ...
        for flag in self.flags:
            ...
            # Check for low usage (potential deprecation candidates)
    ...
    def identify_complex_flags(self) -> list[FeatureFlag]:
        ...
        for flag in self.flags:
            ...
            # High usage indicates complexity
            if flag.usage_count > 10:
                complexity_score += 2

            # Long names indicate complexity
            if len(flag.name) > 30:
                complexity_score += 1

            # Multiple words indicate complexity
            if flag.name.count("_") > 3:
                complexity_score += 1

            # Flags with many usage locations
    ...
    def generate_simplification_recommendations(self) -> list[str]:
        ...
        # Check for overly specific flags
        ...
        # Check for flags that could be grouped
        ...
        # Check for flags with inconsistent naming
    ...
    def _find_groupable_flags(self, flag_names: list[str]) -> list[list[str]]:
        ...
        # Find categories with multiple flags
    ...
    def _find_inconsistent_naming(self, flag_names: list[str]) -> list[str]:
        ...
        patterns = {
            "ENABLE_": [name for name in flag_names if name.startswith("ENABLE_")],
    ...
    def generate_simplified_config(self) -> str:
        ...
        config_lines = [
            ...
            "ENABLE_DISTRIBUTED_SYSTEMS=false",
            "",
    ...
    def get_audit_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_flags": len(self.flags),
            "unused_flags": len([f for f in self.flags if f.usage_count == 0]),
            "deprecated_flags": len([f for f in self.flags if f.is_deprecated]),
            "complex_flags": len([f for f in self.flags if f.usage_count > 10]),
            "categories": list({f.category for f in self.flags if f.category}),
            "most_used_flags": sorted(self.flags, key=lambda f: f.usage_count, reverse=True)[:5],
        }
