...
class CostOptimizer:
    ...
    def __init__(self):
        ...
        self.model_pricing = {
            # OpenAI Models - Updated 2024 pricing
            ...
            # Google Gemini Models - Estimated pricing
            "gemini-pro": {"input": 0.0005 / 1000, "output": 0.0015 / 1000},
            "gemini-pro-vision": {"input": 0.0005 / 1000, "output": 0.0015 / 1000},
            # Meta Llama Models - Estimated pricing for hosted versions
            "llama-2-70b": {"input": 0.0003 / 1000, "output": 0.0003 / 1000},
            "llama-3-70b": {"input": 0.0004 / 1000, "output": 0.0004 / 1000},
        ...
        try:
            ...
            if cost_usd > self.budget_config.per_request_max_usd:
                return StepResult.fail(
                    f"Request cost (${cost_usd:.4f}) exceeds per-request limit (${self.budget_config.per_request_max_usd:.4f})"
                )
            ...
            if daily_spend + cost_usd > self.budget_config.daily_budget_usd:
                return StepResult.fail(
                    f"Request would exceed daily budget (${daily_spend:.2f} + ${cost_usd:.4f} > ${self.budget_config.daily_budget_usd:.2f})"
                )
        ...
        try:
            ...
            for metrics in self.cost_history:
                ...
                if metrics.cache_hit:
                    cache_impact["hits"] += 1
                    # Estimate savings (would need actual cache miss cost)
                    cache_impact["hit_savings"] += metrics.cost_usd * 0.8  # 80% savings estimate
                else:
            ...
            return StepResult.ok(
                data={
                    ...
                    "highest_cost_component": {
                        "component": highest_cost_component[0],
                        "cost_usd": highest_cost_component[1],
                        "percentage": (highest_cost_component[1] / total_cost) * 100,
                    },
                    "most_expensive_model": {
                        "model": most_expensive_model[0],
                        "cost_usd": most_expensive_model[1],
                        "percentage": (most_expensive_model[1] / total_cost) * 100,
                    },
                    "cache_impact": {
                        "hit_rate": cache_hit_rate,
                        "estimated_savings_usd": cache_impact["hit_savings"],
                        "hits": cache_impact["hits"],
                        "misses": cache_impact["misses"],
                    },
                }
            )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "high",
                        "description": f"Cache hit rate is {cache_hit_rate:.1%}, target is 60%+",
                        "potential_savings": "20-40% of total cost",
                        "action": "Implement prompt compression and semantic caching",
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "high",
                        "description": f"{most_expensive['model']} accounts for {most_expensive['percentage']:.1f}% of costs",
                        "potential_savings": f"${most_expensive['cost_usd'] * 0.3:.2f}",
                        "action": f"Consider routing simple tasks to cheaper models instead of {most_expensive['model']}",
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "medium",
                        "description": f"{highest_cost_comp['component']} is the highest cost component",
                        "potential_savings": f"${highest_cost_comp['cost_usd'] * 0.2:.2f}",
                        "action": f"Optimize {highest_cost_comp['component']} operations and caching",
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "high",
                        "description": f"Daily spend (${daily_spend:.2f}) approaching budget limit",
                        "potential_savings": f"${daily_spend * 0.25:.2f}",
                        "action": "Implement request throttling and cost-aware routing",
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "priority": "medium",
                        "description": f"Average cost per request is ${avg_cost:.4f}",
                        "potential_savings": "15-30% of token costs",
                        "action": "Implement prompt compression and template optimization",
                    }
                )
    ...
    def simulate_optimization_impact(self, strategy: str) -> StepResult:
        ...
        try:
            ...
            savings_estimates = {
                "prompt_compression": 0.25,  # 25% reduction
                "cache_utilization": 0.30,  # 30% reduction
                "model_selection": 0.20,  # 20% reduction
                "batch_processing": 0.15,  # 15% reduction
                "streaming_responses": 0.10,  # 10% reduction
            }
            ...
            return StepResult.ok(
                data={
                    ...
                    "roi_estimate": f"{estimated_savings / (total_cost * 0.1) * 100:.1f}%",  # Assume 10% implementation cost
                }
            )
    ...
    def health_check(self) -> StepResult:
        ...
        try:
            return StepResult.ok(
                data={
                    "cost_optimizer_healthy": True,
                    "cost_records": len(self.cost_history),
                    "models_tracked": len(self.model_pricing),
                    "optimization_strategies": len(self.optimization_strategies),
                    "daily_budget": self.budget_config.daily_budget_usd,
                    "daily_spend": self.get_daily_spend(),
                    "budget_utilization": (self.get_daily_spend() / self.budget_config.daily_budget_usd) * 100,
                    "timestamp": time.time(),
                }
            )
