...
class MockVectorSearchTool(BaseTool[StepResult]):
    ...
    def _run(self, query: str, top_k: int = 3) -> StepResult:
        ...
        # Simple keyword matching as vector similarity proxy
        ...
        self._metrics.counter(
            "tool_runs_total",
            labels={"tool": "mock_vector_search", "outcome": "success"},
        ).inc()
        return StepResult.ok(data=data)

    def run(self, query: str, top_k: int = 3) -> StepResult:  # pragma: no cover - thin wrapper
