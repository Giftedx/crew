...
class GraphPlugin:
    ...
    async def store(self, namespace: str, records: Sequence[dict[str, Any]]) -> StepResult:
        ...
        try:
            ...
            # Use suffix as index or default to "graph"
        ...
        try:
            ...
            if not result.success:
                return StepResult.fail(
                    f"Graph search failed: {result.error}",
                    namespace=namespace,
                    step="graph_retrieve",
                )

            # Extract graph summaries from search results
            graphs = result.data.get("graphs", [])

            # For each graph, optionally retrieve full details
            # (current implementation returns graph metadata from search)
            formatted_results = []
            for graph_meta in graphs:
                graph_id = graph_meta.get("graph_id")

                # Option 1: Return just metadata (faster)
                formatted_results.append(
                    {
                        "graph_id": graph_id,
                        "score": graph_meta.get("score", 0.0),
                        "keywords": graph_meta.get("keywords", []),
                        "tags": graph_meta.get("tags", []),
                        "node_count": graph_meta.get("node_count", 0),
                        "edge_count": graph_meta.get("edge_count", 0),
                        "metadata": {
                            "tenant_scoped": graph_meta.get("tenant_scoped", False),
                            "namespace": namespace,
                        },
                    }
                )

                # Option 2: Retrieve full graph (uncomment if needed)
                # full_graph = self._tool.get_graph(graph_id=graph_id, namespace=index)
                # if full_graph.success:
                #     formatted_results.append({
                #         "graph_id": graph_id,
                #         "score": graph_meta.get("score", 0.0),
                #         "nodes": full_graph.data.get("nodes", []),
                #         "edges": full_graph.data.get("edges", []),
                #         "keywords": full_graph.data.get("keywords", []),
                #         "metadata": full_graph.data.get("metadata", {}),
                #     })
