...
class LLMRouterAdapter:
    ...
    def __init__(self, allowlist: list[str] | None = None, policy: RouterPolicy | None = None):
        ...
        self._bandit = ThompsonSamplingBandit()  # ephemeral per adapter
    ...
    def _apply_policy_shortlist(self, arms: list[Arm]) -> list[Arm]:
        ...
        if self.policy == RouterPolicy.COST_AWARE:
            # Prefer cheapest input cost where available
            cheap = sorted(
                arms,
                key=lambda a: float(a.cost_in or 0.0) + float(a.cost_out or 0.0),
            )
            return cheap[: max(1, min(3, len(cheap)))]
        if self.policy == RouterPolicy.LATENCY_AWARE:
            low = sorted(arms, key=lambda a: (a.latency_ms or 10_000))
    ...
    def build_arms(self, required_capabilities: set[str] | None = None) -> list[dict[str, Any]]:
        ...
        # Register arms in the bandit for selection
        ...
        # Emit shortlist size metric (low-cardinality by policy only)
        with contextlib.suppress(Exception):
            get_metrics().histogram(
                "router_shortlist_size",
                float(len(shortlisted)),
                labels={"policy": self.policy.value},
            )
        return [
            {
                "arm_id": a.arm_id,
    ...
    def select(self, required_capabilities: set[str] | None = None) -> StepResult:
        ...
        # Optional forced selection via settings (e.g., env FORCE_ROUTER_SELECTION="provider:model")
        forced_arm_id: str | None = None
        with contextlib.suppress(Exception):
            try:
                from core.secure_config import get_setting  # type: ignore
        ...
        with contextlib.suppress(Exception):
            ...
            # Cost and latency attributes if available
