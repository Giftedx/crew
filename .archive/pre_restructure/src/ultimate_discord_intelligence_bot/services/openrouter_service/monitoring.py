...
class MetricsCollector:
    ...
    def get_current_metrics(self) -> PerformanceMetrics:
        ...
        memory_usage_mb = len(self._latency_samples) * 0.001  # Rough estimate

        # Estimate CPU usage (simplified)
        cpu_usage_percent = min(100, throughput_rps * 0.1)  # Rough estimate
...
class AlertManager:
    ...
    def _generate_alert_message(self, threshold: AlertThreshold, value: float) -> str:
        ...
        """
        return (
            f"{threshold.metric_name} is {threshold.comparison} {threshold.threshold_value} "
            f"(current: {value:.2f}) - {threshold.severity} severity"
        )
