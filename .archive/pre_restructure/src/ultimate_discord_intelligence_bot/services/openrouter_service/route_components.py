...
class CostCalculator:
    ...
    def calculate_costs(
        ...
        # Get effective prices with tenant overrides
    ...
    def enforce_budget_limits(
        ...
        try:
            ...
            # Fallback if request_budget module not available
            def _crt() -> None:
                return None
        ...
        if tracker and not tracker.can_charge(projected_cost, task_type):
            if affordable and affordable != chosen_model:
                # Try affordable fallback
                alt_tokens = self.token_meter.count_tokens("", affordable)  # Use empty prompt for fallback
...
class CacheManager:
    ...
    def _process_semantic_cache_result(
        ...
        if shadow_mode:
            ...
            return None  # Shadow mode but no promotion
        else:
            # Production mode - return cached result
            result = dict(sem_res)
            result["cached"] = True
            result["cache_type"] = "semantic"
            return RouteResult(
...
class MetricsRecorder:
    ...
    def get_labels(self) -> dict[str, str]:
        ...
        try:
            # Try to get labels from metrics, with fallback
    ...
    def record_model_selection(self, task_type: str, model: str, provider_family: str) -> None:
        ...
        try:
            # Use getattr to safely access metrics
...
class OfflineExecutor:
    ...
    def execute(
        ...
        if context.model:  # Ensure model is not None
