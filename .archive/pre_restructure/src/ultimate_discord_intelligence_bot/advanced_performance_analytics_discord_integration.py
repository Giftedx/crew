...
class DiscordNotificationConfig:
    ...
    max_message_length: int = 1900  # Discord limit is 2000, leave some margin
...
class AdvancedPerformanceAnalyticsDiscordIntegration:
    ...
    def __init__(
        ...
        # Discord alert tool instances (initialized in _initialize_discord_tools)
    ...
    def _initialize_discord_tools(self) -> None:
        ...
        try:
            ...
            # Private alert tool for sensitive/internal notifications
            ...
            # Executive alert tool for high-level summaries
    ...
    def _get_alert_tool_for_severity(self, severity: AlertSeverity) -> DiscordPrivateAlertTool | None:
        ...
        """
        # Use severity routing if configured
    ...
    def _format_alert_for_discord(self, alert: PerformanceAlert) -> dict[str, Any]:
        ...
        # Enhance with additional formatting if configured
        if not self.config.include_metrics:
            # Remove metrics section from content
    ...
    async def send_alert_notification(self, alert: PerformanceAlert) -> dict[str, Any]:
        ...
        try:
            ...
            # Send notification using Discord tool
            result = alert_tool._run(
                message=discord_data["content"],
        ...
        try:
            ...
            for severity, severity_alerts in severity_groups.items():
                if len(severity_alerts) == 1:
                    ...
                    # Multiple alerts, create batch notification
...
**Alert Summary:**"""
            ...
            for i, alert in enumerate(alerts[:10], 1):  # Limit to first 10 to avoid length issues
                batch_content += f"\n{i}. **{alert.title}**"
                if alert.metrics:
            ...
            if all_metrics:
                batch_content += "\n\n**Aggregated Metrics:**"
                for metric, values in list(all_metrics.items())[:5]:  # Top 5 metrics
            ...
            if all_recommendations:
                batch_content += "\n\n**Recommendations:**"
                for rec in list(all_recommendations)[:3]:  # Top 3 recommendations
                    batch_content += f"\n• {rec}"
    ...
    async def send_executive_summary(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            # Get analytics summary from the system
    ...
    async def get_notification_statistics(self, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "success_rate": round(
                    status_counts.get("success", 0) / len(recent_notifications) * 100 if recent_notifications else 0,
                    1,
                ),
                "configuration": {
